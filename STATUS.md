...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **143 tracks; primary Arabic governing 9842; reference 614; registry-counted
  10737.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9673 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 143 tracks' existing free-text
  ...
  distribution: **98 Tier 1, 16 Tier 2, 7 Tier 3, 22 Tier 4**. No existing
  track's text, status claims, or per-article data is modified or
  recomputed — 20 tracks with documented per-article confidence variation
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **58 edges** (48
  `repeals_full`, 9 `repeals_partial`, 1 `superseded_by`) plus **2 documented
  ...
  decree-number reuse) and **9 genuinely ambiguous cases** flagged with
  ...
  statute, a confirmed negative finding).
  Output:
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9673 records), preparing the corpus for a
  ...
  **9,817 chunks from 9,673 source records** — only **87 records (0.90%)**
  ...
  its own chunks (validated for all 86, not just a sample), and every
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 143 tracks'
  recorded source authorities/URLs and cross-referencing the verification-tier
  taxonomy. **Exactly 11 tracks flagged `known_source_staleness_risk: true`**
  — `accounting_auditing_law`, `awqaf_law`, `civil_status_law`,
  `engineering_practice_law`, `environmental_law`, `income_tax_law`,
  `nationality_law`, `patent_law`, `press_law`, `saudi_engineers_law`,
  `traffic_law` —
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (224 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 151 deterministic generators
