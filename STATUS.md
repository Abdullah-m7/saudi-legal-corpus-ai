...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **153 tracks; primary Arabic governing 10129; reference 614; registry-counted
  11024.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9960 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 153 tracks' existing free-text
  ...
  distribution: **99 Tier 1, 22 Tier 2, 9 Tier 3, 23 Tier 4**. No existing
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9960 records), preparing the corpus for a
  ...
  **10,116 chunks from 9,960 source records** — only **96 records (0.964%)**
  ...
  its own chunks (validated for all 96, not just a sample), and every
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 153 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (234 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 161 deterministic generators
