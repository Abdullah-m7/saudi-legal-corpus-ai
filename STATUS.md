...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **128 tracks; primary Arabic governing 8908; reference 614; registry-counted
  9803.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (8739 records) as a projection (not added to totals). See
...
- **Retrieval eval pack** — realistic Arabic gold queries over the unified index
  ...
  deterministic results. **Current: top-1 91.5% / top-3 97.5% / top-5 98.6% / MRR@5 0.9450**
  over the 8739-record index with **284 golds** — expanded from 40 (v2: gtp-001..007 +
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (8739 records), preparing the corpus for a
  ...
  **8,877 chunks from 8,739 source records** — only **82 records (0.94%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 128 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (209 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 136 deterministic generators
