...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **152 tracks; primary Arabic governing 10044; reference 614; registry-counted
  10939.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9875 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 152 tracks' existing free-text
  ...
  distribution: **99 Tier 1, 21 Tier 2, 9 Tier 3, 23 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **61 edges** (50
  ...
  decree-number reuse) and **15 genuinely ambiguous cases** flagged with
  ...
  limitation rather than a supersession-graph relationship).
  Output:
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9875 records), preparing the corpus for a
  ...
  **10,028 chunks from 9,875 source records** — only **95 records (0.962%)**
  ...
  its own chunks (validated for all 95, not just a sample), and every
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 152 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (233 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 160 deterministic generators
