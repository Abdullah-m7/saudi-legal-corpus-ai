...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **168 tracks; primary Arabic governing 10680; reference 614; registry-counted
  11575.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (10511 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 168 tracks' existing free-text
  ...
  distribution: **101 Tier 1, 27 Tier 2, 17 Tier 3, 23 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **74 edges** (63
  ...
  decree-number reuse) and **24 genuinely ambiguous cases** flagged with
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (10511 records), preparing the corpus for a
  ...
  **10,840 chunks from 10,511 source records** — only **173 records (1.646%)**
  ...
  its own chunks (validated for all 154, not just a sample), and every
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (249 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 176 deterministic generators
