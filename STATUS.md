...
- **Retrieval eval pack** — 80 realistic Arabic gold queries over the unified index
  ...
  over the 5737-record index with **140 golds** — expanded from 40 (v2: gtp-001..007 +
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (126 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 53 deterministic generators
