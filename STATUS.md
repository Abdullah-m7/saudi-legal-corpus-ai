...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **134 tracks; primary Arabic governing 9434; reference 614; registry-counted
  10329.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9265 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 122 tracks' existing free-text
  ...
  distribution: **89 Tier 1, 10 Tier 2, 5 Tier 3, 18 Tier 4**. No existing
  track's text, status claims, or per-article data is modified or
  recomputed — 13 tracks with documented per-article confidence variation
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **52 edges** (43
  ...
  decree-number reuse) and **5 genuinely ambiguous cases** flagged with
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9265 records), preparing the corpus for a
  ...
  **9,406 chunks from 9,265 source records** — only **84 records (0.91%)**
  ...
  its own chunks (validated for all 84, not just a sample), and every
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 134 tracks'
  recorded source authorities/URLs and cross-referencing the verification-tier
  taxonomy. **Exactly 5 tracks flagged `known_source_staleness_risk: true`**
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (215 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 142 deterministic generators
