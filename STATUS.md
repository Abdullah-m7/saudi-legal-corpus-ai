...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **146 tracks; primary Arabic governing 9910; reference 614; registry-counted
  10805.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9741 records) as a projection (not added to totals). See
...
- **Retrieval eval pack** — realistic Arabic gold queries over the unified index
  ...
  deterministic results. **Current: top-1 91.9% / top-3 97.8% / top-5 98.8% / MRR@5 0.9480**
  over the 9741-record index with **320 golds** — expanded from 40 (v2: gtp-001..007 +
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 146 tracks' existing free-text
  ...
  distribution: **98 Tier 1, 18 Tier 2, 8 Tier 3, 22 Tier 4**. No existing
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9741 records), preparing the corpus for a
  ...
  **9,886 chunks from 9,741 source records** — only **88 records (0.903%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 146 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (227 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 154 deterministic generators
