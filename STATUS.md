...
- **Retrieval eval pack** — 80 realistic Arabic gold queries over the unified index
  ...
  over the 5597-record index with **134 golds** — expanded from 40 (v2: gtp-001..007 +
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (123 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 50 deterministic generators
