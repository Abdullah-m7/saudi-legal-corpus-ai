...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **141 tracks; primary Arabic governing 9702; reference 614; registry-counted
  10597.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9533 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 141 tracks' existing free-text
  ...
  distribution: **98 Tier 1, 14 Tier 2, 7 Tier 3, 22 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **56 edges** (46
  ...
  decree-number reuse) and **8 genuinely ambiguous cases** flagged with
  ...
  clause naming no prior statute, a confirmed negative finding).
  Output:
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9533 records), preparing the corpus for a
  ...
  **9,676 chunks from 9,533 source records** — only **86 records (0.90%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 141 tracks'
  recorded source authorities/URLs and cross-referencing the verification-tier
  taxonomy. **Exactly 10 tracks flagged `known_source_staleness_risk: true`**
  — `accounting_auditing_law`, `awqaf_law`, `engineering_practice_law`,
  `environmental_law`, `income_tax_law`, `nationality_law`, `patent_law`,
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (222 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 149 deterministic generators
