...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **154 tracks; primary Arabic governing 10152; reference 614; registry-counted
  11047.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9983 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 154 tracks' existing free-text
  ...
  distribution: **99 Tier 1, 22 Tier 2, 10 Tier 3, 23 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **62 edges** (51
  ...
  decree-number reuse) and **16 genuinely ambiguous cases** flagged with
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9983 records), preparing the corpus for a
  ...
  **10,140 chunks from 9,983 source records** — only **97 records (0.972%)**
  ...
  its own chunks (validated for all 97, not just a sample), and every
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 154 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (235 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 162 deterministic generators
