...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **135 tracks; primary Arabic governing 9459; reference 614; registry-counted
  10354.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9290 records) as a projection (not added to totals). See
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  decree-number reuse) and **6 genuinely ambiguous cases** flagged with
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9290 records), preparing the corpus for a
  ...
  **9,433 chunks from 9,290 source records** — only **86 records (0.93%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 135 tracks'
  recorded source authorities/URLs and cross-referencing the verification-tier
  taxonomy. **Exactly 6 tracks flagged `known_source_staleness_risk: true`**
  — `traffic_law`, `patent_law`, `income_tax_law`, `environmental_law`,
  `accounting_auditing_law`, `awqaf_law` —
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (216 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 143 deterministic generators
