...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **151 tracks; primary Arabic governing 10034; reference 614; registry-counted
  10929.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9865 records) as a projection (not added to totals). See
...
- **Retrieval eval pack** — realistic Arabic gold queries over the unified index
  ...
  deterministic results. **Current: top-1 92.1% / top-3 97.9% / top-5 98.8% / MRR@5 0.9496**
  over the 9865-record index with **330 golds** — expanded from 40 (v2: gtp-001..007 +
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 151 tracks' existing free-text
  ...
  distribution: **99 Tier 1, 21 Tier 2, 9 Tier 3, 22 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  decree-number reuse) and **14 genuinely ambiguous cases** flagged with
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  ...
  **10,014 chunks from 9,865 source records** — only **92 records (0.933%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 151 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (232 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 159 deterministic generators
