...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **138 tracks; primary Arabic governing 9586; reference 614; registry-counted
  10481.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9417 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 138 tracks' existing free-text
  ...
  distribution: **97 Tier 1, 13 Tier 2, 6 Tier 3, 22 Tier 4**. No existing
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9417 records), preparing the corpus for a
  ...
  **9,560 chunks from 9,417 source records** — only **86 records (0.91%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 138 tracks'
  recorded source authorities/URLs and cross-referencing the verification-tier
  taxonomy. **Exactly 8 tracks flagged `known_source_staleness_risk: true`**
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (219 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 146 deterministic generators
