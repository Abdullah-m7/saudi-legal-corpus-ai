...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **56 tracks; primary Arabic governing 6058; reference 614; registry-counted
  6953.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (5889 records) as a projection (not added to totals). See
...
- **Retrieval eval pack** — 80 realistic Arabic gold queries over the unified index
  ...
  over the 5889-record index with **148 golds** — expanded from 40 (v2: gtp-001..007 +
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (130 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 57 deterministic generators
