...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **127 tracks; primary Arabic governing 8889; reference 614; registry-counted
  9784.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (8720 records) as a projection (not added to totals). See
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **45 edges** (38
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (8720 records), preparing the corpus for a
  ...
  **8,858 chunks from 8,720 source records** — only **82 records (0.94%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 127 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (208 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 135 deterministic generators
