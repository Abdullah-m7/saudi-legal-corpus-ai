...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **159 tracks; primary Arabic governing 10427; reference 614; registry-counted
  11322.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (10258 records) as a projection (not added to totals). See
...
- **Retrieval eval pack** — realistic Arabic gold queries over the unified index
  ...
  deterministic results. **Current: top-1 92.5% / top-3 98.0% / top-5 98.8% / MRR@5 0.9514**
  over the 10258-record index with **346 golds** — expanded from 40 (v2: gtp-001..007 +
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 159 tracks' existing free-text
  ...
  distribution: **99 Tier 1, 23 Tier 2, 14 Tier 3, 23 Tier 4**. No existing
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (10258 records), preparing the corpus for a
  ...
  **10,504 chunks from 10,258 source records** — only **139 records (1.355%)**
  ...
  its own chunks (validated for all 139, not just a sample), and every
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 159 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  ...
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 167 deterministic generators
