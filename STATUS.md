...
- **PDPL implementing-regulation Arabic LLM-ready layer** — 38 enrichment records at
  ...
  generated by `scripts/gen_pdpl_implementing_regulation_arabic_legal_llm.py` over the cleaned
  ...
  `article_text_hash_sha256`, `source_trust`) with honest labels: `record_type =
  cleaned_extracted_arabic_article`, `text_status = EXTRACTED_TEXT_NOT_VERIFIED_OFFICIAL_TEXT`.
  Retrieval metadata is derived deterministically from title/number only — no summary,
  paraphrase, translation, or legal interpretation. JSON Schema
  `schemas/pdpl_implementing_regulation_arabic_legal_llm.schema.json`. Validate:
