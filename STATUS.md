...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 178 tracks' existing free-text
  ...
  distribution: **101 Tier 1, 34 Tier 2, 20 Tier 3, 23 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **77 edges** (66
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (10857 records), preparing the corpus for a
  ...
  **11,192 chunks from 10,857 source records** — only **178 records (1.639%)**
  ...
  its own chunks (validated for all 178, not just a sample), and every
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (259 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 186 deterministic generators
