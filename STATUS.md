...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **136 tracks; primary Arabic governing 9468; reference 614; registry-counted
  10363.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9299 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 136 tracks' existing free-text
  ...
  distribution: **95 Tier 1, 13 Tier 2, 6 Tier 3, 22 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **53 edges** (44
  `repeals_full`, 8 `repeals_partial`, 1 `superseded_by`) plus **2 documented
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9299 records), preparing the corpus for a
  ...
  **9,442 chunks from 9,299 source records** — only **86 records (0.92%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 136 tracks'
  recorded source authorities/URLs and cross-referencing the verification-tier
  taxonomy. **Exactly 7 tracks flagged `known_source_staleness_risk: true`**
  — `traffic_law`, `patent_law`, `income_tax_law`, `environmental_law`,
  `accounting_auditing_law`, `awqaf_law`, `saudi_engineers_law` —
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (217 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 144 deterministic generators
