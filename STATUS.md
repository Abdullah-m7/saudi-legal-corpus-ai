...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **139 tracks; primary Arabic governing 9603; reference 614; registry-counted
  10498.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9434 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 139 tracks' existing free-text
  ...
  distribution: **98 Tier 1, 13 Tier 2, 6 Tier 3, 22 Tier 4**. No existing
  track's text, status claims, or per-article data is modified or
  recomputed — 19 tracks with documented per-article confidence variation
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **55 edges** (45
  ...
  decree-number reuse) and **7 genuinely ambiguous cases** flagged with
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9434 records), preparing the corpus for a
  ...
  **9,577 chunks from 9,434 source records** — only **86 records (0.91%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 139 tracks'
  recorded source authorities/URLs and cross-referencing the verification-tier
  taxonomy. **Exactly 9 tracks flagged `known_source_staleness_risk: true`**
  — `traffic_law`, `patent_law`, `income_tax_law`, `environmental_law`,
  `accounting_auditing_law`, `awqaf_law`, `saudi_engineers_law`, `press_law`,
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (220 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 147 deterministic generators
