...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **158 tracks; primary Arabic governing 10422; reference 614; registry-counted
  11317.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (10253 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 158 tracks' existing free-text
  ...
  distribution: **99 Tier 1, 22 Tier 2, 14 Tier 3, 23 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **70 edges** (59
  ...
  decree-number reuse) and **18 genuinely ambiguous cases** flagged with
  ...
  supersession-graph relationship between two distinct tracks).
  Output:
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (10253 records), preparing the corpus for a
  ...
  **10,498 chunks from 10,253 source records** — only **138 records (1.346%)**
  ...
  its own chunks (validated for all 138, not just a sample), and every
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 158 tracks'
  recorded source authorities/URLs and cross-referencing the verification-tier
  taxonomy. **Exactly 13 tracks flagged `known_source_staleness_risk: true`**
  ...
  `press_law`, `saudi_engineers_law`, `traffic_law` —
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (239 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 166 deterministic generators
