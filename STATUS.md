...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **148 tracks; primary Arabic governing 9932; reference 614; registry-counted
  10827.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9763 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 148 tracks' existing free-text
  ...
  distribution: **98 Tier 1, 20 Tier 2, 8 Tier 3, 22 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  decree-number reuse) and **12 genuinely ambiguous cases** flagged with
  ...
  repeal relationship to the parent statute either).
  Output:
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  ...
  **9,910 chunks from 9,763 source records** — only **90 records (0.922%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 148 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (229 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 156 deterministic generators
