...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **131 tracks; primary Arabic governing 9371; reference 614; registry-counted
  10266.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9202 records) as a projection (not added to totals). See
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **49 edges** (41
  `repeals_full`, 7 `repeals_partial`, 1 `superseded_by`) plus **2 documented
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9202 records), preparing the corpus for a
  ...
  **9,342 chunks from 9,202 source records** — only **83 records (0.90%)**
  ...
  its own chunks (validated for all 83, not just a sample), and every
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 131 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (212 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 139 deterministic generators
