...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **144 tracks; primary Arabic governing 9861; reference 614; registry-counted
  10756.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (9692 records) as a projection (not added to totals). See
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 144 tracks' existing free-text
  ...
  distribution: **98 Tier 1, 16 Tier 2, 8 Tier 3, 22 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  decree-number reuse) and **10 genuinely ambiguous cases** flagged with
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (9692 records), preparing the corpus for a
  ...
  **9,837 chunks from 9,692 source records** — only **88 records (0.91%)**
...
- **Freshness/drift monitor** — a purely additive, two-part tool for
  ...
  network calls, no fabricated timestamps) surveying all 144 tracks'
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (225 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 152 deterministic generators
