...
- **Corpus registry index foundation completed** — a canonical, machine-readable
  ...
  boundaries, and validation targets. **171 tracks; primary Arabic governing 10717; reference 614; registry-counted
  11612.** PDPL and Investment Arabic tracks are **verified against official
  published text** (SDAIA / MISA). The registry also records the unified retrieval
  index (10548 records) as a projection (not added to totals). See
...
- **Retrieval eval pack** — realistic Arabic gold queries over the unified index
  ...
  deterministic results. **Current: top-1 93.0% / top-3 98.1% / top-5 98.9% / MRR@5 0.9546**
  over the 10548-record index with **370 golds** — expanded from 40 (v2: gtp-001..007 +
...
- **Verification-tier taxonomy** — a purely additive, read-only derived
  classification layer over all 171 tracks' existing free-text
  ...
  distribution: **101 Tier 1, 30 Tier 2, 17 Tier 3, 23 Tier 4**. No existing
...
- **Supersession/repeal graph** — a purely additive, read-only derived
  ...
  (never inferred from age or topical similarity alone). **75 edges** (64
  ...
  decree-number reuse) and **26 genuinely ambiguous cases** flagged with
...
- **Embeddings-ready chunking layer** — a purely additive text-segmentation
  layer over the unified index (10548 records), preparing the corpus for a
  ...
  **10,881 chunks from 10,548 source records** — only **176 records (1.669%)**
  ...
  its own chunks (validated for all 176, not just a sample), and every
...
- **`make qa-gate`** — one command, everything must pass: **[1]** every
  `scripts/validate_*.py` in the repository (252 today — discovered from the filesystem, so any new
  validator automatically joins the gate; exclusions require a written reason in the script's
  `EXCLUDED` dict, currently empty); **[2]** generator idempotence — 179 deterministic generators
