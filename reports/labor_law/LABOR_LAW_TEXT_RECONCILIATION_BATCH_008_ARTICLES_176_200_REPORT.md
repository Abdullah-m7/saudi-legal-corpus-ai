...
- لم يتم توليد أي نص قانوني مدمج من النص الأساسي加上نافذة التعديل.
...
لم يتم توليد أي نص قانوني مدمج من النص الأساسيplus نافذة التعديل. جميع المقالات المعدلة تُركت فارغة مع علامة `DO_NOT_INGEST_YET`.
...
make test
```
النتيجة: 2497 اختبار؛ حالات الفشل في حدود المعروف (chinese_remediation فقط). لم يتم إدخال أي فشل جديد. تم استعادة ملفات البيانات الصينية المعدلة أثناء الاختبار باستخدام `git checkout -- data/ reports/chinese_translation_review/`.
