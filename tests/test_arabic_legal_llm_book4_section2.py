"""Arabic Legal LLM-ready layer — Book Four Section 2 (board_and_governance).

5 provision records mapped to [67,68], [71], [72], [75], [77] only. The
legal_rule_summary_ar must exactly match the corresponding Section 2 provision's
...
DATA = os.path.join(LAYER, "book4_section2_ar_legal_llm.json")
...
def test_no_uncovered_articles():
    ...
    assert covered == {67, 68, 71, 72, 75, 77}
...
def test_section1_arabic_llm_unchanged():
