"""Chinese remediation Batch P2-005 tests (P2 expansion batch; 15 articles, Babs 12/13/14).

Expansion track: internal Chinese EXPANDED from the official Arabic governing text (English guidance
only; prior condensed candidate as the baseline) for exactly the 15 authorized P2-005 articles. Each
...
DATA = os.path.join(ROOT, "data", "chinese_remediation_batches", "p2_005",
...
VALIDATOR = os.path.join(ROOT, "scripts", "validate_chinese_remediation_batch_p2_005.py")
...
def _write_tmp(tmp_path, doc):
    p = tmp_path / "p2_005_mutated.json"
...
def test_exact_record_count():
    assert _d()["article_count"] == 15
    assert len(_d()["records"]) == 15
