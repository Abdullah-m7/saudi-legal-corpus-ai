"""Arabic Legal LLM-ready layer — Book Four Section 3 (general_assemblies).

5 provision records mapped to [85,87], [92,93], [99], [101], [102] only. The
legal_rule_summary_ar must exactly match the corresponding Section 3 provision's
arabic_reference_summary. NOT full Book Four Arabic LLM coverage.
"""
...
DATA = os.path.join(LAYER, "book4_section3_ar_legal_llm.json")
...
def test_no_uncovered_articles():
    ...
    assert covered == {85, 87, 92, 93, 99, 101, 102}
...
def test_section1_and_section2_arabic_llm_unchanged():
...
def test_coverage_matrix_80_rows_reclassified_preserved():
