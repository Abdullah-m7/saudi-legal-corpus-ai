"""Chinese remediation Batch P0-003 QA tests (article-by-article review vs Arabic; review only).

QA reviews the 20 P0-003 internal Chinese reference translations against the official Arabic
...
Protected layers untouched. Reads committed artifacts and exercises the validator's rejection paths
(out-of-scope, duplicate, invalid status, missing fields, official claim, human-review-complete,
inconsistent final_status / qa_summary) via temp copies.
"""
...
VALIDATOR = os.path.join(ROOT, "scripts", "validate_chinese_remediation_batch_p0_003_qa.py")
...
def test_qa_is_review_only():
    qa = _qa()
    assert qa["p0_003_chinese_text_modified"] is False
    assert qa["p0_003_data_modified"] is False


def test_source_batch_file_pointer():
    assert _qa()["source_batch_file"] == (
        "data/chinese_remediation_batches/p0_003/"
        "companies_law_m132_1443_zh_internal_remediation_p0_003.json")
...
def test_p0_003_data_unchanged():
    src = _read(SRC)
...
def test_reject_inconsistent_qa_summary(tmp_path):
    doc = copy.deepcopy(_qa())
    doc["qa_summary"]["pass"] = 19  # true count is 20
    assert _run(_write_tmp(tmp_path, doc)).returncode != 0
