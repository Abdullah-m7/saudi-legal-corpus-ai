"""Official English guidance REFERENCE layer — Book Four Section 5 (finance/profits/capital).

Per-article English reference records for the provision-covered Articles 123, 124, 126,
127, 128, 129, 130, 132, 133 (Part 4 — Joint-Stock Company). Articles 134 & 135 exist in
the official English source but are OUT OF SCOPE: the Book Four model-1b source treats
them as cross-reference only. NOT the English Legal LLM-ready layer.
"""
...
DATA = os.path.join(REF, "book4_section5_en_reference.json")
JSONL = os.path.join(REF, "book4_section5_en_reference.jsonl")
...
def test_jsonl_line_count_equals_records():
    ...
    assert len(lines) == len(_records()) == 9
...
def test_articles_134_135_excluded():
    ...
    assert 134 not in nums and 135 not in nums
...
def test_no_duplicates():
    nums = [r["article_number"] for r in _records()]
    assert len(set(nums)) == len(nums) == 9
...
def test_articles_123_124_reserves():
    for n in (123, 124):
        b = _blob(n)
        assert "reserve" in b, n


def test_articles_126_127_capital_increase():
    for n in (126, 127):
        b = _blob(n)
        assert ("capital" in b and ("increase" in b or "increased" in b)), n


def test_articles_128_129_130_preemption():
    for n in (128, 129, 130):
        b = _blob(n)
        assert "preemptive" in b or "pre-emptive" in b or "preemption" in b, n


def test_article_132_company_losses():
    b = _blob(132)
    assert "loss" in b
    assert "60" in b or "180" in b or "half" in b


def test_article_133_capital_decrease():
    b = _blob(133)
    assert "capital" in b and ("decrease" in b or "decreased" in b or "reduction" in b)
...
def test_books_1_3_and_sections_1_4_reference_unchanged():
...
def test_arabic_llm_section5_groups_intact():
    doc = _read(os.path.join(ROOT, "data", "arabic_legal_llm", "book4_section5_ar_legal_llm.json"))
    assert [r["article_numbers"] for r in doc["records"]] == [[123, 124], [126, 127], [128, 129, 130], [132], [133]]
