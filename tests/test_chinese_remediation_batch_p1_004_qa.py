"""Chinese remediation Batch P1-004 QA tests (article-by-article review vs Arabic; Babs 10/12/13/14).

The QA reviews the 16 merged P1-004 internal Chinese RETRANSLATIONS against the official Arabic
...
Review only — no P1-004 data modified. QA covers exactly the 16 P1-004 articles across Babs 10/12/13/14;
each per-article bab matches the P1-004 record and the coverage index; final_status/qa_summary/counts
stay consistent; Chinese stays internal / non-official / non-binding / non-governing; no full
Arabic/English/Chinese text embedded; P1-004 + P1-003 (data + QA) + P1-002 (data + QA) + P1-001 (data +
QA) + all P0 batches/QA and base layers untouched. Reads committed artifacts and exercises the
...
SRC = os.path.join(ROOT, "data", "chinese_remediation_batches", "p1_004",
...
VALIDATOR = os.path.join(ROOT, "scripts", "validate_chinese_remediation_batch_p1_004_qa.py")
...
def _write_tmp(tmp_path, doc):
    p = tmp_path / "p1_004_qa_mutated.json"
...
def test_stage_and_batch():
    q = _q()
    assert q["stage"] == "CHINESE_REMEDIATION_BATCH_P1_004_QA"
    assert q["qa_stage"] == "CHINESE_REMEDIATION_BATCH_P1_004_QA"
    assert q["batch_id"] == "P1-004"
    assert q["source_batch_file"].endswith("p1_004/companies_law_m132_1443_zh_internal_remediation_p1_004.json")


def test_scope_and_babs():
    q = _q()
    assert q["scope_articles"] == ARTS
    assert q["expected_babs"] == [10, 12, 13, 14]
...
def test_expected_bab_distribution():
    ...
    assert {str(k): list(v) for k, v in q["expected_bab_distribution"].items()} == {
        "10": [230, 233], "12": [242, 244, 248, 252, 253, 255, 256],
...
def test_result_pass_all_16():
    q = _q()
    assert q["qa_result"] == "PASS"
    assert q["final_status"] == "QA_PASS"
    assert q["pass_count"] == 16
    assert q["minor_fix_count"] == 0
    assert q["blocked_count"] == 0
    assert q["fail_count"] == 0
    assert q["qa_summary"] == {"article_count": 16, "pass": 16, "minor": 0, "blocked": 0, "fail": 0}
    assert all(r["qa_status"] == "pass" for r in q["per_article_reviews"])


def test_review_only_no_p1_004_modification():
    q = _q()
    assert q["p1_004_chinese_text_modified"] is False
    assert q["p1_004_data_modified"] is False
    assert q["minor_fixes"] == []
    assert len(_read(SRC)["records"]) == 16
...
def test_p1_001_p1_002_p1_003_qa_and_all_p0_qa_unchanged():
...
def test_p1_001_p1_002_p1_003_data_unchanged():
...
def test_reject_wrong_bab(tmp_path):
    doc = copy.deepcopy(_q())
    doc["per_article_reviews"][0]["bab"] = 12  # art 230 is Bab 10
    assert _run(_write_tmp(tmp_path, doc)).returncode != 0
