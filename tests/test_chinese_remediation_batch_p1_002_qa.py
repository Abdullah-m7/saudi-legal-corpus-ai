"""Chinese remediation Batch P1-002 QA tests (article-by-article review vs Arabic; Babs 3/4/5/6).

The QA reviews the 20 merged P1-002 internal Chinese RETRANSLATIONS against the official Arabic
...
Review only — no P1-002 data modified. QA covers exactly the 20 P1-002 articles across Babs 3/4/5/6;
each per-article bab matches the P1-002 record and the coverage index; final_status/qa_summary/counts
stay consistent; Chinese stays internal / non-official / non-binding / non-governing; no full
Arabic/English/Chinese text embedded; P1-002 + P1-001 (data + QA) + all P0 batches/QA and base layers
untouched. Reads committed artifacts and exercises the validator's rejection paths.
"""
...
SRC = os.path.join(ROOT, "data", "chinese_remediation_batches", "p1_002",
...
VALIDATOR = os.path.join(ROOT, "scripts", "validate_chinese_remediation_batch_p1_002_qa.py")
...
def _write_tmp(tmp_path, doc):
    p = tmp_path / "p1_002_qa_mutated.json"
...
def test_stage_and_batch():
    q = _q()
    assert q["stage"] == "CHINESE_REMEDIATION_BATCH_P1_002_QA"
    assert q["qa_stage"] == "CHINESE_REMEDIATION_BATCH_P1_002_QA"
    assert q["batch_id"] == "P1-002"
    assert q["source_batch_file"].endswith("p1_002/companies_law_m132_1443_zh_internal_remediation_p1_002.json")


def test_scope_and_babs():
    q = _q()
    assert q["scope_articles"] == ARTS
    assert q["expected_babs"] == [3, 4, 5, 6]
...
def test_expected_bab_distribution():
    ...
    assert {str(k): list(v) for k, v in q["expected_bab_distribution"].items()} == {
        "3": [54], "4": [71, 72, 77, 90, 99, 101, 102, 108, 117, 132],
...
def test_review_only_no_p1_002_modification():
    q = _q()
    assert q["p1_002_chinese_text_modified"] is False
    assert q["p1_002_data_modified"] is False
...
def test_p1_001_qa_and_all_p0_qa_unchanged():
...
def test_p1_001_data_unchanged():
    p = os.path.join(ROOT, "data", "chinese_remediation_batches", "p1_001",
...
def test_reject_wrong_bab(tmp_path):
    doc = copy.deepcopy(_q())
    doc["per_article_reviews"][0]["bab"] = 4  # art 54 is Bab 3
    assert _run(_write_tmp(tmp_path, doc)).returncode != 0
