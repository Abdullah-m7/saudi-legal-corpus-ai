"""Chinese remediation Batch P2-003 QA tests (P2 expansion QA; review vs Arabic; Babs 7/8/9/10).

Review-only QA of the 20 P2-003 expansions vs the official Arabic (English secondary; prior condensed
candidate as baseline): expansion faithfulness, clause-segment parity, terminology consistency, no
hallucination / omission / over-expansion, source + P2-backlog traceability. No P2-003 data modified.
...
SRC = os.path.join(ROOT, "data", "chinese_remediation_batches", "p2_003",
...
VALIDATOR = os.path.join(ROOT, "scripts", "validate_chinese_remediation_batch_p2_003_qa.py")
...
def _write_tmp(tmp_path, doc):
    p = tmp_path / "p2_003_qa_mutated.json"
...
def test_stage_and_batch():
    q = _q()
    assert q["stage"] == "CHINESE_REMEDIATION_BATCH_P2_003_QA"
    assert q["qa_stage"] == "CHINESE_REMEDIATION_BATCH_P2_003_QA"
    assert q["batch_id"] == "P2-003"
    assert q["source_batch_file"].endswith("p2_003/companies_law_m132_1443_zh_internal_remediation_p2_003.json")
...
def test_review_only_no_modification():
    q = _q()
    assert q["p2_003_chinese_text_modified"] is False
    assert q["p2_003_data_modified"] is False
