"""Chinese remediation Batch P2-003 tests (P2 expansion batch; 20 articles, Babs 7/8/9/10).

Expansion track: internal Chinese EXPANDED from the official Arabic governing text (English guidance
only; prior condensed candidate as the baseline) for exactly the 20 authorized P2-003 articles. Each
...
DATA = os.path.join(ROOT, "data", "chinese_remediation_batches", "p2_003",
...
VALIDATOR = os.path.join(ROOT, "scripts", "validate_chinese_remediation_batch_p2_003.py")
...
def _write_tmp(tmp_path, doc):
    p = tmp_path / "p2_003_mutated.json"
