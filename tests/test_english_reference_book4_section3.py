...
Articles 85, 87, 92, 93, 99, 101, 102 (Part 4 — Joint-Stock Company). Article 100
("Issuing a Decision by Circulation") exists in the official English source but is
OUT OF SCOPE: the reconciled Book Four model-1b source maps circulation to Article
101 only. NOT the English Legal LLM-ready layer.
"""
...
DATA = os.path.join(REF, "book4_section3_en_reference.json")
JSONL = os.path.join(REF, "book4_section3_en_reference.jsonl")
...
# Uncovered Section-3 articles (incl. 100, which is present in the English source
# but out of scope) that must NEVER get an English reference record.
...
def test_no_records_for_103_to_137():
    nums = {r["article_number"] for r in _records()}
    assert not (nums & set(range(103, 138))), nums


def test_no_duplicates():
    nums = [r["article_number"] for r in _records()]
    assert len(set(nums)) == len(nums) == 7
...
def test_article_85_extraordinary_assembly_powers():
    b = _blob(85)
    assert "extraordinary" in b
    assert any(t in b for t in ("power", "amend", "articles of association")), b[:80]


def test_article_87_ordinary_assembly_powers():
    b = _blob(87)
    assert "ordinary" in b
    assert any(t in b for t in ("power", "elect", "appoint", "board")), b[:80]


def test_article_92_ordinary_quorum():
    b = _blob(92)
    assert "quorum" in b or "attended" in b
    assert "ordinary" in b
    assert any(t in b for t in ("quarter", "voting shares", "percent", "%")), b[:80]


def test_article_93_extraordinary_quorum():
    b = _blob(93)
    assert "quorum" in b or "attended" in b
    assert "extraordinary" in b
    assert any(t in b for t in ("half", "voting shares", "percent", "%")), b[:80]


def test_article_99_objection_to_decisions():
    b = _blob(99)
    assert any(t in b for t in ("invalidate", "objection", "petition",
                                "judicial", "decision")), b[:80]


def test_article_101_circulation_quorum():
    b = _blob(101)
    assert "circulation" in b
    assert any(t in b for t in ("quorum", "issuance", "decision", "approved")), b[:80]


def test_article_102_inspection_request():
    b = _blob(102)
    assert "inspection" in b or "inspect" in b
    assert any(t in b for t in ("petition", "judicial", "shareholder", "capital")), b[:80]
...
def test_no_english_llm_directory():
    ...
    assert set(os.path.basename(p) for p in _elf) <= {"book4_section1_en_legal_llm.json", "book4_section2_en_legal_llm.json", "book4_section3_en_legal_llm.json", "book4_section4_en_legal_llm.json"}, _elf


def test_no_english_llm_record_files():
    stray = glob.glob(os.path.join(ROOT, "data", "**", "*_en_legal_llm.json"), recursive=True)
    assert set(os.path.basename(p) for p in stray) <= {"book4_section1_en_legal_llm.json", "book4_section2_en_legal_llm.json", "book4_section3_en_legal_llm.json", "book4_section4_en_legal_llm.json"}, stray
...
def test_books_1_3_and_sections_1_2_reference_unchanged():
...
def test_arabic_llm_section3_groups_intact():
    doc = _read(os.path.join(ROOT, "data", "arabic_legal_llm", "book4_section3_ar_legal_llm.json"))
    assert [r["article_numbers"] for r in doc["records"]] == [[85, 87], [92, 93], [99], [101], [102]]
...
def test_coverage_matrix_80_rows_uncovered_preserved():
    ...
    # 84, 89, 100 are the owner-reconciled reclassified rows; all remain uncovered.
    for n in (84, 89, 100):
