"""Chinese remediation Batch P1-002 tests (P1 retranslation batch; 20 articles, Babs 3/4/5/6).
...
DATA = os.path.join(ROOT, "data", "chinese_remediation_batches", "p1_002",
...
VALIDATOR = os.path.join(ROOT, "scripts", "validate_chinese_remediation_batch_p1_002.py")
...
def _write_tmp(tmp_path, doc):
    p = tmp_path / "p1_002_mutated.json"
...
def test_exact_expected_babs():
    assert _d()["expected_babs"] == [3, 4, 5, 6]
...
def test_reject_wrong_bab(tmp_path):
    doc = copy.deepcopy(_d())
    doc["records"][0]["bab"] = 4  # art 54 is Bab 3
    assert _run(_write_tmp(tmp_path, doc)).returncode != 0
