...
| `abstract_plain.txt` / `policy_significance_plain.txt` | Plain-text forms for pasting into the form — **generated by `build.py`**, never edited by hand. |
...
- [x] **Reframed for *Data & Policy* and expanded to 7,571 words** including
...
- [x] **Policy Significance Statement added (128 words).** Not named in the
      ...
      journal prints them. Abstract trimmed 329 → 307 words, then to **249** when the
      submission form turned out to cap it at 250 — a limit stated nowhere in
      the journal's written instructions. Nothing was dropped but wording.
