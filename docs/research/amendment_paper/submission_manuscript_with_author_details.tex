...
\begin{center}
{\Large\bfseries What Changes, and Where? Legislative Churn Across Saudi Arabian Legislation\par}
\end{center}
\begin{center}
...
most cited. Section~7 sets out why this is suggestive rather
...
asserting it, and the magnitude measure in section~6 is
...
\bigskip
\noindent\textit{[Figure 1 near here --- supplied as a separate file, fig1\_churn.eps]}

\noindent\textbf{Figure 1.} The twelve most changed instruments, split by how they changed. The
...
Figure~1 separates the twelve most changed instruments into
...
\bigskip
\noindent\textit{[Figure 2 near here --- supplied as a separate file, fig2\_citation\_tiers.eps]}

\noindent\textbf{Figure 2.} Mean churn rate by citation tier, over the 189 instruments of at
...
average 13.4 per cent (Figure~2).
...
6 found amendments ranging from a substituted phrase to the
...
\paragraph{The reliance gradient is uncontrolled.} As section~7
...
small share of resolved edges were misresolutions. Section~7's
...
\paragraph{Data availability.} The corpus and the analysis and figure scripts
that reproduce every number and figure reported here are openly available at
...
The companion studies referred to above --- describing the corpus, its
citation network, and its definitional lexicon --- are under review elsewhere.

\paragraph{Declaration of generative AI use.} \emph{Tool:} Claude
(Anthropic), version ANONVERSION. \emph{How it was used:} as a research
assistant throughout --- to write the analysis and figure scripts that produce
every number and figure reported here, to draft and revise the manuscript
text, and to run adversarial checks against its own claims, which caught
several errors before submission. \emph{Reason for use:} the study required
building and auditing a reproducible analysis pipeline over a 13,089-article
corpus, work that is not feasible by hand at that scale for a single
researcher. The author set the research question, directed the analysis, read
and verified the outputs against the underlying records, and is responsible
for the content. Every number and figure is regenerated by the openly
published scripts from the openly published corpus, so that any reader can
reproduce them independently of how they were produced.
...
\paragraph{Biographical note.} Abdullah Almohammedi is an independent
researcher working on legal informatics and the empirical study of legislation.
He built the Saudi Legal Corpus for AI, an openly licensed, article-level,
provenance-annotated corpus of Saudi Arabian legislation covering 290
instruments and 15,689 article records, in which every article is recorded
against an identified official source. His current work uses that corpus to
ask questions about a legal system that are ordinarily argued from examples:
how its instruments cite one another, whether they share a vocabulary, and ---
in this article --- what changes in them over time.
