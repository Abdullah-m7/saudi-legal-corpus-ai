...
\paragraph{Declaration of generative AI use.} Generative AI (Claude,
Anthropic) was used as a research assistant throughout this work: to write the
analysis and figure scripts, to draft and revise the manuscript, and to run
adversarial checks against its own claims. The author set the research
question, directed the analysis, and is responsible for the content. Every
number and figure reported here is regenerated by the openly published scripts
from the openly published corpus, so that any reader can reproduce them
