...
% Artificial Intelligence and Law reviews double-anonymously: the submitted
% manuscript must carry no author identity. \anontrue produces the blinded
% file to upload; \anonfalse restores the identified version for the record
% and for the camera-ready.
\newif\ifanon
\anonfalse

% The companion corpus paper is the author's own, so citing it by name would
% break anonymity. Under \anontrue it is cited as an anonymised entry; the
% identified citation returns with \anonfalse.
\ifanon
  \newcommand{\corpuscite}{anon2026corpus}
\else
  \newcommand{\corpuscite}{almohammedi2026corpus}
\fi
...
records with per-record provenance \citep{\corpuscite} --- and the
...
\citep{\corpuscite} was built specifically to close that gap; the
...
\citep{\corpuscite}. A deterministic extractor scans every record
...
The corpus analysed here, its citation and supersession graphs, and the
analysis and figure scripts that reproduce every number and figure reported
in this paper are openly available under the MIT licence and archived in a
public repository with a citable DOI. The repository and DOI are withheld
here because they identify the author, and will be supplied to the editors
on request and stated in full in the published version.
