...
construct and analyse the first citation network over Saudi statutory law:
...
citations. We then separate \emph{vertical} citations, in which a regulation
elaborates its own parent statute, from \emph{horizontal} citations that
cross instrument families --- a distinction that materially changes the
results, since statutes with long implementing regulations otherwise appear
central for purely hierarchical reasons. Horizontal citations account for
79.3\% of the network, so Saudi legislation is genuinely cross-referential
rather than merely hierarchical. The horizontal network is sparse (174
instruments, 221 edges) and centred on a small set of hubs led by the
Companies Law (46 citations from 26 distinct instruments) and the Labour Law
(36 from 23), while the Capital Market Law is cited deeply but narrowly (26
citations from 9 instruments) --- a depth/breadth contrast that raw citation
counts conceal. Cross-referencing the citation network against a
hand-classified supersession graph, we identify live instruments that still
cite repealed predecessors, and we map the instruments that Saudi legislation
cites but that no consolidated machine-readable source yet covers. We discuss
what these structures imply for legal informatics, retrieval-augmented legal
AI, and legislative maintenance.
\end{abstract}
...
This paper reports the first such measurement. It uses a recently released
...
We therefore assign every instrument to a family by stripping structural
suffixes from its identifier (\texttt{labor\_law} and
\texttt{labor\_implementing\_regulation} both belong to the \texttt{labor}
family), and classify each citation as \emph{vertical} when source and
target share a family and \emph{horizontal} otherwise.

The distinction is consequential. Two instruments that appear prominent on
raw counts --- the Anti-Narcotics Law (15 citations) and the Traffic Law (10)
--- receive those citations from exactly one source each, namely their own
implementing regulation, and disappear from the horizontal ranking entirely.
Unless stated otherwise, the results below concern the horizontal network.
...
\midrule
Vertical citations (within a family) & 85 \\
Horizontal citations (across families) & 325 \\
Horizontal share & 79.3\% \\
\midrule
Horizontal network: instruments & 174 \\
Horizontal network: edges & 221 \\
...
First, the network is \textbf{predominantly horizontal}: 325 of 410
citations, or 79.3\%, cross instrument families. Saudi legislation is
therefore genuinely cross-referential; the citation structure is not an
artefact of statute-plus-regulation drafting. This is worth stating plainly
because the opposite result was entirely plausible \emph{a priori} for a
codified civil-law system with a strong statute/regulation hierarchy.
...
The \textbf{Companies Law} (46 citations from 26 instruments) and the
\textbf{Labour Law} (36 from 23) are the twin hubs of the system. Their
prominence is intelligible: nearly every economic regulation must define who
counts as a company and what an employment relationship entails, so these
two instruments supply definitional infrastructure to the rest of the
corpus. That the commercial and labour statutes --- rather than, say, the
constitutional instruments --- occupy this position tells us something real
about which parts of Saudi law are load-bearing in day-to-day regulation.

Breadth and depth come apart sharply. The \textbf{Capital Market Law} ranks
third by citation count (26) but draws them from only nine instruments: it
...
receive 82 horizontal citations. And a system that grounds answers in
...
We have constructed and analysed the first citation network over Saudi
...
The network is 79.3\% horizontal, sparse, overwhelmingly asymmetric, and
concentrated in a single component of 175 instruments centred on the
Companies Law and the Labour Law. Separating breadth from depth reveals two
different kinds of importance --- the intensively cited Capital Market Law
against the widely cited Criminal Procedure and Electronic Transactions Laws
--- that a raw citation count merges. Intersecting the citation network with
