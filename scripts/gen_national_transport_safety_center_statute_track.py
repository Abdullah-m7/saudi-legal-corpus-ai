#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المركز الوطني لسلامة النقل track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27463),
...
SRC = os.path.join(ROOT, "sources", "national_transport_safety_center_statute", "official_source",
                   "national_transport_safety_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "national_transport_safety_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "national_transport_safety_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "national_transport_safety_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "national_transport_safety_center_statute_arabic_legal_llm",
                        "national_transport_safety_center_statute_legal_llm_001_014.json")

LAW_ID = "sa-national-transport-safety-center-statute"
LAW_AR = "تنظيم المركز الوطني لسلامة النقل"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"national_transport_safety_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27463), published 27/3/1447H (2025-09-19). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "national_transport_safety_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "NATIONAL_TRANSPORT_SAFETY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "national-transport-safety-center-statute-llm-%03d" % idx,
            ...
            "article_path": "national_transport_safety_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Center for Transport Safety -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/3/1447H / 2025-09-19)"),
                "source_authority_ar": ("تنظيم المركز الوطني لسلامة النقل — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "national_transport_safety_center_statute",
        "layer": "NATIONAL_TRANSPORT_SAFETY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "national-transport-safety-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Center for Transport Safety — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Center for Transport Safety records" % (len(ver), len(llm)))
