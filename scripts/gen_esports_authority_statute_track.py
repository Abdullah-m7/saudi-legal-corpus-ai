#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية للهيئة السعودية للألعاب والرياضات الإلكترونية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24289),
published 16/6/1445H (2023-12-29). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "esports_authority_statute", "official_source",
                   "esports_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "esports_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "esports_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "esports_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "esports_authority_statute_arabic_legal_llm",
                        "esports_authority_statute_legal_llm_001_016.json")

LAW_ID = "sa-esports-authority-statute"
LAW_AR = "الترتيبات التنظيمية للهيئة السعودية للألعاب والرياضات الإلكترونية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"esports_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24289), published 16/6/1445H (2023-12-29). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "esports_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "ESPORTS_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "esports-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "esports_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Esports and Electronic Games Authority — Organisational Arrangements -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 16/6/1445H / 2023-12-29)"),
                "source_authority_ar": ("الترتيبات التنظيمية للهيئة السعودية للألعاب والرياضات الإلكترونية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 16/6/1445هـ الموافق 2023-12-29م)"),
    ...
    json.dump({
        "law_key": "esports_authority_statute",
        "layer": "ESPORTS_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "esports-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Saudi Esports and Electronic Games Authority — Organisational Arrangements — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Esports and Electronic Games Authority — Organisational Arrangements records" % (len(ver), len(llm)))
