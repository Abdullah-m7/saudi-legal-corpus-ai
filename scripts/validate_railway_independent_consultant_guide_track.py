#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the دليل الخطوط الحديدية - تعيين استشاري مختص مستقل track.
...
RECORDS = os.path.join(ROOT, "sources", "railway_independent_consultant_guide", "verified",
                       "railway_independent_consultant_guide_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "railway_independent_consultant_guide", "verified",
                       "railway_independent_consultant_guide_verified_summary.json")
LLM = os.path.join(ROOT, "data", "railway_independent_consultant_guide_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"railway_independent_consultant_guide_no_decision_number_in_source", "railway_independent_consultant_guide_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "27/11/1444":
        e.append("[2j] gazette_publication_date_hijri must be 27/11/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-06-16":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-06-16")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "دليل الخطوط الحديدية - تعيين استشاري مختص مستقل":
    ...
    if e:
        print("FAIL: %d error(s) in Railway Guide — Appointment of an Independent Competent Consultant track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Railway Guide — Appointment of an Independent Competent Consultant")
