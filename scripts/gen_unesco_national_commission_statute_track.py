#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم اللجنة الوطنية للتربية والثقافة والعلوم track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28806),
published 4/8/1447H (2026-01-23). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "unesco_national_commission_statute", "official_source",
                   "unesco_national_commission_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "unesco_national_commission_statute", "verified")
RECORDS = os.path.join(OUT_VER, "unesco_national_commission_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "unesco_national_commission_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "unesco_national_commission_statute_arabic_legal_llm",
                        "unesco_national_commission_statute_legal_llm_001_013.json")

LAW_ID = "sa-unesco-national-commission-statute"
LAW_AR = "تنظيم اللجنة الوطنية للتربية والثقافة والعلوم"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"unesco_national_commission_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28806), published 4/8/1447H (2026-01-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "unesco_national_commission_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "UNESCO_NATIONAL_COMMISSION_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "unesco-national-commission-statute-llm-%03d" % idx,
            ...
            "article_path": "unesco_national_commission_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Commission for Education, Culture and Science -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/8/1447H / 2026-01-23)"),
                "source_authority_ar": ("تنظيم اللجنة الوطنية للتربية والثقافة والعلوم — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 4/8/1447هـ الموافق 2026-01-23م)"),
    ...
    json.dump({
        "law_key": "unesco_national_commission_statute",
        "layer": "UNESCO_NATIONAL_COMMISSION_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "unesco-national-commission-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Commission for Education, Culture and Science — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Commission for Education, Culture and Science records" % (len(ver), len(llm)))
