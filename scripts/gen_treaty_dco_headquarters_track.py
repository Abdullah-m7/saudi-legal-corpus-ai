#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية مقر بين حكومة المملكة ومنظمة التعاون الرقمي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21240),
published 20/6/1444H (2023-01-13). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "treaty_dco_headquarters", "official_source",
                   "treaty_dco_headquarters_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_dco_headquarters", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_dco_headquarters_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_dco_headquarters_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_dco_headquarters_arabic_legal_llm",
                        "treaty_dco_headquarters_legal_llm_001_021.json")

LAW_ID = "sa-treaty-dco-headquarters"
LAW_AR = "اتفاقية مقر بين حكومة المملكة ومنظمة التعاون الرقمي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_dco_headquarters_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21240), published 20/6/1444H (2023-01-13). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_dco_headquarters",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_DCO_HEADQUARTERS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-dco-headquarters-llm-%03d" % idx,
            ...
            "article_path": "treaty_dco_headquarters/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Headquarters Agreement between Saudi Arabia and the Digital Cooperation Organization -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/6/1444H / 2023-01-13)"),
                "source_authority_ar": ("اتفاقية مقر بين حكومة المملكة ومنظمة التعاون الرقمي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 20/6/1444هـ الموافق 2023-01-13م)"),
    ...
    json.dump({
        "law_key": "treaty_dco_headquarters",
        "layer": "TREATY_DCO_HEADQUARTERS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-dco-headquarters-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (21 مادة، جميعها أصلية)",
        "title_en": "Headquarters Agreement between Saudi Arabia and the Digital Cooperation Organization — Arabic LLM-ready layer (21 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Headquarters Agreement between Saudi Arabia and the Digital Cooperation Organization records" % (len(ver), len(llm)))
