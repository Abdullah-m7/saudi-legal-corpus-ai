#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد عمل اللجنة الدائمة للبت في الاعتراضات المقدمة من الجهات الحكومية على طلبات تخصيص مناطق المجمعات التعدينية وطلبات منح الرخص التعدينية track.

9 records: 9 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "government_allocation_objections_committee_rules", "verified",
                       "government_allocation_objections_committee_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "government_allocation_objections_committee_rules", "verified",
                       "government_allocation_objections_committee_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "government_allocation_objections_committee_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"government_allocation_objections_committee_rules_no_decision_number_in_source", "government_allocation_objections_committee_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    EXPECTED_NUMBERS = [1, 2, 3, 4, 5, 6, 7, 8, 9]
    MISSING_IN_SOURCE = []
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "26/9/1444":
        e.append("[2j] gazette_publication_date_hijri must be 26/9/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-04-17":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-04-17")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد عمل اللجنة الدائمة للبت في الاعتراضات المقدمة من الجهات الحكومية على طلبات تخصيص مناطق المجمعات التعدينية وطلبات منح الرخص التعدينية":
    ...
    if e:
        print("FAIL: %d error(s) in Rules of Procedure of the Standing Committee for Deciding Objections Submitted by Government Entities to Allocation Requests track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules of Procedure of the Standing Committee for Deciding Objections Submitted by Government Entities to Allocation Requests")
    print("  - 9 records: 9 articles, no appendices; all 9 اصلية")
