#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة تصنيف مخالفات كود البناء السعودي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18456),
published 9/3/1443H (2021-10-15). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "building_code_violations_classification_regulation", "official_source",
                   "building_code_violations_classification_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "building_code_violations_classification_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "building_code_violations_classification_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "building_code_violations_classification_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "building_code_violations_classification_regulation_arabic_legal_llm",
                        "building_code_violations_classification_regulation_legal_llm_001_019.json")

LAW_ID = "sa-building-code-violations-classification-regulation"
LAW_AR = "لائحة تصنيف مخالفات كود البناء السعودي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"building_code_violations_classification_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18456), published 9/3/1443H (2021-10-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "building_code_violations_classification_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "BUILDING_CODE_VIOLATIONS_CLASSIFICATION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "building-code-violations-classification-regulation-llm-%03d" % idx,
            ...
            "article_path": "building_code_violations_classification_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Classifying Saudi Building Code Violations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/3/1443H / 2021-10-15)"),
                "source_authority_ar": ("لائحة تصنيف مخالفات كود البناء السعودي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 9/3/1443هـ الموافق 2021-10-15م)"),
    ...
    json.dump({
        "law_key": "building_code_violations_classification_regulation",
        "layer": "BUILDING_CODE_VIOLATIONS_CLASSIFICATION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "building-code-violations-classification-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (19 مادة، جميعها أصلية)",
        "title_en": "Regulation Classifying Saudi Building Code Violations — Arabic LLM-ready layer (19 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Classifying Saudi Building Code Violations records" % (len(ver), len(llm)))
