#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مدينة الملك عبدالله للطاقة الذرية والمتجددة track.
...
RECORDS = os.path.join(ROOT, "sources", "kacare_statute", "verified",
                       "kacare_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "kacare_statute", "verified",
                       "kacare_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "kacare_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"kacare_statute_no_decision_number_in_source", "kacare_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم مدينة الملك عبدالله للطاقة الذرية والمتجددة":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the King Abdullah City for Atomic and Renewable Energy (K.A.CARE) track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the King Abdullah City for Atomic and Renewable Energy (K.A.CARE)")
