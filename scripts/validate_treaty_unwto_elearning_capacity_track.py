#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية تعاون بين وزارة السياحة في المملكة ومنظمة السياحة العالمية لتنمية القدرات البشرية عن طريق التعليم الإلكتروني track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_unwto_elearning_capacity", "verified",
                       "treaty_unwto_elearning_capacity_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_unwto_elearning_capacity", "verified",
                       "treaty_unwto_elearning_capacity_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_unwto_elearning_capacity_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_unwto_elearning_capacity_no_decision_number_in_source", "treaty_unwto_elearning_capacity_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "3/7/1443":
        e.append("[2j] gazette_publication_date_hijri must be 3/7/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-02-04":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-02-04")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية تعاون بين وزارة السياحة في المملكة ومنظمة السياحة العالمية لتنمية القدرات البشرية عن طريق التعليم الإلكتروني":
    ...
    if e:
        print("FAIL: %d error(s) in Cooperation Agreement between the Ministry of Tourism and UNWTO on E-Learning Capacity Development track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Cooperation Agreement between the Ministry of Tourism and UNWTO on E-Learning Capacity Development")
