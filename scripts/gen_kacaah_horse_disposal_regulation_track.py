#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية للتصرف بالخيل المملوكة لمركز الملك عبدالعزيز للخيل العربية الأصيلة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28752),
...
SRC = os.path.join(ROOT, "sources", "kacaah_horse_disposal_regulation", "official_source",
                   "kacaah_horse_disposal_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "kacaah_horse_disposal_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "kacaah_horse_disposal_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "kacaah_horse_disposal_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "kacaah_horse_disposal_regulation_arabic_legal_llm",
                        "kacaah_horse_disposal_regulation_legal_llm_001_012.json")

LAW_ID = "sa-kacaah-horse-disposal-regulation"
LAW_AR = "اللائحة التنظيمية للتصرف بالخيل المملوكة لمركز الملك عبدالعزيز للخيل العربية الأصيلة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"kacaah_horse_disposal_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28752), published 13/7/1447H (2026-01-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "kacaah_horse_disposal_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "KACAAH_HORSE_DISPOSAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "kacaah-horse-disposal-regulation-llm-%03d" % idx,
            ...
            "article_path": "kacaah_horse_disposal_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for the Disposal of Horses Owned by the King Abdulaziz Arabian Horse Center -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/7/1447H / 2026-01-02)"),
                "source_authority_ar": ("اللائحة التنظيمية للتصرف بالخيل المملوكة لمركز الملك عبدالعزيز للخيل العربية الأصيلة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "kacaah_horse_disposal_regulation",
        "layer": "KACAAH_HORSE_DISPOSAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "kacaah-horse-disposal-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for the Disposal of Horses Owned by the King Abdulaziz Arabian Horse Center — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for the Disposal of Horses Owned by the King Abdulaziz Arabian Horse Center records" % (len(ver), len(llm)))
