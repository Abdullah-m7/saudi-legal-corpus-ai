#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام معالجة المنشآت المالية المهمة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17281),
published 3/5/1442H (2020-12-18). The Gazette IS the official publication of record
...
STRUCTURE: 38 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "financial_institutions_resolution_law", "official_source",
                   "financial_institutions_resolution_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "financial_institutions_resolution_law", "verified")
RECORDS = os.path.join(OUT_VER, "financial_institutions_resolution_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "financial_institutions_resolution_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "financial_institutions_resolution_law_arabic_legal_llm",
                        "financial_institutions_resolution_law_legal_llm_001_038.json")

LAW_ID = "sa-financial-institutions-resolution-law"
LAW_AR = "نظام معالجة المنشآت المالية المهمة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"financial_institutions_resolution_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17281), published 3/5/1442H (2020-12-18). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "financial_institutions_resolution_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "FINANCIAL_INSTITUTIONS_RESOLUTION_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "financial-institutions-resolution-law-llm-%03d" % idx,
            ...
            "article_path": "financial_institutions_resolution_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law on the Resolution of Systemically Important Financial Institutions -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/5/1442H / 2020-12-18)"),
                "source_authority_ar": ("نظام معالجة المنشآت المالية المهمة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 3/5/1442هـ الموافق 2020-12-18م)"),
    ...
    json.dump({
        "law_key": "financial_institutions_resolution_law",
        "layer": "FINANCIAL_INSTITUTIONS_RESOLUTION_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "financial-institutions-resolution-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (38 مادة، جميعها أصلية)",
        "title_en": "Law on the Resolution of Systemically Important Financial Institutions — Arabic LLM-ready layer (38 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law on the Resolution of Systemically Important Financial Institutions records" % (len(ver), len(llm)))
