#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة تنظيم أعمال النظارة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18926),
...
SRC = os.path.join(ROOT, "sources", "nazara_works_regulation", "official_source",
                   "nazara_works_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "nazara_works_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "nazara_works_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "nazara_works_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "nazara_works_regulation_arabic_legal_llm",
                        "nazara_works_regulation_legal_llm_001_032.json")

LAW_ID = "sa-nazara-works-regulation"
LAW_AR = "لائحة تنظيم أعمال النظارة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"nazara_works_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18926), published 3/7/1443H (2022-02-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "nazara_works_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "NAZARA_WORKS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "nazara-works-regulation-llm-%03d" % idx,
            ...
            "article_path": "nazara_works_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Governing Waqf Custodianship (Nazara) Works -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/7/1443H / 2022-02-04)"),
                "source_authority_ar": ("لائحة تنظيم أعمال النظارة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "nazara_works_regulation",
        "layer": "NAZARA_WORKS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "nazara-works-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (32 مادة، جميعها أصلية)",
        "title_en": "Regulation Governing Waqf Custodianship (Nazara) Works — Arabic LLM-ready layer (32 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Governing Waqf Custodianship (Nazara) Works records" % (len(ver), len(llm)))
