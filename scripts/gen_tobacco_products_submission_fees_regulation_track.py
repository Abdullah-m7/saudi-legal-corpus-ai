#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة رسوم تقديم منتجات التبغ track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23467),
...
SRC = os.path.join(ROOT, "sources", "tobacco_products_submission_fees_regulation", "official_source",
                   "tobacco_products_submission_fees_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "tobacco_products_submission_fees_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "tobacco_products_submission_fees_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "tobacco_products_submission_fees_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "tobacco_products_submission_fees_regulation_arabic_legal_llm",
                        "tobacco_products_submission_fees_regulation_legal_llm_001_005.json")

LAW_ID = "sa-tobacco-products-submission-fees-regulation"
LAW_AR = "لائحة رسوم تقديم منتجات التبغ"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"tobacco_products_submission_fees_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23467), published 24/1/1445H (2023-08-11). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "tobacco_products_submission_fees_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "TOBACCO_PRODUCTS_SUBMISSION_FEES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "tobacco-products-submission-fees-regulation-llm-%03d" % idx,
            ...
            "article_path": "tobacco_products_submission_fees_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Fees for the Submission of Tobacco Products -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/1/1445H / 2023-08-11)"),
                "source_authority_ar": ("لائحة رسوم تقديم منتجات التبغ — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "tobacco_products_submission_fees_regulation",
        "layer": "TOBACCO_PRODUCTS_SUBMISSION_FEES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "tobacco-products-submission-fees-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (5 مادة، جميعها أصلية)",
        "title_en": "Regulation on Fees for the Submission of Tobacco Products — Arabic LLM-ready layer (5 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Fees for the Submission of Tobacco Products records" % (len(ver), len(llm)))
