#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة التفتيش القضائي بديوان المظالم track.
...
RECORDS = os.path.join(ROOT, "sources", "bog_judicial_inspection_regulation", "verified",
                       "bog_judicial_inspection_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "bog_judicial_inspection_regulation", "verified",
                       "bog_judicial_inspection_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "bog_judicial_inspection_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"bog_judicial_inspection_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة التفتيش القضائي بديوان المظالم":
    ...
    if e:
        print("FAIL: %d error(s) in Judicial Inspection Regulation of the Board of Grievances track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Judicial Inspection Regulation of the Board of Grievances")
