#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية للجان إصلاح ذات البين في إمارات المناطق track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20079),
published 20/2/1444H (2022-09-16). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "reconciliation_committees_regulation", "official_source",
                   "reconciliation_committees_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "reconciliation_committees_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "reconciliation_committees_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "reconciliation_committees_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "reconciliation_committees_regulation_arabic_legal_llm",
                        "reconciliation_committees_regulation_legal_llm_001_016.json")

LAW_ID = "sa-reconciliation-committees-regulation"
LAW_AR = "اللائحة التنظيمية للجان إصلاح ذات البين في إمارات المناطق"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"reconciliation_committees_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20079), published 20/2/1444H (2022-09-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "reconciliation_committees_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "RECONCILIATION_COMMITTEES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "reconciliation-committees-regulation-llm-%03d" % idx,
            ...
            "article_path": "reconciliation_committees_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for the Reconciliation Committees in the Regional Emirates -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/2/1444H / 2022-09-16)"),
                "source_authority_ar": ("اللائحة التنظيمية للجان إصلاح ذات البين في إمارات المناطق — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 20/2/1444هـ الموافق 2022-09-16م)"),
    ...
    json.dump({
        "law_key": "reconciliation_committees_regulation",
        "layer": "RECONCILIATION_COMMITTEES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "reconciliation-committees-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for the Reconciliation Committees in the Regional Emirates — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for the Reconciliation Committees in the Regional Emirates records" % (len(ver), len(llm)))
