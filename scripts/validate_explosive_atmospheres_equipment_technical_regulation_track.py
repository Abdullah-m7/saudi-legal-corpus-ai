#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الفنية للأجهزة الكهربائية والمعدات المعدة للاستخدام في الأجواء القابلة للانفجار track.
...
RECORDS = os.path.join(ROOT, "sources", "explosive_atmospheres_equipment_technical_regulation", "verified",
                       "explosive_atmospheres_equipment_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "explosive_atmospheres_equipment_technical_regulation", "verified",
                       "explosive_atmospheres_equipment_technical_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "explosive_atmospheres_equipment_technical_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"explosive_atmospheres_equipment_technical_regulation_no_decision_number_in_source", "explosive_atmospheres_equipment_technical_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "3/5/1445":
        e.append("[2j] gazette_publication_date_hijri must be 3/5/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-11-17":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-11-17")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة الفنية للأجهزة الكهربائية والمعدات المعدة للاستخدام في الأجواء القابلة للانفجار":
    ...
    if e:
        print("FAIL: %d error(s) in Technical Regulation for Electrical Apparatus and Equipment Intended for Use in Explosive Atmospheres track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Technical Regulation for Electrical Apparatus and Equipment Intended for Use in Explosive Atmospheres")
