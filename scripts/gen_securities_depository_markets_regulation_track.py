#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة أسواق ومراكز إيداع الأوراق المالية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21491),
...
STRUCTURE: 60 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "securities_depository_markets_regulation", "official_source",
                   "securities_depository_markets_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "securities_depository_markets_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "securities_depository_markets_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "securities_depository_markets_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "securities_depository_markets_regulation_arabic_legal_llm",
                        "securities_depository_markets_regulation_legal_llm_001_060.json")

LAW_ID = "sa-securities-depository-markets-regulation"
LAW_AR = "لائحة أسواق ومراكز إيداع الأوراق المالية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"securities_depository_markets_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21491), published 4/8/1444H (2023-02-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "securities_depository_markets_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SECURITIES_DEPOSITORY_MARKETS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "securities-depository-markets-regulation-llm-%03d" % idx,
            ...
            "article_path": "securities_depository_markets_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation of Securities Exchanges and Depository Centers -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/8/1444H / 2023-02-24)"),
                "source_authority_ar": ("لائحة أسواق ومراكز إيداع الأوراق المالية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "securities_depository_markets_regulation",
        "layer": "SECURITIES_DEPOSITORY_MARKETS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "securities-depository-markets-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (60 مادة، جميعها أصلية)",
        "title_en": "Regulation of Securities Exchanges and Depository Centers — Arabic LLM-ready layer (60 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation of Securities Exchanges and Depository Centers records" % (len(ver), len(llm)))
