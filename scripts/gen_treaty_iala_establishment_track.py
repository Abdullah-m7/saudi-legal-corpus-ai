#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية بشأن إنشاء المنظمة الدولية للمساعدات الملاحية البحرية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20044),
published 13/2/1444H (2022-09-09). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "treaty_iala_establishment", "official_source",
                   "treaty_iala_establishment_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_iala_establishment", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_iala_establishment_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_iala_establishment_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_iala_establishment_arabic_legal_llm",
                        "treaty_iala_establishment_legal_llm_001_022.json")

LAW_ID = "sa-treaty-iala-establishment"
LAW_AR = "اتفاقية بشأن إنشاء المنظمة الدولية للمساعدات الملاحية البحرية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_iala_establishment_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20044), published 13/2/1444H (2022-09-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_iala_establishment",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_IALA_ESTABLISHMENT_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-iala-establishment-llm-%03d" % idx,
            ...
            "article_path": "treaty_iala_establishment/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Convention on the Establishment of the International Organization for Marine Aids to Navigation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/2/1444H / 2022-09-09)"),
                "source_authority_ar": ("اتفاقية بشأن إنشاء المنظمة الدولية للمساعدات الملاحية البحرية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 13/2/1444هـ الموافق 2022-09-09م)"),
    ...
    json.dump({
        "law_key": "treaty_iala_establishment",
        "layer": "TREATY_IALA_ESTABLISHMENT_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-iala-establishment-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (22 مادة، جميعها أصلية)",
        "title_en": "Convention on the Establishment of the International Organization for Marine Aids to Navigation — Arabic LLM-ready layer (22 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Convention on the Establishment of the International Organization for Marine Aids to Navigation records" % (len(ver), len(llm)))
