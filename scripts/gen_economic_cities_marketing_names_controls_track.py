#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط التسميات التسويقية للمدن الاقتصادية والمناطق الاقتصادية الخاصة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4001453),
...
SRC = os.path.join(ROOT, "sources", "economic_cities_marketing_names_controls", "official_source",
                   "economic_cities_marketing_names_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "economic_cities_marketing_names_controls", "verified")
RECORDS = os.path.join(OUT_VER, "economic_cities_marketing_names_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "economic_cities_marketing_names_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "economic_cities_marketing_names_controls_arabic_legal_llm",
                        "economic_cities_marketing_names_controls_legal_llm_001_014.json")

LAW_ID = "sa-economic-cities-marketing-names-controls"
LAW_AR = "ضوابط التسميات التسويقية للمدن الاقتصادية والمناطق الاقتصادية الخاصة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"economic_cities_marketing_names_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4001453), published 10/2/1448H (2026-07-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "economic_cities_marketing_names_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "ECONOMIC_CITIES_MARKETING_NAMES_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "economic-cities-marketing-names-controls-llm-%03d" % idx,
            ...
            "article_path": "economic_cities_marketing_names_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on the Marketing Designations of Economic Cities and Special Economic Zones -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 10/2/1448H / 2026-07-24)"),
                "source_authority_ar": ("ضوابط التسميات التسويقية للمدن الاقتصادية والمناطق الاقتصادية الخاصة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "economic_cities_marketing_names_controls",
        "layer": "ECONOMIC_CITIES_MARKETING_NAMES_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "economic-cities-marketing-names-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Controls on the Marketing Designations of Economic Cities and Special Economic Zones — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on the Marketing Designations of Economic Cities and Special Economic Zones records" % (len(ver), len(llm)))
