#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم الأكاديمية المالية track.
...
RECORDS = os.path.join(ROOT, "sources", "financial_academy_statute", "verified",
                       "financial_academy_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "financial_academy_statute", "verified",
                       "financial_academy_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "financial_academy_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"financial_academy_statute_no_decision_number_in_source", "financial_academy_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "25/8/1444":
        e.append("[2j] gazette_publication_date_hijri must be 25/8/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-03-17":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-03-17")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم الأكاديمية المالية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Financial Academy track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Financial Academy")
