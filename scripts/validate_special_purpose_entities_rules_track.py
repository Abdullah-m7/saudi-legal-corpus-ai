#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة للمنشآت ذات الأغراض الخاصة track.
...
RECORDS = os.path.join(ROOT, "sources", "special_purpose_entities_rules", "verified",
                       "special_purpose_entities_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "special_purpose_entities_rules", "verified",
                       "special_purpose_entities_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "special_purpose_entities_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"special_purpose_entities_rules_no_decision_number_in_source", "special_purpose_entities_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد المنظمة للمنشآت ذات الأغراض الخاصة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing Special Purpose Entities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing Special Purpose Entities")
