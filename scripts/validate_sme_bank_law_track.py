#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام بنك المنشآت الصغيرة والمتوسطة track.
...
RECORDS = os.path.join(ROOT, "sources", "sme_bank_law", "verified",
                       "sme_bank_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "sme_bank_law", "verified",
                       "sme_bank_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "sme_bank_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"sme_bank_law_no_decision_number_in_source", "sme_bank_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام بنك المنشآت الصغيرة والمتوسطة":
    ...
    if e:
        print("FAIL: %d error(s) in Law of the Small and Medium Enterprises Bank track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law of the Small and Medium Enterprises Bank")
