#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم هيئة تنظيم المياه والكهرباء track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17389),
...
SRC = os.path.join(ROOT, "sources", "water_electricity_regulatory_authority_statute", "official_source",
                   "water_electricity_regulatory_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "water_electricity_regulatory_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "water_electricity_regulatory_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "water_electricity_regulatory_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "water_electricity_regulatory_authority_statute_arabic_legal_llm",
                        "water_electricity_regulatory_authority_statute_legal_llm_001_020.json")

LAW_ID = "sa-water-electricity-regulatory-authority-statute"
LAW_AR = "تنظيم هيئة تنظيم المياه والكهرباء"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"water_electricity_regulatory_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17389), published 24/5/1442H (2021-01-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "water_electricity_regulatory_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "WATER_ELECTRICITY_REGULATORY_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "water-electricity-regulatory-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "water_electricity_regulatory_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Water and Electricity Regulatory Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/5/1442H / 2021-01-08)"),
                "source_authority_ar": ("تنظيم هيئة تنظيم المياه والكهرباء — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "water_electricity_regulatory_authority_statute",
        "layer": "WATER_ELECTRICITY_REGULATORY_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "water-electricity-regulatory-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Statute of the Water and Electricity Regulatory Authority — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Water and Electricity Regulatory Authority records" % (len(ver), len(llm)))
