#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام حماية المؤشرات الجغرافية track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4001049),
...
SRC = os.path.join(ROOT, "sources", "geographical_indications_regulation", "official_source",
                   "geographical_indications_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "geographical_indications_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "geographical_indications_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "geographical_indications_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "geographical_indications_regulation_arabic_legal_llm",
                        "geographical_indications_regulation_legal_llm_001_037.json")

LAW_ID = "sa-geographical-indications-regulation"
LAW_AR = "اللائحة التنفيذية لنظام حماية المؤشرات الجغرافية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"geographical_indications_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4001049), published 5/12/1447H (2026-05-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "geographical_indications_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "GEOGRAPHICAL_INDICATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "geographical-indications-regulation-llm-%03d" % idx,
            ...
            "article_path": "geographical_indications_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Geographical Indications Protection Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/12/1447H / 2026-05-22)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام حماية المؤشرات الجغرافية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "geographical_indications_regulation",
        "layer": "GEOGRAPHICAL_INDICATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "geographical-indications-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (37 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Geographical Indications Protection Law — Arabic LLM-ready layer (37 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Geographical Indications Protection Law records" % (len(ver), len(llm)))
