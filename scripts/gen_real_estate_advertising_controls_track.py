#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط الإعلانات العقارية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17734),
published 23/10/1442H (2021-06-04). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "real_estate_advertising_controls", "official_source",
                   "real_estate_advertising_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "real_estate_advertising_controls", "verified")
RECORDS = os.path.join(OUT_VER, "real_estate_advertising_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "real_estate_advertising_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "real_estate_advertising_controls_arabic_legal_llm",
                        "real_estate_advertising_controls_legal_llm_001_009.json")

LAW_ID = "sa-real-estate-advertising-controls"
LAW_AR = "ضوابط الإعلانات العقارية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"real_estate_advertising_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17734), published 23/10/1442H (2021-06-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "real_estate_advertising_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "REAL_ESTATE_ADVERTISING_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "real-estate-advertising-controls-llm-%03d" % idx,
            ...
            "article_path": "real_estate_advertising_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Real Estate Advertising Controls -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/10/1442H / 2021-06-04)"),
                "source_authority_ar": ("ضوابط الإعلانات العقارية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 23/10/1442هـ الموافق 2021-06-04م)"),
    ...
    json.dump({
        "law_key": "real_estate_advertising_controls",
        "layer": "REAL_ESTATE_ADVERTISING_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "real-estate-advertising-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (9 مادة، جميعها أصلية)",
        "title_en": "Real Estate Advertising Controls — Arabic LLM-ready layer (9 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Real Estate Advertising Controls records" % (len(ver), len(llm)))
