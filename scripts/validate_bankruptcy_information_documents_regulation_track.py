#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة المعلومات والوثائق المنصوص عليها في نظام الإفلاس ولائحته التنفيذية track.
...
RECORDS = os.path.join(ROOT, "sources", "bankruptcy_information_documents_regulation", "verified",
                       "bankruptcy_information_documents_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "bankruptcy_information_documents_regulation", "verified",
                       "bankruptcy_information_documents_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "bankruptcy_information_documents_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"bankruptcy_information_documents_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/5/1442":
        e.append("[2j] gazette_publication_date_hijri must be 24/5/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-01-08":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-01-08")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة المعلومات والوثائق المنصوص عليها في نظام الإفلاس ولائحته التنفيذية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on the Information and Documents Prescribed in the Bankruptcy Law and its Implementing Regulation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on the Information and Documents Prescribed in the Bankruptcy Law and its Implementing Regulation")
