#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم هيئة كفاءة الإنفاق والمشروعات الحكومية track.
...
RECORDS = os.path.join(ROOT, "sources", "spending_efficiency_authority_statute", "verified",
                       "spending_efficiency_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "spending_efficiency_authority_statute", "verified",
                       "spending_efficiency_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "spending_efficiency_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"spending_efficiency_authority_statute_no_decision_number_in_source", "spending_efficiency_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم هيئة كفاءة الإنفاق والمشروعات الحكومية":
    ...
    if e:
        print("FAIL: %d error(s) in Spending Efficiency and Government Projects Authority — Statute track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Spending Efficiency and Government Projects Authority — Statute")
