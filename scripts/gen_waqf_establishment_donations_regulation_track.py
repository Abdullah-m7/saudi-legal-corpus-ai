#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة إنشاء الأوقاف أو تمويلها من خلال جمع التبرعات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27532),
published 25/4/1447H (2025-10-17). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "waqf_establishment_donations_regulation", "official_source",
                   "waqf_establishment_donations_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "waqf_establishment_donations_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "waqf_establishment_donations_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "waqf_establishment_donations_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "waqf_establishment_donations_regulation_arabic_legal_llm",
                        "waqf_establishment_donations_regulation_legal_llm_001_012.json")

LAW_ID = "sa-waqf-establishment-donations-regulation"
LAW_AR = "لائحة إنشاء الأوقاف أو تمويلها من خلال جمع التبرعات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"waqf_establishment_donations_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27532), published 25/4/1447H (2025-10-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "waqf_establishment_donations_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "WAQF_ESTABLISHMENT_DONATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "waqf-establishment-donations-regulation-llm-%03d" % idx,
            ...
            "article_path": "waqf_establishment_donations_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Establishing or Funding Endowments through the Collection of Donations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/4/1447H / 2025-10-17)"),
                "source_authority_ar": ("لائحة إنشاء الأوقاف أو تمويلها من خلال جمع التبرعات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 25/4/1447هـ الموافق 2025-10-17م)"),
    ...
    json.dump({
        "law_key": "waqf_establishment_donations_regulation",
        "layer": "WAQF_ESTABLISHMENT_DONATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "waqf-establishment-donations-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Regulation on Establishing or Funding Endowments through the Collection of Donations — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Establishing or Funding Endowments through the Collection of Donations records" % (len(ver), len(llm)))
