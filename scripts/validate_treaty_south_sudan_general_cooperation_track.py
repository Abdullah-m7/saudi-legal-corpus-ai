#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية عامة للتعاون بين حكومتي المملكة وجمهورية جنوب السودان track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_south_sudan_general_cooperation", "verified",
                       "treaty_south_sudan_general_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_south_sudan_general_cooperation", "verified",
                       "treaty_south_sudan_general_cooperation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_south_sudan_general_cooperation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_south_sudan_general_cooperation_no_decision_number_in_source", "treaty_south_sudan_general_cooperation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "26/10/1443":
        e.append("[2j] gazette_publication_date_hijri must be 26/10/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-05-27":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-05-27")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية عامة للتعاون بين حكومتي المملكة وجمهورية جنوب السودان":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–South Sudan General Cooperation Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–South Sudan General Cooperation Agreement")
