#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية تعاون بين وزارة السياحة في المملكة ومنظمة السياحة العالمية لتنمية القدرات البشرية عن طريق التعليم الإلكتروني track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18937),
published 3/7/1443H (2022-02-04). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "treaty_unwto_elearning_capacity", "official_source",
                   "treaty_unwto_elearning_capacity_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_unwto_elearning_capacity", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_unwto_elearning_capacity_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_unwto_elearning_capacity_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_unwto_elearning_capacity_arabic_legal_llm",
                        "treaty_unwto_elearning_capacity_legal_llm_001_011.json")

LAW_ID = "sa-treaty-unwto-elearning-capacity"
LAW_AR = "اتفاقية تعاون بين وزارة السياحة في المملكة ومنظمة السياحة العالمية لتنمية القدرات البشرية عن طريق التعليم الإلكتروني"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_unwto_elearning_capacity_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18937), published 3/7/1443H (2022-02-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_unwto_elearning_capacity",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_UNWTO_ELEARNING_CAPACITY_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-unwto-elearning-capacity-llm-%03d" % idx,
            ...
            "article_path": "treaty_unwto_elearning_capacity/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Cooperation Agreement between the Ministry of Tourism and UNWTO on E-Learning Capacity Development -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/7/1443H / 2022-02-04)"),
                "source_authority_ar": ("اتفاقية تعاون بين وزارة السياحة في المملكة ومنظمة السياحة العالمية لتنمية القدرات البشرية عن طريق التعليم الإلكتروني — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 3/7/1443هـ الموافق 2022-02-04م)"),
    ...
    json.dump({
        "law_key": "treaty_unwto_elearning_capacity",
        "layer": "TREATY_UNWTO_ELEARNING_CAPACITY_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-unwto-elearning-capacity-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Cooperation Agreement between the Ministry of Tourism and UNWTO on E-Learning Capacity Development — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Cooperation Agreement between the Ministry of Tourism and UNWTO on E-Learning Capacity Development records" % (len(ver), len(llm)))
