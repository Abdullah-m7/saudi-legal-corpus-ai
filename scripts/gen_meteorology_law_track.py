#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام الأرصاد track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17600),
...
SRC = os.path.join(ROOT, "sources", "meteorology_law", "official_source",
                   "meteorology_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "meteorology_law", "verified")
RECORDS = os.path.join(OUT_VER, "meteorology_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "meteorology_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "meteorology_law_arabic_legal_llm",
                        "meteorology_law_legal_llm_001_020.json")

LAW_ID = "sa-meteorology-law"
LAW_AR = "نظام الأرصاد"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"meteorology_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17600), published 2/10/1442H (2021-05-14). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "meteorology_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "METEOROLOGY_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "meteorology-law-llm-%03d" % idx,
            ...
            "article_path": "meteorology_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Meteorology Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/10/1442H / 2021-05-14)"),
                "source_authority_ar": ("نظام الأرصاد — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "meteorology_law",
        "layer": "METEOROLOGY_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "meteorology-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Meteorology Law — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Meteorology Law records" % (len(ver), len(llm)))
