#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the النظام الأساس لمستشفى الملك فيصل التخصصي ومركز الأبحاث track.
...
RECORDS = os.path.join(ROOT, "sources", "king_faisal_specialist_hospital_statute", "verified",
                       "king_faisal_specialist_hospital_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "king_faisal_specialist_hospital_statute", "verified",
                       "king_faisal_specialist_hospital_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "king_faisal_specialist_hospital_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"king_faisal_specialist_hospital_statute_no_decision_number_in_source", "king_faisal_specialist_hospital_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "20/5/1443":
        e.append("[2j] gazette_publication_date_hijri must be 20/5/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-12-24":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-12-24")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "النظام الأساس لمستشفى الملك فيصل التخصصي ومركز الأبحاث":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of King Faisal Specialist Hospital and Research Centre track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of King Faisal Specialist Hospital and Research Centre")
