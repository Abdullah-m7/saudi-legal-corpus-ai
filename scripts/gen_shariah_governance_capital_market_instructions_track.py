#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تعليمات الحوكمة الشرعية في مؤسسات السوق المالية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21490),
...
SRC = os.path.join(ROOT, "sources", "shariah_governance_capital_market_instructions", "official_source",
                   "shariah_governance_capital_market_instructions_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "shariah_governance_capital_market_instructions", "verified")
RECORDS = os.path.join(OUT_VER, "shariah_governance_capital_market_instructions_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "shariah_governance_capital_market_instructions_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "shariah_governance_capital_market_instructions_arabic_legal_llm",
                        "shariah_governance_capital_market_instructions_legal_llm_001_016.json")

LAW_ID = "sa-shariah-governance-capital-market-instructions"
LAW_AR = "تعليمات الحوكمة الشرعية في مؤسسات السوق المالية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"shariah_governance_capital_market_instructions_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21490), published 4/8/1444H (2023-02-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "shariah_governance_capital_market_instructions",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SHARIAH_GOVERNANCE_CAPITAL_MARKET_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "shariah-governance-capital-market-instructions-llm-%03d" % idx,
            ...
            "article_path": "shariah_governance_capital_market_instructions/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Shariah Governance Instructions for Capital Market Institutions -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/8/1444H / 2023-02-24)"),
                "source_authority_ar": ("تعليمات الحوكمة الشرعية في مؤسسات السوق المالية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "shariah_governance_capital_market_instructions",
        "layer": "SHARIAH_GOVERNANCE_CAPITAL_MARKET_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "shariah-governance-capital-market-instructions-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Shariah Governance Instructions for Capital Market Institutions — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Shariah Governance Instructions for Capital Market Institutions records" % (len(ver), len(llm)))
