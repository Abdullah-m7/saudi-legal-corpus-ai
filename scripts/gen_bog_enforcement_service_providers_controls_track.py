#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط الاستعانة بمقدمي خدمات التنفيذ أمام ديوان المظالم track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4000230),
published 18/8/1447H (2026-02-06). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "bog_enforcement_service_providers_controls", "official_source",
                   "bog_enforcement_service_providers_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "bog_enforcement_service_providers_controls", "verified")
RECORDS = os.path.join(OUT_VER, "bog_enforcement_service_providers_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "bog_enforcement_service_providers_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "bog_enforcement_service_providers_controls_arabic_legal_llm",
                        "bog_enforcement_service_providers_controls_legal_llm_001_010.json")

LAW_ID = "sa-bog-enforcement-service-providers-controls"
LAW_AR = "ضوابط الاستعانة بمقدمي خدمات التنفيذ أمام ديوان المظالم"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"bog_enforcement_service_providers_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4000230), published 18/8/1447H (2026-02-06). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "bog_enforcement_service_providers_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "BOG_ENFORCEMENT_SERVICE_PROVIDERS_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "bog-enforcement-service-providers-controls-llm-%03d" % idx,
            ...
            "article_path": "bog_enforcement_service_providers_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on Engaging Enforcement Service Providers before the Board of Grievances -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 18/8/1447H / 2026-02-06)"),
                "source_authority_ar": ("ضوابط الاستعانة بمقدمي خدمات التنفيذ أمام ديوان المظالم — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 18/8/1447هـ الموافق 2026-02-06م)"),
    ...
    json.dump({
        "law_key": "bog_enforcement_service_providers_controls",
        "layer": "BOG_ENFORCEMENT_SERVICE_PROVIDERS_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "bog-enforcement-service-providers-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Controls on Engaging Enforcement Service Providers before the Board of Grievances — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on Engaging Enforcement Service Providers before the Board of Grievances records" % (len(ver), len(llm)))
