#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the شروط وإجراءات تأهيل واعتماد مراكز التدريب ومراكز الاختبار لسائقي ومديري التشغيل في أنشطة النقل البري track.
...
RECORDS = os.path.join(ROOT, "sources", "land_transport_training_centers_accreditation", "verified",
                       "land_transport_training_centers_accreditation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "land_transport_training_centers_accreditation", "verified",
                       "land_transport_training_centers_accreditation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "land_transport_training_centers_accreditation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"land_transport_training_centers_accreditation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "شروط وإجراءات تأهيل واعتماد مراكز التدريب ومراكز الاختبار لسائقي ومديري التشغيل في أنشطة النقل البري":
    ...
    if e:
        print("FAIL: %d error(s) in Conditions and Procedures for Qualifying and Accrediting Training and Testing Centres for Land Transport Drivers and Operations Managers track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Conditions and Procedures for Qualifying and Accrediting Training and Testing Centres for Land Transport Drivers and Operations Managers")
