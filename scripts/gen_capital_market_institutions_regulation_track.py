#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة مؤسسات السوق المالية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23232),
published 19/12/1444H (2023-07-07). The Gazette IS the official publication of record
...
STRUCTURE: 99 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "capital_market_institutions_regulation", "official_source",
                   "capital_market_institutions_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "capital_market_institutions_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "capital_market_institutions_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "capital_market_institutions_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "capital_market_institutions_regulation_arabic_legal_llm",
                        "capital_market_institutions_regulation_legal_llm_001_099.json")

LAW_ID = "sa-capital-market-institutions-regulation"
LAW_AR = "لائحة مؤسسات السوق المالية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"capital_market_institutions_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23232), published 19/12/1444H (2023-07-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "capital_market_institutions_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CAPITAL_MARKET_INSTITUTIONS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "capital-market-institutions-regulation-llm-%03d" % idx,
            ...
            "article_path": "capital_market_institutions_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Capital Market Institutions Regulations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 19/12/1444H / 2023-07-07)"),
                "source_authority_ar": ("لائحة مؤسسات السوق المالية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 19/12/1444هـ الموافق 2023-07-07م)"),
    ...
    json.dump({
        "law_key": "capital_market_institutions_regulation",
        "layer": "CAPITAL_MARKET_INSTITUTIONS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "capital-market-institutions-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (99 مادة، جميعها أصلية)",
        "title_en": "Capital Market Institutions Regulations — Arabic LLM-ready layer (99 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Capital Market Institutions Regulations records" % (len(ver), len(llm)))
