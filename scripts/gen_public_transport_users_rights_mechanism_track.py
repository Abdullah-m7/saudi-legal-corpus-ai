#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الآلية التفصيلية للائحة حقوق والتزامات مستخدمي وسائل النقل العام track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24105),
published 10/5/1445H (2023-11-24). The Gazette IS the official publication of record
...
STRUCTURE: 76 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "public_transport_users_rights_mechanism", "official_source",
                   "public_transport_users_rights_mechanism_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "public_transport_users_rights_mechanism", "verified")
RECORDS = os.path.join(OUT_VER, "public_transport_users_rights_mechanism_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "public_transport_users_rights_mechanism_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "public_transport_users_rights_mechanism_arabic_legal_llm",
                        "public_transport_users_rights_mechanism_legal_llm_001_076.json")

LAW_ID = "sa-public-transport-users-rights-mechanism"
LAW_AR = "الآلية التفصيلية للائحة حقوق والتزامات مستخدمي وسائل النقل العام"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"public_transport_users_rights_mechanism_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24105), published 10/5/1445H (2023-11-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "public_transport_users_rights_mechanism",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PUBLIC_TRANSPORT_USERS_RIGHTS_MECHANISM_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "public-transport-users-rights-mechanism-llm-%03d" % idx,
            ...
            "article_path": "public_transport_users_rights_mechanism/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Detailed Mechanism for the Regulation on Rights and Obligations of Public Transport Users -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 10/5/1445H / 2023-11-24)"),
                "source_authority_ar": ("الآلية التفصيلية للائحة حقوق والتزامات مستخدمي وسائل النقل العام — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 10/5/1445هـ الموافق 2023-11-24م)"),
    ...
    json.dump({
        "law_key": "public_transport_users_rights_mechanism",
        "layer": "PUBLIC_TRANSPORT_USERS_RIGHTS_MECHANISM_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "public-transport-users-rights-mechanism-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (76 مادة، جميعها أصلية)",
        "title_en": "Detailed Mechanism for the Regulation on Rights and Obligations of Public Transport Users — Arabic LLM-ready layer (76 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Detailed Mechanism for the Regulation on Rights and Obligations of Public Transport Users records" % (len(ver), len(llm)))
