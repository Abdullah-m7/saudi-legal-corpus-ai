#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Municipal Real Estate Disposal Law track (6 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) — all 6 matched outright (mean 0.974), so none
...
RECORDS = os.path.join(ROOT, "sources", "municipal_realestate", "law", "verified",
                       "municipal_realestate_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "municipal_realestate_arabic_legal_llm",
                   "municipal_realestate_law_legal_llm_001_006.json")
PDF = os.path.join(ROOT, "inputs", "municipal_realestate_official_pdfs",
                   "municipal_realestate_law_moj_official_ar.pdf")
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Municipal Real Estate Disposal Law track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Municipal Real Estate Disposal Law — 6 records (fresh issuance: all 6 اصلية)")
    print("  - trust gate: 6/6 MATCHES_PDF outright (mean 0.974); no visual adjudication needed")
    print("  - numbered 1..6 by ordinal position (no مكرر); no dual-status divergence")
    print("  - IN-FORCE Royal Decree M/64 (15/11/1392H); committed MOJ PDF hash verified; Arabic governs")
    return 0
