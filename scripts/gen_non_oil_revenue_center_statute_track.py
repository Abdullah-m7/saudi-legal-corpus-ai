#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مركز الإيرادات غير النفطية track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/.../4000863),
published 14/11/1447H (2026-05-01). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "non_oil_revenue_center_statute", "official_source",
                   "non_oil_revenue_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "non_oil_revenue_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "non_oil_revenue_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "non_oil_revenue_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "non_oil_revenue_center_statute_arabic_legal_llm",
                        "non_oil_revenue_center_statute_legal_llm_001_016.json")

LAW_ID = "sa-non-oil-revenue-center-statute"
LAW_AR = "تنظيم مركز الإيرادات غير النفطية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"non_oil_revenue_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/.../4000863), published 14/11/1447H (2026-05-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "non_oil_revenue_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "NON_OIL_REVENUE_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "non-oil-revenue-center-statute-llm-%03d" % idx,
            ...
            "article_path": "non_oil_revenue_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Non-Oil Revenue Center -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/11/1447H / 2026-05-01)"),
                "source_authority_ar": ("تنظيم مركز الإيرادات غير النفطية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 14/11/1447هـ الموافق 2026-05-01م)"),
    ...
    json.dump({
        "law_key": "non_oil_revenue_center_statute",
        "layer": "NON_OIL_REVENUE_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "non-oil-revenue-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Statute of the Non-Oil Revenue Center — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Non-Oil Revenue Center records" % (len(ver), len(llm)))
