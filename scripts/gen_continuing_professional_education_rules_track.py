#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد برامج التعليم المهني المستمر track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23654),
...
SRC = os.path.join(ROOT, "sources", "continuing_professional_education_rules", "official_source",
                   "continuing_professional_education_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "continuing_professional_education_rules", "verified")
RECORDS = os.path.join(OUT_VER, "continuing_professional_education_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "continuing_professional_education_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "continuing_professional_education_rules_arabic_legal_llm",
                        "continuing_professional_education_rules_legal_llm_001_009.json")

LAW_ID = "sa-continuing-professional-education-rules"
LAW_AR = "قواعد برامج التعليم المهني المستمر"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"continuing_professional_education_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23654), published 24/2/1445H (2023-09-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "continuing_professional_education_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "CONTINUING_PROFESSIONAL_EDUCATION_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "continuing-professional-education-rules-llm-%03d" % idx,
            ...
            "article_path": "continuing_professional_education_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules for Continuing Professional Education Programs -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/2/1445H / 2023-09-09)"),
                "source_authority_ar": ("قواعد برامج التعليم المهني المستمر — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "continuing_professional_education_rules",
        "layer": "CONTINUING_PROFESSIONAL_EDUCATION_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "continuing-professional-education-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (9 مادة، جميعها أصلية)",
        "title_en": "Rules for Continuing Professional Education Programs — Arabic LLM-ready layer (9 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules for Continuing Professional Education Programs records" % (len(ver), len(llm)))
