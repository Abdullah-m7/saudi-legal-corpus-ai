#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام مكافحة الاحتيال المالي وخيانة الأمانة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17528),
published 18/9/1442H (2021-04-30). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "financial_fraud_law", "official_source",
                   "financial_fraud_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "financial_fraud_law", "verified")
RECORDS = os.path.join(OUT_VER, "financial_fraud_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "financial_fraud_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "financial_fraud_law_arabic_legal_llm",
                        "financial_fraud_law_legal_llm_001_011.json")

LAW_ID = "sa-financial-fraud-law"
LAW_AR = "نظام مكافحة الاحتيال المالي وخيانة الأمانة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"financial_fraud_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17528), published 18/9/1442H (2021-04-30). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "financial_fraud_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "FINANCIAL_FRAUD_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "financial-fraud-law-llm-%03d" % idx,
            ...
            "article_path": "financial_fraud_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Anti-Financial-Fraud and Breach of Trust Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 18/9/1442H / 2021-04-30)"),
                "source_authority_ar": ("نظام مكافحة الاحتيال المالي وخيانة الأمانة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 18/9/1442هـ الموافق 2021-04-30م)"),
    ...
    json.dump({
        "law_key": "financial_fraud_law",
        "layer": "FINANCIAL_FRAUD_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "financial-fraud-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Anti-Financial-Fraud and Breach of Trust Law — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Anti-Financial-Fraud and Breach of Trust Law records" % (len(ver), len(llm)))
