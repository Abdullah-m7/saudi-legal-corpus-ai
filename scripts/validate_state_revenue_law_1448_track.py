#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام إيرادات الدولة track.

11 records: 30 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "state_revenue_law_1448", "verified",
                       "state_revenue_law_1448_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "state_revenue_law_1448", "verified",
                       "state_revenue_law_1448_verified_summary.json")
LLM = os.path.join(ROOT, "data", "state_revenue_law_1448_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "state_revenue_law_1448_council_decision_excludes_the_national_development_fund",
    "state_revenue_law_1448_published_but_not_yet_in_force",
    "state_revenue_law_1448_replaces_the_1431h_state_revenue_law",
    "state_revenue_law_1448_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    EXPECTED_NUMBERS = list(range(1, 31))
    MISSING_IN_SOURCE = []
    ...
    if src.get("document") != "نظام إيرادات الدولة":
    ...
    if e:
        print("FAIL: %d error(s) in State Revenue Law (1448H) track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: State Revenue Law (1448H)")
