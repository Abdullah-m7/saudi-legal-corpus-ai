#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام الأساس لشركة الصحة القابضة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19568),
...
SRC = os.path.join(ROOT, "sources", "health_holding_company_statute", "official_source",
                   "health_holding_company_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "health_holding_company_statute", "verified")
RECORDS = os.path.join(OUT_VER, "health_holding_company_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "health_holding_company_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "health_holding_company_statute_arabic_legal_llm",
                        "health_holding_company_statute_legal_llm_001_038.json")

LAW_ID = "sa-health-holding-company-statute"
LAW_AR = "النظام الأساس لشركة الصحة القابضة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"health_holding_company_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19568), published 11/11/1443H (2022-06-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "health_holding_company_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "HEALTH_HOLDING_COMPANY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "health-holding-company-statute-llm-%03d" % idx,
            ...
            "article_path": "health_holding_company_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Articles of Association of the Health Holding Company -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 11/11/1443H / 2022-06-10)"),
                "source_authority_ar": ("النظام الأساس لشركة الصحة القابضة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "health_holding_company_statute",
        "layer": "HEALTH_HOLDING_COMPANY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "health-holding-company-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (38 مادة، جميعها أصلية)",
        "title_en": "Articles of Association of the Health Holding Company — Arabic LLM-ready layer (38 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Articles of Association of the Health Holding Company records" % (len(ver), len(llm)))
