#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد التنظيمية لقياس حجم المنشأة track.
...
RECORDS = os.path.join(ROOT, "sources", "enterprise_size_measurement_rules", "verified",
                       "enterprise_size_measurement_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "enterprise_size_measurement_rules", "verified",
                       "enterprise_size_measurement_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "enterprise_size_measurement_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"enterprise_size_measurement_rules_no_decision_number_in_source", "enterprise_size_measurement_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد التنظيمية لقياس حجم المنشأة":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for Measuring Enterprise Size track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for Measuring Enterprise Size")
