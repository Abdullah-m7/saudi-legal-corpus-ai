#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام الحرف والصناعات اليدوية track.
...
RECORDS = os.path.join(ROOT, "sources", "handicrafts_law", "verified",
                       "handicrafts_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "handicrafts_law", "verified",
                       "handicrafts_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "handicrafts_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"handicrafts_law_no_decision_number_in_source", "handicrafts_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام الحرف والصناعات اليدوية":
    ...
    if e:
        print("FAIL: %d error(s) in Handicrafts and Handmade Industries Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Handicrafts and Handmade Industries Law")
