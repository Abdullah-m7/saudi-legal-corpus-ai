#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الاتفاقية العامة للتعاون بين حكومتي المملكة وسلوفاكيا track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_slovakia_general_cooperation", "verified",
                       "treaty_slovakia_general_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_slovakia_general_cooperation", "verified",
                       "treaty_slovakia_general_cooperation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_slovakia_general_cooperation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_slovakia_general_cooperation_no_decision_number_in_source", "treaty_slovakia_general_cooperation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الاتفاقية العامة للتعاون بين حكومتي المملكة وسلوفاكيا":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Slovakia General Cooperation Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Slovakia General Cooperation Agreement")
