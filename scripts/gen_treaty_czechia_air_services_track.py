#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية بين جمهورية التشيك والمملكة في مجال الخدمات الجوية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21203),
...
SRC = os.path.join(ROOT, "sources", "treaty_czechia_air_services", "official_source",
                   "treaty_czechia_air_services_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_czechia_air_services", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_czechia_air_services_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_czechia_air_services_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_czechia_air_services_arabic_legal_llm",
                        "treaty_czechia_air_services_legal_llm_001_024.json")

LAW_ID = "sa-treaty-czechia-air-services"
LAW_AR = "اتفاقية بين جمهورية التشيك والمملكة في مجال الخدمات الجوية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_czechia_air_services_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21203), published 13/6/1444H (2023-01-06). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_czechia_air_services",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_CZECHIA_AIR_SERVICES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-czechia-air-services-llm-%03d" % idx,
            ...
            "article_path": "treaty_czechia_air_services/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Arabia–Czech Republic Air Services Agreement -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/6/1444H / 2023-01-06)"),
                "source_authority_ar": ("اتفاقية بين جمهورية التشيك والمملكة في مجال الخدمات الجوية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_czechia_air_services",
        "layer": "TREATY_CZECHIA_AIR_SERVICES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-czechia-air-services-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (24 مادة، جميعها أصلية)",
        "title_en": "Saudi Arabia–Czech Republic Air Services Agreement — Arabic LLM-ready layer (24 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Arabia–Czech Republic Air Services Agreement records" % (len(ver), len(llm)))
