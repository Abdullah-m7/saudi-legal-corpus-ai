#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد مناطق الإيداع track.
...
RECORDS = os.path.join(ROOT, "sources", "deposit_zones_rules", "verified",
                       "deposit_zones_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "deposit_zones_rules", "verified",
                       "deposit_zones_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "deposit_zones_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"deposit_zones_rules_no_decision_number_in_source", "deposit_zones_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد مناطق الإيداع":
    ...
    if e:
        print("FAIL: %d error(s) in Rules of Deposit Zones track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules of Deposit Zones")
