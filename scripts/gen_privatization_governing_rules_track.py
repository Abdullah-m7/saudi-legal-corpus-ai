#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة للتخصيص track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18360),
...
SRC = os.path.join(ROOT, "sources", "privatization_governing_rules", "official_source",
                   "privatization_governing_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "privatization_governing_rules", "verified")
RECORDS = os.path.join(OUT_VER, "privatization_governing_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "privatization_governing_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "privatization_governing_rules_arabic_legal_llm",
                        "privatization_governing_rules_legal_llm_001_024.json")

LAW_ID = "sa-privatization-governing-rules"
LAW_AR = "القواعد المنظمة للتخصيص"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"privatization_governing_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18360), published 24/2/1443H (2021-10-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "privatization_governing_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PRIVATIZATION_GOVERNING_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "privatization-governing-rules-llm-%03d" % idx,
            ...
            "article_path": "privatization_governing_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing Privatization -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/2/1443H / 2021-10-01)"),
                "source_authority_ar": ("القواعد المنظمة للتخصيص — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "privatization_governing_rules",
        "layer": "PRIVATIZATION_GOVERNING_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "privatization-governing-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (24 مادة، جميعها أصلية)",
        "title_en": "Rules Governing Privatization — Arabic LLM-ready layer (24 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing Privatization records" % (len(ver), len(llm)))
