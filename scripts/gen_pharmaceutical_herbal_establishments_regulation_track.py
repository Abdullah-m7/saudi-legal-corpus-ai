#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام المنشآت والمستحضرات الصيدلانية والعشبية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17304),
published 16/6/1442H (2021-01-29). The Gazette IS the official publication of record
...
STRUCTURE: 42 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "pharmaceutical_herbal_establishments_regulation", "official_source",
                   "pharmaceutical_herbal_establishments_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "pharmaceutical_herbal_establishments_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "pharmaceutical_herbal_establishments_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "pharmaceutical_herbal_establishments_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "pharmaceutical_herbal_establishments_regulation_arabic_legal_llm",
                        "pharmaceutical_herbal_establishments_regulation_legal_llm_001_042.json")

LAW_ID = "sa-pharmaceutical-herbal-establishments-regulation"
LAW_AR = "اللائحة التنفيذية لنظام المنشآت والمستحضرات الصيدلانية والعشبية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"pharmaceutical_herbal_establishments_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17304), published 16/6/1442H (2021-01-29). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "pharmaceutical_herbal_establishments_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "PHARMACEUTICAL_HERBAL_ESTABLISHMENTS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "pharmaceutical-herbal-establishments-regulation-llm-%03d" % idx,
            ...
            "article_path": "pharmaceutical_herbal_establishments_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Law of Pharmaceutical and Herbal Establishments and Products -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 16/6/1442H / 2021-01-29)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام المنشآت والمستحضرات الصيدلانية والعشبية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 16/6/1442هـ الموافق 2021-01-29م)"),
    ...
    json.dump({
        "law_key": "pharmaceutical_herbal_establishments_regulation",
        "layer": "PHARMACEUTICAL_HERBAL_ESTABLISHMENTS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "pharmaceutical-herbal-establishments-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (42 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Law of Pharmaceutical and Herbal Establishments and Products — Arabic LLM-ready layer (42 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Law of Pharmaceutical and Herbal Establishments and Products records" % (len(ver), len(llm)))
