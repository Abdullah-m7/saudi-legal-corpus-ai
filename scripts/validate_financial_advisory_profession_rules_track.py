#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة لمهنة الاستشارات المالية (لغير الأوراق المالية) track.
...
RECORDS = os.path.join(ROOT, "sources", "financial_advisory_profession_rules", "verified",
                       "financial_advisory_profession_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "financial_advisory_profession_rules", "verified",
                       "financial_advisory_profession_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "financial_advisory_profession_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"financial_advisory_profession_rules_no_decision_number_in_source", "financial_advisory_profession_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "11/8/1447":
        e.append("[2j] gazette_publication_date_hijri must be 11/8/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-01-30":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-01-30")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "القواعد المنظمة لمهنة الاستشارات المالية (لغير الأوراق المالية)":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing the Financial Advisory Profession (Non-Securities) track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing the Financial Advisory Profession (Non-Securities)")
