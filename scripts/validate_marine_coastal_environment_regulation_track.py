#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنظام البيئة للإدارة المستدامة للبيئة البحرية والساحلية track.
...
RECORDS = os.path.join(ROOT, "sources", "marine_coastal_environment_regulation", "verified",
                       "marine_coastal_environment_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "marine_coastal_environment_regulation", "verified",
                       "marine_coastal_environment_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "marine_coastal_environment_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"marine_coastal_environment_regulation_no_decision_number_in_source", "marine_coastal_environment_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "3/2/1443":
        e.append("[2j] gazette_publication_date_hijri must be 3/2/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-09-10":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-09-10")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنفيذية لنظام البيئة للإدارة المستدامة للبيئة البحرية والساحلية":
    ...
    if e:
        print("FAIL: %d error(s) in Implementing Regulation of the Environment Law for the Sustainable Management of the Marine and Coastal Environment track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Implementing Regulation of the Environment Law for the Sustainable Management of the Marine and Coastal Environment")
