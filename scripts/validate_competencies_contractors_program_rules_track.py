#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة لبرنامج الكفاءات والمتعاقدين track.
...
RECORDS = os.path.join(ROOT, "sources", "competencies_contractors_program_rules", "verified",
                       "competencies_contractors_program_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "competencies_contractors_program_rules", "verified",
                       "competencies_contractors_program_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "competencies_contractors_program_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"competencies_contractors_program_rules_no_decision_number_in_source", "competencies_contractors_program_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "28/1/1444":
        e.append("[2j] gazette_publication_date_hijri must be 28/1/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-08-26":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-08-26")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "القواعد المنظمة لبرنامج الكفاءات والمتعاقدين":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing the Competencies and Contractors Program track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing the Competencies and Contractors Program")
