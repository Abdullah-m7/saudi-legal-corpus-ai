#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the دليل السياسات والممارسات الوطنية للاستزراع المائي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24151),
published 17/5/1445H (2023-12-01). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "aquaculture_national_policies_guide", "official_source",
                   "aquaculture_national_policies_guide_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "aquaculture_national_policies_guide", "verified")
RECORDS = os.path.join(OUT_VER, "aquaculture_national_policies_guide_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "aquaculture_national_policies_guide_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "aquaculture_national_policies_guide_arabic_legal_llm",
                        "aquaculture_national_policies_guide_legal_llm_001_031.json")

LAW_ID = "sa-aquaculture-national-policies-guide"
LAW_AR = "دليل السياسات والممارسات الوطنية للاستزراع المائي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"aquaculture_national_policies_guide_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24151), published 17/5/1445H (2023-12-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "aquaculture_national_policies_guide",
            "law_component": "guide",
            "language": "ar",
            "record_layer": "AQUACULTURE_NATIONAL_POLICIES_GUIDE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "aquaculture-national-policies-guide-llm-%03d" % idx,
            ...
            "article_path": "aquaculture_national_policies_guide/guide/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("National Aquaculture Policies and Practices Guide -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/5/1445H / 2023-12-01)"),
                "source_authority_ar": ("دليل السياسات والممارسات الوطنية للاستزراع المائي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 17/5/1445هـ الموافق 2023-12-01م)"),
    ...
    json.dump({
        "law_key": "aquaculture_national_policies_guide",
        "layer": "AQUACULTURE_NATIONAL_POLICIES_GUIDE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "aquaculture-national-policies-guide-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "guide",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (31 مادة، جميعها أصلية)",
        "title_en": "National Aquaculture Policies and Practices Guide — Arabic LLM-ready layer (31 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready National Aquaculture Policies and Practices Guide records" % (len(ver), len(llm)))
