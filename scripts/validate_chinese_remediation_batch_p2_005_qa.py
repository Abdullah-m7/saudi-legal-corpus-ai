#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate the Batch P2-005 QA (P2 expansion QA; article-by-article vs the official Arabic; Babs 12/13/14).

Confirms the QA covers exactly the 15 P2-005 articles, that every per-article bab is in [12,13,14] and
equals both the P2-005 remediation record bab and the coverage-index expected_bab_number, that
...
internal / non-official / non-binding / non-governing posture, points at the P2-005 remediation file,
embeds no full text, and touches no protected layer (P2-005 data, P2-001 + P2-002 data + QA, all P1 + P0
batches + QA, and base corpora). Review only. No p3_* dirs; no full-281 / trilingual. Read-only.

Usage: validate_chinese_remediation_batch_p2_005_qa.py [QA_JSON_PATH]
...
SRC_REL = "data/chinese_remediation_batches/p2_005/companies_law_m132_1443_zh_internal_remediation_p2_005.json"
...
REQ_TOP = ("stage", "qa_stage", "batch_id", "source_batch_file", "scope_articles", "expected_babs",
           ...
           "trilingual_alignment_created", "p2_005_chinese_text_modified", "p2_005_data_modified",
...
def main(argv=None) -> int:
    ...
    problems = []
    for p, label in ((qa_path, "QA JSON"), (MD, "Arabic QA report"), (SRC, "P2-005 remediation file")):
    ...
    if qa.get("stage") != "CHINESE_REMEDIATION_BATCH_P2_005_QA":
        problems.append("stage must be CHINESE_REMEDIATION_BATCH_P2_005_QA")
    if qa.get("qa_stage") != "CHINESE_REMEDIATION_BATCH_P2_005_QA":
        problems.append("qa_stage must be CHINESE_REMEDIATION_BATCH_P2_005_QA")
    if qa.get("batch_id") != "P2-005":
        problems.append("batch_id must be P2-005")
    if qa.get("source_batch_file") != SRC_REL:
        problems.append("source_batch_file must point to the P2-005 remediation JSON")
    if qa.get("scope_articles") != ARTS:
        problems.append("scope_articles must exactly match the P2-005 list")
    if qa.get("article_count") != 15:
        problems.append("article_count must be 15")
    if qa.get("expected_babs") != [12, 13, 14]:
        problems.append("expected_babs must be [12, 13, 14]")
    ...
    if nums != ARTS:
        problems.append("per_article_reviews must cover exactly the 15 P2-005 articles in order")
    ...
    for r in recs:
        ...
        if r.get("bab") not in BABS:
            problems.append("art %s bab must be in [12,13,14]" % n)
        if n in srec and r.get("bab") != srec[n].get("bab"):
            problems.append("art %s QA bab %r != P2-005 record bab %r" % (n, r.get("bab"), srec[n].get("bab")))
    ...
    if qa.get("p2_005_chinese_text_modified") is True or qa.get("p2_005_data_modified") is True:
        if not fixes:
            problems.append("p2_005 marked modified but no minor_fixes recorded")
    else:
        if fixes:
            problems.append("minor_fixes recorded but p2_005 marked unmodified")
    ...
    if len(src["records"]) != 15:
        problems.append("P2-005 remediation must remain 15 records")
    ...
    print("=" * 60)
    print("Chinese remediation Batch P2-005 QA validation (P2 expansion QA)")
    print("=" * 60)
    if problems:
        for p in problems:
            print("  -", p)
        print("RESULT: %d problem(s) found ✗" % len(problems))
        return 1
    print("[PASS] Batch P2-005 QA: 15 articles across Babs [12,13,14] reviewed article-by-article vs the "
          "official Arabic (English secondary; prior condensed candidate as the expansion baseline); "
          "each QA bab matches the P2-005 record and the coverage index; for every pass, source "
          ...
          "counts consistent; internal/non-official/non-binding/non-governing; review-only (P2-005 data "
