#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الاتفاقية العربية لمنع ومكافحة الاستنساخ البشري track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24450),
...
SRC = os.path.join(ROOT, "sources", "treaty_arab_anti_human_cloning", "official_source",
                   "treaty_arab_anti_human_cloning_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_arab_anti_human_cloning", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_arab_anti_human_cloning_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_arab_anti_human_cloning_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_arab_anti_human_cloning_arabic_legal_llm",
                        "treaty_arab_anti_human_cloning_legal_llm_001_036.json")

LAW_ID = "sa-treaty-arab-anti-human-cloning"
LAW_AR = "الاتفاقية العربية لمنع ومكافحة الاستنساخ البشري"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_arab_anti_human_cloning_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24450), published 21/7/1445H (2024-02-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_arab_anti_human_cloning",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_ARAB_ANTI_HUMAN_CLONING_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-arab-anti-human-cloning-llm-%03d" % idx,
            ...
            "article_path": "treaty_arab_anti_human_cloning/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Arab Convention on the Prevention and Combating of Human Cloning -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/7/1445H / 2024-02-02)"),
                "source_authority_ar": ("الاتفاقية العربية لمنع ومكافحة الاستنساخ البشري — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_arab_anti_human_cloning",
        "layer": "TREATY_ARAB_ANTI_HUMAN_CLONING_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-arab-anti-human-cloning-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (36 مادة، جميعها أصلية)",
        "title_en": "Arab Convention on the Prevention and Combating of Human Cloning — Arabic LLM-ready layer (36 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Arab Convention on the Prevention and Combating of Human Cloning records" % (len(ver), len(llm)))
