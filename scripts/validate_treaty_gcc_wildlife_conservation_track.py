#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية المحافظة على الحياة الفطرية ومواطنها الطبيعية في مجلس التعاون لدول الخليج العربية track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_gcc_wildlife_conservation", "verified",
                       "treaty_gcc_wildlife_conservation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_gcc_wildlife_conservation", "verified",
                       "treaty_gcc_wildlife_conservation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_gcc_wildlife_conservation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_gcc_wildlife_conservation_no_decision_number_in_source", "treaty_gcc_wildlife_conservation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "6/5/1443":
        e.append("[2j] gazette_publication_date_hijri must be 6/5/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-12-10":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-12-10")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية المحافظة على الحياة الفطرية ومواطنها الطبيعية في مجلس التعاون لدول الخليج العربية":
    ...
    if e:
        print("FAIL: %d error(s) in GCC Agreement on the Conservation of Wildlife and Its Natural Habitats track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: GCC Agreement on the Conservation of Wildlife and Its Natural Habitats")
