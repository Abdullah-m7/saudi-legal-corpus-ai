#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية للهيئة العامة للأمن الغذائي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21311),
...
SRC = os.path.join(ROOT, "sources", "food_security_authority_statute", "official_source",
                   "food_security_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "food_security_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "food_security_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "food_security_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "food_security_authority_statute_arabic_legal_llm",
                        "food_security_authority_statute_legal_llm_001_016.json")

LAW_ID = "sa-food-security-authority-statute"
LAW_AR = "الترتيبات التنظيمية للهيئة العامة للأمن الغذائي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"food_security_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21311), published 5/7/1444H (2023-01-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "food_security_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "FOOD_SECURITY_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "food-security-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "food_security_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the General Food Security Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/7/1444H / 2023-01-27)"),
                "source_authority_ar": ("الترتيبات التنظيمية للهيئة العامة للأمن الغذائي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "food_security_authority_statute",
        "layer": "FOOD_SECURITY_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "food-security-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the General Food Security Authority — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the General Food Security Authority records" % (len(ver), len(llm)))
