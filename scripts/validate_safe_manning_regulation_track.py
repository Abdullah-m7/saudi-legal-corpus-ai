#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الحد الأدنى للتطقيم الآمن للسفن track.
...
RECORDS = os.path.join(ROOT, "sources", "safe_manning_regulation", "verified",
                       "safe_manning_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "safe_manning_regulation", "verified",
                       "safe_manning_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "safe_manning_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"safe_manning_regulation_no_decision_number_in_source", "safe_manning_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة الحد الأدنى للتطقيم الآمن للسفن":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on the Minimum Safe Manning of Ships track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on the Minimum Safe Manning of Ships")
