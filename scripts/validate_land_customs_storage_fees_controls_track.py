#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط رسوم تخزين البضائع بالمنافذ الجمركية البرية track.
...
RECORDS = os.path.join(ROOT, "sources", "land_customs_storage_fees_controls", "verified",
                       "land_customs_storage_fees_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "land_customs_storage_fees_controls", "verified",
                       "land_customs_storage_fees_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "land_customs_storage_fees_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"land_customs_storage_fees_controls_no_decision_number_in_source", "land_customs_storage_fees_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "11/3/1444":
        e.append("[2j] gazette_publication_date_hijri must be 11/3/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-10-07":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-10-07")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط رسوم تخزين البضائع بالمنافذ الجمركية البرية":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on Goods Storage Fees at Land Customs Ports track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on Goods Storage Fees at Land Customs Ports")
