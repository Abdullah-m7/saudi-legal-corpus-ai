#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد المستفيد الحقيقي للجمعيات والمؤسسات الأهلية track.
...
RECORDS = os.path.join(ROOT, "sources", "nonprofit_beneficial_owner_rules", "verified",
                       "nonprofit_beneficial_owner_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "nonprofit_beneficial_owner_rules", "verified",
                       "nonprofit_beneficial_owner_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "nonprofit_beneficial_owner_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"nonprofit_beneficial_owner_rules_no_decision_number_in_source", "nonprofit_beneficial_owner_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "28/2/1447":
        e.append("[2j] gazette_publication_date_hijri must be 28/2/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-08-22":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-08-22")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد المستفيد الحقيقي للجمعيات والمؤسسات الأهلية":
    ...
    if e:
        print("FAIL: %d error(s) in Beneficial Owner Rules for Associations and Non-Profit Foundations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Beneficial Owner Rules for Associations and Non-Profit Foundations")
