#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة الفنية للصهاريج (صهاريج نقل الغاز الجاف) track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26830),
...
SRC = os.path.join(ROOT, "sources", "dry_gas_tankers_technical_regulation", "official_source",
                   "dry_gas_tankers_technical_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "dry_gas_tankers_technical_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "dry_gas_tankers_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "dry_gas_tankers_technical_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "dry_gas_tankers_technical_regulation_arabic_legal_llm",
                        "dry_gas_tankers_technical_regulation_legal_llm_001_011.json")

LAW_ID = "sa-dry-gas-tankers-technical-regulation"
LAW_AR = "اللائحة الفنية للصهاريج (صهاريج نقل الغاز الجاف)"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"dry_gas_tankers_technical_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26830), published 5/6/1446H (2024-12-06). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "dry_gas_tankers_technical_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "DRY_GAS_TANKERS_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "dry-gas-tankers-technical-regulation-llm-%03d" % idx,
            ...
            "article_path": "dry_gas_tankers_technical_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Technical Regulation for Tankers (Dry Gas Transport Tankers) -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/6/1446H / 2024-12-06)"),
                "source_authority_ar": ("اللائحة الفنية للصهاريج (صهاريج نقل الغاز الجاف) — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "dry_gas_tankers_technical_regulation",
        "layer": "DRY_GAS_TANKERS_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "dry-gas-tankers-technical-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Technical Regulation for Tankers (Dry Gas Transport Tankers) — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Technical Regulation for Tankers (Dry Gas Transport Tankers) records" % (len(ver), len(llm)))
