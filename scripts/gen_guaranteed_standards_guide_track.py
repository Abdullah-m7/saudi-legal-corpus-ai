#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the دليل المعايير المضمونة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23862),
...
SRC = os.path.join(ROOT, "sources", "guaranteed_standards_guide", "official_source",
                   "guaranteed_standards_guide_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "guaranteed_standards_guide", "verified")
RECORDS = os.path.join(OUT_VER, "guaranteed_standards_guide_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "guaranteed_standards_guide_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "guaranteed_standards_guide_arabic_legal_llm",
                        "guaranteed_standards_guide_legal_llm_001_009.json")

LAW_ID = "sa-guaranteed-standards-guide"
LAW_AR = "دليل المعايير المضمونة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"guaranteed_standards_guide_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23862), published 28/3/1445H (2023-10-13). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "guaranteed_standards_guide",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "GUARANTEED_STANDARDS_GUIDE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "guaranteed-standards-guide-llm-%03d" % idx,
            ...
            "article_path": "guaranteed_standards_guide/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Guaranteed Standards Guide -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/3/1445H / 2023-10-13)"),
                "source_authority_ar": ("دليل المعايير المضمونة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "guaranteed_standards_guide",
        "layer": "GUARANTEED_STANDARDS_GUIDE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "guaranteed-standards-guide-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (9 مادة، جميعها أصلية)",
        "title_en": "Guaranteed Standards Guide — Arabic LLM-ready layer (9 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Guaranteed Standards Guide records" % (len(ver), len(llm)))
