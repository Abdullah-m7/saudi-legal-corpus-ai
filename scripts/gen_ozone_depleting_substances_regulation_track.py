#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية للمواد المستنفدة لطبقة الأوزون ومركبات الكربون الهيدروفلورية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17837),
...
SRC = os.path.join(ROOT, "sources", "ozone_depleting_substances_regulation", "official_source",
                   "ozone_depleting_substances_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "ozone_depleting_substances_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "ozone_depleting_substances_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "ozone_depleting_substances_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "ozone_depleting_substances_regulation_arabic_legal_llm",
                        "ozone_depleting_substances_regulation_legal_llm_001_009.json")

LAW_ID = "sa-ozone-depleting-substances-regulation"
LAW_AR = "اللائحة التنفيذية للمواد المستنفدة لطبقة الأوزون ومركبات الكربون الهيدروفلورية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"ozone_depleting_substances_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17837), published 15/11/1442H (2021-06-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "ozone_depleting_substances_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "OZONE_DEPLETING_SUBSTANCES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "ozone-depleting-substances-regulation-llm-%03d" % idx,
            ...
            "article_path": "ozone_depleting_substances_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation for Ozone-Depleting Substances and Hydrofluorocarbons -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/11/1442H / 2021-06-25)"),
                "source_authority_ar": ("اللائحة التنفيذية للمواد المستنفدة لطبقة الأوزون ومركبات الكربون الهيدروفلورية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "ozone_depleting_substances_regulation",
        "layer": "OZONE_DEPLETING_SUBSTANCES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "ozone-depleting-substances-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (9 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation for Ozone-Depleting Substances and Hydrofluorocarbons — Arabic LLM-ready layer (9 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation for Ozone-Depleting Substances and Hydrofluorocarbons records" % (len(ver), len(llm)))
