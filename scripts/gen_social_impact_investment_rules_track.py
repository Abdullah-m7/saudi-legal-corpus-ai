#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد تفعيل الاستثمار ذي الأثر الاجتماعي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27373),
...
SRC = os.path.join(ROOT, "sources", "social_impact_investment_rules", "official_source",
                   "social_impact_investment_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "social_impact_investment_rules", "verified")
RECORDS = os.path.join(OUT_VER, "social_impact_investment_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "social_impact_investment_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "social_impact_investment_rules_arabic_legal_llm",
                        "social_impact_investment_rules_legal_llm_001_014.json")

LAW_ID = "sa-social-impact-investment-rules"
LAW_AR = "قواعد تفعيل الاستثمار ذي الأثر الاجتماعي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"social_impact_investment_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27373), published 28/2/1447H (2025-08-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "social_impact_investment_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SOCIAL_IMPACT_INVESTMENT_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "social-impact-investment-rules-llm-%03d" % idx,
            ...
            "article_path": "social_impact_investment_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules for Activating Social Impact Investment -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/2/1447H / 2025-08-22)"),
                "source_authority_ar": ("قواعد تفعيل الاستثمار ذي الأثر الاجتماعي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "social_impact_investment_rules",
        "layer": "SOCIAL_IMPACT_INVESTMENT_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "social-impact-investment-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Rules for Activating Social Impact Investment — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules for Activating Social Impact Investment records" % (len(ver), len(llm)))
