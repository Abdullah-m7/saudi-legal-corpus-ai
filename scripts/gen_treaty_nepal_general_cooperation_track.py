#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية عامة للتعاون بين حكومتي المملكة وجمهورية نيبال track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21253),
...
SRC = os.path.join(ROOT, "sources", "treaty_nepal_general_cooperation", "official_source",
                   "treaty_nepal_general_cooperation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_nepal_general_cooperation", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_nepal_general_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_nepal_general_cooperation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_nepal_general_cooperation_arabic_legal_llm",
                        "treaty_nepal_general_cooperation_legal_llm_001_010.json")

LAW_ID = "sa-treaty-nepal-general-cooperation"
LAW_AR = "اتفاقية عامة للتعاون بين حكومتي المملكة وجمهورية نيبال"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_nepal_general_cooperation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21253), published 20/6/1444H (2023-01-13). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_nepal_general_cooperation",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_NEPAL_GENERAL_COOPERATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-nepal-general-cooperation-llm-%03d" % idx,
            ...
            "article_path": "treaty_nepal_general_cooperation/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Arabia–Nepal General Cooperation Agreement -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/6/1444H / 2023-01-13)"),
                "source_authority_ar": ("اتفاقية عامة للتعاون بين حكومتي المملكة وجمهورية نيبال — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_nepal_general_cooperation",
        "layer": "TREATY_NEPAL_GENERAL_COOPERATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-nepal-general-cooperation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Saudi Arabia–Nepal General Cooperation Agreement — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Arabia–Nepal General Cooperation Agreement records" % (len(ver), len(llm)))
