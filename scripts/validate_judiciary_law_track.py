#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Law of the Judiciary track (85 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ
PDF, hash re-verified) EXCEPT the 3 visually-adjudicated enumerated/short
articles (9, 71, 85) confirmed verbatim on the rendered pages; the numbered
sequence is a complete 1..85 (no مكرر); and every article carries an explained
...
RECORDS = os.path.join(ROOT, "sources", "judiciary", "law", "verified",
                       "judiciary_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "judiciary_arabic_legal_llm",
                   "judiciary_law_legal_llm_001_085.json")
PDF = os.path.join(ROOT, "inputs", "judiciary_official_pdfs",
                   "judiciary_law_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 85
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 82, "معدلة": 3}
VISUALLY_ADJUDICATED = {"qada_law_art_009", "qada_law_art_071", "qada_law_art_085"}
AR = "ء-ي"
...
def main():
    ...
    # [1] structure: complete 1..85, no mukarrar
    nums = sorted(int(re.match(r"qada_law_art_(\d{3})", k).group(1)) for k in arts
    ...
    if e:
        ...
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Law of the Judiciary — 85 records (consolidated: 82 اصلية / 3 معدلة)")
    print("  - trust gate: every article MATCHES_PDF; 3 enumerated/short articles (9, 71, 85) visually adjudicated")
    print("  - complete 1..85 (no مكرر); committed official MOJ PDF hash verified")
    print("  - 3 amended articles (5, 35, 72) carry amendment history; no repealed/added; no dual-status")
