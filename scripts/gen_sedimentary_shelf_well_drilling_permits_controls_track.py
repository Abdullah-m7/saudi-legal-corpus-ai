#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط إصدار رخص حفر الآبار الجديدة على الرف الرسوبي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27278),
...
SRC = os.path.join(ROOT, "sources", "sedimentary_shelf_well_drilling_permits_controls", "official_source",
                   "sedimentary_shelf_well_drilling_permits_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "sedimentary_shelf_well_drilling_permits_controls", "verified")
RECORDS = os.path.join(OUT_VER, "sedimentary_shelf_well_drilling_permits_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "sedimentary_shelf_well_drilling_permits_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "sedimentary_shelf_well_drilling_permits_controls_arabic_legal_llm",
                        "sedimentary_shelf_well_drilling_permits_controls_legal_llm_001_008.json")

LAW_ID = "sa-sedimentary-shelf-well-drilling-permits-controls"
LAW_AR = "ضوابط إصدار رخص حفر الآبار الجديدة على الرف الرسوبي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"sedimentary_shelf_well_drilling_permits_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27278), published 23/1/1447H (2025-07-18). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "sedimentary_shelf_well_drilling_permits_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SEDIMENTARY_SHELF_WELL_DRILLING_PERMITS_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "sedimentary-shelf-well-drilling-permits-controls-llm-%03d" % idx,
            ...
            "article_path": "sedimentary_shelf_well_drilling_permits_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on Issuing Permits to Drill New Wells on the Sedimentary Shelf -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/1/1447H / 2025-07-18)"),
                "source_authority_ar": ("ضوابط إصدار رخص حفر الآبار الجديدة على الرف الرسوبي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "sedimentary_shelf_well_drilling_permits_controls",
        "layer": "SEDIMENTARY_SHELF_WELL_DRILLING_PERMITS_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "sedimentary-shelf-well-drilling-permits-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Controls on Issuing Permits to Drill New Wells on the Sedimentary Shelf — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on Issuing Permits to Drill New Wells on the Sedimentary Shelf records" % (len(ver), len(llm)))
