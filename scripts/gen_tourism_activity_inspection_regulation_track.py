#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة التفتيش على الأنشطة السياحية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21123),
...
SRC = os.path.join(ROOT, "sources", "tourism_activity_inspection_regulation", "official_source",
                   "tourism_activity_inspection_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "tourism_activity_inspection_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "tourism_activity_inspection_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "tourism_activity_inspection_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "tourism_activity_inspection_regulation_arabic_legal_llm",
                        "tourism_activity_inspection_regulation_legal_llm_001_023.json")

LAW_ID = "sa-tourism-activity-inspection-regulation"
LAW_AR = "لائحة التفتيش على الأنشطة السياحية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"tourism_activity_inspection_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21123), published 1/6/1444H (2022-12-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "tourism_activity_inspection_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "TOURISM_ACTIVITY_INSPECTION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "tourism-activity-inspection-regulation-llm-%03d" % idx,
            ...
            "article_path": "tourism_activity_inspection_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on the Inspection of Tourism Activities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/6/1444H / 2022-12-25)"),
                "source_authority_ar": ("لائحة التفتيش على الأنشطة السياحية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "tourism_activity_inspection_regulation",
        "layer": "TOURISM_ACTIVITY_INSPECTION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "tourism-activity-inspection-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (23 مادة، جميعها أصلية)",
        "title_en": "Regulation on the Inspection of Tourism Activities — Arabic LLM-ready layer (23 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on the Inspection of Tourism Activities records" % (len(ver), len(llm)))
