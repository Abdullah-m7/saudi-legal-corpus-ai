#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مركز الإقامة المميزة track.
...
RECORDS = os.path.join(ROOT, "sources", "premium_residency_center_statute", "verified",
                       "premium_residency_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "premium_residency_center_statute", "verified",
                       "premium_residency_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "premium_residency_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"premium_residency_center_statute_no_decision_number_in_source", "premium_residency_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "4/11/1443":
        e.append("[2j] gazette_publication_date_hijri must be 4/11/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-06-03":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-06-03")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم مركز الإقامة المميزة":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Premium Residency Center track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Premium Residency Center")
