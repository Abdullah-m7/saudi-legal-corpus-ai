#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية عامة للتعاون بين حكومة المملكة وحكومة تشاد track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_chad_general_cooperation", "verified",
                       "treaty_chad_general_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_chad_general_cooperation", "verified",
                       "treaty_chad_general_cooperation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_chad_general_cooperation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_chad_general_cooperation_no_decision_number_in_source", "treaty_chad_general_cooperation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية عامة للتعاون بين حكومة المملكة وحكومة تشاد":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Chad General Cooperation Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Chad General Cooperation Agreement")
