#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد البيع بالمزاد العلني track.
...
RECORDS = os.path.join(ROOT, "sources", "public_auction_sale_rules", "verified",
                       "public_auction_sale_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "public_auction_sale_rules", "verified",
                       "public_auction_sale_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "public_auction_sale_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"public_auction_sale_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد البيع بالمزاد العلني":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing Sale by Public Auction track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing Sale by Public Auction")
