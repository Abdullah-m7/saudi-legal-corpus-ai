#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام الأساس للمركز العالمي لمكافحة الفكر المتطرف track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4001035),
published 5/12/1447H (2026-05-22). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "extremism_countering_center_statute", "official_source",
                   "extremism_countering_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "extremism_countering_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "extremism_countering_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "extremism_countering_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "extremism_countering_center_statute_arabic_legal_llm",
                        "extremism_countering_center_statute_legal_llm_001_014.json")

LAW_ID = "sa-extremism-countering-center-statute"
LAW_AR = "النظام الأساس للمركز العالمي لمكافحة الفكر المتطرف"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"extremism_countering_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4001035), published 5/12/1447H (2026-05-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "extremism_countering_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "EXTREMISM_COUNTERING_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "extremism-countering-center-statute-llm-%03d" % idx,
            ...
            "article_path": "extremism_countering_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Global Center for Combating Extremist Ideology -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/12/1447H / 2026-05-22)"),
                "source_authority_ar": ("النظام الأساس للمركز العالمي لمكافحة الفكر المتطرف — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 5/12/1447هـ الموافق 2026-05-22م)"),
    ...
    json.dump({
        "law_key": "extremism_countering_center_statute",
        "layer": "EXTREMISM_COUNTERING_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "extremism-countering-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Statute of the Global Center for Combating Extremist Ideology — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Global Center for Combating Extremist Ideology records" % (len(ver), len(llm)))
