#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم المركز الوطني للصقور track.
...
RECORDS = os.path.join(ROOT, "sources", "falcon_center_statute", "verified",
                       "falcon_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "falcon_center_statute", "verified",
                       "falcon_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "falcon_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"falcon_center_statute_no_decision_number_in_source", "falcon_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم المركز الوطني للصقور":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the National Falcon Center track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the National Falcon Center")
