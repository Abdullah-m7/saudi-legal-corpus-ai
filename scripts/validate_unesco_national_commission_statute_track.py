#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم اللجنة الوطنية للتربية والثقافة والعلوم track.
...
RECORDS = os.path.join(ROOT, "sources", "unesco_national_commission_statute", "verified",
                       "unesco_national_commission_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "unesco_national_commission_statute", "verified",
                       "unesco_national_commission_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "unesco_national_commission_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"unesco_national_commission_statute_no_decision_number_in_source", "unesco_national_commission_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "4/8/1447":
        e.append("[2j] gazette_publication_date_hijri must be 4/8/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-01-23":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-01-23")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم اللجنة الوطنية للتربية والثقافة والعلوم":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the National Commission for Education, Culture and Science track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the National Commission for Education, Culture and Science")
