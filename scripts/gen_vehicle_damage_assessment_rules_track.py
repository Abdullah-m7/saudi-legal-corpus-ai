#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة لفرع تقييم أضرار المركبات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24972),
published 9/11/1445H (2024-05-17). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "vehicle_damage_assessment_rules", "official_source",
                   "vehicle_damage_assessment_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "vehicle_damage_assessment_rules", "verified")
RECORDS = os.path.join(OUT_VER, "vehicle_damage_assessment_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "vehicle_damage_assessment_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "vehicle_damage_assessment_rules_arabic_legal_llm",
                        "vehicle_damage_assessment_rules_legal_llm_001_040.json")

LAW_ID = "sa-vehicle-damage-assessment-rules"
LAW_AR = "القواعد المنظمة لفرع تقييم أضرار المركبات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"vehicle_damage_assessment_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24972), published 9/11/1445H (2024-05-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "vehicle_damage_assessment_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "VEHICLE_DAMAGE_ASSESSMENT_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "vehicle-damage-assessment-rules-llm-%03d" % idx,
            ...
            "article_path": "vehicle_damage_assessment_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing the Vehicle Damage Assessment Branch -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/11/1445H / 2024-05-17)"),
                "source_authority_ar": ("القواعد المنظمة لفرع تقييم أضرار المركبات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 9/11/1445هـ الموافق 2024-05-17م)"),
    ...
    json.dump({
        "law_key": "vehicle_damage_assessment_rules",
        "layer": "VEHICLE_DAMAGE_ASSESSMENT_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "vehicle-damage-assessment-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (40 مادة، جميعها أصلية)",
        "title_en": "Rules Governing the Vehicle Damage Assessment Branch — Arabic LLM-ready layer (40 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing the Vehicle Damage Assessment Branch records" % (len(ver), len(llm)))
