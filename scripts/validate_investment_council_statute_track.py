#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لمجلس الاستثمار track.
...
RECORDS = os.path.join(ROOT, "sources", "investment_council_statute", "verified",
                       "investment_council_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "investment_council_statute", "verified",
                       "investment_council_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "investment_council_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"investment_council_statute_no_decision_number_in_source", "investment_council_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "16/2/1445":
        e.append("[2j] gazette_publication_date_hijri must be 16/2/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-09-01":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-09-01")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية لمجلس الاستثمار":
    ...
    if e:
        print("FAIL: %d error(s) in Investment Council — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Investment Council — Organisational Arrangements")
