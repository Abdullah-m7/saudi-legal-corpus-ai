#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم هيئة الحكومة الرقمية track.
...
RECORDS = os.path.join(ROOT, "sources", "digital_government_authority_statute", "verified",
                       "digital_government_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "digital_government_authority_statute", "verified",
                       "digital_government_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "digital_government_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"digital_government_authority_statute_no_decision_number_in_source", "digital_government_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "6/8/1442":
        e.append("[2j] gazette_publication_date_hijri must be 6/8/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-03-19":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-03-19")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم هيئة الحكومة الرقمية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Digital Government Authority track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Digital Government Authority")
