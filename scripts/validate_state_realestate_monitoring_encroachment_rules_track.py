#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد مراقبة عقارات الدولة وإزالة التعديات track.
...
RECORDS = os.path.join(ROOT, "sources", "state_realestate_monitoring_encroachment_rules", "verified",
                       "state_realestate_monitoring_encroachment_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "state_realestate_monitoring_encroachment_rules", "verified",
                       "state_realestate_monitoring_encroachment_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "state_realestate_monitoring_encroachment_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"state_realestate_monitoring_encroachment_rules_no_decision_number_in_source", "state_realestate_monitoring_encroachment_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد مراقبة عقارات الدولة وإزالة التعديات":
    ...
    if e:
        print("FAIL: %d error(s) in Rules on Monitoring State Real Estate and Removing Encroachments track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules on Monitoring State Real Estate and Removing Encroachments")
