#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط الاستعانة بمقدمي خدمات التنفيذ أمام ديوان المظالم track.
...
RECORDS = os.path.join(ROOT, "sources", "bog_enforcement_service_providers_controls", "verified",
                       "bog_enforcement_service_providers_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "bog_enforcement_service_providers_controls", "verified",
                       "bog_enforcement_service_providers_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "bog_enforcement_service_providers_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"bog_enforcement_service_providers_controls_no_decision_number_in_source", "bog_enforcement_service_providers_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "18/8/1447":
        e.append("[2j] gazette_publication_date_hijri must be 18/8/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-02-06":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-02-06")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط الاستعانة بمقدمي خدمات التنفيذ أمام ديوان المظالم":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on Engaging Enforcement Service Providers before the Board of Grievances track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on Engaging Enforcement Service Providers before the Board of Grievances")
