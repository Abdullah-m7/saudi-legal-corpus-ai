#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the شروط ومتطلبات تأهيل الجهات الاستشارية الهندسية لمشاريع بيع أو تأجير الوحدات العقارية على الخارطة track.
...
RECORDS = os.path.join(ROOT, "sources", "offplan_engineering_consultants_qualification", "verified",
                       "offplan_engineering_consultants_qualification_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "offplan_engineering_consultants_qualification", "verified",
                       "offplan_engineering_consultants_qualification_verified_summary.json")
LLM = os.path.join(ROOT, "data", "offplan_engineering_consultants_qualification_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"offplan_engineering_consultants_qualification_no_decision_number_in_source", "offplan_engineering_consultants_qualification_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "14/3/1445":
        e.append("[2j] gazette_publication_date_hijri must be 14/3/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-09-29":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-09-29")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "شروط ومتطلبات تأهيل الجهات الاستشارية الهندسية لمشاريع بيع أو تأجير الوحدات العقارية على الخارطة":
    ...
    if e:
        print("FAIL: %d error(s) in Qualification Conditions and Requirements for Engineering Consultancies on Off-Plan Projects track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Qualification Conditions and Requirements for Engineering Consultancies on Off-Plan Projects")
