#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مركز الإحالات الطبية track.
...
RECORDS = os.path.join(ROOT, "sources", "medical_referrals_center_statute", "verified",
                       "medical_referrals_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "medical_referrals_center_statute", "verified",
                       "medical_referrals_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "medical_referrals_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"medical_referrals_center_statute_no_decision_number_in_source", "medical_referrals_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "7/2/1447":
        e.append("[2j] gazette_publication_date_hijri must be 7/2/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-08-01":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-08-01")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم مركز الإحالات الطبية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Medical Referrals Center track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Medical Referrals Center")
