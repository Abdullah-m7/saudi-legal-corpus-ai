#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم المؤسسة العامة للري track.
...
RECORDS = os.path.join(ROOT, "sources", "general_irrigation_corporation_statute", "verified",
                       "general_irrigation_corporation_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "general_irrigation_corporation_statute", "verified",
                       "general_irrigation_corporation_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "general_irrigation_corporation_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"general_irrigation_corporation_statute_no_decision_number_in_source", "general_irrigation_corporation_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم المؤسسة العامة للري":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the General Irrigation Corporation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the General Irrigation Corporation")
