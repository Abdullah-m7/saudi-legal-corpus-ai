...
# -*- coding: utf-8 -*-
"""
Render Book Four — Section 1 (provisions) outputs.

Writes:
  content/ar/book4_section1.md
  content/zh/book4_section1.md
  content/bilingual/book4_section1_bilingual.md
  dist/book4_section1.html   (clearly labelled: Section 1 provisions ONLY, not full Book Four)
...
MD_AR = os.path.join(ROOT, "content", "ar", "book4_section1.md")
MD_ZH = os.path.join(ROOT, "content", "zh", "book4_section1.md")
MD_BI = os.path.join(ROOT, "content", "bilingual", "book4_section1_bilingual.md")
HTML_OUT = os.path.join(ROOT, "dist", "book4_section1.html")

SECTION_LABEL_AR = "الباب الرابع — القسم 1: التأسيس ورأس المال (المواد 58–66) — أحكام مختارة فقط"
SECTION_LABEL_ZH = "第四编 — 第一节：设立与资本（第58–66条）— 仅择要条款（非全节全文）"
INCOMPLETE_BANNER = ("⚠ 仅为第四编第一节的择要条款（源文件明确涵盖的第58、59、60、66条）；"
                     "并非完整的第四编，第61–65条未涵盖，仍为 needs_official_text_check。 / "
                     "أحكام مختارة من القسم الأول فقط (المواد المغطاة صراحةً: 58، 59، 60، 66)؛ "
                     "ليست الباب الرابع كاملاً — المواد 61–65 غير مغطاة (needs_official_text_check).")
...
def render_markdown(doc, spec):
    ...
    ar = [f"# {SECTION_LABEL_AR}\n",
          ...
          f"\n**نموذج 1b (أحكام):** أحكام لمواد مغطاة صراحةً فقط ({arts}). المواد 61–65 غير مغطاة.\n"]
    zh = [f"# {SECTION_LABEL_ZH}\n",
          f"> {INCOMPLETE_BANNER}\n",
          f"\n**免责声明：** {spec.disclaimer_zh}\n",
          f"\n**模型 1b（专题条款）：** 仅涵盖源文件明确的条款（{arts}）。第61–65条未涵盖。\n"]
    ...
    for p in provs:
        ...
        zh.append(f"\n## 第{'、'.join(str(a) for a in p['source_article_numbers'])}条 — {p['provision_title_zh']}\n")
        zh.append(p["chinese_translation"] + "\n")
        bi.append(f"\n## المادة {anums} / 第{'、'.join(str(a) for a in p['source_article_numbers'])}条 — "
...
def render_html(doc, spec):
    ...
    for p in provs:
        ...
        parts.append("</div>")
    parts.append('<footer><p>HTML = searchable/copyable Section-1 provisions view · '
