...
# -*- coding: utf-8 -*-
"""
Render Book Four — Section 3 (provisions) outputs.

Writes:
  content/ar/book4_section3.md
  content/zh/book4_section3.md
  content/bilingual/book4_section3_bilingual.md
  dist/book4_section3.html   (clearly labelled: Section 3 provisions ONLY, not full Book Four)
...
MD_AR = os.path.join(ROOT, "content", "ar", "book4_section3.md")
MD_ZH = os.path.join(ROOT, "content", "zh", "book4_section3.md")
MD_BI = os.path.join(ROOT, "content", "bilingual", "book4_section3_bilingual.md")
HTML_OUT = os.path.join(ROOT, "dist", "book4_section3.html")

SECTION_LABEL_AR = "الباب الرابع — القسم 3: الجمعية العامة (المواد 84–102) — أحكام مختارة فقط"
SECTION_LABEL_ZH = "第四编 — 第三节：股东大会（第84–102条）— 仅择要条款（非全节全文）"
INCOMPLETE_BANNER = ("⚠ 仅为第四编第三节的择要条款（经与源文件核对后明确涵盖的第85、87、92、93、99、101、102条）；"
                     "并非完整的第四编，第84、89、100条及第86、88、90、91、94–98条未涵盖，仍为 needs_official_text_check。 / "
                     "أحكام مختارة من القسم الثالث فقط (المواد المغطاة صراحةً بعد المطابقة: 85، 87، 92، 93، 99، 101، 102)؛ "
                     "ليست الباب الرابع كاملاً — المواد 84، 89، 100 وغيرها غير مغطاة (needs_official_text_check).")
...
def render_markdown(doc, spec):
    ...
    ar = [f"# {SECTION_LABEL_AR}\n",
          ...
          f"\n**نموذج 1b (أحكام):** أحكام لمواد مغطاة صراحةً فقط ({arts}). باقي مواد القسم غير مغطاة.\n"]
    zh = [f"# {SECTION_LABEL_ZH}\n",
          f"> {INCOMPLETE_BANNER}\n",
          f"\n**免责声明：** {spec.disclaimer_zh}\n",
          f"\n**模型 1b（专题条款）：** 仅涵盖源文件明确的条款（{arts}）。本节其余条款未涵盖。\n"]
...
def render_html(doc, spec):
    ...
    for p in provs:
        ...
        parts.append("</div>")
    parts.append('<footer><p>HTML = searchable/copyable Section-3 provisions view · '
