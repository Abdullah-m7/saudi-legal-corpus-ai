#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the التنظيم الخاص بالعسكريين في هيئة الرقابة ومكافحة الفساد track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23129),
published 1/12/1444H (2023-06-19). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "nazaha_military_personnel_statute", "official_source",
                   "nazaha_military_personnel_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "nazaha_military_personnel_statute", "verified")
RECORDS = os.path.join(OUT_VER, "nazaha_military_personnel_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "nazaha_military_personnel_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "nazaha_military_personnel_statute_arabic_legal_llm",
                        "nazaha_military_personnel_statute_legal_llm_001_012.json")

LAW_ID = "sa-nazaha-military-personnel-statute"
LAW_AR = "التنظيم الخاص بالعسكريين في هيئة الرقابة ومكافحة الفساد"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"nazaha_military_personnel_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23129), published 1/12/1444H (2023-06-19). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "nazaha_military_personnel_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "NAZAHA_MILITARY_PERSONNEL_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "nazaha-military-personnel-statute-llm-%03d" % idx,
            ...
            "article_path": "nazaha_military_personnel_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Provisions Governing Military Personnel at the Oversight and Anti-Corruption Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/12/1444H / 2023-06-19)"),
                "source_authority_ar": ("التنظيم الخاص بالعسكريين في هيئة الرقابة ومكافحة الفساد — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 1/12/1444هـ الموافق 2023-06-19م)"),
    ...
    json.dump({
        "law_key": "nazaha_military_personnel_statute",
        "layer": "NAZAHA_MILITARY_PERSONNEL_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "nazaha-military-personnel-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Provisions Governing Military Personnel at the Oversight and Anti-Corruption Authority — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Provisions Governing Military Personnel at the Oversight and Anti-Corruption Authority records" % (len(ver), len(llm)))
