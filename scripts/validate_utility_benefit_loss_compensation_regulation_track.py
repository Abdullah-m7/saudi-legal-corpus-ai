#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الضوابط الخاصة بتحديد تكاليف الإصلاح وتقدير التعويضات عن المنفعة التي فقدها المرفق track.

10 records: 10 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "utility_benefit_loss_compensation_regulation", "verified",
                       "utility_benefit_loss_compensation_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "utility_benefit_loss_compensation_regulation", "verified",
                       "utility_benefit_loss_compensation_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "utility_benefit_loss_compensation_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"utility_benefit_loss_compensation_regulation_no_decision_number_in_source", "utility_benefit_loss_compensation_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "22/8/1443":
        e.append("[2j] gazette_publication_date_hijri must be 22/8/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-03-25":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-03-25")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة الضوابط الخاصة بتحديد تكاليف الإصلاح وتقدير التعويضات عن المنفعة التي فقدها المرفق":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation of Controls for Determining Repair Costs and Estimating Compensation for Benefit Lost by the Utility track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation of Controls for Determining Repair Costs and Estimating Compensation for Benefit Lost by the Utility")
    print("  - 10 records: 10 articles, no appendices; all 10 اصلية")
