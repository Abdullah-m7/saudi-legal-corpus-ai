#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية للاتفاقية الدولية بشأن ضبط النظم السفينية المقاومة لالتصاق الشوائب ذات الآثار المؤذية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26868),
published 26/6/1446H (2024-12-27). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "antifouling_systems_regulation", "official_source",
                   "antifouling_systems_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "antifouling_systems_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "antifouling_systems_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "antifouling_systems_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "antifouling_systems_regulation_arabic_legal_llm",
                        "antifouling_systems_regulation_legal_llm_001_020.json")

LAW_ID = "sa-antifouling-systems-regulation"
LAW_AR = "اللائحة التنفيذية للاتفاقية الدولية بشأن ضبط النظم السفينية المقاومة لالتصاق الشوائب ذات الآثار المؤذية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"antifouling_systems_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26868), published 26/6/1446H (2024-12-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "antifouling_systems_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "ANTIFOULING_SYSTEMS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "antifouling-systems-regulation-llm-%03d" % idx,
            ...
            "article_path": "antifouling_systems_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the International Convention on the Control of Harmful Anti-Fouling Systems on Ships -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 26/6/1446H / 2024-12-27)"),
                "source_authority_ar": ("اللائحة التنفيذية للاتفاقية الدولية بشأن ضبط النظم السفينية المقاومة لالتصاق الشوائب ذات الآثار المؤذية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 26/6/1446هـ الموافق 2024-12-27م)"),
    ...
    json.dump({
        "law_key": "antifouling_systems_regulation",
        "layer": "ANTIFOULING_SYSTEMS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "antifouling-systems-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the International Convention on the Control of Harmful Anti-Fouling Systems on Ships — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the International Convention on the Control of Harmful Anti-Fouling Systems on Ships records" % (len(ver), len(llm)))
