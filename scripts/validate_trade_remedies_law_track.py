#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام المعالجات التجارية في التجارة الدولية track.
...
RECORDS = os.path.join(ROOT, "sources", "trade_remedies_law", "verified",
                       "trade_remedies_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "trade_remedies_law", "verified",
                       "trade_remedies_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "trade_remedies_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"trade_remedies_law_no_decision_number_in_source", "trade_remedies_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام المعالجات التجارية في التجارة الدولية":
    ...
    if e:
        print("FAIL: %d error(s) in Law of Trade Remedies in International Trade track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law of Trade Remedies in International Trade")
