#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الإجراءات والتعليمات الخاصة بالشركات المدرجة أسهمها في السوق التي بلغت خسائرها المتراكمة 20% فأكثر من رأس مالها track.

6 records: 6 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "listed_companies_accumulated_losses_instructions", "verified",
                       "listed_companies_accumulated_losses_instructions_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "listed_companies_accumulated_losses_instructions", "verified",
                       "listed_companies_accumulated_losses_instructions_verified_summary.json")
LLM = os.path.join(ROOT, "data", "listed_companies_accumulated_losses_instructions_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"listed_companies_accumulated_losses_instructions_no_decision_number_in_source", "listed_companies_accumulated_losses_instructions_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "23/9/1444":
        e.append("[2j] gazette_publication_date_hijri must be 23/9/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-04-14":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-04-14")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الإجراءات والتعليمات الخاصة بالشركات المدرجة أسهمها في السوق التي بلغت خسائرها المتراكمة 20% فأكثر من رأس مالها":
    ...
    if e:
        print("FAIL: %d error(s) in Procedures and Instructions for Listed Companies Whose Accumulated Losses Reach 20 Percent or More of Capital track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Procedures and Instructions for Listed Companies Whose Accumulated Losses Reach 20 Percent or More of Capital")
    print("  - 6 records: 6 articles, no appendices; all 6 اصلية")
