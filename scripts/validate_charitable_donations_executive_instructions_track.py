#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the التعليمات التنفيذية للائحة جمع التبرعات للوجوه الخيرية track.
...
RECORDS = os.path.join(ROOT, "sources", "charitable_donations_executive_instructions", "verified",
                       "charitable_donations_executive_instructions_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "charitable_donations_executive_instructions", "verified",
                       "charitable_donations_executive_instructions_verified_summary.json")
LLM = os.path.join(ROOT, "data", "charitable_donations_executive_instructions_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"charitable_donations_executive_instructions_no_decision_number_in_source", "charitable_donations_executive_instructions_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "8/10/1444":
        e.append("[2j] gazette_publication_date_hijri must be 8/10/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-04-28":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-04-28")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "التعليمات التنفيذية للائحة جمع التبرعات للوجوه الخيرية":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Instructions for the Charitable Donations Collection Regulation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Instructions for the Charitable Donations Collection Regulation")
