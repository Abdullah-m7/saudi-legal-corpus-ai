#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة ضبط المخالفات وإيقاع العقوبات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4000460),
...
SRC = os.path.join(ROOT, "sources", "violations_penalties_regulation", "official_source",
                   "violations_penalties_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "violations_penalties_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "violations_penalties_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "violations_penalties_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "violations_penalties_regulation_arabic_legal_llm",
                        "violations_penalties_regulation_legal_llm_001_018.json")

LAW_ID = "sa-violations-penalties-regulation"
LAW_AR = "لائحة ضبط المخالفات وإيقاع العقوبات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"violations_penalties_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4000460), published 10/9/1447H (2026-02-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "violations_penalties_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "VIOLATIONS_PENALTIES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "violations-penalties-regulation-llm-%03d" % idx,
            ...
            "article_path": "violations_penalties_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Recording Violations and Imposing Penalties -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 10/9/1447H / 2026-02-27)"),
                "source_authority_ar": ("لائحة ضبط المخالفات وإيقاع العقوبات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "violations_penalties_regulation",
        "layer": "VIOLATIONS_PENALTIES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "violations-penalties-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Regulation on Recording Violations and Imposing Penalties — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Recording Violations and Imposing Penalties records" % (len(ver), len(llm)))
