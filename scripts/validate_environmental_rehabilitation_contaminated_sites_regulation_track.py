#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لإعادة التأهيل البيئي للمواقع المتدهورة ومعالجة المواقع الملوثة track.

8 records: 8 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "environmental_rehabilitation_contaminated_sites_regulation", "verified",
                       "environmental_rehabilitation_contaminated_sites_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "environmental_rehabilitation_contaminated_sites_regulation", "verified",
                       "environmental_rehabilitation_contaminated_sites_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "environmental_rehabilitation_contaminated_sites_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"environmental_rehabilitation_contaminated_sites_regulation_no_decision_number_in_source", "environmental_rehabilitation_contaminated_sites_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("gazette_publication_date_gregorian") != "2021-06-25":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-06-25")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنفيذية لإعادة التأهيل البيئي للمواقع المتدهورة ومعالجة المواقع الملوثة":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation for the Environmental Rehabilitation of Degraded Sites and the Treatment of Contaminated Sites track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation for the Environmental Rehabilitation of Degraded Sites and the Treatment of Contaminated Sites")
    print("  - 8 records: 8 articles, no appendices; all 8 اصلية")
