#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المركز الوطني لكفاءة وترشيد المياه track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17200),
published 30/6/1442H (2021-02-12). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "water_efficiency_center_statute", "official_source",
                   "water_efficiency_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "water_efficiency_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "water_efficiency_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "water_efficiency_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "water_efficiency_center_statute_arabic_legal_llm",
                        "water_efficiency_center_statute_legal_llm_001_014.json")

LAW_ID = "sa-water-efficiency-center-statute"
LAW_AR = "تنظيم المركز الوطني لكفاءة وترشيد المياه"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"water_efficiency_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17200), published 30/6/1442H (2021-02-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "water_efficiency_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "WATER_EFFICIENCY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "water-efficiency-center-statute-llm-%03d" % idx,
            ...
            "article_path": "water_efficiency_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Center for Water Efficiency and Rationalisation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 30/6/1442H / 2021-02-12)"),
                "source_authority_ar": ("تنظيم المركز الوطني لكفاءة وترشيد المياه — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 30/6/1442هـ الموافق 2021-02-12م)"),
    ...
    json.dump({
        "law_key": "water_efficiency_center_statute",
        "layer": "WATER_EFFICIENCY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "water-efficiency-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Center for Water Efficiency and Rationalisation — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Center for Water Efficiency and Rationalisation records" % (len(ver), len(llm)))
