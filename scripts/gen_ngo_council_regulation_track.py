#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة مجلس المؤسسات الأهلية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21366),
...
STRUCTURE: 39 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "ngo_council_regulation", "official_source",
                   "ngo_council_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "ngo_council_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "ngo_council_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "ngo_council_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "ngo_council_regulation_arabic_legal_llm",
                        "ngo_council_regulation_legal_llm_001_039.json")

LAW_ID = "sa-ngo-council-regulation"
LAW_AR = "لائحة مجلس المؤسسات الأهلية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"ngo_council_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21366), published 12/7/1444H (2023-02-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "ngo_council_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "NGO_COUNCIL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "ngo-council-regulation-llm-%03d" % idx,
            ...
            "article_path": "ngo_council_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation of the Council of Non-Profit Foundations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 12/7/1444H / 2023-02-03)"),
                "source_authority_ar": ("لائحة مجلس المؤسسات الأهلية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "ngo_council_regulation",
        "layer": "NGO_COUNCIL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "ngo-council-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (39 مادة، جميعها أصلية)",
        "title_en": "Regulation of the Council of Non-Profit Foundations — Arabic LLM-ready layer (39 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation of the Council of Non-Profit Foundations records" % (len(ver), len(llm)))
