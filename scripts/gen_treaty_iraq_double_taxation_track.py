#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية بين حكومة المملكة وحكومة جمهورية العراق لتجنب الازدواج الضريبي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18425),
...
SRC = os.path.join(ROOT, "sources", "treaty_iraq_double_taxation", "official_source",
                   "treaty_iraq_double_taxation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_iraq_double_taxation", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_iraq_double_taxation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_iraq_double_taxation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_iraq_double_taxation_arabic_legal_llm",
                        "treaty_iraq_double_taxation_legal_llm_001_031.json")

LAW_ID = "sa-treaty-iraq-double-taxation"
LAW_AR = "اتفاقية بين حكومة المملكة وحكومة جمهورية العراق لتجنب الازدواج الضريبي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_iraq_double_taxation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18425), published 9/3/1443H (2021-10-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_iraq_double_taxation",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_IRAQ_DOUBLE_TAXATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-iraq-double-taxation-llm-%03d" % idx,
            ...
            "article_path": "treaty_iraq_double_taxation/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Arabia–Iraq Double Taxation Avoidance Agreement -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/3/1443H / 2021-10-15)"),
                "source_authority_ar": ("اتفاقية بين حكومة المملكة وحكومة جمهورية العراق لتجنب الازدواج الضريبي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_iraq_double_taxation",
        "layer": "TREATY_IRAQ_DOUBLE_TAXATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-iraq-double-taxation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (31 مادة، جميعها أصلية)",
        "title_en": "Saudi Arabia–Iraq Double Taxation Avoidance Agreement — Arabic LLM-ready layer (31 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Arabia–Iraq Double Taxation Avoidance Agreement records" % (len(ver), len(llm)))
