#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام الانضباط الوظيفي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18319),
...
SRC = os.path.join(ROOT, "sources", "job_discipline_law", "official_source",
                   "job_discipline_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "job_discipline_law", "verified")
RECORDS = os.path.join(OUT_VER, "job_discipline_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "job_discipline_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "job_discipline_law_arabic_legal_llm",
                        "job_discipline_law_legal_llm_001_025.json")

LAW_ID = "sa-job-discipline-law"
LAW_AR = "نظام الانضباط الوظيفي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"job_discipline_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18319), published 17/2/1443H (2021-09-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "job_discipline_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "JOB_DISCIPLINE_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "job-discipline-law-llm-%03d" % idx,
            ...
            "article_path": "job_discipline_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Job Discipline Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/2/1443H / 2021-09-24)"),
                "source_authority_ar": ("نظام الانضباط الوظيفي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "job_discipline_law",
        "layer": "JOB_DISCIPLINE_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "job-discipline-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (25 مادة، جميعها أصلية)",
        "title_en": "Job Discipline Law — Arabic LLM-ready layer (25 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Job Discipline Law records" % (len(ver), len(llm)))
