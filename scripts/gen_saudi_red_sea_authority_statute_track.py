#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم الهيئة السعودية للبحر الأحمر track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24011),
published 26/4/1445H (2023-11-10). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "saudi_red_sea_authority_statute", "official_source",
                   "saudi_red_sea_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "saudi_red_sea_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "saudi_red_sea_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "saudi_red_sea_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "saudi_red_sea_authority_statute_arabic_legal_llm",
                        "saudi_red_sea_authority_statute_legal_llm_001_013.json")

LAW_ID = "sa-saudi-red-sea-authority-statute"
LAW_AR = "تنظيم الهيئة السعودية للبحر الأحمر"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"saudi_red_sea_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24011), published 26/4/1445H (2023-11-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "saudi_red_sea_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "SAUDI_RED_SEA_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "saudi-red-sea-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "saudi_red_sea_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Saudi Red Sea Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 26/4/1445H / 2023-11-10)"),
                "source_authority_ar": ("تنظيم الهيئة السعودية للبحر الأحمر — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 26/4/1445هـ الموافق 2023-11-10م)"),
    ...
    json.dump({
        "law_key": "saudi_red_sea_authority_statute",
        "layer": "SAUDI_RED_SEA_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "saudi-red-sea-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Statute of the Saudi Red Sea Authority — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Saudi Red Sea Authority records" % (len(ver), len(llm)))
