#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية بين حكومة المملكة وحكومة جمهورية البرازيل بشأن تنظيم إجراءات منح مواطني البلدين تأشيرة زيارة track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_brazil_visit_visas", "verified",
                       "treaty_brazil_visit_visas_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_brazil_visit_visas", "verified",
                       "treaty_brazil_visit_visas_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_brazil_visit_visas_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_brazil_visit_visas_no_decision_number_in_source", "treaty_brazil_visit_visas_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية بين حكومة المملكة وحكومة جمهورية البرازيل بشأن تنظيم إجراءات منح مواطني البلدين تأشيرة زيارة":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Brazil Agreement on Visit Visa Procedures track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Brazil Agreement on Visit Visa Procedures")
