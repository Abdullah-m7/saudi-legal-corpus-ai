#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة المنظمة لإصدار الترخيص الملاحي ورخصة العمل track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18916),
published 25/6/1443H (2022-01-28). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "navigation_licence_work_permit_regulation", "official_source",
                   "navigation_licence_work_permit_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "navigation_licence_work_permit_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "navigation_licence_work_permit_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "navigation_licence_work_permit_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "navigation_licence_work_permit_regulation_arabic_legal_llm",
                        "navigation_licence_work_permit_regulation_legal_llm_001_018.json")

LAW_ID = "sa-navigation-licence-work-permit-regulation"
LAW_AR = "اللائحة المنظمة لإصدار الترخيص الملاحي ورخصة العمل"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"navigation_licence_work_permit_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18916), published 25/6/1443H (2022-01-28). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "navigation_licence_work_permit_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "NAVIGATION_LICENCE_WORK_PERMIT_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "navigation-licence-work-permit-regulation-llm-%03d" % idx,
            ...
            "article_path": "navigation_licence_work_permit_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Governing the Issuance of the Navigation Licence and the Work Permit -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/6/1443H / 2022-01-28)"),
                "source_authority_ar": ("اللائحة المنظمة لإصدار الترخيص الملاحي ورخصة العمل — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 25/6/1443هـ الموافق 2022-01-28م)"),
    ...
    json.dump({
        "law_key": "navigation_licence_work_permit_regulation",
        "layer": "NAVIGATION_LICENCE_WORK_PERMIT_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "navigation-licence-work-permit-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Regulation Governing the Issuance of the Navigation Licence and the Work Permit — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Governing the Issuance of the Navigation Licence and the Work Permit records" % (len(ver), len(llm)))
