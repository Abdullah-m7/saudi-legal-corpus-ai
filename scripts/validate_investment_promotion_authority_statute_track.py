#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية للهيئة السعودية لتسويق الاستثمار track.
...
RECORDS = os.path.join(ROOT, "sources", "investment_promotion_authority_statute", "verified",
                       "investment_promotion_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "investment_promotion_authority_statute", "verified",
                       "investment_promotion_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "investment_promotion_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"investment_promotion_authority_statute_no_decision_number_in_source", "investment_promotion_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية للهيئة السعودية لتسويق الاستثمار":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Investment Promotion Authority — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Investment Promotion Authority — Organisational Arrangements")
