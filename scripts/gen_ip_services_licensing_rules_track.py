...
server-rendered HTML page (uqn.gov.sa/details?p=17334),
...
SRC = os.path.join(ROOT, "sources", "ip_services_licensing_rules", "official_source",
                   "ip_services_licensing_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "ip_services_licensing_rules", "verified")
RECORDS = os.path.join(OUT_VER, "ip_services_licensing_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "ip_services_licensing_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "ip_services_licensing_rules_arabic_legal_llm",
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17334), published 9/6/1442H (2021-01-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "ip_services_licensing_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "IP_SERVICES_LICENSING_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "ip-services-licensing-rules-llm-%03d" % idx,
            ...
            "article_path": "ip_services_licensing_rules/rules/%s" % key,
    ...
    json.dump({
        "law_key": "ip_services_licensing_rules",
        "layer": "IP_SERVICES_LICENSING_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "ip-services-licensing-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (25 مادة، جميعها أصلية)",
        "title_en": "Licensing Rules for the Provision of Intellectual Property Services — Arabic LLM-ready layer (25 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Licensing Rules for the Provision of Intellectual Property Services records" % (len(ver), len(llm)))
