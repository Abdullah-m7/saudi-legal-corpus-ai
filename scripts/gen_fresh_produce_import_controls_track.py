#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the شروط وضوابط استيراد الخضار والفاكهة الطازجة للمملكة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18673),
...
SRC = os.path.join(ROOT, "sources", "fresh_produce_import_controls", "official_source",
                   "fresh_produce_import_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "fresh_produce_import_controls", "verified")
RECORDS = os.path.join(OUT_VER, "fresh_produce_import_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "fresh_produce_import_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "fresh_produce_import_controls_arabic_legal_llm",
                        "fresh_produce_import_controls_legal_llm_001_007.json")

LAW_ID = "sa-fresh-produce-import-controls"
LAW_AR = "شروط وضوابط استيراد الخضار والفاكهة الطازجة للمملكة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"fresh_produce_import_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18673), published 6/5/1443H (2021-12-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "fresh_produce_import_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "FRESH_PRODUCE_IMPORT_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "fresh-produce-import-controls-llm-%03d" % idx,
            ...
            "article_path": "fresh_produce_import_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Conditions and Controls for Importing Fresh Vegetables and Fruit -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/5/1443H / 2021-12-10)"),
                "source_authority_ar": ("شروط وضوابط استيراد الخضار والفاكهة الطازجة للمملكة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "fresh_produce_import_controls",
        "layer": "FRESH_PRODUCE_IMPORT_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "fresh-produce-import-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (7 مادة، جميعها أصلية)",
        "title_en": "Conditions and Controls for Importing Fresh Vegetables and Fruit — Arabic LLM-ready layer (7 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Conditions and Controls for Importing Fresh Vegetables and Fruit records" % (len(ver), len(llm)))
