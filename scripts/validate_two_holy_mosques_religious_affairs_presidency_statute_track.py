#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لرئاسة الشؤون الدينية بالمسجد الحرام والمسجد النبوي track.
...
RECORDS = os.path.join(ROOT, "sources", "two_holy_mosques_religious_affairs_presidency_statute", "verified",
                       "two_holy_mosques_religious_affairs_presidency_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "two_holy_mosques_religious_affairs_presidency_statute", "verified",
                       "two_holy_mosques_religious_affairs_presidency_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "two_holy_mosques_religious_affairs_presidency_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"two_holy_mosques_religious_affairs_presidency_statute_no_decision_number_in_source", "two_holy_mosques_religious_affairs_presidency_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "5/6/1446":
        e.append("[2j] gazette_publication_date_hijri must be 5/6/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-12-06":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-12-06")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية لرئاسة الشؤون الدينية بالمسجد الحرام والمسجد النبوي":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the Presidency of Religious Affairs of the Grand Mosque and the Prophet's Mosque track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the Presidency of Religious Affairs of the Grand Mosque and the Prophet's Mosque")
