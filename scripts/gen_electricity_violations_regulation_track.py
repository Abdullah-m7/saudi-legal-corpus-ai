#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة ضبط وإثبات مخالفات أحكام نظام الكهرباء والفصل فيها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23890),
...
STRUCTURE: 26 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "electricity_violations_regulation", "official_source",
                   "electricity_violations_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "electricity_violations_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "electricity_violations_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "electricity_violations_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "electricity_violations_regulation_arabic_legal_llm",
                        "electricity_violations_regulation_legal_llm_001_026.json")

LAW_ID = "sa-electricity-violations-regulation"
LAW_AR = "لائحة ضبط وإثبات مخالفات أحكام نظام الكهرباء والفصل فيها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"electricity_violations_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23890), published 5/4/1445H (2023-10-20). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "electricity_violations_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "ELECTRICITY_VIOLATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "electricity-violations-regulation-llm-%03d" % idx,
            ...
            "article_path": "electricity_violations_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Recording and Proving Violations of the Electricity Law and their Adjudication -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/4/1445H / 2023-10-20)"),
                "source_authority_ar": ("لائحة ضبط وإثبات مخالفات أحكام نظام الكهرباء والفصل فيها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "electricity_violations_regulation",
        "layer": "ELECTRICITY_VIOLATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "electricity-violations-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (26 مادة، جميعها أصلية)",
        "title_en": "Regulation on Recording and Proving Violations of the Electricity Law and their Adjudication — Arabic LLM-ready layer (26 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Recording and Proving Violations of the Electricity Law and their Adjudication records" % (len(ver), len(llm)))
