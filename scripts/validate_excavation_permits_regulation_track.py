#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة تراخيص الحفريات track.
...
RECORDS = os.path.join(ROOT, "sources", "excavation_permits_regulation", "verified",
                       "excavation_permits_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "excavation_permits_regulation", "verified",
                       "excavation_permits_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "excavation_permits_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"excavation_permits_regulation_no_decision_number_in_source", "excavation_permits_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "13/8/1445":
        e.append("[2j] gazette_publication_date_hijri must be 13/8/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-02-23":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-02-23")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة تراخيص الحفريات":
    ...
    if e:
        print("FAIL: %d error(s) in Excavation Permits Regulation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Excavation Permits Regulation")
