#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الإطار التنظيمي لمشاريع النقل العام track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19759),
...
SRC = os.path.join(ROOT, "sources", "public_transport_projects_framework", "official_source",
                   "public_transport_projects_framework_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "public_transport_projects_framework", "verified")
RECORDS = os.path.join(OUT_VER, "public_transport_projects_framework_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "public_transport_projects_framework_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "public_transport_projects_framework_arabic_legal_llm",
                        "public_transport_projects_framework_legal_llm_001_025.json")

LAW_ID = "sa-public-transport-projects-framework"
LAW_AR = "الإطار التنظيمي لمشاريع النقل العام"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"public_transport_projects_framework_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19759), published 23/12/1443H (2022-07-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "public_transport_projects_framework",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PUBLIC_TRANSPORT_PROJECTS_FRAMEWORK_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "public-transport-projects-framework-llm-%03d" % idx,
            ...
            "article_path": "public_transport_projects_framework/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Framework for Public Transport Projects -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/12/1443H / 2022-07-22)"),
                "source_authority_ar": ("الإطار التنظيمي لمشاريع النقل العام — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "public_transport_projects_framework",
        "layer": "PUBLIC_TRANSPORT_PROJECTS_FRAMEWORK_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "public-transport-projects-framework-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (25 مادة، جميعها أصلية)",
        "title_en": "Regulatory Framework for Public Transport Projects — Arabic LLM-ready layer (25 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Framework for Public Transport Projects records" % (len(ver), len(llm)))
