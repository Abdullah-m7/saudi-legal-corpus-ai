#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية لمنظم الرحلات البحرية السياحية track.
...
RECORDS = os.path.join(ROOT, "sources", "maritime_tour_operator_regulation", "verified",
                       "maritime_tour_operator_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "maritime_tour_operator_regulation", "verified",
                       "maritime_tour_operator_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "maritime_tour_operator_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"maritime_tour_operator_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية لمنظم الرحلات البحرية السياحية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for the Maritime Tour Operator track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for the Maritime Tour Operator")
