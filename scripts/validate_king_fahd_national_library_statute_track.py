#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مكتبة الملك فهد الوطنية track.
...
RECORDS = os.path.join(ROOT, "sources", "king_fahd_national_library_statute", "verified",
                       "king_fahd_national_library_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "king_fahd_national_library_statute", "verified",
                       "king_fahd_national_library_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "king_fahd_national_library_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "king_fahd_national_library_statute_found_by_walking_a_url_section_the_index_never_covered",
    "king_fahd_national_library_statute_no_decision_number_in_source",
    "king_fahd_national_library_statute_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "15/10/1447":
        e.append("[2j] gazette_publication_date_hijri must be 15/10/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-04-03":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-04-03")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم مكتبة الملك فهد الوطنية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the King Fahd National Library track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the King Fahd National Library")
