#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد والإجراءات التنفيذية للائحة التدريب الأهلي 1442ه track.
...
RECORDS = os.path.join(ROOT, "sources", "private_training_executive_rules", "verified",
                       "private_training_executive_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "private_training_executive_rules", "verified",
                       "private_training_executive_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "private_training_executive_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"private_training_executive_rules_no_decision_number_in_source", "private_training_executive_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد والإجراءات التنفيذية للائحة التدريب الأهلي 1442ه":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Rules and Procedures for the Private Training Regulation 1442H track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Rules and Procedures for the Private Training Regulation 1442H")
