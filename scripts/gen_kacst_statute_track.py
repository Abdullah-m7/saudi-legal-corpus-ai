#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مدينة الملك عبدالعزيز للعلوم والتقنية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21484),
...
SRC = os.path.join(ROOT, "sources", "kacst_statute", "official_source",
                   "kacst_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "kacst_statute", "verified")
RECORDS = os.path.join(OUT_VER, "kacst_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "kacst_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "kacst_statute_arabic_legal_llm",
                        "kacst_statute_legal_llm_001_014.json")

LAW_ID = "sa-kacst-statute"
LAW_AR = "تنظيم مدينة الملك عبدالعزيز للعلوم والتقنية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"kacst_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21484), published 4/8/1444H (2023-02-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "kacst_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "KACST_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "kacst-statute-llm-%03d" % idx,
            ...
            "article_path": "kacst_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the King Abdulaziz City for Science and Technology -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/8/1444H / 2023-02-24)"),
                "source_authority_ar": ("تنظيم مدينة الملك عبدالعزيز للعلوم والتقنية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "kacst_statute",
        "layer": "KACST_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "kacst-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Statute of the King Abdulaziz City for Science and Technology — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the King Abdulaziz City for Science and Technology records" % (len(ver), len(llm)))
