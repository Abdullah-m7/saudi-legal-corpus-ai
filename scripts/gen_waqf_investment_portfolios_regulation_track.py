#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة إنشاء المحافظ الاستثمارية الوقفية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20893),
...
SRC = os.path.join(ROOT, "sources", "waqf_investment_portfolios_regulation", "official_source",
                   "waqf_investment_portfolios_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "waqf_investment_portfolios_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "waqf_investment_portfolios_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "waqf_investment_portfolios_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "waqf_investment_portfolios_regulation_arabic_legal_llm",
                        "waqf_investment_portfolios_regulation_legal_llm_001_013.json")

LAW_ID = "sa-waqf-investment-portfolios-regulation"
LAW_AR = "لائحة إنشاء المحافظ الاستثمارية الوقفية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"waqf_investment_portfolios_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20893), published 24/4/1444H (2022-11-18). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "waqf_investment_portfolios_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "WAQF_INVESTMENT_PORTFOLIOS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "waqf-investment-portfolios-regulation-llm-%03d" % idx,
            ...
            "article_path": "waqf_investment_portfolios_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on the Establishment of Waqf Investment Portfolios -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/4/1444H / 2022-11-18)"),
                "source_authority_ar": ("لائحة إنشاء المحافظ الاستثمارية الوقفية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "waqf_investment_portfolios_regulation",
        "layer": "WAQF_INVESTMENT_PORTFOLIOS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "waqf-investment-portfolios-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Regulation on the Establishment of Waqf Investment Portfolios — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on the Establishment of Waqf Investment Portfolios records" % (len(ver), len(llm)))
