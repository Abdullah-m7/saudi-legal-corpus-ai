#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة مرافق الإيواء السياحي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17128),
...
SRC = os.path.join(ROOT, "sources", "tourist_accommodation_facilities_regulation", "official_source",
                   "tourist_accommodation_facilities_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "tourist_accommodation_facilities_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "tourist_accommodation_facilities_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "tourist_accommodation_facilities_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "tourist_accommodation_facilities_regulation_arabic_legal_llm",
                        "tourist_accommodation_facilities_regulation_legal_llm_001_040.json")

LAW_ID = "sa-tourist-accommodation-facilities-regulation"
LAW_AR = "لائحة مرافق الإيواء السياحي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"tourist_accommodation_facilities_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17128), published 28/7/1442H (2021-03-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "tourist_accommodation_facilities_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "TOURIST_ACCOMMODATION_FACILITIES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "tourist-accommodation-facilities-regulation-llm-%03d" % idx,
            ...
            "article_path": "tourist_accommodation_facilities_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Tourist Accommodation Facilities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/7/1442H / 2021-03-12)"),
                "source_authority_ar": ("لائحة مرافق الإيواء السياحي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "tourist_accommodation_facilities_regulation",
        "layer": "TOURIST_ACCOMMODATION_FACILITIES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "tourist-accommodation-facilities-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (40 مادة، جميعها أصلية)",
        "title_en": "Regulation on Tourist Accommodation Facilities — Arabic LLM-ready layer (40 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Tourist Accommodation Facilities records" % (len(ver), len(llm)))
