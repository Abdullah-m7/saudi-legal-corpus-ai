#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية لاهاي بشأن إلغاء إلزامية المصادقة على الوثائق العمومية الأجنبية «أبوستيل» track.
...
RECORDS = os.path.join(ROOT, "sources", "hague_apostille_convention", "verified",
                       "hague_apostille_convention_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "hague_apostille_convention", "verified",
                       "hague_apostille_convention_verified_summary.json")
LLM = os.path.join(ROOT, "data", "hague_apostille_convention_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"hague_apostille_convention_no_decision_number_in_source", "hague_apostille_convention_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "4/6/1443":
        e.append("[2j] gazette_publication_date_hijri must be 4/6/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-01-07":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-01-07")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية لاهاي بشأن إلغاء إلزامية المصادقة على الوثائق العمومية الأجنبية «أبوستيل»":
    ...
    if e:
        print("FAIL: %d error(s) in Hague Convention Abolishing the Requirement of Legalisation for Foreign Public Documents track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Hague Convention Abolishing the Requirement of Legalisation for Foreign Public Documents")
