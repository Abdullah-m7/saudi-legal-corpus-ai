#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد وإجراءات عمل لجان الفصل في المنازعات والمخالفات التأمينية track.

46 records: 46 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "insurance_disputes_committees_rules", "verified",
                       "insurance_disputes_committees_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "insurance_disputes_committees_rules", "verified",
                       "insurance_disputes_committees_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "insurance_disputes_committees_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"insurance_disputes_committees_rules_no_decision_number_in_source", "insurance_disputes_committees_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if sorted(nums) != list(range(1, N_ARTICLES + 1)):
        e.append("[1] article numbers not 1..%d" % N_ARTICLES)
    ...
    if cov != set(range(1, N_ARTICLES + 1)):
        e.append("[2c] chapter_structure coverage != 1..%d" % N_ARTICLES)
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "19/12/1447":
        e.append("[2j] gazette_publication_date_hijri must be 19/12/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-06-05":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-06-05")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد وإجراءات عمل لجان الفصل في المنازعات والمخالفات التأمينية":
    ...
    if e:
        print("FAIL: %d error(s) in Rules and Procedures of the Committees for Resolution of Insurance Disputes and Violations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules and Procedures of the Committees for Resolution of Insurance Disputes and Violations")
    print("  - 46 records: 46 articles, no appendices; all 46 اصلية")
