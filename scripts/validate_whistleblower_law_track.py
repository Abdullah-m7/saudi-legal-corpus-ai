#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Whistleblower, Witness, Expert and Victim Protection Law track (37 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) EXCEPT the 1 documented article (26) whose PDF text-layer
...
position 1..37 (no مكرر); and every article carries an explained legal_status
...
ISSUANCE: all 37 اصلية; none amended, repealed or added. Tatweel is banned EXCEPT
...
RECORDS = os.path.join(ROOT, "sources", "whistleblower", "law", "verified",
                       "whistleblower_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "whistleblower_arabic_legal_llm",
                   "whistleblower_law_legal_llm_001_037.json")
PDF = os.path.join(ROOT, "inputs", "whistleblower_official_pdfs",
                   "whistleblower_law_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 37
KEY_RE = r"whistleblower_art_(\d{3})$"
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 37}
VISUALLY_ADJUDICATED = {"whistleblower_art_026"}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Whistleblower Law track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Whistleblower Law — 37 records (fresh issuance: all 37 اصلية)")
    print("  - trust gate: 36/37 MATCHES_PDF outright (mean 0.9582, min 0.741); article 26 adjudicated visually verbatim")
    print("  - numbered 1..37 by ordinal position (no مكرر); no dual-status divergence")
    print("  - IN-FORCE Royal Decree M/148 (8/8/1445H); committed MOJ PDF hash verified; Arabic governs")
    return 0
