#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة لإصدار شهادات الاعتماد لجهات التحكم وجهات المعالجة track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/.../4000533),
...
SRC = os.path.join(ROOT, "sources", "pdpl_accreditation_certificates_rules", "official_source",
                   "pdpl_accreditation_certificates_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "pdpl_accreditation_certificates_rules", "verified")
RECORDS = os.path.join(OUT_VER, "pdpl_accreditation_certificates_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "pdpl_accreditation_certificates_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "pdpl_accreditation_certificates_rules_arabic_legal_llm",
                        "pdpl_accreditation_certificates_rules_legal_llm_001_015.json")

LAW_ID = "sa-pdpl-accreditation-certificates-rules"
LAW_AR = "القواعد المنظمة لإصدار شهادات الاعتماد لجهات التحكم وجهات المعالجة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"pdpl_accreditation_certificates_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/.../4000533), published 17/9/1447H (2026-03-06). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "pdpl_accreditation_certificates_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PDPL_ACCREDITATION_CERTIFICATES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "pdpl-accreditation-certificates-rules-llm-%03d" % idx,
            ...
            "article_path": "pdpl_accreditation_certificates_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules for Issuing Accreditation Certificates to Controllers and Processors -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/9/1447H / 2026-03-06)"),
                "source_authority_ar": ("القواعد المنظمة لإصدار شهادات الاعتماد لجهات التحكم وجهات المعالجة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "pdpl_accreditation_certificates_rules",
        "layer": "PDPL_ACCREDITATION_CERTIFICATES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "pdpl-accreditation-certificates-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Rules for Issuing Accreditation Certificates to Controllers and Processors — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules for Issuing Accreditation Certificates to Controllers and Processors records" % (len(ver), len(llm)))
