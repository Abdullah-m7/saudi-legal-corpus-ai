#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة المقاصة النهائية وترتيبات الضمان المرتبطة بها track.
...
RECORDS = os.path.join(ROOT, "sources", "final_clearing_collateral_regulation", "verified",
                       "final_clearing_collateral_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "final_clearing_collateral_regulation", "verified",
                       "final_clearing_collateral_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "final_clearing_collateral_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"final_clearing_collateral_regulation_no_decision_number_in_source", "final_clearing_collateral_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "13/3/1447":
        e.append("[2j] gazette_publication_date_hijri must be 13/3/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-09-05":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-09-05")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة المقاصة النهائية وترتيبات الضمان المرتبطة بها":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on Final Clearing and its Associated Collateral Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on Final Clearing and its Associated Collateral Arrangements")
