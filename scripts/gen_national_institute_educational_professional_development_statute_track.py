#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المعهد الوطني للتطوير المهني التعليمي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24647),
published 27/8/1445H (2024-03-08). The Gazette IS the official publication of record
...
STRUCTURE: 17 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "national_institute_educational_professional_development_statute", "official_source",
                   "national_institute_educational_professional_development_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "national_institute_educational_professional_development_statute", "verified")
RECORDS = os.path.join(OUT_VER, "national_institute_educational_professional_development_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "national_institute_educational_professional_development_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "national_institute_educational_professional_development_statute_arabic_legal_llm",
                        "national_institute_educational_professional_development_statute_legal_llm_001_017.json")

LAW_ID = "sa-national-institute-educational-professional-development-statute"
LAW_AR = "تنظيم المعهد الوطني للتطوير المهني التعليمي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"national_institute_educational_professional_development_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24647), published 27/8/1445H (2024-03-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "national_institute_educational_professional_development_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "NATIONAL_INSTITUTE_EDUCATIONAL_PROFESSIONAL_DEVELOPMENT_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "national-institute-educational-professional-development-statute-llm-%03d" % idx,
            ...
            "article_path": "national_institute_educational_professional_development_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Institute for Educational Professional Development -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/8/1445H / 2024-03-08)"),
                "source_authority_ar": ("تنظيم المعهد الوطني للتطوير المهني التعليمي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 27/8/1445هـ الموافق 2024-03-08م)"),
    ...
    json.dump({
        "law_key": "national_institute_educational_professional_development_statute",
        "layer": "NATIONAL_INSTITUTE_EDUCATIONAL_PROFESSIONAL_DEVELOPMENT_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "national-institute-educational-professional-development-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Institute for Educational Professional Development — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Institute for Educational Professional Development records" % (len(ver), len(llm)))
