#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد التنظيمية للصناديق العائلية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21362),
...
OUT_VER = os.path.join(ROOT, "sources", "family_funds_rules", "verified")
RECORDS = os.path.join(OUT_VER, "family_funds_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "family_funds_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "family_funds_rules_arabic_legal_llm",
                        "family_funds_rules_legal_llm_001_036.json")

LAW_ID = "sa-family-funds-rules"
LAW_AR = "القواعد التنظيمية للصناديق العائلية"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21362), published 12/7/1444H (2023-02-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "family_funds_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "FAMILY_FUNDS_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "family-funds-rules-llm-%03d" % idx,
            ...
            "article_path": "family_funds_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for Family Funds -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 12/7/1444H / 2023-02-03)"),
                "source_authority_ar": ("القواعد التنظيمية للصناديق العائلية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "family_funds_rules",
        "layer": "FAMILY_FUNDS_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "family-funds-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (36 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for Family Funds — Arabic LLM-ready layer (36 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for Family Funds records" % (len(ver), len(llm)))
