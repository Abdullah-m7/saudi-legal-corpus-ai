#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة لفرع تقييم أضرار المركبات track.
...
RECORDS = os.path.join(ROOT, "sources", "vehicle_damage_assessment_rules", "verified",
                       "vehicle_damage_assessment_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "vehicle_damage_assessment_rules", "verified",
                       "vehicle_damage_assessment_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "vehicle_damage_assessment_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"vehicle_damage_assessment_rules_no_decision_number_in_source", "vehicle_damage_assessment_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "9/11/1445":
        e.append("[2j] gazette_publication_date_hijri must be 9/11/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-05-17":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-05-17")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "القواعد المنظمة لفرع تقييم أضرار المركبات":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing the Vehicle Damage Assessment Branch track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing the Vehicle Damage Assessment Branch")
