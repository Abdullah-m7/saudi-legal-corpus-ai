#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد البيع بالمزاد العلني track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25189),
...
SRC = os.path.join(ROOT, "sources", "public_auction_sale_rules", "official_source",
                   "public_auction_sale_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "public_auction_sale_rules", "verified")
RECORDS = os.path.join(OUT_VER, "public_auction_sale_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "public_auction_sale_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "public_auction_sale_rules_arabic_legal_llm",
                        "public_auction_sale_rules_legal_llm_001_018.json")

LAW_ID = "sa-public-auction-sale-rules"
LAW_AR = "قواعد البيع بالمزاد العلني"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"public_auction_sale_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25189), published 22/12/1445H (2024-06-28). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "public_auction_sale_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PUBLIC_AUCTION_SALE_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "public-auction-sale-rules-llm-%03d" % idx,
            ...
            "article_path": "public_auction_sale_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing Sale by Public Auction -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/12/1445H / 2024-06-28)"),
                "source_authority_ar": ("قواعد البيع بالمزاد العلني — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "public_auction_sale_rules",
        "layer": "PUBLIC_AUCTION_SALE_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "public-auction-sale-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Rules Governing Sale by Public Auction — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing Sale by Public Auction records" % (len(ver), len(llm)))
