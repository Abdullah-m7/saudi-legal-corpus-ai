#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة نشاط شبكة الغاز للأغراض السكنية والتجارية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4000453),
published 10/9/1447H (2026-02-27). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "residential_commercial_gas_network_regulation", "official_source",
                   "residential_commercial_gas_network_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "residential_commercial_gas_network_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "residential_commercial_gas_network_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "residential_commercial_gas_network_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "residential_commercial_gas_network_regulation_arabic_legal_llm",
                        "residential_commercial_gas_network_regulation_legal_llm_001_020.json")

LAW_ID = "sa-residential-commercial-gas-network-regulation"
LAW_AR = "لائحة نشاط شبكة الغاز للأغراض السكنية والتجارية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"residential_commercial_gas_network_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4000453), published 10/9/1447H (2026-02-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "residential_commercial_gas_network_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "RESIDENTIAL_COMMERCIAL_GAS_NETWORK_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "residential-commercial-gas-network-regulation-llm-%03d" % idx,
            ...
            "article_path": "residential_commercial_gas_network_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation of the Gas Network Activity for Residential and Commercial Purposes -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 10/9/1447H / 2026-02-27)"),
                "source_authority_ar": ("لائحة نشاط شبكة الغاز للأغراض السكنية والتجارية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 10/9/1447هـ الموافق 2026-02-27م)"),
    ...
    json.dump({
        "law_key": "residential_commercial_gas_network_regulation",
        "layer": "RESIDENTIAL_COMMERCIAL_GAS_NETWORK_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "residential-commercial-gas-network-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Regulation of the Gas Network Activity for Residential and Commercial Purposes — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation of the Gas Network Activity for Residential and Commercial Purposes records" % (len(ver), len(llm)))
