#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة للاستثمار الأجنبي في الأوراق المالية track.
...
SRC = os.path.join(ROOT, "sources", "foreign_investment_securities_rules", "official_source",
                   "foreign_investment_securities_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "foreign_investment_securities_rules", "verified")
RECORDS = os.path.join(OUT_VER, "foreign_investment_securities_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "foreign_investment_securities_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "foreign_investment_securities_rules_arabic_legal_llm",
                        "foreign_investment_securities_rules_legal_llm_001_011.json")

LAW_ID = "sa-foreign-investment-securities-rules"
LAW_AR = "القواعد المنظمة للاستثمار الأجنبي في الأوراق المالية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"foreign_investment_securities_rules_art_(\d{3})$"
...
def main():
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "foreign_investment_securities_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "FOREIGN_INVESTMENT_SECURITIES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "foreign-investment-securities-rules-llm-%03d" % idx,
            ...
            "article_path": "foreign_investment_securities_rules/rules/%s" % key,
    ...
    json.dump({
        "law_key": "foreign_investment_securities_rules",
        "layer": "FOREIGN_INVESTMENT_SECURITIES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "foreign-investment-securities-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Rules Governing Foreign Investment in Securities — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing Foreign Investment in Securities records" % (len(ver), len(llm)))
