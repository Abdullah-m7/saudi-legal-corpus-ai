#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام الأساس لمستشفى الملك فيصل التخصصي ومركز الأبحاث track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18778),
published 20/5/1443H (2021-12-24). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "king_faisal_specialist_hospital_statute", "official_source",
                   "king_faisal_specialist_hospital_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "king_faisal_specialist_hospital_statute", "verified")
RECORDS = os.path.join(OUT_VER, "king_faisal_specialist_hospital_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "king_faisal_specialist_hospital_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "king_faisal_specialist_hospital_statute_arabic_legal_llm",
                        "king_faisal_specialist_hospital_statute_legal_llm_001_021.json")

LAW_ID = "sa-king-faisal-specialist-hospital-statute"
LAW_AR = "النظام الأساس لمستشفى الملك فيصل التخصصي ومركز الأبحاث"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"king_faisal_specialist_hospital_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18778), published 20/5/1443H (2021-12-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "king_faisal_specialist_hospital_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "KING_FAISAL_SPECIALIST_HOSPITAL_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "king-faisal-specialist-hospital-statute-llm-%03d" % idx,
            ...
            "article_path": "king_faisal_specialist_hospital_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of King Faisal Specialist Hospital and Research Centre -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/5/1443H / 2021-12-24)"),
                "source_authority_ar": ("النظام الأساس لمستشفى الملك فيصل التخصصي ومركز الأبحاث — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 20/5/1443هـ الموافق 2021-12-24م)"),
    ...
    json.dump({
        "law_key": "king_faisal_specialist_hospital_statute",
        "layer": "KING_FAISAL_SPECIALIST_HOSPITAL_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "king-faisal-specialist-hospital-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (21 مادة، جميعها أصلية)",
        "title_en": "Statute of King Faisal Specialist Hospital and Research Centre — Arabic LLM-ready layer (21 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of King Faisal Specialist Hospital and Research Centre records" % (len(ver), len(llm)))
