#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم وكالة الأنباء السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "saudi_press_agency_statute", "verified",
                       "saudi_press_agency_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "saudi_press_agency_statute", "verified",
                       "saudi_press_agency_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "saudi_press_agency_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"saudi_press_agency_statute_no_decision_number_in_source", "saudi_press_agency_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم وكالة الأنباء السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Saudi Press Agency track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Saudi Press Agency")
