#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد العامة لشهادة زمالة الهيئة السعودية للمقيمين المعتمدين track.
...
RECORDS = os.path.join(ROOT, "sources", "accredited_valuers_fellowship_rules", "verified",
                       "accredited_valuers_fellowship_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "accredited_valuers_fellowship_rules", "verified",
                       "accredited_valuers_fellowship_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "accredited_valuers_fellowship_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"accredited_valuers_fellowship_rules_no_decision_number_in_source", "accredited_valuers_fellowship_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد العامة لشهادة زمالة الهيئة السعودية للمقيمين المعتمدين":
    ...
    if e:
        print("FAIL: %d error(s) in General Rules for the Fellowship Certificate of the Saudi Authority for Accredited Valuers track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: General Rules for the Fellowship Certificate of the Saudi Authority for Accredited Valuers")
