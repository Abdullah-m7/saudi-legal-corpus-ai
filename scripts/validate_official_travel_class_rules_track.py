#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد تحديد درجات إركاب الموظفين والعاملين في الأجهزة العامة من مدنيين وعسكريين track.
...
RECORDS = os.path.join(ROOT, "sources", "official_travel_class_rules", "verified",
                       "official_travel_class_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "official_travel_class_rules", "verified",
                       "official_travel_class_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "official_travel_class_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"official_travel_class_rules_no_decision_number_in_source", "official_travel_class_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد تحديد درجات إركاب الموظفين والعاملين في الأجهزة العامة من مدنيين وعسكريين":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Determining Travel Classes for Civil and Military Employees of Public Bodies track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Determining Travel Classes for Civil and Military Employees of Public Bodies")
