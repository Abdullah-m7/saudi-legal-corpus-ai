#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية بين المملكة ومنظمة السياحة العالمية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17434),
published 27/8/1442H (2021-04-09). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "treaty_unwto_cooperation", "official_source",
                   "treaty_unwto_cooperation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_unwto_cooperation", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_unwto_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_unwto_cooperation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_unwto_cooperation_arabic_legal_llm",
                        "treaty_unwto_cooperation_legal_llm_001_010.json")

LAW_ID = "sa-treaty-unwto-cooperation"
LAW_AR = "اتفاقية بين المملكة ومنظمة السياحة العالمية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_unwto_cooperation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17434), published 27/8/1442H (2021-04-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_unwto_cooperation",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_UNWTO_COOPERATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-unwto-cooperation-llm-%03d" % idx,
            ...
            "article_path": "treaty_unwto_cooperation/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Agreement between Saudi Arabia and the World Tourism Organization -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/8/1442H / 2021-04-09)"),
                "source_authority_ar": ("اتفاقية بين المملكة ومنظمة السياحة العالمية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 27/8/1442هـ الموافق 2021-04-09م)"),
    ...
    json.dump({
        "law_key": "treaty_unwto_cooperation",
        "layer": "TREATY_UNWTO_COOPERATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-unwto-cooperation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Agreement between Saudi Arabia and the World Tourism Organization — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Agreement between Saudi Arabia and the World Tourism Organization records" % (len(ver), len(llm)))
