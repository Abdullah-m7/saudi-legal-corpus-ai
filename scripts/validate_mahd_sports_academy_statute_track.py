#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لأكاديمية مهد الرياضية track.
...
RECORDS = os.path.join(ROOT, "sources", "mahd_sports_academy_statute", "verified",
                       "mahd_sports_academy_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "mahd_sports_academy_statute", "verified",
                       "mahd_sports_academy_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "mahd_sports_academy_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"mahd_sports_academy_statute_no_decision_number_in_source", "mahd_sports_academy_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية لأكاديمية مهد الرياضية":
    ...
    if e:
        print("FAIL: %d error(s) in Mahd Sports Academy — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Mahd Sports Academy — Organisational Arrangements")
