#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الاتفاقية العربية لتنظيم نقل الركاب على الطرق بين الدول العربية وعبرها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21393),
published 19/7/1444H (2023-02-10). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "treaty_arab_road_passenger_transport", "official_source",
                   "treaty_arab_road_passenger_transport_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_arab_road_passenger_transport", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_arab_road_passenger_transport_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_arab_road_passenger_transport_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_arab_road_passenger_transport_arabic_legal_llm",
                        "treaty_arab_road_passenger_transport_legal_llm_001_030.json")

LAW_ID = "sa-treaty-arab-road-passenger-transport"
LAW_AR = "الاتفاقية العربية لتنظيم نقل الركاب على الطرق بين الدول العربية وعبرها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_arab_road_passenger_transport_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21393), published 19/7/1444H (2023-02-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_arab_road_passenger_transport",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_ARAB_ROAD_PASSENGER_TRANSPORT_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-arab-road-passenger-transport-llm-%03d" % idx,
            ...
            "article_path": "treaty_arab_road_passenger_transport/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Arab Agreement Regulating Road Passenger Transport between and across Arab States -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 19/7/1444H / 2023-02-10)"),
                "source_authority_ar": ("الاتفاقية العربية لتنظيم نقل الركاب على الطرق بين الدول العربية وعبرها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 19/7/1444هـ الموافق 2023-02-10م)"),
    ...
    json.dump({
        "law_key": "treaty_arab_road_passenger_transport",
        "layer": "TREATY_ARAB_ROAD_PASSENGER_TRANSPORT_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-arab-road-passenger-transport-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (30 مادة، جميعها أصلية)",
        "title_en": "Arab Agreement Regulating Road Passenger Transport between and across Arab States — Arabic LLM-ready layer (30 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Arab Agreement Regulating Road Passenger Transport between and across Arab States records" % (len(ver), len(llm)))
