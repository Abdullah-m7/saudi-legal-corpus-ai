#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم الهيئة العليا للأمن الصناعي track.
...
RECORDS = os.path.join(ROOT, "sources", "high_industrial_security_authority_statute", "verified",
                       "high_industrial_security_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "high_industrial_security_authority_statute", "verified",
                       "high_industrial_security_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "high_industrial_security_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"high_industrial_security_authority_statute_no_decision_number_in_source", "high_industrial_security_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم الهيئة العليا للأمن الصناعي":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the High Commission for Industrial Security track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the High Commission for Industrial Security")
