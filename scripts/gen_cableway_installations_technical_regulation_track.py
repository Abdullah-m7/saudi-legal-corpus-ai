#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة الفنية لتركيبات العربات المعلقة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23539),
published 9/2/1445H (2023-08-25). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "cableway_installations_technical_regulation", "official_source",
                   "cableway_installations_technical_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "cableway_installations_technical_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "cableway_installations_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "cableway_installations_technical_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "cableway_installations_technical_regulation_arabic_legal_llm",
                        "cableway_installations_technical_regulation_legal_llm_001_015.json")

LAW_ID = "sa-cableway-installations-technical-regulation"
LAW_AR = "اللائحة الفنية لتركيبات العربات المعلقة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"cableway_installations_technical_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23539), published 9/2/1445H (2023-08-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "cableway_installations_technical_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CABLEWAY_INSTALLATIONS_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "cableway-installations-technical-regulation-llm-%03d" % idx,
            ...
            "article_path": "cableway_installations_technical_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Technical Regulation for Cableway Installations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/2/1445H / 2023-08-25)"),
                "source_authority_ar": ("اللائحة الفنية لتركيبات العربات المعلقة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 9/2/1445هـ الموافق 2023-08-25م)"),
    ...
    json.dump({
        "law_key": "cableway_installations_technical_regulation",
        "layer": "CABLEWAY_INSTALLATIONS_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "cableway-installations-technical-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Technical Regulation for Cableway Installations — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Technical Regulation for Cableway Installations records" % (len(ver), len(llm)))
