#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة حقوق والتزامات مستخدمي وسائل النقل العام track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23458),
...
SRC = os.path.join(ROOT, "sources", "public_transport_users_rights_regulation", "official_source",
                   "public_transport_users_rights_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "public_transport_users_rights_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "public_transport_users_rights_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "public_transport_users_rights_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "public_transport_users_rights_regulation_arabic_legal_llm",
                        "public_transport_users_rights_regulation_legal_llm_001_021.json")

LAW_ID = "sa-public-transport-users-rights-regulation"
LAW_AR = "لائحة حقوق والتزامات مستخدمي وسائل النقل العام"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"public_transport_users_rights_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23458), published 24/1/1445H (2023-08-11). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "public_transport_users_rights_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "PUBLIC_TRANSPORT_USERS_RIGHTS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "public-transport-users-rights-regulation-llm-%03d" % idx,
            ...
            "article_path": "public_transport_users_rights_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on the Rights and Obligations of Public Transport Users -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/1/1445H / 2023-08-11)"),
                "source_authority_ar": ("لائحة حقوق والتزامات مستخدمي وسائل النقل العام — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "public_transport_users_rights_regulation",
        "layer": "PUBLIC_TRANSPORT_USERS_RIGHTS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "public-transport-users-rights-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (21 مادة، جميعها أصلية)",
        "title_en": "Regulation on the Rights and Obligations of Public Transport Users — Arabic LLM-ready layer (21 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on the Rights and Obligations of Public Transport Users records" % (len(ver), len(llm)))
