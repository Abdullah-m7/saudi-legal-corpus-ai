#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط بند الكفاءات المتميزة المخصص لأغراض التحفيز في الجهات العامة track.
...
RECORDS = os.path.join(ROOT, "sources", "distinguished_competencies_incentive_controls", "verified",
                       "distinguished_competencies_incentive_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "distinguished_competencies_incentive_controls", "verified",
                       "distinguished_competencies_incentive_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "distinguished_competencies_incentive_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"distinguished_competencies_incentive_controls_no_decision_number_in_source", "distinguished_competencies_incentive_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "13/7/1447":
        e.append("[2j] gazette_publication_date_hijri must be 13/7/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-01-02":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-01-02")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط بند الكفاءات المتميزة المخصص لأغراض التحفيز في الجهات العامة":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on the Distinguished Competencies Item Allocated for Incentives in Public Bodies track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on the Distinguished Competencies Item Allocated for Incentives in Public Bodies")
