#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة إجراءات الفصل في منازعات الأوراق المالية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19178),
...
STRUCTURE: 84 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "securities_disputes_rules", "official_source",
                   "securities_disputes_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "securities_disputes_rules", "verified")
RECORDS = os.path.join(OUT_VER, "securities_disputes_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "securities_disputes_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "securities_disputes_rules_arabic_legal_llm",
                        "securities_disputes_rules_legal_llm_001_084.json")

LAW_ID = "sa-securities-disputes-rules"
LAW_AR = "لائحة إجراءات الفصل في منازعات الأوراق المالية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"securities_disputes_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19178), published 22/8/1443H (2022-03-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "securities_disputes_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SECURITIES_DISPUTES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "securities-disputes-rules-llm-%03d" % idx,
            ...
            "article_path": "securities_disputes_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules of Procedure for the Resolution of Securities Disputes -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/8/1443H / 2022-03-25)"),
                "source_authority_ar": ("لائحة إجراءات الفصل في منازعات الأوراق المالية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "securities_disputes_rules",
        "layer": "SECURITIES_DISPUTES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "securities-disputes-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (84 مادة، جميعها أصلية)",
        "title_en": "Rules of Procedure for the Resolution of Securities Disputes — Arabic LLM-ready layer (84 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules of Procedure for the Resolution of Securities Disputes records" % (len(ver), len(llm)))
