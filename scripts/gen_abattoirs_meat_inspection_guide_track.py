#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the دليل المسالخ وفحص اللحوم ونقلها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24683),
published 5/9/1445H (2024-03-15). The Gazette IS the official publication of record
...
STRUCTURE: 70 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "abattoirs_meat_inspection_guide", "official_source",
                   "abattoirs_meat_inspection_guide_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "abattoirs_meat_inspection_guide", "verified")
RECORDS = os.path.join(OUT_VER, "abattoirs_meat_inspection_guide_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "abattoirs_meat_inspection_guide_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "abattoirs_meat_inspection_guide_arabic_legal_llm",
                        "abattoirs_meat_inspection_guide_legal_llm_001_070.json")

LAW_ID = "sa-abattoirs-meat-inspection-guide"
LAW_AR = "دليل المسالخ وفحص اللحوم ونقلها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"abattoirs_meat_inspection_guide_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24683), published 5/9/1445H (2024-03-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "abattoirs_meat_inspection_guide",
            "law_component": "guide",
            "language": "ar",
            "record_layer": "ABATTOIRS_MEAT_INSPECTION_GUIDE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "abattoirs-meat-inspection-guide-llm-%03d" % idx,
            ...
            "article_path": "abattoirs_meat_inspection_guide/guide/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Guide to Abattoirs, Meat Inspection and Transport -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/9/1445H / 2024-03-15)"),
                "source_authority_ar": ("دليل المسالخ وفحص اللحوم ونقلها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 5/9/1445هـ الموافق 2024-03-15م)"),
    ...
    json.dump({
        "law_key": "abattoirs_meat_inspection_guide",
        "layer": "ABATTOIRS_MEAT_INSPECTION_GUIDE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "abattoirs-meat-inspection-guide-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "guide",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (70 مادة، جميعها أصلية)",
        "title_en": "Guide to Abattoirs, Meat Inspection and Transport — Arabic LLM-ready layer (70 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Guide to Abattoirs, Meat Inspection and Transport records" % (len(ver), len(llm)))
