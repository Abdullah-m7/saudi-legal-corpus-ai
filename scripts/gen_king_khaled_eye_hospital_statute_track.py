#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام الأساس لمستشفى الملك خالد التخصصي للعيون ومركز الأبحاث track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26710),
published 29/4/1446H (2024-11-01). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "king_khaled_eye_hospital_statute", "official_source",
                   "king_khaled_eye_hospital_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "king_khaled_eye_hospital_statute", "verified")
RECORDS = os.path.join(OUT_VER, "king_khaled_eye_hospital_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "king_khaled_eye_hospital_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "king_khaled_eye_hospital_statute_arabic_legal_llm",
                        "king_khaled_eye_hospital_statute_legal_llm_001_021.json")

LAW_ID = "sa-king-khaled-eye-hospital-statute"
LAW_AR = "النظام الأساس لمستشفى الملك خالد التخصصي للعيون ومركز الأبحاث"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"king_khaled_eye_hospital_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26710), published 29/4/1446H (2024-11-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "king_khaled_eye_hospital_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "KING_KHALED_EYE_HOSPITAL_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "king-khaled-eye-hospital-statute-llm-%03d" % idx,
            ...
            "article_path": "king_khaled_eye_hospital_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of King Khaled Eye Specialist Hospital and Research Centre -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 29/4/1446H / 2024-11-01)"),
                "source_authority_ar": ("النظام الأساس لمستشفى الملك خالد التخصصي للعيون ومركز الأبحاث — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 29/4/1446هـ الموافق 2024-11-01م)"),
    ...
    json.dump({
        "law_key": "king_khaled_eye_hospital_statute",
        "layer": "KING_KHALED_EYE_HOSPITAL_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "king-khaled-eye-hospital-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (21 مادة، جميعها أصلية)",
        "title_en": "Statute of King Khaled Eye Specialist Hospital and Research Centre — Arabic LLM-ready layer (21 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of King Khaled Eye Specialist Hospital and Research Centre records" % (len(ver), len(llm)))
