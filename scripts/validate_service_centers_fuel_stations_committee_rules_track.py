#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد أعمال اللجنة التنفيذية الدائمة لمراكز الخدمة ومحطات الوقود track.
...
RECORDS = os.path.join(ROOT, "sources", "service_centers_fuel_stations_committee_rules", "verified",
                       "service_centers_fuel_stations_committee_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "service_centers_fuel_stations_committee_rules", "verified",
                       "service_centers_fuel_stations_committee_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "service_centers_fuel_stations_committee_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"service_centers_fuel_stations_committee_rules_no_decision_number_in_source", "service_centers_fuel_stations_committee_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد أعمال اللجنة التنفيذية الدائمة لمراكز الخدمة ومحطات الوقود":
    ...
    if e:
        print("FAIL: %d error(s) in Rules of Procedure of the Permanent Executive Committee for Service Centers and Fuel Stations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules of Procedure of the Permanent Executive Committee for Service Centers and Fuel Stations")
