#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم حوكمة إجراءات منع التصدير وتقييده track.
...
RECORDS = os.path.join(ROOT, "sources", "export_restriction_governance_statute", "verified",
                       "export_restriction_governance_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "export_restriction_governance_statute", "verified",
                       "export_restriction_governance_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "export_restriction_governance_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"export_restriction_governance_statute_no_decision_number_in_source", "export_restriction_governance_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "11/6/1443":
        e.append("[2j] gazette_publication_date_hijri must be 11/6/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-01-14":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-01-14")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم حوكمة إجراءات منع التصدير وتقييده":
    ...
    if e:
        print("FAIL: %d error(s) in Governance Statute for Export Prohibition and Restriction Procedures track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Governance Statute for Export Prohibition and Restriction Procedures")
