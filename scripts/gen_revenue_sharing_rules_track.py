#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة للمشاركة في الدخل track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19855),
...
SRC = os.path.join(ROOT, "sources", "revenue_sharing_rules", "official_source",
                   "revenue_sharing_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "revenue_sharing_rules", "verified")
RECORDS = os.path.join(OUT_VER, "revenue_sharing_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "revenue_sharing_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "revenue_sharing_rules_arabic_legal_llm",
                        "revenue_sharing_rules_legal_llm_001_031.json")

LAW_ID = "sa-revenue-sharing-rules"
LAW_AR = "القواعد المنظمة للمشاركة في الدخل"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"revenue_sharing_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19855), published 7/1/1444H (2022-08-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "revenue_sharing_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "REVENUE_SHARING_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "revenue-sharing-rules-llm-%03d" % idx,
            ...
            "article_path": "revenue_sharing_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing Revenue Sharing -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/1/1444H / 2022-08-05)"),
                "source_authority_ar": ("القواعد المنظمة للمشاركة في الدخل — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "revenue_sharing_rules",
        "layer": "REVENUE_SHARING_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "revenue-sharing-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (31 مادة، جميعها أصلية)",
        "title_en": "Rules Governing Revenue Sharing — Arabic LLM-ready layer (31 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing Revenue Sharing records" % (len(ver), len(llm)))
