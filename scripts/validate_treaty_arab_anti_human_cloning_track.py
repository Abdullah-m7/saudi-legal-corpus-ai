#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الاتفاقية العربية لمنع ومكافحة الاستنساخ البشري track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_arab_anti_human_cloning", "verified",
                       "treaty_arab_anti_human_cloning_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_arab_anti_human_cloning", "verified",
                       "treaty_arab_anti_human_cloning_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_arab_anti_human_cloning_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_arab_anti_human_cloning_no_decision_number_in_source", "treaty_arab_anti_human_cloning_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الاتفاقية العربية لمنع ومكافحة الاستنساخ البشري":
    ...
    if e:
        print("FAIL: %d error(s) in Arab Convention on the Prevention and Combating of Human Cloning track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Arab Convention on the Prevention and Combating of Human Cloning")
