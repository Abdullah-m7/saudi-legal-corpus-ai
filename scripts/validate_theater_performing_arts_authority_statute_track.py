#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم هيئة المسرح والفنون الأدائية track.
...
RECORDS = os.path.join(ROOT, "sources", "theater_performing_arts_authority_statute", "verified",
                       "theater_performing_arts_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "theater_performing_arts_authority_statute", "verified",
                       "theater_performing_arts_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "theater_performing_arts_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"theater_performing_arts_authority_statute_no_decision_number_in_source", "theater_performing_arts_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم هيئة المسرح والفنون الأدائية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Theater and Performing Arts Commission track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Theater and Performing Arts Commission")
