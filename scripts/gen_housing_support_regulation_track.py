#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لتنظيم الدعم السكني track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27511),
published 11/4/1447H (2025-10-03). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "housing_support_regulation", "official_source",
                   "housing_support_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "housing_support_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "housing_support_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "housing_support_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "housing_support_regulation_arabic_legal_llm",
                        "housing_support_regulation_legal_llm_001_053.json")

LAW_ID = "sa-housing-support-regulation"
LAW_AR = "اللائحة التنفيذية لتنظيم الدعم السكني"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"housing_support_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27511), published 11/4/1447H (2025-10-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "housing_support_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "HOUSING_SUPPORT_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "housing-support-regulation-llm-%03d" % idx,
            ...
            "article_path": "housing_support_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Housing Support Framework -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 11/4/1447H / 2025-10-03)"),
                "source_authority_ar": ("اللائحة التنفيذية لتنظيم الدعم السكني — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 11/4/1447هـ الموافق 2025-10-03م)"),
    ...
    json.dump({
        "law_key": "housing_support_regulation",
        "layer": "HOUSING_SUPPORT_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "housing-support-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (53 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Housing Support Framework — Arabic LLM-ready layer (53 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Housing Support Framework records" % (len(ver), len(llm)))
