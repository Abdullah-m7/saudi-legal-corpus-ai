#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم هيئة الزكاة والضريبة والجمارك track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17596),
...
SRC = os.path.join(ROOT, "sources", "zatca_statute", "official_source",
                   "zatca_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "zatca_statute", "verified")
RECORDS = os.path.join(OUT_VER, "zatca_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "zatca_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "zatca_statute_arabic_legal_llm",
                        "zatca_statute_legal_llm_001_016.json")

LAW_ID = "sa-zatca-statute"
LAW_AR = "تنظيم هيئة الزكاة والضريبة والجمارك"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"zatca_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17596), published 2/10/1442H (2021-05-14). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "zatca_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "ZATCA_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "zatca-statute-llm-%03d" % idx,
            ...
            "article_path": "zatca_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Zakat, Tax and Customs Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/10/1442H / 2021-05-14)"),
                "source_authority_ar": ("تنظيم هيئة الزكاة والضريبة والجمارك — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "zatca_statute",
        "layer": "ZATCA_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "zatca-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Statute of the Zakat, Tax and Customs Authority — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Zakat, Tax and Customs Authority records" % (len(ver), len(llm)))
