#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الفنية لمعدات وتجهيزات المركبات ذات الاستخدام الخاص track.
...
RECORDS = os.path.join(ROOT, "sources", "special_use_vehicle_equipment_technical_regulation", "verified",
                       "special_use_vehicle_equipment_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "special_use_vehicle_equipment_technical_regulation", "verified",
                       "special_use_vehicle_equipment_technical_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "special_use_vehicle_equipment_technical_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"special_use_vehicle_equipment_technical_regulation_no_decision_number_in_source", "special_use_vehicle_equipment_technical_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "19/2/1446":
        e.append("[2j] gazette_publication_date_hijri must be 19/2/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-08-23":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-08-23")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة الفنية لمعدات وتجهيزات المركبات ذات الاستخدام الخاص":
    ...
    if e:
        print("FAIL: %d error(s) in Technical Regulation for Special-Use Vehicle Equipment and Fittings track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Technical Regulation for Special-Use Vehicle Equipment and Fittings")
