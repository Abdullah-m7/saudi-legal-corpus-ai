#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة الوجهات السياحية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21124),
...
SRC = os.path.join(ROOT, "sources", "tourist_destinations_regulation", "official_source",
                   "tourist_destinations_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "tourist_destinations_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "tourist_destinations_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "tourist_destinations_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "tourist_destinations_regulation_arabic_legal_llm",
                        "tourist_destinations_regulation_legal_llm_001_008.json")

LAW_ID = "sa-tourist-destinations-regulation"
LAW_AR = "لائحة الوجهات السياحية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"tourist_destinations_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21124), published 1/6/1444H (2022-12-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "tourist_destinations_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "TOURIST_DESTINATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "tourist-destinations-regulation-llm-%03d" % idx,
            ...
            "article_path": "tourist_destinations_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Tourist Destinations Regulation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/6/1444H / 2022-12-25)"),
                "source_authority_ar": ("لائحة الوجهات السياحية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "tourist_destinations_regulation",
        "layer": "TOURIST_DESTINATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "tourist-destinations-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Tourist Destinations Regulation — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Tourist Destinations Regulation records" % (len(ver), len(llm)))
