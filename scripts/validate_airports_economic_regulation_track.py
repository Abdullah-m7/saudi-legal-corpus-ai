#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الاقتصادية للمطارات track.
...
RECORDS = os.path.join(ROOT, "sources", "airports_economic_regulation", "verified",
                       "airports_economic_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "airports_economic_regulation", "verified",
                       "airports_economic_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "airports_economic_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"airports_economic_regulation_no_decision_number_in_source", "airports_economic_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة الاقتصادية للمطارات":
    ...
    if e:
        print("FAIL: %d error(s) in Economic Regulation for Airports track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Economic Regulation for Airports")
