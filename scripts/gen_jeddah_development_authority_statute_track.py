#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لهيئة تطوير محافظة جدة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23819),
published 21/3/1445H (2023-10-06). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "jeddah_development_authority_statute", "official_source",
                   "jeddah_development_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "jeddah_development_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "jeddah_development_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "jeddah_development_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "jeddah_development_authority_statute_arabic_legal_llm",
                        "jeddah_development_authority_statute_legal_llm_001_015.json")

LAW_ID = "sa-jeddah-development-authority-statute"
LAW_AR = "الترتيبات التنظيمية لهيئة تطوير محافظة جدة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"jeddah_development_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23819), published 21/3/1445H (2023-10-06). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "jeddah_development_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "JEDDAH_DEVELOPMENT_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "jeddah-development-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "jeddah_development_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Jeddah Governorate Development Authority — Organisational Arrangements -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/3/1445H / 2023-10-06)"),
                "source_authority_ar": ("الترتيبات التنظيمية لهيئة تطوير محافظة جدة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 21/3/1445هـ الموافق 2023-10-06م)"),
    ...
    json.dump({
        "law_key": "jeddah_development_authority_statute",
        "layer": "JEDDAH_DEVELOPMENT_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "jeddah-development-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Jeddah Governorate Development Authority — Organisational Arrangements — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Jeddah Governorate Development Authority — Organisational Arrangements records" % (len(ver), len(llm)))
