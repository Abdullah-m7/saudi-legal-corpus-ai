#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة تنظيم أعمال النظارة track.
...
RECORDS = os.path.join(ROOT, "sources", "nazara_works_regulation", "verified",
                       "nazara_works_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "nazara_works_regulation", "verified",
                       "nazara_works_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "nazara_works_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"nazara_works_regulation_no_decision_number_in_source", "nazara_works_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة تنظيم أعمال النظارة":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation Governing Waqf Custodianship (Nazara) Works track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation Governing Waqf Custodianship (Nazara) Works")
