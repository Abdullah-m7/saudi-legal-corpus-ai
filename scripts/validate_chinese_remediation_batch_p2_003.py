#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate Chinese remediation Batch P2-003 (P2 expansion batch; 20 articles, Babs 7/8/9/10).
...
without changing meaning. Confirms the batch covers exactly the 20 authorized P2-003 articles with
verbatim-hashed internal Chinese, that every record's bab is in [7,8,9,10] and equals the coverage-index
expected_bab_number, that each record links to the (unchanged) prior candidate record and to its P2
finding in the remediation backlog, carries the internal / non-official / non-binding / non-governing
posture under the repository review model, with qa_status pending_future_qa, and touches no protected
layer (P2-001 + P2-002 batches + QA, all P1 + P0 batches + QA, and the base corpora). No p3_* batch
dirs, no full Chinese 281 layer, no trilingual alignment. Read-only and idempotent.

Usage: validate_chinese_remediation_batch_p2_003.py [DATA_JSON_PATH]
...
DATA_DEFAULT = os.path.join(ROOT, "data", "chinese_remediation_batches", "p2_003",
                            "companies_law_m132_1443_zh_internal_remediation_p2_003.json")
MD = os.path.join(ROOT, "reports", "chinese_translation_review",
                  "CHINESE_REMEDIATION_BATCH_P2_003_AR.md")
...
# Already-merged P2 batches that must remain intact.
PRIOR_P2 = {
...
def main(argv=None) -> int:
    ...
    if d.get("stage") != "CHINESE_REMEDIATION_BATCH_P2_003":
        problems.append("stage must be CHINESE_REMEDIATION_BATCH_P2_003")
    if d.get("batch_id") != "P2-003":
    ...
    if nums != ARTS:
        problems.append("record article numbers must be exactly the P2-003 list, no extras")
    ...
    print("=" * 60)
    print("Chinese remediation Batch P2-003 validation (P2 expansion batch)")
    print("=" * 60)
    if problems:
        for p in problems:
            print("  -", p)
        print("RESULT: %d problem(s) found ✗" % len(problems))
        return 1
    print("[PASS] Batch P2-003: 20 authorized articles across Babs [7,8,9,10]; verbatim-hashed internal "
          "Chinese EXPANDED from the official Arabic (English guidance only; existing candidate as the "
          "starting point); each record links to the unchanged prior candidate and its P2 backlog "
          "finding; internal/non-official/non-binding/non-governing; qa_status pending_future_qa; "
          "P2-001 + P2-002 + all P1 + P0 batches + QA + base corpora unchanged; no p3_* dirs; no "
          "full-281 / trilingual.")
