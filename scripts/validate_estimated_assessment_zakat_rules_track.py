#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد حساب زكاة مكلفي التقديري track.
...
RECORDS = os.path.join(ROOT, "sources", "estimated_assessment_zakat_rules", "verified",
                       "estimated_assessment_zakat_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "estimated_assessment_zakat_rules", "verified",
                       "estimated_assessment_zakat_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "estimated_assessment_zakat_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"estimated_assessment_zakat_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "14/7/1442":
        e.append("[2j] gazette_publication_date_hijri must be 14/7/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-02-26":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-02-26")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد حساب زكاة مكلفي التقديري":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Computing Zakat of Estimated-Assessment Taxpayers track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Computing Zakat of Estimated-Assessment Taxpayers")
