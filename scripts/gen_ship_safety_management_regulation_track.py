#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لإدارة السلامة وتشغيل السفن track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19273),
...
SRC = os.path.join(ROOT, "sources", "ship_safety_management_regulation", "official_source",
                   "ship_safety_management_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "ship_safety_management_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "ship_safety_management_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "ship_safety_management_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "ship_safety_management_regulation_arabic_legal_llm",
                        "ship_safety_management_regulation_legal_llm_001_014.json")

LAW_ID = "sa-ship-safety-management-regulation"
LAW_AR = "اللائحة التنفيذية لإدارة السلامة وتشغيل السفن"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"ship_safety_management_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19273), published 7/9/1443H (2022-04-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "ship_safety_management_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SHIP_SAFETY_MANAGEMENT_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "ship-safety-management-regulation-llm-%03d" % idx,
            ...
            "article_path": "ship_safety_management_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation on Safety Management and Ship Operation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/9/1443H / 2022-04-08)"),
                "source_authority_ar": ("اللائحة التنفيذية لإدارة السلامة وتشغيل السفن — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "ship_safety_management_regulation",
        "layer": "SHIP_SAFETY_MANAGEMENT_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "ship-safety-management-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation on Safety Management and Ship Operation — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation on Safety Management and Ship Operation records" % (len(ver), len(llm)))
