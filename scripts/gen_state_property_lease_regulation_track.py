#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام استئجار الدولة للعقار track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21318),
...
STRUCTURE: 79 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "state_property_lease_regulation", "official_source",
                   "state_property_lease_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "state_property_lease_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "state_property_lease_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "state_property_lease_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "state_property_lease_regulation_arabic_legal_llm",
                        "state_property_lease_regulation_legal_llm_001_079.json")

LAW_ID = "sa-state-property-lease-regulation"
LAW_AR = "اللائحة التنفيذية لنظام استئجار الدولة للعقار"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"state_property_lease_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21318), published 5/7/1444H (2023-01-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "state_property_lease_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "STATE_PROPERTY_LEASE_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "state-property-lease-regulation-llm-%03d" % idx,
            ...
            "article_path": "state_property_lease_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Law on State Leasing of Real Estate -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/7/1444H / 2023-01-27)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام استئجار الدولة للعقار — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "state_property_lease_regulation",
        "layer": "STATE_PROPERTY_LEASE_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "state-property-lease-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (79 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Law on State Leasing of Real Estate — Arabic LLM-ready layer (79 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Law on State Leasing of Real Estate records" % (len(ver), len(llm)))
