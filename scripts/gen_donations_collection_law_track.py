#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام جمع التبرعات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25376),
...
SRC = os.path.join(ROOT, "sources", "donations_collection_law", "official_source",
                   "donations_collection_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "donations_collection_law", "verified")
RECORDS = os.path.join(OUT_VER, "donations_collection_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "donations_collection_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "donations_collection_law_arabic_legal_llm",
                        "donations_collection_law_legal_llm_001_023.json")

LAW_ID = "sa-donations-collection-law"
LAW_AR = "نظام جمع التبرعات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"donations_collection_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25376), published 19/2/1446H (2024-08-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "donations_collection_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "DONATIONS_COLLECTION_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "donations-collection-law-llm-%03d" % idx,
            ...
            "article_path": "donations_collection_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law on the Collection of Donations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 19/2/1446H / 2024-08-23)"),
                "source_authority_ar": ("نظام جمع التبرعات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "donations_collection_law",
        "layer": "DONATIONS_COLLECTION_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "donations-collection-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (23 مادة، جميعها أصلية)",
        "title_en": "Law on the Collection of Donations — Arabic LLM-ready layer (23 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law on the Collection of Donations records" % (len(ver), len(llm)))
