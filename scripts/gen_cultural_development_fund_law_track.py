#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام صندوق التنمية الثقافي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17343),
...
SRC = os.path.join(ROOT, "sources", "cultural_development_fund_law", "official_source",
                   "cultural_development_fund_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "cultural_development_fund_law", "verified")
RECORDS = os.path.join(OUT_VER, "cultural_development_fund_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "cultural_development_fund_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "cultural_development_fund_law_arabic_legal_llm",
                        "cultural_development_fund_law_legal_llm_001_015.json")

LAW_ID = "sa-cultural-development-fund-law"
LAW_AR = "نظام صندوق التنمية الثقافي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"cultural_development_fund_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17343), published 2/6/1442H (2021-01-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "cultural_development_fund_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "CULTURAL_DEVELOPMENT_FUND_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "cultural-development-fund-law-llm-%03d" % idx,
            ...
            "article_path": "cultural_development_fund_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law of the Cultural Development Fund -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/6/1442H / 2021-01-15)"),
                "source_authority_ar": ("نظام صندوق التنمية الثقافي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "cultural_development_fund_law",
        "layer": "CULTURAL_DEVELOPMENT_FUND_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "cultural-development-fund-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Law of the Cultural Development Fund — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law of the Cultural Development Fund records" % (len(ver), len(llm)))
