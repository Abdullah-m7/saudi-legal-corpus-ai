#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد الأسواق الحرة track.
...
RECORDS = os.path.join(ROOT, "sources", "duty_free_markets_rules", "verified",
                       "duty_free_markets_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "duty_free_markets_rules", "verified",
                       "duty_free_markets_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "duty_free_markets_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"duty_free_markets_rules_no_decision_number_in_source", "duty_free_markets_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد الأسواق الحرة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules of Duty-Free Markets track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules of Duty-Free Markets")
