#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام بنك المنشآت الصغيرة والمتوسطة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20827),
...
SRC = os.path.join(ROOT, "sources", "sme_bank_law", "official_source",
                   "sme_bank_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "sme_bank_law", "verified")
RECORDS = os.path.join(OUT_VER, "sme_bank_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "sme_bank_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "sme_bank_law_arabic_legal_llm",
                        "sme_bank_law_legal_llm_001_015.json")

LAW_ID = "sa-sme-bank-law"
LAW_AR = "نظام بنك المنشآت الصغيرة والمتوسطة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"sme_bank_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20827), published 17/4/1444H (2022-11-11). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "sme_bank_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "SME_BANK_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "sme-bank-law-llm-%03d" % idx,
            ...
            "article_path": "sme_bank_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law of the Small and Medium Enterprises Bank -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/4/1444H / 2022-11-11)"),
                "source_authority_ar": ("نظام بنك المنشآت الصغيرة والمتوسطة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "sme_bank_law",
        "layer": "SME_BANK_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "sme-bank-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Law of the Small and Medium Enterprises Bank — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law of the Small and Medium Enterprises Bank records" % (len(ver), len(llm)))
