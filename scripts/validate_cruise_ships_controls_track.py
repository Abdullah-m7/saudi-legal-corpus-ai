#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الضوابط والإجراءات لسفن الرحلات السياحية الكروز track.
...
RECORDS = os.path.join(ROOT, "sources", "cruise_ships_controls", "verified",
                       "cruise_ships_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "cruise_ships_controls", "verified",
                       "cruise_ships_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "cruise_ships_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"cruise_ships_controls_no_decision_number_in_source", "cruise_ships_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الضوابط والإجراءات لسفن الرحلات السياحية الكروز":
    ...
    if e:
        print("FAIL: %d error(s) in Controls and Procedures for Cruise Ships track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls and Procedures for Cruise Ships")
