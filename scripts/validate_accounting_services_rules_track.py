#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة لخدمات المحاسبة track.
...
RECORDS = os.path.join(ROOT, "sources", "accounting_services_rules", "verified",
                       "accounting_services_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "accounting_services_rules", "verified",
                       "accounting_services_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "accounting_services_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"accounting_services_rules_no_decision_number_in_source", "accounting_services_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "19/10/1443":
        e.append("[2j] gazette_publication_date_hijri must be 19/10/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-05-20":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-05-20")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "القواعد المنظمة لخدمات المحاسبة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing Accounting Services track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing Accounting Services")
