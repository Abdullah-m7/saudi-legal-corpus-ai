#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم الهيئة العامة لعقارات الدولة track.
...
RECORDS = os.path.join(ROOT, "sources", "state_property_authority_statute", "verified",
                       "state_property_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "state_property_authority_statute", "verified",
                       "state_property_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "state_property_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"state_property_authority_statute_no_decision_number_in_source", "state_property_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم الهيئة العامة لعقارات الدولة":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the General Authority for State Real Estate track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the General Authority for State Real Estate")
