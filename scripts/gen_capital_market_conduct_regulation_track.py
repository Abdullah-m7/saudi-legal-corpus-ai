#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة سلوكيات السوق المالية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17610),
published 2/10/1442H (2021-05-14). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "capital_market_conduct_regulation", "official_source",
                   "capital_market_conduct_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "capital_market_conduct_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "capital_market_conduct_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "capital_market_conduct_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "capital_market_conduct_regulation_arabic_legal_llm",
                        "capital_market_conduct_regulation_legal_llm_001_021.json")

LAW_ID = "sa-capital-market-conduct-regulation"
LAW_AR = "لائحة سلوكيات السوق المالية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"capital_market_conduct_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17610), published 2/10/1442H (2021-05-14). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "capital_market_conduct_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CAPITAL_MARKET_CONDUCT_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "capital-market-conduct-regulation-llm-%03d" % idx,
            ...
            "article_path": "capital_market_conduct_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Market Conduct Regulations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/10/1442H / 2021-05-14)"),
                "source_authority_ar": ("لائحة سلوكيات السوق المالية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 2/10/1442هـ الموافق 2021-05-14م)"),
    ...
    json.dump({
        "law_key": "capital_market_conduct_regulation",
        "layer": "CAPITAL_MARKET_CONDUCT_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "capital-market-conduct-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (21 مادة، جميعها أصلية)",
        "title_en": "Market Conduct Regulations — Arabic LLM-ready layer (21 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Market Conduct Regulations records" % (len(ver), len(llm)))
