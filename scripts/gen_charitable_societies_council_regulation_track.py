#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة مجلس الجمعيات الأهلية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21364),
...
STRUCTURE: 65 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "charitable_societies_council_regulation", "official_source",
                   "charitable_societies_council_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "charitable_societies_council_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "charitable_societies_council_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "charitable_societies_council_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "charitable_societies_council_regulation_arabic_legal_llm",
                        "charitable_societies_council_regulation_legal_llm_001_065.json")

LAW_ID = "sa-charitable-societies-council-regulation"
LAW_AR = "لائحة مجلس الجمعيات الأهلية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"charitable_societies_council_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21364), published 12/7/1444H (2023-02-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "charitable_societies_council_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CHARITABLE_SOCIETIES_COUNCIL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "charitable-societies-council-regulation-llm-%03d" % idx,
            ...
            "article_path": "charitable_societies_council_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation of the Council of Charitable Societies -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 12/7/1444H / 2023-02-03)"),
                "source_authority_ar": ("لائحة مجلس الجمعيات الأهلية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "charitable_societies_council_regulation",
        "layer": "CHARITABLE_SOCIETIES_COUNCIL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "charitable-societies-council-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (65 مادة، جميعها أصلية)",
        "title_en": "Regulation of the Council of Charitable Societies — Arabic LLM-ready layer (65 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation of the Council of Charitable Societies records" % (len(ver), len(llm)))
