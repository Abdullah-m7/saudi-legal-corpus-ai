#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية مقر بين حكومة المملكة واتحاد النقل الجوي الدولي (إياتا) track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_iata_headquarters", "verified",
                       "treaty_iata_headquarters_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_iata_headquarters", "verified",
                       "treaty_iata_headquarters_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_iata_headquarters_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_iata_headquarters_no_decision_number_in_source", "treaty_iata_headquarters_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية مقر بين حكومة المملكة واتحاد النقل الجوي الدولي (إياتا)":
    ...
    if e:
        print("FAIL: %d error(s) in Headquarters Agreement between Saudi Arabia and IATA track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Headquarters Agreement between Saudi Arabia and IATA")
