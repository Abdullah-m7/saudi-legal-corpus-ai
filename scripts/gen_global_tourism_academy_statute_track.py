#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية للأكاديمية العالمية للسياحة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18870),
published 18/6/1443H (2022-01-21). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "global_tourism_academy_statute", "official_source",
                   "global_tourism_academy_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "global_tourism_academy_statute", "verified")
RECORDS = os.path.join(OUT_VER, "global_tourism_academy_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "global_tourism_academy_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "global_tourism_academy_statute_arabic_legal_llm",
                        "global_tourism_academy_statute_legal_llm_001_014.json")

LAW_ID = "sa-global-tourism-academy-statute"
LAW_AR = "الترتيبات التنظيمية للأكاديمية العالمية للسياحة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"global_tourism_academy_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18870), published 18/6/1443H (2022-01-21). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "global_tourism_academy_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "GLOBAL_TOURISM_ACADEMY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "global-tourism-academy-statute-llm-%03d" % idx,
            ...
            "article_path": "global_tourism_academy_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Global Academy of Tourism — Organisational Arrangements -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 18/6/1443H / 2022-01-21)"),
                "source_authority_ar": ("الترتيبات التنظيمية للأكاديمية العالمية للسياحة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 18/6/1443هـ الموافق 2022-01-21م)"),
    ...
    json.dump({
        "law_key": "global_tourism_academy_statute",
        "layer": "GLOBAL_TOURISM_ACADEMY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "global-tourism-academy-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Global Academy of Tourism — Organisational Arrangements — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Global Academy of Tourism — Organisational Arrangements records" % (len(ver), len(llm)))
