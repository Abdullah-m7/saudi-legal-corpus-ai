#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد مناطق الإيداع track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24398),
...
SRC = os.path.join(ROOT, "sources", "deposit_zones_rules", "official_source",
                   "deposit_zones_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "deposit_zones_rules", "verified")
RECORDS = os.path.join(OUT_VER, "deposit_zones_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "deposit_zones_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "deposit_zones_rules_arabic_legal_llm",
                        "deposit_zones_rules_legal_llm_001_025.json")

LAW_ID = "sa-deposit-zones-rules"
LAW_AR = "قواعد مناطق الإيداع"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"deposit_zones_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24398), published 14/7/1445H (2024-01-26). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "deposit_zones_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "DEPOSIT_ZONES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "deposit-zones-rules-llm-%03d" % idx,
            ...
            "article_path": "deposit_zones_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules of Deposit Zones -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/7/1445H / 2024-01-26)"),
                "source_authority_ar": ("قواعد مناطق الإيداع — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "deposit_zones_rules",
        "layer": "DEPOSIT_ZONES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "deposit-zones-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (25 مادة، جميعها أصلية)",
        "title_en": "Rules of Deposit Zones — Arabic LLM-ready layer (25 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules of Deposit Zones records" % (len(ver), len(llm)))
