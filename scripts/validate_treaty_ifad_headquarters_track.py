#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية بين حكومة المملكة والصندوق الدولي للتنمية الزراعية track.

17 records: 17 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_ifad_headquarters", "verified",
                       "treaty_ifad_headquarters_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_ifad_headquarters", "verified",
                       "treaty_ifad_headquarters_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_ifad_headquarters_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "treaty_ifad_headquarters_no_decision_number_in_source",
    "treaty_ifad_headquarters_source_is_gazette_html_only",
    "treaty_ifad_headquarters_is_a_ratified_international_agreement",
    # The reason this track exists at all: its article headings carry no colon
    # and no dash, and the pipeline had to be taught to read them. If that
    # disclosure ever goes missing, a reader loses the only notice that these
    # seventeen articles were segmented on a weaker mark than the rest of the
    # corpus, under four conditions rather than one.
    "treaty_ifad_headquarters_headings_carry_no_terminator",
}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "11/9/1442":
        e.append("[2j] gazette_publication_date_hijri must be 11/9/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-04-23":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-04-23")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية بين حكومة المملكة والصندوق الدولي للتنمية الزراعية":
    ...
    if e:
        print("FAIL: %d error(s) in Agreement between the Government of Saudi Arabia and the International Fund for Agricultural Development track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Agreement between the Government of Saudi Arabia and the International Fund for Agricultural Development")
    print("  - 17 records: 17 articles, no appendices; all 17 اصلية")
