#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط بقاء المركبات المسجلة في إحدى دول مجلس التعاون في السعودية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4000446),
...
SRC = os.path.join(ROOT, "sources", "gcc_registered_vehicles_stay_controls", "official_source",
                   "gcc_registered_vehicles_stay_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "gcc_registered_vehicles_stay_controls", "verified")
RECORDS = os.path.join(OUT_VER, "gcc_registered_vehicles_stay_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "gcc_registered_vehicles_stay_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "gcc_registered_vehicles_stay_controls_arabic_legal_llm",
                        "gcc_registered_vehicles_stay_controls_legal_llm_001_007.json")

LAW_ID = "sa-gcc-registered-vehicles-stay-controls"
LAW_AR = "ضوابط بقاء المركبات المسجلة في إحدى دول مجلس التعاون في السعودية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"gcc_registered_vehicles_stay_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4000446), published 10/9/1447H (2026-02-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "gcc_registered_vehicles_stay_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "GCC_REGISTERED_VEHICLES_STAY_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "gcc-registered-vehicles-stay-controls-llm-%03d" % idx,
            ...
            "article_path": "gcc_registered_vehicles_stay_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on the Stay in Saudi Arabia of Vehicles Registered in a GCC State -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 10/9/1447H / 2026-02-27)"),
                "source_authority_ar": ("ضوابط بقاء المركبات المسجلة في إحدى دول مجلس التعاون في السعودية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "gcc_registered_vehicles_stay_controls",
        "layer": "GCC_REGISTERED_VEHICLES_STAY_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "gcc-registered-vehicles-stay-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (7 مادة، جميعها أصلية)",
        "title_en": "Controls on the Stay in Saudi Arabia of Vehicles Registered in a GCC State — Arabic LLM-ready layer (7 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on the Stay in Saudi Arabia of Vehicles Registered in a GCC State records" % (len(ver), len(llm)))
