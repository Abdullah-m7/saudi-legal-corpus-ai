#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لمركز ذاكرة الثقافة السعودية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4001288),
published 18/1/1448H (2026-07-03). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "saudi_culture_memory_center_statute", "official_source",
                   "saudi_culture_memory_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "saudi_culture_memory_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "saudi_culture_memory_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "saudi_culture_memory_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "saudi_culture_memory_center_statute_arabic_legal_llm",
                        "saudi_culture_memory_center_statute_legal_llm_001_012.json")

LAW_ID = "sa-saudi-culture-memory-center-statute"
LAW_AR = "الترتيبات التنظيمية لمركز ذاكرة الثقافة السعودية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"saudi_culture_memory_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4001288), published 18/1/1448H (2026-07-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "saudi_culture_memory_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "SAUDI_CULTURE_MEMORY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "saudi-culture-memory-center-statute-llm-%03d" % idx,
            ...
            "article_path": "saudi_culture_memory_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the Center for the Memory of Saudi Culture -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 18/1/1448H / 2026-07-03)"),
                "source_authority_ar": ("الترتيبات التنظيمية لمركز ذاكرة الثقافة السعودية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 18/1/1448هـ الموافق 2026-07-03م)"),
    ...
    json.dump({
        "law_key": "saudi_culture_memory_center_statute",
        "layer": "SAUDI_CULTURE_MEMORY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "saudi-culture-memory-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the Center for the Memory of Saudi Culture — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the Center for the Memory of Saudi Culture records" % (len(ver), len(llm)))
