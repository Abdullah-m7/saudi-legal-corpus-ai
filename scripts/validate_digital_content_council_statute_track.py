#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مجلس المحتوى الرقمي track.
...
RECORDS = os.path.join(ROOT, "sources", "digital_content_council_statute", "verified",
                       "digital_content_council_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "digital_content_council_statute", "verified",
                       "digital_content_council_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "digital_content_council_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"digital_content_council_statute_no_decision_number_in_source", "digital_content_council_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم مجلس المحتوى الرقمي":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Digital Content Council track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Digital Content Council")
