#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مركز التأمين الصحي الوطني track.
...
RECORDS = os.path.join(ROOT, "sources", "national_health_insurance_center_statute", "verified",
                       "national_health_insurance_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "national_health_insurance_center_statute", "verified",
                       "national_health_insurance_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "national_health_insurance_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"national_health_insurance_center_statute_no_decision_number_in_source", "national_health_insurance_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم مركز التأمين الصحي الوطني":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the National Health Insurance Center track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the National Health Insurance Center")
