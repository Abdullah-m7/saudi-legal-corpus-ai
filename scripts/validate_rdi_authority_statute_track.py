#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لهيئة تنمية البحث والتطوير والابتكار track.
...
RECORDS = os.path.join(ROOT, "sources", "rdi_authority_statute", "verified",
                       "rdi_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "rdi_authority_statute", "verified",
                       "rdi_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "rdi_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"rdi_authority_statute_no_decision_number_in_source", "rdi_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية لهيئة تنمية البحث والتطوير والابتكار":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the Research, Development and Innovation Authority track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the Research, Development and Innovation Authority")
