#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لهيئة تطوير الأحساء track.
...
RECORDS = os.path.join(ROOT, "sources", "alahsa_development_authority_statute", "verified",
                       "alahsa_development_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "alahsa_development_authority_statute", "verified",
                       "alahsa_development_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "alahsa_development_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"alahsa_development_authority_statute_no_decision_number_in_source", "alahsa_development_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "17/4/1444":
        e.append("[2j] gazette_publication_date_hijri must be 17/4/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-11-11":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-11-11")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية لهيئة تطوير الأحساء":
    ...
    if e:
        print("FAIL: %d error(s) in Al-Ahsa Development Authority — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Al-Ahsa Development Authority — Organisational Arrangements")
