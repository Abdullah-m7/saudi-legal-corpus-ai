#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة لخدمات محاسبة الزكاة والضريبة track.
...
RECORDS = os.path.join(ROOT, "sources", "zakat_tax_accounting_services_rules", "verified",
                       "zakat_tax_accounting_services_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "zakat_tax_accounting_services_rules", "verified",
                       "zakat_tax_accounting_services_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "zakat_tax_accounting_services_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"zakat_tax_accounting_services_rules_no_decision_number_in_source", "zakat_tax_accounting_services_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد المنظمة لخدمات محاسبة الزكاة والضريبة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing Zakat and Tax Accounting Services track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing Zakat and Tax Accounting Services")
