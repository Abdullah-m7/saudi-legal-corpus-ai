#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مركز الإقامة المميزة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19536),
published 4/11/1443H (2022-06-03). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "premium_residency_center_statute", "official_source",
                   "premium_residency_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "premium_residency_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "premium_residency_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "premium_residency_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "premium_residency_center_statute_arabic_legal_llm",
                        "premium_residency_center_statute_legal_llm_001_014.json")

LAW_ID = "sa-premium-residency-center-statute"
LAW_AR = "تنظيم مركز الإقامة المميزة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"premium_residency_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19536), published 4/11/1443H (2022-06-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "premium_residency_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "PREMIUM_RESIDENCY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "premium-residency-center-statute-llm-%03d" % idx,
            ...
            "article_path": "premium_residency_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Premium Residency Center -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/11/1443H / 2022-06-03)"),
                "source_authority_ar": ("تنظيم مركز الإقامة المميزة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 4/11/1443هـ الموافق 2022-06-03م)"),
    ...
    json.dump({
        "law_key": "premium_residency_center_statute",
        "layer": "PREMIUM_RESIDENCY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "premium-residency-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Statute of the Premium Residency Center — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Premium Residency Center records" % (len(ver), len(llm)))
