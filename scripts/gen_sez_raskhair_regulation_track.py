#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية للمنطقة الاقتصادية الخاصة برأس الخير track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28790),
...
SRC = os.path.join(ROOT, "sources", "sez_raskhair_regulation", "official_source",
                   "sez_raskhair_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "sez_raskhair_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "sez_raskhair_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "sez_raskhair_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "sez_raskhair_regulation_arabic_legal_llm",
                        "sez_raskhair_regulation_legal_llm_001_034.json")

LAW_ID = "sa-sez-raskhair-regulation"
LAW_AR = "اللائحة التنظيمية للمنطقة الاقتصادية الخاصة برأس الخير"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"sez_raskhair_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28790), published 27/7/1447H (2026-01-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "sez_raskhair_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SEZ_RASKHAIR_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "sez-raskhair-regulation-llm-%03d" % idx,
            ...
            "article_path": "sez_raskhair_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules of the Ras Al-Khair Special Economic Zone -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/7/1447H / 2026-01-16)"),
                "source_authority_ar": ("اللائحة التنظيمية للمنطقة الاقتصادية الخاصة برأس الخير — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "sez_raskhair_regulation",
        "layer": "SEZ_RASKHAIR_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "sez-raskhair-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (34 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules of the Ras Al-Khair Special Economic Zone — Arabic LLM-ready layer (34 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules of the Ras Al-Khair Special Economic Zone records" % (len(ver), len(llm)))
