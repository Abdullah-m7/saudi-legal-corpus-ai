#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية نظام ربط أنظمة المدفوعات بين دول مجلس التعاون لدول الخليج العربية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18010),
published 13/12/1442H (2021-07-23). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "treaty_gcc_payment_systems_linkage", "official_source",
                   "treaty_gcc_payment_systems_linkage_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_gcc_payment_systems_linkage", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_gcc_payment_systems_linkage_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_gcc_payment_systems_linkage_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_gcc_payment_systems_linkage_arabic_legal_llm",
                        "treaty_gcc_payment_systems_linkage_legal_llm_001_014.json")

LAW_ID = "sa-treaty-gcc-payment-systems-linkage"
LAW_AR = "اتفاقية نظام ربط أنظمة المدفوعات بين دول مجلس التعاون لدول الخليج العربية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_gcc_payment_systems_linkage_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18010), published 13/12/1442H (2021-07-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_gcc_payment_systems_linkage",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_GCC_PAYMENT_SYSTEMS_LINKAGE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-gcc-payment-systems-linkage-llm-%03d" % idx,
            ...
            "article_path": "treaty_gcc_payment_systems_linkage/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("GCC Payment Systems Linkage Agreement -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/12/1442H / 2021-07-23)"),
                "source_authority_ar": ("اتفاقية نظام ربط أنظمة المدفوعات بين دول مجلس التعاون لدول الخليج العربية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 13/12/1442هـ الموافق 2021-07-23م)"),
    ...
    json.dump({
        "law_key": "treaty_gcc_payment_systems_linkage",
        "layer": "TREATY_GCC_PAYMENT_SYSTEMS_LINKAGE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-gcc-payment-systems-linkage-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "GCC Payment Systems Linkage Agreement — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready GCC Payment Systems Linkage Agreement records" % (len(ver), len(llm)))
