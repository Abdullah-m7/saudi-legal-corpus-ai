#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لمجلس المخاطر الوطنية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19381),
...
SRC = os.path.join(ROOT, "sources", "national_risk_council_statute", "official_source",
                   "national_risk_council_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "national_risk_council_statute", "verified")
RECORDS = os.path.join(OUT_VER, "national_risk_council_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "national_risk_council_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "national_risk_council_statute_arabic_legal_llm",
                        "national_risk_council_statute_legal_llm_001_019.json")

LAW_ID = "sa-national-risk-council-statute"
LAW_AR = "الترتيبات التنظيمية لمجلس المخاطر الوطنية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"national_risk_council_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19381), published 7/9/1443H (2022-04-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "national_risk_council_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "NATIONAL_RISK_COUNCIL_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "national-risk-council-statute-llm-%03d" % idx,
            ...
            "article_path": "national_risk_council_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the National Risk Council -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/9/1443H / 2022-04-08)"),
                "source_authority_ar": ("الترتيبات التنظيمية لمجلس المخاطر الوطنية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "national_risk_council_statute",
        "layer": "NATIONAL_RISK_COUNCIL_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "national-risk-council-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (19 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the National Risk Council — Arabic LLM-ready layer (19 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the National Risk Council records" % (len(ver), len(llm)))
