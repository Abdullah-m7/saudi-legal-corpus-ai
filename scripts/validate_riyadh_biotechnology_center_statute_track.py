#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the النظام الأساس لمؤسسة مركز الرياض للتقنية الحيوية track.
...
RECORDS = os.path.join(ROOT, "sources", "riyadh_biotechnology_center_statute", "verified",
                       "riyadh_biotechnology_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "riyadh_biotechnology_center_statute", "verified",
                       "riyadh_biotechnology_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "riyadh_biotechnology_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"riyadh_biotechnology_center_statute_no_decision_number_in_source", "riyadh_biotechnology_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "25/4/1447":
        e.append("[2j] gazette_publication_date_hijri must be 25/4/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-10-17":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-10-17")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "النظام الأساس لمؤسسة مركز الرياض للتقنية الحيوية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Riyadh Center for Biotechnology Foundation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Riyadh Center for Biotechnology Foundation")
