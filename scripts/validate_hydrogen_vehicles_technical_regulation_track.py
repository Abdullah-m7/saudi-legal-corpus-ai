#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الفنية للمركبات الهيدروجينية track.
...
RECORDS = os.path.join(ROOT, "sources", "hydrogen_vehicles_technical_regulation", "verified",
                       "hydrogen_vehicles_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "hydrogen_vehicles_technical_regulation", "verified",
                       "hydrogen_vehicles_technical_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "hydrogen_vehicles_technical_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"hydrogen_vehicles_technical_regulation_no_decision_number_in_source", "hydrogen_vehicles_technical_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة الفنية للمركبات الهيدروجينية":
    ...
    if e:
        print("FAIL: %d error(s) in Technical Regulation for Hydrogen Vehicles track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Technical Regulation for Hydrogen Vehicles")
