#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate the Batch P2-003 QA (P2 expansion QA; article-by-article vs the official Arabic; Babs 7/8/9/10).

Confirms the QA covers exactly the 20 P2-003 articles, that every per-article bab is in [7,8,9,10] and
equals both the P2-003 remediation record bab and the coverage-index expected_bab_number, that
expected_bab_distribution matches the authorized split, that counts/final_status stay consistent, that
source traceability + the P2 backlog finding are re-verified for each pass, that the QA carries the
internal / non-official / non-binding / non-governing posture, points at the P2-003 remediation file,
embeds no full text, and touches no protected layer (P2-003 data, P2-001 + P2-002 data + QA, all P1 + P0
batches + QA, and base corpora). Review only. No p3_* dirs; no full-281 / trilingual. Read-only.

Usage: validate_chinese_remediation_batch_p2_003_qa.py [QA_JSON_PATH]
...
QA_DEFAULT = os.path.join(ROOT, "reports", "chinese_translation_review",
                          "chinese_remediation_batch_p2_003_qa.json")
MD = os.path.join(ROOT, "reports", "chinese_translation_review",
                  "CHINESE_REMEDIATION_BATCH_P2_003_QA_AR.md")
SRC_REL = "data/chinese_remediation_batches/p2_003/companies_law_m132_1443_zh_internal_remediation_p2_003.json"
...
ARTS = [187, 193, 195, 196, 197, 198, 199, 202, 203, 204, 208, 209, 210, 214, 215, 216, 217, 219, 223, 224]
DIST = {'7': [187, 193, 195, 196], '8': [197, 198, 199, 202, 203, 204, 208, 209, 210, 214, 215], '9': [216, 217, 219], '10': [223, 224]}
BABS = set([7, 8, 9, 10])
...
REQ_TOP = ("stage", "qa_stage", "batch_id", "source_batch_file", "scope_articles", "expected_babs",
           ...
           "trilingual_alignment_created", "p2_003_chinese_text_modified", "p2_003_data_modified",
...
def main(argv=None) -> int:
    ...
    problems = []
    for p, label in ((qa_path, "QA JSON"), (MD, "Arabic QA report"), (SRC, "P2-003 remediation file")):
    ...
    if qa.get("stage") != "CHINESE_REMEDIATION_BATCH_P2_003_QA":
        problems.append("stage must be CHINESE_REMEDIATION_BATCH_P2_003_QA")
    if qa.get("qa_stage") != "CHINESE_REMEDIATION_BATCH_P2_003_QA":
        problems.append("qa_stage must be CHINESE_REMEDIATION_BATCH_P2_003_QA")
    if qa.get("batch_id") != "P2-003":
        problems.append("batch_id must be P2-003")
    if qa.get("source_batch_file") != SRC_REL:
        problems.append("source_batch_file must point to the P2-003 remediation JSON")
    if qa.get("scope_articles") != ARTS:
        problems.append("scope_articles must exactly match the P2-003 list")
    if qa.get("article_count") != 20:
        problems.append("article_count must be 20")
    if qa.get("expected_babs") != [7, 8, 9, 10]:
        problems.append("expected_babs must be [7, 8, 9, 10]")
    dist = qa.get("expected_bab_distribution") or {}
    if {str(k): list(v) for k, v in dist.items()} != DIST:
        problems.append("expected_bab_distribution must match the authorized split")
    ...
    if nums != ARTS:
        problems.append("per_article_reviews must cover exactly the 20 P2-003 articles in order")
    ...
    for r in recs:
        ...
        if r.get("bab") not in BABS:
            problems.append("art %s bab must be in [7,8,9,10]" % n)
        if n in srec and r.get("bab") != srec[n].get("bab"):
            problems.append("art %s QA bab %r != P2-003 record bab %r" % (n, r.get("bab"), srec[n].get("bab")))
    ...
    if qa.get("p2_003_chinese_text_modified") is True or qa.get("p2_003_data_modified") is True:
        if not fixes:
            problems.append("p2_003 marked modified but no minor_fixes recorded")
    else:
        if fixes:
            problems.append("minor_fixes recorded but p2_003 marked unmodified")
    ...
    if len(src["records"]) != 20:
        problems.append("P2-003 remediation must remain 20 records")
    for label, (sub, fn, cnt) in PRIOR_P2.items():
    ...
    print("=" * 60)
    print("Chinese remediation Batch P2-003 QA validation (P2 expansion QA)")
    print("=" * 60)
    if problems:
        for p in problems:
            print("  -", p)
        print("RESULT: %d problem(s) found ✗" % len(problems))
        return 1
    print("[PASS] Batch P2-003 QA: 20 articles across Babs [7,8,9,10] reviewed article-by-article vs the "
          "official Arabic (English secondary; prior condensed candidate as the expansion baseline); "
          "each QA bab matches the P2-003 record and the coverage index; for every pass, source "
          "traceability and the P2 backlog finding are re-verified and expansion-faithful / "
          "no-hallucination / no-omission / no-over-expansion / approved-for-future-layer are recorded; "
          "counts consistent; internal/non-official/non-binding/non-governing; review-only (P2-003 data "
          "unmodified); P2-001 + P2-002 data + QA + all P1 + P0 batches + QA + base corpora unchanged; "
          "no p3_* dirs; no full-281 / trilingual.")
