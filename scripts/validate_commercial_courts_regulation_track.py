#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Implementing Regulation of the Commercial Courts Law (281 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) EXCEPT the 8 visually-adjudicated numbered-list articles (3, 41,
55, 90, 144, 155, 255, 267) confirmed verbatim on the rendered pages; the numbered
sequence is a complete 1..281 (no مكرر); and every article is اصلية (this is a
fresh full 1441H issuance) with its is_repealed/is_amended/is_added flags all
False. The section-API status equals the PDF status for every article (no
...
RECORDS = os.path.join(ROOT, "sources", "commercial_courts", "regulation", "verified",
                       "commercial_courts_regulation_verified_records.jsonl")
...
PDF = os.path.join(ROOT, "inputs", "commercial_courts_official_pdfs",
                   "commercial_courts_regulation_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 281
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 281}
VISUALLY_ADJUDICATED = {"commercial_courts_reg_art_%03d" % n for n in (3, 41, 55, 90, 144, 155, 255, 267)}
...
def main():
    ...
    # [1] structure: complete 1..281, no mukarrar
    nums = sorted(int(re.match(r"commercial_courts_reg_art_(\d{3})", k).group(1)) for k in arts
    ...
    if e:
        print("FAIL: %d error(s) in Commercial Courts Regulation track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Implementing Regulation of the Commercial Courts Law — 281 records (fresh 1441H issuance, all اصلية)")
    print("  - trust gate: 273/281 MATCHES_PDF; 8 numbered-list articles (3,41,55,90,144,155,255,267) visually adjudicated verbatim")
    print("  - complete 1..281 (no مكرر); committed official MOJ PDF hash verified; no dual-status divergence")
