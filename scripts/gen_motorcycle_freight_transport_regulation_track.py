#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنشاط نقل البضائع عبر الدراجة الآلية لأغراض تجارية track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4001550),
published 24/2/1448H (2026-08-07). The Gazette IS the official publication of record
...
STRUCTURE: 29 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "motorcycle_freight_transport_regulation", "official_source",
                   "motorcycle_freight_transport_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "motorcycle_freight_transport_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "motorcycle_freight_transport_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "motorcycle_freight_transport_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "motorcycle_freight_transport_regulation_arabic_legal_llm",
                        "motorcycle_freight_transport_regulation_legal_llm_001_029.json")

LAW_ID = "sa-motorcycle-freight-transport-regulation"
LAW_AR = "اللائحة التنفيذية لنشاط نقل البضائع عبر الدراجة الآلية لأغراض تجارية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"motorcycle_freight_transport_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4001550), published 24/2/1448H (2026-08-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "motorcycle_freight_transport_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MOTORCYCLE_FREIGHT_TRANSPORT_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "motorcycle-freight-transport-regulation-llm-%03d" % idx,
            ...
            "article_path": "motorcycle_freight_transport_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation for Commercial Freight Transport by Motorcycle -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/2/1448H / 2026-08-07)"),
                "source_authority_ar": ("اللائحة التنفيذية لنشاط نقل البضائع عبر الدراجة الآلية لأغراض تجارية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 24/2/1448هـ الموافق 2026-08-07م)"),
    ...
    json.dump({
        "law_key": "motorcycle_freight_transport_regulation",
        "layer": "MOTORCYCLE_FREIGHT_TRANSPORT_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "motorcycle-freight-transport-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (29 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation for Commercial Freight Transport by Motorcycle — Arabic LLM-ready layer (29 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation for Commercial Freight Transport by Motorcycle records" % (len(ver), len(llm)))
