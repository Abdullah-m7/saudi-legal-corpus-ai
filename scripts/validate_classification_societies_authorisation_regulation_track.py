#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة شروط تفويض هيئات التصنيف في المملكة العربية السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "classification_societies_authorisation_regulation", "verified",
                       "classification_societies_authorisation_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "classification_societies_authorisation_regulation", "verified",
                       "classification_societies_authorisation_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "classification_societies_authorisation_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"classification_societies_authorisation_regulation_no_decision_number_in_source", "classification_societies_authorisation_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "30/6/1445":
        e.append("[2j] gazette_publication_date_hijri must be 30/6/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-01-12":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-01-12")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة شروط تفويض هيئات التصنيف في المملكة العربية السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on the Conditions for Authorising Classification Societies in the Kingdom track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on the Conditions for Authorising Classification Societies in the Kingdom")
