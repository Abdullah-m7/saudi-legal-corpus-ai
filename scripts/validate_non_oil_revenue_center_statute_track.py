#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مركز الإيرادات غير النفطية track.
...
RECORDS = os.path.join(ROOT, "sources", "non_oil_revenue_center_statute", "verified",
                       "non_oil_revenue_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "non_oil_revenue_center_statute", "verified",
                       "non_oil_revenue_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "non_oil_revenue_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "non_oil_revenue_center_statute_found_by_walking_a_url_section_the_index_never_covered",
    "non_oil_revenue_center_statute_no_decision_number_in_source",
    "non_oil_revenue_center_statute_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم مركز الإيرادات غير النفطية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Non-Oil Revenue Center track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Non-Oil Revenue Center")
