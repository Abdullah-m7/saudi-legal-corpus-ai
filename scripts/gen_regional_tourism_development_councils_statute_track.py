#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مجالس التنمية السياحية في المناطق track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19346),
...
SRC = os.path.join(ROOT, "sources", "regional_tourism_development_councils_statute", "official_source",
                   "regional_tourism_development_councils_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "regional_tourism_development_councils_statute", "verified")
RECORDS = os.path.join(OUT_VER, "regional_tourism_development_councils_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "regional_tourism_development_councils_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "regional_tourism_development_councils_statute_arabic_legal_llm",
                        "regional_tourism_development_councils_statute_legal_llm_001_013.json")

LAW_ID = "sa-regional-tourism-development-councils-statute"
LAW_AR = "تنظيم مجالس التنمية السياحية في المناطق"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"regional_tourism_development_councils_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19346), published 21/9/1443H (2022-04-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "regional_tourism_development_councils_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "REGIONAL_TOURISM_DEVELOPMENT_COUNCILS_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "regional-tourism-development-councils-statute-llm-%03d" % idx,
            ...
            "article_path": "regional_tourism_development_councils_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Regional Tourism Development Councils -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/9/1443H / 2022-04-22)"),
                "source_authority_ar": ("تنظيم مجالس التنمية السياحية في المناطق — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "regional_tourism_development_councils_statute",
        "layer": "REGIONAL_TOURISM_DEVELOPMENT_COUNCILS_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "regional-tourism-development-councils-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Statute of the Regional Tourism Development Councils — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Regional Tourism Development Councils records" % (len(ver), len(llm)))
