#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد الأسماء التجارية في المناطق الاقتصادية الخاصة track.
...
RECORDS = os.path.join(ROOT, "sources", "sez_trade_names_rules", "verified",
                       "sez_trade_names_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "sez_trade_names_rules", "verified",
                       "sez_trade_names_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "sez_trade_names_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"sez_trade_names_rules_no_decision_number_in_source", "sez_trade_names_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد الأسماء التجارية في المناطق الاقتصادية الخاصة":
    ...
    if e:
        print("FAIL: %d error(s) in Trade Names Rules in the Special Economic Zones track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Trade Names Rules in the Special Economic Zones")
