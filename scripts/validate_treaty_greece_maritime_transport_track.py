#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية تعاون بين حكومتي المملكة واليونان في مجال النقل البحري track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_greece_maritime_transport", "verified",
                       "treaty_greece_maritime_transport_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_greece_maritime_transport", "verified",
                       "treaty_greece_maritime_transport_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_greece_maritime_transport_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_greece_maritime_transport_no_decision_number_in_source", "treaty_greece_maritime_transport_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية تعاون بين حكومتي المملكة واليونان في مجال النقل البحري":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Greece Maritime Transport Cooperation Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Greece Maritime Transport Cooperation Agreement")
