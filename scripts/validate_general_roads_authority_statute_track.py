#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية للهيئة العامة للطرق track.
...
RECORDS = os.path.join(ROOT, "sources", "general_roads_authority_statute", "verified",
                       "general_roads_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "general_roads_authority_statute", "verified",
                       "general_roads_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "general_roads_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"general_roads_authority_statute_no_decision_number_in_source", "general_roads_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية للهيئة العامة للطرق":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the General Authority for Roads track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the General Authority for Roads")
