#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the Chambers of Commerce Law track (نظام الغرف التجارية م/37).
...
independent-date snapshots (2025-05 + 2026-01): all 66 article bodies are
byte-identical between them (zero differences). Both raw snapshots are committed
under inputs/chambers_of_commerce_boe_snapshots/ with recorded sha256, and the
concatenated corpus text carries a recorded sha256.

Royal Decree M/37 dated 22/4/1442H. IN FORCE (ساري). Fresh consolidated issuance:
all 66 اصلية (0 معدلة / 0 ملغاة / 0 مضافة).
...
OUT_VER = os.path.join(ROOT, "sources", "chambers_of_commerce", "law", "verified")
RECORDS = os.path.join(OUT_VER, "chambers_of_commerce_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "chambers_of_commerce_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "chambers_of_commerce_arabic_legal_llm",
                        "chambers_of_commerce_law_legal_llm_001_066.json")

LAW_ID = "sa-chambers-of-commerce-law-m37-1442"
LAW_AR = "نظام الغرف التجارية"
CORPUS = "chambers_of_commerce"
STATUS = "BOE_OFFICIAL_PORTAL_ARCHIVE_CROSS_SNAPSHOT_VERIFIED"
KEY_RE = r"chambers_of_commerce_art_(\d{3})(_mukarrar)?$"
LLM_FILE_BASENAME = "chambers_of_commerce_law_legal_llm_001_066.json"
N_ARTICLES = 66
RECID_PREFIX = "chambers-of-commerce-law-llm-art"
SEARCH_TERM = "نظام الغرف التجارية"
...
def main():
    ...
    for key in keys:
        ...
        ver.append({"law_key": CORPUS, "law_component": "law", "language": "ar",
                    "record_layer": "CHAMBERS_OF_COMMERCE_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({"law_key": CORPUS, "layer": "CHAMBERS_OF_COMMERCE_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({"layer_id": "sa-chambers-of-commerce-law-arabic-legal-llm-full", "law_id": LAW_ID,
               "law_component": "law",
               "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (66 مادة؛ إصدار كامل: 66 أصلية)",
               "title_en": "Saudi Chambers of Commerce Law — Arabic LLM-ready layer (66 records)",
               "record_type": "verified_arabic_article", "language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Chambers of Commerce Law records" % (len(ver), len(llm)))
