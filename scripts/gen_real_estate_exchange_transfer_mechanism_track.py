#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the آلية نقل أصول البورصة العقارية وموظفيها ومشاريعها ومبادراتها إلى الهيئة العامة للعقار track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4001545),
...
SRC = os.path.join(ROOT, "sources", "real_estate_exchange_transfer_mechanism", "official_source",
                   "real_estate_exchange_transfer_mechanism_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "real_estate_exchange_transfer_mechanism", "verified")
RECORDS = os.path.join(OUT_VER, "real_estate_exchange_transfer_mechanism_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "real_estate_exchange_transfer_mechanism_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "real_estate_exchange_transfer_mechanism_arabic_legal_llm",
                        "real_estate_exchange_transfer_mechanism_legal_llm_001_008.json")

LAW_ID = "sa-real-estate-exchange-transfer-mechanism"
LAW_AR = "آلية نقل أصول البورصة العقارية وموظفيها ومشاريعها ومبادراتها إلى الهيئة العامة للعقار"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"real_estate_exchange_transfer_mechanism_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4001545), published 24/2/1448H (2026-08-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "real_estate_exchange_transfer_mechanism",
            "law_component": "mechanism",
            "language": "ar",
            "record_layer": "REAL_ESTATE_EXCHANGE_TRANSFER_MECHANISM_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "real-estate-exchange-transfer-mechanism-llm-%03d" % idx,
            ...
            "article_path": "real_estate_exchange_transfer_mechanism/mechanism/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Mechanism for Transferring the Real Estate Exchange to the Real Estate General Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/2/1448H / 2026-08-07)"),
                "source_authority_ar": ("آلية نقل أصول البورصة العقارية وموظفيها ومشاريعها ومبادراتها إلى الهيئة العامة للعقار — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "real_estate_exchange_transfer_mechanism",
        "layer": "REAL_ESTATE_EXCHANGE_TRANSFER_MECHANISM_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "real-estate-exchange-transfer-mechanism-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "mechanism",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Mechanism for Transferring the Real Estate Exchange to the Real Estate General Authority — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Mechanism for Transferring the Real Estate Exchange to the Real Estate General Authority records" % (len(ver), len(llm)))
