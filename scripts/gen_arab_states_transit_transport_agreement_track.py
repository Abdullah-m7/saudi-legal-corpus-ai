#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية تنظيم النقل بالعبور «الترانزيت» بين الدول العربية (المعدلة) track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21346),
published 12/7/1444H (2023-02-03). The Gazette IS the official publication of record
...
STRUCTURE: 45 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "arab_states_transit_transport_agreement", "official_source",
                   "arab_states_transit_transport_agreement_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "arab_states_transit_transport_agreement", "verified")
RECORDS = os.path.join(OUT_VER, "arab_states_transit_transport_agreement_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "arab_states_transit_transport_agreement_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "arab_states_transit_transport_agreement_arabic_legal_llm",
                        "arab_states_transit_transport_agreement_legal_llm_001_045.json")

LAW_ID = "sa-arab-states-transit-transport-agreement"
LAW_AR = "اتفاقية تنظيم النقل بالعبور «الترانزيت» بين الدول العربية (المعدلة)"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"arab_states_transit_transport_agreement_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21346), published 12/7/1444H (2023-02-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "arab_states_transit_transport_agreement",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "ARAB_STATES_TRANSIT_TRANSPORT_AGREEMENT_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "arab-states-transit-transport-agreement-llm-%03d" % idx,
            ...
            "article_path": "arab_states_transit_transport_agreement/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Agreement on the Regulation of Transit Transport among Arab States (Amended) -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 12/7/1444H / 2023-02-03)"),
                "source_authority_ar": ("اتفاقية تنظيم النقل بالعبور «الترانزيت» بين الدول العربية (المعدلة) — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 12/7/1444هـ الموافق 2023-02-03م)"),
    ...
    json.dump({
        "law_key": "arab_states_transit_transport_agreement",
        "layer": "ARAB_STATES_TRANSIT_TRANSPORT_AGREEMENT_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "arab-states-transit-transport-agreement-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (45 مادة، جميعها أصلية)",
        "title_en": "Agreement on the Regulation of Transit Transport among Arab States (Amended) — Arabic LLM-ready layer (45 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Agreement on the Regulation of Transit Transport among Arab States (Amended) records" % (len(ver), len(llm)))
