#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد والإجراءات التنفيذية اللازمة لتطبيق تعريفة استهلاك الكهرباء لنشاط الحوسبة السحابية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17472),
published 4/9/1442H (2021-04-16). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "cloud_computing_electricity_tariff_executive_rules", "official_source",
                   "cloud_computing_electricity_tariff_executive_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "cloud_computing_electricity_tariff_executive_rules", "verified")
RECORDS = os.path.join(OUT_VER, "cloud_computing_electricity_tariff_executive_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "cloud_computing_electricity_tariff_executive_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "cloud_computing_electricity_tariff_executive_rules_arabic_legal_llm",
                        "cloud_computing_electricity_tariff_executive_rules_legal_llm_001_008.json")

LAW_ID = "sa-cloud-computing-electricity-tariff-executive-rules"
LAW_AR = "القواعد والإجراءات التنفيذية اللازمة لتطبيق تعريفة استهلاك الكهرباء لنشاط الحوسبة السحابية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"cloud_computing_electricity_tariff_executive_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17472), published 4/9/1442H (2021-04-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "cloud_computing_electricity_tariff_executive_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "CLOUD_COMPUTING_ELECTRICITY_TARIFF_EXECUTIVE_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "cloud-computing-electricity-tariff-executive-rules-llm-%03d" % idx,
            ...
            "article_path": "cloud_computing_electricity_tariff_executive_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Rules and Procedures for Applying the Electricity Consumption Tariff to Cloud Computing Activity -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/9/1442H / 2021-04-16)"),
                "source_authority_ar": ("القواعد والإجراءات التنفيذية اللازمة لتطبيق تعريفة استهلاك الكهرباء لنشاط الحوسبة السحابية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 4/9/1442هـ الموافق 2021-04-16م)"),
    ...
    json.dump({
        "law_key": "cloud_computing_electricity_tariff_executive_rules",
        "layer": "CLOUD_COMPUTING_ELECTRICITY_TARIFF_EXECUTIVE_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "cloud-computing-electricity-tariff-executive-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Executive Rules and Procedures for Applying the Electricity Consumption Tariff to Cloud Computing Activity — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Rules and Procedures for Applying the Electricity Consumption Tariff to Cloud Computing Activity records" % (len(ver), len(llm)))
