#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية عامة للتعاون بين حكومتي المملكة وإستونيا track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_estonia_general_cooperation", "verified",
                       "treaty_estonia_general_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_estonia_general_cooperation", "verified",
                       "treaty_estonia_general_cooperation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_estonia_general_cooperation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_estonia_general_cooperation_no_decision_number_in_source", "treaty_estonia_general_cooperation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "25/8/1447":
        e.append("[2j] gazette_publication_date_hijri must be 25/8/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-02-13":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-02-13")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية عامة للتعاون بين حكومتي المملكة وإستونيا":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Estonia General Cooperation Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Estonia General Cooperation Agreement")
