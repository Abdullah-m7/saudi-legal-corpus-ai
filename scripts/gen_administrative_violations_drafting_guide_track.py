#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الدليل الاسترشادي لاقتراح سن أحكام المخالفة الإدارية وجزائها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18348),
published 17/2/1443H (2021-09-24). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "administrative_violations_drafting_guide", "official_source",
                   "administrative_violations_drafting_guide_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "administrative_violations_drafting_guide", "verified")
RECORDS = os.path.join(OUT_VER, "administrative_violations_drafting_guide_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "administrative_violations_drafting_guide_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "administrative_violations_drafting_guide_arabic_legal_llm",
                        "administrative_violations_drafting_guide_legal_llm_001_006.json")

LAW_ID = "sa-administrative-violations-drafting-guide"
LAW_AR = "الدليل الاسترشادي لاقتراح سن أحكام المخالفة الإدارية وجزائها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"administrative_violations_drafting_guide_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18348), published 17/2/1443H (2021-09-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "administrative_violations_drafting_guide",
            "law_component": "guide",
            "language": "ar",
            "record_layer": "ADMINISTRATIVE_VIOLATIONS_DRAFTING_GUIDE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "administrative-violations-drafting-guide-llm-%03d" % idx,
            ...
            "article_path": "administrative_violations_drafting_guide/guide/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Guidance Manual for Proposing Administrative Violation Provisions and Their Penalties -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/2/1443H / 2021-09-24)"),
                "source_authority_ar": ("الدليل الاسترشادي لاقتراح سن أحكام المخالفة الإدارية وجزائها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 17/2/1443هـ الموافق 2021-09-24م)"),
    ...
    json.dump({
        "law_key": "administrative_violations_drafting_guide",
        "layer": "ADMINISTRATIVE_VIOLATIONS_DRAFTING_GUIDE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "administrative-violations-drafting-guide-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "guide",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (6 مادة، جميعها أصلية)",
        "title_en": "Guidance Manual for Proposing Administrative Violation Provisions and Their Penalties — Arabic LLM-ready layer (6 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Guidance Manual for Proposing Administrative Violation Provisions and Their Penalties records" % (len(ver), len(llm)))
