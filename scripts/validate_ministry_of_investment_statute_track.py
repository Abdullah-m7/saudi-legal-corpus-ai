#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم وزارة الاستثمار track.
...
RECORDS = os.path.join(ROOT, "sources", "ministry_of_investment_statute", "verified",
                       "ministry_of_investment_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "ministry_of_investment_statute", "verified",
                       "ministry_of_investment_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "ministry_of_investment_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"ministry_of_investment_statute_no_decision_number_in_source", "ministry_of_investment_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم وزارة الاستثمار":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Ministry of Investment track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Ministry of Investment")
