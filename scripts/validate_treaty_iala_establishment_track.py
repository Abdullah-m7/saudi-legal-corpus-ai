#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية بشأن إنشاء المنظمة الدولية للمساعدات الملاحية البحرية track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_iala_establishment", "verified",
                       "treaty_iala_establishment_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_iala_establishment", "verified",
                       "treaty_iala_establishment_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_iala_establishment_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_iala_establishment_no_decision_number_in_source", "treaty_iala_establishment_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "13/2/1444":
        e.append("[2j] gazette_publication_date_hijri must be 13/2/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-09-09":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-09-09")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية بشأن إنشاء المنظمة الدولية للمساعدات الملاحية البحرية":
    ...
    if e:
        print("FAIL: %d error(s) in Convention on the Establishment of the International Organization for Marine Aids to Navigation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Convention on the Establishment of the International Organization for Marine Aids to Navigation")
