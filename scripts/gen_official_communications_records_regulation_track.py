#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة الاتصالات الرسمية والمحافظة على الوثائق ومعلوماتها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24487),
published 28/7/1445H (2024-02-09). The Gazette IS the official publication of record
...
STRUCTURE: 87 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "official_communications_records_regulation", "official_source",
                   "official_communications_records_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "official_communications_records_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "official_communications_records_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "official_communications_records_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "official_communications_records_regulation_arabic_legal_llm",
                        "official_communications_records_regulation_legal_llm_001_087.json")

LAW_ID = "sa-official-communications-records-regulation"
LAW_AR = "لائحة الاتصالات الرسمية والمحافظة على الوثائق ومعلوماتها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"official_communications_records_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24487), published 28/7/1445H (2024-02-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "official_communications_records_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "OFFICIAL_COMMUNICATIONS_RECORDS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "official-communications-records-regulation-llm-%03d" % idx,
            ...
            "article_path": "official_communications_records_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Official Communications and the Preservation of Records and their Information -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/7/1445H / 2024-02-09)"),
                "source_authority_ar": ("لائحة الاتصالات الرسمية والمحافظة على الوثائق ومعلوماتها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 28/7/1445هـ الموافق 2024-02-09م)"),
    ...
    json.dump({
        "law_key": "official_communications_records_regulation",
        "layer": "OFFICIAL_COMMUNICATIONS_RECORDS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "official-communications-records-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (87 مادة، جميعها أصلية)",
        "title_en": "Regulation on Official Communications and the Preservation of Records and their Information — Arabic LLM-ready layer (87 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Official Communications and the Preservation of Records and their Information records" % (len(ver), len(llm)))
