#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام البيئة للإدارة المستدامة للبيئة البحرية والساحلية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18271),
published 3/2/1443H (2021-09-10). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "marine_coastal_environment_regulation", "official_source",
                   "marine_coastal_environment_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "marine_coastal_environment_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "marine_coastal_environment_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "marine_coastal_environment_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "marine_coastal_environment_regulation_arabic_legal_llm",
                        "marine_coastal_environment_regulation_legal_llm_001_007.json")

LAW_ID = "sa-marine-coastal-environment-regulation"
LAW_AR = "اللائحة التنفيذية لنظام البيئة للإدارة المستدامة للبيئة البحرية والساحلية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"marine_coastal_environment_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18271), published 3/2/1443H (2021-09-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "marine_coastal_environment_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MARINE_COASTAL_ENVIRONMENT_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "marine-coastal-environment-regulation-llm-%03d" % idx,
            ...
            "article_path": "marine_coastal_environment_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Environment Law for the Sustainable Management of the Marine and Coastal Environment -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/2/1443H / 2021-09-10)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام البيئة للإدارة المستدامة للبيئة البحرية والساحلية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 3/2/1443هـ الموافق 2021-09-10م)"),
    ...
    json.dump({
        "law_key": "marine_coastal_environment_regulation",
        "layer": "MARINE_COASTAL_ENVIRONMENT_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "marine-coastal-environment-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (7 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Environment Law for the Sustainable Management of the Marine and Coastal Environment — Arabic LLM-ready layer (7 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Environment Law for the Sustainable Management of the Marine and Coastal Environment records" % (len(ver), len(llm)))
