#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة سلوكيات السوق المالية track.
...
RECORDS = os.path.join(ROOT, "sources", "capital_market_conduct_regulation", "verified",
                       "capital_market_conduct_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "capital_market_conduct_regulation", "verified",
                       "capital_market_conduct_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "capital_market_conduct_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"capital_market_conduct_regulation_no_decision_number_in_source", "capital_market_conduct_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "2/10/1442":
        e.append("[2j] gazette_publication_date_hijri must be 2/10/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-05-14":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-05-14")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة سلوكيات السوق المالية":
    ...
    if e:
        print("FAIL: %d error(s) in Market Conduct Regulations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Market Conduct Regulations")
