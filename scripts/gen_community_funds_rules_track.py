#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة للصناديق الأهلية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25321),
...
SRC = os.path.join(ROOT, "sources", "community_funds_rules", "official_source",
                   "community_funds_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "community_funds_rules", "verified")
RECORDS = os.path.join(OUT_VER, "community_funds_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "community_funds_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "community_funds_rules_arabic_legal_llm",
                        "community_funds_rules_legal_llm_001_010.json")

LAW_ID = "sa-community-funds-rules"
LAW_AR = "القواعد المنظمة للصناديق الأهلية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"community_funds_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25321), published 5/2/1446H (2024-08-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "community_funds_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "COMMUNITY_FUNDS_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "community-funds-rules-llm-%03d" % idx,
            ...
            "article_path": "community_funds_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing Community Funds -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/2/1446H / 2024-08-09)"),
                "source_authority_ar": ("القواعد المنظمة للصناديق الأهلية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "community_funds_rules",
        "layer": "COMMUNITY_FUNDS_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "community-funds-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Rules Governing Community Funds — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing Community Funds records" % (len(ver), len(llm)))
