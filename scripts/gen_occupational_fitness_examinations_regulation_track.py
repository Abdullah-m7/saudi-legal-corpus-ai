#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة فحوصات اللياقة المهنية والأمراض غير المعدية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28551),
published 2/5/1447H (2025-10-24). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "occupational_fitness_examinations_regulation", "official_source",
                   "occupational_fitness_examinations_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "occupational_fitness_examinations_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "occupational_fitness_examinations_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "occupational_fitness_examinations_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "occupational_fitness_examinations_regulation_arabic_legal_llm",
                        "occupational_fitness_examinations_regulation_legal_llm_001_021.json")

LAW_ID = "sa-occupational-fitness-examinations-regulation"
LAW_AR = "لائحة فحوصات اللياقة المهنية والأمراض غير المعدية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"occupational_fitness_examinations_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28551), published 2/5/1447H (2025-10-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "occupational_fitness_examinations_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "OCCUPATIONAL_FITNESS_EXAMINATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "occupational-fitness-examinations-regulation-llm-%03d" % idx,
            ...
            "article_path": "occupational_fitness_examinations_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Occupational Fitness Examinations and Non-Communicable Diseases -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/5/1447H / 2025-10-24)"),
                "source_authority_ar": ("لائحة فحوصات اللياقة المهنية والأمراض غير المعدية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 2/5/1447هـ الموافق 2025-10-24م)"),
    ...
    json.dump({
        "law_key": "occupational_fitness_examinations_regulation",
        "layer": "OCCUPATIONAL_FITNESS_EXAMINATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "occupational-fitness-examinations-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (21 مادة، جميعها أصلية)",
        "title_en": "Regulation on Occupational Fitness Examinations and Non-Communicable Diseases — Arabic LLM-ready layer (21 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Occupational Fitness Examinations and Non-Communicable Diseases records" % (len(ver), len(llm)))
