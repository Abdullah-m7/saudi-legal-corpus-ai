#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام الأساس لمؤسسة مركز الرياض للتقنية الحيوية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27534),
...
SRC = os.path.join(ROOT, "sources", "riyadh_biotechnology_center_statute", "official_source",
                   "riyadh_biotechnology_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "riyadh_biotechnology_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "riyadh_biotechnology_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "riyadh_biotechnology_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "riyadh_biotechnology_center_statute_arabic_legal_llm",
                        "riyadh_biotechnology_center_statute_legal_llm_001_019.json")

LAW_ID = "sa-riyadh-biotechnology-center-statute"
LAW_AR = "النظام الأساس لمؤسسة مركز الرياض للتقنية الحيوية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"riyadh_biotechnology_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27534), published 25/4/1447H (2025-10-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "riyadh_biotechnology_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "RIYADH_BIOTECHNOLOGY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "riyadh-biotechnology-center-statute-llm-%03d" % idx,
            ...
            "article_path": "riyadh_biotechnology_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Riyadh Center for Biotechnology Foundation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/4/1447H / 2025-10-17)"),
                "source_authority_ar": ("النظام الأساس لمؤسسة مركز الرياض للتقنية الحيوية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "riyadh_biotechnology_center_statute",
        "layer": "RIYADH_BIOTECHNOLOGY_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "riyadh-biotechnology-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (19 مادة، جميعها أصلية)",
        "title_en": "Statute of the Riyadh Center for Biotechnology Foundation — Arabic LLM-ready layer (19 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Riyadh Center for Biotechnology Foundation records" % (len(ver), len(llm)))
