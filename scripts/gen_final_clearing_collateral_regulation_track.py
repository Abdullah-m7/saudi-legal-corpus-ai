#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة المقاصة النهائية وترتيبات الضمان المرتبطة بها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27415),
...
SRC = os.path.join(ROOT, "sources", "final_clearing_collateral_regulation", "official_source",
                   "final_clearing_collateral_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "final_clearing_collateral_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "final_clearing_collateral_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "final_clearing_collateral_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "final_clearing_collateral_regulation_arabic_legal_llm",
                        "final_clearing_collateral_regulation_legal_llm_001_015.json")

LAW_ID = "sa-final-clearing-collateral-regulation"
LAW_AR = "لائحة المقاصة النهائية وترتيبات الضمان المرتبطة بها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"final_clearing_collateral_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27415), published 13/3/1447H (2025-09-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "final_clearing_collateral_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "FINAL_CLEARING_COLLATERAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "final-clearing-collateral-regulation-llm-%03d" % idx,
            ...
            "article_path": "final_clearing_collateral_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Final Clearing and its Associated Collateral Arrangements -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/3/1447H / 2025-09-05)"),
                "source_authority_ar": ("لائحة المقاصة النهائية وترتيبات الضمان المرتبطة بها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "final_clearing_collateral_regulation",
        "layer": "FINAL_CLEARING_COLLATERAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "final-clearing-collateral-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Regulation on Final Clearing and its Associated Collateral Arrangements — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Final Clearing and its Associated Collateral Arrangements records" % (len(ver), len(llm)))
