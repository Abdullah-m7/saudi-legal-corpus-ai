#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط رسوم تخزين البضائع بالمنافذ الجمركية البرية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20299),
published 11/3/1444H (2022-10-07). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "land_customs_storage_fees_controls", "official_source",
                   "land_customs_storage_fees_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "land_customs_storage_fees_controls", "verified")
RECORDS = os.path.join(OUT_VER, "land_customs_storage_fees_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "land_customs_storage_fees_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "land_customs_storage_fees_controls_arabic_legal_llm",
                        "land_customs_storage_fees_controls_legal_llm_001_011.json")

LAW_ID = "sa-land-customs-storage-fees-controls"
LAW_AR = "ضوابط رسوم تخزين البضائع بالمنافذ الجمركية البرية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"land_customs_storage_fees_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20299), published 11/3/1444H (2022-10-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "land_customs_storage_fees_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "LAND_CUSTOMS_STORAGE_FEES_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "land-customs-storage-fees-controls-llm-%03d" % idx,
            ...
            "article_path": "land_customs_storage_fees_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on Goods Storage Fees at Land Customs Ports -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 11/3/1444H / 2022-10-07)"),
                "source_authority_ar": ("ضوابط رسوم تخزين البضائع بالمنافذ الجمركية البرية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 11/3/1444هـ الموافق 2022-10-07م)"),
    ...
    json.dump({
        "law_key": "land_customs_storage_fees_controls",
        "layer": "LAND_CUSTOMS_STORAGE_FEES_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "land-customs-storage-fees-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Controls on Goods Storage Fees at Land Customs Ports — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on Goods Storage Fees at Land Customs Ports records" % (len(ver), len(llm)))
