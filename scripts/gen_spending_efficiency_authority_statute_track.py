#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم هيئة كفاءة الإنفاق والمشروعات الحكومية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17029),
...
SRC = os.path.join(ROOT, "sources", "spending_efficiency_authority_statute", "official_source",
                   "spending_efficiency_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "spending_efficiency_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "spending_efficiency_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "spending_efficiency_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "spending_efficiency_authority_statute_arabic_legal_llm",
                        "spending_efficiency_authority_statute_legal_llm_001_014.json")

LAW_ID = "sa-spending-efficiency-authority-statute"
LAW_AR = "تنظيم هيئة كفاءة الإنفاق والمشروعات الحكومية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"spending_efficiency_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17029), published 21/7/1442H (2021-03-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "spending_efficiency_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "SPENDING_EFFICIENCY_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "spending-efficiency-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "spending_efficiency_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Spending Efficiency and Government Projects Authority — Statute -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/7/1442H / 2021-03-05)"),
                "source_authority_ar": ("تنظيم هيئة كفاءة الإنفاق والمشروعات الحكومية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "spending_efficiency_authority_statute",
        "layer": "SPENDING_EFFICIENCY_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "spending-efficiency-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Spending Efficiency and Government Projects Authority — Statute — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Spending Efficiency and Government Projects Authority — Statute records" % (len(ver), len(llm)))
