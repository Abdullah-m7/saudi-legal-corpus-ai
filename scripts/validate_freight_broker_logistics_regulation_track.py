#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة المنظمة لنشاط وسيط الشحن (الخدمات اللوجستية) track.
...
RECORDS = os.path.join(ROOT, "sources", "freight_broker_logistics_regulation", "verified",
                       "freight_broker_logistics_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "freight_broker_logistics_regulation", "verified",
                       "freight_broker_logistics_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "freight_broker_logistics_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"freight_broker_logistics_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "28/3/1445":
        e.append("[2j] gazette_publication_date_hijri must be 28/3/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-10-13":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-10-13")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة المنظمة لنشاط وسيط الشحن (الخدمات اللوجستية)":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation Governing Freight Brokerage (Logistics Services) track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation Governing Freight Brokerage (Logistics Services)")
