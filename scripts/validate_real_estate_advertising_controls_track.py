#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط الإعلانات العقارية track.
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_advertising_controls", "verified",
                       "real_estate_advertising_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "real_estate_advertising_controls", "verified",
                       "real_estate_advertising_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "real_estate_advertising_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"real_estate_advertising_controls_no_decision_number_in_source", "real_estate_advertising_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "ضوابط الإعلانات العقارية":
    ...
    if e:
        print("FAIL: %d error(s) in Real Estate Advertising Controls track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Real Estate Advertising Controls")
