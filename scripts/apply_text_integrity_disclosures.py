...
VERIFICATION = os.path.join(ROOT, "reports", "corpus_text_integrity_audit",
                            "pdf_source_encoding_verification.json")

# Common to every damaged track: what the fault is, and why it is not repaired.
DAMAGE_HEAD = (
    "**تحفُّظ على سلامة النص**: يحمل {n} سجلاً من {total} في هذا المسار أثر انقلاب رابطة "
    "لام-ألف في النص العربي — «االطلاع» موضع «الاطلاع»، و«اإلدارية» موضع «الإدارية»، "
    "و«إعالن» موضع «إعلان»، و«الالزمة» موضع «اللازمة».")

# Where the fault came from — asserted only where it was actually tested.
DAMAGE_VERIFIED = (
    "\n\n**وقد تُحقِّق من منشأ هذا الخلل، فهو في ملف المصدر الرسمي نفسه لا في قراءتنا له**: "
    "أُعيد جلب ملف الـPDF الذي يستشهد به هذا المسار ({pdf})، واستُخرج نصّه بأداة مستقلة "
    "تماماً عن كل ما بُني به هذا المستودع (poppler/pdftotext) وبأنماط التخطيط الثلاثة "
    "جميعها، فظهرت فيه **{hits}** موضعاً من مواضع العطب نفسها. أي أن **الطبقة النصية "
    "للملف الرسمي مُرمَّزة هكذا**، ولا يستطيع أي مستخرِج أن يُخرج منها أفضل من ذلك، وأن "
    "**النص المخزَّن هنا قراءةٌ أمينة للملف الرسمي**. والصفحة المطبوعة تبدو سليمةً لعين "
    "القارئ؛ العطب في ما تقرؤه الآلة وحده. (التحقق: scripts/verify_pdf_source_encoding.py، "
    "وتفصيله في reports/corpus_text_integrity_audit/pdf_source_encoding_verification.json)"
    "\n\n**وتصويبٌ لما قيل هنا سابقاً**: كان هذا التحفُّظ يقول إن «الجهة نشرت النص سليماً "
    "وإن الخلل في النقل». **وذلك خطأ**، قيل قبل إجراء هذا الاختبار، ويناقضه الاختبار.")

DAMAGE_UNTESTED = (
    "\n\n**ومنشأ الخلل لم يُختبر في هذا المسار**: لا يستشهد المسار بملف PDF بعينه يمكن "
    "إعادة جلبه واستخراجه، فلا يُقال هنا أهو في المصدر أم في القراءة. وقد ثبت في مسارين "
    "آخرين من مسارات هذا المستودع أن الطبقة النصية للملف الرسمي هي التي تحمله "
    "(scripts/verify_pdf_source_encoding.py)، وهي قرينة لا دليل على هذا المسار بعينه.")

DAMAGE_TAIL = (
    "\n\n**ولم يُصحَّح حرف واحد.** الانقلاب **يُحوِّل كلمات عربية صحيحة إلى كلمات عربية "
    "صحيحة أخرى**: «العالقة» (المُعلَّقة) قلبُ «العلاقة»، و«عالمة» (التي تعلم) قلبُ «علامة»، "
    "و«صالحية» قلبُ «صلاحية» — و«الشوائب **العالقة** ببدن السفينة» و«القضايا **العالقة** لدى "
    "المحامين» صوابٌ يقول ما يعنيه. فلا قاعدةَ آليةٌ تفصل العطب عن الصواب؛ الفصل يحتاج "
    "الجملة لا الرسم. وجُرِّب إصلاحٌ مشروطٌ بأن يكون الشكل المُصحَّح مشهوداً في نصوص المستودع "
    "السليمة فأخفق: الاستخراج يشطر الكلمة أيضاً («خ اال المدة» وأصلها «خلال المدة»)، وبعض "
    "«الشواهد» عطبٌ مماثل تسرَّب إلى مسارٍ آخر.\n\n"
    "المُفصَح عنه أن **رسم بعض الكلمات في هذه السجلات لا يطابق الرسم الصحيح** وإن بقي المعنى "
    "مقروءاً، فلا يصح النسخ الحرفي منها قبل المقابلة بالصفحة المطبوعة. ورفعُ هذا التحفُّظ "
    "يكون بنسخٍ بشريٍّ من الصفحة المُصيَّرة، لا بقاعدة ولا بإعادة استخراج. "
    "(رصدَه scripts/audit_corpus_text_integrity.py)")


def damage_text(n, total, verified):
    body = DAMAGE_HEAD.format(n=n, total=total)
    if verified:
        body += DAMAGE_VERIFIED.format(pdf=verified["pdf"], hits=verified["hits"])
    else:
        body += DAMAGE_UNTESTED
    return body + DAMAGE_TAIL
...
def main():
    ...
    verified = {}
    if os.path.isfile(VERIFICATION):
        for c in json.load(open(VERIFICATION, encoding="utf-8"))["checks"]:
            if c.get("verdict") == "damage_is_in_the_official_pdf":
                hits = max((m["damage_hits"] for m in c["extraction_modes"].values()), default=0)
                verified[c["track_id"]] = {"pdf": c["pdf"], "hits": hits}
    ...
    for row in rep["encoding_damage"]:
        ...
        note(put(os.path.join(ROOT, row["source_artifact"]),
                 ...
                 damage_text(row["records_affected"], row["total_records"],
                             verified.get(row["track_id"])),
                 True))
