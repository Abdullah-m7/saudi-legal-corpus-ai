#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة الفنية للمركبات المستوردة المستعملة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17922),
...
SRC = os.path.join(ROOT, "sources", "used_imported_vehicles_technical_regulation", "official_source",
                   "used_imported_vehicles_technical_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "used_imported_vehicles_technical_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "used_imported_vehicles_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "used_imported_vehicles_technical_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "used_imported_vehicles_technical_regulation_arabic_legal_llm",
                        "used_imported_vehicles_technical_regulation_legal_llm_001_011.json")

LAW_ID = "sa-used-imported-vehicles-technical-regulation"
LAW_AR = "اللائحة الفنية للمركبات المستوردة المستعملة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"used_imported_vehicles_technical_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17922), published 29/11/1442H (2021-07-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "used_imported_vehicles_technical_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "USED_IMPORTED_VEHICLES_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "used-imported-vehicles-technical-regulation-llm-%03d" % idx,
            ...
            "article_path": "used_imported_vehicles_technical_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Technical Regulation for Used Imported Vehicles -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 29/11/1442H / 2021-07-09)"),
                "source_authority_ar": ("اللائحة الفنية للمركبات المستوردة المستعملة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "used_imported_vehicles_technical_regulation",
        "layer": "USED_IMPORTED_VEHICLES_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "used-imported-vehicles-technical-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Technical Regulation for Used Imported Vehicles — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Technical Regulation for Used Imported Vehicles records" % (len(ver), len(llm)))
