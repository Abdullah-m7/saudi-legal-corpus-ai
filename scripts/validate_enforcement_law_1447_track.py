#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام التنفيذ track.

11 records: 65 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "enforcement_law_1447", "verified",
                       "enforcement_law_1447_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "enforcement_law_1447", "verified",
                       "enforcement_law_1447_verified_summary.json")
LLM = os.path.join(ROOT, "data", "enforcement_law_1447_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "enforcement_law_1447_found_by_walking_a_url_section_the_index_never_covered",
    "enforcement_law_1447_no_decision_number_in_source",
    "enforcement_law_1447_published_but_not_yet_in_force",
    "enforcement_law_1447_replaces_the_1433h_enforcement_law",
    "enforcement_law_1447_source_is_gazette_html_only",
    "enforcement_law_1447_the_implementing_regulation_is_not_yet_issued",
}
AR = "ء-ي"
...
def main():
    ...
    EXPECTED_NUMBERS = list(range(1, 66))
    MISSING_IN_SOURCE = []
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "14/11/1447":
        e.append("[2j] gazette_publication_date_hijri must be 14/11/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-05-01":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-05-01")
    if src.get("legal_status_ar") != "منشور ولم يبدأ العمل به بعد":
        e.append("[2j] legal_status_ar must be منشور ولم يبدأ العمل به بعد")
    if src.get("document") != "نظام التنفيذ":
    ...
    if e:
        print("FAIL: %d error(s) in Enforcement Law (1447H) track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Enforcement Law (1447H)")
    print("  - 65 records: 65 articles, no appendices; all 65 اصلية")
