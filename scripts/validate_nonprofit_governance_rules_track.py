#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد حوكمة الجمعيات والمؤسسات الأهلية track.
...
RECORDS = os.path.join(ROOT, "sources", "nonprofit_governance_rules", "verified",
                       "nonprofit_governance_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "nonprofit_governance_rules", "verified",
                       "nonprofit_governance_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "nonprofit_governance_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"nonprofit_governance_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/8/1447":
        e.append("[2j] gazette_publication_date_hijri must be 24/8/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-02-12":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-02-12")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد حوكمة الجمعيات والمؤسسات الأهلية":
    ...
    if e:
        print("FAIL: %d error(s) in Governance Rules for Associations and Non-Profit Foundations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Governance Rules for Associations and Non-Profit Foundations")
