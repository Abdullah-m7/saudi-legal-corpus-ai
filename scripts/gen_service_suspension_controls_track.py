#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط إيقاف الخدمات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21578),
published 25/8/1444H (2023-03-17). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "service_suspension_controls", "official_source",
                   "service_suspension_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "service_suspension_controls", "verified")
RECORDS = os.path.join(OUT_VER, "service_suspension_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "service_suspension_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "service_suspension_controls_arabic_legal_llm",
                        "service_suspension_controls_legal_llm_001_006.json")

LAW_ID = "sa-service-suspension-controls"
LAW_AR = "ضوابط إيقاف الخدمات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"service_suspension_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21578), published 25/8/1444H (2023-03-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "service_suspension_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SERVICE_SUSPENSION_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "service-suspension-controls-llm-%03d" % idx,
            ...
            "article_path": "service_suspension_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on the Suspension of Services -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/8/1444H / 2023-03-17)"),
                "source_authority_ar": ("ضوابط إيقاف الخدمات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 25/8/1444هـ الموافق 2023-03-17م)"),
    ...
    json.dump({
        "law_key": "service_suspension_controls",
        "layer": "SERVICE_SUSPENSION_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "service-suspension-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (6 مادة، جميعها أصلية)",
        "title_en": "Controls on the Suspension of Services — Arabic LLM-ready layer (6 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on the Suspension of Services records" % (len(ver), len(llm)))
