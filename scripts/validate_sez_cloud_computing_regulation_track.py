#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية للمنطقة الاقتصادية الخاصة بالحوسبة السحابية المعلوماتية track.
...
RECORDS = os.path.join(ROOT, "sources", "sez_cloud_computing_regulation", "verified",
                       "sez_cloud_computing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "sez_cloud_computing_regulation", "verified",
                       "sez_cloud_computing_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "sez_cloud_computing_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"sez_cloud_computing_regulation_no_decision_number_in_source", "sez_cloud_computing_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "27/7/1447":
        e.append("[2j] gazette_publication_date_hijri must be 27/7/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-01-16":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-01-16")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنظيمية للمنطقة الاقتصادية الخاصة بالحوسبة السحابية المعلوماتية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules of the Cloud Computing Special Economic Zone track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules of the Cloud Computing Special Economic Zone")
