#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط بقاء المركبات المسجلة في إحدى دول مجلس التعاون في السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "gcc_registered_vehicles_stay_controls", "verified",
                       "gcc_registered_vehicles_stay_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "gcc_registered_vehicles_stay_controls", "verified",
                       "gcc_registered_vehicles_stay_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "gcc_registered_vehicles_stay_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"gcc_registered_vehicles_stay_controls_no_decision_number_in_source", "gcc_registered_vehicles_stay_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "ضوابط بقاء المركبات المسجلة في إحدى دول مجلس التعاون في السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on the Stay in Saudi Arabia of Vehicles Registered in a GCC State track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on the Stay in Saudi Arabia of Vehicles Registered in a GCC State")
