#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مجالس التنمية السياحية في المناطق track.
...
RECORDS = os.path.join(ROOT, "sources", "regional_tourism_development_councils_statute", "verified",
                       "regional_tourism_development_councils_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "regional_tourism_development_councils_statute", "verified",
                       "regional_tourism_development_councils_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "regional_tourism_development_councils_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"regional_tourism_development_councils_statute_no_decision_number_in_source", "regional_tourism_development_councils_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Regional Tourism Development Councils track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Regional Tourism Development Councils")
