#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة تخصيص الطاقة track.
...
RECORDS = os.path.join(ROOT, "sources", "energy_allocation_regulation", "verified",
                       "energy_allocation_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "energy_allocation_regulation", "verified",
                       "energy_allocation_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "energy_allocation_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"energy_allocation_regulation_no_decision_number_in_source", "energy_allocation_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة تخصيص الطاقة":
    ...
    if e:
        print("FAIL: %d error(s) in Energy Allocation Regulation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Energy Allocation Regulation")
