#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام الأنشطة الترفيهية والأنشطة المساندة لها track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4001278),
...
SRC = os.path.join(ROOT, "sources", "entertainment_activities_law", "official_source",
                   "entertainment_activities_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "entertainment_activities_law", "verified")
RECORDS = os.path.join(OUT_VER, "entertainment_activities_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "entertainment_activities_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "entertainment_activities_law_arabic_legal_llm",
                        "entertainment_activities_law_legal_llm_001_016.json")

LAW_ID = "sa-entertainment-activities-law"
LAW_AR = "نظام الأنشطة الترفيهية والأنشطة المساندة لها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"entertainment_activities_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4001278), published 18/1/1448H (2026-07-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "entertainment_activities_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "ENTERTAINMENT_ACTIVITIES_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "entertainment-activities-law-llm-%03d" % idx,
            ...
            "article_path": "entertainment_activities_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law of Entertainment Activities and Supporting Activities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 18/1/1448H / 2026-07-03)"),
                "source_authority_ar": ("نظام الأنشطة الترفيهية والأنشطة المساندة لها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "entertainment_activities_law",
        "layer": "ENTERTAINMENT_ACTIVITIES_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "entertainment-activities-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Law of Entertainment Activities and Supporting Activities — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law of Entertainment Activities and Supporting Activities records" % (len(ver), len(llm)))
