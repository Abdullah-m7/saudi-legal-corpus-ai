#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لتنظيم الترخيص لمكاتب المحاماة الأجنبية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20150),
...
SRC = os.path.join(ROOT, "sources", "foreign_law_firms_licensing_regulation", "official_source",
                   "foreign_law_firms_licensing_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "foreign_law_firms_licensing_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "foreign_law_firms_licensing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "foreign_law_firms_licensing_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "foreign_law_firms_licensing_regulation_arabic_legal_llm",
                        "foreign_law_firms_licensing_regulation_legal_llm_001_024.json")

LAW_ID = "sa-foreign-law-firms-licensing-regulation"
LAW_AR = "اللائحة التنفيذية لتنظيم الترخيص لمكاتب المحاماة الأجنبية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"foreign_law_firms_licensing_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20150), published 27/2/1444H (2022-09-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "foreign_law_firms_licensing_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "FOREIGN_LAW_FIRMS_LICENSING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "foreign-law-firms-licensing-regulation-llm-%03d" % idx,
            ...
            "article_path": "foreign_law_firms_licensing_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation for Licensing Foreign Law Firms -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/2/1444H / 2022-09-23)"),
                "source_authority_ar": ("اللائحة التنفيذية لتنظيم الترخيص لمكاتب المحاماة الأجنبية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "foreign_law_firms_licensing_regulation",
        "layer": "FOREIGN_LAW_FIRMS_LICENSING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "foreign-law-firms-licensing-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (24 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation for Licensing Foreign Law Firms — Arabic LLM-ready layer (24 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation for Licensing Foreign Law Firms records" % (len(ver), len(llm)))
