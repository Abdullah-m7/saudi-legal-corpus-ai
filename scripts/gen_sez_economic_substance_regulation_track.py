#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة المتطلبات الاقتصادية الفعلية للمناطق الاقتصادية الخاصة track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4001553),
...
SRC = os.path.join(ROOT, "sources", "sez_economic_substance_regulation", "official_source",
                   "sez_economic_substance_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "sez_economic_substance_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "sez_economic_substance_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "sez_economic_substance_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "sez_economic_substance_regulation_arabic_legal_llm",
                        "sez_economic_substance_regulation_legal_llm_001_009.json")

LAW_ID = "sa-sez-economic-substance-regulation"
LAW_AR = "لائحة المتطلبات الاقتصادية الفعلية للمناطق الاقتصادية الخاصة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"sez_economic_substance_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4001553), published 24/2/1448H (2026-08-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "sez_economic_substance_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SEZ_ECONOMIC_SUBSTANCE_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "sez-economic-substance-regulation-llm-%03d" % idx,
            ...
            "article_path": "sez_economic_substance_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Economic Substance Requirements Regulations of Special Economic Zones -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/2/1448H / 2026-08-07)"),
                "source_authority_ar": ("لائحة المتطلبات الاقتصادية الفعلية للمناطق الاقتصادية الخاصة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "sez_economic_substance_regulation",
        "layer": "SEZ_ECONOMIC_SUBSTANCE_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "sez-economic-substance-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (9 مواد، جميعها أصلية)",
        "title_en": "Economic Substance Requirements Regulations of Special Economic Zones — Arabic LLM-ready layer (9 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Economic Substance Requirements Regulations of Special Economic Zones records" % (len(ver), len(llm)))
