#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد إقرار الأحكام المنظمة لشؤون العاملين في الأجهزة العامة وتعويضاتهم track.
...
server-rendered HTML page (uqn.gov.sa/details?p=22967),
...
SRC = os.path.join(ROOT, "sources", "public_agencies_staff_provisions_rules", "official_source",
                   "public_agencies_staff_provisions_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "public_agencies_staff_provisions_rules", "verified")
RECORDS = os.path.join(OUT_VER, "public_agencies_staff_provisions_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "public_agencies_staff_provisions_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "public_agencies_staff_provisions_rules_arabic_legal_llm",
                        "public_agencies_staff_provisions_rules_legal_llm_001_012.json")

LAW_ID = "sa-public-agencies-staff-provisions-rules"
LAW_AR = "قواعد إقرار الأحكام المنظمة لشؤون العاملين في الأجهزة العامة وتعويضاتهم"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"public_agencies_staff_provisions_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=22967), published 13/11/1444H (2023-06-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "public_agencies_staff_provisions_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PUBLIC_AGENCIES_STAFF_PROVISIONS_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "public-agencies-staff-provisions-rules-llm-%03d" % idx,
            ...
            "article_path": "public_agencies_staff_provisions_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules for Approving the Provisions Governing the Affairs and Compensation of Staff in Public Agencies -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/11/1444H / 2023-06-02)"),
                "source_authority_ar": ("قواعد إقرار الأحكام المنظمة لشؤون العاملين في الأجهزة العامة وتعويضاتهم — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "public_agencies_staff_provisions_rules",
        "layer": "PUBLIC_AGENCIES_STAFF_PROVISIONS_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "public-agencies-staff-provisions-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Rules for Approving the Provisions Governing the Affairs and Compensation of Staff in Public Agencies — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules for Approving the Provisions Governing the Affairs and Compensation of Staff in Public Agencies records" % (len(ver), len(llm)))
