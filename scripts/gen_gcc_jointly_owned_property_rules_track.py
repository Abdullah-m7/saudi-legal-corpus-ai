#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد الموحدة لملاك العقارات المشتركة بدول مجلس التعاون لدول الخليج العربية track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4001558),
...
SRC = os.path.join(ROOT, "sources", "gcc_jointly_owned_property_rules", "official_source",
                   "gcc_jointly_owned_property_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "gcc_jointly_owned_property_rules", "verified")
RECORDS = os.path.join(OUT_VER, "gcc_jointly_owned_property_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "gcc_jointly_owned_property_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "gcc_jointly_owned_property_rules_arabic_legal_llm",
                        "gcc_jointly_owned_property_rules_legal_llm_001_014.json")

LAW_ID = "sa-gcc-jointly-owned-property-rules"
LAW_AR = "القواعد الموحدة لملاك العقارات المشتركة بدول مجلس التعاون لدول الخليج العربية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"gcc_jointly_owned_property_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4001558), published 24/2/1448H (2026-08-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "gcc_jointly_owned_property_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "GCC_JOINTLY_OWNED_PROPERTY_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "gcc-jointly-owned-property-rules-llm-%03d" % idx,
            ...
            "article_path": "gcc_jointly_owned_property_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("GCC Unified Rules for Owners of Jointly Owned Property -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/2/1448H / 2026-08-07)"),
                "source_authority_ar": ("القواعد الموحدة لملاك العقارات المشتركة بدول مجلس التعاون لدول الخليج العربية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "gcc_jointly_owned_property_rules",
        "layer": "GCC_JOINTLY_OWNED_PROPERTY_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "gcc-jointly-owned-property-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "GCC Unified Rules for Owners of Jointly Owned Property — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready GCC Unified Rules for Owners of Jointly Owned Property records" % (len(ver), len(llm)))
