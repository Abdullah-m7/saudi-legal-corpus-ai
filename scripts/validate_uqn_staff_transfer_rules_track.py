#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد وترتيبات نقل موظفي وعمال جريدة أم القرى إلى وكالة الأنباء السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "uqn_staff_transfer_rules", "verified",
                       "uqn_staff_transfer_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "uqn_staff_transfer_rules", "verified",
                       "uqn_staff_transfer_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "uqn_staff_transfer_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"uqn_staff_transfer_rules_no_decision_number_in_source", "uqn_staff_transfer_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "18/3/1444":
        e.append("[2j] gazette_publication_date_hijri must be 18/3/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-10-14":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-10-14")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد وترتيبات نقل موظفي وعمال جريدة أم القرى إلى وكالة الأنباء السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in Rules and Arrangements for Transferring Umm Al-Qura Newspaper Staff to the Saudi Press Agency track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules and Arrangements for Transferring Umm Al-Qura Newspaper Staff to the Saudi Press Agency")
