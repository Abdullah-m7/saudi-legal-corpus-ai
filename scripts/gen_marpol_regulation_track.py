#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لمنع التلوث البحري من السفن (ماربول) 1973م، وبروتوكولاتها وتعديلاتها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19054),
published 1/8/1443H (2022-03-04). The Gazette IS the official publication of record
...
STRUCTURE: 100 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "marpol_regulation", "official_source",
                   "marpol_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "marpol_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "marpol_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "marpol_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "marpol_regulation_arabic_legal_llm",
                        "marpol_regulation_legal_llm_001_100.json")

LAW_ID = "sa-marpol-regulation"
LAW_AR = "اللائحة التنفيذية لمنع التلوث البحري من السفن (ماربول) 1973م، وبروتوكولاتها وتعديلاتها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"marpol_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19054), published 1/8/1443H (2022-03-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "marpol_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MARPOL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "marpol-regulation-llm-%03d" % idx,
            ...
            "article_path": "marpol_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation for the Prevention of Marine Pollution from Ships (MARPOL 1973) and its Protocols and Amendments -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/8/1443H / 2022-03-04)"),
                "source_authority_ar": ("اللائحة التنفيذية لمنع التلوث البحري من السفن (ماربول) 1973م، وبروتوكولاتها وتعديلاتها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 1/8/1443هـ الموافق 2022-03-04م)"),
    ...
    json.dump({
        "law_key": "marpol_regulation",
        "layer": "MARPOL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "marpol-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (100 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation for the Prevention of Marine Pollution from Ships (MARPOL 1973) and its Protocols and Amendments — Arabic LLM-ready layer (100 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation for the Prevention of Marine Pollution from Ships (MARPOL 1973) and its Protocols and Amendments records" % (len(ver), len(llm)))
