#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم الهيئة الوطنية لإدارة الطوارئ track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26696),
published 24/4/1446H (2024-10-27). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "national_emergency_management_authority_statute", "official_source",
                   "national_emergency_management_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "national_emergency_management_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "national_emergency_management_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "national_emergency_management_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "national_emergency_management_authority_statute_arabic_legal_llm",
                        "national_emergency_management_authority_statute_legal_llm_001_019.json")

LAW_ID = "sa-national-emergency-management-authority-statute"
LAW_AR = "تنظيم الهيئة الوطنية لإدارة الطوارئ"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"national_emergency_management_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26696), published 24/4/1446H (2024-10-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "national_emergency_management_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "NATIONAL_EMERGENCY_MANAGEMENT_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "national-emergency-management-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "national_emergency_management_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Authority for Emergency Management -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/4/1446H / 2024-10-27)"),
                "source_authority_ar": ("تنظيم الهيئة الوطنية لإدارة الطوارئ — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 24/4/1446هـ الموافق 2024-10-27م)"),
    ...
    json.dump({
        "law_key": "national_emergency_management_authority_statute",
        "layer": "NATIONAL_EMERGENCY_MANAGEMENT_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "national-emergency-management-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (19 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Authority for Emergency Management — Arabic LLM-ready layer (19 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Authority for Emergency Management records" % (len(ver), len(llm)))
