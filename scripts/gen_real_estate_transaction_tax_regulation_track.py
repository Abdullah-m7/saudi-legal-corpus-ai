#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لضريبة التصرفات العقارية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19944),
published 21/1/1444H (2022-08-19). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "real_estate_transaction_tax_regulation", "official_source",
                   "real_estate_transaction_tax_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "real_estate_transaction_tax_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "real_estate_transaction_tax_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "real_estate_transaction_tax_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "real_estate_transaction_tax_regulation_arabic_legal_llm",
                        "real_estate_transaction_tax_regulation_legal_llm_001_015.json")

LAW_ID = "sa-real-estate-transaction-tax-regulation"
LAW_AR = "اللائحة التنفيذية لضريبة التصرفات العقارية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"real_estate_transaction_tax_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19944), published 21/1/1444H (2022-08-19). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "real_estate_transaction_tax_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "REAL_ESTATE_TRANSACTION_TAX_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "real-estate-transaction-tax-regulation-llm-%03d" % idx,
            ...
            "article_path": "real_estate_transaction_tax_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Real Estate Transaction Tax -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/1/1444H / 2022-08-19)"),
                "source_authority_ar": ("اللائحة التنفيذية لضريبة التصرفات العقارية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 21/1/1444هـ الموافق 2022-08-19م)"),
    ...
    json.dump({
        "law_key": "real_estate_transaction_tax_regulation",
        "layer": "REAL_ESTATE_TRANSACTION_TAX_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "real-estate-transaction-tax-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Real Estate Transaction Tax — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Real Estate Transaction Tax records" % (len(ver), len(llm)))
