#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المركز الوطني لتنمية القطاع غير الربحي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17766),
...
SRC = os.path.join(ROOT, "sources", "nonprofit_sector_development_center_statute", "official_source",
                   "nonprofit_sector_development_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "nonprofit_sector_development_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "nonprofit_sector_development_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "nonprofit_sector_development_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "nonprofit_sector_development_center_statute_arabic_legal_llm",
                        "nonprofit_sector_development_center_statute_legal_llm_001_015.json")

LAW_ID = "sa-nonprofit-sector-development-center-statute"
LAW_AR = "تنظيم المركز الوطني لتنمية القطاع غير الربحي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"nonprofit_sector_development_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17766), published 1/11/1442H (2021-06-11). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "nonprofit_sector_development_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "NONPROFIT_SECTOR_DEVELOPMENT_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "nonprofit-sector-development-center-statute-llm-%03d" % idx,
            ...
            "article_path": "nonprofit_sector_development_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Center for Non-Profit Sector Development -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/11/1442H / 2021-06-11)"),
                "source_authority_ar": ("تنظيم المركز الوطني لتنمية القطاع غير الربحي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "nonprofit_sector_development_center_statute",
        "layer": "NONPROFIT_SECTOR_DEVELOPMENT_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "nonprofit-sector-development-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Center for Non-Profit Sector Development — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Center for Non-Profit Sector Development records" % (len(ver), len(llm)))
