#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية المحافظة على الحياة الفطرية ومواطنها الطبيعية في مجلس التعاون لدول الخليج العربية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18678),
published 6/5/1443H (2021-12-10). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "treaty_gcc_wildlife_conservation", "official_source",
                   "treaty_gcc_wildlife_conservation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_gcc_wildlife_conservation", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_gcc_wildlife_conservation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_gcc_wildlife_conservation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_gcc_wildlife_conservation_arabic_legal_llm",
                        "treaty_gcc_wildlife_conservation_legal_llm_001_018.json")

LAW_ID = "sa-treaty-gcc-wildlife-conservation"
LAW_AR = "اتفاقية المحافظة على الحياة الفطرية ومواطنها الطبيعية في مجلس التعاون لدول الخليج العربية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_gcc_wildlife_conservation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18678), published 6/5/1443H (2021-12-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_gcc_wildlife_conservation",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_GCC_WILDLIFE_CONSERVATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-gcc-wildlife-conservation-llm-%03d" % idx,
            ...
            "article_path": "treaty_gcc_wildlife_conservation/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("GCC Agreement on the Conservation of Wildlife and Its Natural Habitats -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/5/1443H / 2021-12-10)"),
                "source_authority_ar": ("اتفاقية المحافظة على الحياة الفطرية ومواطنها الطبيعية في مجلس التعاون لدول الخليج العربية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 6/5/1443هـ الموافق 2021-12-10م)"),
    ...
    json.dump({
        "law_key": "treaty_gcc_wildlife_conservation",
        "layer": "TREATY_GCC_WILDLIFE_CONSERVATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-gcc-wildlife-conservation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "GCC Agreement on the Conservation of Wildlife and Its Natural Habitats — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready GCC Agreement on the Conservation of Wildlife and Its Natural Habitats records" % (len(ver), len(llm)))
