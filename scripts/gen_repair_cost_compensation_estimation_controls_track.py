#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط تحديد تكاليف الإصلاح وتقدير التعويضات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23892),
published 5/4/1445H (2023-10-20). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "repair_cost_compensation_estimation_controls", "official_source",
                   "repair_cost_compensation_estimation_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "repair_cost_compensation_estimation_controls", "verified")
RECORDS = os.path.join(OUT_VER, "repair_cost_compensation_estimation_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "repair_cost_compensation_estimation_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "repair_cost_compensation_estimation_controls_arabic_legal_llm",
                        "repair_cost_compensation_estimation_controls_legal_llm_001_007.json")

LAW_ID = "sa-repair-cost-compensation-estimation-controls"
LAW_AR = "ضوابط تحديد تكاليف الإصلاح وتقدير التعويضات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"repair_cost_compensation_estimation_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23892), published 5/4/1445H (2023-10-20). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "repair_cost_compensation_estimation_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "REPAIR_COST_COMPENSATION_ESTIMATION_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "repair-cost-compensation-estimation-controls-llm-%03d" % idx,
            ...
            "article_path": "repair_cost_compensation_estimation_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls for Determining Repair Costs and Estimating Compensation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/4/1445H / 2023-10-20)"),
                "source_authority_ar": ("ضوابط تحديد تكاليف الإصلاح وتقدير التعويضات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 5/4/1445هـ الموافق 2023-10-20م)"),
    ...
    json.dump({
        "law_key": "repair_cost_compensation_estimation_controls",
        "layer": "REPAIR_COST_COMPENSATION_ESTIMATION_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "repair-cost-compensation-estimation-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (6 مادة، جميعها أصلية)",
        "title_en": "Controls for Determining Repair Costs and Estimating Compensation — Arabic LLM-ready layer (6 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls for Determining Repair Costs and Estimating Compensation records" % (len(ver), len(llm)))
