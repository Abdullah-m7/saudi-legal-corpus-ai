#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الضوابط والإجراءات للوكيل الملاحي السياحي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26581),
...
SRC = os.path.join(ROOT, "sources", "maritime_tourism_agent_controls", "official_source",
                   "maritime_tourism_agent_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "maritime_tourism_agent_controls", "verified")
RECORDS = os.path.join(OUT_VER, "maritime_tourism_agent_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "maritime_tourism_agent_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "maritime_tourism_agent_controls_arabic_legal_llm",
                        "maritime_tourism_agent_controls_legal_llm_001_010.json")

LAW_ID = "sa-maritime-tourism-agent-controls"
LAW_AR = "الضوابط والإجراءات للوكيل الملاحي السياحي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"maritime_tourism_agent_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26581), published 1/4/1446H (2024-10-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "maritime_tourism_agent_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "MARITIME_TOURISM_AGENT_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "maritime-tourism-agent-controls-llm-%03d" % idx,
            ...
            "article_path": "maritime_tourism_agent_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls and Procedures for the Maritime Tourism Agent -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/4/1446H / 2024-10-04)"),
                "source_authority_ar": ("الضوابط والإجراءات للوكيل الملاحي السياحي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "maritime_tourism_agent_controls",
        "layer": "MARITIME_TOURISM_AGENT_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "maritime-tourism-agent-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Controls and Procedures for the Maritime Tourism Agent — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls and Procedures for the Maritime Tourism Agent records" % (len(ver), len(llm)))
