...
# -*- coding: utf-8 -*-
"""
Render Book Four — Section 4 (provisions) outputs.

Writes:
  content/ar/book4_section4.md
  content/zh/book4_section4.md
  content/bilingual/book4_section4_bilingual.md
  dist/book4_section4.html   (clearly labelled: Section 4 provisions ONLY, not full Book Four)
...
MD_AR = os.path.join(ROOT, "content", "ar", "book4_section4.md")
MD_ZH = os.path.join(ROOT, "content", "zh", "book4_section4.md")
MD_BI = os.path.join(ROOT, "content", "bilingual", "book4_section4_bilingual.md")
HTML_OUT = os.path.join(ROOT, "dist", "book4_section4.html")

SECTION_LABEL_AR = "الباب الرابع — القسم 4: الأسهم وأدوات الدين والصكوك (المواد 103–120) — أحكام مختارة فقط"
SECTION_LABEL_ZH = "第四编 — 第四节：股份、债务工具与融资凭证（第103–120条）— 仅择要条款（非全节全文）"
INCOMPLETE_BANNER = ("⚠ 仅为第四编第四节的择要条款（经与源文件核对后明确涵盖的第108、113、115、117条）；"
                     "并非完整的第四编。第110条（经业主决定 Option 1 予以重分类）及第103–107、109、111、112、"
                     "114、116、118–120条未涵盖，仍为 needs_official_text_check。 / "
                     "أحكام مختارة من القسم الرابع فقط (المواد المغطاة صراحةً بعد المطابقة: 108، 113، 115، 117)؛ "
                     "ليست الباب الرابع كاملاً — المادة 110 (أُعيد تصنيفها بقرار المالك: الخيار 1) وبقية مواد القسم "
                     "غير مغطاة (needs_official_text_check).")
...
def render_markdown(doc, spec):
    ...
    ar = [f"# {SECTION_LABEL_AR}\n",
          ...
          f"\n**نموذج 1b (أحكام):** أحكام لمواد مغطاة صراحةً فقط ({arts}). المادة 110 وباقي مواد القسم غير مغطاة (needs_official_text_check).\n"]
    zh = [f"# {SECTION_LABEL_ZH}\n",
          f"> {INCOMPLETE_BANNER}\n",
          f"\n**免责声明：** {spec.disclaimer_zh}\n",
          f"\n**模型 1b（专题条款）：** 仅涵盖源文件明确的条款（{arts}）。第110条及本节其余条款未涵盖（needs_official_text_check）。\n"]
...
def render_html(doc, spec):
    ...
    for p in provs:
        ...
        parts.append("</div>")
    parts.append('<footer><p>HTML = searchable/copyable Section-4 provisions view · '
