#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط تملك الشركات المدرجة والصناديق الاستثمارية والمنشآت ذات الأغراض الخاصة للعقار في المملكة track.

11 records: 4 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "listed_entities_property_ownership_controls", "verified",
                       "listed_entities_property_ownership_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "listed_entities_property_ownership_controls", "verified",
                       "listed_entities_property_ownership_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "listed_entities_property_ownership_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "listed_entities_property_ownership_controls_commencement_is_tied_to_another_instrument",
    "listed_entities_property_ownership_controls_found_by_walking_a_url_section_the_index_never_covered",
    "listed_entities_property_ownership_controls_no_decision_number_in_source",
    "listed_entities_property_ownership_controls_scope_includes_makkah_and_madinah",
    "listed_entities_property_ownership_controls_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    EXPECTED_NUMBERS = list(range(1, 5))
    MISSING_IN_SOURCE = []
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "7/11/1447":
        e.append("[2j] gazette_publication_date_hijri must be 7/11/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-04-24":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-04-24")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط تملك الشركات المدرجة والصناديق الاستثمارية والمنشآت ذات الأغراض الخاصة للعقار في المملكة":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on Property Ownership by Listed Companies, Investment Funds and Special Purpose Entities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on Property Ownership by Listed Companies, Investment Funds and Special Purpose Entities")
    print("  - 4 records: 4 articles, no appendices; all 4 اصلية")
