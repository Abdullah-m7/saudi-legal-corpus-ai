#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة للتعاقد مع السعوديين في الخارج وسلم الأجور track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/.../4000905),
published 21/11/1447H (2026-05-08). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "overseas_saudi_contracting_rules", "official_source",
                   "overseas_saudi_contracting_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "overseas_saudi_contracting_rules", "verified")
RECORDS = os.path.join(OUT_VER, "overseas_saudi_contracting_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "overseas_saudi_contracting_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "overseas_saudi_contracting_rules_arabic_legal_llm",
                        "overseas_saudi_contracting_rules_legal_llm_001_006.json")

LAW_ID = "sa-overseas-saudi-contracting-rules"
LAW_AR = "القواعد المنظمة للتعاقد مع السعوديين في الخارج وسلم الأجور"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"overseas_saudi_contracting_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/.../4000905), published 21/11/1447H (2026-05-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "overseas_saudi_contracting_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "OVERSEAS_SAUDI_CONTRACTING_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "overseas-saudi-contracting-rules-llm-%03d" % idx,
            ...
            "article_path": "overseas_saudi_contracting_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules for Contracting with Saudis Abroad and the Wage Scale -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/11/1447H / 2026-05-08)"),
                "source_authority_ar": ("القواعد المنظمة للتعاقد مع السعوديين في الخارج وسلم الأجور — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 21/11/1447هـ الموافق 2026-05-08م)"),
    ...
    json.dump({
        "law_key": "overseas_saudi_contracting_rules",
        "layer": "OVERSEAS_SAUDI_CONTRACTING_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "overseas-saudi-contracting-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (6 مادة، جميعها أصلية)",
        "title_en": "Rules for Contracting with Saudis Abroad and the Wage Scale — Arabic LLM-ready layer (6 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules for Contracting with Saudis Abroad and the Wage Scale records" % (len(ver), len(llm)))
