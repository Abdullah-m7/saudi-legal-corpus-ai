#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الضوابط والمتطلبات والمواصفات الفنية والقواعد الإجرائية اللازمة لتنفيذ أحكام لائحة الفوترة الإلكترونية track.
...
RECORDS = os.path.join(ROOT, "sources", "electricity_tariff_technical_controls", "verified",
                       "electricity_tariff_technical_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "electricity_tariff_technical_controls", "verified",
                       "electricity_tariff_technical_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "electricity_tariff_technical_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"electricity_tariff_technical_controls_no_decision_number_in_source", "electricity_tariff_technical_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "16/10/1442":
        e.append("[2j] gazette_publication_date_hijri must be 16/10/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-05-28":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-05-28")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الضوابط والمتطلبات والمواصفات الفنية والقواعد الإجرائية اللازمة لتنفيذ أحكام لائحة الفوترة الإلكترونية":
    ...
    if e:
        print("FAIL: %d error(s) in Technical Controls, Requirements, Specifications and Procedural Rules for Electricity Consumption Tariff track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Technical Controls, Requirements, Specifications and Procedural Rules for Electricity Consumption Tariff")
