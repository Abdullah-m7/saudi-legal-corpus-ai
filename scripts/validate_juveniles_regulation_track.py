#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Juveniles Law Implementing Regulation track (13 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) EXCEPT the 1 documented article (4) whose PDF text-layer
...
position 1..13 (no مكرر); and every article carries an explained legal_status
...
ISSUANCE: all 13 اصلية; none amended, repealed or added. Tatweel is banned EXCEPT
...
RECORDS = os.path.join(ROOT, "sources", "juveniles", "implementing_regulation", "verified",
                       "juveniles_regulation_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "juveniles_arabic_legal_llm",
                   "juveniles_regulation_legal_llm_001_013.json")
PDF = os.path.join(ROOT, "inputs", "juveniles_reg_official_pdfs",
                   "juveniles_regulation_moj_official_ar.pdf")
...
VISUALLY_ADJUDICATED = {"juveniles_reg_art_004"}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Juveniles Law Implementing Regulation track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Juveniles Law Implementing Regulation — 13 records (fresh issuance: all 13 اصلية)")
    print("  - trust gate: 12/13 MATCHES_PDF outright (mean 0.947); article 4 adjudicated visually verbatim")
    print("  - numbered 1..13 by ordinal position (no مكرر); no dual-status divergence")
    print("  - IN-FORCE Council of Ministers Decision 237 (16/4/1442H) implementing M/113; committed MOJ PDF hash verified; Arabic governs")
    return 0
