#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد ومعايير أسماء المرافق العامة track.
...
RECORDS = os.path.join(ROOT, "sources", "public_facility_names_rules", "verified",
                       "public_facility_names_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "public_facility_names_rules", "verified",
                       "public_facility_names_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "public_facility_names_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"public_facility_names_rules_no_decision_number_in_source", "public_facility_names_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد ومعايير أسماء المرافق العامة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules and Standards for the Names of Public Facilities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules and Standards for the Names of Public Facilities")
