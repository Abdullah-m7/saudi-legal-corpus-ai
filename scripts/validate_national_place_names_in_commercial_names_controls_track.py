#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط استخدام اسم السعودية وأسماء المدن والمناطق والأماكن العامة في الاسم التجاري track.

4 records: 4 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "national_place_names_in_commercial_names_controls", "verified",
                       "national_place_names_in_commercial_names_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "national_place_names_in_commercial_names_controls", "verified",
                       "national_place_names_in_commercial_names_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "national_place_names_in_commercial_names_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"national_place_names_in_commercial_names_controls_no_decision_number_in_source", "national_place_names_in_commercial_names_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "23/6/1445":
        e.append("[2j] gazette_publication_date_hijri must be 23/6/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-01-05":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-01-05")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط استخدام اسم السعودية وأسماء المدن والمناطق والأماكن العامة في الاسم التجاري":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on Using the Name of Saudi Arabia and Names of Cities, Regions and Public Places in Commercial Names track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on Using the Name of Saudi Arabia and Names of Cities, Regions and Public Places in Commercial Names")
