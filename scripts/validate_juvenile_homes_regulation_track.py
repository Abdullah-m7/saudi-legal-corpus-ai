#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية لدور الأحداث track.
...
RECORDS = os.path.join(ROOT, "sources", "juvenile_homes_regulation", "verified",
                       "juvenile_homes_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "juvenile_homes_regulation", "verified",
                       "juvenile_homes_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "juvenile_homes_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"juvenile_homes_regulation_no_decision_number_in_source", "juvenile_homes_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية لدور الأحداث":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for Juvenile Homes track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for Juvenile Homes")
