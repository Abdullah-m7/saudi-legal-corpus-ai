#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الدليل الاسترشادي لحوكمة الجهات العامة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25324),
...
SRC = os.path.join(ROOT, "sources", "public_entities_governance_guide", "official_source",
                   "public_entities_governance_guide_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "public_entities_governance_guide", "verified")
RECORDS = os.path.join(OUT_VER, "public_entities_governance_guide_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "public_entities_governance_guide_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "public_entities_governance_guide_arabic_legal_llm",
                        "public_entities_governance_guide_legal_llm_001_006.json")

LAW_ID = "sa-public-entities-governance-guide"
LAW_AR = "الدليل الاسترشادي لحوكمة الجهات العامة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"public_entities_governance_guide_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25324), published 5/2/1446H (2024-08-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "public_entities_governance_guide",
            "law_component": "guide",
            "language": "ar",
            "record_layer": "PUBLIC_ENTITIES_GOVERNANCE_GUIDE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "public-entities-governance-guide-llm-%03d" % idx,
            ...
            "article_path": "public_entities_governance_guide/guide/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Guidance Manual for the Governance of Public Entities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/2/1446H / 2024-08-09)"),
                "source_authority_ar": ("الدليل الاسترشادي لحوكمة الجهات العامة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "public_entities_governance_guide",
        "layer": "PUBLIC_ENTITIES_GOVERNANCE_GUIDE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "public-entities-governance-guide-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "guide",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (6 مادة، جميعها أصلية)",
        "title_en": "Guidance Manual for the Governance of Public Entities — Arabic LLM-ready layer (6 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Guidance Manual for the Governance of Public Entities records" % (len(ver), len(llm)))
