#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية للمنطقة الاقتصادية الخاصة بجازان track.
...
RECORDS = os.path.join(ROOT, "sources", "sez_jazan_regulation", "verified",
                       "sez_jazan_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "sez_jazan_regulation", "verified",
                       "sez_jazan_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "sez_jazan_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"sez_jazan_regulation_no_decision_number_in_source", "sez_jazan_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية للمنطقة الاقتصادية الخاصة بجازان":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules of the Jazan Special Economic Zone track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules of the Jazan Special Economic Zone")
