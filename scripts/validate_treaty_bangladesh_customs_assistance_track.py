#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية بين حكومتي المملكة وبنغلاديش الشعبية حول التعاون والمساعدة المتبادلة في المسائل الجمركية track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_bangladesh_customs_assistance", "verified",
                       "treaty_bangladesh_customs_assistance_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_bangladesh_customs_assistance", "verified",
                       "treaty_bangladesh_customs_assistance_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_bangladesh_customs_assistance_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_bangladesh_customs_assistance_no_decision_number_in_source", "treaty_bangladesh_customs_assistance_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "6/2/1444":
        e.append("[2j] gazette_publication_date_hijri must be 6/2/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-09-02":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-09-02")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية بين حكومتي المملكة وبنغلاديش الشعبية حول التعاون والمساعدة المتبادلة في المسائل الجمركية":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Bangladesh Agreement on Cooperation and Mutual Assistance in Customs Matters track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Bangladesh Agreement on Cooperation and Mutual Assistance in Customs Matters")
