#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية للهيئة العامة للعناية بشؤون المسجد الحرام والمسجد النبوي track.

16 records: 16 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "two_holy_mosques_authority_statute", "verified",
                       "two_holy_mosques_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "two_holy_mosques_authority_statute", "verified",
                       "two_holy_mosques_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "two_holy_mosques_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"two_holy_mosques_authority_statute_no_decision_number_in_source", "two_holy_mosques_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية للهيئة العامة للعناية بشؤون المسجد الحرام والمسجد النبوي":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the General Authority for the Care of the Affairs of the Grand Mosque and the Prophet's Mosque track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the General Authority for the Care of the Affairs of the Grand Mosque and the Prophet's Mosque")
    print("  - 16 records: 16 articles, no appendices; all 16 اصلية")
