#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة لبرنامج الكفاءات والمتعاقدين track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19971),
published 28/1/1444H (2022-08-26). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "competencies_contractors_program_rules", "official_source",
                   "competencies_contractors_program_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "competencies_contractors_program_rules", "verified")
RECORDS = os.path.join(OUT_VER, "competencies_contractors_program_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "competencies_contractors_program_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "competencies_contractors_program_rules_arabic_legal_llm",
                        "competencies_contractors_program_rules_legal_llm_001_005.json")

LAW_ID = "sa-competencies-contractors-program-rules"
LAW_AR = "القواعد المنظمة لبرنامج الكفاءات والمتعاقدين"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"competencies_contractors_program_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19971), published 28/1/1444H (2022-08-26). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "competencies_contractors_program_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "COMPETENCIES_CONTRACTORS_PROGRAM_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "competencies-contractors-program-rules-llm-%03d" % idx,
            ...
            "article_path": "competencies_contractors_program_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing the Competencies and Contractors Program -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/1/1444H / 2022-08-26)"),
                "source_authority_ar": ("القواعد المنظمة لبرنامج الكفاءات والمتعاقدين — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 28/1/1444هـ الموافق 2022-08-26م)"),
    ...
    json.dump({
        "law_key": "competencies_contractors_program_rules",
        "layer": "COMPETENCIES_CONTRACTORS_PROGRAM_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "competencies-contractors-program-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (5 مادة، جميعها أصلية)",
        "title_en": "Rules Governing the Competencies and Contractors Program — Arabic LLM-ready layer (5 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing the Competencies and Contractors Program records" % (len(ver), len(llm)))
