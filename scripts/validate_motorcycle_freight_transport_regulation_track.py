#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنشاط نقل البضائع عبر الدراجة الآلية لأغراض تجارية track.

11 records: 29 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "motorcycle_freight_transport_regulation", "verified",
                       "motorcycle_freight_transport_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "motorcycle_freight_transport_regulation", "verified",
                       "motorcycle_freight_transport_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "motorcycle_freight_transport_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "motorcycle_freight_transport_regulation_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    EXPECTED_NUMBERS = list(range(1, 30))
    MISSING_IN_SOURCE = []
    ...
    if src.get("document") != "اللائحة التنفيذية لنشاط نقل البضائع عبر الدراجة الآلية لأغراض تجارية":
    ...
    if e:
        print("FAIL: %d error(s) in Implementing Regulation for Commercial Freight Transport by Motorcycle track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Implementing Regulation for Commercial Freight Transport by Motorcycle")
    print("  - 29 records: 29 articles, no appendices; all 29 اصلية")
