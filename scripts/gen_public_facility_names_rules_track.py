#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد ومعايير أسماء المرافق العامة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28753),
...
SRC = os.path.join(ROOT, "sources", "public_facility_names_rules", "official_source",
                   "public_facility_names_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "public_facility_names_rules", "verified")
RECORDS = os.path.join(OUT_VER, "public_facility_names_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "public_facility_names_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "public_facility_names_rules_arabic_legal_llm",
                        "public_facility_names_rules_legal_llm_001_012.json")

LAW_ID = "sa-public-facility-names-rules"
LAW_AR = "قواعد ومعايير أسماء المرافق العامة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"public_facility_names_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28753), published 13/7/1447H (2026-01-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "public_facility_names_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PUBLIC_FACILITY_NAMES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "public-facility-names-rules-llm-%03d" % idx,
            ...
            "article_path": "public_facility_names_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules and Standards for the Names of Public Facilities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/7/1447H / 2026-01-02)"),
                "source_authority_ar": ("قواعد ومعايير أسماء المرافق العامة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "public_facility_names_rules",
        "layer": "PUBLIC_FACILITY_NAMES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "public-facility-names-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Rules and Standards for the Names of Public Facilities — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules and Standards for the Names of Public Facilities records" % (len(ver), len(llm)))
