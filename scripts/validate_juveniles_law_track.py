#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Juveniles Law track (24 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) EXCEPT the 2 documented articles (5, 20) whose PDF text-layer
...
position 1..24 (no مكرر); and every article carries an explained legal_status
...
ISSUANCE: all 24 اصلية; none amended, repealed or added. Tatweel is banned EXCEPT
...
RECORDS = os.path.join(ROOT, "sources", "juveniles", "law", "verified",
                       "juveniles_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "juveniles_arabic_legal_llm",
                   "juveniles_law_legal_llm_001_024.json")
PDF = os.path.join(ROOT, "inputs", "juveniles_official_pdfs",
                   "juveniles_law_moj_official_ar.pdf")
...
VISUALLY_ADJUDICATED = {"juveniles_art_005", "juveniles_art_020"}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Juveniles Law track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Juveniles Law — 24 records (fresh issuance: all 24 اصلية)")
    print("  - trust gate: 22/24 MATCHES_PDF outright (mean 0.951); the 2 articles (5, 20) adjudicated visually verbatim")
    print("  - numbered 1..24 by ordinal position (no مكرر); no dual-status divergence")
    print("  - IN-FORCE Royal Decree M/113 (19/11/1439H); committed MOJ PDF hash verified; Arabic governs")
    return 0
