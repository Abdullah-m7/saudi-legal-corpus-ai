#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية عامة للتعاون بين المملكة وجمهورية رواندا track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18639),
published 28/4/1443H (2021-12-03). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "treaty_rwanda_general_cooperation", "official_source",
                   "treaty_rwanda_general_cooperation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_rwanda_general_cooperation", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_rwanda_general_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_rwanda_general_cooperation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_rwanda_general_cooperation_arabic_legal_llm",
                        "treaty_rwanda_general_cooperation_legal_llm_001_011.json")

LAW_ID = "sa-treaty-rwanda-general-cooperation"
LAW_AR = "اتفاقية عامة للتعاون بين المملكة وجمهورية رواندا"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_rwanda_general_cooperation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18639), published 28/4/1443H (2021-12-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_rwanda_general_cooperation",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_RWANDA_GENERAL_COOPERATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-rwanda-general-cooperation-llm-%03d" % idx,
            ...
            "article_path": "treaty_rwanda_general_cooperation/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Arabia–Rwanda General Cooperation Agreement -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/4/1443H / 2021-12-03)"),
                "source_authority_ar": ("اتفاقية عامة للتعاون بين المملكة وجمهورية رواندا — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 28/4/1443هـ الموافق 2021-12-03م)"),
    ...
    json.dump({
        "law_key": "treaty_rwanda_general_cooperation",
        "layer": "TREATY_RWANDA_GENERAL_COOPERATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-rwanda-general-cooperation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Saudi Arabia–Rwanda General Cooperation Agreement — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Arabia–Rwanda General Cooperation Agreement records" % (len(ver), len(llm)))
