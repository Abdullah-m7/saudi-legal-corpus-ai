#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد وإجراءات عمل لجان الفصل في المنازعات والمخالفات التأمينية track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4001121),
published 19/12/1447H (2026-06-05). The Gazette IS the official publication of record
...
STRUCTURE: 46 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "insurance_disputes_committees_rules", "official_source",
                   "insurance_disputes_committees_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "insurance_disputes_committees_rules", "verified")
RECORDS = os.path.join(OUT_VER, "insurance_disputes_committees_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "insurance_disputes_committees_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "insurance_disputes_committees_rules_arabic_legal_llm",
                        "insurance_disputes_committees_rules_legal_llm_001_046.json")

LAW_ID = "sa-insurance-disputes-committees-rules"
LAW_AR = "قواعد وإجراءات عمل لجان الفصل في المنازعات والمخالفات التأمينية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"insurance_disputes_committees_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4001121), published 19/12/1447H (2026-06-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "insurance_disputes_committees_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "INSURANCE_DISPUTES_COMMITTEES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "insurance-disputes-committees-rules-llm-%03d" % idx,
            ...
            "article_path": "insurance_disputes_committees_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules and Procedures of the Committees for Resolution of Insurance Disputes and Violations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 19/12/1447H / 2026-06-05)"),
                "source_authority_ar": ("قواعد وإجراءات عمل لجان الفصل في المنازعات والمخالفات التأمينية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 19/12/1447هـ الموافق 2026-06-05م)"),
    ...
    json.dump({
        "law_key": "insurance_disputes_committees_rules",
        "layer": "INSURANCE_DISPUTES_COMMITTEES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "insurance-disputes-committees-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (46 مادة، جميعها أصلية)",
        "title_en": "Rules and Procedures of the Committees for Resolution of Insurance Disputes and Violations — Arabic LLM-ready layer (46 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules and Procedures of the Committees for Resolution of Insurance Disputes and Violations records" % (len(ver), len(llm)))
