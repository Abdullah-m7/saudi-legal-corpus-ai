#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية لتأجير اليخوت الضخمة track.
...
RECORDS = os.path.join(ROOT, "sources", "superyacht_chartering_regulation", "verified",
                       "superyacht_chartering_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "superyacht_chartering_regulation", "verified",
                       "superyacht_chartering_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "superyacht_chartering_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"superyacht_chartering_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية لتأجير اليخوت الضخمة":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Regulation for Superyacht Chartering track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Regulation for Superyacht Chartering")
