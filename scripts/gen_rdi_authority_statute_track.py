#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لهيئة تنمية البحث والتطوير والابتكار track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21485),
...
SRC = os.path.join(ROOT, "sources", "rdi_authority_statute", "official_source",
                   "rdi_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "rdi_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "rdi_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "rdi_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "rdi_authority_statute_arabic_legal_llm",
                        "rdi_authority_statute_legal_llm_001_015.json")

LAW_ID = "sa-rdi-authority-statute"
LAW_AR = "الترتيبات التنظيمية لهيئة تنمية البحث والتطوير والابتكار"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"rdi_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21485), published 4/8/1444H (2023-02-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "rdi_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "RDI_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "rdi-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "rdi_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the Research, Development and Innovation Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/8/1444H / 2023-02-24)"),
                "source_authority_ar": ("الترتيبات التنظيمية لهيئة تنمية البحث والتطوير والابتكار — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "rdi_authority_statute",
        "layer": "RDI_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "rdi-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the Research, Development and Innovation Authority — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the Research, Development and Innovation Authority records" % (len(ver), len(llm)))
