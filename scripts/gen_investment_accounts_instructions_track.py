#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تعليمات الحسابات الاستثمارية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27344),
...
SRC = os.path.join(ROOT, "sources", "investment_accounts_instructions", "official_source",
                   "investment_accounts_instructions_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "investment_accounts_instructions", "verified")
RECORDS = os.path.join(OUT_VER, "investment_accounts_instructions_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "investment_accounts_instructions_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "investment_accounts_instructions_arabic_legal_llm",
                        "investment_accounts_instructions_legal_llm_001_017.json")

LAW_ID = "sa-investment-accounts-instructions"
LAW_AR = "تعليمات الحسابات الاستثمارية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"investment_accounts_instructions_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27344), published 21/2/1447H (2025-08-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "investment_accounts_instructions",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "INVESTMENT_ACCOUNTS_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "investment-accounts-instructions-llm-%03d" % idx,
            ...
            "article_path": "investment_accounts_instructions/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Investment Accounts Instructions -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/2/1447H / 2025-08-15)"),
                "source_authority_ar": ("تعليمات الحسابات الاستثمارية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "investment_accounts_instructions",
        "layer": "INVESTMENT_ACCOUNTS_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "investment-accounts-instructions-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Investment Accounts Instructions — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Investment Accounts Instructions records" % (len(ver), len(llm)))
