#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية لتأشيرات العمل المؤقت والعمل المؤقت لخدمات الحج والعمرة track.
...
RECORDS = os.path.join(ROOT, "sources", "temporary_work_visas_regulation", "verified",
                       "temporary_work_visas_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "temporary_work_visas_regulation", "verified",
                       "temporary_work_visas_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "temporary_work_visas_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"temporary_work_visas_regulation_no_decision_number_in_source", "temporary_work_visas_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "8/4/1446":
        e.append("[2j] gazette_publication_date_hijri must be 8/4/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-10-11":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-10-11")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنظيمية لتأشيرات العمل المؤقت والعمل المؤقت لخدمات الحج والعمرة":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for Temporary Work Visas and Temporary Work for Hajj and Umrah Services track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for Temporary Work Visas and Temporary Work for Hajj and Umrah Services")
