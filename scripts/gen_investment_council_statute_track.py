#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لمجلس الاستثمار track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23567),
published 16/2/1445H (2023-09-01). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "investment_council_statute", "official_source",
                   "investment_council_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "investment_council_statute", "verified")
RECORDS = os.path.join(OUT_VER, "investment_council_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "investment_council_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "investment_council_statute_arabic_legal_llm",
                        "investment_council_statute_legal_llm_001_008.json")

LAW_ID = "sa-investment-council-statute"
LAW_AR = "الترتيبات التنظيمية لمجلس الاستثمار"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"investment_council_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23567), published 16/2/1445H (2023-09-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "investment_council_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "INVESTMENT_COUNCIL_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "investment-council-statute-llm-%03d" % idx,
            ...
            "article_path": "investment_council_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Investment Council — Organisational Arrangements -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 16/2/1445H / 2023-09-01)"),
                "source_authority_ar": ("الترتيبات التنظيمية لمجلس الاستثمار — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 16/2/1445هـ الموافق 2023-09-01م)"),
    ...
    json.dump({
        "law_key": "investment_council_statute",
        "layer": "INVESTMENT_COUNCIL_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "investment-council-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Investment Council — Organisational Arrangements — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Investment Council — Organisational Arrangements records" % (len(ver), len(llm)))
