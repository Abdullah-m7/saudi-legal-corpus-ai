#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the النظام (القانون) الموحد للنقل البري الدولي بين دول مجلس التعاون لدول الخليج العربية track.
...
RECORDS = os.path.join(ROOT, "sources", "gcc_road_transport_law", "verified",
                       "gcc_road_transport_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "gcc_road_transport_law", "verified",
                       "gcc_road_transport_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "gcc_road_transport_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"gcc_road_transport_law_no_decision_number_in_source", "gcc_road_transport_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "2/11/1445":
        e.append("[2j] gazette_publication_date_hijri must be 2/11/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-05-10":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-05-10")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "النظام (القانون) الموحد للنقل البري الدولي بين دول مجلس التعاون لدول الخليج العربية":
    ...
    if e:
        print("FAIL: %d error(s) in Unified Law of International Road Transport for the GCC States track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Unified Law of International Road Transport for the GCC States")
