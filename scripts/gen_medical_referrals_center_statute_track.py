#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مركز الإحالات الطبية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27310),
published 7/2/1447H (2025-08-01). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "medical_referrals_center_statute", "official_source",
                   "medical_referrals_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "medical_referrals_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "medical_referrals_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "medical_referrals_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "medical_referrals_center_statute_arabic_legal_llm",
                        "medical_referrals_center_statute_legal_llm_001_011.json")

LAW_ID = "sa-medical-referrals-center-statute"
LAW_AR = "تنظيم مركز الإحالات الطبية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"medical_referrals_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27310), published 7/2/1447H (2025-08-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "medical_referrals_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "MEDICAL_REFERRALS_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "medical-referrals-center-statute-llm-%03d" % idx,
            ...
            "article_path": "medical_referrals_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Medical Referrals Center -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/2/1447H / 2025-08-01)"),
                "source_authority_ar": ("تنظيم مركز الإحالات الطبية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 7/2/1447هـ الموافق 2025-08-01م)"),
    ...
    json.dump({
        "law_key": "medical_referrals_center_statute",
        "layer": "MEDICAL_REFERRALS_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "medical-referrals-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Statute of the Medical Referrals Center — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Medical Referrals Center records" % (len(ver), len(llm)))
