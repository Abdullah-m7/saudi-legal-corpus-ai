...
Checks:
  ...
  3.  Exactly 268 track entries, one per REQUIRED_TRACK_IDS (imported from
  ...
  9.  Read-only: this validator does not modify data/corpus_registry/corpus_registry.json,
      any of the 268 tracks' own files, or the committed output file itself.
...
def main() -> int:
    ...
    # [3] Exactly 268 tracks, matching REQUIRED_TRACK_IDS exactly (no missing, no extras)
    ...
    check("[3a] Track count == 268...", len(tracks) == 268, f"count={len(tracks)}")
    ...
    # [7] summary_by_tier matches a fresh recount; counts sum to total_tracks == 268
    ...
    check("[7b] total_tracks == 268...", out.get("total_tracks") == 268,
