...
Checks:
  ...
  7.  `summary_by_tier` in the output matches a fresh recount of the `tracks` array, and
      the four tier counts sum to `total_tracks` == 749.
...
def main() -> int:
    ...
    # [3] Exactly 749 tracks, matching REQUIRED_TRACK_IDS exactly (no missing, no extras)
    ...
    # [7] summary_by_tier matches a fresh recount; counts sum to total_tracks == 749
    ...
    check("[7b] total_tracks == 749...", out.get("total_tracks") == 749,
