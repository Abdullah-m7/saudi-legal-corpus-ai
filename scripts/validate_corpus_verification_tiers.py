...
Checks:
  ...
  3.  Exactly 132 track entries, one per REQUIRED_TRACK_IDS (imported from
      scripts/validate_corpus_registry.py — the same canonical 132-track-id list the
  ...
  7.  `summary_by_tier` in the output matches a fresh recount of the `tracks` array, and
      the four tier counts sum to `total_tracks` == 132.
  ...
  9.  Read-only: this validator does not modify data/corpus_registry/corpus_registry.json,
      any of the 132 tracks' own files, or the committed output file itself.
...
def main() -> int:
    ...
    # [3] Exactly 132 tracks, matching REQUIRED_TRACK_IDS exactly (no missing, no extras)
    ...
    check("[3a] Track count == 132...", len(tracks) == 132, f"count={len(tracks)}")
    ...
    # [7] summary_by_tier matches a fresh recount; counts sum to total_tracks == 132
    ...
    check("[7b] total_tracks == 132...", out.get("total_tracks") == 132,
