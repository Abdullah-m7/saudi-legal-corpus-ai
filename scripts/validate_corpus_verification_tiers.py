...
Checks:
  ...
  3.  Exactly 164 track entries, one per REQUIRED_TRACK_IDS (imported from
      scripts/validate_corpus_registry.py — the same canonical 164-track-id list the
  ...
  7.  `summary_by_tier` in the output matches a fresh recount of the `tracks` array, and
      the four tier counts sum to `total_tracks` == 164.
  ...
  9.  Read-only: this validator does not modify data/corpus_registry/corpus_registry.json,
      any of the 164 tracks' own files, or the committed output file itself.
...
def main() -> int:
    ...
    # [3] Exactly 164 tracks, matching REQUIRED_TRACK_IDS exactly (no missing, no extras)
    ...
    check("[3a] Track count == 164...", len(tracks) == 164, f"count={len(tracks)}")
    ...
    # [7] summary_by_tier matches a fresh recount; counts sum to total_tracks == 164
    ...
    check("[7b] total_tracks == 164...", out.get("total_tracks") == 164,
