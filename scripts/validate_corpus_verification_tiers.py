...
Checks:
  ...
  3.  Exactly 183 track entries, one per REQUIRED_TRACK_IDS (imported from
  ...
  9.  Read-only: this validator does not modify data/corpus_registry/corpus_registry.json,
      any of the 183 tracks' own files, or the committed output file itself.
...
def main() -> int:
    ...
    # [3] Exactly 183 tracks, matching REQUIRED_TRACK_IDS exactly (no missing, no extras)
    ...
    check("[3a] Track count == 183...", len(tracks) == 183, f"count={len(tracks)}")
    ...
    # [7] summary_by_tier matches a fresh recount; counts sum to total_tracks == 183
    ...
    check("[7b] total_tracks == 183...", out.get("total_tracks") == 183,
