...
Checks:
  ...
  3.  Exactly 156 track entries, one per REQUIRED_TRACK_IDS (imported from
      scripts/validate_corpus_registry.py — the same canonical 156-track-id list the
  ...
  7.  `summary_by_tier` in the output matches a fresh recount of the `tracks` array, and
      the four tier counts sum to `total_tracks` == 156.
  ...
  9.  Read-only: this validator does not modify data/corpus_registry/corpus_registry.json,
      any of the 156 tracks' own files, or the committed output file itself.
...
def main() -> int:
    ...
    # [3] Exactly 156 tracks, matching REQUIRED_TRACK_IDS exactly (no missing, no extras)
    ...
    check("[3a] Track count == 156...", len(tracks) == 156, f"count={len(tracks)}")
    ...
    # [7] summary_by_tier matches a fresh recount; counts sum to total_tracks == 156
    ...
    check("[7b] total_tracks == 156...", out.get("total_tracks") == 156,
