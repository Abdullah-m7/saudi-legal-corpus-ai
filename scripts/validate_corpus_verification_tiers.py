...
Checks:
  ...
  3.  Exactly 291 track entries, one per REQUIRED_TRACK_IDS (imported from
  ...
  9.  Read-only: this validator does not modify data/corpus_registry/corpus_registry.json,
      any of the 291 tracks' own files, or the committed output file itself.
...
def main() -> int:
    ...
    # [3] Exactly 291 tracks, matching REQUIRED_TRACK_IDS exactly (no missing, no extras)
    ...
    # [7] summary_by_tier matches a fresh recount; counts sum to total_tracks == 291
