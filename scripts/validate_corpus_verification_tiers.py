...
Checks:
  ...
  3.  Exactly 135 track entries, one per REQUIRED_TRACK_IDS (imported from
      scripts/validate_corpus_registry.py — the same canonical 135-track-id list the
  ...
  7.  `summary_by_tier` in the output matches a fresh recount of the `tracks` array, and
      the four tier counts sum to `total_tracks` == 135.
  ...
  9.  Read-only: this validator does not modify data/corpus_registry/corpus_registry.json,
      any of the 135 tracks' own files, or the committed output file itself.
...
def main() -> int:
    ...
    # [3] Exactly 135 tracks, matching REQUIRED_TRACK_IDS exactly (no missing, no extras)
    ...
    check("[3a] Track count == 135...", len(tracks) == 135, f"count={len(tracks)}")
    ...
    # [7] summary_by_tier matches a fresh recount; counts sum to total_tracks == 135
    ...
    check("[7b] total_tracks == 135...", out.get("total_tracks") == 135,
