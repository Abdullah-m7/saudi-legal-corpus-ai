#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية بين حكومة المملكة وحكومة جمهورية العراق لتجنب الازدواج الضريبي track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_iraq_double_taxation", "verified",
                       "treaty_iraq_double_taxation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_iraq_double_taxation", "verified",
                       "treaty_iraq_double_taxation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_iraq_double_taxation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_iraq_double_taxation_no_decision_number_in_source", "treaty_iraq_double_taxation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية بين حكومة المملكة وحكومة جمهورية العراق لتجنب الازدواج الضريبي":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Iraq Double Taxation Avoidance Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Iraq Double Taxation Avoidance Agreement")
