#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية للهيئة العامة للطرق track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19895),
...
SRC = os.path.join(ROOT, "sources", "general_roads_authority_statute", "official_source",
                   "general_roads_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "general_roads_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "general_roads_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "general_roads_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "general_roads_authority_statute_arabic_legal_llm",
                        "general_roads_authority_statute_legal_llm_001_014.json")

LAW_ID = "sa-general-roads-authority-statute"
LAW_AR = "الترتيبات التنظيمية للهيئة العامة للطرق"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"general_roads_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19895), published 14/1/1444H (2022-08-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "general_roads_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "GENERAL_ROADS_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "general-roads-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "general_roads_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the General Authority for Roads -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/1/1444H / 2022-08-12)"),
                "source_authority_ar": ("الترتيبات التنظيمية للهيئة العامة للطرق — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "general_roads_authority_statute",
        "layer": "GENERAL_ROADS_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "general-roads-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the General Authority for Roads — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the General Authority for Roads records" % (len(ver), len(llm)))
