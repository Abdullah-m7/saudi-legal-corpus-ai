#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط تخصيص واسترداد ومناقلة العقارات بين الجهات الحكومية track.
...
RECORDS = os.path.join(ROOT, "sources", "government_property_allocation_transfer_controls", "verified",
                       "government_property_allocation_transfer_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "government_property_allocation_transfer_controls", "verified",
                       "government_property_allocation_transfer_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "government_property_allocation_transfer_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"government_property_allocation_transfer_controls_no_decision_number_in_source", "government_property_allocation_transfer_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "9/6/1442":
        e.append("[2j] gazette_publication_date_hijri must be 9/6/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-01-22":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-01-22")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط تخصيص واسترداد ومناقلة العقارات بين الجهات الحكومية":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on the Allocation, Recovery and Transfer of Real Estate between Government Entities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on the Allocation, Recovery and Transfer of Real Estate between Government Entities")
