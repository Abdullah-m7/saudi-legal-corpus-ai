#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام حماية المؤشرات الجغرافية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28662),
published 7/6/1447H (2025-11-28). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "geographical_indications_protection_law", "official_source",
                   "geographical_indications_protection_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "geographical_indications_protection_law", "verified")
RECORDS = os.path.join(OUT_VER, "geographical_indications_protection_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "geographical_indications_protection_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "geographical_indications_protection_law_arabic_legal_llm",
                        "geographical_indications_protection_law_legal_llm_001_027.json")

LAW_ID = "sa-geographical-indications-protection-law"
LAW_AR = "نظام حماية المؤشرات الجغرافية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"geographical_indications_protection_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28662), published 7/6/1447H (2025-11-28). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "geographical_indications_protection_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "GEOGRAPHICAL_INDICATIONS_PROTECTION_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "geographical-indications-protection-law-llm-%03d" % idx,
            ...
            "article_path": "geographical_indications_protection_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law on the Protection of Geographical Indications -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/6/1447H / 2025-11-28)"),
                "source_authority_ar": ("نظام حماية المؤشرات الجغرافية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 7/6/1447هـ الموافق 2025-11-28م)"),
    ...
    json.dump({
        "law_key": "geographical_indications_protection_law",
        "layer": "GEOGRAPHICAL_INDICATIONS_PROTECTION_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "geographical-indications-protection-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (27 مادة، جميعها أصلية)",
        "title_en": "Law on the Protection of Geographical Indications — Arabic LLM-ready layer (27 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law on the Protection of Geographical Indications records" % (len(ver), len(llm)))
