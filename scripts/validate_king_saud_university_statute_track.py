#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the النظام الأساس لجامعة الملك سعود track.
...
RECORDS = os.path.join(ROOT, "sources", "king_saud_university_statute", "verified",
                       "king_saud_university_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "king_saud_university_statute", "verified",
                       "king_saud_university_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "king_saud_university_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"king_saud_university_statute_no_decision_number_in_source", "king_saud_university_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if sc.get("اصلية", 0) != 21 or sc.get("معدلة", 0) != 1:
        e.append("[2] status split %d اصلية / %d معدلة, expected 21 / 1"
    ...
    if src.get("document") != "النظام الأساس لجامعة الملك سعود":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of King Saud University track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of King Saud University")
