#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة التراخيص - هيئة الأفلام track.
...
RECORDS = os.path.join(ROOT, "sources", "film_authority_licensing_regulation", "verified",
                       "film_authority_licensing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "film_authority_licensing_regulation", "verified",
                       "film_authority_licensing_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "film_authority_licensing_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"film_authority_licensing_regulation_no_decision_number_in_source", "film_authority_licensing_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة التراخيص - هيئة الأفلام":
    ...
    if e:
        print("FAIL: %d error(s) in Licensing Regulation — Film Authority track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Licensing Regulation — Film Authority")
