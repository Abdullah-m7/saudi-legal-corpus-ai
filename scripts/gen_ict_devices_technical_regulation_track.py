#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة الفنية لأجهزة الاتصالات وتقنية المعلومات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25398),
...
SRC = os.path.join(ROOT, "sources", "ict_devices_technical_regulation", "official_source",
                   "ict_devices_technical_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "ict_devices_technical_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "ict_devices_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "ict_devices_technical_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "ict_devices_technical_regulation_arabic_legal_llm",
                        "ict_devices_technical_regulation_legal_llm_001_012.json")

LAW_ID = "sa-ict-devices-technical-regulation"
LAW_AR = "اللائحة الفنية لأجهزة الاتصالات وتقنية المعلومات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"ict_devices_technical_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25398), published 12/2/1446H (2024-08-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "ict_devices_technical_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "ICT_DEVICES_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "ict-devices-technical-regulation-llm-%03d" % idx,
            ...
            "article_path": "ict_devices_technical_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Technical Regulation for Communications and Information Technology Devices -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 12/2/1446H / 2024-08-16)"),
                "source_authority_ar": ("اللائحة الفنية لأجهزة الاتصالات وتقنية المعلومات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "ict_devices_technical_regulation",
        "layer": "ICT_DEVICES_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "ict-devices-technical-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Technical Regulation for Communications and Information Technology Devices — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Technical Regulation for Communications and Information Technology Devices records" % (len(ver), len(llm)))
