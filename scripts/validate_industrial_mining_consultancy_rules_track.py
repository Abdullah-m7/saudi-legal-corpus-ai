#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد تنظيم ممارسة مهنتي الاستشارات الصناعية والاستشارات التعدينية track.
...
RECORDS = os.path.join(ROOT, "sources", "industrial_mining_consultancy_rules", "verified",
                       "industrial_mining_consultancy_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "industrial_mining_consultancy_rules", "verified",
                       "industrial_mining_consultancy_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "industrial_mining_consultancy_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"industrial_mining_consultancy_rules_no_decision_number_in_source", "industrial_mining_consultancy_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "3/1/1445":
        e.append("[2j] gazette_publication_date_hijri must be 3/1/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-07-21":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-07-21")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد تنظيم ممارسة مهنتي الاستشارات الصناعية والاستشارات التعدينية":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing the Practice of the Industrial Consultancy and Mining Consultancy Professions track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing the Practice of the Industrial Consultancy and Mining Consultancy Professions")
