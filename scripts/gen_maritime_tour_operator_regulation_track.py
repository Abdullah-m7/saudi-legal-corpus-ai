#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية لمنظم الرحلات البحرية السياحية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23415),
...
SRC = os.path.join(ROOT, "sources", "maritime_tour_operator_regulation", "official_source",
                   "maritime_tour_operator_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "maritime_tour_operator_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "maritime_tour_operator_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "maritime_tour_operator_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "maritime_tour_operator_regulation_arabic_legal_llm",
                        "maritime_tour_operator_regulation_legal_llm_001_010.json")

LAW_ID = "sa-maritime-tour-operator-regulation"
LAW_AR = "اللائحة التنظيمية لمنظم الرحلات البحرية السياحية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"maritime_tour_operator_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23415), published 17/1/1445H (2023-08-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "maritime_tour_operator_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MARITIME_TOUR_OPERATOR_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "maritime-tour-operator-regulation-llm-%03d" % idx,
            ...
            "article_path": "maritime_tour_operator_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for the Maritime Tour Operator -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/1/1445H / 2023-08-04)"),
                "source_authority_ar": ("اللائحة التنظيمية لمنظم الرحلات البحرية السياحية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "maritime_tour_operator_regulation",
        "layer": "MARITIME_TOUR_OPERATOR_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "maritime-tour-operator-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for the Maritime Tour Operator — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for the Maritime Tour Operator records" % (len(ver), len(llm)))
