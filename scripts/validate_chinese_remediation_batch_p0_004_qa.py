#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate the Batch P0-004 QA (article-by-article review vs the official Arabic; review only).

Confirms the QA covers exactly the 20 P0-004 articles across Babs 4/5/6, that every per-article bab
is in [4,5,6] and equals both the P0-004 remediation record bab and the coverage-index
expected_bab_number, that expected_bab_distribution matches the authorized split, that only allowed
qa_status values appear, that final_status/qa_summary stay consistent with the per-article statuses,
carries the correct legal-hierarchy / non-official / non-binding / non-governing posture with human
review pending, points at the P0-004 remediation file, embeds no full Arabic/English/Chinese text,
and touches no protected layer (P0-001..P0-004 and QA, plus the base corpora). Read-only, idempotent.

Usage: validate_chinese_remediation_batch_p0_004_qa.py [QA_JSON_PATH]
...
SRC_REL = "data/chinese_remediation_batches/p0_004/companies_law_m132_1443_zh_internal_remediation_p0_004.json"
...
REQ_TOP = ("stage", "batch_id", "source_batch_file", "scope_articles", "expected_babs",
           ...
           "trilingual_alignment_created", "p0_004_chinese_text_modified", "p0_004_data_modified",
...
def main(argv=None) -> int:
    ...
    problems = []
    for p, label in ((qa_path, "QA JSON"), (MD, "Arabic QA report"), (SRC, "P0-004 remediation file")):
    ...
    if qa.get("stage") != "CHINESE_REMEDIATION_BATCH_P0_004_QA":
        problems.append("stage must be CHINESE_REMEDIATION_BATCH_P0_004_QA")
    if qa.get("batch_id") != "P0-004":
        problems.append("batch_id must be P0-004")
    if qa.get("source_batch_file") != SRC_REL:
        problems.append("source_batch_file must point to the P0-004 remediation JSON")
    if qa.get("scope_articles") != ARTS:
        problems.append("scope_articles must exactly match the P0-004 list")
    if qa.get("expected_babs") != [4, 5, 6]:
    ...
    if {str(k): list(v) for k, v in dist.items()} != DIST:
        problems.append("expected_bab_distribution must match the authorized Bab 4/5/6 split")
    for f in ("full_chinese_translation_claimed", "official_chinese_translation_claimed",
              ...
              "p0_004_chinese_text_modified", "p0_004_data_modified"):
    ...
    if nums != ARTS:
        problems.append("per_article_reviews must cover exactly the 20 P0-004 articles in order")
    ...
    for r in recs:
        ...
        if n in src_bab and r.get("bab") != src_bab[n]:
            problems.append("art %s QA bab %r != P0-004 record bab %r" % (n, r.get("bab"), src_bab[n]))
    ...
    # P0-004 remediation file must be intact (read-only; unchanged scope/posture)
    if [r["article_number"] for r in src["records"]] != ARTS:
        problems.append("P0-004 remediation file scope changed (must remain the 20 articles)")
    if src.get("human_legal_review_status") != "pending_human_legal_review":
        problems.append("P0-004 remediation human_legal_review_status must remain pending")
    for f in ("official_chinese_translation_claimed", "chinese_binding_claimed",
        ...
        if src.get(f) is not False:
            problems.append("P0-004 remediation posture flag %s must remain false" % f)

    # sibling batches / QA unchanged
    ...
    if problems:
        ...
        print("RESULT: %d problem(s) found ✗" % len(problems))
        return 1
    print("[PASS] QA of 20 P0-004 articles (Babs 4/5/6) vs official Arabic (English secondary); "
          "each QA bab matches the P0-004 record and the coverage index; distribution 4:[136,137] "
          "5:[140..148] 6:[159..180]; allowed qa_status only; legal hierarchy Arabic-governing / "
          "Chinese internal / non-official / non-binding / non-governing; human review pending; "
          "final_status=%s (pass=%d minor=%d blocked=%d fail=%d) consistent with summary; no full "
          "Arabic/English/Chinese text embedded; P0-004 remediation intact; P0-001..P0-003 + their "
