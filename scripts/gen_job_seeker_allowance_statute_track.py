#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم إعانة البحث عن عمل track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17599),
...
SRC = os.path.join(ROOT, "sources", "job_seeker_allowance_statute", "official_source",
                   "job_seeker_allowance_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "job_seeker_allowance_statute", "verified")
RECORDS = os.path.join(OUT_VER, "job_seeker_allowance_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "job_seeker_allowance_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "job_seeker_allowance_statute_arabic_legal_llm",
                        "job_seeker_allowance_statute_legal_llm_001_013.json")

LAW_ID = "sa-job-seeker-allowance-statute"
LAW_AR = "تنظيم إعانة البحث عن عمل"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"job_seeker_allowance_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17599), published 2/10/1442H (2021-05-14). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "job_seeker_allowance_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "JOB_SEEKER_ALLOWANCE_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "job-seeker-allowance-statute-llm-%03d" % idx,
            ...
            "article_path": "job_seeker_allowance_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Job Search Allowance -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/10/1442H / 2021-05-14)"),
                "source_authority_ar": ("تنظيم إعانة البحث عن عمل — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "job_seeker_allowance_statute",
        "layer": "JOB_SEEKER_ALLOWANCE_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "job-seeker-allowance-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Statute of the Job Search Allowance — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Job Search Allowance records" % (len(ver), len(llm)))
