#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم المركز الوطني لتنمية القطاع غير الربحي track.
...
RECORDS = os.path.join(ROOT, "sources", "nonprofit_sector_development_center_statute", "verified",
                       "nonprofit_sector_development_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "nonprofit_sector_development_center_statute", "verified",
                       "nonprofit_sector_development_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "nonprofit_sector_development_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"nonprofit_sector_development_center_statute_no_decision_number_in_source", "nonprofit_sector_development_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم المركز الوطني لتنمية القطاع غير الربحي":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the National Center for Non-Profit Sector Development track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the National Center for Non-Profit Sector Development")
