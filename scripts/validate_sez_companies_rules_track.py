#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد الشركات في المناطق الاقتصادية الخاصة track.

108 records: 108 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "sez_companies_rules", "verified",
                       "sez_companies_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "sez_companies_rules", "verified",
                       "sez_companies_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "sez_companies_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"sez_companies_rules_no_decision_number_in_source", "sez_companies_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد الشركات في المناطق الاقتصادية الخاصة":
    ...
    if e:
        print("FAIL: %d error(s) in Companies Rules in the Special Economic Zones track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Companies Rules in the Special Economic Zones")
    print("  - 108 records: 108 articles, no appendices; all 108 اصلية")
