#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية للمنشآت الاجتماعية غير الحكومية للأشخاص ذوي الإعاقة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24650),
...
SRC = os.path.join(ROOT, "sources", "disability_nongov_social_facilities_regulation", "official_source",
                   "disability_nongov_social_facilities_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "disability_nongov_social_facilities_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "disability_nongov_social_facilities_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "disability_nongov_social_facilities_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "disability_nongov_social_facilities_regulation_arabic_legal_llm",
                        "disability_nongov_social_facilities_regulation_legal_llm_001_030.json")

LAW_ID = "sa-disability-nongov-social-facilities-regulation"
LAW_AR = "اللائحة التنظيمية للمنشآت الاجتماعية غير الحكومية للأشخاص ذوي الإعاقة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"disability_nongov_social_facilities_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24650), published 27/8/1445H (2024-03-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "disability_nongov_social_facilities_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "DISABILITY_NONGOV_SOCIAL_FACILITIES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "disability-nongov-social-facilities-regulation-llm-%03d" % idx,
            ...
            "article_path": "disability_nongov_social_facilities_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for Non-Governmental Social Facilities for Persons with Disabilities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/8/1445H / 2024-03-08)"),
                "source_authority_ar": ("اللائحة التنظيمية للمنشآت الاجتماعية غير الحكومية للأشخاص ذوي الإعاقة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "disability_nongov_social_facilities_regulation",
        "layer": "DISABILITY_NONGOV_SOCIAL_FACILITIES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "disability-nongov-social-facilities-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (30 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for Non-Governmental Social Facilities for Persons with Disabilities — Arabic LLM-ready layer (30 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for Non-Governmental Social Facilities for Persons with Disabilities records" % (len(ver), len(llm)))
