#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لمجلس المخاطر الوطنية track.
...
RECORDS = os.path.join(ROOT, "sources", "national_risk_council_statute", "verified",
                       "national_risk_council_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "national_risk_council_statute", "verified",
                       "national_risk_council_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "national_risk_council_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"national_risk_council_statute_no_decision_number_in_source", "national_risk_council_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية لمجلس المخاطر الوطنية":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the National Risk Council track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the National Risk Council")
