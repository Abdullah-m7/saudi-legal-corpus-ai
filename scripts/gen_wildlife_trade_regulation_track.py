#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية للإتجار بالكائنات الفطرية ومنتجاتها ومشتقاتها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17881),
...
SRC = os.path.join(ROOT, "sources", "wildlife_trade_regulation", "official_source",
                   "wildlife_trade_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "wildlife_trade_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "wildlife_trade_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "wildlife_trade_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "wildlife_trade_regulation_arabic_legal_llm",
                        "wildlife_trade_regulation_legal_llm_001_013.json")

LAW_ID = "sa-wildlife-trade-regulation"
LAW_AR = "اللائحة التنفيذية للإتجار بالكائنات الفطرية ومنتجاتها ومشتقاتها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"wildlife_trade_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17881), published 22/11/1442H (2021-07-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "wildlife_trade_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "WILDLIFE_TRADE_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "wildlife-trade-regulation-llm-%03d" % idx,
            ...
            "article_path": "wildlife_trade_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation on Trade in Wild Organisms, their Products and Derivatives -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/11/1442H / 2021-07-02)"),
                "source_authority_ar": ("اللائحة التنفيذية للإتجار بالكائنات الفطرية ومنتجاتها ومشتقاتها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "wildlife_trade_regulation",
        "layer": "WILDLIFE_TRADE_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "wildlife-trade-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation on Trade in Wild Organisms, their Products and Derivatives — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation on Trade in Wild Organisms, their Products and Derivatives records" % (len(ver), len(llm)))
