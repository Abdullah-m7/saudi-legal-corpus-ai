#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد تنظيم ممارسة مهنتي الاستشارات الصناعية والاستشارات التعدينية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23312),
published 3/1/1445H (2023-07-21). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "industrial_mining_consultancy_rules", "official_source",
                   "industrial_mining_consultancy_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "industrial_mining_consultancy_rules", "verified")
RECORDS = os.path.join(OUT_VER, "industrial_mining_consultancy_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "industrial_mining_consultancy_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "industrial_mining_consultancy_rules_arabic_legal_llm",
                        "industrial_mining_consultancy_rules_legal_llm_001_014.json")

LAW_ID = "sa-industrial-mining-consultancy-rules"
LAW_AR = "قواعد تنظيم ممارسة مهنتي الاستشارات الصناعية والاستشارات التعدينية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"industrial_mining_consultancy_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23312), published 3/1/1445H (2023-07-21). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "industrial_mining_consultancy_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "INDUSTRIAL_MINING_CONSULTANCY_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "industrial-mining-consultancy-rules-llm-%03d" % idx,
            ...
            "article_path": "industrial_mining_consultancy_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing the Practice of the Industrial Consultancy and Mining Consultancy Professions -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/1/1445H / 2023-07-21)"),
                "source_authority_ar": ("قواعد تنظيم ممارسة مهنتي الاستشارات الصناعية والاستشارات التعدينية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 3/1/1445هـ الموافق 2023-07-21م)"),
    ...
    json.dump({
        "law_key": "industrial_mining_consultancy_rules",
        "layer": "INDUSTRIAL_MINING_CONSULTANCY_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "industrial-mining-consultancy-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Rules Governing the Practice of the Industrial Consultancy and Mining Consultancy Professions — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing the Practice of the Industrial Consultancy and Mining Consultancy Professions records" % (len(ver), len(llm)))
