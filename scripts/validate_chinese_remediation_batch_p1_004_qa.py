#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate the Batch P1-004 QA (article-by-article review vs the official Arabic; Babs 10/12/13/14).

Confirms the QA covers exactly the 16 P1-004 articles across Babs 10/12/13/14, that every per-article
bab is in [10,12,13,14] and equals both the P1-004 remediation record bab and the coverage-index
expected_bab_number, that expected_bab_distribution matches the authorized split, that only allowed
qa_status values appear, that final_status/qa_summary/pass-minor-blocked-fail counts stay consistent
with the per-article statuses, that source traceability is re-verified (Arabic-source / English-guidance
/ prior-candidate / semantic-QA-P1 links match the live records), that the QA carries the correct
legal-hierarchy / non-official / non-binding / non-governing posture under the repository review model,
points at the P1-004 remediation file, embeds no full Arabic/English/Chinese text, and touches no
protected layer (P1-004 data, P1-003 data + QA, P1-002 data + QA, P1-001 data + QA, all P0 batches + QA,
and the base corpora). Review only: P1-004 data must be unmodified unless a concrete minor fix is
recorded. No p1_005+/P2/P3 batch dirs; no full Chinese 281 / trilingual. Read-only, idempotent.

Usage: validate_chinese_remediation_batch_p1_004_qa.py [QA_JSON_PATH]
...
MD = os.path.join(ROOT, "reports", "chinese_translation_review",
                  "CHINESE_REMEDIATION_BATCH_P1_004_QA_AR.md")
SRC_REL = "data/chinese_remediation_batches/p1_004/companies_law_m132_1443_zh_internal_remediation_p1_004.json"
...
P1_001_SRC = os.path.join(ROOT, "data", "chinese_remediation_batches", "p1_001",
                          "companies_law_m132_1443_zh_internal_remediation_p1_001.json")
P1_002_SRC = os.path.join(ROOT, "data", "chinese_remediation_batches", "p1_002",
                          "companies_law_m132_1443_zh_internal_remediation_p1_002.json")
P1_003_SRC = os.path.join(ROOT, "data", "chinese_remediation_batches", "p1_003",
...
# P0-002..P0-005 QA plus the P1-001, P1-002 and P1-003 QA are all QA_PASS and must remain untouched.
...
DIST = {"10": [230, 233], "12": [242, 244, 248, 252, 253, 255, 256],
...
REQ_TOP = ("stage", "qa_stage", "batch_id", "source_batch_file", "scope_articles", "expected_babs",
           ...
           "trilingual_alignment_created", "p1_004_chinese_text_modified", "p1_004_data_modified",
...
def main(argv=None) -> int:
    ...
    problems = []
    for p, label in ((qa_path, "QA JSON"), (MD, "Arabic QA report"), (SRC, "P1-004 remediation file")):
    ...
    if qa.get("stage") != "CHINESE_REMEDIATION_BATCH_P1_004_QA":
        problems.append("stage must be CHINESE_REMEDIATION_BATCH_P1_004_QA")
    if qa.get("qa_stage") != "CHINESE_REMEDIATION_BATCH_P1_004_QA":
        problems.append("qa_stage must be CHINESE_REMEDIATION_BATCH_P1_004_QA")
    if qa.get("batch_id") != "P1-004":
        problems.append("batch_id must be P1-004")
    if qa.get("source_batch_file") != SRC_REL:
        problems.append("source_batch_file must point to the P1-004 remediation JSON")
    if qa.get("scope_articles") != ARTS:
        problems.append("scope_articles must exactly match the P1-004 list")
    if qa.get("article_count") != 16:
        problems.append("article_count must be 16")
    if qa.get("expected_babs") != [10, 12, 13, 14]:
    ...
    if {str(k): list(v) for k, v in dist.items()} != DIST:
        problems.append("expected_bab_distribution must match the authorized Bab 10/12/13/14 split")
    ...
    if nums != ARTS:
        problems.append("per_article_reviews must cover exactly the 16 P1-004 articles in order")
    ...
    for r in recs:
        ...
        if n in srec and r.get("bab") != srec[n].get("bab"):
            problems.append("art %s QA bab %r != P1-004 record bab %r" % (n, r.get("bab"), srec[n].get("bab")))
    ...
    if qa.get("p1_004_chinese_text_modified") is True or qa.get("p1_004_data_modified") is True:
        if not fixes:
            problems.append("p1_004 marked modified but no minor_fixes recorded")
    else:
        if fixes:
            problems.append("minor_fixes recorded but p1_004 marked unmodified")
    ...
    # P1-004 remediation intact; P1-003 + P1-002 + P1-001 remediation + all P0 batches + QA unchanged
    if len(src["records"]) != 16:
        problems.append("P1-004 remediation must remain 16 records")
    for label, path in (("P1-001", P1_001_SRC), ("P1-002", P1_002_SRC), ("P1-003", P1_003_SRC)):
        if not os.path.exists(path) or len(_read(path)["records"]) != 20:
            problems.append("%s remediation must remain 20 records (untouched)" % label)
    ...
    # P1-001..P1-004 authorized; only p1_005+/P2/P3 forbidden; no full-281 / trilingual artifacts
    ...
    if later:
        problems.append("only P1-001..P1-004 authorized; no p1_005+/P2/P3 batch dirs: %s"
    ...
    print("=" * 60)
    print("Chinese remediation Batch P1-004 QA validation")
    print("=" * 60)
    if problems:
        for p in problems:
            print("  -", p)
        print("RESULT: %d problem(s) found ✗" % len(problems))
        return 1
    print("[PASS] Batch P1-004 QA: 16 articles across Babs [10,12,13,14] reviewed article-by-article vs "
          "the official Arabic (English secondary); each QA bab matches the P1-004 record and the "
          "coverage index; source traceability re-verified for every pass (Arabic/English/prior-"
          "candidate/semantic-QA-P1 links); final_status/qa_summary/counts consistent with per-article "
          "statuses; legal hierarchy internal/non-official/non-binding/non-governing under the "
          "repository review model; no full Arabic/English/Chinese text embedded; review-only (P1-004 "
          "data unmodified); P1-003 data + QA, P1-002 data + QA, P1-001 data + QA and all P0 batches + "
          "QA + Chinese candidate 189 + old Chinese 5/23 + Arabic/English/English-reference 281 + "
          "Arabic source + Chinese sources 14 + OCR queue unchanged; no other P1/P2/P3 dirs; no "
          "full-281 / trilingual.")
