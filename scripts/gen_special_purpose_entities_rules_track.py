#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة للمنشآت ذات الأغراض الخاصة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17849),
...
SRC = os.path.join(ROOT, "sources", "special_purpose_entities_rules", "official_source",
                   "special_purpose_entities_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "special_purpose_entities_rules", "verified")
RECORDS = os.path.join(OUT_VER, "special_purpose_entities_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "special_purpose_entities_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "special_purpose_entities_rules_arabic_legal_llm",
                        "special_purpose_entities_rules_legal_llm_001_055.json")

LAW_ID = "sa-special-purpose-entities-rules"
LAW_AR = "القواعد المنظمة للمنشآت ذات الأغراض الخاصة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"special_purpose_entities_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17849), published 15/11/1442H (2021-06-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "special_purpose_entities_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SPECIAL_PURPOSE_ENTITIES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "special-purpose-entities-rules-llm-%03d" % idx,
            ...
            "article_path": "special_purpose_entities_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing Special Purpose Entities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/11/1442H / 2021-06-25)"),
                "source_authority_ar": ("القواعد المنظمة للمنشآت ذات الأغراض الخاصة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "special_purpose_entities_rules",
        "layer": "SPECIAL_PURPOSE_ENTITIES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "special-purpose-entities-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (55 مادة، جميعها أصلية)",
        "title_en": "Rules Governing Special Purpose Entities — Arabic LLM-ready layer (55 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing Special Purpose Entities records" % (len(ver), len(llm)))
