#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لمركز الأمير محمد بن سلمان العالمي للخط العربي track.
...
RECORDS = os.path.join(ROOT, "sources", "arabic_calligraphy_center_statute", "verified",
                       "arabic_calligraphy_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "arabic_calligraphy_center_statute", "verified",
                       "arabic_calligraphy_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "arabic_calligraphy_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"arabic_calligraphy_center_statute_no_decision_number_in_source", "arabic_calligraphy_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "3/9/1447":
        e.append("[2j] gazette_publication_date_hijri must be 3/9/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-02-20":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-02-20")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية لمركز الأمير محمد بن سلمان العالمي للخط العربي":
    ...
    if e:
        print("FAIL: %d error(s) in Prince Mohammed bin Salman Global Center for Arabic Calligraphy — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Prince Mohammed bin Salman Global Center for Arabic Calligraphy — Organisational Arrangements")
