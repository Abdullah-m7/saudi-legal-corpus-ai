#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط تربية النحل في محمية الملك عبدالعزيز الملكية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26890),
...
SRC = os.path.join(ROOT, "sources", "king_abdulaziz_reserve_beekeeping_controls", "official_source",
                   "king_abdulaziz_reserve_beekeeping_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "king_abdulaziz_reserve_beekeeping_controls", "verified")
RECORDS = os.path.join(OUT_VER, "king_abdulaziz_reserve_beekeeping_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "king_abdulaziz_reserve_beekeeping_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "king_abdulaziz_reserve_beekeeping_controls_arabic_legal_llm",
                        "king_abdulaziz_reserve_beekeeping_controls_legal_llm_001_009.json")

LAW_ID = "sa-king-abdulaziz-reserve-beekeeping-controls"
LAW_AR = "ضوابط تربية النحل في محمية الملك عبدالعزيز الملكية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"king_abdulaziz_reserve_beekeeping_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26890), published 10/7/1446H (2025-01-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "king_abdulaziz_reserve_beekeeping_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "KING_ABDULAZIZ_RESERVE_BEEKEEPING_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "king-abdulaziz-reserve-beekeeping-controls-llm-%03d" % idx,
            ...
            "article_path": "king_abdulaziz_reserve_beekeeping_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls for Beekeeping in the King Abdulaziz Royal Reserve -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 10/7/1446H / 2025-01-10)"),
                "source_authority_ar": ("ضوابط تربية النحل في محمية الملك عبدالعزيز الملكية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "king_abdulaziz_reserve_beekeeping_controls",
        "layer": "KING_ABDULAZIZ_RESERVE_BEEKEEPING_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "king-abdulaziz-reserve-beekeeping-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (9 مادة، جميعها أصلية)",
        "title_en": "Controls for Beekeeping in the King Abdulaziz Royal Reserve — Arabic LLM-ready layer (9 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls for Beekeeping in the King Abdulaziz Royal Reserve records" % (len(ver), len(llm)))
