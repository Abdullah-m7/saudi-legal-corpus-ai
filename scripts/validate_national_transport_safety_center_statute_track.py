#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم المركز الوطني لسلامة النقل track.
...
RECORDS = os.path.join(ROOT, "sources", "national_transport_safety_center_statute", "verified",
                       "national_transport_safety_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "national_transport_safety_center_statute", "verified",
                       "national_transport_safety_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "national_transport_safety_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"national_transport_safety_center_statute_no_decision_number_in_source", "national_transport_safety_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "27/3/1447":
        e.append("[2j] gazette_publication_date_hijri must be 27/3/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-09-19":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-09-19")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم المركز الوطني لسلامة النقل":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the National Center for Transport Safety track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the National Center for Transport Safety")
