#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم الهيئة العامة لعقارات الدولة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17382),
...
SRC = os.path.join(ROOT, "sources", "state_property_authority_statute", "official_source",
                   "state_property_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "state_property_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "state_property_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "state_property_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "state_property_authority_statute_arabic_legal_llm",
                        "state_property_authority_statute_legal_llm_001_016.json")

LAW_ID = "sa-state-property-authority-statute"
LAW_AR = "تنظيم الهيئة العامة لعقارات الدولة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"state_property_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17382), published 24/5/1442H (2021-01-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "state_property_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "STATE_PROPERTY_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "state-property-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "state_property_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the General Authority for State Real Estate -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/5/1442H / 2021-01-08)"),
                "source_authority_ar": ("تنظيم الهيئة العامة لعقارات الدولة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "state_property_authority_statute",
        "layer": "STATE_PROPERTY_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "state-property-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Statute of the General Authority for State Real Estate — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the General Authority for State Real Estate records" % (len(ver), len(llm)))
