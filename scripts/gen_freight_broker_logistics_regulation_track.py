#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة المنظمة لنشاط وسيط الشحن (الخدمات اللوجستية) track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23863),
published 28/3/1445H (2023-10-13). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "freight_broker_logistics_regulation", "official_source",
                   "freight_broker_logistics_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "freight_broker_logistics_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "freight_broker_logistics_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "freight_broker_logistics_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "freight_broker_logistics_regulation_arabic_legal_llm",
                        "freight_broker_logistics_regulation_legal_llm_001_031.json")

LAW_ID = "sa-freight-broker-logistics-regulation"
LAW_AR = "اللائحة المنظمة لنشاط وسيط الشحن (الخدمات اللوجستية)"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"freight_broker_logistics_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23863), published 28/3/1445H (2023-10-13). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "freight_broker_logistics_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "FREIGHT_BROKER_LOGISTICS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "freight-broker-logistics-regulation-llm-%03d" % idx,
            ...
            "article_path": "freight_broker_logistics_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Governing Freight Brokerage (Logistics Services) -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/3/1445H / 2023-10-13)"),
                "source_authority_ar": ("اللائحة المنظمة لنشاط وسيط الشحن (الخدمات اللوجستية) — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 28/3/1445هـ الموافق 2023-10-13م)"),
    ...
    json.dump({
        "law_key": "freight_broker_logistics_regulation",
        "layer": "FREIGHT_BROKER_LOGISTICS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "freight-broker-logistics-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (31 مادة، جميعها أصلية)",
        "title_en": "Regulation Governing Freight Brokerage (Logistics Services) — Arabic LLM-ready layer (31 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Governing Freight Brokerage (Logistics Services) records" % (len(ver), len(llm)))
