#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تعليمات الحسابات الاستثمارية track.
...
RECORDS = os.path.join(ROOT, "sources", "investment_accounts_instructions", "verified",
                       "investment_accounts_instructions_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "investment_accounts_instructions", "verified",
                       "investment_accounts_instructions_verified_summary.json")
LLM = os.path.join(ROOT, "data", "investment_accounts_instructions_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"investment_accounts_instructions_no_decision_number_in_source", "investment_accounts_instructions_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تعليمات الحسابات الاستثمارية":
    ...
    if e:
        print("FAIL: %d error(s) in Investment Accounts Instructions track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Investment Accounts Instructions")
