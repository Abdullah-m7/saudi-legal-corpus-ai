#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة المنتجات الاستثمارية الوقفية track.

CITATION: قرار مجلس الإدارة رقم (21/1/47/ت) وتاريخ 26/11/1447هـ
Text fetched directly from the Umm Al-Qura Official Gazette's own
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4001176),
published 26/12/1447H (2026-06-12). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "waqf_investment_products_regulation", "official_source",
                   "waqf_investment_products_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "waqf_investment_products_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "waqf_investment_products_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "waqf_investment_products_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "waqf_investment_products_regulation_arabic_legal_llm",
                        "waqf_investment_products_regulation_legal_llm_001_037.json")

LAW_ID = "sa-waqf-investment-products-regulation"
LAW_AR = "لائحة المنتجات الاستثمارية الوقفية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"waqf_investment_products_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4001176), published 26/12/1447H (2026-06-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "waqf_investment_products_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "WAQF_INVESTMENT_PRODUCTS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "waqf-investment-products-regulation-llm-%03d" % idx,
            ...
            "article_path": "waqf_investment_products_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Waqf Investment Products -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 26/12/1447H / 2026-06-12)"),
                "source_authority_ar": ("لائحة المنتجات الاستثمارية الوقفية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 26/12/1447هـ الموافق 2026-06-12م)"),
    ...
    json.dump({
        "law_key": "waqf_investment_products_regulation",
        "layer": "WAQF_INVESTMENT_PRODUCTS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "waqf-investment-products-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (37 مادة، جميعها أصلية)",
        "title_en": "Regulation on Waqf Investment Products — Arabic LLM-ready layer (37 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Waqf Investment Products records" % (len(ver), len(llm)))
