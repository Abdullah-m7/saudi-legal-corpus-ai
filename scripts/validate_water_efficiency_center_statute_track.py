#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم المركز الوطني لكفاءة وترشيد المياه track.
...
RECORDS = os.path.join(ROOT, "sources", "water_efficiency_center_statute", "verified",
                       "water_efficiency_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "water_efficiency_center_statute", "verified",
                       "water_efficiency_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "water_efficiency_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"water_efficiency_center_statute_no_decision_number_in_source", "water_efficiency_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "30/6/1442":
        e.append("[2j] gazette_publication_date_hijri must be 30/6/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-02-12":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-02-12")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم المركز الوطني لكفاءة وترشيد المياه":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the National Center for Water Efficiency and Rationalisation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the National Center for Water Efficiency and Rationalisation")
