#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد حوكمة الجمعيات والمؤسسات الأهلية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4000254),
published 24/8/1447H (2026-02-12). The Gazette IS the official publication of record
...
STRUCTURE: 47 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "nonprofit_governance_rules", "official_source",
                   "nonprofit_governance_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "nonprofit_governance_rules", "verified")
RECORDS = os.path.join(OUT_VER, "nonprofit_governance_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "nonprofit_governance_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "nonprofit_governance_rules_arabic_legal_llm",
                        "nonprofit_governance_rules_legal_llm_001_047.json")

LAW_ID = "sa-nonprofit-governance-rules"
LAW_AR = "قواعد حوكمة الجمعيات والمؤسسات الأهلية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"nonprofit_governance_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4000254), published 24/8/1447H (2026-02-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "nonprofit_governance_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "NONPROFIT_GOVERNANCE_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "nonprofit-governance-rules-llm-%03d" % idx,
            ...
            "article_path": "nonprofit_governance_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Governance Rules for Associations and Non-Profit Foundations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/8/1447H / 2026-02-12)"),
                "source_authority_ar": ("قواعد حوكمة الجمعيات والمؤسسات الأهلية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 24/8/1447هـ الموافق 2026-02-12م)"),
    ...
    json.dump({
        "law_key": "nonprofit_governance_rules",
        "layer": "NONPROFIT_GOVERNANCE_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "nonprofit-governance-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (47 مادة، جميعها أصلية)",
        "title_en": "Governance Rules for Associations and Non-Profit Foundations — Arabic LLM-ready layer (47 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Governance Rules for Associations and Non-Profit Foundations records" % (len(ver), len(llm)))
