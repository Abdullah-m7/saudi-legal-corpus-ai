#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة العامة لنماذج المطابقة track.
...
RECORDS = os.path.join(ROOT, "sources", "conformity_models_general_regulation", "verified",
                       "conformity_models_general_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "conformity_models_general_regulation", "verified",
                       "conformity_models_general_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "conformity_models_general_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"conformity_models_general_regulation_no_decision_number_in_source", "conformity_models_general_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة العامة لنماذج المطابقة":
    ...
    if e:
        print("FAIL: %d error(s) in General Regulation for Conformity Assessment Models track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: General Regulation for Conformity Assessment Models")
