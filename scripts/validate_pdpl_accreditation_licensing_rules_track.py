#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة لتراخيص أنشطة إصدار شهادات الاعتماد لجهات التحكم والمعالجة وأنشطة التدقيق والفحص لمعالجة البيانات الشخصية track.

11 records: 19 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "pdpl_accreditation_licensing_rules", "verified",
                       "pdpl_accreditation_licensing_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "pdpl_accreditation_licensing_rules", "verified",
                       "pdpl_accreditation_licensing_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "pdpl_accreditation_licensing_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "pdpl_accreditation_licensing_rules_found_by_walking_a_url_section_the_index_never_covered",
    "pdpl_accreditation_licensing_rules_no_decision_number_in_source",
    "pdpl_accreditation_licensing_rules_sibling_rules_published_the_same_day",
    "pdpl_accreditation_licensing_rules_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    EXPECTED_NUMBERS = list(range(1, 20))
    MISSING_IN_SOURCE = []
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "17/9/1447":
        e.append("[2j] gazette_publication_date_hijri must be 17/9/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-03-06":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-03-06")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "القواعد المنظمة لتراخيص أنشطة إصدار شهادات الاعتماد لجهات التحكم والمعالجة وأنشطة التدقيق والفحص لمعالجة البيانات الشخصية":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Licensing Accreditation-Certificate Issuance and Audit/Inspection Activities for Personal Data Processing track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Licensing Accreditation-Certificate Issuance and Audit/Inspection Activities for Personal Data Processing")
