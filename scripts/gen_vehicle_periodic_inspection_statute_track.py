#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم الفحص الفني الدوري للمركبات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=16993),
...
SRC = os.path.join(ROOT, "sources", "vehicle_periodic_inspection_statute", "official_source",
                   "vehicle_periodic_inspection_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "vehicle_periodic_inspection_statute", "verified")
RECORDS = os.path.join(OUT_VER, "vehicle_periodic_inspection_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "vehicle_periodic_inspection_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "vehicle_periodic_inspection_statute_arabic_legal_llm",
                        "vehicle_periodic_inspection_statute_legal_llm_001_016.json")

LAW_ID = "sa-vehicle-periodic-inspection-statute"
LAW_AR = "تنظيم الفحص الفني الدوري للمركبات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"vehicle_periodic_inspection_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=16993), published 28/7/1442H (2021-03-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "vehicle_periodic_inspection_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "VEHICLE_PERIODIC_INSPECTION_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "vehicle-periodic-inspection-statute-llm-%03d" % idx,
            ...
            "article_path": "vehicle_periodic_inspection_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute for the Periodic Technical Inspection of Vehicles -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/7/1442H / 2021-03-12)"),
                "source_authority_ar": ("تنظيم الفحص الفني الدوري للمركبات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "vehicle_periodic_inspection_statute",
        "layer": "VEHICLE_PERIODIC_INSPECTION_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "vehicle-periodic-inspection-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Statute for the Periodic Technical Inspection of Vehicles — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute for the Periodic Technical Inspection of Vehicles records" % (len(ver), len(llm)))
