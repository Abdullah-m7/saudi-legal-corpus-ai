#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد عمل لجنة النظر في مخالفات أحكام نظام حقوق الأشخاص ذوي الإعاقة track.
...
RECORDS = os.path.join(ROOT, "sources", "disability_rights_violations_committee_rules", "verified",
                       "disability_rights_violations_committee_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "disability_rights_violations_committee_rules", "verified",
                       "disability_rights_violations_committee_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "disability_rights_violations_committee_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"disability_rights_violations_committee_rules_no_decision_number_in_source", "disability_rights_violations_committee_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/3/1446":
        e.append("[2j] gazette_publication_date_hijri must be 24/3/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-09-27":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-09-27")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد عمل لجنة النظر في مخالفات أحكام نظام حقوق الأشخاص ذوي الإعاقة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules of Procedure of the Committee for Adjudicating Violations of the Law on the Rights of Persons with Disabilities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules of Procedure of the Committee for Adjudicating Violations of the Law on the Rights of Persons with Disabilities")
