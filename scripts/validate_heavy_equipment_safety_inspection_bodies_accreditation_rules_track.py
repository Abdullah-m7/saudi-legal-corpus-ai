#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط اعتماد جهات التفتيش والرقابة على سلامة المعدات الثقيلة track.

12 records: 12 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "heavy_equipment_safety_inspection_bodies_accreditation_rules", "verified",
                       "heavy_equipment_safety_inspection_bodies_accreditation_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "heavy_equipment_safety_inspection_bodies_accreditation_rules", "verified",
                       "heavy_equipment_safety_inspection_bodies_accreditation_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "heavy_equipment_safety_inspection_bodies_accreditation_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"heavy_equipment_safety_inspection_bodies_accreditation_rules_no_decision_number_in_source", "heavy_equipment_safety_inspection_bodies_accreditation_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "17/10/1445":
        e.append("[2j] gazette_publication_date_hijri must be 17/10/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-04-26":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-04-26")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط اعتماد جهات التفتيش والرقابة على سلامة المعدات الثقيلة":
    ...
    if e:
        print("FAIL: %d error(s) in Controls for Accrediting Inspection and Oversight Bodies for Heavy Equipment Safety track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls for Accrediting Inspection and Oversight Bodies for Heavy Equipment Safety")
    print("  - 12 records: 12 articles, no appendices; all 12 اصلية")
