#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام الشركات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21325),
published 25/6/1444H (2023-01-18). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "companies_law_implementing_regulation", "official_source",
                   "companies_law_implementing_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "companies_law_implementing_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "companies_law_implementing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "companies_law_implementing_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "companies_law_implementing_regulation_arabic_legal_llm",
                        "companies_law_implementing_regulation_legal_llm_001_095.json")

LAW_ID = "sa-companies-law-implementing-regulation"
LAW_AR = "اللائحة التنفيذية لنظام الشركات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"companies_law_implementing_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21325), published 25/6/1444H (2023-01-18). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "companies_law_implementing_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "COMPANIES_LAW_IMPLEMENTING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "companies-law-implementing-regulation-llm-%03d" % idx,
            ...
            "article_path": "companies_law_implementing_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Companies Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/6/1444H / 2023-01-18)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام الشركات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 25/6/1444هـ الموافق 2023-01-18م)"),
    ...
    json.dump({
        "law_key": "companies_law_implementing_regulation",
        "layer": "COMPANIES_LAW_IMPLEMENTING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "companies-law-implementing-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (95 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Companies Law — Arabic LLM-ready layer (95 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Companies Law records" % (len(ver), len(llm)))
