#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام الإحصاء track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27339),
...
SRC = os.path.join(ROOT, "sources", "statistics_law", "official_source",
                   "statistics_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "statistics_law", "verified")
RECORDS = os.path.join(OUT_VER, "statistics_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "statistics_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "statistics_law_arabic_legal_llm",
                        "statistics_law_legal_llm_001_022.json")

LAW_ID = "sa-statistics-law"
LAW_AR = "نظام الإحصاء"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"statistics_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27339), published 21/2/1447H (2025-08-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "statistics_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "STATISTICS_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "statistics-law-llm-%03d" % idx,
            ...
            "article_path": "statistics_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statistics Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/2/1447H / 2025-08-15)"),
                "source_authority_ar": ("نظام الإحصاء — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "statistics_law",
        "layer": "STATISTICS_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "statistics-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (22 مادة، جميعها أصلية)",
        "title_en": "Statistics Law — Arabic LLM-ready layer (22 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statistics Law records" % (len(ver), len(llm)))
