#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the السياسة الوطنية للغة العربية في السعودية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28846),
...
SRC = os.path.join(ROOT, "sources", "arabic_language_national_policy", "official_source",
                   "arabic_language_national_policy_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "arabic_language_national_policy", "verified")
RECORDS = os.path.join(OUT_VER, "arabic_language_national_policy_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "arabic_language_national_policy_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "arabic_language_national_policy_arabic_legal_llm",
                        "arabic_language_national_policy_legal_llm_001_005.json")

LAW_ID = "sa-arabic-language-national-policy"
LAW_AR = "السياسة الوطنية للغة العربية في السعودية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"arabic_language_national_policy_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28846), published 25/8/1447H (2026-02-13). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "arabic_language_national_policy",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "ARABIC_LANGUAGE_NATIONAL_POLICY_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "arabic-language-national-policy-llm-%03d" % idx,
            ...
            "article_path": "arabic_language_national_policy/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("National Policy for the Arabic Language in Saudi Arabia -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/8/1447H / 2026-02-13)"),
                "source_authority_ar": ("السياسة الوطنية للغة العربية في السعودية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "arabic_language_national_policy",
        "layer": "ARABIC_LANGUAGE_NATIONAL_POLICY_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "arabic-language-national-policy-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (5 مادة، جميعها أصلية)",
        "title_en": "National Policy for the Arabic Language in Saudi Arabia — Arabic LLM-ready layer (5 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready National Policy for the Arabic Language in Saudi Arabia records" % (len(ver), len(llm)))
