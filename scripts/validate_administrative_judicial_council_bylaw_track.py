#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الداخلية لمجلس القضاء الإداري track.
...
RECORDS = os.path.join(ROOT, "sources", "administrative_judicial_council_bylaw", "verified",
                       "administrative_judicial_council_bylaw_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "administrative_judicial_council_bylaw", "verified",
                       "administrative_judicial_council_bylaw_verified_summary.json")
LLM = os.path.join(ROOT, "data", "administrative_judicial_council_bylaw_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"administrative_judicial_council_bylaw_no_decision_number_in_source", "administrative_judicial_council_bylaw_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "22/11/1442":
        e.append("[2j] gazette_publication_date_hijri must be 22/11/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-07-02":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-07-02")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة الداخلية لمجلس القضاء الإداري":
    ...
    if e:
        print("FAIL: %d error(s) in Internal Bylaw of the Administrative Judicial Council track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Internal Bylaw of the Administrative Judicial Council")
