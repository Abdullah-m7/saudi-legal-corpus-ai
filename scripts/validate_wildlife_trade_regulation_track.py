#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية للإتجار بالكائنات الفطرية ومنتجاتها ومشتقاتها track.
...
RECORDS = os.path.join(ROOT, "sources", "wildlife_trade_regulation", "verified",
                       "wildlife_trade_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "wildlife_trade_regulation", "verified",
                       "wildlife_trade_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "wildlife_trade_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"wildlife_trade_regulation_no_decision_number_in_source", "wildlife_trade_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية للإتجار بالكائنات الفطرية ومنتجاتها ومشتقاتها":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation on Trade in Wild Organisms, their Products and Derivatives track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation on Trade in Wild Organisms, their Products and Derivatives")
