#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد أتعاب الأمناء والخبراء track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17438),
...
SRC = os.path.join(ROOT, "sources", "trustees_experts_fees_rules", "official_source",
                   "trustees_experts_fees_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "trustees_experts_fees_rules", "verified")
RECORDS = os.path.join(OUT_VER, "trustees_experts_fees_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "trustees_experts_fees_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "trustees_experts_fees_rules_arabic_legal_llm",
                        "trustees_experts_fees_rules_legal_llm_001_017.json")

LAW_ID = "sa-trustees-experts-fees-rules"
LAW_AR = "قواعد أتعاب الأمناء والخبراء"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"trustees_experts_fees_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17438), published 27/8/1442H (2021-04-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "trustees_experts_fees_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "TRUSTEES_EXPERTS_FEES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "trustees-experts-fees-rules-llm-%03d" % idx,
            ...
            "article_path": "trustees_experts_fees_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules on the Fees of Trustees and Experts -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/8/1442H / 2021-04-09)"),
                "source_authority_ar": ("قواعد أتعاب الأمناء والخبراء — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "trustees_experts_fees_rules",
        "layer": "TRUSTEES_EXPERTS_FEES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "trustees-experts-fees-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Rules on the Fees of Trustees and Experts — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules on the Fees of Trustees and Experts records" % (len(ver), len(llm)))
