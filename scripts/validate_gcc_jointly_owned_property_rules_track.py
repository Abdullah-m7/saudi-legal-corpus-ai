#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد الموحدة لملاك العقارات المشتركة بدول مجلس التعاون لدول الخليج العربية track.

11 records: 14 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "gcc_jointly_owned_property_rules", "verified",
                       "gcc_jointly_owned_property_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "gcc_jointly_owned_property_rules", "verified",
                       "gcc_jointly_owned_property_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "gcc_jointly_owned_property_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "gcc_jointly_owned_property_rules_is_a_gcc_instrument_not_domestic_legislation",
    "gcc_jointly_owned_property_rules_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد الموحدة لملاك العقارات المشتركة بدول مجلس التعاون لدول الخليج العربية":
    ...
    if e:
        print("FAIL: %d error(s) in GCC Unified Rules for Owners of Jointly Owned Property track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: GCC Unified Rules for Owners of Jointly Owned Property")
