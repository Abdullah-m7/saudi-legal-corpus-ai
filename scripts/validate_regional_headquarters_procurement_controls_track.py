#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط تعاقد الجهات الحكومية مع الشركات التي ليس لها مقر إقليمي في المملكة والأطراف ذوي العلاقة track.
...
RECORDS = os.path.join(ROOT, "sources", "regional_headquarters_procurement_controls", "verified",
                       "regional_headquarters_procurement_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "regional_headquarters_procurement_controls", "verified",
                       "regional_headquarters_procurement_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "regional_headquarters_procurement_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"regional_headquarters_procurement_controls_no_decision_number_in_source", "regional_headquarters_procurement_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "13/6/1444":
        e.append("[2j] gazette_publication_date_hijri must be 13/6/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-01-06":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-01-06")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط تعاقد الجهات الحكومية مع الشركات التي ليس لها مقر إقليمي في المملكة والأطراف ذوي العلاقة":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on Government Entities Contracting with Companies Lacking a Regional Headquarters in the Kingdom track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on Government Entities Contracting with Companies Lacking a Regional Headquarters in the Kingdom")
