#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام تصنيف المقاولين track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27425),
published 13/3/1447H (2025-09-05). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "contractors_classification_regulation", "official_source",
                   "contractors_classification_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "contractors_classification_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "contractors_classification_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "contractors_classification_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "contractors_classification_regulation_arabic_legal_llm",
                        "contractors_classification_regulation_legal_llm_001_020.json")

LAW_ID = "sa-contractors-classification-regulation"
LAW_AR = "اللائحة التنفيذية لنظام تصنيف المقاولين"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"contractors_classification_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27425), published 13/3/1447H (2025-09-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "contractors_classification_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CONTRACTORS_CLASSIFICATION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "contractors-classification-regulation-llm-%03d" % idx,
            ...
            "article_path": "contractors_classification_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Contractors Classification Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/3/1447H / 2025-09-05)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام تصنيف المقاولين — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 13/3/1447هـ الموافق 2025-09-05م)"),
    ...
    json.dump({
        "law_key": "contractors_classification_regulation",
        "layer": "CONTRACTORS_CLASSIFICATION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "contractors-classification-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Contractors Classification Law — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Contractors Classification Law records" % (len(ver), len(llm)))
