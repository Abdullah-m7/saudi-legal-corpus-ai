#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية للهيئة العامة للعناية بشؤون المسجد الحرام والمسجد النبوي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26834),
...
SRC = os.path.join(ROOT, "sources", "two_holy_mosques_authority_statute", "official_source",
                   "two_holy_mosques_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "two_holy_mosques_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "two_holy_mosques_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "two_holy_mosques_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "two_holy_mosques_authority_statute_arabic_legal_llm",
                        "two_holy_mosques_authority_statute_legal_llm_001_016.json")

LAW_ID = "sa-two-holy-mosques-authority-statute"
LAW_AR = "الترتيبات التنظيمية للهيئة العامة للعناية بشؤون المسجد الحرام والمسجد النبوي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"two_holy_mosques_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26834), published 5/6/1446H (2024-12-06). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "two_holy_mosques_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "TWO_HOLY_MOSQUES_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "two-holy-mosques-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "two_holy_mosques_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the General Authority for the Care of the Affairs of the Grand Mosque and the Prophet's Mosque -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/6/1446H / 2024-12-06)"),
                "source_authority_ar": ("الترتيبات التنظيمية للهيئة العامة للعناية بشؤون المسجد الحرام والمسجد النبوي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "two_holy_mosques_authority_statute",
        "layer": "TWO_HOLY_MOSQUES_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "two-holy-mosques-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the General Authority for the Care of the Affairs of the Grand Mosque and the Prophet's Mosque — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the General Authority for the Care of the Affairs of the Grand Mosque and the Prophet's Mosque records" % (len(ver), len(llm)))
