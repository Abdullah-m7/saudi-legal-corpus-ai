#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الضوابط والاجراءات لليخوت الزايرة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26662),
...
SRC = os.path.join(ROOT, "sources", "visiting_yachts_controls", "official_source",
                   "visiting_yachts_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "visiting_yachts_controls", "verified")
RECORDS = os.path.join(OUT_VER, "visiting_yachts_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "visiting_yachts_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "visiting_yachts_controls_arabic_legal_llm",
                        "visiting_yachts_controls_legal_llm_001_013.json")

LAW_ID = "sa-visiting-yachts-controls"
LAW_AR = "الضوابط والاجراءات لليخوت الزايرة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"visiting_yachts_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26662), published 20/4/1446H (2024-10-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "visiting_yachts_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "VISITING_YACHTS_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "visiting-yachts-controls-llm-%03d" % idx,
            ...
            "article_path": "visiting_yachts_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls and Procedures for Visiting Yachts -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/4/1446H / 2024-10-23)"),
                "source_authority_ar": ("الضوابط والاجراءات لليخوت الزايرة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "visiting_yachts_controls",
        "layer": "VISITING_YACHTS_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "visiting-yachts-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Controls and Procedures for Visiting Yachts — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls and Procedures for Visiting Yachts records" % (len(ver), len(llm)))
