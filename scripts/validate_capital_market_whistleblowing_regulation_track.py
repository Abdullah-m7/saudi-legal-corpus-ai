#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الإبلاغ عن مخالفات نظام السوق المالية track.
...
RECORDS = os.path.join(ROOT, "sources", "capital_market_whistleblowing_regulation", "verified",
                       "capital_market_whistleblowing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "capital_market_whistleblowing_regulation", "verified",
                       "capital_market_whistleblowing_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "capital_market_whistleblowing_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"capital_market_whistleblowing_regulation_no_decision_number_in_source", "capital_market_whistleblowing_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة الإبلاغ عن مخالفات نظام السوق المالية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on Reporting Violations of the Capital Market Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on Reporting Violations of the Capital Market Law")
