#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية لدور الأحداث track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17494),
...
SRC = os.path.join(ROOT, "sources", "juvenile_homes_regulation", "official_source",
                   "juvenile_homes_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "juvenile_homes_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "juvenile_homes_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "juvenile_homes_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "juvenile_homes_regulation_arabic_legal_llm",
                        "juvenile_homes_regulation_legal_llm_001_036.json")

LAW_ID = "sa-juvenile-homes-regulation"
LAW_AR = "اللائحة التنظيمية لدور الأحداث"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"juvenile_homes_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17494), published 11/9/1442H (2021-04-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "juvenile_homes_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "JUVENILE_HOMES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "juvenile-homes-regulation-llm-%03d" % idx,
            ...
            "article_path": "juvenile_homes_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for Juvenile Homes -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 11/9/1442H / 2021-04-23)"),
                "source_authority_ar": ("اللائحة التنظيمية لدور الأحداث — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "juvenile_homes_regulation",
        "layer": "JUVENILE_HOMES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "juvenile-homes-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (36 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for Juvenile Homes — Arabic LLM-ready layer (36 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for Juvenile Homes records" % (len(ver), len(llm)))
