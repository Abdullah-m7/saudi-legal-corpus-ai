#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية للمنشآت الاجتماعية غير الحكومية للأشخاص ذوي الإعاقة track.
...
RECORDS = os.path.join(ROOT, "sources", "disability_nongov_social_facilities_regulation", "verified",
                       "disability_nongov_social_facilities_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "disability_nongov_social_facilities_regulation", "verified",
                       "disability_nongov_social_facilities_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "disability_nongov_social_facilities_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"disability_nongov_social_facilities_regulation_no_decision_number_in_source", "disability_nongov_social_facilities_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية للمنشآت الاجتماعية غير الحكومية للأشخاص ذوي الإعاقة":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for Non-Governmental Social Facilities for Persons with Disabilities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for Non-Governmental Social Facilities for Persons with Disabilities")
