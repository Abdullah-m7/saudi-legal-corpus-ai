#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the السياسة الوطنية للسلامة والصحة المهنية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17210),
published 23/6/1442H (2021-02-05). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "occupational_safety_health_national_policy", "official_source",
                   "occupational_safety_health_national_policy_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "occupational_safety_health_national_policy", "verified")
RECORDS = os.path.join(OUT_VER, "occupational_safety_health_national_policy_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "occupational_safety_health_national_policy_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "occupational_safety_health_national_policy_arabic_legal_llm",
                        "occupational_safety_health_national_policy_legal_llm_001_005.json")

LAW_ID = "sa-occupational-safety-health-national-policy"
LAW_AR = "السياسة الوطنية للسلامة والصحة المهنية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"occupational_safety_health_national_policy_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17210), published 23/6/1442H (2021-02-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "occupational_safety_health_national_policy",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "OCCUPATIONAL_SAFETY_HEALTH_NATIONAL_POLICY_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "occupational-safety-health-national-policy-llm-%03d" % idx,
            ...
            "article_path": "occupational_safety_health_national_policy/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("National Policy for Occupational Safety and Health -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/6/1442H / 2021-02-05)"),
                "source_authority_ar": ("السياسة الوطنية للسلامة والصحة المهنية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 23/6/1442هـ الموافق 2021-02-05م)"),
    ...
    json.dump({
        "law_key": "occupational_safety_health_national_policy",
        "layer": "OCCUPATIONAL_SAFETY_HEALTH_NATIONAL_POLICY_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "occupational-safety-health-national-policy-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (5 مادة، جميعها أصلية)",
        "title_en": "National Policy for Occupational Safety and Health — Arabic LLM-ready layer (5 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready National Policy for Occupational Safety and Health records" % (len(ver), len(llm)))
