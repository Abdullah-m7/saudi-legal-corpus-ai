#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Notarization Law track (57 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) — all 57 matched outright, none required visual adjudication;
articles are numbered by ordinal position 1..57 (no مكرر); and every article
...
(no dual-status divergence). Consolidated amended law: 52 اصلية / 5 معدلة
(arts 11, 12, 15, 38, 40), each carrying its history; none repealed or added.
...
RECORDS = os.path.join(ROOT, "sources", "tawtheeq", "law", "verified",
                       "tawtheeq_law_verified_records.jsonl")
...
PDF = os.path.join(ROOT, "inputs", "tawtheeq_official_pdfs",
                   "tawtheeq_law_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 57
KEY_RE = r"tawtheeq_art_(\d{3})(_mukarrar)?$"
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 52, "معدلة": 5}
VISUALLY_ADJUDICATED = set()
EXPECTED_AMENDED = {11, 12, 15, 38, 40}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Notarization Law track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Notarization Law — 57 records (consolidated: 52 اصلية / 5 معدلة)")
    print("  - trust gate: 57/57 MATCHES_PDF outright (mean 0.964, min 0.902); no visual adjudication needed")
    print("  - numbered 1..57 by ordinal position (no مكرر); no dual-status divergence")
    print("  - 5 amended articles (11, 12, 15, 38, 40) carry history (M/21 1447H; M/191 1444H); committed MOJ PDF hash verified")
