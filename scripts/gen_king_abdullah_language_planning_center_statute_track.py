#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مركز الملك عبدالله بن عبدالعزيز للتخطيط والسياسات اللغوية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18507),
...
SRC = os.path.join(ROOT, "sources", "king_abdullah_language_planning_center_statute", "official_source",
                   "king_abdullah_language_planning_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "king_abdullah_language_planning_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "king_abdullah_language_planning_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "king_abdullah_language_planning_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "king_abdullah_language_planning_center_statute_arabic_legal_llm",
                        "king_abdullah_language_planning_center_statute_legal_llm_001_015.json")

LAW_ID = "sa-king-abdullah-language-planning-center-statute"
LAW_AR = "تنظيم مركز الملك عبدالله بن عبدالعزيز للتخطيط والسياسات اللغوية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"king_abdullah_language_planning_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18507), published 30/3/1443H (2021-11-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "king_abdullah_language_planning_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "KING_ABDULLAH_LANGUAGE_PLANNING_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "king-abdullah-language-planning-center-statute-llm-%03d" % idx,
            ...
            "article_path": "king_abdullah_language_planning_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the King Abdullah bin Abdulaziz Center for Language Planning and Policy -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 30/3/1443H / 2021-11-05)"),
                "source_authority_ar": ("تنظيم مركز الملك عبدالله بن عبدالعزيز للتخطيط والسياسات اللغوية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "king_abdullah_language_planning_center_statute",
        "layer": "KING_ABDULLAH_LANGUAGE_PLANNING_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "king-abdullah-language-planning-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Statute of the King Abdullah bin Abdulaziz Center for Language Planning and Policy — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the King Abdullah bin Abdulaziz Center for Language Planning and Policy records" % (len(ver), len(llm)))
