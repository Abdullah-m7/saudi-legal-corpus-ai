#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة تنظيم أعمال الإطفاء والإنقاذ بالمديرية العامة للدفاع المدني track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18510),
published 30/3/1443H (2021-11-05). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "civil_defense_firefighting_rescue_regulation", "official_source",
                   "civil_defense_firefighting_rescue_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "civil_defense_firefighting_rescue_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "civil_defense_firefighting_rescue_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "civil_defense_firefighting_rescue_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "civil_defense_firefighting_rescue_regulation_arabic_legal_llm",
                        "civil_defense_firefighting_rescue_regulation_legal_llm_001_009.json")

LAW_ID = "sa-civil-defense-firefighting-rescue-regulation"
LAW_AR = "لائحة تنظيم أعمال الإطفاء والإنقاذ بالمديرية العامة للدفاع المدني"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"civil_defense_firefighting_rescue_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18510), published 30/3/1443H (2021-11-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "civil_defense_firefighting_rescue_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CIVIL_DEFENSE_FIREFIGHTING_RESCUE_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "civil-defense-firefighting-rescue-regulation-llm-%03d" % idx,
            ...
            "article_path": "civil_defense_firefighting_rescue_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Governing Firefighting and Rescue Works at the General Directorate of Civil Defense -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 30/3/1443H / 2021-11-05)"),
                "source_authority_ar": ("لائحة تنظيم أعمال الإطفاء والإنقاذ بالمديرية العامة للدفاع المدني — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 30/3/1443هـ الموافق 2021-11-05م)"),
    ...
    json.dump({
        "law_key": "civil_defense_firefighting_rescue_regulation",
        "layer": "CIVIL_DEFENSE_FIREFIGHTING_RESCUE_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "civil-defense-firefighting-rescue-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (9 مادة، جميعها أصلية)",
        "title_en": "Regulation Governing Firefighting and Rescue Works at the General Directorate of Civil Defense — Arabic LLM-ready layer (9 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Governing Firefighting and Rescue Works at the General Directorate of Civil Defense records" % (len(ver), len(llm)))
