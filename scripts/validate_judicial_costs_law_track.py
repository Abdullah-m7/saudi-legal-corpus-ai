#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Judicial Costs Law track (23 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) EXCEPT the visually-adjudicated numbered-list article 12
...
issuance: all 23 اصلية (0 معدلة / 0 ملغاة / 0 مضافة). Tatweel is banned EXCEPT
...
RECORDS = os.path.join(ROOT, "sources", "judicial_costs", "law", "verified",
                       "judicial_costs_law_verified_records.jsonl")
...
PDF = os.path.join(ROOT, "inputs", "judicial_costs_official_pdfs",
                   "judicial_costs_law_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 23
KEY_RE = r"judicial_costs_art_(\d{3})"
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 23}
VISUALLY_ADJUDICATED = {"judicial_costs_art_012"}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Judicial Costs Law track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Judicial Costs Law — 23 records (fresh issuance: all 23 اصلية)")
    print("  - trust gate: 22/23 MATCHES_PDF; the numbered-list article 12 visually adjudicated verbatim")
    print("  - complete 1..23 (no مكرر); committed official MOJ PDF hash verified; no dual-status divergence")
    print("  - Royal Decree M/16 (10/2/1443H); the statute governing judicial (litigation) costs")
