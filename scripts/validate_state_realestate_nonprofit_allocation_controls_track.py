#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط تخصيص عقارات الدولة للقطاع غير الربحي track.
...
RECORDS = os.path.join(ROOT, "sources", "state_realestate_nonprofit_allocation_controls", "verified",
                       "state_realestate_nonprofit_allocation_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "state_realestate_nonprofit_allocation_controls", "verified",
                       "state_realestate_nonprofit_allocation_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "state_realestate_nonprofit_allocation_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"state_realestate_nonprofit_allocation_controls_no_decision_number_in_source", "state_realestate_nonprofit_allocation_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "26/2/1446":
        e.append("[2j] gazette_publication_date_hijri must be 26/2/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-08-30":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-08-30")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط تخصيص عقارات الدولة للقطاع غير الربحي":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on Allocating State Real Estate to the Non-Profit Sector track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on Allocating State Real Estate to the Non-Profit Sector")
