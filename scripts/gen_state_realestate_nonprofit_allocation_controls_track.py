#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط تخصيص عقارات الدولة للقطاع غير الربحي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25409),
published 26/2/1446H (2024-08-30). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "state_realestate_nonprofit_allocation_controls", "official_source",
                   "state_realestate_nonprofit_allocation_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "state_realestate_nonprofit_allocation_controls", "verified")
RECORDS = os.path.join(OUT_VER, "state_realestate_nonprofit_allocation_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "state_realestate_nonprofit_allocation_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "state_realestate_nonprofit_allocation_controls_arabic_legal_llm",
                        "state_realestate_nonprofit_allocation_controls_legal_llm_001_010.json")

LAW_ID = "sa-state-realestate-nonprofit-allocation-controls"
LAW_AR = "ضوابط تخصيص عقارات الدولة للقطاع غير الربحي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"state_realestate_nonprofit_allocation_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25409), published 26/2/1446H (2024-08-30). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "state_realestate_nonprofit_allocation_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "STATE_REALESTATE_NONPROFIT_ALLOCATION_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "state-realestate-nonprofit-allocation-controls-llm-%03d" % idx,
            ...
            "article_path": "state_realestate_nonprofit_allocation_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on Allocating State Real Estate to the Non-Profit Sector -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 26/2/1446H / 2024-08-30)"),
                "source_authority_ar": ("ضوابط تخصيص عقارات الدولة للقطاع غير الربحي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 26/2/1446هـ الموافق 2024-08-30م)"),
    ...
    json.dump({
        "law_key": "state_realestate_nonprofit_allocation_controls",
        "layer": "STATE_REALESTATE_NONPROFIT_ALLOCATION_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "state-realestate-nonprofit-allocation-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Controls on Allocating State Real Estate to the Non-Profit Sector — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on Allocating State Real Estate to the Non-Profit Sector records" % (len(ver), len(llm)))
