#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة الفنية لمعدات وتجهيزات المركبات ذات الاستخدام الخاص track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25393),
published 19/2/1446H (2024-08-23). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "special_use_vehicle_equipment_technical_regulation", "official_source",
                   "special_use_vehicle_equipment_technical_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "special_use_vehicle_equipment_technical_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "special_use_vehicle_equipment_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "special_use_vehicle_equipment_technical_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "special_use_vehicle_equipment_technical_regulation_arabic_legal_llm",
                        "special_use_vehicle_equipment_technical_regulation_legal_llm_001_011.json")

LAW_ID = "sa-special-use-vehicle-equipment-technical-regulation"
LAW_AR = "اللائحة الفنية لمعدات وتجهيزات المركبات ذات الاستخدام الخاص"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"special_use_vehicle_equipment_technical_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25393), published 19/2/1446H (2024-08-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "special_use_vehicle_equipment_technical_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SPECIAL_USE_VEHICLE_EQUIPMENT_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "special-use-vehicle-equipment-technical-regulation-llm-%03d" % idx,
            ...
            "article_path": "special_use_vehicle_equipment_technical_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Technical Regulation for Special-Use Vehicle Equipment and Fittings -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 19/2/1446H / 2024-08-23)"),
                "source_authority_ar": ("اللائحة الفنية لمعدات وتجهيزات المركبات ذات الاستخدام الخاص — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 19/2/1446هـ الموافق 2024-08-23م)"),
    ...
    json.dump({
        "law_key": "special_use_vehicle_equipment_technical_regulation",
        "layer": "SPECIAL_USE_VEHICLE_EQUIPMENT_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "special-use-vehicle-equipment-technical-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Technical Regulation for Special-Use Vehicle Equipment and Fittings — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Technical Regulation for Special-Use Vehicle Equipment and Fittings records" % (len(ver), len(llm)))
