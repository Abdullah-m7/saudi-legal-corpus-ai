#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the دليل التربية الريفية للثروة الحيوانية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24685),
...
STRUCTURE: 75 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "rural_livestock_husbandry_guide", "official_source",
                   "rural_livestock_husbandry_guide_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "rural_livestock_husbandry_guide", "verified")
RECORDS = os.path.join(OUT_VER, "rural_livestock_husbandry_guide_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "rural_livestock_husbandry_guide_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "rural_livestock_husbandry_guide_arabic_legal_llm",
                        "rural_livestock_husbandry_guide_legal_llm_001_075.json")

LAW_ID = "sa-rural-livestock-husbandry-guide"
LAW_AR = "دليل التربية الريفية للثروة الحيوانية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"rural_livestock_husbandry_guide_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24685), published 5/9/1445H (2024-03-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "rural_livestock_husbandry_guide",
            "law_component": "guide",
            "language": "ar",
            "record_layer": "RURAL_LIVESTOCK_HUSBANDRY_GUIDE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "rural-livestock-husbandry-guide-llm-%03d" % idx,
            ...
            "article_path": "rural_livestock_husbandry_guide/guide/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Guide to Rural Livestock Husbandry -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/9/1445H / 2024-03-15)"),
                "source_authority_ar": ("دليل التربية الريفية للثروة الحيوانية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "rural_livestock_husbandry_guide",
        "layer": "RURAL_LIVESTOCK_HUSBANDRY_GUIDE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "rural-livestock-husbandry-guide-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "guide",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (75 مادة، جميعها أصلية)",
        "title_en": "Guide to Rural Livestock Husbandry — Arabic LLM-ready layer (75 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Guide to Rural Livestock Husbandry records" % (len(ver), len(llm)))
