#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية للاتفاقية الدولية لضبط وإدارة مياه الصابورة (الاتزان) والرواسب في السفن لعام 2004م track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19276),
published 7/9/1443H (2022-04-08). The Gazette IS the official publication of record
...
STRUCTURE: 31 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "ballast_water_regulation", "official_source",
                   "ballast_water_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "ballast_water_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "ballast_water_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "ballast_water_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "ballast_water_regulation_arabic_legal_llm",
                        "ballast_water_regulation_legal_llm_001_031.json")

LAW_ID = "sa-ballast-water-regulation"
LAW_AR = "اللائحة التنفيذية للاتفاقية الدولية لضبط وإدارة مياه الصابورة (الاتزان) والرواسب في السفن لعام 2004م"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"ballast_water_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19276), published 7/9/1443H (2022-04-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "ballast_water_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "BALLAST_WATER_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "ballast-water-regulation-llm-%03d" % idx,
            ...
            "article_path": "ballast_water_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the International Convention for the Control and Management of Ships Ballast Water and Sediments (2004) -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/9/1443H / 2022-04-08)"),
                "source_authority_ar": ("اللائحة التنفيذية للاتفاقية الدولية لضبط وإدارة مياه الصابورة (الاتزان) والرواسب في السفن لعام 2004م — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 7/9/1443هـ الموافق 2022-04-08م)"),
    ...
    json.dump({
        "law_key": "ballast_water_regulation",
        "layer": "BALLAST_WATER_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "ballast-water-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (31 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the International Convention for the Control and Management of Ships Ballast Water and Sediments (2004) — Arabic LLM-ready layer (31 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the International Convention for the Control and Management of Ships Ballast Water and Sediments (2004) records" % (len(ver), len(llm)))
