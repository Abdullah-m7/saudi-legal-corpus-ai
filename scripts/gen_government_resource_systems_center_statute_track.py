#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المركز الوطني لنظم الموارد الحكومية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24200),
...
SRC = os.path.join(ROOT, "sources", "government_resource_systems_center_statute", "official_source",
                   "government_resource_systems_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "government_resource_systems_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "government_resource_systems_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "government_resource_systems_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "government_resource_systems_center_statute_arabic_legal_llm",
                        "government_resource_systems_center_statute_legal_llm_001_015.json")

LAW_ID = "sa-government-resource-systems-center-statute"
LAW_AR = "تنظيم المركز الوطني لنظم الموارد الحكومية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"government_resource_systems_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24200), published 24/5/1445H (2023-12-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "government_resource_systems_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "GOVERNMENT_RESOURCE_SYSTEMS_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "government-resource-systems-center-statute-llm-%03d" % idx,
            ...
            "article_path": "government_resource_systems_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Center for Government Resource Systems -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/5/1445H / 2023-12-08)"),
                "source_authority_ar": ("تنظيم المركز الوطني لنظم الموارد الحكومية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "government_resource_systems_center_statute",
        "layer": "GOVERNMENT_RESOURCE_SYSTEMS_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "government-resource-systems-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Center for Government Resource Systems — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Center for Government Resource Systems records" % (len(ver), len(llm)))
