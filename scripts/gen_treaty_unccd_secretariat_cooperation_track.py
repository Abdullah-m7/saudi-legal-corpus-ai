#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية للتعاون بين وزارة البيئة والمياه والزراعة في المملكة وأمانة اتفاقية الأمم المتحدة لمكافحة التصحر track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19132),
published 15/8/1443H (2022-03-18). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "treaty_unccd_secretariat_cooperation", "official_source",
                   "treaty_unccd_secretariat_cooperation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_unccd_secretariat_cooperation", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_unccd_secretariat_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_unccd_secretariat_cooperation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_unccd_secretariat_cooperation_arabic_legal_llm",
                        "treaty_unccd_secretariat_cooperation_legal_llm_001_014.json")

LAW_ID = "sa-treaty-unccd-secretariat-cooperation"
LAW_AR = "اتفاقية للتعاون بين وزارة البيئة والمياه والزراعة في المملكة وأمانة اتفاقية الأمم المتحدة لمكافحة التصحر"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_unccd_secretariat_cooperation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19132), published 15/8/1443H (2022-03-18). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_unccd_secretariat_cooperation",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_UNCCD_SECRETARIAT_COOPERATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-unccd-secretariat-cooperation-llm-%03d" % idx,
            ...
            "article_path": "treaty_unccd_secretariat_cooperation/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Cooperation Agreement between MEWA and the UNCCD Secretariat -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/8/1443H / 2022-03-18)"),
                "source_authority_ar": ("اتفاقية للتعاون بين وزارة البيئة والمياه والزراعة في المملكة وأمانة اتفاقية الأمم المتحدة لمكافحة التصحر — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 15/8/1443هـ الموافق 2022-03-18م)"),
    ...
    json.dump({
        "law_key": "treaty_unccd_secretariat_cooperation",
        "layer": "TREATY_UNCCD_SECRETARIAT_COOPERATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-unccd-secretariat-cooperation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Cooperation Agreement between MEWA and the UNCCD Secretariat — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Cooperation Agreement between MEWA and the UNCCD Secretariat records" % (len(ver), len(llm)))
