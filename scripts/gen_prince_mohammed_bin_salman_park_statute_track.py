#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مؤسسة حديقة الأمير محمد بن سلمان track.
...
server-rendered HTML page (uqn.gov.sa/details?p=22832),
published 29/10/1444H (2023-05-19). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "prince_mohammed_bin_salman_park_statute", "official_source",
                   "prince_mohammed_bin_salman_park_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "prince_mohammed_bin_salman_park_statute", "verified")
RECORDS = os.path.join(OUT_VER, "prince_mohammed_bin_salman_park_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "prince_mohammed_bin_salman_park_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "prince_mohammed_bin_salman_park_statute_arabic_legal_llm",
                        "prince_mohammed_bin_salman_park_statute_legal_llm_001_015.json")

LAW_ID = "sa-prince-mohammed-bin-salman-park-statute"
LAW_AR = "تنظيم مؤسسة حديقة الأمير محمد بن سلمان"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"prince_mohammed_bin_salman_park_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=22832), published 29/10/1444H (2023-05-19). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "prince_mohammed_bin_salman_park_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "PRINCE_MOHAMMED_BIN_SALMAN_PARK_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "prince-mohammed-bin-salman-park-statute-llm-%03d" % idx,
            ...
            "article_path": "prince_mohammed_bin_salman_park_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Prince Mohammed bin Salman Park Foundation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 29/10/1444H / 2023-05-19)"),
                "source_authority_ar": ("تنظيم مؤسسة حديقة الأمير محمد بن سلمان — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 29/10/1444هـ الموافق 2023-05-19م)"),
    ...
    json.dump({
        "law_key": "prince_mohammed_bin_salman_park_statute",
        "layer": "PRINCE_MOHAMMED_BIN_SALMAN_PARK_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "prince-mohammed-bin-salman-park-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Statute of the Prince Mohammed bin Salman Park Foundation — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Prince Mohammed bin Salman Park Foundation records" % (len(ver), len(llm)))
