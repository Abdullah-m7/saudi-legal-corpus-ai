#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط زراعة القمح والأعلاف الموسمية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26672),
published 22/4/1446H (2024-10-25). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "wheat_seasonal_fodder_cultivation_controls", "official_source",
                   "wheat_seasonal_fodder_cultivation_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "wheat_seasonal_fodder_cultivation_controls", "verified")
RECORDS = os.path.join(OUT_VER, "wheat_seasonal_fodder_cultivation_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "wheat_seasonal_fodder_cultivation_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "wheat_seasonal_fodder_cultivation_controls_arabic_legal_llm",
                        "wheat_seasonal_fodder_cultivation_controls_legal_llm_001_006.json")

LAW_ID = "sa-wheat-seasonal-fodder-cultivation-controls"
LAW_AR = "ضوابط زراعة القمح والأعلاف الموسمية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"wheat_seasonal_fodder_cultivation_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26672), published 22/4/1446H (2024-10-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "wheat_seasonal_fodder_cultivation_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "WHEAT_SEASONAL_FODDER_CULTIVATION_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "wheat-seasonal-fodder-cultivation-controls-llm-%03d" % idx,
            ...
            "article_path": "wheat_seasonal_fodder_cultivation_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls for the Cultivation of Wheat and Seasonal Fodder -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/4/1446H / 2024-10-25)"),
                "source_authority_ar": ("ضوابط زراعة القمح والأعلاف الموسمية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 22/4/1446هـ الموافق 2024-10-25م)"),
    ...
    json.dump({
        "law_key": "wheat_seasonal_fodder_cultivation_controls",
        "layer": "WHEAT_SEASONAL_FODDER_CULTIVATION_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "wheat-seasonal-fodder-cultivation-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (6 مادة، جميعها أصلية)",
        "title_en": "Controls for the Cultivation of Wheat and Seasonal Fodder — Arabic LLM-ready layer (6 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls for the Cultivation of Wheat and Seasonal Fodder records" % (len(ver), len(llm)))
