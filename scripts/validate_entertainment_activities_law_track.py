#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام الأنشطة الترفيهية والأنشطة المساندة لها track.
...
RECORDS = os.path.join(ROOT, "sources", "entertainment_activities_law", "verified",
                       "entertainment_activities_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "entertainment_activities_law", "verified",
                       "entertainment_activities_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "entertainment_activities_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"entertainment_activities_law_no_decision_number_in_source", "entertainment_activities_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام الأنشطة الترفيهية والأنشطة المساندة لها":
    ...
    if e:
        print("FAIL: %d error(s) in Law of Entertainment Activities and Supporting Activities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law of Entertainment Activities and Supporting Activities")
