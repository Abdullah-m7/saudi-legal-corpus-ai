#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام صندوق التنمية العقارية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17349),
published 2/6/1442H (2021-01-15). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "real_estate_development_fund_law", "official_source",
                   "real_estate_development_fund_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "real_estate_development_fund_law", "verified")
RECORDS = os.path.join(OUT_VER, "real_estate_development_fund_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "real_estate_development_fund_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "real_estate_development_fund_law_arabic_legal_llm",
                        "real_estate_development_fund_law_legal_llm_001_018.json")

LAW_ID = "sa-real-estate-development-fund-law"
LAW_AR = "نظام صندوق التنمية العقارية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"real_estate_development_fund_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17349), published 2/6/1442H (2021-01-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "real_estate_development_fund_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "REAL_ESTATE_DEVELOPMENT_FUND_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "real-estate-development-fund-law-llm-%03d" % idx,
            ...
            "article_path": "real_estate_development_fund_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law of the Real Estate Development Fund -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/6/1442H / 2021-01-15)"),
                "source_authority_ar": ("نظام صندوق التنمية العقارية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 2/6/1442هـ الموافق 2021-01-15م)"),
    ...
    json.dump({
        "law_key": "real_estate_development_fund_law",
        "layer": "REAL_ESTATE_DEVELOPMENT_FUND_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "real-estate-development-fund-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Law of the Real Estate Development Fund — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law of the Real Estate Development Fund records" % (len(ver), len(llm)))
