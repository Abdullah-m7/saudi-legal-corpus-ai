#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط حساب الضمان للمساهمات العقارية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24520),
...
SRC = os.path.join(ROOT, "sources", "real_estate_contributions_escrow_controls", "official_source",
                   "real_estate_contributions_escrow_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "real_estate_contributions_escrow_controls", "verified")
RECORDS = os.path.join(OUT_VER, "real_estate_contributions_escrow_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "real_estate_contributions_escrow_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "real_estate_contributions_escrow_controls_arabic_legal_llm",
                        "real_estate_contributions_escrow_controls_legal_llm_001_007.json")

LAW_ID = "sa-real-estate-contributions-escrow-controls"
LAW_AR = "ضوابط حساب الضمان للمساهمات العقارية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"real_estate_contributions_escrow_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24520), published 6/8/1445H (2024-02-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "real_estate_contributions_escrow_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "REAL_ESTATE_CONTRIBUTIONS_ESCROW_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "real-estate-contributions-escrow-controls-llm-%03d" % idx,
            ...
            "article_path": "real_estate_contributions_escrow_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls for the Escrow Account of Real Estate Contributions -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/8/1445H / 2024-02-16)"),
                "source_authority_ar": ("ضوابط حساب الضمان للمساهمات العقارية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "real_estate_contributions_escrow_controls",
        "layer": "REAL_ESTATE_CONTRIBUTIONS_ESCROW_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "real-estate-contributions-escrow-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (7 مادة، جميعها أصلية)",
        "title_en": "Controls for the Escrow Account of Real Estate Contributions — Arabic LLM-ready layer (7 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls for the Escrow Account of Real Estate Contributions records" % (len(ver), len(llm)))
