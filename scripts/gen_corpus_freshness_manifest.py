...
and each of the 130 tracks' own `official_source.json`-equivalent file, and produces a single
...
scripts/gen_corpus_verification_tiers.py:
  - It does NOT modify the registry, the verification-tiers file, or any of the 130 tracks'
...
def resolve_official_source_path(track_id: str, data_paths: list[str]) -> str | None:
    ...
    special = {
        "companies_law": "data/legal_corpus_factory/law_profiles/sa_companies_law_m132_1443.profile.json",
...
def main() -> int:
    ...
    out = {
        ...
        "notes": (
            "Purely additive, read-only survey layer over this corpus's 130 tracks, built "
            ...
            "tiers file, or any of the 130 tracks' own files. For a LIVE (network-dependent, "
