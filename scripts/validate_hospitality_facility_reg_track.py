#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Regulation of the Tourism Hospitality
Facility track (لائحة مرفق الضيافة السياحي).

36 records: 35 numbered articles (المادة الأولى .. الخامسة والثلاثون) across
6 chapters + 1 appendix-type record (جدول المقابل المالي, a large
multi-part fee table). All 36 اصلية.

VERIFICATION TIER (effectively TIER_1) -- see the generator docstring and
the official_source JSON's verification_methodology_note: the decision
number/date is printed on the official PDF's own cover page, and confirmed
by a scanned image of the signed Ministerial Decision letter on its own
page 2. This validator only checks internal self-consistency of the
ingested text and that every discrepancy is disclosed.
"""
...
RECORDS = os.path.join(ROOT, "sources", "hospitality_facility",
                       "verified",
                       "hospitality_facility_reg_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "hospitality_facility",
                       "verified",
                       "hospitality_facility_reg_verified_summary.json")
LLM = os.path.join(ROOT, "data",
                   "hospitality_facility_reg_arabic_legal_llm",
...
EXPECTED_COUNTS = {"اصلية": 36, "معدلة": 0, "ملغاة": 0, "مضافة": 0}

FLAGGED_DISCREPANCY_KEYS = {
    "hospitality_facility_reg_argaam_batch_confirmed",
    "hospitality_facility_reg_gazette_date_unconfirmed",
    "hospitality_facility_reg_fee_table_consolidated_single_record",
}
AR = "ء-ي"
...
def main():
    ...
    # [2c] chapter_structure coverage (articles 1-35, appendices 1-1)
    ...
    # [2j] anchor facts
    if src.get("decree") != "القرار الوزاري رقم (2289)" \
            or src.get("decree_date_hijri") != "19/5/1444":
        e.append("[2j] decree/decree_date_hijri mismatch with verified 2289, 19/5/1444H")
    ...
    art12 = arts.get("hospitality_facility_reg_art_012", {})
    if "التصنيف" not in art12.get("text", ""):
        e.append("[2j] Article 12 must address classification (التصنيف)")
    fee = arts.get("hospitality_facility_reg_appendix_001", {})
    if "22,500" not in fee.get("text", ""):
        e.append("[2j] Fee table must contain the 22,500-riyal top-tier license fee")
    ...
    if e:
        print("FAIL: %d error(s) in Regulation of the Tourism Hospitality Facility track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation of the Tourism Hospitality Facility")
    print("  - 36 records: 35 numbered articles across 6 chapters + 1 fee-table appendix;")
    print("    all 36 اصلية")
    print("  - Founding: Ministerial Decision (2289), 19/5/1444H, printed on cover + signed")
    print("    decision letter embedded in the official PDF's own page 2")
    print("  - VERIFICATION TIER: effectively TIER_1 — primary signed-document capture,")
    print("    founding Royal Decree citation (M/18, 26/1/1444H) matches this corpus's own")
    print("    tourism_law track")
    return 0
