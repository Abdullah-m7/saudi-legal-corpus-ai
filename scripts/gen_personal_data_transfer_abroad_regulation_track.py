#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة نقل البيانات الشخصية خارج المملكة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23597),
published 22/2/1445H (2023-09-07). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "personal_data_transfer_abroad_regulation", "official_source",
                   "personal_data_transfer_abroad_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "personal_data_transfer_abroad_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "personal_data_transfer_abroad_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "personal_data_transfer_abroad_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "personal_data_transfer_abroad_regulation_arabic_legal_llm",
                        "personal_data_transfer_abroad_regulation_legal_llm_001_010.json")

LAW_ID = "sa-personal-data-transfer-abroad-regulation"
LAW_AR = "لائحة نقل البيانات الشخصية خارج المملكة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"personal_data_transfer_abroad_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23597), published 22/2/1445H (2023-09-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "personal_data_transfer_abroad_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "PERSONAL_DATA_TRANSFER_ABROAD_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "personal-data-transfer-abroad-regulation-llm-%03d" % idx,
            ...
            "article_path": "personal_data_transfer_abroad_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on the Transfer of Personal Data outside the Kingdom -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/2/1445H / 2023-09-07)"),
                "source_authority_ar": ("لائحة نقل البيانات الشخصية خارج المملكة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 22/2/1445هـ الموافق 2023-09-07م)"),
    ...
    json.dump({
        "law_key": "personal_data_transfer_abroad_regulation",
        "layer": "PERSONAL_DATA_TRANSFER_ABROAD_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "personal-data-transfer-abroad-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Regulation on the Transfer of Personal Data outside the Kingdom — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on the Transfer of Personal Data outside the Kingdom records" % (len(ver), len(llm)))
