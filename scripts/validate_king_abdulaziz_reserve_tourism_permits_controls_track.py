#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط التصريح للأنشطة السياحية في محمية الملك عبدالعزيز الملكية track.
...
RECORDS = os.path.join(ROOT, "sources", "king_abdulaziz_reserve_tourism_permits_controls", "verified",
                       "king_abdulaziz_reserve_tourism_permits_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "king_abdulaziz_reserve_tourism_permits_controls", "verified",
                       "king_abdulaziz_reserve_tourism_permits_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "king_abdulaziz_reserve_tourism_permits_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"king_abdulaziz_reserve_tourism_permits_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "10/7/1446":
        e.append("[2j] gazette_publication_date_hijri must be 10/7/1446")
    if src.get("gazette_publication_date_gregorian") != "2025-01-10":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-01-10")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط التصريح للأنشطة السياحية في محمية الملك عبدالعزيز الملكية":
    ...
    if e:
        print("FAIL: %d error(s) in Controls for Permitting Tourism Activities in the King Abdulaziz Royal Reserve track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls for Permitting Tourism Activities in the King Abdulaziz Royal Reserve")
