#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنظام صندوق التنمية العقارية الصادر بالمرسوم الملكي رقم (م/47) وتاريخ 23 /05/ 1442ه track.
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_development_fund_implementing_regulation", "verified",
                       "real_estate_development_fund_implementing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "real_estate_development_fund_implementing_regulation", "verified",
                       "real_estate_development_fund_implementing_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "real_estate_development_fund_implementing_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"real_estate_development_fund_implementing_regulation_no_decision_number_in_source", "real_estate_development_fund_implementing_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "25/9/1442":
        e.append("[2j] gazette_publication_date_hijri must be 25/9/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-05-07":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-05-07")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنفيذية لنظام صندوق التنمية العقارية الصادر بالمرسوم الملكي رقم (م/47) وتاريخ 23 /05/ 1442ه":
    ...
    if e:
        print("FAIL: %d error(s) in Implementing Regulation of the Real Estate Development Fund Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Implementing Regulation of the Real Estate Development Fund Law")
