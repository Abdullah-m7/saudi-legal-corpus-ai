#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية لليخوت الخاصة الزائرة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23416),
...
SRC = os.path.join(ROOT, "sources", "visiting_private_yachts_regulation", "official_source",
                   "visiting_private_yachts_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "visiting_private_yachts_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "visiting_private_yachts_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "visiting_private_yachts_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "visiting_private_yachts_regulation_arabic_legal_llm",
                        "visiting_private_yachts_regulation_legal_llm_001_011.json")

LAW_ID = "sa-visiting-private-yachts-regulation"
LAW_AR = "اللائحة التنظيمية لليخوت الخاصة الزائرة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"visiting_private_yachts_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23416), published 17/1/1445H (2023-08-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "visiting_private_yachts_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "VISITING_PRIVATE_YACHTS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "visiting-private-yachts-regulation-llm-%03d" % idx,
            ...
            "article_path": "visiting_private_yachts_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Regulation for Visiting Private Yachts -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/1/1445H / 2023-08-04)"),
                "source_authority_ar": ("اللائحة التنظيمية لليخوت الخاصة الزائرة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "visiting_private_yachts_regulation",
        "layer": "VISITING_PRIVATE_YACHTS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "visiting-private-yachts-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Organisational Regulation for Visiting Private Yachts — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Regulation for Visiting Private Yachts records" % (len(ver), len(llm)))
