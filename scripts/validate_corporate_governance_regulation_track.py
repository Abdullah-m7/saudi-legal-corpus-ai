#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة حوكمة الشركات track.
...
RECORDS = os.path.join(ROOT, "sources", "corporate_governance_regulation", "verified",
                       "corporate_governance_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "corporate_governance_regulation", "verified",
                       "corporate_governance_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "corporate_governance_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"corporate_governance_regulation_no_decision_number_in_source", "corporate_governance_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة حوكمة الشركات":
    ...
    if e:
        print("FAIL: %d error(s) in Corporate Governance Regulations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Corporate Governance Regulations")
