#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة الضوابط الخاصة بتحديد تكاليف الإصلاح وتقدير التعويضات عن المنفعة التي فقدها المرفق track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19187),
published 22/8/1443H (2022-03-25). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "utility_benefit_loss_compensation_regulation", "official_source",
                   "utility_benefit_loss_compensation_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "utility_benefit_loss_compensation_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "utility_benefit_loss_compensation_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "utility_benefit_loss_compensation_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "utility_benefit_loss_compensation_regulation_arabic_legal_llm",
                        "utility_benefit_loss_compensation_regulation_legal_llm_001_010.json")

LAW_ID = "sa-utility-benefit-loss-compensation-regulation"
LAW_AR = "لائحة الضوابط الخاصة بتحديد تكاليف الإصلاح وتقدير التعويضات عن المنفعة التي فقدها المرفق"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"utility_benefit_loss_compensation_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19187), published 22/8/1443H (2022-03-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "utility_benefit_loss_compensation_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "UTILITY_BENEFIT_LOSS_COMPENSATION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "utility-benefit-loss-compensation-regulation-llm-%03d" % idx,
            ...
            "article_path": "utility_benefit_loss_compensation_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation of Controls for Determining Repair Costs and Estimating Compensation for Benefit Lost by the Utility -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/8/1443H / 2022-03-25)"),
                "source_authority_ar": ("لائحة الضوابط الخاصة بتحديد تكاليف الإصلاح وتقدير التعويضات عن المنفعة التي فقدها المرفق — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 22/8/1443هـ الموافق 2022-03-25م)"),
    ...
    json.dump({
        "law_key": "utility_benefit_loss_compensation_regulation",
        "layer": "UTILITY_BENEFIT_LOSS_COMPENSATION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "utility-benefit-loss-compensation-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Regulation of Controls for Determining Repair Costs and Estimating Compensation for Benefit Lost by the Utility — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation of Controls for Determining Repair Costs and Estimating Compensation for Benefit Lost by the Utility records" % (len(ver), len(llm)))
