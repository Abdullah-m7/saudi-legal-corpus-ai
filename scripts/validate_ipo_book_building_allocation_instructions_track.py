#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تعليمات بناء سجل الأوامر وتخصيص الأسهم في الاكتتابات الأولية track.
...
RECORDS = os.path.join(ROOT, "sources", "ipo_book_building_allocation_instructions", "verified",
                       "ipo_book_building_allocation_instructions_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "ipo_book_building_allocation_instructions", "verified",
                       "ipo_book_building_allocation_instructions_verified_summary.json")
LLM = os.path.join(ROOT, "data", "ipo_book_building_allocation_instructions_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"ipo_book_building_allocation_instructions_no_decision_number_in_source", "ipo_book_building_allocation_instructions_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تعليمات بناء سجل الأوامر وتخصيص الأسهم في الاكتتابات الأولية":
    ...
    if e:
        print("FAIL: %d error(s) in Instructions on Book Building and Share Allocation in Initial Public Offerings track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Instructions on Book Building and Share Allocation in Initial Public Offerings")
