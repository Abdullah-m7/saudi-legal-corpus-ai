#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام الأساس لمؤسسة الرياض غير الربحية (مؤسسة مستقلة ذات طبيعة خاصة وغير هادفة للربح) track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25193),
published 22/12/1445H (2024-06-28). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "riyadh_nonprofit_foundation_statute", "official_source",
                   "riyadh_nonprofit_foundation_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "riyadh_nonprofit_foundation_statute", "verified")
RECORDS = os.path.join(OUT_VER, "riyadh_nonprofit_foundation_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "riyadh_nonprofit_foundation_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "riyadh_nonprofit_foundation_statute_arabic_legal_llm",
                        "riyadh_nonprofit_foundation_statute_legal_llm_001_018.json")

LAW_ID = "sa-riyadh-nonprofit-foundation-statute"
LAW_AR = "النظام الأساس لمؤسسة الرياض غير الربحية (مؤسسة مستقلة ذات طبيعة خاصة وغير هادفة للربح)"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"riyadh_nonprofit_foundation_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25193), published 22/12/1445H (2024-06-28). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "riyadh_nonprofit_foundation_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "RIYADH_NONPROFIT_FOUNDATION_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "riyadh-nonprofit-foundation-statute-llm-%03d" % idx,
            ...
            "article_path": "riyadh_nonprofit_foundation_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Riyadh Non-Profit Foundation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/12/1445H / 2024-06-28)"),
                "source_authority_ar": ("النظام الأساس لمؤسسة الرياض غير الربحية (مؤسسة مستقلة ذات طبيعة خاصة وغير هادفة للربح) — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 22/12/1445هـ الموافق 2024-06-28م)"),
    ...
    json.dump({
        "law_key": "riyadh_nonprofit_foundation_statute",
        "layer": "RIYADH_NONPROFIT_FOUNDATION_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "riyadh-nonprofit-foundation-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Statute of the Riyadh Non-Profit Foundation — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Riyadh Non-Profit Foundation records" % (len(ver), len(llm)))
