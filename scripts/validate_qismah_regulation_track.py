#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Regulation on the Division of Jointly-Owned Property track (48 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified); no article required visual adjudication (mean 0.9661, min
0.913). Articles are numbered by ordinal position 1..48 (no مكرر, no chapter/
section wrapper — this regulation's statute structure is a flat article list);
and every article carries an explained legal_status
...
ISSUANCE: all 48 اصلية; none amended, repealed or added. Tatweel is banned EXCEPT
...
RECORDS = os.path.join(ROOT, "sources", "qismah", "regulation", "verified",
                       "qismah_regulation_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "qismah_arabic_legal_llm",
                   "qismah_regulation_legal_llm_001_048.json")
PDF = os.path.join(ROOT, "inputs", "qismah_official_pdfs",
                   "qismah_regulation_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 48
KEY_RE = r"qismah_art_(\d{3})$"
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 48}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Qismah Regulation track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on the Division of Jointly-Owned Property — 48 records (fresh issuance: all 48 اصلية)")
    print("  - trust gate: all 48/48 MATCHES_PDF outright (mean 0.9661, min 0.913); no visual adjudication needed")
    print("  - numbered 1..48 by ordinal position (no مكرر); no dual-status divergence")
    print("  - IN-FORCE Minister of Justice Decision 1610 (19/5/1439H); committed MOJ PDF hash verified; Arabic governs")
    return 0
