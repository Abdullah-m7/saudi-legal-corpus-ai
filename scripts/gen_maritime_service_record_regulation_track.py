#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة المنظمة لسجل الخدمة البحرية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20006),
...
SRC = os.path.join(ROOT, "sources", "maritime_service_record_regulation", "official_source",
                   "maritime_service_record_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "maritime_service_record_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "maritime_service_record_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "maritime_service_record_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "maritime_service_record_regulation_arabic_legal_llm",
                        "maritime_service_record_regulation_legal_llm_001_012.json")

LAW_ID = "sa-maritime-service-record-regulation"
LAW_AR = "اللائحة المنظمة لسجل الخدمة البحرية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"maritime_service_record_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20006), published 6/2/1444H (2022-09-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "maritime_service_record_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MARITIME_SERVICE_RECORD_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "maritime-service-record-regulation-llm-%03d" % idx,
            ...
            "article_path": "maritime_service_record_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Governing the Maritime Service Record -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/2/1444H / 2022-09-02)"),
                "source_authority_ar": ("اللائحة المنظمة لسجل الخدمة البحرية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "maritime_service_record_regulation",
        "layer": "MARITIME_SERVICE_RECORD_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "maritime-service-record-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Regulation Governing the Maritime Service Record — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Governing the Maritime Service Record records" % (len(ver), len(llm)))
