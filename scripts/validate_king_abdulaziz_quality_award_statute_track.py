#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لجائزة الملك عبدالعزيز للجودة track.
...
RECORDS = os.path.join(ROOT, "sources", "king_abdulaziz_quality_award_statute", "verified",
                       "king_abdulaziz_quality_award_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "king_abdulaziz_quality_award_statute", "verified",
                       "king_abdulaziz_quality_award_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "king_abdulaziz_quality_award_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"king_abdulaziz_quality_award_statute_no_decision_number_in_source", "king_abdulaziz_quality_award_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "7/7/1442":
        e.append("[2j] gazette_publication_date_hijri must be 7/7/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-02-19":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-02-19")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية لجائزة الملك عبدالعزيز للجودة":
    ...
    if e:
        print("FAIL: %d error(s) in King Abdulaziz Quality Award — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: King Abdulaziz Quality Award — Organisational Arrangements")
