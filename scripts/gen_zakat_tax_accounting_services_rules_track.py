#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة لخدمات محاسبة الزكاة والضريبة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26573),
...
SRC = os.path.join(ROOT, "sources", "zakat_tax_accounting_services_rules", "official_source",
                   "zakat_tax_accounting_services_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "zakat_tax_accounting_services_rules", "verified")
RECORDS = os.path.join(OUT_VER, "zakat_tax_accounting_services_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "zakat_tax_accounting_services_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "zakat_tax_accounting_services_rules_arabic_legal_llm",
                        "zakat_tax_accounting_services_rules_legal_llm_001_019.json")

LAW_ID = "sa-zakat-tax-accounting-services-rules"
LAW_AR = "القواعد المنظمة لخدمات محاسبة الزكاة والضريبة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"zakat_tax_accounting_services_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26573), published 1/4/1446H (2024-10-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "zakat_tax_accounting_services_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "ZAKAT_TAX_ACCOUNTING_SERVICES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "zakat-tax-accounting-services-rules-llm-%03d" % idx,
            ...
            "article_path": "zakat_tax_accounting_services_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing Zakat and Tax Accounting Services -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/4/1446H / 2024-10-04)"),
                "source_authority_ar": ("القواعد المنظمة لخدمات محاسبة الزكاة والضريبة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "zakat_tax_accounting_services_rules",
        "layer": "ZAKAT_TAX_ACCOUNTING_SERVICES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "zakat-tax-accounting-services-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (19 مادة، جميعها أصلية)",
        "title_en": "Rules Governing Zakat and Tax Accounting Services — Arabic LLM-ready layer (19 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing Zakat and Tax Accounting Services records" % (len(ver), len(llm)))
