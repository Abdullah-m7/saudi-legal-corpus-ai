#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام استئجار الدولة للعقار track.
...
RECORDS = os.path.join(ROOT, "sources", "state_property_lease_law", "verified",
                       "state_property_lease_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "state_property_lease_law", "verified",
                       "state_property_lease_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "state_property_lease_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"state_property_lease_law_no_decision_number_in_source", "state_property_lease_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام استئجار الدولة للعقار":
    ...
    if e:
        print("FAIL: %d error(s) in Law on State Leasing of Real Estate track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law on State Leasing of Real Estate")
