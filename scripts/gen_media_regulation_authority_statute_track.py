#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم الهيئة العامة لتنظيم الإعلام track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23698),
...
SRC = os.path.join(ROOT, "sources", "media_regulation_authority_statute", "official_source",
                   "media_regulation_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "media_regulation_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "media_regulation_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "media_regulation_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "media_regulation_authority_statute_arabic_legal_llm",
                        "media_regulation_authority_statute_legal_llm_001_016.json")

LAW_ID = "sa-media-regulation-authority-statute"
LAW_AR = "تنظيم الهيئة العامة لتنظيم الإعلام"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"media_regulation_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23698), published 30/2/1445H (2023-09-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "media_regulation_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "MEDIA_REGULATION_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "media-regulation-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "media_regulation_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the General Authority for Media Regulation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 30/2/1445H / 2023-09-15)"),
                "source_authority_ar": ("تنظيم الهيئة العامة لتنظيم الإعلام — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "media_regulation_authority_statute",
        "layer": "MEDIA_REGULATION_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "media-regulation-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Statute of the General Authority for Media Regulation — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the General Authority for Media Regulation records" % (len(ver), len(llm)))
