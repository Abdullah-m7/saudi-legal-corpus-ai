#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام مكافحة التسول track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18341),
...
SRC = os.path.join(ROOT, "sources", "anti_begging_law", "official_source",
                   "anti_begging_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "anti_begging_law", "verified")
RECORDS = os.path.join(OUT_VER, "anti_begging_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "anti_begging_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "anti_begging_law_arabic_legal_llm",
                        "anti_begging_law_legal_llm_001_010.json")

LAW_ID = "sa-anti-begging-law"
LAW_AR = "نظام مكافحة التسول"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"anti_begging_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18341), published 17/2/1443H (2021-09-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "anti_begging_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "ANTI_BEGGING_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "anti-begging-law-llm-%03d" % idx,
            ...
            "article_path": "anti_begging_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Anti-Begging Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/2/1443H / 2021-09-24)"),
                "source_authority_ar": ("نظام مكافحة التسول — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "anti_begging_law",
        "layer": "ANTI_BEGGING_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "anti-begging-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Anti-Begging Law — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Anti-Begging Law records" % (len(ver), len(llm)))
