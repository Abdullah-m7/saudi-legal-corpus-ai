#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the التعليمات الخاصة بصناديق استثمار التمويل المباشر track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19302),
...
SRC = os.path.join(ROOT, "sources", "direct_finance_investment_funds_instructions", "official_source",
                   "direct_finance_investment_funds_instructions_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "direct_finance_investment_funds_instructions", "verified")
RECORDS = os.path.join(OUT_VER, "direct_finance_investment_funds_instructions_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "direct_finance_investment_funds_instructions_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "direct_finance_investment_funds_instructions_arabic_legal_llm",
                        "direct_finance_investment_funds_instructions_legal_llm_001_010.json")

LAW_ID = "sa-direct-finance-investment-funds-instructions"
LAW_AR = "التعليمات الخاصة بصناديق استثمار التمويل المباشر"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"direct_finance_investment_funds_instructions_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19302), published 14/9/1443H (2022-04-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "direct_finance_investment_funds_instructions",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "DIRECT_FINANCE_INVESTMENT_FUNDS_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "direct-finance-investment-funds-instructions-llm-%03d" % idx,
            ...
            "article_path": "direct_finance_investment_funds_instructions/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Instructions for Direct Financing Investment Funds -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/9/1443H / 2022-04-15)"),
                "source_authority_ar": ("التعليمات الخاصة بصناديق استثمار التمويل المباشر — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "direct_finance_investment_funds_instructions",
        "layer": "DIRECT_FINANCE_INVESTMENT_FUNDS_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "direct-finance-investment-funds-instructions-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Instructions for Direct Financing Investment Funds — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Instructions for Direct Financing Investment Funds records" % (len(ver), len(llm)))
