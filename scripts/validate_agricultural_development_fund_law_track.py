#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام صندوق التنمية الزراعية track.
...
RECORDS = os.path.join(ROOT, "sources", "agricultural_development_fund_law", "verified",
                       "agricultural_development_fund_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "agricultural_development_fund_law", "verified",
                       "agricultural_development_fund_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "agricultural_development_fund_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"agricultural_development_fund_law_no_decision_number_in_source", "agricultural_development_fund_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام صندوق التنمية الزراعية":
    ...
    if e:
        print("FAIL: %d error(s) in Law of the Agricultural Development Fund track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law of the Agricultural Development Fund")
