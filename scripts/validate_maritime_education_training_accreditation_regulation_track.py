#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لاعتماد مؤسسات التعليم والتدريب البحري track.
...
RECORDS = os.path.join(ROOT, "sources", "maritime_education_training_accreditation_regulation", "verified",
                       "maritime_education_training_accreditation_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "maritime_education_training_accreditation_regulation", "verified",
                       "maritime_education_training_accreditation_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "maritime_education_training_accreditation_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"maritime_education_training_accreditation_regulation_no_decision_number_in_source", "maritime_education_training_accreditation_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "4/8/1444":
        e.append("[2j] gazette_publication_date_hijri must be 4/8/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-02-24":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-02-24")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنفيذية لاعتماد مؤسسات التعليم والتدريب البحري":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation for the Accreditation of Maritime Education and Training Institutions track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation for the Accreditation of Maritime Education and Training Institutions")
