#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الاقتصادية لخدمات النقل الجوي track.
...
RECORDS = os.path.join(ROOT, "sources", "air_transport_services_economic_regulation", "verified",
                       "air_transport_services_economic_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "air_transport_services_economic_regulation", "verified",
                       "air_transport_services_economic_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "air_transport_services_economic_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"air_transport_services_economic_regulation_no_decision_number_in_source", "air_transport_services_economic_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة الاقتصادية لخدمات النقل الجوي":
    ...
    if e:
        print("FAIL: %d error(s) in Economic Regulation for Air Transport Services track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Economic Regulation for Air Transport Services")
