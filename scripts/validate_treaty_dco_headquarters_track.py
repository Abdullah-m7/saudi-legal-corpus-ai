#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية مقر بين حكومة المملكة ومنظمة التعاون الرقمي track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_dco_headquarters", "verified",
                       "treaty_dco_headquarters_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_dco_headquarters", "verified",
                       "treaty_dco_headquarters_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_dco_headquarters_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_dco_headquarters_no_decision_number_in_source", "treaty_dco_headquarters_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "20/6/1444":
        e.append("[2j] gazette_publication_date_hijri must be 20/6/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-01-13":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-01-13")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية مقر بين حكومة المملكة ومنظمة التعاون الرقمي":
    ...
    if e:
        print("FAIL: %d error(s) in Headquarters Agreement between Saudi Arabia and the Digital Cooperation Organization track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Headquarters Agreement between Saudi Arabia and the Digital Cooperation Organization")
