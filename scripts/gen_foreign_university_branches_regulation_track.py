#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية لفروع الجامعات الأجنبية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23897),
...
SRC = os.path.join(ROOT, "sources", "foreign_university_branches_regulation", "official_source",
                   "foreign_university_branches_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "foreign_university_branches_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "foreign_university_branches_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "foreign_university_branches_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "foreign_university_branches_regulation_arabic_legal_llm",
                        "foreign_university_branches_regulation_legal_llm_001_026.json")

LAW_ID = "sa-foreign-university-branches-regulation"
LAW_AR = "اللائحة التنظيمية لفروع الجامعات الأجنبية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"foreign_university_branches_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23897), published 5/4/1445H (2023-10-20). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "foreign_university_branches_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "FOREIGN_UNIVERSITY_BRANCHES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "foreign-university-branches-regulation-llm-%03d" % idx,
            ...
            "article_path": "foreign_university_branches_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for Branches of Foreign Universities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/4/1445H / 2023-10-20)"),
                "source_authority_ar": ("اللائحة التنظيمية لفروع الجامعات الأجنبية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "foreign_university_branches_regulation",
        "layer": "FOREIGN_UNIVERSITY_BRANCHES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "foreign-university-branches-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (26 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for Branches of Foreign Universities — Arabic LLM-ready layer (26 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for Branches of Foreign Universities records" % (len(ver), len(llm)))
