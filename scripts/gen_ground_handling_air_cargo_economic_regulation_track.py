#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة الاقتصادية لخدمات المناولة الأرضية وخدمات الشحن الجوي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23978),
published 15/4/1445H (2023-10-30). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "ground_handling_air_cargo_economic_regulation", "official_source",
                   "ground_handling_air_cargo_economic_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "ground_handling_air_cargo_economic_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "ground_handling_air_cargo_economic_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "ground_handling_air_cargo_economic_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "ground_handling_air_cargo_economic_regulation_arabic_legal_llm",
                        "ground_handling_air_cargo_economic_regulation_legal_llm_001_024.json")

LAW_ID = "sa-ground-handling-air-cargo-economic-regulation"
LAW_AR = "اللائحة الاقتصادية لخدمات المناولة الأرضية وخدمات الشحن الجوي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"ground_handling_air_cargo_economic_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23978), published 15/4/1445H (2023-10-30). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "ground_handling_air_cargo_economic_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "GROUND_HANDLING_AIR_CARGO_ECONOMIC_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "ground-handling-air-cargo-economic-regulation-llm-%03d" % idx,
            ...
            "article_path": "ground_handling_air_cargo_economic_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Economic Regulation for Ground Handling and Air Cargo Services -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/4/1445H / 2023-10-30)"),
                "source_authority_ar": ("اللائحة الاقتصادية لخدمات المناولة الأرضية وخدمات الشحن الجوي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 15/4/1445هـ الموافق 2023-10-30م)"),
    ...
    json.dump({
        "law_key": "ground_handling_air_cargo_economic_regulation",
        "layer": "GROUND_HANDLING_AIR_CARGO_ECONOMIC_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "ground-handling-air-cargo-economic-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (24 مادة، جميعها أصلية)",
        "title_en": "Economic Regulation for Ground Handling and Air Cargo Services — Arabic LLM-ready layer (24 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Economic Regulation for Ground Handling and Air Cargo Services records" % (len(ver), len(llm)))
