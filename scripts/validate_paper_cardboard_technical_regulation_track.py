#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الفنية للورق والكرتون track.
...
RECORDS = os.path.join(ROOT, "sources", "paper_cardboard_technical_regulation", "verified",
                       "paper_cardboard_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "paper_cardboard_technical_regulation", "verified",
                       "paper_cardboard_technical_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "paper_cardboard_technical_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"paper_cardboard_technical_regulation_no_decision_number_in_source", "paper_cardboard_technical_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة الفنية للورق والكرتون":
    ...
    if e:
        print("FAIL: %d error(s) in Technical Regulation for Paper and Cardboard track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Technical Regulation for Paper and Cardboard")
