#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مدينة الملك عبدالله للطاقة الذرية والمتجددة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19805),
...
SRC = os.path.join(ROOT, "sources", "kacare_statute", "official_source",
                   "kacare_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "kacare_statute", "verified")
RECORDS = os.path.join(OUT_VER, "kacare_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "kacare_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "kacare_statute_arabic_legal_llm",
                        "kacare_statute_legal_llm_001_015.json")

LAW_ID = "sa-kacare-statute"
LAW_AR = "تنظيم مدينة الملك عبدالله للطاقة الذرية والمتجددة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"kacare_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19805), published 30/12/1443H (2022-07-29). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "kacare_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "KACARE_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "kacare-statute-llm-%03d" % idx,
            ...
            "article_path": "kacare_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the King Abdullah City for Atomic and Renewable Energy (K.A.CARE) -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 30/12/1443H / 2022-07-29)"),
                "source_authority_ar": ("تنظيم مدينة الملك عبدالله للطاقة الذرية والمتجددة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "kacare_statute",
        "layer": "KACARE_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "kacare-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Statute of the King Abdullah City for Atomic and Renewable Energy (K.A.CARE) — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the King Abdullah City for Atomic and Renewable Energy (K.A.CARE) records" % (len(ver), len(llm)))
