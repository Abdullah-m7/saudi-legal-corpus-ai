#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الفنية لتركيبات العربات المعلقة track.
...
RECORDS = os.path.join(ROOT, "sources", "cableway_installations_technical_regulation", "verified",
                       "cableway_installations_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "cableway_installations_technical_regulation", "verified",
                       "cableway_installations_technical_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "cableway_installations_technical_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"cableway_installations_technical_regulation_no_decision_number_in_source", "cableway_installations_technical_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "9/2/1445":
        e.append("[2j] gazette_publication_date_hijri must be 9/2/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-08-25":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-08-25")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة الفنية لتركيبات العربات المعلقة":
    ...
    if e:
        print("FAIL: %d error(s) in Technical Regulation for Cableway Installations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Technical Regulation for Cableway Installations")
