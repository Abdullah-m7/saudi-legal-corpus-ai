#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام إجراءات التراخيص البلدية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24611),
...
SRC = os.path.join(ROOT, "sources", "municipal_licensing_procedures_regulation", "official_source",
                   "municipal_licensing_procedures_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "municipal_licensing_procedures_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "municipal_licensing_procedures_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "municipal_licensing_procedures_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "municipal_licensing_procedures_regulation_arabic_legal_llm",
                        "municipal_licensing_procedures_regulation_legal_llm_001_016.json")

LAW_ID = "sa-municipal-licensing-procedures-regulation"
LAW_AR = "اللائحة التنفيذية لنظام إجراءات التراخيص البلدية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"municipal_licensing_procedures_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24611), published 20/8/1445H (2024-03-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "municipal_licensing_procedures_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MUNICIPAL_LICENSING_PROCEDURES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "municipal-licensing-procedures-regulation-llm-%03d" % idx,
            ...
            "article_path": "municipal_licensing_procedures_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Municipal Licensing Procedures Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/8/1445H / 2024-03-01)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام إجراءات التراخيص البلدية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "municipal_licensing_procedures_regulation",
        "layer": "MUNICIPAL_LICENSING_PROCEDURES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "municipal-licensing-procedures-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Municipal Licensing Procedures Law — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Municipal Licensing Procedures Law records" % (len(ver), len(llm)))
