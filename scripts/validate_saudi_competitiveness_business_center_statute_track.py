#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم المركز السعودي للتنافسية والأعمال track.

11 records: 16 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "saudi_competitiveness_business_center_statute", "verified",
                       "saudi_competitiveness_business_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "saudi_competitiveness_business_center_statute", "verified",
                       "saudi_competitiveness_business_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "saudi_competitiveness_business_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "saudi_competitiveness_business_center_statute_found_by_walking_a_url_section_the_index_never_covered",
    "saudi_competitiveness_business_center_statute_no_decision_number_in_source",
    "saudi_competitiveness_business_center_statute_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/9/1447":
        e.append("[2j] gazette_publication_date_hijri must be 24/9/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-03-13":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-03-13")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم المركز السعودي للتنافسية والأعمال":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Saudi Center for Competitiveness and Business track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Saudi Center for Competitiveness and Business")
