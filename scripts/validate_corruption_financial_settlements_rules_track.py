#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد إجراء التسويات المالية مع من ارتكبوا جرائم فساد من ذوي الصفة الطبيعية أو الاعتبارية track.
...
RECORDS = os.path.join(ROOT, "sources", "corruption_financial_settlements_rules", "verified",
                       "corruption_financial_settlements_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "corruption_financial_settlements_rules", "verified",
                       "corruption_financial_settlements_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "corruption_financial_settlements_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"corruption_financial_settlements_rules_no_decision_number_in_source", "corruption_financial_settlements_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "1/8/1446":
        e.append("[2j] gazette_publication_date_hijri must be 1/8/1446")
    if src.get("gazette_publication_date_gregorian") != "2025-01-31":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-01-31")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد إجراء التسويات المالية مع من ارتكبوا جرائم فساد من ذوي الصفة الطبيعية أو الاعتبارية":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Financial Settlements with Natural Persons Who Committed Corruption Offences track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Financial Settlements with Natural Persons Who Committed Corruption Offences")
