#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الدليل الاسترشادي لحوكمة الجهات العامة track.
...
RECORDS = os.path.join(ROOT, "sources", "public_entities_governance_guide", "verified",
                       "public_entities_governance_guide_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "public_entities_governance_guide", "verified",
                       "public_entities_governance_guide_verified_summary.json")
LLM = os.path.join(ROOT, "data", "public_entities_governance_guide_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"public_entities_governance_guide_no_decision_number_in_source", "public_entities_governance_guide_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الدليل الاسترشادي لحوكمة الجهات العامة":
    ...
    if e:
        print("FAIL: %d error(s) in Guidance Manual for the Governance of Public Entities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Guidance Manual for the Governance of Public Entities")
