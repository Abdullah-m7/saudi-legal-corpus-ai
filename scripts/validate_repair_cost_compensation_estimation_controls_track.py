#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط تحديد تكاليف الإصلاح وتقدير التعويضات track.
...
RECORDS = os.path.join(ROOT, "sources", "repair_cost_compensation_estimation_controls", "verified",
                       "repair_cost_compensation_estimation_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "repair_cost_compensation_estimation_controls", "verified",
                       "repair_cost_compensation_estimation_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "repair_cost_compensation_estimation_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"repair_cost_compensation_estimation_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "ضوابط تحديد تكاليف الإصلاح وتقدير التعويضات":
    ...
    if e:
        print("FAIL: %d error(s) in Controls for Determining Repair Costs and Estimating Compensation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls for Determining Repair Costs and Estimating Compensation")
