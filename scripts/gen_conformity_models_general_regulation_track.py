#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة العامة لنماذج المطابقة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24065),
...
SRC = os.path.join(ROOT, "sources", "conformity_models_general_regulation", "official_source",
                   "conformity_models_general_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "conformity_models_general_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "conformity_models_general_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "conformity_models_general_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "conformity_models_general_regulation_arabic_legal_llm",
                        "conformity_models_general_regulation_legal_llm_001_007.json")

LAW_ID = "sa-conformity-models-general-regulation"
LAW_AR = "اللائحة العامة لنماذج المطابقة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"conformity_models_general_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24065), published 3/5/1445H (2023-11-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "conformity_models_general_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CONFORMITY_MODELS_GENERAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "conformity-models-general-regulation-llm-%03d" % idx,
            ...
            "article_path": "conformity_models_general_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("General Regulation for Conformity Assessment Models -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/5/1445H / 2023-11-17)"),
                "source_authority_ar": ("اللائحة العامة لنماذج المطابقة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "conformity_models_general_regulation",
        "layer": "CONFORMITY_MODELS_GENERAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "conformity-models-general-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (7 مادة، جميعها أصلية)",
        "title_en": "General Regulation for Conformity Assessment Models — Arabic LLM-ready layer (7 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready General Regulation for Conformity Assessment Models records" % (len(ver), len(llm)))
