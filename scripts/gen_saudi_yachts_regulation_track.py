#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية لليخوت السعودية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25451),
published 2/3/1446H (2024-09-05). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "saudi_yachts_regulation", "official_source",
                   "saudi_yachts_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "saudi_yachts_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "saudi_yachts_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "saudi_yachts_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "saudi_yachts_regulation_arabic_legal_llm",
                        "saudi_yachts_regulation_legal_llm_001_019.json")

LAW_ID = "sa-saudi-yachts-regulation"
LAW_AR = "اللائحة التنظيمية لليخوت السعودية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"saudi_yachts_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25451), published 2/3/1446H (2024-09-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "saudi_yachts_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SAUDI_YACHTS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "saudi-yachts-regulation-llm-%03d" % idx,
            ...
            "article_path": "saudi_yachts_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for Saudi Yachts -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/3/1446H / 2024-09-05)"),
                "source_authority_ar": ("اللائحة التنظيمية لليخوت السعودية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 2/3/1446هـ الموافق 2024-09-05م)"),
    ...
    json.dump({
        "law_key": "saudi_yachts_regulation",
        "layer": "SAUDI_YACHTS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "saudi-yachts-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (19 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for Saudi Yachts — Arabic LLM-ready layer (19 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for Saudi Yachts records" % (len(ver), len(llm)))
