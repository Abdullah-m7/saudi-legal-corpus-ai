#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة للاستثمار الأجنبي في الأوراق المالية track.
...
RECORDS = os.path.join(ROOT, "sources", "foreign_investment_securities_rules", "verified",
                       "foreign_investment_securities_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "foreign_investment_securities_rules", "verified",
                       "foreign_investment_securities_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "foreign_investment_securities_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"foreign_investment_securities_rules_no_decision_number_in_source", "foreign_investment_securities_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing Foreign Investment in Securities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing Foreign Investment in Securities")
