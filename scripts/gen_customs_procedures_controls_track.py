#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الضوابط المنظمة للإجراءات الجمركية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24297),
...
STRUCTURE: 48 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "customs_procedures_controls", "official_source",
                   "customs_procedures_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "customs_procedures_controls", "verified")
RECORDS = os.path.join(OUT_VER, "customs_procedures_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "customs_procedures_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "customs_procedures_controls_arabic_legal_llm",
                        "customs_procedures_controls_legal_llm_001_048.json")

LAW_ID = "sa-customs-procedures-controls"
LAW_AR = "الضوابط المنظمة للإجراءات الجمركية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"customs_procedures_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24297), published 16/6/1445H (2023-12-29). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "customs_procedures_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "CUSTOMS_PROCEDURES_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "customs-procedures-controls-llm-%03d" % idx,
            ...
            "article_path": "customs_procedures_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls Governing Customs Procedures -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 16/6/1445H / 2023-12-29)"),
                "source_authority_ar": ("الضوابط المنظمة للإجراءات الجمركية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "customs_procedures_controls",
        "layer": "CUSTOMS_PROCEDURES_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "customs-procedures-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (48 مادة، جميعها أصلية)",
        "title_en": "Controls Governing Customs Procedures — Arabic LLM-ready layer (48 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls Governing Customs Procedures records" % (len(ver), len(llm)))
