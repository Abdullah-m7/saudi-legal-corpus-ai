#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الضوابط العامة لإقامة مزادات الإبل المؤقتة خارج أسواق النفع العام track.
...
RECORDS = os.path.join(ROOT, "sources", "temporary_camel_auctions_controls", "verified",
                       "temporary_camel_auctions_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "temporary_camel_auctions_controls", "verified",
                       "temporary_camel_auctions_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "temporary_camel_auctions_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"temporary_camel_auctions_controls_no_decision_number_in_source", "temporary_camel_auctions_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "6/8/1445":
        e.append("[2j] gazette_publication_date_hijri must be 6/8/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-02-16":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-02-16")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الضوابط العامة لإقامة مزادات الإبل المؤقتة خارج أسواق النفع العام":
    ...
    if e:
        print("FAIL: %d error(s) in General Controls for Holding Temporary Camel Auctions Outside Public-Benefit Markets track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: General Controls for Holding Temporary Camel Auctions Outside Public-Benefit Markets")
