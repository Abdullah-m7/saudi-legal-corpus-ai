#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية للمكتب الاستراتيجي لتطوير منطقة الجوف track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19658),
...
SRC = os.path.join(ROOT, "sources", "jouf_strategic_office_statute", "official_source",
                   "jouf_strategic_office_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "jouf_strategic_office_statute", "verified")
RECORDS = os.path.join(OUT_VER, "jouf_strategic_office_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "jouf_strategic_office_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "jouf_strategic_office_statute_arabic_legal_llm",
                        "jouf_strategic_office_statute_legal_llm_001_013.json")

LAW_ID = "sa-jouf-strategic-office-statute"
LAW_AR = "الترتيبات التنظيمية للمكتب الاستراتيجي لتطوير منطقة الجوف"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"jouf_strategic_office_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19658), published 25/11/1443H (2022-06-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "jouf_strategic_office_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "JOUF_STRATEGIC_OFFICE_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "jouf-strategic-office-statute-llm-%03d" % idx,
            ...
            "article_path": "jouf_strategic_office_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the Strategic Office for the Development of the Al-Jouf Region -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/11/1443H / 2022-06-24)"),
                "source_authority_ar": ("الترتيبات التنظيمية للمكتب الاستراتيجي لتطوير منطقة الجوف — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "jouf_strategic_office_statute",
        "layer": "JOUF_STRATEGIC_OFFICE_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "jouf-strategic-office-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the Strategic Office for the Development of the Al-Jouf Region — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the Strategic Office for the Development of the Al-Jouf Region records" % (len(ver), len(llm)))
