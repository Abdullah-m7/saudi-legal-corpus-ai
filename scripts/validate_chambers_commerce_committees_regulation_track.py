#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة اللجان الوطنية واللجان القطاعية في الغرف التجارية track.
...
RECORDS = os.path.join(ROOT, "sources", "chambers_commerce_committees_regulation", "verified",
                       "chambers_commerce_committees_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "chambers_commerce_committees_regulation", "verified",
                       "chambers_commerce_committees_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "chambers_commerce_committees_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"chambers_commerce_committees_regulation_no_decision_number_in_source", "chambers_commerce_committees_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/5/1445":
        e.append("[2j] gazette_publication_date_hijri must be 24/5/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-12-08":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-12-08")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة اللجان الوطنية واللجان القطاعية في الغرف التجارية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation of National and Sectoral Committees in the Chambers of Commerce track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation of National and Sectoral Committees in the Chambers of Commerce")
