#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد أعمال اللجنة التنفيذية الدائمة لمراكز الخدمة ومحطات الوقود track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17559),
...
SRC = os.path.join(ROOT, "sources", "service_centers_fuel_stations_committee_rules", "official_source",
                   "service_centers_fuel_stations_committee_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "service_centers_fuel_stations_committee_rules", "verified")
RECORDS = os.path.join(OUT_VER, "service_centers_fuel_stations_committee_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "service_centers_fuel_stations_committee_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "service_centers_fuel_stations_committee_rules_arabic_legal_llm",
                        "service_centers_fuel_stations_committee_rules_legal_llm_001_010.json")

LAW_ID = "sa-service-centers-fuel-stations-committee-rules"
LAW_AR = "قواعد أعمال اللجنة التنفيذية الدائمة لمراكز الخدمة ومحطات الوقود"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"service_centers_fuel_stations_committee_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17559), published 25/9/1442H (2021-05-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "service_centers_fuel_stations_committee_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SERVICE_CENTERS_FUEL_STATIONS_COMMITTEE_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "service-centers-fuel-stations-committee-rules-llm-%03d" % idx,
            ...
            "article_path": "service_centers_fuel_stations_committee_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules of Procedure of the Permanent Executive Committee for Service Centers and Fuel Stations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/9/1442H / 2021-05-07)"),
                "source_authority_ar": ("قواعد أعمال اللجنة التنفيذية الدائمة لمراكز الخدمة ومحطات الوقود — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "service_centers_fuel_stations_committee_rules",
        "layer": "SERVICE_CENTERS_FUEL_STATIONS_COMMITTEE_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "service-centers-fuel-stations-committee-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Rules of Procedure of the Permanent Executive Committee for Service Centers and Fuel Stations — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules of Procedure of the Permanent Executive Committee for Service Centers and Fuel Stations records" % (len(ver), len(llm)))
