#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية لليخوت الخاصة الزائرة track.
...
RECORDS = os.path.join(ROOT, "sources", "visiting_private_yachts_regulation", "verified",
                       "visiting_private_yachts_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "visiting_private_yachts_regulation", "verified",
                       "visiting_private_yachts_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "visiting_private_yachts_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"visiting_private_yachts_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية لليخوت الخاصة الزائرة":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Regulation for Visiting Private Yachts track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Regulation for Visiting Private Yachts")
