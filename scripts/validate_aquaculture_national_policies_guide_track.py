#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the دليل السياسات والممارسات الوطنية للاستزراع المائي track.
...
RECORDS = os.path.join(ROOT, "sources", "aquaculture_national_policies_guide", "verified",
                       "aquaculture_national_policies_guide_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "aquaculture_national_policies_guide", "verified",
                       "aquaculture_national_policies_guide_verified_summary.json")
LLM = os.path.join(ROOT, "data", "aquaculture_national_policies_guide_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"aquaculture_national_policies_guide_no_decision_number_in_source", "aquaculture_national_policies_guide_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "17/5/1445":
        e.append("[2j] gazette_publication_date_hijri must be 17/5/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-12-01":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-12-01")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "دليل السياسات والممارسات الوطنية للاستزراع المائي":
    ...
    if e:
        print("FAIL: %d error(s) in National Aquaculture Policies and Practices Guide track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: National Aquaculture Policies and Practices Guide")
