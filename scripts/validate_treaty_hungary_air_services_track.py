#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية بين حكومتي المملكة وهنغاريا في مجال خدمات النقل الجوي track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_hungary_air_services", "verified",
                       "treaty_hungary_air_services_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_hungary_air_services", "verified",
                       "treaty_hungary_air_services_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_hungary_air_services_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_hungary_air_services_no_decision_number_in_source", "treaty_hungary_air_services_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "30/12/1443":
        e.append("[2j] gazette_publication_date_hijri must be 30/12/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-07-29":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-07-29")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية بين حكومتي المملكة وهنغاريا في مجال خدمات النقل الجوي":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Hungary Air Services Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Hungary Air Services Agreement")
