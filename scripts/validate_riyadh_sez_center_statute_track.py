#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لمركز المناطق الاقتصادية الخاصة بمدينة الرياض track.
...
RECORDS = os.path.join(ROOT, "sources", "riyadh_sez_center_statute", "verified",
                       "riyadh_sez_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "riyadh_sez_center_statute", "verified",
                       "riyadh_sez_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "riyadh_sez_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"riyadh_sez_center_statute_no_decision_number_in_source", "riyadh_sez_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "6/6/1444":
        e.append("[2j] gazette_publication_date_hijri must be 6/6/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-12-30":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-12-30")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية لمركز المناطق الاقتصادية الخاصة بمدينة الرياض":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the Special Economic Zones Center in Riyadh track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the Special Economic Zones Center in Riyadh")
