#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط مكافآت أعضاء مجالس إدارة الأجهزة واللجان المنبثقة عنها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23732),
published 6/3/1445H (2023-09-21). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "board_committee_remuneration_controls", "official_source",
                   "board_committee_remuneration_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "board_committee_remuneration_controls", "verified")
RECORDS = os.path.join(OUT_VER, "board_committee_remuneration_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "board_committee_remuneration_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "board_committee_remuneration_controls_arabic_legal_llm",
                        "board_committee_remuneration_controls_legal_llm_001_008.json")

LAW_ID = "sa-board-committee-remuneration-controls"
LAW_AR = "ضوابط مكافآت أعضاء مجالس إدارة الأجهزة واللجان المنبثقة عنها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"board_committee_remuneration_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23732), published 6/3/1445H (2023-09-21). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "board_committee_remuneration_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "BOARD_COMMITTEE_REMUNERATION_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "board-committee-remuneration-controls-llm-%03d" % idx,
            ...
            "article_path": "board_committee_remuneration_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on Remuneration of Members of Boards of Government Bodies and Their Committees -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/3/1445H / 2023-09-21)"),
                "source_authority_ar": ("ضوابط مكافآت أعضاء مجالس إدارة الأجهزة واللجان المنبثقة عنها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 6/3/1445هـ الموافق 2023-09-21م)"),
    ...
    json.dump({
        "law_key": "board_committee_remuneration_controls",
        "layer": "BOARD_COMMITTEE_REMUNERATION_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "board-committee-remuneration-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Controls on Remuneration of Members of Boards of Government Bodies and Their Committees — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on Remuneration of Members of Boards of Government Bodies and Their Committees records" % (len(ver), len(llm)))
