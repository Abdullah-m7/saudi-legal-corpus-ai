#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Executive Regulation for Protected Areas
track (اللائحة التنفيذية للمناطق المحمية).

11 records: 10 numbered articles (المادة الأولى .. العاشرة, no chapter
division) + 1 appendix-type record (الجدول (١) المخالفات والعقوبات، 32
rows). All 11 اصلية.

VERIFICATION TIER: TIER_2 -- see the generator docstring and the
official_source JSON's verification_methodology_note: single primary text
source (official mewa.gov.sa PDF, visually read), confirmed byte-identical
to the same file independently hosted on ncw.gov.sa (the competent Center).
This validator only checks internal self-consistency of the ingested text
and that every discrepancy is disclosed.
"""
...
RECORDS = os.path.join(ROOT, "sources", "environmental_protected_areas",
                       "verified",
                       "environmental_protected_areas_reg_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "environmental_protected_areas",
                       "verified",
                       "environmental_protected_areas_reg_verified_summary.json")
LLM = os.path.join(ROOT, "data",
                   "environmental_protected_areas_reg_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "environmental_protected_areas_reg_no_decision_number",
    "environmental_protected_areas_reg_gazette_page_inaccessible",
    "environmental_protected_areas_reg_possible_pending_update",
}
AR = "ء-ي"
...
def main():
    ...
    # [2c] chapter_structure coverage (articles 1-10, appendices 1-1)
    ...
    art9 = arts.get("environmental_protected_areas_reg_art_009", {})
    if "يحظر" not in art9.get("text", ""):
        e.append("[2j] Article 9 must contain prohibitions (يحظر)")
    penalties = arts.get("environmental_protected_areas_reg_appendix_001", {})
    if "الجدول (1)" not in penalties.get("text", ""):
        e.append("[2j] The penalties appendix must be labeled 'الجدول (1)' verbatim")
    if "500,000" not in penalties.get("text", "") and "500,000)" not in penalties.get("text", ""):
        e.append("[2j] Penalties table must contain the 500,000-riyal maximum fine")
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation for Protected Areas track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation for Protected Areas")
    print("  - 11 records: 10 articles (no chapters) + 1 appendix (32-row penalties table);")
    print("    all 11 اصلية")
    print("  - Single primary text source (official mewa.gov.sa PDF, visually read),")
    print("    confirmed byte-identical to the same file independently hosted on ncw.gov.sa")
    print("    (the competent Center)")
    print("  - VERIFICATION TIER: TIER_2")
    return 0
