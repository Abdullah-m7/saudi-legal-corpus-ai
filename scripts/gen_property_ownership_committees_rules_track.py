#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد وآلية عمل لجان النظر في طلبات تملك العقارات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19437),
...
SRC = os.path.join(ROOT, "sources", "property_ownership_committees_rules", "official_source",
                   "property_ownership_committees_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "property_ownership_committees_rules", "verified")
RECORDS = os.path.join(OUT_VER, "property_ownership_committees_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "property_ownership_committees_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "property_ownership_committees_rules_arabic_legal_llm",
                        "property_ownership_committees_rules_legal_llm_001_030.json")

LAW_ID = "sa-property-ownership-committees-rules"
LAW_AR = "قواعد وآلية عمل لجان النظر في طلبات تملك العقارات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"property_ownership_committees_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19437), published 12/10/1443H (2022-05-13). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "property_ownership_committees_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PROPERTY_OWNERSHIP_COMMITTEES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "property-ownership-committees-rules-llm-%03d" % idx,
            ...
            "article_path": "property_ownership_committees_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules and Working Mechanism of the Committees Reviewing Property Ownership Applications -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 12/10/1443H / 2022-05-13)"),
                "source_authority_ar": ("قواعد وآلية عمل لجان النظر في طلبات تملك العقارات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "property_ownership_committees_rules",
        "layer": "PROPERTY_OWNERSHIP_COMMITTEES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "property-ownership-committees-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (30 مادة، جميعها أصلية)",
        "title_en": "Rules and Working Mechanism of the Committees Reviewing Property Ownership Applications — Arabic LLM-ready layer (30 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules and Working Mechanism of the Committees Reviewing Property Ownership Applications records" % (len(ver), len(llm)))
