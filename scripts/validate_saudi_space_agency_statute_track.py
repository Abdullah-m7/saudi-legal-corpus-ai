#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم وكالة الفضاء السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "saudi_space_agency_statute", "verified",
                       "saudi_space_agency_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "saudi_space_agency_statute", "verified",
                       "saudi_space_agency_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "saudi_space_agency_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"saudi_space_agency_statute_no_decision_number_in_source", "saudi_space_agency_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "5/12/1444":
        e.append("[2j] gazette_publication_date_hijri must be 5/12/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-06-23":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-06-23")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم وكالة الفضاء السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Saudi Space Agency track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Saudi Space Agency")
