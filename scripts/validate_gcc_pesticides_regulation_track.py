#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لقانون (نظام) المبيدات في دول مجلس التعاون لدول الخليج العربية بالمملكة track.
...
RECORDS = os.path.join(ROOT, "sources", "gcc_pesticides_regulation", "verified",
                       "gcc_pesticides_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "gcc_pesticides_regulation", "verified",
                       "gcc_pesticides_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "gcc_pesticides_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"gcc_pesticides_regulation_no_decision_number_in_source", "gcc_pesticides_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية لقانون (نظام) المبيدات في دول مجلس التعاون لدول الخليج العربية بالمملكة":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation of the GCC Pesticides Law in the Kingdom track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation of the GCC Pesticides Law in the Kingdom")
