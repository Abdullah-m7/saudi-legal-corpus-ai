#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد عمل لجنة النظر في مخالفات أحكام نظام حقوق الأشخاص ذوي الإعاقة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26539),
published 24/3/1446H (2024-09-27). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "disability_rights_violations_committee_rules", "official_source",
                   "disability_rights_violations_committee_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "disability_rights_violations_committee_rules", "verified")
RECORDS = os.path.join(OUT_VER, "disability_rights_violations_committee_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "disability_rights_violations_committee_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "disability_rights_violations_committee_rules_arabic_legal_llm",
                        "disability_rights_violations_committee_rules_legal_llm_001_018.json")

LAW_ID = "sa-disability-rights-violations-committee-rules"
LAW_AR = "قواعد عمل لجنة النظر في مخالفات أحكام نظام حقوق الأشخاص ذوي الإعاقة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"disability_rights_violations_committee_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26539), published 24/3/1446H (2024-09-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "disability_rights_violations_committee_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "DISABILITY_RIGHTS_VIOLATIONS_COMMITTEE_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "disability-rights-violations-committee-rules-llm-%03d" % idx,
            ...
            "article_path": "disability_rights_violations_committee_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules of Procedure of the Committee for Adjudicating Violations of the Law on the Rights of Persons with Disabilities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/3/1446H / 2024-09-27)"),
                "source_authority_ar": ("قواعد عمل لجنة النظر في مخالفات أحكام نظام حقوق الأشخاص ذوي الإعاقة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 24/3/1446هـ الموافق 2024-09-27م)"),
    ...
    json.dump({
        "law_key": "disability_rights_violations_committee_rules",
        "layer": "DISABILITY_RIGHTS_VIOLATIONS_COMMITTEE_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "disability-rights-violations-committee-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Rules of Procedure of the Committee for Adjudicating Violations of the Law on the Rights of Persons with Disabilities — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules of Procedure of the Committee for Adjudicating Violations of the Law on the Rights of Persons with Disabilities records" % (len(ver), len(llm)))
