#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط حساب الضمان للمساهمات العقارية track.
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_contributions_escrow_controls", "verified",
                       "real_estate_contributions_escrow_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "real_estate_contributions_escrow_controls", "verified",
                       "real_estate_contributions_escrow_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "real_estate_contributions_escrow_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"real_estate_contributions_escrow_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "ضوابط حساب الضمان للمساهمات العقارية":
    ...
    if e:
        print("FAIL: %d error(s) in Controls for the Escrow Account of Real Estate Contributions track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls for the Escrow Account of Real Estate Contributions")
