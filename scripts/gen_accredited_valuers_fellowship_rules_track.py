#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد العامة لشهادة زمالة الهيئة السعودية للمقيمين المعتمدين track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23650),
...
SRC = os.path.join(ROOT, "sources", "accredited_valuers_fellowship_rules", "official_source",
                   "accredited_valuers_fellowship_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "accredited_valuers_fellowship_rules", "verified")
RECORDS = os.path.join(OUT_VER, "accredited_valuers_fellowship_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "accredited_valuers_fellowship_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "accredited_valuers_fellowship_rules_arabic_legal_llm",
                        "accredited_valuers_fellowship_rules_legal_llm_001_017.json")

LAW_ID = "sa-accredited-valuers-fellowship-rules"
LAW_AR = "القواعد العامة لشهادة زمالة الهيئة السعودية للمقيمين المعتمدين"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"accredited_valuers_fellowship_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23650), published 24/2/1445H (2023-09-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "accredited_valuers_fellowship_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "ACCREDITED_VALUERS_FELLOWSHIP_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "accredited-valuers-fellowship-rules-llm-%03d" % idx,
            ...
            "article_path": "accredited_valuers_fellowship_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("General Rules for the Fellowship Certificate of the Saudi Authority for Accredited Valuers -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/2/1445H / 2023-09-09)"),
                "source_authority_ar": ("القواعد العامة لشهادة زمالة الهيئة السعودية للمقيمين المعتمدين — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "accredited_valuers_fellowship_rules",
        "layer": "ACCREDITED_VALUERS_FELLOWSHIP_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "accredited-valuers-fellowship-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "General Rules for the Fellowship Certificate of the Saudi Authority for Accredited Valuers — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready General Rules for the Fellowship Certificate of the Saudi Authority for Accredited Valuers records" % (len(ver), len(llm)))
