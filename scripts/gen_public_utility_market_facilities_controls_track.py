#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط المنشآت الواقعة بأسواق النفع العام للخضار والفواكه واللحوم والبيض والأسماك track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27218),
published 2/1/1447H (2025-06-27). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "public_utility_market_facilities_controls", "official_source",
                   "public_utility_market_facilities_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "public_utility_market_facilities_controls", "verified")
RECORDS = os.path.join(OUT_VER, "public_utility_market_facilities_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "public_utility_market_facilities_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "public_utility_market_facilities_controls_arabic_legal_llm",
                        "public_utility_market_facilities_controls_legal_llm_001_015.json")

LAW_ID = "sa-public-utility-market-facilities-controls"
LAW_AR = "ضوابط المنشآت الواقعة بأسواق النفع العام للخضار والفواكه واللحوم والبيض والأسماك"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"public_utility_market_facilities_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27218), published 2/1/1447H (2025-06-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "public_utility_market_facilities_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PUBLIC_UTILITY_MARKET_FACILITIES_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "public-utility-market-facilities-controls-llm-%03d" % idx,
            ...
            "article_path": "public_utility_market_facilities_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls for Facilities Located in Public Utility Markets for Vegetables, Fruits, Meat, Eggs and Fish -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/1/1447H / 2025-06-27)"),
                "source_authority_ar": ("ضوابط المنشآت الواقعة بأسواق النفع العام للخضار والفواكه واللحوم والبيض والأسماك — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 2/1/1447هـ الموافق 2025-06-27م)"),
    ...
    json.dump({
        "law_key": "public_utility_market_facilities_controls",
        "layer": "PUBLIC_UTILITY_MARKET_FACILITIES_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "public-utility-market-facilities-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Controls for Facilities Located in Public Utility Markets for Vegetables, Fruits, Meat, Eggs and Fish — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls for Facilities Located in Public Utility Markets for Vegetables, Fruits, Meat, Eggs and Fish records" % (len(ver), len(llm)))
