#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية للمركز الوطني للنخيل والتمور track.
...
RECORDS = os.path.join(ROOT, "sources", "palms_dates_center_statute", "verified",
                       "palms_dates_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "palms_dates_center_statute", "verified",
                       "palms_dates_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "palms_dates_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"palms_dates_center_statute_no_decision_number_in_source", "palms_dates_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "26/5/1444":
        e.append("[2j] gazette_publication_date_hijri must be 26/5/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-12-20":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-12-20")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية للمركز الوطني للنخيل والتمور":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the National Center for Palms and Dates track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the National Center for Palms and Dates")
