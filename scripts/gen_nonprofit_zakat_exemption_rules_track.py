#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد عدم خضوع الجمعيات والمؤسسات الأهلية والوحدات التدريبية لجباية الزكاة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21808),
...
SRC = os.path.join(ROOT, "sources", "nonprofit_zakat_exemption_rules", "official_source",
                   "nonprofit_zakat_exemption_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "nonprofit_zakat_exemption_rules", "verified")
RECORDS = os.path.join(OUT_VER, "nonprofit_zakat_exemption_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "nonprofit_zakat_exemption_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "nonprofit_zakat_exemption_rules_arabic_legal_llm",
                        "nonprofit_zakat_exemption_rules_legal_llm_001_007.json")

LAW_ID = "sa-nonprofit-zakat-exemption-rules"
LAW_AR = "قواعد عدم خضوع الجمعيات والمؤسسات الأهلية والوحدات التدريبية لجباية الزكاة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"nonprofit_zakat_exemption_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21808), published 22/10/1444H (2023-05-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "nonprofit_zakat_exemption_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "NONPROFIT_ZAKAT_EXEMPTION_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "nonprofit-zakat-exemption-rules-llm-%03d" % idx,
            ...
            "article_path": "nonprofit_zakat_exemption_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules on the Non-Subjection of Associations, Non-Profit Foundations and Training Units to Zakat Collection -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/10/1444H / 2023-05-12)"),
                "source_authority_ar": ("قواعد عدم خضوع الجمعيات والمؤسسات الأهلية والوحدات التدريبية لجباية الزكاة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "nonprofit_zakat_exemption_rules",
        "layer": "NONPROFIT_ZAKAT_EXEMPTION_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "nonprofit-zakat-exemption-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (7 مادة، جميعها أصلية)",
        "title_en": "Rules on the Non-Subjection of Associations, Non-Profit Foundations and Training Units to Zakat Collection — Arabic LLM-ready layer (7 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules on the Non-Subjection of Associations, Non-Profit Foundations and Training Units to Zakat Collection records" % (len(ver), len(llm)))
