#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد الضريبية للمقرات الإقليمية track.
...
RECORDS = os.path.join(ROOT, "sources", "regional_headquarters_tax_rules", "verified",
                       "regional_headquarters_tax_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "regional_headquarters_tax_rules", "verified",
                       "regional_headquarters_tax_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "regional_headquarters_tax_rules_arabic_legal_llm",
...
def main():
    ...
    if src.get("document") != "القواعد الضريبية للمقرات الإقليمية":
    ...
    if e:
        print("FAIL: %d error(s) in Tax Rules for Regional Headquarters track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Tax Rules for Regional Headquarters")
