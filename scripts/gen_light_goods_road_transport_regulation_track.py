#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة المنظمة لنشاط النقل الخفيف للبضائع على الطرق track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18044),
published 20/12/1442H (2021-07-30). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "light_goods_road_transport_regulation", "official_source",
                   "light_goods_road_transport_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "light_goods_road_transport_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "light_goods_road_transport_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "light_goods_road_transport_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "light_goods_road_transport_regulation_arabic_legal_llm",
                        "light_goods_road_transport_regulation_legal_llm_001_027.json")

LAW_ID = "sa-light-goods-road-transport-regulation"
LAW_AR = "اللائحة المنظمة لنشاط النقل الخفيف للبضائع على الطرق"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"light_goods_road_transport_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18044), published 20/12/1442H (2021-07-30). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "light_goods_road_transport_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "LIGHT_GOODS_ROAD_TRANSPORT_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "light-goods-road-transport-regulation-llm-%03d" % idx,
            ...
            "article_path": "light_goods_road_transport_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Governing Light Goods Transport by Road -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/12/1442H / 2021-07-30)"),
                "source_authority_ar": ("اللائحة المنظمة لنشاط النقل الخفيف للبضائع على الطرق — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 20/12/1442هـ الموافق 2021-07-30م)"),
    ...
    json.dump({
        "law_key": "light_goods_road_transport_regulation",
        "layer": "LIGHT_GOODS_ROAD_TRANSPORT_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "light-goods-road-transport-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (27 مادة، جميعها أصلية)",
        "title_en": "Regulation Governing Light Goods Transport by Road — Arabic LLM-ready layer (27 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Governing Light Goods Transport by Road records" % (len(ver), len(llm)))
