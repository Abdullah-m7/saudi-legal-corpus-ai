#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم الهيئة السعودية للبحر الأحمر track.
...
RECORDS = os.path.join(ROOT, "sources", "saudi_red_sea_authority_statute", "verified",
                       "saudi_red_sea_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "saudi_red_sea_authority_statute", "verified",
                       "saudi_red_sea_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "saudi_red_sea_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"saudi_red_sea_authority_statute_no_decision_number_in_source", "saudi_red_sea_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "26/4/1445":
        e.append("[2j] gazette_publication_date_hijri must be 26/4/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-11-10":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-11-10")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم الهيئة السعودية للبحر الأحمر":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Saudi Red Sea Authority track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Saudi Red Sea Authority")
