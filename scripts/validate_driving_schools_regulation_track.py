#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الأحكام المنظمة لإنشاء وتشغيل مدارس لتعليم قيادة المركبات ومزاولة مهنة تعليم القيادة track.
...
RECORDS = os.path.join(ROOT, "sources", "driving_schools_regulation", "verified",
                       "driving_schools_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "driving_schools_regulation", "verified",
                       "driving_schools_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "driving_schools_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"driving_schools_regulation_no_decision_number_in_source", "driving_schools_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة الأحكام المنظمة لإنشاء وتشغيل مدارس لتعليم قيادة المركبات ومزاولة مهنة تعليم القيادة":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation Governing the Establishment and Operation of Vehicle Driving Schools track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation Governing the Establishment and Operation of Vehicle Driving Schools")
