#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام التنفيذ track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/.../4000869),
...
SRC = os.path.join(ROOT, "sources", "enforcement_law_1447", "official_source",
                   "enforcement_law_1447_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "enforcement_law_1447", "verified")
RECORDS = os.path.join(OUT_VER, "enforcement_law_1447_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "enforcement_law_1447_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "enforcement_law_1447_arabic_legal_llm",
                        "enforcement_law_1447_legal_llm_001_065.json")

LAW_ID = "sa-enforcement-law-1447"
LAW_AR = "نظام التنفيذ"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"enforcement_law_1447_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/.../4000869), published 14/11/1447H (2026-05-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "enforcement_law_1447",
            "law_component": "law",
            "language": "ar",
            "record_layer": "ENFORCEMENT_LAW_1447_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "enforcement-law-1447-llm-%03d" % idx,
            ...
            "article_path": "enforcement_law_1447/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Enforcement Law (1447H) -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/11/1447H / 2026-05-01)"),
                "source_authority_ar": ("نظام التنفيذ — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "enforcement_law_1447",
        "layer": "ENFORCEMENT_LAW_1447_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "enforcement-law-1447-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (65 مادة، جميعها أصلية)",
        "title_en": "Enforcement Law (1447H) — Arabic LLM-ready layer (65 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Enforcement Law (1447H) records" % (len(ver), len(llm)))
