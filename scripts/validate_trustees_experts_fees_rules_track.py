#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد أتعاب الأمناء والخبراء track.
...
RECORDS = os.path.join(ROOT, "sources", "trustees_experts_fees_rules", "verified",
                       "trustees_experts_fees_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "trustees_experts_fees_rules", "verified",
                       "trustees_experts_fees_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "trustees_experts_fees_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"trustees_experts_fees_rules_no_decision_number_in_source", "trustees_experts_fees_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "27/8/1442":
        e.append("[2j] gazette_publication_date_hijri must be 27/8/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-04-09":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-04-09")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد أتعاب الأمناء والخبراء":
    ...
    if e:
        print("FAIL: %d error(s) in Rules on the Fees of Trustees and Experts track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules on the Fees of Trustees and Experts")
