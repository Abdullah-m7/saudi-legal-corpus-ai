#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لمركز برنامج تعزيز الشراكة مع القطاع الخاص (شريك) track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21520),
...
SRC = os.path.join(ROOT, "sources", "shareek_program_center_statute", "official_source",
                   "shareek_program_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "shareek_program_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "shareek_program_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "shareek_program_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "shareek_program_center_statute_arabic_legal_llm",
                        "shareek_program_center_statute_legal_llm_001_011.json")

LAW_ID = "sa-shareek-program-center-statute"
LAW_AR = "الترتيبات التنظيمية لمركز برنامج تعزيز الشراكة مع القطاع الخاص (شريك)"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"shareek_program_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21520), published 11/8/1444H (2023-03-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "shareek_program_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "SHAREEK_PROGRAM_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "shareek-program-center-statute-llm-%03d" % idx,
            ...
            "article_path": "shareek_program_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the Center of the Private Sector Partnership Enhancement Program (Shareek) -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 11/8/1444H / 2023-03-03)"),
                "source_authority_ar": ("الترتيبات التنظيمية لمركز برنامج تعزيز الشراكة مع القطاع الخاص (شريك) — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "shareek_program_center_statute",
        "layer": "SHAREEK_PROGRAM_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "shareek-program-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the Center of the Private Sector Partnership Enhancement Program (Shareek) — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the Center of the Private Sector Partnership Enhancement Program (Shareek) records" % (len(ver), len(llm)))
