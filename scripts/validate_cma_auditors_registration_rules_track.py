#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد تسجيل مراجعي حسابات المنشآت الخاضعة لإشراف الهيئة track.
...
RECORDS = os.path.join(ROOT, "sources", "cma_auditors_registration_rules", "verified",
                       "cma_auditors_registration_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "cma_auditors_registration_rules", "verified",
                       "cma_auditors_registration_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "cma_auditors_registration_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"cma_auditors_registration_rules_no_decision_number_in_source", "cma_auditors_registration_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "2/12/1443":
        e.append("[2j] gazette_publication_date_hijri must be 2/12/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-07-01":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-07-01")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد تسجيل مراجعي حسابات المنشآت الخاضعة لإشراف الهيئة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Registering Auditors of Entities Supervised by the Authority track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Registering Auditors of Entities Supervised by the Authority")
