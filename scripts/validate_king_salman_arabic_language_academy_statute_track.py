#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مجمع الملك سلمان العالمي للغة العربية track.
...
RECORDS = os.path.join(ROOT, "sources", "king_salman_arabic_language_academy_statute", "verified",
                       "king_salman_arabic_language_academy_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "king_salman_arabic_language_academy_statute", "verified",
                       "king_salman_arabic_language_academy_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "king_salman_arabic_language_academy_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"king_salman_arabic_language_academy_statute_no_decision_number_in_source", "king_salman_arabic_language_academy_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "20/1/1442":
        e.append("[2j] gazette_publication_date_hijri must be 20/1/1442")
    if src.get("gazette_publication_date_gregorian") != "2020-09-08":
        e.append("[2j] gazette_publication_date_gregorian must be 2020-09-08")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم مجمع الملك سلمان العالمي للغة العربية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the King Salman Global Academy for the Arabic Language track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the King Salman Global Academy for the Arabic Language")
