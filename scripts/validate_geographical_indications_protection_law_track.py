#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام حماية المؤشرات الجغرافية track.
...
RECORDS = os.path.join(ROOT, "sources", "geographical_indications_protection_law", "verified",
                       "geographical_indications_protection_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "geographical_indications_protection_law", "verified",
                       "geographical_indications_protection_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "geographical_indications_protection_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"geographical_indications_protection_law_no_decision_number_in_source", "geographical_indications_protection_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "7/6/1447":
        e.append("[2j] gazette_publication_date_hijri must be 7/6/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-11-28":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-11-28")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "نظام حماية المؤشرات الجغرافية":
    ...
    if e:
        print("FAIL: %d error(s) in Law on the Protection of Geographical Indications track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law on the Protection of Geographical Indications")
