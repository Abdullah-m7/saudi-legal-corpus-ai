#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد إقرار الأحكام المنظمة لشؤون العاملين في الأجهزة العامة وتعويضاتهم track.
...
RECORDS = os.path.join(ROOT, "sources", "public_agencies_staff_provisions_rules", "verified",
                       "public_agencies_staff_provisions_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "public_agencies_staff_provisions_rules", "verified",
                       "public_agencies_staff_provisions_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "public_agencies_staff_provisions_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"public_agencies_staff_provisions_rules_no_decision_number_in_source", "public_agencies_staff_provisions_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد إقرار الأحكام المنظمة لشؤون العاملين في الأجهزة العامة وتعويضاتهم":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Approving the Provisions Governing the Affairs and Compensation of Staff in Public Agencies track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Approving the Provisions Governing the Affairs and Compensation of Staff in Public Agencies")
