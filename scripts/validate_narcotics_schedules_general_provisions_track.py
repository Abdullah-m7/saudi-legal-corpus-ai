#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the أحكام عامة على الجداول المرفقة بنظام مكافحة المخدرات والمؤثرات العقلية track.
...
RECORDS = os.path.join(ROOT, "sources", "narcotics_schedules_general_provisions", "verified",
                       "narcotics_schedules_general_provisions_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "narcotics_schedules_general_provisions", "verified",
                       "narcotics_schedules_general_provisions_verified_summary.json")
LLM = os.path.join(ROOT, "data", "narcotics_schedules_general_provisions_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"narcotics_schedules_general_provisions_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "أحكام عامة على الجداول المرفقة بنظام مكافحة المخدرات والمؤثرات العقلية":
    ...
    if e:
        print("FAIL: %d error(s) in General Provisions on the Schedules Annexed to the Anti-Narcotics Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: General Provisions on the Schedules Annexed to the Anti-Narcotics Law")
