#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Investment Regulations Arabic LLM enrichment layer."""
...
LAYER = os.path.join(ROOT, "data", "investment_arabic_legal_llm",
                     "investment_regulation_legal_llm_001_037.json")
SCHEMA = os.path.join(ROOT, "schemas", "investment_regulation_legal_llm.schema.json")
...
def main():
    ...
    for r in records:
        ...
        if r.get("record_id") != "inv-reg-llm-art-%03d" % n:
            errors.append("[3] art %s: record_id %r" % (n, r.get("record_id")))
        if r.get("article_path") != "investment/regulation/articles/%03d" % n:
    ...
    if errors:
        print("FAIL: %d error(s) in Investment Regulations LLM layer:" % len(errors))
        for e in errors:
            print("  - %s" % e)
        return 1

    print("PASS: %d Investment Regulations LLM-ready records" % len(records))
