#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام الحرف والصناعات اليدوية track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4000207),
...
SRC = os.path.join(ROOT, "sources", "handicrafts_law", "official_source",
                   "handicrafts_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "handicrafts_law", "verified")
RECORDS = os.path.join(OUT_VER, "handicrafts_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "handicrafts_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "handicrafts_law_arabic_legal_llm",
                        "handicrafts_law_legal_llm_001_018.json")

LAW_ID = "sa-handicrafts-law"
LAW_AR = "نظام الحرف والصناعات اليدوية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"handicrafts_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4000207), published 4/8/1447H (2026-01-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "handicrafts_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "HANDICRAFTS_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "handicrafts-law-llm-%03d" % idx,
            ...
            "article_path": "handicrafts_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Handicrafts and Handmade Industries Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/8/1447H / 2026-01-23)"),
                "source_authority_ar": ("نظام الحرف والصناعات اليدوية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "handicrafts_law",
        "layer": "HANDICRAFTS_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "handicrafts-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Handicrafts and Handmade Industries Law — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Handicrafts and Handmade Industries Law records" % (len(ver), len(llm)))
