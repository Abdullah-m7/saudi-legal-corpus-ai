#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية بين حكومتي المملكة و قطر في مجال خدمات النقل الجوي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19016),
...
SRC = os.path.join(ROOT, "sources", "treaty_qatar_air_services", "official_source",
                   "treaty_qatar_air_services_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_qatar_air_services", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_qatar_air_services_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_qatar_air_services_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_qatar_air_services_arabic_legal_llm",
                        "treaty_qatar_air_services_legal_llm_001_037.json")

LAW_ID = "sa-treaty-qatar-air-services"
LAW_AR = "اتفاقية بين حكومتي المملكة و قطر في مجال خدمات النقل الجوي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_qatar_air_services_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19016), published 24/7/1443H (2022-02-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_qatar_air_services",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_QATAR_AIR_SERVICES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-qatar-air-services-llm-%03d" % idx,
            ...
            "article_path": "treaty_qatar_air_services/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Arabia–Qatar Air Services Agreement -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/7/1443H / 2022-02-25)"),
                "source_authority_ar": ("اتفاقية بين حكومتي المملكة و قطر في مجال خدمات النقل الجوي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_qatar_air_services",
        "layer": "TREATY_QATAR_AIR_SERVICES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-qatar-air-services-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (37 مادة، جميعها أصلية)",
        "title_en": "Saudi Arabia–Qatar Air Services Agreement — Arabic LLM-ready layer (37 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Arabia–Qatar Air Services Agreement records" % (len(ver), len(llm)))
