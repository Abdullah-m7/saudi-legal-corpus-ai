#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام حماية حقوق المؤلف track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19857),
published 7/1/1444H (2022-08-05). The Gazette IS the official publication of record
...
STRUCTURE: 52 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "copyright_protection_implementing_regulation", "official_source",
                   "copyright_protection_implementing_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "copyright_protection_implementing_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "copyright_protection_implementing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "copyright_protection_implementing_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "copyright_protection_implementing_regulation_arabic_legal_llm",
                        "copyright_protection_implementing_regulation_legal_llm_001_052.json")

LAW_ID = "sa-copyright-protection-implementing-regulation"
LAW_AR = "اللائحة التنفيذية لنظام حماية حقوق المؤلف"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"copyright_protection_implementing_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19857), published 7/1/1444H (2022-08-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "copyright_protection_implementing_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "COPYRIGHT_PROTECTION_IMPLEMENTING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "copyright-protection-implementing-regulation-llm-%03d" % idx,
            ...
            "article_path": "copyright_protection_implementing_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Copyright Protection Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/1/1444H / 2022-08-05)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام حماية حقوق المؤلف — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 7/1/1444هـ الموافق 2022-08-05م)"),
    ...
    json.dump({
        "law_key": "copyright_protection_implementing_regulation",
        "layer": "COPYRIGHT_PROTECTION_IMPLEMENTING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "copyright-protection-implementing-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (52 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Copyright Protection Law — Arabic LLM-ready layer (52 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Copyright Protection Law records" % (len(ver), len(llm)))
