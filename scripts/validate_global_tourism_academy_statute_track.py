#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية للأكاديمية العالمية للسياحة track.
...
RECORDS = os.path.join(ROOT, "sources", "global_tourism_academy_statute", "verified",
                       "global_tourism_academy_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "global_tourism_academy_statute", "verified",
                       "global_tourism_academy_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "global_tourism_academy_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"global_tourism_academy_statute_no_decision_number_in_source", "global_tourism_academy_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "18/6/1443":
        e.append("[2j] gazette_publication_date_hijri must be 18/6/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-01-21":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-01-21")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية للأكاديمية العالمية للسياحة":
    ...
    if e:
        print("FAIL: %d error(s) in Global Academy of Tourism — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Global Academy of Tourism — Organisational Arrangements")
