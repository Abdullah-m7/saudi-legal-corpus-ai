...
server-rendered HTML page (uqn.gov.sa/details?p=17848),
...
SRC = os.path.join(ROOT, "sources", "securities_offering_obligations_rules", "official_source",
                   "securities_offering_obligations_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "securities_offering_obligations_rules", "verified")
RECORDS = os.path.join(OUT_VER, "securities_offering_obligations_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "securities_offering_obligations_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "securities_offering_obligations_rules_arabic_legal_llm",
                        "securities_offering_obligations_rules_legal_llm_001_103.json")

LAW_ID = "sa-securities-offering-obligations-rules"
LAW_AR = "قواعد طرح الأوراق المالية والالتزامات المستمرة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"securities_offering_obligations_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17848), published 15/11/1442H (2021-06-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "securities_offering_obligations_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SECURITIES_OFFERING_OBLIGATIONS_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "securities-offering-obligations-rules-llm-%03d" % idx,
            ...
            "article_path": "securities_offering_obligations_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules on the Offer of Securities and Continuing Obligations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/11/1442H / 2021-06-25)"),
                "source_authority_ar": ("قواعد طرح الأوراق المالية والالتزامات المستمرة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "securities_offering_obligations_rules",
        "layer": "SECURITIES_OFFERING_OBLIGATIONS_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "securities-offering-obligations-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (103 مادة، جميعها أصلية)",
        "title_en": "Rules on the Offer of Securities and Continuing Obligations — Arabic LLM-ready layer (103 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules on the Offer of Securities and Continuing Obligations records" % (len(ver), len(llm)))
