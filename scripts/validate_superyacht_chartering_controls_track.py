#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الضوابط والاجراءت لتاجير اليخوت الضخمة track.
...
RECORDS = os.path.join(ROOT, "sources", "superyacht_chartering_controls", "verified",
                       "superyacht_chartering_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "superyacht_chartering_controls", "verified",
                       "superyacht_chartering_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "superyacht_chartering_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"superyacht_chartering_controls_no_decision_number_in_source", "superyacht_chartering_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الضوابط والاجراءت لتاجير اليخوت الضخمة":
    ...
    if e:
        print("FAIL: %d error(s) in Controls and Procedures for Chartering Superyachts track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls and Procedures for Chartering Superyachts")
