#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد الكفاية المالية المعدلة track.

118 records: 118 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "capital_adequacy_rules", "verified",
                       "capital_adequacy_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "capital_adequacy_rules", "verified",
                       "capital_adequacy_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "capital_adequacy_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"capital_adequacy_rules_no_decision_number_in_source", "capital_adequacy_rules_source_is_gazette_html_only", "capital_adequacy_rules_source_numbering_gap"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "18/8/1444":
        e.append("[2j] gazette_publication_date_hijri must be 18/8/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-03-10":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-03-10")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد الكفاية المالية المعدلة":
    ...
    if e:
        print("FAIL: %d error(s) in Amended Prudential (Capital Adequacy) Rules track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Amended Prudential (Capital Adequacy) Rules")
    print("  - 118 records: 118 articles, no appendices; all 118 اصلية")
