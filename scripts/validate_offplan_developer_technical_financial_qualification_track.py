#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد ومعايير التأهيل الفني والمالي للمطورين العقاريين لمشاريع البيع على الخارطة track.
...
RECORDS = os.path.join(ROOT, "sources", "offplan_developer_technical_financial_qualification", "verified",
                       "offplan_developer_technical_financial_qualification_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "offplan_developer_technical_financial_qualification", "verified",
                       "offplan_developer_technical_financial_qualification_verified_summary.json")
LLM = os.path.join(ROOT, "data", "offplan_developer_technical_financial_qualification_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"offplan_developer_technical_financial_qualification_no_decision_number_in_source", "offplan_developer_technical_financial_qualification_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "23/3/1443":
        e.append("[2j] gazette_publication_date_hijri must be 23/3/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-10-29":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-10-29")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد ومعايير التأهيل الفني والمالي للمطورين العقاريين لمشاريع البيع على الخارطة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules and Criteria for the Technical and Financial Qualification of Real Estate Developers of Infrastructure Projects track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules and Criteria for the Technical and Financial Qualification of Real Estate Developers of Infrastructure Projects")
