#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the دليل الخطوط الحديدية - تعيين استشاري مختص مستقل track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23092),
published 27/11/1444H (2023-06-16). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "railway_independent_consultant_guide", "official_source",
                   "railway_independent_consultant_guide_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "railway_independent_consultant_guide", "verified")
RECORDS = os.path.join(OUT_VER, "railway_independent_consultant_guide_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "railway_independent_consultant_guide_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "railway_independent_consultant_guide_arabic_legal_llm",
                        "railway_independent_consultant_guide_legal_llm_001_004.json")

LAW_ID = "sa-railway-independent-consultant-guide"
LAW_AR = "دليل الخطوط الحديدية - تعيين استشاري مختص مستقل"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"railway_independent_consultant_guide_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23092), published 27/11/1444H (2023-06-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "railway_independent_consultant_guide",
            "law_component": "guide",
            "language": "ar",
            "record_layer": "RAILWAY_INDEPENDENT_CONSULTANT_GUIDE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "railway-independent-consultant-guide-llm-%03d" % idx,
            ...
            "article_path": "railway_independent_consultant_guide/guide/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Railway Guide — Appointment of an Independent Competent Consultant -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/11/1444H / 2023-06-16)"),
                "source_authority_ar": ("دليل الخطوط الحديدية - تعيين استشاري مختص مستقل — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 27/11/1444هـ الموافق 2023-06-16م)"),
    ...
    json.dump({
        "law_key": "railway_independent_consultant_guide",
        "layer": "RAILWAY_INDEPENDENT_CONSULTANT_GUIDE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "railway-independent-consultant-guide-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "guide",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (4 مادة، جميعها أصلية)",
        "title_en": "Railway Guide — Appointment of an Independent Competent Consultant — Arabic LLM-ready layer (4 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Railway Guide — Appointment of an Independent Competent Consultant records" % (len(ver), len(llm)))
