#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم هيئة الحكومة الرقمية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=16946),
published 6/8/1442H (2021-03-19). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "digital_government_authority_statute", "official_source",
                   "digital_government_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "digital_government_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "digital_government_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "digital_government_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "digital_government_authority_statute_arabic_legal_llm",
                        "digital_government_authority_statute_legal_llm_001_018.json")

LAW_ID = "sa-digital-government-authority-statute"
LAW_AR = "تنظيم هيئة الحكومة الرقمية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"digital_government_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=16946), published 6/8/1442H (2021-03-19). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "digital_government_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "DIGITAL_GOVERNMENT_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "digital-government-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "digital_government_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Digital Government Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/8/1442H / 2021-03-19)"),
                "source_authority_ar": ("تنظيم هيئة الحكومة الرقمية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 6/8/1442هـ الموافق 2021-03-19م)"),
    ...
    json.dump({
        "law_key": "digital_government_authority_statute",
        "layer": "DIGITAL_GOVERNMENT_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "digital-government-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Statute of the Digital Government Authority — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Digital Government Authority records" % (len(ver), len(llm)))
