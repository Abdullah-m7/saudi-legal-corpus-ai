#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط المنشآت الواقعة بأسواق النفع العام للخضار والفواكه واللحوم والبيض والأسماك track.

15 records: 15 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "public_utility_market_facilities_controls", "verified",
                       "public_utility_market_facilities_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "public_utility_market_facilities_controls", "verified",
                       "public_utility_market_facilities_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "public_utility_market_facilities_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"public_utility_market_facilities_controls_no_decision_number_in_source", "public_utility_market_facilities_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "2/1/1447":
        e.append("[2j] gazette_publication_date_hijri must be 2/1/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-06-27":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-06-27")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط المنشآت الواقعة بأسواق النفع العام للخضار والفواكه واللحوم والبيض والأسماك":
    ...
    if e:
        print("FAIL: %d error(s) in Controls for Facilities Located in Public Utility Markets for Vegetables, Fruits, Meat, Eggs and Fish track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls for Facilities Located in Public Utility Markets for Vegetables, Fruits, Meat, Eggs and Fish")
