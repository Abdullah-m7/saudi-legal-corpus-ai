#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the إجراءات وشروط الترخيص لممارسة النقل العام بالحافلات داخل المدن track.
...
RECORDS = os.path.join(ROOT, "sources", "intracity_bus_transport_licensing", "verified",
                       "intracity_bus_transport_licensing_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "intracity_bus_transport_licensing", "verified",
                       "intracity_bus_transport_licensing_verified_summary.json")
LLM = os.path.join(ROOT, "data", "intracity_bus_transport_licensing_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"intracity_bus_transport_licensing_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "إجراءات وشروط الترخيص لممارسة النقل العام بالحافلات داخل المدن":
    ...
    if e:
        print("FAIL: %d error(s) in Procedures and Conditions for Licensing Intra-City Public Bus Transport track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Procedures and Conditions for Licensing Intra-City Public Bus Transport")
