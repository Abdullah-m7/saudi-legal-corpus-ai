#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة إنشاء المحافظ الاستثمارية الوقفية track.
...
RECORDS = os.path.join(ROOT, "sources", "waqf_investment_portfolios_regulation", "verified",
                       "waqf_investment_portfolios_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "waqf_investment_portfolios_regulation", "verified",
                       "waqf_investment_portfolios_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "waqf_investment_portfolios_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"waqf_investment_portfolios_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة إنشاء المحافظ الاستثمارية الوقفية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on the Establishment of Waqf Investment Portfolios track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on the Establishment of Waqf Investment Portfolios")
