#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد نظر دعاوى إلغاء القرارات المتعلقة بتطبيق أحكام أوامر الطوارئ track.
...
RECORDS = os.path.join(ROOT, "sources", "emergency_orders_annulment_claims_rules", "verified",
                       "emergency_orders_annulment_claims_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "emergency_orders_annulment_claims_rules", "verified",
                       "emergency_orders_annulment_claims_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "emergency_orders_annulment_claims_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"emergency_orders_annulment_claims_rules_no_decision_number_in_source", "emergency_orders_annulment_claims_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد نظر دعاوى إلغاء القرارات المتعلقة بتطبيق أحكام أوامر الطوارئ":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Hearing Claims to Annul Decisions Related to Applying the Provisions of Emergency Orders track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Hearing Claims to Annul Decisions Related to Applying the Provisions of Emergency Orders")
