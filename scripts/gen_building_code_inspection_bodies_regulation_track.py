#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة المتطلبات العامة لتعيين جهات التفتيش والمفتشين لأعمال كود البناء السعودي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17667),
published 9/10/1442H (2021-05-21). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "building_code_inspection_bodies_regulation", "official_source",
                   "building_code_inspection_bodies_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "building_code_inspection_bodies_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "building_code_inspection_bodies_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "building_code_inspection_bodies_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "building_code_inspection_bodies_regulation_arabic_legal_llm",
                        "building_code_inspection_bodies_regulation_legal_llm_001_018.json")

LAW_ID = "sa-building-code-inspection-bodies-regulation"
LAW_AR = "لائحة المتطلبات العامة لتعيين جهات التفتيش والمفتشين لأعمال كود البناء السعودي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"building_code_inspection_bodies_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17667), published 9/10/1442H (2021-05-21). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "building_code_inspection_bodies_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "BUILDING_CODE_INSPECTION_BODIES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "building-code-inspection-bodies-regulation-llm-%03d" % idx,
            ...
            "article_path": "building_code_inspection_bodies_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on General Requirements for Designating Inspection Bodies and Inspectors for Saudi Building Code Works -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/10/1442H / 2021-05-21)"),
                "source_authority_ar": ("لائحة المتطلبات العامة لتعيين جهات التفتيش والمفتشين لأعمال كود البناء السعودي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 9/10/1442هـ الموافق 2021-05-21م)"),
    ...
    json.dump({
        "law_key": "building_code_inspection_bodies_regulation",
        "layer": "BUILDING_CODE_INSPECTION_BODIES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "building-code-inspection-bodies-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Regulation on General Requirements for Designating Inspection Bodies and Inspectors for Saudi Building Code Works — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on General Requirements for Designating Inspection Bodies and Inspectors for Saudi Building Code Works records" % (len(ver), len(llm)))
