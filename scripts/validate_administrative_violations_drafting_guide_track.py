#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الدليل الاسترشادي لاقتراح سن أحكام المخالفة الإدارية وجزائها track.
...
RECORDS = os.path.join(ROOT, "sources", "administrative_violations_drafting_guide", "verified",
                       "administrative_violations_drafting_guide_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "administrative_violations_drafting_guide", "verified",
                       "administrative_violations_drafting_guide_verified_summary.json")
LLM = os.path.join(ROOT, "data", "administrative_violations_drafting_guide_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"administrative_violations_drafting_guide_no_decision_number_in_source", "administrative_violations_drafting_guide_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "17/2/1443":
        e.append("[2j] gazette_publication_date_hijri must be 17/2/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-09-24":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-09-24")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الدليل الاسترشادي لاقتراح سن أحكام المخالفة الإدارية وجزائها":
    ...
    if e:
        print("FAIL: %d error(s) in Guidance Manual for Proposing Administrative Violation Provisions and Their Penalties track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Guidance Manual for Proposing Administrative Violation Provisions and Their Penalties")
