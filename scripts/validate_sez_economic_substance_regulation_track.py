#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة المتطلبات الاقتصادية الفعلية للمناطق الاقتصادية الخاصة track.

11 records: 9 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "sez_economic_substance_regulation", "verified",
                       "sez_economic_substance_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "sez_economic_substance_regulation", "verified",
                       "sez_economic_substance_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "sez_economic_substance_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "sez_economic_substance_regulation_distinct_from_the_regional_headquarters_tax_rules",
    "sez_economic_substance_regulation_no_decision_number_in_source",
    "sez_economic_substance_regulation_source_is_gazette_html_only",
    "sez_economic_substance_regulation_text_is_the_arabic_column_of_a_bilingual_table",
}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/2/1448":
        e.append("[2j] gazette_publication_date_hijri must be 24/2/1448")
    if src.get("gazette_publication_date_gregorian") != "2026-08-07":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-08-07")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة المتطلبات الاقتصادية الفعلية للمناطق الاقتصادية الخاصة":
    ...
    if e:
        print("FAIL: %d error(s) in Economic Substance Requirements Regulations of Special Economic Zones track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Economic Substance Requirements Regulations of Special Economic Zones")
