...
EDGES = [
    ...
    {
        "from_track_id": "environmental_service_providers",
        "relation": "repeals_full",
        "target_track_id": None,
        "target_description_ar": "اللائحة التنفيذية لمقدمي الخدمات البيئية لنظام البيئة (النسخة السابقة)",
        "target_decree": "Ministerial Decision No. (582979/1/1442), 14/11/1442H",
        "note": "environmental_service_providers (Ministerial Decision "
                "1515009/1, 3/7/1446H, published Umm Al-Qura issue 5063) "
                "wholly replaces its own prior founding Decision No. "
                "(582979/1/1442), 14/11/1442H, per its own verbatim clause "
                "(ثانياً): \"تحل هذه اللائحة محل اللائحة التنفيذية لمقدمي "
                "الخدمات البيئية لنظام البيئة، الصادرة بالقرار الوزاري رقم "
                "(582979/1/1442) بتاريخ 14/11/1442هـ\" -- a confirmed "
                "self-supersession of a genuinely distinct prior decision "
                "number, mirroring the law_practice_reg, "
                "environmental_inspection_audit, and "
                "environmental_violations_penalties precedents already "
                "modeled as real edges in this graph. Not separately "
                "ingested in this corpus (one-instrument-per-pass rule).",
    ...
    # --- repealed_by: the HELD track is the one repealed, and nothing replaces it -----
    {
        "from_track_id": "regional_tourism_development_councils_statute",
        "relation": "repealed_by",
        "target_track_id": None,
        "target_description_ar": "قرار مجلس الوزراء رقم (471) وتاريخ 10/7/1447هـ — إلغاء "
                                 "مجلس التنمية السياحي ومجالس التنمية السياحية في المناطق",
        "target_decree": "Council of Ministers Decision 471, 10/7/1447H "
                         "(published in Umm Al-Qura 9 January 2026)",
        "repeal_effective_ar": "ثانياً: إلغاء مجالس التنمية السياحية في المناطق المنشأة "
                               "بموجب المادة (الثانية) من تنظيمها الصادر بقرار مجلس الوزراء "
                               "رقم (520) وتاريخ 1443/9/11هـ، وإلغاء تنظيمها الصادر بالقرار "
                               "نفسه.",
        "note": "The repealing decision is quoted verbatim above from its own gazette page "
                "(uqn.gov.sa/details?p=28774). Identity of the repealed instrument rests on "
                "THREE independent points, none of which is the decree number the track "
                "itself never recorded: (1) the decision names «تنظيم مجالس التنمية "
                "السياحية في المناطق» word for word, which is this track's title; (2) it "
                "describes the councils as established by «المادة (الثانية) من تنظيمها», "
                "and article 2 of the text held here IS the establishing article — «ينشأ في "
                "كل منطقة مجلس للتنمية السياحية»; (3) it dates the repealed statute to "
                "Council of Ministers Decision 520 of 11/9/1443H, and this track was "
                "gazetted on 21/9/1443H — the ordinary ten-day lag between decision and "
                "publication. NOTHING REPLACES IT: the decision abolished the councils and "
                "created no successor, so there is no later text to read instead, which is "
                "why this is not `superseded_by`. HOW IT WAS FOUND: not by the currency "
                "audit, which classed the notice as a lead because its title names no "
                "instrument type — «إلغاء مجلس ...» names a BODY — but by regenerating the "
                "423 generators the QA gate's idempotence phase had never covered, one of "
                "which withdrew a stale warning and exposed the page behind it.",
        "source_ref": "sources/regional_tourism_development_councils_statute/official_source/"
                      "regional_tourism_development_councils_statute_official_source.json "
                      "(legal_status_ar, known_unresolved_discrepancies)",
    },
    {
...
AMBIGUOUS_OR_EXCLUDED_CASES = [
    ...
    {
        "tracks_involved": ["environmental_fees"],
        "issue": "confirmed negative repeal finding -- only a generic conflict-repeal clause, no named predecessor",
        "note": "environmental_fees (Minister of Environment Decision "
                "618660/1/1442, 05/12/1442H) carries only a generic "
                "conflict-repeal clause naming no specific prior instrument, "
                "and unlike three of its siblings (environmental_inspection_"
                "audit, environmental_violations_penalties, environmental_"
                "service_providers), no later wholesale re-issuance/amending "
                "decision was found -- qanoniah.com shows it ساري with no "
                "version history. Consistent with this being the first fees "
                "Implementing Regulation under the 2020 Environmental Law "
                "(M/165). No edge modeled.",
...
def build_graph(registry):
    ...
    for e in EDGES:
        ...
        if e["relation"] == "repealed_by":
            edge["repeal_effective_ar"] = e["repeal_effective_ar"]
    ...
    graph = {
        ...
        "relation_types": {
            ...
            "superseded_by": "track A is itself confirmed superseded by a "
                              "future/newer instrument.",
            "repealed_by": "track A is ITSELF repealed outright by a later "
                            "instrument that puts nothing in its place. Distinct "
                            "from `superseded_by`, where a successor exists and is "
                            "the text to read instead: here there is no successor, "
                            "so the track's own text is the last word on a rule that "
                            "no longer applies. Read `repeal_effective_ar` for the "
