#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة الترخيص البلدي لمزاولة المهن والحرف track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18607),
published 21/4/1443H (2021-11-26). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "municipal_professions_crafts_licensing_regulation", "official_source",
                   "municipal_professions_crafts_licensing_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "municipal_professions_crafts_licensing_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "municipal_professions_crafts_licensing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "municipal_professions_crafts_licensing_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "municipal_professions_crafts_licensing_regulation_arabic_legal_llm",
                        "municipal_professions_crafts_licensing_regulation_legal_llm_001_020.json")

LAW_ID = "sa-municipal-professions-crafts-licensing-regulation"
LAW_AR = "لائحة الترخيص البلدي لمزاولة المهن والحرف"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"municipal_professions_crafts_licensing_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18607), published 21/4/1443H (2021-11-26). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "municipal_professions_crafts_licensing_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MUNICIPAL_PROFESSIONS_CRAFTS_LICENSING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "municipal-professions-crafts-licensing-regulation-llm-%03d" % idx,
            ...
            "article_path": "municipal_professions_crafts_licensing_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Municipal Licensing Regulation for the Practice of Professions and Crafts -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/4/1443H / 2021-11-26)"),
                "source_authority_ar": ("لائحة الترخيص البلدي لمزاولة المهن والحرف — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 21/4/1443هـ الموافق 2021-11-26م)"),
    ...
    json.dump({
        "law_key": "municipal_professions_crafts_licensing_regulation",
        "layer": "MUNICIPAL_PROFESSIONS_CRAFTS_LICENSING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "municipal-professions-crafts-licensing-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Municipal Licensing Regulation for the Practice of Professions and Crafts — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Municipal Licensing Regulation for the Practice of Professions and Crafts records" % (len(ver), len(llm)))
