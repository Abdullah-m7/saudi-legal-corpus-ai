#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية للآثار والتراث العمراني track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19022),
...
STRUCTURE: 113 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "antiquities_heritage_regulation", "official_source",
                   "antiquities_heritage_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "antiquities_heritage_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "antiquities_heritage_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "antiquities_heritage_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "antiquities_heritage_regulation_arabic_legal_llm",
                        "antiquities_heritage_regulation_legal_llm_001_113.json")

LAW_ID = "sa-antiquities-heritage-regulation"
LAW_AR = "اللائحة التنفيذية للآثار والتراث العمراني"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"antiquities_heritage_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19022), published 24/7/1443H (2022-02-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "antiquities_heritage_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "ANTIQUITIES_HERITAGE_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "antiquities-heritage-regulation-llm-%03d" % idx,
            ...
            "article_path": "antiquities_heritage_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation for Antiquities and Urban Heritage -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/7/1443H / 2022-02-25)"),
                "source_authority_ar": ("اللائحة التنفيذية للآثار والتراث العمراني — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "antiquities_heritage_regulation",
        "layer": "ANTIQUITIES_HERITAGE_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "antiquities-heritage-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (113 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation for Antiquities and Urban Heritage — Arabic LLM-ready layer (113 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation for Antiquities and Urban Heritage records" % (len(ver), len(llm)))
