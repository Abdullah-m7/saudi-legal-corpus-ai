#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة تصحيح أوضاع مخالفي نظام مكافحة التستر track.

5 records: 5 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "anti_concealment_status_correction_regulation", "verified",
                       "anti_concealment_status_correction_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "anti_concealment_status_correction_regulation", "verified",
                       "anti_concealment_status_correction_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "anti_concealment_status_correction_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"anti_concealment_status_correction_regulation_no_decision_number_in_source", "anti_concealment_status_correction_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "21/7/1442":
        e.append("[2j] gazette_publication_date_hijri must be 21/7/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-03-05":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-03-05")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة تصحيح أوضاع مخالفي نظام مكافحة التستر":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation for Correcting the Status of Anti-Concealment Law Violators track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation for Correcting the Status of Anti-Concealment Law Violators")
    print("  - 5 records: 5 articles, no appendices; all 5 اصلية")
