#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مكتبة الملك فهد الوطنية track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/.../4000670),
published 15/10/1447H (2026-04-03). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "king_fahd_national_library_statute", "official_source",
                   "king_fahd_national_library_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "king_fahd_national_library_statute", "verified")
RECORDS = os.path.join(OUT_VER, "king_fahd_national_library_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "king_fahd_national_library_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "king_fahd_national_library_statute_arabic_legal_llm",
                        "king_fahd_national_library_statute_legal_llm_001_015.json")

LAW_ID = "sa-king-fahd-national-library-statute"
LAW_AR = "تنظيم مكتبة الملك فهد الوطنية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"king_fahd_national_library_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/.../4000670), published 15/10/1447H (2026-04-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "king_fahd_national_library_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "KING_FAHD_NATIONAL_LIBRARY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "king-fahd-national-library-statute-llm-%03d" % idx,
            ...
            "article_path": "king_fahd_national_library_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the King Fahd National Library -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/10/1447H / 2026-04-03)"),
                "source_authority_ar": ("تنظيم مكتبة الملك فهد الوطنية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 15/10/1447هـ الموافق 2026-04-03م)"),
    ...
    json.dump({
        "law_key": "king_fahd_national_library_statute",
        "layer": "KING_FAHD_NATIONAL_LIBRARY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "king-fahd-national-library-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Statute of the King Fahd National Library — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the King Fahd National Library records" % (len(ver), len(llm)))
