#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة لإصدار شهادات الاعتماد لجهات التحكم وجهات المعالجة track.

11 records: 15 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "pdpl_accreditation_certificates_rules", "verified",
                       "pdpl_accreditation_certificates_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "pdpl_accreditation_certificates_rules", "verified",
                       "pdpl_accreditation_certificates_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "pdpl_accreditation_certificates_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "pdpl_accreditation_certificates_rules_found_by_walking_a_url_section_the_index_never_covered",
    "pdpl_accreditation_certificates_rules_no_decision_number_in_source",
    "pdpl_accreditation_certificates_rules_sibling_rules_published_the_same_day",
    "pdpl_accreditation_certificates_rules_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد المنظمة لإصدار شهادات الاعتماد لجهات التحكم وجهات المعالجة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Issuing Accreditation Certificates to Controllers and Processors track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Issuing Accreditation Certificates to Controllers and Processors")
    print("  - 15 records: 15 articles, no appendices; all 15 اصلية")
