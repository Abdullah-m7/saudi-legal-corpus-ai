#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد التنظيمية لقياس حجم المنشأة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23822),
...
SRC = os.path.join(ROOT, "sources", "enterprise_size_measurement_rules", "official_source",
                   "enterprise_size_measurement_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "enterprise_size_measurement_rules", "verified")
RECORDS = os.path.join(OUT_VER, "enterprise_size_measurement_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "enterprise_size_measurement_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "enterprise_size_measurement_rules_arabic_legal_llm",
                        "enterprise_size_measurement_rules_legal_llm_001_013.json")

LAW_ID = "sa-enterprise-size-measurement-rules"
LAW_AR = "القواعد التنظيمية لقياس حجم المنشأة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"enterprise_size_measurement_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23822), published 21/3/1445H (2023-10-06). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "enterprise_size_measurement_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "ENTERPRISE_SIZE_MEASUREMENT_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "enterprise-size-measurement-rules-llm-%03d" % idx,
            ...
            "article_path": "enterprise_size_measurement_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for Measuring Enterprise Size -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/3/1445H / 2023-10-06)"),
                "source_authority_ar": ("القواعد التنظيمية لقياس حجم المنشأة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "enterprise_size_measurement_rules",
        "layer": "ENTERPRISE_SIZE_MEASUREMENT_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "enterprise-size-measurement-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for Measuring Enterprise Size — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for Measuring Enterprise Size records" % (len(ver), len(llm)))
