#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لرسوم الأراضي البيضاء track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27369),
...
SRC = os.path.join(ROOT, "sources", "white_land_fees_executive_regulation", "official_source",
                   "white_land_fees_executive_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "white_land_fees_executive_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "white_land_fees_executive_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "white_land_fees_executive_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "white_land_fees_executive_regulation_arabic_legal_llm",
                        "white_land_fees_executive_regulation_legal_llm_001_016.json")

LAW_ID = "sa-white-land-fees-executive-regulation"
LAW_AR = "اللائحة التنفيذية لرسوم الأراضي البيضاء"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"white_land_fees_executive_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27369), published 28/2/1447H (2025-08-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "white_land_fees_executive_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "WHITE_LAND_FEES_EXECUTIVE_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "white-land-fees-executive-regulation-llm-%03d" % idx,
            ...
            "article_path": "white_land_fees_executive_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the White Land Fees -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/2/1447H / 2025-08-22)"),
                "source_authority_ar": ("اللائحة التنفيذية لرسوم الأراضي البيضاء — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "white_land_fees_executive_regulation",
        "layer": "WHITE_LAND_FEES_EXECUTIVE_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "white-land-fees-executive-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the White Land Fees — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the White Land Fees records" % (len(ver), len(llm)))
