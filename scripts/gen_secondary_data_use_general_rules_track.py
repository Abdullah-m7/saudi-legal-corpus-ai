#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد العامة للاستخدام الثانوي للبيانات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28787),
published 27/7/1447H (2026-01-16). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "secondary_data_use_general_rules", "official_source",
                   "secondary_data_use_general_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "secondary_data_use_general_rules", "verified")
RECORDS = os.path.join(OUT_VER, "secondary_data_use_general_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "secondary_data_use_general_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "secondary_data_use_general_rules_arabic_legal_llm",
                        "secondary_data_use_general_rules_legal_llm_001_007.json")

LAW_ID = "sa-secondary-data-use-general-rules"
LAW_AR = "القواعد العامة للاستخدام الثانوي للبيانات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"secondary_data_use_general_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28787), published 27/7/1447H (2026-01-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "secondary_data_use_general_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SECONDARY_DATA_USE_GENERAL_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "secondary-data-use-general-rules-llm-%03d" % idx,
            ...
            "article_path": "secondary_data_use_general_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("General Rules for the Secondary Use of Data -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/7/1447H / 2026-01-16)"),
                "source_authority_ar": ("القواعد العامة للاستخدام الثانوي للبيانات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 27/7/1447هـ الموافق 2026-01-16م)"),
    ...
    json.dump({
        "law_key": "secondary_data_use_general_rules",
        "layer": "SECONDARY_DATA_USE_GENERAL_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "secondary-data-use-general-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (7 مادة، جميعها أصلية)",
        "title_en": "General Rules for the Secondary Use of Data — Arabic LLM-ready layer (7 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready General Rules for the Secondary Use of Data records" % (len(ver), len(llm)))
