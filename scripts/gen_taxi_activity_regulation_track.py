#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة المنظمة لنشاط الأجرة ووسيط الأجرة والتوجيه track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23377),
...
STRUCTURE: 55 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "taxi_activity_regulation", "official_source",
                   "taxi_activity_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "taxi_activity_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "taxi_activity_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "taxi_activity_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "taxi_activity_regulation_arabic_legal_llm",
                        "taxi_activity_regulation_legal_llm_001_055.json")

LAW_ID = "sa-taxi-activity-regulation"
LAW_AR = "اللائحة المنظمة لنشاط الأجرة ووسيط الأجرة والتوجيه"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"taxi_activity_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23377), published 10/1/1445H (2023-07-28). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "taxi_activity_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "TAXI_ACTIVITY_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "taxi-activity-regulation-llm-%03d" % idx,
            ...
            "article_path": "taxi_activity_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Governing Taxi, Taxi-Brokerage and Dispatch Activity -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 10/1/1445H / 2023-07-28)"),
                "source_authority_ar": ("اللائحة المنظمة لنشاط الأجرة ووسيط الأجرة والتوجيه — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "taxi_activity_regulation",
        "layer": "TAXI_ACTIVITY_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "taxi-activity-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (55 مادة، جميعها أصلية)",
        "title_en": "Regulation Governing Taxi, Taxi-Brokerage and Dispatch Activity — Arabic LLM-ready layer (55 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Governing Taxi, Taxi-Brokerage and Dispatch Activity records" % (len(ver), len(llm)))
