#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the معايير ترخيص المنصات العقارية الإلكترونية وتصنيفها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17737),
...
SRC = os.path.join(ROOT, "sources", "realestate_platform_licensing_criteria", "official_source",
                   "realestate_platform_licensing_criteria_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "realestate_platform_licensing_criteria", "verified")
RECORDS = os.path.join(OUT_VER, "realestate_platform_licensing_criteria_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "realestate_platform_licensing_criteria_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "realestate_platform_licensing_criteria_arabic_legal_llm",
                        "realestate_platform_licensing_criteria_legal_llm_001_010.json")

LAW_ID = "sa-realestate-platform-licensing-criteria"
LAW_AR = "معايير ترخيص المنصات العقارية الإلكترونية وتصنيفها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"realestate_platform_licensing_criteria_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17737), published 23/10/1442H (2021-06-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "realestate_platform_licensing_criteria",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "REALESTATE_PLATFORM_LICENSING_CRITERIA_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "realestate-platform-licensing-criteria-llm-%03d" % idx,
            ...
            "article_path": "realestate_platform_licensing_criteria/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Licensing and Classification Criteria for Electronic Real Estate Platforms -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/10/1442H / 2021-06-04)"),
                "source_authority_ar": ("معايير ترخيص المنصات العقارية الإلكترونية وتصنيفها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "realestate_platform_licensing_criteria",
        "layer": "REALESTATE_PLATFORM_LICENSING_CRITERIA_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "realestate-platform-licensing-criteria-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Licensing and Classification Criteria for Electronic Real Estate Platforms — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Licensing and Classification Criteria for Electronic Real Estate Platforms records" % (len(ver), len(llm)))
