#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة للصناديق الأهلية track.
...
RECORDS = os.path.join(ROOT, "sources", "community_funds_rules", "verified",
                       "community_funds_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "community_funds_rules", "verified",
                       "community_funds_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "community_funds_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"community_funds_rules_no_decision_number_in_source", "community_funds_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد المنظمة للصناديق الأهلية":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing Community Funds track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing Community Funds")
