#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Bankruptcy Law track (231 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) EXCEPT the 6 visually-adjudicated list/reference articles (39,
94, 145, 158, 196, 230) confirmed verbatim on the rendered pages; the numbered
sequence is a complete 1..231 (no مكرر); and every article carries an explained
...
dual-status divergence). Exactly two articles (46, 147) are معدلة, each carrying
its amendment history; none are repealed or added. Tatweel is banned EXCEPT the
...
RECORDS = os.path.join(ROOT, "sources", "bankruptcy", "law", "verified",
                       "bankruptcy_law_verified_records.jsonl")
...
PDF = os.path.join(ROOT, "inputs", "bankruptcy_official_pdfs",
                   "bankruptcy_law_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 231
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 229, "معدلة": 2}
VISUALLY_ADJUDICATED = {"bankruptcy_art_%03d" % n for n in (39, 94, 145, 158, 196, 230)}
EXPECTED_AMENDED = {46, 147}
...
def main():
    ...
    # [1] structure: complete 1..231, no mukarrar
    nums = sorted(int(re.match(r"bankruptcy_art_(\d{3})", k).group(1)) for k in arts
    ...
    for k, a in arts.items():
        ...
        sc[ls] += 1
        if ls == "معدلة":
            amended_nums.add(int(re.match(r"bankruptcy_art_(\d{3})", k).group(1)))
    ...
    if e:
        print("FAIL: %d error(s) in Bankruptcy Law track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Bankruptcy Law — 231 records (consolidated: 229 اصلية / 2 معدلة)")
    print("  - trust gate: 225/231 MATCHES_PDF; 6 list/reference articles (39,94,145,158,196,230) visually adjudicated verbatim")
    print("  - complete 1..231 (no مكرر); committed official MOJ PDF hash verified; no dual-status divergence")
    print("  - 2 amended articles (46, 147) carry history; per art 230 the law repeals old commercial-court/settlement provisions")
