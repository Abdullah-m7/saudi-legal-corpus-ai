#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة الأحكام المنظمة لإنشاء وتشغيل مدارس لتعليم قيادة المركبات ومزاولة مهنة تعليم القيادة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18426),
...
SRC = os.path.join(ROOT, "sources", "driving_schools_regulation", "official_source",
                   "driving_schools_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "driving_schools_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "driving_schools_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "driving_schools_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "driving_schools_regulation_arabic_legal_llm",
                        "driving_schools_regulation_legal_llm_001_023.json")

LAW_ID = "sa-driving-schools-regulation"
LAW_AR = "لائحة الأحكام المنظمة لإنشاء وتشغيل مدارس لتعليم قيادة المركبات ومزاولة مهنة تعليم القيادة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"driving_schools_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18426), published 9/3/1443H (2021-10-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "driving_schools_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "DRIVING_SCHOOLS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "driving-schools-regulation-llm-%03d" % idx,
            ...
            "article_path": "driving_schools_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Governing the Establishment and Operation of Vehicle Driving Schools -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/3/1443H / 2021-10-15)"),
                "source_authority_ar": ("لائحة الأحكام المنظمة لإنشاء وتشغيل مدارس لتعليم قيادة المركبات ومزاولة مهنة تعليم القيادة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "driving_schools_regulation",
        "layer": "DRIVING_SCHOOLS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "driving-schools-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (23 مادة، جميعها أصلية)",
        "title_en": "Regulation Governing the Establishment and Operation of Vehicle Driving Schools — Arabic LLM-ready layer (23 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Governing the Establishment and Operation of Vehicle Driving Schools records" % (len(ver), len(llm)))
