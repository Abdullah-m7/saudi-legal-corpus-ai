#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط شراء واستئجار الجهات الحكومية للمركبات track.
...
RECORDS = os.path.join(ROOT, "sources", "government_vehicle_purchase_lease_controls", "verified",
                       "government_vehicle_purchase_lease_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "government_vehicle_purchase_lease_controls", "verified",
                       "government_vehicle_purchase_lease_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "government_vehicle_purchase_lease_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"government_vehicle_purchase_lease_controls_no_decision_number_in_source", "government_vehicle_purchase_lease_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "22/8/1446":
        e.append("[2j] gazette_publication_date_hijri must be 22/8/1446")
    if src.get("gazette_publication_date_gregorian") != "2025-02-21":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-02-21")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط شراء واستئجار الجهات الحكومية للمركبات":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on the Purchase and Lease of Vehicles by Government Bodies track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on the Purchase and Lease of Vehicles by Government Bodies")
