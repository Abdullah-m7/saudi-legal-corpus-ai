#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنظام الحرف والصناعات اليدوية track.
...
RECORDS = os.path.join(ROOT, "sources", "handicrafts_regulation", "verified",
                       "handicrafts_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "handicrafts_regulation", "verified",
                       "handicrafts_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "handicrafts_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"handicrafts_regulation_article_one_heading_absent_in_source", "handicrafts_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية لنظام الحرف والصناعات اليدوية":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation of the Handicrafts and Handmade Industries Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation of the Handicrafts and Handmade Industries Law")
