#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية بين حكومة المملكة والصندوق الدولي للتنمية الزراعية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17489),
...
STRUCTURE: 17 numbered articles, no appendix records. All اصلية. The source
heads them «المادة الأولى» with no colon and no dash; see the official_source
artifact for the four conditions that had to hold before they were read.
...
SRC = os.path.join(ROOT, "sources", "treaty_ifad_headquarters", "official_source",
                   "treaty_ifad_headquarters_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_ifad_headquarters", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_ifad_headquarters_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_ifad_headquarters_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_ifad_headquarters_arabic_legal_llm",
                        "treaty_ifad_headquarters_legal_llm_001_017.json")

LAW_ID = "sa-treaty-ifad-headquarters"
LAW_AR = "اتفاقية بين حكومة المملكة والصندوق الدولي للتنمية الزراعية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_ifad_headquarters_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17489), published 11/9/1442H (2021-04-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_ifad_headquarters",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_IFAD_HEADQUARTERS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-ifad-headquarters-llm-%03d" % idx,
            ...
            "article_path": "treaty_ifad_headquarters/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Agreement between the Government of Saudi Arabia and the International Fund for Agricultural Development -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 11/9/1442H / 2021-04-23)"),
                "source_authority_ar": ("اتفاقية بين حكومة المملكة والصندوق الدولي للتنمية الزراعية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_ifad_headquarters",
        "layer": "TREATY_IFAD_HEADQUARTERS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-ifad-headquarters-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Agreement between the Government of Saudi Arabia and the International Fund for Agricultural Development — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Agreement between the Government of Saudi Arabia and the International Fund for Agricultural Development records" % (len(ver), len(llm)))
