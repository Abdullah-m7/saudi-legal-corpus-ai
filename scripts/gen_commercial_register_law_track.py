#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the Commercial Register Law track (نظام السجل التجاري م/83).
...
independent-date snapshots (2025-01 + 2025-04): all 29 article bodies are
...
Royal Decree M/83 dated 19/3/1446H. IN FORCE (ساري). Fresh full issuance: all 29
اصلية; supersedes the former Commercial Register Law (Royal Decree M/1, 1416H).
...
SRC = os.path.join(ROOT, "sources", "commercial_register", "law", "official_source",
                   "commercial_register_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "commercial_register", "law", "verified")
RECORDS = os.path.join(OUT_VER, "commercial_register_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "commercial_register_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "commercial_register_arabic_legal_llm",
                        "commercial_register_law_legal_llm_001_029.json")

LAW_ID = "sa-commercial-register-law-m83-1446"
LAW_AR = "نظام السجل التجاري"
CORPUS = "commercial_register"
STATUS = "BOE_OFFICIAL_PORTAL_ARCHIVE_CROSS_SNAPSHOT_VERIFIED"
KEY_RE = r"commercial_register_art_(\d{3})(_mukarrar)?$"
LLM_FILE_BASENAME = "commercial_register_law_legal_llm_001_029.json"
N_ARTICLES = 29
RECID_PREFIX = "commercial-register-law-llm-art"
SEARCH_TERM = "نظام السجل التجاري"
...
def main():
    ...
    for key in keys:
        ...
        ver.append({"law_key": CORPUS, "law_component": "law", "language": "ar",
                    "record_layer": "COMMERCIAL_REGISTER_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({"law_key": CORPUS, "layer": "COMMERCIAL_REGISTER_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({"layer_id": "sa-commercial-register-law-arabic-legal-llm-full", "law_id": LAW_ID,
               "law_component": "law",
               "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (29 مادة؛ إصدار كامل: 29 أصلية)",
               "title_en": "Saudi Commercial Register Law — Arabic LLM-ready layer (29 records)",
               "record_type": "verified_arabic_article", "language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Commercial Register Law records" % (len(ver), len(llm)))
