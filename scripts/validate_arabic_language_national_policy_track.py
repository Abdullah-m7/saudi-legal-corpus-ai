#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the السياسة الوطنية للغة العربية في السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "arabic_language_national_policy", "verified",
                       "arabic_language_national_policy_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "arabic_language_national_policy", "verified",
                       "arabic_language_national_policy_verified_summary.json")
LLM = os.path.join(ROOT, "data", "arabic_language_national_policy_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"arabic_language_national_policy_no_decision_number_in_source", "arabic_language_national_policy_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "السياسة الوطنية للغة العربية في السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in National Policy for the Arabic Language in Saudi Arabia track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: National Policy for the Arabic Language in Saudi Arabia")
