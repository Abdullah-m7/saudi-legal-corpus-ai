#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Commercial Register Law track (29 records).

Trust gate (BOE-archive provenance): the numbered sequence is a complete 1..29
(no مكرر); every article is اصلية (fresh full issuance by Royal Decree M/83,
1446H) consistent with its is_repealed/is_amended/is_added flags (all False); the
...
RECORDS = os.path.join(ROOT, "sources", "commercial_register", "law", "verified",
                       "commercial_register_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "commercial_register_arabic_legal_llm",
                   "commercial_register_law_legal_llm_001_029.json")
STATUS = "BOE_OFFICIAL_PORTAL_ARCHIVE_CROSS_SNAPSHOT_VERIFIED"
N = 29
KEY_RE = r"commercial_register_art_(\d{3})"
LAW_NAME = "Commercial Register Law"
...
def main():
    ...
    print("    (byte-identical) + corpus text re-hash to recorded sha256; Royal Decree M/83 (1446H); ساري")
