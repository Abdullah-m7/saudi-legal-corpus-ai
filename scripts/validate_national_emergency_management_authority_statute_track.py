#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم الهيئة الوطنية لإدارة الطوارئ track.
...
RECORDS = os.path.join(ROOT, "sources", "national_emergency_management_authority_statute", "verified",
                       "national_emergency_management_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "national_emergency_management_authority_statute", "verified",
                       "national_emergency_management_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "national_emergency_management_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"national_emergency_management_authority_statute_no_decision_number_in_source", "national_emergency_management_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/4/1446":
        e.append("[2j] gazette_publication_date_hijri must be 24/4/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-10-27":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-10-27")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم الهيئة الوطنية لإدارة الطوارئ":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the National Authority for Emergency Management track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the National Authority for Emergency Management")
