#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام جمع التبرعات track.
...
RECORDS = os.path.join(ROOT, "sources", "donations_collection_law", "verified",
                       "donations_collection_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "donations_collection_law", "verified",
                       "donations_collection_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "donations_collection_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"donations_collection_law_no_decision_number_in_source", "donations_collection_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام جمع التبرعات":
    ...
    if e:
        print("FAIL: %d error(s) in Law on the Collection of Donations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law on the Collection of Donations")
