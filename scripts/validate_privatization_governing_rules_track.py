#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة للتخصيص track.
...
RECORDS = os.path.join(ROOT, "sources", "privatization_governing_rules", "verified",
                       "privatization_governing_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "privatization_governing_rules", "verified",
                       "privatization_governing_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "privatization_governing_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"privatization_governing_rules_no_decision_number_in_source", "privatization_governing_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد المنظمة للتخصيص":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing Privatization track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing Privatization")
