#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد الضريبية للمقرات الإقليمية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24526),
...
SRC = os.path.join(ROOT, "sources", "regional_headquarters_tax_rules", "official_source",
                   "regional_headquarters_tax_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "regional_headquarters_tax_rules", "verified")
RECORDS = os.path.join(OUT_VER, "regional_headquarters_tax_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "regional_headquarters_tax_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "regional_headquarters_tax_rules_arabic_legal_llm",
                        "regional_headquarters_tax_rules_legal_llm_001_016.json")

LAW_ID = "sa-regional-headquarters-tax-rules"
LAW_AR = "القواعد الضريبية للمقرات الإقليمية"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24526), published 6/8/1445H (2024-02-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "regional_headquarters_tax_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "REGIONAL_HEADQUARTERS_TAX_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "regional-headquarters-tax-rules-llm-%03d" % idx,
            ...
            "article_path": "regional_headquarters_tax_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Tax Rules for Regional Headquarters -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/8/1445H / 2024-02-16)"),
                "source_authority_ar": ("القواعد الضريبية للمقرات الإقليمية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "regional_headquarters_tax_rules",
        "layer": "REGIONAL_HEADQUARTERS_TAX_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "regional-headquarters-tax-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Tax Rules for Regional Headquarters — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Tax Rules for Regional Headquarters records" % (len(ver), len(llm)))
