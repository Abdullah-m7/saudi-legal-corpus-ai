#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة التراخيص - هيئة المتاحف track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28570),
...
SRC = os.path.join(ROOT, "sources", "museums_authority_licensing_regulation", "official_source",
                   "museums_authority_licensing_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "museums_authority_licensing_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "museums_authority_licensing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "museums_authority_licensing_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "museums_authority_licensing_regulation_arabic_legal_llm",
                        "museums_authority_licensing_regulation_legal_llm_001_023.json")

LAW_ID = "sa-museums-authority-licensing-regulation"
LAW_AR = "لائحة التراخيص - هيئة المتاحف"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"museums_authority_licensing_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28570), published 9/5/1447H (2025-10-31). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "museums_authority_licensing_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MUSEUMS_AUTHORITY_LICENSING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "museums-authority-licensing-regulation-llm-%03d" % idx,
            ...
            "article_path": "museums_authority_licensing_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Licensing Regulation — Museums Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/5/1447H / 2025-10-31)"),
                "source_authority_ar": ("لائحة التراخيص - هيئة المتاحف — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "museums_authority_licensing_regulation",
        "layer": "MUSEUMS_AUTHORITY_LICENSING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "museums-authority-licensing-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (23 مادة، جميعها أصلية)",
        "title_en": "Licensing Regulation — Museums Authority — Arabic LLM-ready layer (23 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Licensing Regulation — Museums Authority records" % (len(ver), len(llm)))
