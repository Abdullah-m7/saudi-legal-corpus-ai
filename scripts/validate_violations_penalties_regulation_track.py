#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة ضبط المخالفات وإيقاع العقوبات track.
...
RECORDS = os.path.join(ROOT, "sources", "violations_penalties_regulation", "verified",
                       "violations_penalties_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "violations_penalties_regulation", "verified",
                       "violations_penalties_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "violations_penalties_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"violations_penalties_regulation_no_decision_number_in_source", "violations_penalties_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة ضبط المخالفات وإيقاع العقوبات":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on Recording Violations and Imposing Penalties track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on Recording Violations and Imposing Penalties")
