#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام استخدام كاميرات المراقبة الأمنية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23050),
...
SRC = os.path.join(ROOT, "sources", "security_cameras_law_regulation", "official_source",
                   "security_cameras_law_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "security_cameras_law_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "security_cameras_law_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "security_cameras_law_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "security_cameras_law_regulation_arabic_legal_llm",
                        "security_cameras_law_regulation_legal_llm_001_020.json")

LAW_ID = "sa-security-cameras-law-regulation"
LAW_AR = "اللائحة التنفيذية لنظام استخدام كاميرات المراقبة الأمنية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"security_cameras_law_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23050), published 20/11/1444H (2023-06-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "security_cameras_law_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SECURITY_CAMERAS_LAW_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "security-cameras-law-regulation-llm-%03d" % idx,
            ...
            "article_path": "security_cameras_law_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Law on the Use of Security Surveillance Cameras -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/11/1444H / 2023-06-09)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام استخدام كاميرات المراقبة الأمنية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "security_cameras_law_regulation",
        "layer": "SECURITY_CAMERAS_LAW_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "security-cameras-law-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Law on the Use of Security Surveillance Cameras — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Law on the Use of Security Surveillance Cameras records" % (len(ver), len(llm)))
