#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد وآلية عمل لجان النظر في طلبات تملك العقارات track.
...
RECORDS = os.path.join(ROOT, "sources", "property_ownership_committees_rules", "verified",
                       "property_ownership_committees_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "property_ownership_committees_rules", "verified",
                       "property_ownership_committees_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "property_ownership_committees_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"property_ownership_committees_rules_no_decision_number_in_source", "property_ownership_committees_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "12/10/1443":
        e.append("[2j] gazette_publication_date_hijri must be 12/10/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-05-13":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-05-13")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد وآلية عمل لجان النظر في طلبات تملك العقارات":
    ...
    if e:
        print("FAIL: %d error(s) in Rules and Working Mechanism of the Committees Reviewing Property Ownership Applications track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules and Working Mechanism of the Committees Reviewing Property Ownership Applications")
