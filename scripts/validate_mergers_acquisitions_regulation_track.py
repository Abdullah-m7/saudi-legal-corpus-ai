#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الاندماج والاستحواذ track.
...
RECORDS = os.path.join(ROOT, "sources", "mergers_acquisitions_regulation", "verified",
                       "mergers_acquisitions_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "mergers_acquisitions_regulation", "verified",
                       "mergers_acquisitions_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "mergers_acquisitions_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"mergers_acquisitions_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة الاندماج والاستحواذ":
    ...
    if e:
        print("FAIL: %d error(s) in Merger and Acquisition Regulations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Merger and Acquisition Regulations")
