#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الإطار التنظيمي للتسجيل البيني للمنتجات المالية بين الجهات المنظمة للأسواق المالية بدول المجلس track.
...
RECORDS = os.path.join(ROOT, "sources", "gcc_financial_products_cross_registration_framework", "verified",
                       "gcc_financial_products_cross_registration_framework_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "gcc_financial_products_cross_registration_framework", "verified",
                       "gcc_financial_products_cross_registration_framework_verified_summary.json")
LLM = os.path.join(ROOT, "data", "gcc_financial_products_cross_registration_framework_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"gcc_financial_products_cross_registration_framework_no_decision_number_in_source", "gcc_financial_products_cross_registration_framework_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الإطار التنظيمي للتسجيل البيني للمنتجات المالية بين الجهات المنظمة للأسواق المالية بدول المجلس":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Framework for Cross-Registration of Financial Products among GCC Market Regulators track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Framework for Cross-Registration of Financial Products among GCC Market Regulators")
