#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate the Batch P2-002 QA (second P2 expansion QA; article-by-article vs the official Arabic).

Confirms the QA covers exactly the 20 P2-002 articles across Babs 4/5/6/7, that every per-article bab is
in [4,5,6,7] and equals both the P2-002 remediation record bab and the coverage-index expected_bab_number,
...
SRC_REL = "data/chinese_remediation_batches/p2_002/companies_law_m132_1443_zh_internal_remediation_p2_002.json"
...
# P0-002..P0-005 QA, the P1-001..P1-004 QA, and the P2-001 QA are all QA_PASS and must remain untouched.
...
# P1 remediation batches: (dir, file, expected record count)
P1_BATCHES = {
...
# P2-001 remediation batch must remain intact.
P2_001_BATCH = ("p2_001", "companies_law_m132_1443_zh_internal_remediation_p2_001.json", 20)

ARTS = [60, 66, 75, 113, 115, 133, 139, 142, 150, 151, 152, 153, 155, 158, 162, 173, 181, 182, 184, 186]
DIST = {"4": [60, 66, 75, 113, 115, 133],
...
REQ_TOP = ("stage", "qa_stage", "batch_id", "source_batch_file", "scope_articles", "expected_babs",
           ...
           "trilingual_alignment_created", "p2_002_chinese_text_modified", "p2_002_data_modified",
...
def main(argv=None) -> int:
    ...
    problems = []
    for p, label in ((qa_path, "QA JSON"), (MD, "Arabic QA report"), (SRC, "P2-002 remediation file")):
    ...
    if qa.get("stage") != "CHINESE_REMEDIATION_BATCH_P2_002_QA":
        problems.append("stage must be CHINESE_REMEDIATION_BATCH_P2_002_QA")
    if qa.get("qa_stage") != "CHINESE_REMEDIATION_BATCH_P2_002_QA":
        problems.append("qa_stage must be CHINESE_REMEDIATION_BATCH_P2_002_QA")
    if qa.get("batch_id") != "P2-002":
        problems.append("batch_id must be P2-002")
    if qa.get("source_batch_file") != SRC_REL:
        problems.append("source_batch_file must point to the P2-002 remediation JSON")
    if qa.get("scope_articles") != ARTS:
        problems.append("scope_articles must exactly match the P2-002 list")
    if qa.get("article_count") != 20:
        problems.append("article_count must be 20")
    if qa.get("expected_babs") != [4, 5, 6, 7]:
        problems.append("expected_babs must be [4, 5, 6, 7]")
    dist = qa.get("expected_bab_distribution") or {}
    if {str(k): list(v) for k, v in dist.items()} != DIST:
        problems.append("expected_bab_distribution must match the authorized Bab 4/5/6/7 split")
    ...
    if nums != ARTS:
        problems.append("per_article_reviews must cover exactly the 20 P2-002 articles in order")
    ...
    for r in recs:
        ...
        if r.get("bab") not in BABS:
            problems.append("art %s bab must be in [4,5,6,7]" % n)
        if n in srec and r.get("bab") != srec[n].get("bab"):
            problems.append("art %s QA bab %r != P2-002 record bab %r" % (n, r.get("bab"), srec[n].get("bab")))
    ...
    if qa.get("p2_002_chinese_text_modified") is True or qa.get("p2_002_data_modified") is True:
        if not fixes:
            problems.append("p2_002 marked modified but no minor_fixes recorded")
    else:
        if fixes:
            problems.append("minor_fixes recorded but p2_002 marked unmodified")
    ...
    # P2-002 remediation intact; P2-001 + all P1 remediation + all P0 batches + QA unchanged
    if len(src["records"]) != 20:
        problems.append("P2-002 remediation must remain 20 records")
    sub, fn, cnt = P2_001_BATCH
    p2p = os.path.join(ROOT, "data", "chinese_remediation_batches", sub, fn)
    if not os.path.exists(p2p) or len(_read(p2p)["records"]) != cnt:
        problems.append("P2-001 remediation must remain %d records (untouched)" % cnt)
    ...
    print("=" * 60)
    print("Chinese remediation Batch P2-002 QA validation (second P2 expansion QA)")
    print("=" * 60)
    if problems:
        for p in problems:
            print("  -", p)
        print("RESULT: %d problem(s) found ✗" % len(problems))
        return 1
    print("[PASS] Batch P2-002 QA: 20 articles across Babs [4,5,6,7] reviewed article-by-article vs the "
          "official Arabic (English secondary; prior condensed candidate as the expansion baseline); "
          "each QA bab matches the P2-002 record and the coverage index; for every pass, source "
          ...
          "Chinese text embedded; review-only (P2-002 data unmodified); P2-001 data + QA, all P1 data + "
          "QA and all P0 batches + QA + Chinese candidate 189 + old Chinese 5/23 + Arabic/English/"
          "English-reference 281 + Arabic source + Chinese sources 14 + OCR queue unchanged; no other "
          "P2/P3 dirs; no full-281 / trilingual.")
