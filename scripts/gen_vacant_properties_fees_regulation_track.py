#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لرسوم العقارات الشاغرة track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4000950),
...
SRC = os.path.join(ROOT, "sources", "vacant_properties_fees_regulation", "official_source",
                   "vacant_properties_fees_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "vacant_properties_fees_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "vacant_properties_fees_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "vacant_properties_fees_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "vacant_properties_fees_regulation_arabic_legal_llm",
                        "vacant_properties_fees_regulation_legal_llm_001_018.json")

LAW_ID = "sa-vacant-properties-fees-regulation"
LAW_AR = "اللائحة التنفيذية لرسوم العقارات الشاغرة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"vacant_properties_fees_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4000950), published 28/11/1447H (2026-05-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "vacant_properties_fees_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "VACANT_PROPERTIES_FEES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "vacant-properties-fees-regulation-llm-%03d" % idx,
            ...
            "article_path": "vacant_properties_fees_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation for Vacant Properties Fees -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/11/1447H / 2026-05-15)"),
                "source_authority_ar": ("اللائحة التنفيذية لرسوم العقارات الشاغرة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "vacant_properties_fees_regulation",
        "layer": "VACANT_PROPERTIES_FEES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "vacant-properties-fees-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation for Vacant Properties Fees — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation for Vacant Properties Fees records" % (len(ver), len(llm)))
