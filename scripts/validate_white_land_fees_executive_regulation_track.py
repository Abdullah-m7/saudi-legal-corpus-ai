#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لرسوم الأراضي البيضاء track.
...
RECORDS = os.path.join(ROOT, "sources", "white_land_fees_executive_regulation", "verified",
                       "white_land_fees_executive_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "white_land_fees_executive_regulation", "verified",
                       "white_land_fees_executive_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "white_land_fees_executive_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"white_land_fees_executive_regulation_no_decision_number_in_source", "white_land_fees_executive_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية لرسوم الأراضي البيضاء":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation of the White Land Fees track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation of the White Land Fees")
