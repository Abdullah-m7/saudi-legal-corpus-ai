#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط إعفاء البضائع المعادة من الرسوم الجمركية track.
...
RECORDS = os.path.join(ROOT, "sources", "returned_goods_customs_exemption_controls", "verified",
                       "returned_goods_customs_exemption_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "returned_goods_customs_exemption_controls", "verified",
                       "returned_goods_customs_exemption_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "returned_goods_customs_exemption_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"returned_goods_customs_exemption_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "29/12/1445":
        e.append("[2j] gazette_publication_date_hijri must be 29/12/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-07-05":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-07-05")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط إعفاء البضائع المعادة من الرسوم الجمركية":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on the Exemption of Returned Goods from Customs Duties track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on the Exemption of Returned Goods from Customs Duties")
