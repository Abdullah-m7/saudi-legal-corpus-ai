#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لأكاديمية مهد الرياضية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19893),
...
SRC = os.path.join(ROOT, "sources", "mahd_sports_academy_statute", "official_source",
                   "mahd_sports_academy_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "mahd_sports_academy_statute", "verified")
RECORDS = os.path.join(OUT_VER, "mahd_sports_academy_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "mahd_sports_academy_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "mahd_sports_academy_statute_arabic_legal_llm",
                        "mahd_sports_academy_statute_legal_llm_001_013.json")

LAW_ID = "sa-mahd-sports-academy-statute"
LAW_AR = "الترتيبات التنظيمية لأكاديمية مهد الرياضية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"mahd_sports_academy_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19893), published 14/1/1444H (2022-08-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "mahd_sports_academy_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "MAHD_SPORTS_ACADEMY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "mahd-sports-academy-statute-llm-%03d" % idx,
            ...
            "article_path": "mahd_sports_academy_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Mahd Sports Academy — Organisational Arrangements -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/1/1444H / 2022-08-12)"),
                "source_authority_ar": ("الترتيبات التنظيمية لأكاديمية مهد الرياضية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "mahd_sports_academy_statute",
        "layer": "MAHD_SPORTS_ACADEMY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "mahd-sports-academy-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Mahd Sports Academy — Organisational Arrangements — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Mahd Sports Academy — Organisational Arrangements records" % (len(ver), len(llm)))
