#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية تعاون في مجال الطاقة بين المملكة وأوزبكستان track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_uzbekistan_energy_cooperation", "verified",
                       "treaty_uzbekistan_energy_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_uzbekistan_energy_cooperation", "verified",
                       "treaty_uzbekistan_energy_cooperation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_uzbekistan_energy_cooperation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_uzbekistan_energy_cooperation_no_decision_number_in_source", "treaty_uzbekistan_energy_cooperation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية تعاون في مجال الطاقة بين المملكة وأوزبكستان":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Uzbekistan Energy Cooperation Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Uzbekistan Energy Cooperation Agreement")
