#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مركز سارة السديري لدراسات المرأة track.
...
RECORDS = os.path.join(ROOT, "sources", "sarah_sudairi_womens_studies_center_statute", "verified",
                       "sarah_sudairi_womens_studies_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "sarah_sudairi_womens_studies_center_statute", "verified",
                       "sarah_sudairi_womens_studies_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "sarah_sudairi_womens_studies_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"sarah_sudairi_womens_studies_center_statute_no_decision_number_in_source", "sarah_sudairi_womens_studies_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "2/9/1444":
        e.append("[2j] gazette_publication_date_hijri must be 2/9/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-03-24":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-03-24")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم مركز سارة السديري لدراسات المرأة":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Sarah Al-Sudairi Center for Women's Studies track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Sarah Al-Sudairi Center for Women's Studies")
