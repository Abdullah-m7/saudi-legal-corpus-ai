#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنظام تصنيف المقاولين track.
...
RECORDS = os.path.join(ROOT, "sources", "contractors_classification_regulation", "verified",
                       "contractors_classification_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "contractors_classification_regulation", "verified",
                       "contractors_classification_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "contractors_classification_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"contractors_classification_regulation_no_decision_number_in_source", "contractors_classification_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية لنظام تصنيف المقاولين":
    ...
    if e:
        print("FAIL: %d error(s) in Implementing Regulation of the Contractors Classification Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Implementing Regulation of the Contractors Classification Law")
