#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد وإجراءات عمل لجان تسوية الخلافات والمنازعات الزكوية والضريبية track.
...
RECORDS = os.path.join(ROOT, "sources", "zakat_tax_dispute_settlement_committees_rules", "verified",
                       "zakat_tax_dispute_settlement_committees_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "zakat_tax_dispute_settlement_committees_rules", "verified",
                       "zakat_tax_dispute_settlement_committees_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "zakat_tax_dispute_settlement_committees_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"zakat_tax_dispute_settlement_committees_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "16/11/1445":
        e.append("[2j] gazette_publication_date_hijri must be 16/11/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-05-24":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-05-24")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد وإجراءات عمل لجان تسوية الخلافات والمنازعات الزكوية والضريبية":
    ...
    if e:
        print("FAIL: %d error(s) in Rules and Procedures of the Zakat and Tax Dispute and Disagreement Settlement Committees track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules and Procedures of the Zakat and Tax Dispute and Disagreement Settlement Committees")
