#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لتنمية الغطاء النباتي ومكافحة التصحر track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17863),
...
SRC = os.path.join(ROOT, "sources", "vegetation_cover_desertification_regulation", "official_source",
                   "vegetation_cover_desertification_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "vegetation_cover_desertification_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "vegetation_cover_desertification_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "vegetation_cover_desertification_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "vegetation_cover_desertification_regulation_arabic_legal_llm",
                        "vegetation_cover_desertification_regulation_legal_llm_001_011.json")

LAW_ID = "sa-vegetation-cover-desertification-regulation"
LAW_AR = "اللائحة التنفيذية لتنمية الغطاء النباتي ومكافحة التصحر"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"vegetation_cover_desertification_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17863), published 15/11/1442H (2021-06-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "vegetation_cover_desertification_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "VEGETATION_COVER_DESERTIFICATION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "vegetation-cover-desertification-regulation-llm-%03d" % idx,
            ...
            "article_path": "vegetation_cover_desertification_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation for the Development of Vegetation Cover and Combating Desertification -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/11/1442H / 2021-06-25)"),
                "source_authority_ar": ("اللائحة التنفيذية لتنمية الغطاء النباتي ومكافحة التصحر — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "vegetation_cover_desertification_regulation",
        "layer": "VEGETATION_COVER_DESERTIFICATION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "vegetation-cover-desertification-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation for the Development of Vegetation Cover and Combating Desertification — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation for the Development of Vegetation Cover and Combating Desertification records" % (len(ver), len(llm)))
