#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم هيئة الأدب والنشر والترجمة track.
...
RECORDS = os.path.join(ROOT, "sources", "literature_publishing_translation_authority_statute", "verified",
                       "literature_publishing_translation_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "literature_publishing_translation_authority_statute", "verified",
                       "literature_publishing_translation_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "literature_publishing_translation_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"literature_publishing_translation_authority_statute_no_decision_number_in_source", "literature_publishing_translation_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "9/12/1445":
        e.append("[2j] gazette_publication_date_hijri must be 9/12/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-06-15":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-06-15")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم هيئة الأدب والنشر والترجمة":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Literature, Publishing and Translation Commission track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Literature, Publishing and Translation Commission")
