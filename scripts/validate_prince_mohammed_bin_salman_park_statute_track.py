#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مؤسسة حديقة الأمير محمد بن سلمان track.
...
RECORDS = os.path.join(ROOT, "sources", "prince_mohammed_bin_salman_park_statute", "verified",
                       "prince_mohammed_bin_salman_park_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "prince_mohammed_bin_salman_park_statute", "verified",
                       "prince_mohammed_bin_salman_park_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "prince_mohammed_bin_salman_park_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"prince_mohammed_bin_salman_park_statute_no_decision_number_in_source", "prince_mohammed_bin_salman_park_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "29/10/1444":
        e.append("[2j] gazette_publication_date_hijri must be 29/10/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-05-19":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-05-19")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم مؤسسة حديقة الأمير محمد بن سلمان":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Prince Mohammed bin Salman Park Foundation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Prince Mohammed bin Salman Park Foundation")
