#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المركز الوطني للصقور track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4000954),
...
SRC = os.path.join(ROOT, "sources", "falcon_center_statute", "official_source",
                   "falcon_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "falcon_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "falcon_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "falcon_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "falcon_center_statute_arabic_legal_llm",
                        "falcon_center_statute_legal_llm_001_017.json")

LAW_ID = "sa-falcon-center-statute"
LAW_AR = "تنظيم المركز الوطني للصقور"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"falcon_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4000954), published 28/11/1447H (2026-05-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "falcon_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "FALCON_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "falcon-center-statute-llm-%03d" % idx,
            ...
            "article_path": "falcon_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Falcon Center -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/11/1447H / 2026-05-15)"),
                "source_authority_ar": ("تنظيم المركز الوطني للصقور — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "falcon_center_statute",
        "layer": "FALCON_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "falcon-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Falcon Center — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Falcon Center records" % (len(ver), len(llm)))
