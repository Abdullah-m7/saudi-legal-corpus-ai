#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية مقر بين حكومة المملكة واتحاد النقل الجوي الدولي (إياتا) track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18664),
...
SRC = os.path.join(ROOT, "sources", "treaty_iata_headquarters", "official_source",
                   "treaty_iata_headquarters_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_iata_headquarters", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_iata_headquarters_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_iata_headquarters_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_iata_headquarters_arabic_legal_llm",
                        "treaty_iata_headquarters_legal_llm_001_012.json")

LAW_ID = "sa-treaty-iata-headquarters"
LAW_AR = "اتفاقية مقر بين حكومة المملكة واتحاد النقل الجوي الدولي (إياتا)"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_iata_headquarters_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18664), published 6/5/1443H (2021-12-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_iata_headquarters",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_IATA_HEADQUARTERS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-iata-headquarters-llm-%03d" % idx,
            ...
            "article_path": "treaty_iata_headquarters/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Headquarters Agreement between Saudi Arabia and IATA -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/5/1443H / 2021-12-10)"),
                "source_authority_ar": ("اتفاقية مقر بين حكومة المملكة واتحاد النقل الجوي الدولي (إياتا) — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_iata_headquarters",
        "layer": "TREATY_IATA_HEADQUARTERS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-iata-headquarters-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Headquarters Agreement between Saudi Arabia and IATA — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Headquarters Agreement between Saudi Arabia and IATA records" % (len(ver), len(llm)))
