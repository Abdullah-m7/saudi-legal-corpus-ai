#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم حوكمة إجراءات منع التصدير وتقييده track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18836),
published 11/6/1443H (2022-01-14). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "export_restriction_governance_statute", "official_source",
                   "export_restriction_governance_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "export_restriction_governance_statute", "verified")
RECORDS = os.path.join(OUT_VER, "export_restriction_governance_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "export_restriction_governance_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "export_restriction_governance_statute_arabic_legal_llm",
                        "export_restriction_governance_statute_legal_llm_001_021.json")

LAW_ID = "sa-export-restriction-governance-statute"
LAW_AR = "تنظيم حوكمة إجراءات منع التصدير وتقييده"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"export_restriction_governance_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18836), published 11/6/1443H (2022-01-14). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "export_restriction_governance_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "EXPORT_RESTRICTION_GOVERNANCE_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "export-restriction-governance-statute-llm-%03d" % idx,
            ...
            "article_path": "export_restriction_governance_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Governance Statute for Export Prohibition and Restriction Procedures -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 11/6/1443H / 2022-01-14)"),
                "source_authority_ar": ("تنظيم حوكمة إجراءات منع التصدير وتقييده — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 11/6/1443هـ الموافق 2022-01-14م)"),
    ...
    json.dump({
        "law_key": "export_restriction_governance_statute",
        "layer": "EXPORT_RESTRICTION_GOVERNANCE_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "export-restriction-governance-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (21 مادة، جميعها أصلية)",
        "title_en": "Governance Statute for Export Prohibition and Restriction Procedures — Arabic LLM-ready layer (21 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Governance Statute for Export Prohibition and Restriction Procedures records" % (len(ver), len(llm)))
