#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المجلس الوطني للسلامة والصحة المهنية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18987),
published 17/7/1443H (2022-02-18). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "occupational_safety_health_council_statute", "official_source",
                   "occupational_safety_health_council_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "occupational_safety_health_council_statute", "verified")
RECORDS = os.path.join(OUT_VER, "occupational_safety_health_council_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "occupational_safety_health_council_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "occupational_safety_health_council_statute_arabic_legal_llm",
                        "occupational_safety_health_council_statute_legal_llm_001_013.json")

LAW_ID = "sa-occupational-safety-health-council-statute"
LAW_AR = "تنظيم المجلس الوطني للسلامة والصحة المهنية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"occupational_safety_health_council_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18987), published 17/7/1443H (2022-02-18). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "occupational_safety_health_council_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "OCCUPATIONAL_SAFETY_HEALTH_COUNCIL_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "occupational-safety-health-council-statute-llm-%03d" % idx,
            ...
            "article_path": "occupational_safety_health_council_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Council for Occupational Safety and Health -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/7/1443H / 2022-02-18)"),
                "source_authority_ar": ("تنظيم المجلس الوطني للسلامة والصحة المهنية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 17/7/1443هـ الموافق 2022-02-18م)"),
    ...
    json.dump({
        "law_key": "occupational_safety_health_council_statute",
        "layer": "OCCUPATIONAL_SAFETY_HEALTH_COUNCIL_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "occupational-safety-health-council-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Council for Occupational Safety and Health — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Council for Occupational Safety and Health records" % (len(ver), len(llm)))
