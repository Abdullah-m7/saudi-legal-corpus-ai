#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate the Batch P0-003 QA (article-by-article review vs the official Arabic; review only).

Confirms the QA covers exactly the 20 P0-003 articles (Bab 4), uses only allowed qa_status values,
carries the correct legal-hierarchy / non-official / non-binding / non-governing posture with human
review pending, points at the P0-003 remediation file, keeps final_status/qa_summary consistent with
the per-article statuses, embeds no full Arabic/English/Chinese text, and touches no protected layer
(P0-001, P0-001 QA/minor-fixes, P0-002, P0-002 QA, P0-003 data, and the base corpora). Read-only and
idempotent.

Usage: validate_chinese_remediation_batch_p0_003_qa.py [QA_JSON_PATH]
...
MD = os.path.join(ROOT, "reports", "chinese_translation_review",
                  "CHINESE_REMEDIATION_BATCH_P0_003_QA_AR.md")
SRC = os.path.join(ROOT, "data", "chinese_remediation_batches", "p0_003",
                   "companies_law_m132_1443_zh_internal_remediation_p0_003.json")
SRC_REL = "data/chinese_remediation_batches/p0_003/companies_law_m132_1443_zh_internal_remediation_p0_003.json"
...
REQ_TOP = ("stage", "batch_id", "source_batch_file", "scope_articles", "expected_babs", "qa_method",
           ...
           "trilingual_alignment_created", "p0_003_chinese_text_modified", "p0_003_data_modified",
...
def main(argv=None) -> int:
    ...
    problems = []
    for p, label in ((qa_path, "QA JSON"), (MD, "Arabic QA report"), (SRC, "P0-003 remediation file")):
    ...
    if qa.get("stage") != "CHINESE_REMEDIATION_BATCH_P0_003_QA":
        problems.append("stage must be CHINESE_REMEDIATION_BATCH_P0_003_QA")
    if qa.get("batch_id") != "P0-003":
        problems.append("batch_id must be P0-003")
    if qa.get("source_batch_file") != SRC_REL:
        problems.append("source_batch_file must point to the P0-003 remediation JSON")
    if qa.get("scope_articles") != ARTS:
        problems.append("scope_articles must exactly match the P0-003 list")
    ...
    for f in ("full_chinese_translation_claimed", "official_chinese_translation_claimed",
              ...
              "p0_003_chinese_text_modified", "p0_003_data_modified"):
    ...
    if nums != ARTS:
        problems.append("per_article_reviews must cover exactly the 20 P0-003 articles in order")
    ...
    # P0-003 remediation file must be intact (read-only; unchanged scope/posture)
    if [r["article_number"] for r in src["records"]] != ARTS:
        problems.append("P0-003 remediation file scope changed (must remain the 20 articles)")
    if src.get("human_legal_review_status") != "pending_human_legal_review":
        problems.append("P0-003 remediation human_legal_review_status must remain pending")
    for f in ("official_chinese_translation_claimed", "chinese_binding_claimed",
        ...
        if src.get(f) is not False:
            problems.append("P0-003 remediation posture flag %s must remain false" % f)
    ...
    if not os.path.exists(P0_002_QA):
        ...
        p2qa = _read(P0_002_QA)
        if p2qa.get("final_status") != "QA_PASS" or p2qa.get("batch_id") != "P0-002":
    ...
    # no later-priority batch files created (P0-004 is now an authorized sibling batch)
    for later in ("p0_005",):
        if os.path.isdir(os.path.join(ROOT, "data", "chinese_remediation_batches", later)):
            problems.append("later batch dir %s must not exist" % later)

    print("=" * 60)
    print("Chinese remediation Batch P0-003 QA validation")
    print("=" * 60)
    if problems:
        for p in problems:
            print("  -", p)
        print("RESULT: %d problem(s) found ✗" % len(problems))
        return 1
    print("[PASS] QA of 20 P0-003 articles vs official Arabic (English secondary); allowed "
          ...
          "blocked=%d fail=%d) consistent with summary; no full Arabic/English/Chinese text "
          "embedded; P0-003 remediation intact; P0-001 + P0-002 + P0-002 QA + Chinese candidate "
