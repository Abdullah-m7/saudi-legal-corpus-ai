#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام التنفيذ أمام ديوان المظالم track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23627),
...
STRUCTURE: 37 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "bog_enforcement_implementing_regulation", "official_source",
                   "bog_enforcement_implementing_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "bog_enforcement_implementing_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "bog_enforcement_implementing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "bog_enforcement_implementing_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "bog_enforcement_implementing_regulation_arabic_legal_llm",
                        "bog_enforcement_implementing_regulation_legal_llm_001_037.json")

LAW_ID = "sa-bog-enforcement-implementing-regulation"
LAW_AR = "اللائحة التنفيذية لنظام التنفيذ أمام ديوان المظالم"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"bog_enforcement_implementing_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23627), published 23/2/1445H (2023-09-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "bog_enforcement_implementing_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "BOG_ENFORCEMENT_IMPLEMENTING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "bog-enforcement-implementing-regulation-llm-%03d" % idx,
            ...
            "article_path": "bog_enforcement_implementing_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Enforcement Law before the Board of Grievances -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/2/1445H / 2023-09-08)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام التنفيذ أمام ديوان المظالم — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "bog_enforcement_implementing_regulation",
        "layer": "BOG_ENFORCEMENT_IMPLEMENTING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "bog-enforcement-implementing-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (37 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Enforcement Law before the Board of Grievances — Arabic LLM-ready layer (37 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Enforcement Law before the Board of Grievances records" % (len(ver), len(llm)))
