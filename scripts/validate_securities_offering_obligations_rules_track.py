...
RECORDS = os.path.join(ROOT, "sources", "securities_offering_obligations_rules", "verified",
                       "securities_offering_obligations_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "securities_offering_obligations_rules", "verified",
                       "securities_offering_obligations_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "securities_offering_obligations_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"securities_offering_obligations_rules_no_decision_number_in_source", "securities_offering_obligations_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Rules on the Offer of Securities and Continuing Obligations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules on the Offer of Securities and Continuing Obligations")
    print("  - 103 records: 103 articles, no appendices; all 103 اصلية")
