#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم وكالة الأنباء السعودية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23102),
...
SRC = os.path.join(ROOT, "sources", "saudi_press_agency_statute", "official_source",
                   "saudi_press_agency_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "saudi_press_agency_statute", "verified")
RECORDS = os.path.join(OUT_VER, "saudi_press_agency_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "saudi_press_agency_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "saudi_press_agency_statute_arabic_legal_llm",
                        "saudi_press_agency_statute_legal_llm_001_016.json")

LAW_ID = "sa-saudi-press-agency-statute"
LAW_AR = "تنظيم وكالة الأنباء السعودية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"saudi_press_agency_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23102), published 27/11/1444H (2023-06-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "saudi_press_agency_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "SAUDI_PRESS_AGENCY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "saudi-press-agency-statute-llm-%03d" % idx,
            ...
            "article_path": "saudi_press_agency_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Saudi Press Agency -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 27/11/1444H / 2023-06-16)"),
                "source_authority_ar": ("تنظيم وكالة الأنباء السعودية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "saudi_press_agency_statute",
        "layer": "SAUDI_PRESS_AGENCY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "saudi-press-agency-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Statute of the Saudi Press Agency — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Saudi Press Agency records" % (len(ver), len(llm)))
