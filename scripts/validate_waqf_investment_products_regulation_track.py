#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة المنتجات الاستثمارية الوقفية track.
...
RECORDS = os.path.join(ROOT, "sources", "waqf_investment_products_regulation", "verified",
                       "waqf_investment_products_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "waqf_investment_products_regulation", "verified",
                       "waqf_investment_products_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "waqf_investment_products_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"waqf_investment_products_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "26/12/1447":
        e.append("[2j] gazette_publication_date_hijri must be 26/12/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-06-12":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-06-12")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة المنتجات الاستثمارية الوقفية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on Waqf Investment Products track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on Waqf Investment Products")
