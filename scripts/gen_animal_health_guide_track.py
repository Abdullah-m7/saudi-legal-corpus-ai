#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the دليل الصحة الحيوانية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24606),
...
STRUCTURE: 83 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "animal_health_guide", "official_source",
                   "animal_health_guide_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "animal_health_guide", "verified")
RECORDS = os.path.join(OUT_VER, "animal_health_guide_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "animal_health_guide_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "animal_health_guide_arabic_legal_llm",
                        "animal_health_guide_legal_llm_001_083.json")

LAW_ID = "sa-animal-health-guide"
LAW_AR = "دليل الصحة الحيوانية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"animal_health_guide_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24606), published 20/8/1445H (2024-03-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "animal_health_guide",
            "law_component": "guide",
            "language": "ar",
            "record_layer": "ANIMAL_HEALTH_GUIDE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "animal-health-guide-llm-%03d" % idx,
            ...
            "article_path": "animal_health_guide/guide/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Animal Health Guide -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/8/1445H / 2024-03-01)"),
                "source_authority_ar": ("دليل الصحة الحيوانية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "animal_health_guide",
        "layer": "ANIMAL_HEALTH_GUIDE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "animal-health-guide-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "guide",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (83 مادة، جميعها أصلية)",
        "title_en": "Animal Health Guide — Arabic LLM-ready layer (83 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Animal Health Guide records" % (len(ver), len(llm)))
