#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد عدم خضوع المكلف المملوك لوقف لجباية الزكاة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21213),
...
SRC = os.path.join(ROOT, "sources", "waqf_owned_taxpayer_zakat_rules", "official_source",
                   "waqf_owned_taxpayer_zakat_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "waqf_owned_taxpayer_zakat_rules", "verified")
RECORDS = os.path.join(OUT_VER, "waqf_owned_taxpayer_zakat_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "waqf_owned_taxpayer_zakat_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "waqf_owned_taxpayer_zakat_rules_arabic_legal_llm",
                        "waqf_owned_taxpayer_zakat_rules_legal_llm_001_006.json")

LAW_ID = "sa-waqf-owned-taxpayer-zakat-rules"
LAW_AR = "قواعد عدم خضوع المكلف المملوك لوقف لجباية الزكاة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"waqf_owned_taxpayer_zakat_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21213), published 13/6/1444H (2023-01-06). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "waqf_owned_taxpayer_zakat_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "WAQF_OWNED_TAXPAYER_ZAKAT_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "waqf-owned-taxpayer-zakat-rules-llm-%03d" % idx,
            ...
            "article_path": "waqf_owned_taxpayer_zakat_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules on the Non-Subjection of a Waqf-Owned Taxpayer to Zakat Collection -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/6/1444H / 2023-01-06)"),
                "source_authority_ar": ("قواعد عدم خضوع المكلف المملوك لوقف لجباية الزكاة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "waqf_owned_taxpayer_zakat_rules",
        "layer": "WAQF_OWNED_TAXPAYER_ZAKAT_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "waqf-owned-taxpayer-zakat-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (6 مادة، جميعها أصلية)",
        "title_en": "Rules on the Non-Subjection of a Waqf-Owned Taxpayer to Zakat Collection — Arabic LLM-ready layer (6 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules on the Non-Subjection of a Waqf-Owned Taxpayer to Zakat Collection records" % (len(ver), len(llm)))
