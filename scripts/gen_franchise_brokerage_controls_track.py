#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط ممارسة أنشطة الوساطة في الامتياز التجاري track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18344),
...
SRC = os.path.join(ROOT, "sources", "franchise_brokerage_controls", "official_source",
                   "franchise_brokerage_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "franchise_brokerage_controls", "verified")
RECORDS = os.path.join(OUT_VER, "franchise_brokerage_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "franchise_brokerage_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "franchise_brokerage_controls_arabic_legal_llm",
                        "franchise_brokerage_controls_legal_llm_001_017.json")

LAW_ID = "sa-franchise-brokerage-controls"
LAW_AR = "ضوابط ممارسة أنشطة الوساطة في الامتياز التجاري"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"franchise_brokerage_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18344), published 17/2/1443H (2021-09-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "franchise_brokerage_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "FRANCHISE_BROKERAGE_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "franchise-brokerage-controls-llm-%03d" % idx,
            ...
            "article_path": "franchise_brokerage_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on the Practice of Franchise Brokerage Activities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/2/1443H / 2021-09-24)"),
                "source_authority_ar": ("ضوابط ممارسة أنشطة الوساطة في الامتياز التجاري — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "franchise_brokerage_controls",
        "layer": "FRANCHISE_BROKERAGE_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "franchise-brokerage-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Controls on the Practice of Franchise Brokerage Activities — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on the Practice of Franchise Brokerage Activities records" % (len(ver), len(llm)))
