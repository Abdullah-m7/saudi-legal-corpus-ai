#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة تنظيم أعمال الإطفاء والإنقاذ بالمديرية العامة للدفاع المدني track.
...
RECORDS = os.path.join(ROOT, "sources", "civil_defense_firefighting_rescue_regulation", "verified",
                       "civil_defense_firefighting_rescue_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "civil_defense_firefighting_rescue_regulation", "verified",
                       "civil_defense_firefighting_rescue_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "civil_defense_firefighting_rescue_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"civil_defense_firefighting_rescue_regulation_no_decision_number_in_source", "civil_defense_firefighting_rescue_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "30/3/1443":
        e.append("[2j] gazette_publication_date_hijri must be 30/3/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-11-05":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-11-05")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة تنظيم أعمال الإطفاء والإنقاذ بالمديرية العامة للدفاع المدني":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation Governing Firefighting and Rescue Works at the General Directorate of Civil Defense track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation Governing Firefighting and Rescue Works at the General Directorate of Civil Defense")
