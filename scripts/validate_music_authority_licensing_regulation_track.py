#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة التراخيص - هيئة الموسيقى track.
...
RECORDS = os.path.join(ROOT, "sources", "music_authority_licensing_regulation", "verified",
                       "music_authority_licensing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "music_authority_licensing_regulation", "verified",
                       "music_authority_licensing_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "music_authority_licensing_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"music_authority_licensing_regulation_no_decision_number_in_source", "music_authority_licensing_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة التراخيص - هيئة الموسيقى":
    ...
    if e:
        print("FAIL: %d error(s) in Licensing Regulation — Music Authority track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Licensing Regulation — Music Authority")
