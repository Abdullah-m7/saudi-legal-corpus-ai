#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the السياسة الاقتصادية للطيران المدني في المملكة العربية السعودية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23976),
...
SRC = os.path.join(ROOT, "sources", "civil_aviation_economic_policy", "official_source",
                   "civil_aviation_economic_policy_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "civil_aviation_economic_policy", "verified")
RECORDS = os.path.join(OUT_VER, "civil_aviation_economic_policy_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "civil_aviation_economic_policy_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "civil_aviation_economic_policy_arabic_legal_llm",
                        "civil_aviation_economic_policy_legal_llm_001_004.json")

LAW_ID = "sa-civil-aviation-economic-policy"
LAW_AR = "السياسة الاقتصادية للطيران المدني في المملكة العربية السعودية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"civil_aviation_economic_policy_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23976), published 15/4/1445H (2023-10-30). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "civil_aviation_economic_policy",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "CIVIL_AVIATION_ECONOMIC_POLICY_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "civil-aviation-economic-policy-llm-%03d" % idx,
            ...
            "article_path": "civil_aviation_economic_policy/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Economic Policy for Civil Aviation in Saudi Arabia -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/4/1445H / 2023-10-30)"),
                "source_authority_ar": ("السياسة الاقتصادية للطيران المدني في المملكة العربية السعودية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "civil_aviation_economic_policy",
        "layer": "CIVIL_AVIATION_ECONOMIC_POLICY_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "civil-aviation-economic-policy-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (4 مادة، جميعها أصلية)",
        "title_en": "Economic Policy for Civil Aviation in Saudi Arabia — Arabic LLM-ready layer (4 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Economic Policy for Civil Aviation in Saudi Arabia records" % (len(ver), len(llm)))
