#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة الإبلاغ عن مخالفات نظام السوق المالية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19095),
...
SRC = os.path.join(ROOT, "sources", "capital_market_whistleblowing_regulation", "official_source",
                   "capital_market_whistleblowing_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "capital_market_whistleblowing_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "capital_market_whistleblowing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "capital_market_whistleblowing_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "capital_market_whistleblowing_regulation_arabic_legal_llm",
                        "capital_market_whistleblowing_regulation_legal_llm_001_014.json")

LAW_ID = "sa-capital-market-whistleblowing-regulation"
LAW_AR = "لائحة الإبلاغ عن مخالفات نظام السوق المالية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"capital_market_whistleblowing_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19095), published 8/8/1443H (2022-03-11). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "capital_market_whistleblowing_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CAPITAL_MARKET_WHISTLEBLOWING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "capital-market-whistleblowing-regulation-llm-%03d" % idx,
            ...
            "article_path": "capital_market_whistleblowing_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Reporting Violations of the Capital Market Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 8/8/1443H / 2022-03-11)"),
                "source_authority_ar": ("لائحة الإبلاغ عن مخالفات نظام السوق المالية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "capital_market_whistleblowing_regulation",
        "layer": "CAPITAL_MARKET_WHISTLEBLOWING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "capital-market-whistleblowing-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Regulation on Reporting Violations of the Capital Market Law — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Reporting Violations of the Capital Market Law records" % (len(ver), len(llm)))
