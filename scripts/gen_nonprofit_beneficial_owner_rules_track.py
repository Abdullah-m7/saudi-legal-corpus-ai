#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد المستفيد الحقيقي للجمعيات والمؤسسات الأهلية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27371),
published 28/2/1447H (2025-08-22). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "nonprofit_beneficial_owner_rules", "official_source",
                   "nonprofit_beneficial_owner_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "nonprofit_beneficial_owner_rules", "verified")
RECORDS = os.path.join(OUT_VER, "nonprofit_beneficial_owner_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "nonprofit_beneficial_owner_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "nonprofit_beneficial_owner_rules_arabic_legal_llm",
                        "nonprofit_beneficial_owner_rules_legal_llm_001_013.json")

LAW_ID = "sa-nonprofit-beneficial-owner-rules"
LAW_AR = "قواعد المستفيد الحقيقي للجمعيات والمؤسسات الأهلية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"nonprofit_beneficial_owner_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27371), published 28/2/1447H (2025-08-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "nonprofit_beneficial_owner_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "NONPROFIT_BENEFICIAL_OWNER_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "nonprofit-beneficial-owner-rules-llm-%03d" % idx,
            ...
            "article_path": "nonprofit_beneficial_owner_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Beneficial Owner Rules for Associations and Non-Profit Foundations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/2/1447H / 2025-08-22)"),
                "source_authority_ar": ("قواعد المستفيد الحقيقي للجمعيات والمؤسسات الأهلية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 28/2/1447هـ الموافق 2025-08-22م)"),
    ...
    json.dump({
        "law_key": "nonprofit_beneficial_owner_rules",
        "layer": "NONPROFIT_BENEFICIAL_OWNER_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "nonprofit-beneficial-owner-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Beneficial Owner Rules for Associations and Non-Profit Foundations — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Beneficial Owner Rules for Associations and Non-Profit Foundations records" % (len(ver), len(llm)))
