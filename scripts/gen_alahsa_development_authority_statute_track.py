#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لهيئة تطوير الأحساء track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20838),
published 17/4/1444H (2022-11-11). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "alahsa_development_authority_statute", "official_source",
                   "alahsa_development_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "alahsa_development_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "alahsa_development_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "alahsa_development_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "alahsa_development_authority_statute_arabic_legal_llm",
                        "alahsa_development_authority_statute_legal_llm_001_015.json")

LAW_ID = "sa-alahsa-development-authority-statute"
LAW_AR = "الترتيبات التنظيمية لهيئة تطوير الأحساء"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"alahsa_development_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20838), published 17/4/1444H (2022-11-11). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "alahsa_development_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "ALAHSA_DEVELOPMENT_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "alahsa-development-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "alahsa_development_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Al-Ahsa Development Authority — Organisational Arrangements -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/4/1444H / 2022-11-11)"),
                "source_authority_ar": ("الترتيبات التنظيمية لهيئة تطوير الأحساء — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 17/4/1444هـ الموافق 2022-11-11م)"),
    ...
    json.dump({
        "law_key": "alahsa_development_authority_statute",
        "layer": "ALAHSA_DEVELOPMENT_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "alahsa-development-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Al-Ahsa Development Authority — Organisational Arrangements — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Al-Ahsa Development Authority — Organisational Arrangements records" % (len(ver), len(llm)))
