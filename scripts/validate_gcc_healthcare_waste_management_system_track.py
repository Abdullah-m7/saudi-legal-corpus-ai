#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the النظام الموحد لإدارة نفايات الرعاية الصحية بدول مجلس التعاون لدول الخليج العربي track.
...
RECORDS = os.path.join(ROOT, "sources", "gcc_healthcare_waste_management_system", "verified",
                       "gcc_healthcare_waste_management_system_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "gcc_healthcare_waste_management_system", "verified",
                       "gcc_healthcare_waste_management_system_verified_summary.json")
LLM = os.path.join(ROOT, "data", "gcc_healthcare_waste_management_system_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"gcc_healthcare_waste_management_system_no_decision_number_in_source", "gcc_healthcare_waste_management_system_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "النظام الموحد لإدارة نفايات الرعاية الصحية بدول مجلس التعاون لدول الخليج العربي":
    ...
    if e:
        print("FAIL: %d error(s) in GCC Unified System for Healthcare Waste Management track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: GCC Unified System for Healthcare Waste Management")
