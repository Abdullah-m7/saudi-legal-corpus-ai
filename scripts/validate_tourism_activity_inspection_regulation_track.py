#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة التفتيش على الأنشطة السياحية track.
...
RECORDS = os.path.join(ROOT, "sources", "tourism_activity_inspection_regulation", "verified",
                       "tourism_activity_inspection_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "tourism_activity_inspection_regulation", "verified",
                       "tourism_activity_inspection_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "tourism_activity_inspection_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"tourism_activity_inspection_regulation_no_decision_number_in_source", "tourism_activity_inspection_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة التفتيش على الأنشطة السياحية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on the Inspection of Tourism Activities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on the Inspection of Tourism Activities")
