#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the آلية عمل وحوكمة إبرام الاتفاقيات التجارية وذات الصلة بالتجارة track.
...
RECORDS = os.path.join(ROOT, "sources", "trade_agreements_governance_mechanism", "verified",
                       "trade_agreements_governance_mechanism_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "trade_agreements_governance_mechanism", "verified",
                       "trade_agreements_governance_mechanism_verified_summary.json")
LLM = os.path.join(ROOT, "data", "trade_agreements_governance_mechanism_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"trade_agreements_governance_mechanism_no_decision_number_in_source", "trade_agreements_governance_mechanism_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/2/1443":
        e.append("[2j] gazette_publication_date_hijri must be 24/2/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-10-01":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-10-01")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "آلية عمل وحوكمة إبرام الاتفاقيات التجارية وذات الصلة بالتجارة":
    ...
    if e:
        print("FAIL: %d error(s) in Mechanism for Concluding and Governing Trade and Trade-Related Agreements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Mechanism for Concluding and Governing Trade and Trade-Related Agreements")
