#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام البريد track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20932),
...
STRUCTURE: 92 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "postal_law_regulation", "official_source",
                   "postal_law_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "postal_law_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "postal_law_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "postal_law_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "postal_law_regulation_arabic_legal_llm",
                        "postal_law_regulation_legal_llm_001_092.json")

LAW_ID = "sa-postal-law-regulation"
LAW_AR = "اللائحة التنفيذية لنظام البريد"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"postal_law_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20932), published 1/5/1444H (2022-11-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "postal_law_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "POSTAL_LAW_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "postal-law-regulation-llm-%03d" % idx,
            ...
            "article_path": "postal_law_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Postal Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/5/1444H / 2022-11-25)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام البريد — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "postal_law_regulation",
        "layer": "POSTAL_LAW_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "postal-law-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (92 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Postal Law — Arabic LLM-ready layer (92 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Postal Law records" % (len(ver), len(llm)))
