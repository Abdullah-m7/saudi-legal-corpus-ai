#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة لجنة النظر في مخالفات نظام السياحة ولوائحه track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21126),
published 1/6/1444H (2022-12-25). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "tourism_violations_committee_regulation", "official_source",
                   "tourism_violations_committee_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "tourism_violations_committee_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "tourism_violations_committee_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "tourism_violations_committee_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "tourism_violations_committee_regulation_arabic_legal_llm",
                        "tourism_violations_committee_regulation_legal_llm_001_023.json")

LAW_ID = "sa-tourism-violations-committee-regulation"
LAW_AR = "لائحة لجنة النظر في مخالفات نظام السياحة ولوائحه"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"tourism_violations_committee_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21126), published 1/6/1444H (2022-12-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "tourism_violations_committee_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "TOURISM_VIOLATIONS_COMMITTEE_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "tourism-violations-committee-regulation-llm-%03d" % idx,
            ...
            "article_path": "tourism_violations_committee_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation of the Committee for Adjudicating Violations of the Tourism Law and its Regulations -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/6/1444H / 2022-12-25)"),
                "source_authority_ar": ("لائحة لجنة النظر في مخالفات نظام السياحة ولوائحه — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 1/6/1444هـ الموافق 2022-12-25م)"),
    ...
    json.dump({
        "law_key": "tourism_violations_committee_regulation",
        "layer": "TOURISM_VIOLATIONS_COMMITTEE_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "tourism-violations-committee-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (23 مادة، جميعها أصلية)",
        "title_en": "Regulation of the Committee for Adjudicating Violations of the Tourism Law and its Regulations — Arabic LLM-ready layer (23 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation of the Committee for Adjudicating Violations of the Tourism Law and its Regulations records" % (len(ver), len(llm)))
