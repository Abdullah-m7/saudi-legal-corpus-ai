#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط تخصيص عقارات الدولة واستردادها track.
...
RECORDS = os.path.join(ROOT, "sources", "state_property_allocation_recovery_controls", "verified",
                       "state_property_allocation_recovery_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "state_property_allocation_recovery_controls", "verified",
                       "state_property_allocation_recovery_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "state_property_allocation_recovery_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"state_property_allocation_recovery_controls_no_decision_number_in_source", "state_property_allocation_recovery_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "ضوابط تخصيص عقارات الدولة واستردادها":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on the Allocation and Recovery of State Real Estate track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on the Allocation and Recovery of State Real Estate")
