#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the النظام الأساس لشركة الصحة القابضة track.
...
RECORDS = os.path.join(ROOT, "sources", "health_holding_company_statute", "verified",
                       "health_holding_company_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "health_holding_company_statute", "verified",
                       "health_holding_company_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "health_holding_company_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"health_holding_company_statute_no_decision_number_in_source", "health_holding_company_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "11/11/1443":
        e.append("[2j] gazette_publication_date_hijri must be 11/11/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-06-10":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-06-10")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "النظام الأساس لشركة الصحة القابضة":
    ...
    if e:
        print("FAIL: %d error(s) in Articles of Association of the Health Holding Company track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Articles of Association of the Health Holding Company")
