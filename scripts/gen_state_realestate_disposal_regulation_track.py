#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة التصرف في عقارات الدولة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24399),
...
STRUCTURE: 74 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "state_realestate_disposal_regulation", "official_source",
                   "state_realestate_disposal_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "state_realestate_disposal_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "state_realestate_disposal_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "state_realestate_disposal_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "state_realestate_disposal_regulation_arabic_legal_llm",
                        "state_realestate_disposal_regulation_legal_llm_001_074.json")

LAW_ID = "sa-state-realestate-disposal-regulation"
LAW_AR = "لائحة التصرف في عقارات الدولة"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24399), published 14/7/1445H (2024-01-26). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "state_realestate_disposal_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "STATE_REALESTATE_DISPOSAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "state-realestate-disposal-regulation-llm-%03d" % idx,
            ...
            "article_path": "state_realestate_disposal_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on the Disposal of State Real Estate -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/7/1445H / 2024-01-26)"),
                "source_authority_ar": ("لائحة التصرف في عقارات الدولة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "state_realestate_disposal_regulation",
        "layer": "STATE_REALESTATE_DISPOSAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "state-realestate-disposal-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (74 مادة، جميعها أصلية)",
        "title_en": "Regulation on the Disposal of State Real Estate — Arabic LLM-ready layer (74 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on the Disposal of State Real Estate records" % (len(ver), len(llm)))
