#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة شروط تفويض هيئات التصنيف في المملكة العربية السعودية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24357),
published 30/6/1445H (2024-01-12). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "classification_societies_authorisation_regulation", "official_source",
                   "classification_societies_authorisation_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "classification_societies_authorisation_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "classification_societies_authorisation_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "classification_societies_authorisation_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "classification_societies_authorisation_regulation_arabic_legal_llm",
                        "classification_societies_authorisation_regulation_legal_llm_001_012.json")

LAW_ID = "sa-classification-societies-authorisation-regulation"
LAW_AR = "لائحة شروط تفويض هيئات التصنيف في المملكة العربية السعودية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"classification_societies_authorisation_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24357), published 30/6/1445H (2024-01-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "classification_societies_authorisation_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CLASSIFICATION_SOCIETIES_AUTHORISATION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "classification-societies-authorisation-regulation-llm-%03d" % idx,
            ...
            "article_path": "classification_societies_authorisation_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on the Conditions for Authorising Classification Societies in the Kingdom -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 30/6/1445H / 2024-01-12)"),
                "source_authority_ar": ("لائحة شروط تفويض هيئات التصنيف في المملكة العربية السعودية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 30/6/1445هـ الموافق 2024-01-12م)"),
    ...
    json.dump({
        "law_key": "classification_societies_authorisation_regulation",
        "layer": "CLASSIFICATION_SOCIETIES_AUTHORISATION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "classification-societies-authorisation-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Regulation on the Conditions for Authorising Classification Societies in the Kingdom — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on the Conditions for Authorising Classification Societies in the Kingdom records" % (len(ver), len(llm)))
