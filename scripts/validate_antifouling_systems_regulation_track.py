#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية للاتفاقية الدولية بشأن ضبط النظم السفينية المقاومة لالتصاق الشوائب ذات الآثار المؤذية track.
...
RECORDS = os.path.join(ROOT, "sources", "antifouling_systems_regulation", "verified",
                       "antifouling_systems_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "antifouling_systems_regulation", "verified",
                       "antifouling_systems_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "antifouling_systems_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"antifouling_systems_regulation_no_decision_number_in_source", "antifouling_systems_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "26/6/1446":
        e.append("[2j] gazette_publication_date_hijri must be 26/6/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-12-27":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-12-27")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنفيذية للاتفاقية الدولية بشأن ضبط النظم السفينية المقاومة لالتصاق الشوائب ذات الآثار المؤذية":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation of the International Convention on the Control of Harmful Anti-Fouling Systems on Ships track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation of the International Convention on the Control of Harmful Anti-Fouling Systems on Ships")
