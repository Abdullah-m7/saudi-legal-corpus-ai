#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام استخدام كاميرات المراقبة الأمنية track.
...
RECORDS = os.path.join(ROOT, "sources", "security_cameras_law", "verified",
                       "security_cameras_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "security_cameras_law", "verified",
                       "security_cameras_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "security_cameras_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"security_cameras_law_no_decision_number_in_source", "security_cameras_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام استخدام كاميرات المراقبة الأمنية":
    ...
    if e:
        print("FAIL: %d error(s) in Law on the Use of Security Surveillance Cameras track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law on the Use of Security Surveillance Cameras")
