#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية بين المملكة ومنظمة السياحة العالمية track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_unwto_cooperation", "verified",
                       "treaty_unwto_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_unwto_cooperation", "verified",
                       "treaty_unwto_cooperation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_unwto_cooperation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_unwto_cooperation_no_decision_number_in_source", "treaty_unwto_cooperation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية بين المملكة ومنظمة السياحة العالمية":
    ...
    if e:
        print("FAIL: %d error(s) in Agreement between Saudi Arabia and the World Tourism Organization track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Agreement between Saudi Arabia and the World Tourism Organization")
