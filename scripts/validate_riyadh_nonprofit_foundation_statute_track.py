#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the النظام الأساس لمؤسسة الرياض غير الربحية (مؤسسة مستقلة ذات طبيعة خاصة وغير هادفة للربح) track.
...
RECORDS = os.path.join(ROOT, "sources", "riyadh_nonprofit_foundation_statute", "verified",
                       "riyadh_nonprofit_foundation_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "riyadh_nonprofit_foundation_statute", "verified",
                       "riyadh_nonprofit_foundation_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "riyadh_nonprofit_foundation_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"riyadh_nonprofit_foundation_statute_no_decision_number_in_source", "riyadh_nonprofit_foundation_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "22/12/1445":
        e.append("[2j] gazette_publication_date_hijri must be 22/12/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-06-28":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-06-28")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "النظام الأساس لمؤسسة الرياض غير الربحية (مؤسسة مستقلة ذات طبيعة خاصة وغير هادفة للربح)":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Riyadh Non-Profit Foundation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Riyadh Non-Profit Foundation")
