#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد الأسماء التجارية في المناطق الاقتصادية الخاصة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4001349),
...
SRC = os.path.join(ROOT, "sources", "sez_trade_names_rules", "official_source",
                   "sez_trade_names_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "sez_trade_names_rules", "verified")
RECORDS = os.path.join(OUT_VER, "sez_trade_names_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "sez_trade_names_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "sez_trade_names_rules_arabic_legal_llm",
                        "sez_trade_names_rules_legal_llm_001_025.json")

LAW_ID = "sa-sez-trade-names-rules"
LAW_AR = "قواعد الأسماء التجارية في المناطق الاقتصادية الخاصة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"sez_trade_names_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4001349), published 25/1/1448H (2026-07-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "sez_trade_names_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SEZ_TRADE_NAMES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "sez-trade-names-rules-llm-%03d" % idx,
            ...
            "article_path": "sez_trade_names_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Trade Names Rules in the Special Economic Zones -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/1/1448H / 2026-07-10)"),
                "source_authority_ar": ("قواعد الأسماء التجارية في المناطق الاقتصادية الخاصة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "sez_trade_names_rules",
        "layer": "SEZ_TRADE_NAMES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "sez-trade-names-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (25 مادة، جميعها أصلية)",
        "title_en": "Trade Names Rules in the Special Economic Zones — Arabic LLM-ready layer (25 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Trade Names Rules in the Special Economic Zones records" % (len(ver), len(llm)))
