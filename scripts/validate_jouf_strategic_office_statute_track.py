#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية للمكتب الاستراتيجي لتطوير منطقة الجوف track.
...
RECORDS = os.path.join(ROOT, "sources", "jouf_strategic_office_statute", "verified",
                       "jouf_strategic_office_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "jouf_strategic_office_statute", "verified",
                       "jouf_strategic_office_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "jouf_strategic_office_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"jouf_strategic_office_statute_no_decision_number_in_source", "jouf_strategic_office_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية للمكتب الاستراتيجي لتطوير منطقة الجوف":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the Strategic Office for the Development of the Al-Jouf Region track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the Strategic Office for the Development of the Al-Jouf Region")
