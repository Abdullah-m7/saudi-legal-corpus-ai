#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the المعايير المهنية لتقييم أضرار المركبات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25048),
published 23/11/1445H (2024-05-31). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "vehicle_damage_assessment_standards", "official_source",
                   "vehicle_damage_assessment_standards_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "vehicle_damage_assessment_standards", "verified")
RECORDS = os.path.join(OUT_VER, "vehicle_damage_assessment_standards_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "vehicle_damage_assessment_standards_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "vehicle_damage_assessment_standards_arabic_legal_llm",
                        "vehicle_damage_assessment_standards_legal_llm_001_004.json")

LAW_ID = "sa-vehicle-damage-assessment-standards"
LAW_AR = "المعايير المهنية لتقييم أضرار المركبات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"vehicle_damage_assessment_standards_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25048), published 23/11/1445H (2024-05-31). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "vehicle_damage_assessment_standards",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "VEHICLE_DAMAGE_ASSESSMENT_STANDARDS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "vehicle-damage-assessment-standards-llm-%03d" % idx,
            ...
            "article_path": "vehicle_damage_assessment_standards/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Professional Standards for Vehicle Damage Assessment -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/11/1445H / 2024-05-31)"),
                "source_authority_ar": ("المعايير المهنية لتقييم أضرار المركبات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 23/11/1445هـ الموافق 2024-05-31م)"),
    ...
    json.dump({
        "law_key": "vehicle_damage_assessment_standards",
        "layer": "VEHICLE_DAMAGE_ASSESSMENT_STANDARDS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "vehicle-damage-assessment-standards-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (4 مادة، جميعها أصلية)",
        "title_en": "Professional Standards for Vehicle Damage Assessment — Arabic LLM-ready layer (4 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Professional Standards for Vehicle Damage Assessment records" % (len(ver), len(llm)))
