#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام إدارة المواد الكيميائية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27388),
published 6/3/1447H (2025-08-29). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "chemicals_management_regulation", "official_source",
                   "chemicals_management_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "chemicals_management_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "chemicals_management_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "chemicals_management_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "chemicals_management_regulation_arabic_legal_llm",
                        "chemicals_management_regulation_legal_llm_001_018.json")

LAW_ID = "sa-chemicals-management-regulation"
LAW_AR = "اللائحة التنفيذية لنظام إدارة المواد الكيميائية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"chemicals_management_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27388), published 6/3/1447H (2025-08-29). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "chemicals_management_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CHEMICALS_MANAGEMENT_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "chemicals-management-regulation-llm-%03d" % idx,
            ...
            "article_path": "chemicals_management_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Chemicals Management Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/3/1447H / 2025-08-29)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام إدارة المواد الكيميائية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 6/3/1447هـ الموافق 2025-08-29م)"),
    ...
    json.dump({
        "law_key": "chemicals_management_regulation",
        "layer": "CHEMICALS_MANAGEMENT_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "chemicals-management-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Chemicals Management Law — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Chemicals Management Law records" % (len(ver), len(llm)))
