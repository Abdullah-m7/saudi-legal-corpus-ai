#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مجلس المحتوى الرقمي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18386),
...
SRC = os.path.join(ROOT, "sources", "digital_content_council_statute", "official_source",
                   "digital_content_council_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "digital_content_council_statute", "verified")
RECORDS = os.path.join(OUT_VER, "digital_content_council_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "digital_content_council_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "digital_content_council_statute_arabic_legal_llm",
                        "digital_content_council_statute_legal_llm_001_014.json")

LAW_ID = "sa-digital-content-council-statute"
LAW_AR = "تنظيم مجلس المحتوى الرقمي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"digital_content_council_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18386), published 2/3/1443H (2021-10-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "digital_content_council_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "DIGITAL_CONTENT_COUNCIL_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "digital-content-council-statute-llm-%03d" % idx,
            ...
            "article_path": "digital_content_council_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Digital Content Council -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/3/1443H / 2021-10-08)"),
                "source_authority_ar": ("تنظيم مجلس المحتوى الرقمي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "digital_content_council_statute",
        "layer": "DIGITAL_CONTENT_COUNCIL_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "digital-content-council-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Statute of the Digital Content Council — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Digital Content Council records" % (len(ver), len(llm)))
