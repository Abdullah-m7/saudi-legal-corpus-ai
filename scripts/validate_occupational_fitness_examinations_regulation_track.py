#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة فحوصات اللياقة المهنية والأمراض غير المعدية track.
...
RECORDS = os.path.join(ROOT, "sources", "occupational_fitness_examinations_regulation", "verified",
                       "occupational_fitness_examinations_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "occupational_fitness_examinations_regulation", "verified",
                       "occupational_fitness_examinations_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "occupational_fitness_examinations_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"occupational_fitness_examinations_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "2/5/1447":
        e.append("[2j] gazette_publication_date_hijri must be 2/5/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-10-24":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-10-24")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة فحوصات اللياقة المهنية والأمراض غير المعدية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on Occupational Fitness Examinations and Non-Communicable Diseases track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on Occupational Fitness Examinations and Non-Communicable Diseases")
