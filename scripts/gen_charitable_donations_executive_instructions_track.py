#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the التعليمات التنفيذية للائحة جمع التبرعات للوجوه الخيرية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21781),
published 8/10/1444H (2023-04-28). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "charitable_donations_executive_instructions", "official_source",
                   "charitable_donations_executive_instructions_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "charitable_donations_executive_instructions", "verified")
RECORDS = os.path.join(OUT_VER, "charitable_donations_executive_instructions_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "charitable_donations_executive_instructions_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "charitable_donations_executive_instructions_arabic_legal_llm",
                        "charitable_donations_executive_instructions_legal_llm_001_024.json")

LAW_ID = "sa-charitable-donations-executive-instructions"
LAW_AR = "التعليمات التنفيذية للائحة جمع التبرعات للوجوه الخيرية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"charitable_donations_executive_instructions_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21781), published 8/10/1444H (2023-04-28). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "charitable_donations_executive_instructions",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "CHARITABLE_DONATIONS_EXECUTIVE_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "charitable-donations-executive-instructions-llm-%03d" % idx,
            ...
            "article_path": "charitable_donations_executive_instructions/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Instructions for the Charitable Donations Collection Regulation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 8/10/1444H / 2023-04-28)"),
                "source_authority_ar": ("التعليمات التنفيذية للائحة جمع التبرعات للوجوه الخيرية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 8/10/1444هـ الموافق 2023-04-28م)"),
    ...
    json.dump({
        "law_key": "charitable_donations_executive_instructions",
        "layer": "CHARITABLE_DONATIONS_EXECUTIVE_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "charitable-donations-executive-instructions-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (24 مادة، جميعها أصلية)",
        "title_en": "Executive Instructions for the Charitable Donations Collection Regulation — Arabic LLM-ready layer (24 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Instructions for the Charitable Donations Collection Regulation records" % (len(ver), len(llm)))
