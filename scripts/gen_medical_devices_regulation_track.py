#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام الأجهزة والمستلزمات الطبية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18610),
...
STRUCTURE: 49 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "medical_devices_regulation", "official_source",
                   "medical_devices_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "medical_devices_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "medical_devices_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "medical_devices_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "medical_devices_regulation_arabic_legal_llm",
                        "medical_devices_regulation_legal_llm_001_049.json")

LAW_ID = "sa-medical-devices-regulation"
LAW_AR = "اللائحة التنفيذية لنظام الأجهزة والمستلزمات الطبية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"medical_devices_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18610), published 21/4/1443H (2021-11-26). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "medical_devices_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MEDICAL_DEVICES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "medical-devices-regulation-llm-%03d" % idx,
            ...
            "article_path": "medical_devices_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Medical Devices and Supplies Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 21/4/1443H / 2021-11-26)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام الأجهزة والمستلزمات الطبية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "medical_devices_regulation",
        "layer": "MEDICAL_DEVICES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "medical-devices-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (49 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Medical Devices and Supplies Law — Arabic LLM-ready layer (49 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Medical Devices and Supplies Law records" % (len(ver), len(llm)))
