#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة عضوية الهيئة السعودية للتخصصات الصحية track.
...
def main():
    ...
    if src.get("document") != "لائحة عضوية الهيئة السعودية للتخصصات الصحية":
    ...
    if e:
        print("FAIL: %d error(s) in Membership Regulation of the Saudi Commission for Health Specialties track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Membership Regulation of the Saudi Commission for Health Specialties")
