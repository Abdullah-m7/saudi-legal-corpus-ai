#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة مخالفات دعم صندوق تنمية الموارد البشرية \"هدف\" track.
...
RECORDS = os.path.join(ROOT, "sources", "hrdf_support_violations_regulation", "verified",
                       "hrdf_support_violations_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "hrdf_support_violations_regulation", "verified",
                       "hrdf_support_violations_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "hrdf_support_violations_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"hrdf_support_violations_regulation_no_decision_number_in_source", "hrdf_support_violations_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة مخالفات دعم صندوق تنمية الموارد البشرية \"هدف\"":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on Violations of Human Resources Development Fund (HADAF) Support track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on Violations of Human Resources Development Fund (HADAF) Support")
