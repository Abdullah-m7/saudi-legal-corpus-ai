#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط مكافآت أعضاء مجالس إدارة الأجهزة واللجان المنبثقة عنها track.
...
RECORDS = os.path.join(ROOT, "sources", "board_committee_remuneration_controls", "verified",
                       "board_committee_remuneration_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "board_committee_remuneration_controls", "verified",
                       "board_committee_remuneration_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "board_committee_remuneration_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"board_committee_remuneration_controls_no_decision_number_in_source", "board_committee_remuneration_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "6/3/1445":
        e.append("[2j] gazette_publication_date_hijri must be 6/3/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-09-21":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-09-21")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط مكافآت أعضاء مجالس إدارة الأجهزة واللجان المنبثقة عنها":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on Remuneration of Members of Boards of Government Bodies and Their Committees track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on Remuneration of Members of Boards of Government Bodies and Their Committees")
