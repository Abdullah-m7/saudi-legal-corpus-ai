#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة لخدمات المحاسبة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19481),
published 19/10/1443H (2022-05-20). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "accounting_services_rules", "official_source",
                   "accounting_services_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "accounting_services_rules", "verified")
RECORDS = os.path.join(OUT_VER, "accounting_services_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "accounting_services_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "accounting_services_rules_arabic_legal_llm",
                        "accounting_services_rules_legal_llm_001_018.json")

LAW_ID = "sa-accounting-services-rules"
LAW_AR = "القواعد المنظمة لخدمات المحاسبة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"accounting_services_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19481), published 19/10/1443H (2022-05-20). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "accounting_services_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "ACCOUNTING_SERVICES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "accounting-services-rules-llm-%03d" % idx,
            ...
            "article_path": "accounting_services_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing Accounting Services -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 19/10/1443H / 2022-05-20)"),
                "source_authority_ar": ("القواعد المنظمة لخدمات المحاسبة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 19/10/1443هـ الموافق 2022-05-20م)"),
    ...
    json.dump({
        "law_key": "accounting_services_rules",
        "layer": "ACCOUNTING_SERVICES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "accounting-services-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Rules Governing Accounting Services — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing Accounting Services records" % (len(ver), len(llm)))
