#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية لتصنيف الوسائط البحرية المستخدمة في السياحة الساحلية track.
...
RECORDS = os.path.join(ROOT, "sources", "coastal_tourism_craft_classification_regulation", "verified",
                       "coastal_tourism_craft_classification_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "coastal_tourism_craft_classification_regulation", "verified",
                       "coastal_tourism_craft_classification_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "coastal_tourism_craft_classification_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"coastal_tourism_craft_classification_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "17/1/1445":
        e.append("[2j] gazette_publication_date_hijri must be 17/1/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-08-04":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-08-04")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنظيمية لتصنيف الوسائط البحرية المستخدمة في السياحة الساحلية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for Classifying Maritime Craft Used in Coastal Tourism track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for Classifying Maritime Craft Used in Coastal Tourism")
