#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد الشركات في المناطق الاقتصادية الخاصة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4001347),
...
STRUCTURE: 108 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "sez_companies_rules", "official_source",
                   "sez_companies_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "sez_companies_rules", "verified")
RECORDS = os.path.join(OUT_VER, "sez_companies_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "sez_companies_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "sez_companies_rules_arabic_legal_llm",
                        "sez_companies_rules_legal_llm_001_108.json")

LAW_ID = "sa-sez-companies-rules"
LAW_AR = "قواعد الشركات في المناطق الاقتصادية الخاصة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"sez_companies_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4001347), published 25/1/1448H (2026-07-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "sez_companies_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SEZ_COMPANIES_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "sez-companies-rules-llm-%03d" % idx,
            ...
            "article_path": "sez_companies_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Companies Rules in the Special Economic Zones -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/1/1448H / 2026-07-10)"),
                "source_authority_ar": ("قواعد الشركات في المناطق الاقتصادية الخاصة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "sez_companies_rules",
        "layer": "SEZ_COMPANIES_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "sez-companies-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (108 مادة، جميعها أصلية)",
        "title_en": "Companies Rules in the Special Economic Zones — Arabic LLM-ready layer (108 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Companies Rules in the Special Economic Zones records" % (len(ver), len(llm)))
