#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم هيئة التراث track.
...
RECORDS = os.path.join(ROOT, "sources", "heritage_authority_statute", "verified",
                       "heritage_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "heritage_authority_statute", "verified",
                       "heritage_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "heritage_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"heritage_authority_statute_no_decision_number_in_source", "heritage_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم هيئة التراث":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Heritage Commission track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Heritage Commission")
