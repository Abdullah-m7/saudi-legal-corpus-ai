#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة الاقتصادية لخدمات النقل الجوي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23977),
...
SRC = os.path.join(ROOT, "sources", "air_transport_services_economic_regulation", "official_source",
                   "air_transport_services_economic_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "air_transport_services_economic_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "air_transport_services_economic_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "air_transport_services_economic_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "air_transport_services_economic_regulation_arabic_legal_llm",
                        "air_transport_services_economic_regulation_legal_llm_001_025.json")

LAW_ID = "sa-air-transport-services-economic-regulation"
LAW_AR = "اللائحة الاقتصادية لخدمات النقل الجوي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"air_transport_services_economic_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23977), published 15/4/1445H (2023-10-30). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "air_transport_services_economic_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "AIR_TRANSPORT_SERVICES_ECONOMIC_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "air-transport-services-economic-regulation-llm-%03d" % idx,
            ...
            "article_path": "air_transport_services_economic_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Economic Regulation for Air Transport Services -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/4/1445H / 2023-10-30)"),
                "source_authority_ar": ("اللائحة الاقتصادية لخدمات النقل الجوي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "air_transport_services_economic_regulation",
        "layer": "AIR_TRANSPORT_SERVICES_ECONOMIC_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "air-transport-services-economic-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (25 مادة، جميعها أصلية)",
        "title_en": "Economic Regulation for Air Transport Services — Arabic LLM-ready layer (25 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Economic Regulation for Air Transport Services records" % (len(ver), len(llm)))
