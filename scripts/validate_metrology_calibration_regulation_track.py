#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنظام القياس والمعايرة track.
...
RECORDS = os.path.join(ROOT, "sources", "metrology_calibration_regulation", "verified",
                       "metrology_calibration_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "metrology_calibration_regulation", "verified",
                       "metrology_calibration_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "metrology_calibration_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"metrology_calibration_regulation_no_decision_number_in_source", "metrology_calibration_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية لنظام القياس والمعايرة":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation of the Metrology and Calibration Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation of the Metrology and Calibration Law")
