#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الاتفاقية العربية لتنظيم نقل الركاب على الطرق بين الدول العربية وعبرها track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_arab_road_passenger_transport", "verified",
                       "treaty_arab_road_passenger_transport_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_arab_road_passenger_transport", "verified",
                       "treaty_arab_road_passenger_transport_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_arab_road_passenger_transport_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_arab_road_passenger_transport_no_decision_number_in_source", "treaty_arab_road_passenger_transport_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "19/7/1444":
        e.append("[2j] gazette_publication_date_hijri must be 19/7/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-02-10":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-02-10")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الاتفاقية العربية لتنظيم نقل الركاب على الطرق بين الدول العربية وعبرها":
    ...
    if e:
        print("FAIL: %d error(s) in Arab Agreement Regulating Road Passenger Transport between and across Arab States track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Arab Agreement Regulating Road Passenger Transport between and across Arab States")
