#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة لجنة النظر في مخالفات نظام السياحة ولوائحه track.
...
RECORDS = os.path.join(ROOT, "sources", "tourism_violations_committee_regulation", "verified",
                       "tourism_violations_committee_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "tourism_violations_committee_regulation", "verified",
                       "tourism_violations_committee_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "tourism_violations_committee_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"tourism_violations_committee_regulation_no_decision_number_in_source", "tourism_violations_committee_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "1/6/1444":
        e.append("[2j] gazette_publication_date_hijri must be 1/6/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-12-25":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-12-25")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة لجنة النظر في مخالفات نظام السياحة ولوائحه":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation of the Committee for Adjudicating Violations of the Tourism Law and its Regulations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation of the Committee for Adjudicating Violations of the Tourism Law and its Regulations")
