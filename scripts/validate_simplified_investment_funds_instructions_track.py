#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تعليمات صناديق الاستثمار المبسطة track.
...
RECORDS = os.path.join(ROOT, "sources", "simplified_investment_funds_instructions", "verified",
                       "simplified_investment_funds_instructions_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "simplified_investment_funds_instructions", "verified",
                       "simplified_investment_funds_instructions_verified_summary.json")
LLM = os.path.join(ROOT, "data", "simplified_investment_funds_instructions_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"simplified_investment_funds_instructions_no_decision_number_in_source", "simplified_investment_funds_instructions_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "28/11/1447":
        e.append("[2j] gazette_publication_date_hijri must be 28/11/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-05-15":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-05-15")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تعليمات صناديق الاستثمار المبسطة":
    ...
    if e:
        print("FAIL: %d error(s) in Instructions for Simplified Investment Funds track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Instructions for Simplified Investment Funds")
