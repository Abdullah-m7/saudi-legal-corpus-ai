#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the المعايير المهنية لتقييم أضرار المركبات track.
...
RECORDS = os.path.join(ROOT, "sources", "vehicle_damage_assessment_standards", "verified",
                       "vehicle_damage_assessment_standards_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "vehicle_damage_assessment_standards", "verified",
                       "vehicle_damage_assessment_standards_verified_summary.json")
LLM = os.path.join(ROOT, "data", "vehicle_damage_assessment_standards_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"vehicle_damage_assessment_standards_no_decision_number_in_source", "vehicle_damage_assessment_standards_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "23/11/1445":
        e.append("[2j] gazette_publication_date_hijri must be 23/11/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-05-31":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-05-31")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "المعايير المهنية لتقييم أضرار المركبات":
    ...
    if e:
        print("FAIL: %d error(s) in Professional Standards for Vehicle Damage Assessment track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Professional Standards for Vehicle Damage Assessment")
