#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية للمركز الوطني للتفتيش والرقابة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23960),
published 12/4/1445H (2023-10-27). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "national_inspection_control_center_statute", "official_source",
                   "national_inspection_control_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "national_inspection_control_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "national_inspection_control_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "national_inspection_control_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "national_inspection_control_center_statute_arabic_legal_llm",
                        "national_inspection_control_center_statute_legal_llm_001_015.json")

LAW_ID = "sa-national-inspection-control-center-statute"
LAW_AR = "الترتيبات التنظيمية للمركز الوطني للتفتيش والرقابة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"national_inspection_control_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23960), published 12/4/1445H (2023-10-27). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "national_inspection_control_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "NATIONAL_INSPECTION_CONTROL_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "national-inspection-control-center-statute-llm-%03d" % idx,
            ...
            "article_path": "national_inspection_control_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the National Center for Inspection and Control -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 12/4/1445H / 2023-10-27)"),
                "source_authority_ar": ("الترتيبات التنظيمية للمركز الوطني للتفتيش والرقابة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 12/4/1445هـ الموافق 2023-10-27م)"),
    ...
    json.dump({
        "law_key": "national_inspection_control_center_statute",
        "layer": "NATIONAL_INSPECTION_CONTROL_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "national-inspection-control-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the National Center for Inspection and Control — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the National Center for Inspection and Control records" % (len(ver), len(llm)))
