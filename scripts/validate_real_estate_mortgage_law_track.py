#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Registered Real Estate Mortgage Law track (46 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) EXCEPT the 2 documented long articles (11, 14) whose PDF
text-layer reordered/split clauses — these were adjudicated VISUALLY VERBATIM on
the rendered official PDF pages and are flagged MATCHES_PDF_VISUALLY_ADJUDICATED;
articles are numbered by ordinal position 1..46 (no مكرر); and every article
...
(no dual-status divergence). FRESH FULL ISSUANCE: all 46 اصلية; none amended,
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_mortgage", "law", "verified",
                       "real_estate_mortgage_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "real_estate_mortgage_arabic_legal_llm",
                   "real_estate_mortgage_law_legal_llm_001_046.json")
PDF = os.path.join(ROOT, "inputs", "real_estate_mortgage_official_pdfs",
                   "real_estate_mortgage_law_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 46
KEY_RE = r"real_estate_mortgage_art_(\d{3})$"
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 46}
VISUALLY_ADJUDICATED = {"real_estate_mortgage_art_011", "real_estate_mortgage_art_014"}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Registered Real Estate Mortgage Law track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Registered Real Estate Mortgage Law — 46 records (fresh issuance: all 46 اصلية)")
    print("  - trust gate: 44/46 MATCHES_PDF outright; the 2 long articles (11, 14) adjudicated visually verbatim")
    print("  - numbered 1..46 by ordinal position (no مكرر); no dual-status divergence")
    print("  - IN-FORCE Royal Decree M/49 (13/8/1433H); committed MOJ PDF hash verified; Arabic governs")
    return 0
