#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة تراخيص الحفريات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24563),
...
SRC = os.path.join(ROOT, "sources", "excavation_permits_regulation", "official_source",
                   "excavation_permits_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "excavation_permits_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "excavation_permits_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "excavation_permits_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "excavation_permits_regulation_arabic_legal_llm",
                        "excavation_permits_regulation_legal_llm_001_014.json")

LAW_ID = "sa-excavation-permits-regulation"
LAW_AR = "لائحة تراخيص الحفريات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"excavation_permits_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24563), published 13/8/1445H (2024-02-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "excavation_permits_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "EXCAVATION_PERMITS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "excavation-permits-regulation-llm-%03d" % idx,
            ...
            "article_path": "excavation_permits_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Excavation Permits Regulation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/8/1445H / 2024-02-23)"),
                "source_authority_ar": ("لائحة تراخيص الحفريات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "excavation_permits_regulation",
        "layer": "EXCAVATION_PERMITS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "excavation-permits-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Excavation Permits Regulation — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Excavation Permits Regulation records" % (len(ver), len(llm)))
