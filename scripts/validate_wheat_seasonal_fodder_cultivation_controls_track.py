#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط زراعة القمح والأعلاف الموسمية track.
...
RECORDS = os.path.join(ROOT, "sources", "wheat_seasonal_fodder_cultivation_controls", "verified",
                       "wheat_seasonal_fodder_cultivation_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "wheat_seasonal_fodder_cultivation_controls", "verified",
                       "wheat_seasonal_fodder_cultivation_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "wheat_seasonal_fodder_cultivation_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"wheat_seasonal_fodder_cultivation_controls_no_decision_number_in_source", "wheat_seasonal_fodder_cultivation_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "22/4/1446":
        e.append("[2j] gazette_publication_date_hijri must be 22/4/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-10-25":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-10-25")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط زراعة القمح والأعلاف الموسمية":
    ...
    if e:
        print("FAIL: %d error(s) in Controls for the Cultivation of Wheat and Seasonal Fodder track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls for the Cultivation of Wheat and Seasonal Fodder")
