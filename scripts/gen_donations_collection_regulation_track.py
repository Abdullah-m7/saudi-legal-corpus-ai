#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام جمع التبرعات track.
...
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4000948),
...
SRC = os.path.join(ROOT, "sources", "donations_collection_regulation", "official_source",
                   "donations_collection_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "donations_collection_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "donations_collection_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "donations_collection_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "donations_collection_regulation_arabic_legal_llm",
                        "donations_collection_regulation_legal_llm_001_017.json")

LAW_ID = "sa-donations-collection-regulation"
LAW_AR = "اللائحة التنفيذية لنظام جمع التبرعات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"donations_collection_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4000948), published 28/11/1447H (2026-05-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "donations_collection_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "DONATIONS_COLLECTION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "donations-collection-regulation-llm-%03d" % idx,
            ...
            "article_path": "donations_collection_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Donations Collection Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/11/1447H / 2026-05-15)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام جمع التبرعات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "donations_collection_regulation",
        "layer": "DONATIONS_COLLECTION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "donations-collection-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Donations Collection Law — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Donations Collection Law records" % (len(ver), len(llm)))
