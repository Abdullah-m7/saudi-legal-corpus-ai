#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد الفحص والرقابة والضبط track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26569),
...
SRC = os.path.join(ROOT, "sources", "inspection_control_seizure_rules", "official_source",
                   "inspection_control_seizure_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "inspection_control_seizure_rules", "verified")
RECORDS = os.path.join(OUT_VER, "inspection_control_seizure_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "inspection_control_seizure_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "inspection_control_seizure_rules_arabic_legal_llm",
                        "inspection_control_seizure_rules_legal_llm_001_027.json")

LAW_ID = "sa-inspection-control-seizure-rules"
LAW_AR = "قواعد الفحص والرقابة والضبط"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"inspection_control_seizure_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26569), published 1/4/1446H (2024-10-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "inspection_control_seizure_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "INSPECTION_CONTROL_SEIZURE_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "inspection-control-seizure-rules-llm-%03d" % idx,
            ...
            "article_path": "inspection_control_seizure_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules of Inspection, Supervision and Seizure -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/4/1446H / 2024-10-04)"),
                "source_authority_ar": ("قواعد الفحص والرقابة والضبط — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "inspection_control_seizure_rules",
        "layer": "INSPECTION_CONTROL_SEIZURE_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "inspection-control-seizure-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (27 مادة، جميعها أصلية)",
        "title_en": "Rules of Inspection, Supervision and Seizure — Arabic LLM-ready layer (27 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules of Inspection, Supervision and Seizure records" % (len(ver), len(llm)))
