#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Commercial Books Law track (16 records).

Trust gate (BOE-archive provenance): the numbered sequence is a complete 1..16
(no مكرر); every article is اصلية (fresh full issuance by Royal Decree M/61,
1409H) consistent with its is_repealed/is_amended/is_added flags (all False); the
...
RECORDS = os.path.join(ROOT, "sources", "commercial_books", "law", "verified",
                       "commercial_books_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "commercial_books_arabic_legal_llm",
                   "commercial_books_law_legal_llm_001_016.json")
STATUS = "BOE_OFFICIAL_PORTAL_ARCHIVE_CROSS_SNAPSHOT_VERIFIED"
N = 16
KEY_RE = r"commercial_books_art_(\d{3})"
LAW_NAME = "Commercial Books Law"
...
def main():
    ...
    print("    (byte-identical) + corpus text re-hash to recorded sha256; Royal Decree M/61 (1409H); ساري")
