#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة لعمل أمناء الإفلاس والخبراء track.
...
RECORDS = os.path.join(ROOT, "sources", "bankruptcy_trustees_experts_rules", "verified",
                       "bankruptcy_trustees_experts_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "bankruptcy_trustees_experts_rules", "verified",
                       "bankruptcy_trustees_experts_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "bankruptcy_trustees_experts_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"bankruptcy_trustees_experts_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد المنظمة لعمل أمناء الإفلاس والخبراء":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Governing the Work of Bankruptcy Trustees and Experts track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Governing the Work of Bankruptcy Trustees and Experts")
