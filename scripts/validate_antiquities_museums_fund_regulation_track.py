#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لصندوق الآثار والمتاحف والتراث العمراني track.
...
RECORDS = os.path.join(ROOT, "sources", "antiquities_museums_fund_regulation", "verified",
                       "antiquities_museums_fund_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "antiquities_museums_fund_regulation", "verified",
                       "antiquities_museums_fund_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "antiquities_museums_fund_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"antiquities_museums_fund_regulation_no_decision_number_in_source", "antiquities_museums_fund_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية لصندوق الآثار والمتاحف والتراث العمراني":
    ...
    if e:
        print("FAIL: %d error(s) in Implementing Regulation of the Antiquities, Museums and Urban Heritage Fund track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Implementing Regulation of the Antiquities, Museums and Urban Heritage Fund")
