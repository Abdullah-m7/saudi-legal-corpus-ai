#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام الشركات الخاصة بشركات المساهمة المدرجة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4001295),
...
STRUCTURE: 69 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "listed_jsc_companies_regulation", "official_source",
                   "listed_jsc_companies_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "listed_jsc_companies_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "listed_jsc_companies_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "listed_jsc_companies_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "listed_jsc_companies_regulation_arabic_legal_llm",
                        "listed_jsc_companies_regulation_legal_llm_001_069.json")

LAW_ID = "sa-listed-jsc-companies-regulation"
LAW_AR = "اللائحة التنفيذية لنظام الشركات الخاصة بشركات المساهمة المدرجة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"listed_jsc_companies_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4001295), published 18/1/1448H (2026-07-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "listed_jsc_companies_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "LISTED_JSC_COMPANIES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "listed-jsc-companies-regulation-llm-%03d" % idx,
            ...
            "article_path": "listed_jsc_companies_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Companies Law for Listed Joint Stock Companies -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 18/1/1448H / 2026-07-03)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام الشركات الخاصة بشركات المساهمة المدرجة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "listed_jsc_companies_regulation",
        "layer": "LISTED_JSC_COMPANIES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "listed-jsc-companies-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (69 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Companies Law for Listed Joint Stock Companies — Arabic LLM-ready layer (69 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Companies Law for Listed Joint Stock Companies records" % (len(ver), len(llm)))
