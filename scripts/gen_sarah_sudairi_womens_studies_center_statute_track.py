#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مركز سارة السديري لدراسات المرأة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21610),
published 2/9/1444H (2023-03-24). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "sarah_sudairi_womens_studies_center_statute", "official_source",
                   "sarah_sudairi_womens_studies_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "sarah_sudairi_womens_studies_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "sarah_sudairi_womens_studies_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "sarah_sudairi_womens_studies_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "sarah_sudairi_womens_studies_center_statute_arabic_legal_llm",
                        "sarah_sudairi_womens_studies_center_statute_legal_llm_001_012.json")

LAW_ID = "sa-sarah-sudairi-womens-studies-center-statute"
LAW_AR = "تنظيم مركز سارة السديري لدراسات المرأة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"sarah_sudairi_womens_studies_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21610), published 2/9/1444H (2023-03-24). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "sarah_sudairi_womens_studies_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "SARAH_SUDAIRI_WOMENS_STUDIES_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "sarah-sudairi-womens-studies-center-statute-llm-%03d" % idx,
            ...
            "article_path": "sarah_sudairi_womens_studies_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Sarah Al-Sudairi Center for Women's Studies -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/9/1444H / 2023-03-24)"),
                "source_authority_ar": ("تنظيم مركز سارة السديري لدراسات المرأة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 2/9/1444هـ الموافق 2023-03-24م)"),
    ...
    json.dump({
        "law_key": "sarah_sudairi_womens_studies_center_statute",
        "layer": "SARAH_SUDAIRI_WOMENS_STUDIES_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "sarah-sudairi-womens-studies-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Statute of the Sarah Al-Sudairi Center for Women's Studies — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Sarah Al-Sudairi Center for Women's Studies records" % (len(ver), len(llm)))
