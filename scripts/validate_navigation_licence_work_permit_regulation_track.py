#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة المنظمة لإصدار الترخيص الملاحي ورخصة العمل track.
...
RECORDS = os.path.join(ROOT, "sources", "navigation_licence_work_permit_regulation", "verified",
                       "navigation_licence_work_permit_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "navigation_licence_work_permit_regulation", "verified",
                       "navigation_licence_work_permit_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "navigation_licence_work_permit_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"navigation_licence_work_permit_regulation_no_decision_number_in_source", "navigation_licence_work_permit_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "25/6/1443":
        e.append("[2j] gazette_publication_date_hijri must be 25/6/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-01-28":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-01-28")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة المنظمة لإصدار الترخيص الملاحي ورخصة العمل":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation Governing the Issuance of the Navigation Licence and the Work Permit track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation Governing the Issuance of the Navigation Licence and the Work Permit")
