#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الاستشارات السياحية track.
...
RECORDS = os.path.join(ROOT, "sources", "tourism_consultancy_regulation", "verified",
                       "tourism_consultancy_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "tourism_consultancy_regulation", "verified",
                       "tourism_consultancy_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "tourism_consultancy_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"tourism_consultancy_regulation_no_decision_number_in_source", "tourism_consultancy_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة الاستشارات السياحية":
    ...
    if e:
        print("FAIL: %d error(s) in Tourism Consultancy Regulation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Tourism Consultancy Regulation")
