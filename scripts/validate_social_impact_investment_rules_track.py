#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد تفعيل الاستثمار ذي الأثر الاجتماعي track.
...
RECORDS = os.path.join(ROOT, "sources", "social_impact_investment_rules", "verified",
                       "social_impact_investment_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "social_impact_investment_rules", "verified",
                       "social_impact_investment_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "social_impact_investment_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"social_impact_investment_rules_no_decision_number_in_source", "social_impact_investment_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد تفعيل الاستثمار ذي الأثر الاجتماعي":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Activating Social Impact Investment track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Activating Social Impact Investment")
