#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام الحرف والصناعات اليدوية track.

CITATION: القرار الوزاري رقم (2347/ق) وتاريخ 10/08/1447هـ
Text fetched directly from the Umm Al-Qura Official Gazette's own
server-rendered HTML page (uqn.gov.sa/decisions-and-regulations/4000428),
...
SRC = os.path.join(ROOT, "sources", "handicrafts_regulation", "official_source",
                   "handicrafts_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "handicrafts_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "handicrafts_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "handicrafts_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "handicrafts_regulation_arabic_legal_llm",
                        "handicrafts_regulation_legal_llm_001_014.json")

LAW_ID = "sa-handicrafts-regulation"
LAW_AR = "اللائحة التنفيذية لنظام الحرف والصناعات اليدوية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"handicrafts_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/decisions-and-regulations/4000428), published 3/9/1447H (2026-02-20). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "handicrafts_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "HANDICRAFTS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "handicrafts-regulation-llm-%03d" % idx,
            ...
            "article_path": "handicrafts_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Handicrafts and Handmade Industries Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/9/1447H / 2026-02-20)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام الحرف والصناعات اليدوية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "handicrafts_regulation",
        "layer": "HANDICRAFTS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "handicrafts-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Handicrafts and Handmade Industries Law — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Handicrafts and Handmade Industries Law records" % (len(ver), len(llm)))
