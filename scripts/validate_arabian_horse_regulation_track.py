#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية للخيل العربية الأصيلة track.
...
RECORDS = os.path.join(ROOT, "sources", "arabian_horse_regulation", "verified",
                       "arabian_horse_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "arabian_horse_regulation", "verified",
                       "arabian_horse_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "arabian_horse_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"arabian_horse_regulation_no_decision_number_in_source", "arabian_horse_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية للخيل العربية الأصيلة":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for the Purebred Arabian Horse track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for the Purebred Arabian Horse")
