#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط استئجار الجهات الحكومية للعقارات خارج المملكة العربية السعودية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26970),
...
SRC = os.path.join(ROOT, "sources", "government_foreign_property_lease_controls", "official_source",
                   "government_foreign_property_lease_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "government_foreign_property_lease_controls", "verified")
RECORDS = os.path.join(OUT_VER, "government_foreign_property_lease_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "government_foreign_property_lease_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "government_foreign_property_lease_controls_arabic_legal_llm",
                        "government_foreign_property_lease_controls_legal_llm_001_018.json")

LAW_ID = "sa-government-foreign-property-lease-controls"
LAW_AR = "ضوابط استئجار الجهات الحكومية للعقارات خارج المملكة العربية السعودية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"government_foreign_property_lease_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26970), published 1/8/1446H (2025-01-31). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "government_foreign_property_lease_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "GOVERNMENT_FOREIGN_PROPERTY_LEASE_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "government-foreign-property-lease-controls-llm-%03d" % idx,
            ...
            "article_path": "government_foreign_property_lease_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on the Leasing of Real Estate Abroad by Government Entities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/8/1446H / 2025-01-31)"),
                "source_authority_ar": ("ضوابط استئجار الجهات الحكومية للعقارات خارج المملكة العربية السعودية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "government_foreign_property_lease_controls",
        "layer": "GOVERNMENT_FOREIGN_PROPERTY_LEASE_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "government-foreign-property-lease-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Controls on the Leasing of Real Estate Abroad by Government Entities — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on the Leasing of Real Estate Abroad by Government Entities records" % (len(ver), len(llm)))
