#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الفنية لأجهزة الاتصالات وتقنية المعلومات track.
...
RECORDS = os.path.join(ROOT, "sources", "ict_devices_technical_regulation", "verified",
                       "ict_devices_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "ict_devices_technical_regulation", "verified",
                       "ict_devices_technical_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "ict_devices_technical_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"ict_devices_technical_regulation_no_decision_number_in_source", "ict_devices_technical_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة الفنية لأجهزة الاتصالات وتقنية المعلومات":
    ...
    if e:
        print("FAIL: %d error(s) in Technical Regulation for Communications and Information Technology Devices track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Technical Regulation for Communications and Information Technology Devices")
