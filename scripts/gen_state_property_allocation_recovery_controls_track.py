#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط تخصيص عقارات الدولة واستردادها track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26570),
...
SRC = os.path.join(ROOT, "sources", "state_property_allocation_recovery_controls", "official_source",
                   "state_property_allocation_recovery_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "state_property_allocation_recovery_controls", "verified")
RECORDS = os.path.join(OUT_VER, "state_property_allocation_recovery_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "state_property_allocation_recovery_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "state_property_allocation_recovery_controls_arabic_legal_llm",
                        "state_property_allocation_recovery_controls_legal_llm_001_018.json")

LAW_ID = "sa-state-property-allocation-recovery-controls"
LAW_AR = "ضوابط تخصيص عقارات الدولة واستردادها"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"state_property_allocation_recovery_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26570), published 1/4/1446H (2024-10-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "state_property_allocation_recovery_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "STATE_PROPERTY_ALLOCATION_RECOVERY_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "state-property-allocation-recovery-controls-llm-%03d" % idx,
            ...
            "article_path": "state_property_allocation_recovery_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on the Allocation and Recovery of State Real Estate -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/4/1446H / 2024-10-04)"),
                "source_authority_ar": ("ضوابط تخصيص عقارات الدولة واستردادها — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "state_property_allocation_recovery_controls",
        "layer": "STATE_PROPERTY_ALLOCATION_RECOVERY_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "state-property-allocation-recovery-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Controls on the Allocation and Recovery of State Real Estate — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on the Allocation and Recovery of State Real Estate records" % (len(ver), len(llm)))
