#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الإرشاد السياحي track.
...
RECORDS = os.path.join(ROOT, "sources", "tourist_guidance_regulation", "verified",
                       "tourist_guidance_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "tourist_guidance_regulation", "verified",
                       "tourist_guidance_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "tourist_guidance_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"tourist_guidance_regulation_no_decision_number_in_source", "tourist_guidance_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة الإرشاد السياحي":
    ...
    if e:
        print("FAIL: %d error(s) in Tourist Guidance Regulation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Tourist Guidance Regulation")
