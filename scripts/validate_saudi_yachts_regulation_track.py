#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية لليخوت السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "saudi_yachts_regulation", "verified",
                       "saudi_yachts_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "saudi_yachts_regulation", "verified",
                       "saudi_yachts_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "saudi_yachts_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"saudi_yachts_regulation_no_decision_number_in_source", "saudi_yachts_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "2/3/1446":
        e.append("[2j] gazette_publication_date_hijri must be 2/3/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-09-05":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-09-05")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنظيمية لليخوت السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for Saudi Yachts track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for Saudi Yachts")
