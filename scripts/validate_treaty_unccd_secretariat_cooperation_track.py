#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية للتعاون بين وزارة البيئة والمياه والزراعة في المملكة وأمانة اتفاقية الأمم المتحدة لمكافحة التصحر track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_unccd_secretariat_cooperation", "verified",
                       "treaty_unccd_secretariat_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_unccd_secretariat_cooperation", "verified",
                       "treaty_unccd_secretariat_cooperation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_unccd_secretariat_cooperation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_unccd_secretariat_cooperation_no_decision_number_in_source", "treaty_unccd_secretariat_cooperation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "15/8/1443":
        e.append("[2j] gazette_publication_date_hijri must be 15/8/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-03-18":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-03-18")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية للتعاون بين وزارة البيئة والمياه والزراعة في المملكة وأمانة اتفاقية الأمم المتحدة لمكافحة التصحر":
    ...
    if e:
        print("FAIL: %d error(s) in Cooperation Agreement between MEWA and the UNCCD Secretariat track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Cooperation Agreement between MEWA and the UNCCD Secretariat")
