#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم إعانة البحث عن عمل track.
...
RECORDS = os.path.join(ROOT, "sources", "job_seeker_allowance_statute", "verified",
                       "job_seeker_allowance_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "job_seeker_allowance_statute", "verified",
                       "job_seeker_allowance_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "job_seeker_allowance_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"job_seeker_allowance_statute_no_decision_number_in_source", "job_seeker_allowance_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم إعانة البحث عن عمل":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Job Search Allowance track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Job Search Allowance")
