#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية للمتاحف track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19024),
...
STRUCTURE: 91 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "museums_regulation", "official_source",
                   "museums_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "museums_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "museums_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "museums_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "museums_regulation_arabic_legal_llm",
                        "museums_regulation_legal_llm_001_091.json")

LAW_ID = "sa-museums-regulation"
LAW_AR = "اللائحة التنفيذية للمتاحف"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"museums_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19024), published 24/7/1443H (2022-02-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "museums_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MUSEUMS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "museums-regulation-llm-%03d" % idx,
            ...
            "article_path": "museums_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation for Museums -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/7/1443H / 2022-02-25)"),
                "source_authority_ar": ("اللائحة التنفيذية للمتاحف — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "museums_regulation",
        "layer": "MUSEUMS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "museums-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (91 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation for Museums — Arabic LLM-ready layer (91 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation for Museums records" % (len(ver), len(llm)))
