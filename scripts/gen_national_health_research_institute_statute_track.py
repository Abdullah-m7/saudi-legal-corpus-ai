#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المعهد الوطني لأبحاث الصحة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27537),
...
SRC = os.path.join(ROOT, "sources", "national_health_research_institute_statute", "official_source",
                   "national_health_research_institute_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "national_health_research_institute_statute", "verified")
RECORDS = os.path.join(OUT_VER, "national_health_research_institute_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "national_health_research_institute_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "national_health_research_institute_statute_arabic_legal_llm",
                        "national_health_research_institute_statute_legal_llm_001_015.json")

LAW_ID = "sa-national-health-research-institute-statute"
LAW_AR = "تنظيم المعهد الوطني لأبحاث الصحة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"national_health_research_institute_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27537), published 25/4/1447H (2025-10-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "national_health_research_institute_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "NATIONAL_HEALTH_RESEARCH_INSTITUTE_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "national-health-research-institute-statute-llm-%03d" % idx,
            ...
            "article_path": "national_health_research_institute_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the National Institute for Health Research -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/4/1447H / 2025-10-17)"),
                "source_authority_ar": ("تنظيم المعهد الوطني لأبحاث الصحة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "national_health_research_institute_statute",
        "layer": "NATIONAL_HEALTH_RESEARCH_INSTITUTE_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "national-health-research-institute-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Statute of the National Institute for Health Research — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the National Institute for Health Research records" % (len(ver), len(llm)))
