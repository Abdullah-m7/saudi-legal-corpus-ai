#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الفنية للحد من المواد الخطرة في الأجهزة والمعدات الكهربائية والإلكترونية track.
...
RECORDS = os.path.join(ROOT, "sources", "hazardous_substances_electrical_equipment_regulation", "verified",
                       "hazardous_substances_electrical_equipment_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "hazardous_substances_electrical_equipment_regulation", "verified",
                       "hazardous_substances_electrical_equipment_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "hazardous_substances_electrical_equipment_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"hazardous_substances_electrical_equipment_regulation_no_decision_number_in_source", "hazardous_substances_electrical_equipment_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "29/11/1442":
        e.append("[2j] gazette_publication_date_hijri must be 29/11/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-07-09":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-07-09")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة الفنية للحد من المواد الخطرة في الأجهزة والمعدات الكهربائية والإلكترونية":
    ...
    if e:
        print("FAIL: %d error(s) in Technical Regulation on the Restriction of Hazardous Substances in Electrical and Electronic Equipment track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Technical Regulation on the Restriction of Hazardous Substances in Electrical and Electronic Equipment")
