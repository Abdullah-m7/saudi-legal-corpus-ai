#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام صندوق التنمية العقارية الصادر بالمرسوم الملكي رقم (م/47) وتاريخ 23 /05/ 1442ه track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17566),
published 25/9/1442H (2021-05-07). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "real_estate_development_fund_implementing_regulation", "official_source",
                   "real_estate_development_fund_implementing_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "real_estate_development_fund_implementing_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "real_estate_development_fund_implementing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "real_estate_development_fund_implementing_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "real_estate_development_fund_implementing_regulation_arabic_legal_llm",
                        "real_estate_development_fund_implementing_regulation_legal_llm_001_025.json")

LAW_ID = "sa-real-estate-development-fund-implementing-regulation"
LAW_AR = "اللائحة التنفيذية لنظام صندوق التنمية العقارية الصادر بالمرسوم الملكي رقم (م/47) وتاريخ 23 /05/ 1442ه"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"real_estate_development_fund_implementing_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17566), published 25/9/1442H (2021-05-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "real_estate_development_fund_implementing_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "REAL_ESTATE_DEVELOPMENT_FUND_IMPLEMENTING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "real-estate-development-fund-implementing-regulation-llm-%03d" % idx,
            ...
            "article_path": "real_estate_development_fund_implementing_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Real Estate Development Fund Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/9/1442H / 2021-05-07)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام صندوق التنمية العقارية الصادر بالمرسوم الملكي رقم (م/47) وتاريخ 23 /05/ 1442ه — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 25/9/1442هـ الموافق 2021-05-07م)"),
    ...
    json.dump({
        "law_key": "real_estate_development_fund_implementing_regulation",
        "layer": "REAL_ESTATE_DEVELOPMENT_FUND_IMPLEMENTING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "real-estate-development-fund-implementing-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (25 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Real Estate Development Fund Law — Arabic LLM-ready layer (25 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Real Estate Development Fund Law records" % (len(ver), len(llm)))
