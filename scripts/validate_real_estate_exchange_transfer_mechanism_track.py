#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the آلية نقل أصول البورصة العقارية وموظفيها ومشاريعها ومبادراتها إلى الهيئة العامة للعقار track.

11 records: 8 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_exchange_transfer_mechanism", "verified",
                       "real_estate_exchange_transfer_mechanism_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "real_estate_exchange_transfer_mechanism", "verified",
                       "real_estate_exchange_transfer_mechanism_verified_summary.json")
LLM = os.path.join(ROOT, "data", "real_estate_exchange_transfer_mechanism_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "real_estate_exchange_transfer_mechanism_source_is_gazette_html_only",
}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "آلية نقل أصول البورصة العقارية وموظفيها ومشاريعها ومبادراتها إلى الهيئة العامة للعقار":
    ...
    if e:
        print("FAIL: %d error(s) in Mechanism for Transferring the Real Estate Exchange to the Real Estate General Authority track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Mechanism for Transferring the Real Estate Exchange to the Real Estate General Authority")
