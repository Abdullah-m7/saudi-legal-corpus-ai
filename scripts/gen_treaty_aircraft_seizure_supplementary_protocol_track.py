#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the البروتوكول المكمل لاتفاقية قمع الاستيلاء غير المشروع على الطائرات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17319),
published 20/8/1442H (2021-04-02). The Gazette IS the official publication of record
...
STRUCTURE: 24 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "treaty_aircraft_seizure_supplementary_protocol", "official_source",
                   "treaty_aircraft_seizure_supplementary_protocol_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_aircraft_seizure_supplementary_protocol", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_aircraft_seizure_supplementary_protocol_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_aircraft_seizure_supplementary_protocol_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_aircraft_seizure_supplementary_protocol_arabic_legal_llm",
                        "treaty_aircraft_seizure_supplementary_protocol_legal_llm_001_025.json")

LAW_ID = "sa-treaty-aircraft-seizure-supplementary-protocol"
LAW_AR = "البروتوكول المكمل لاتفاقية قمع الاستيلاء غير المشروع على الطائرات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_aircraft_seizure_supplementary_protocol_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17319), published 20/8/1442H (2021-04-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_aircraft_seizure_supplementary_protocol",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_AIRCRAFT_SEIZURE_SUPPLEMENTARY_PROTOCOL_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-aircraft-seizure-supplementary-protocol-llm-%03d" % idx,
            ...
            "article_path": "treaty_aircraft_seizure_supplementary_protocol/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Supplementary Protocol to the Convention for the Suppression of Unlawful Seizure of Aircraft -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/8/1442H / 2021-04-02)"),
                "source_authority_ar": ("البروتوكول المكمل لاتفاقية قمع الاستيلاء غير المشروع على الطائرات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 20/8/1442هـ الموافق 2021-04-02م)"),
    ...
    json.dump({
        "law_key": "treaty_aircraft_seizure_supplementary_protocol",
        "layer": "TREATY_AIRCRAFT_SEIZURE_SUPPLEMENTARY_PROTOCOL_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-aircraft-seizure-supplementary-protocol-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (24 مادة، جميعها أصلية)",
        "title_en": "Supplementary Protocol to the Convention for the Suppression of Unlawful Seizure of Aircraft — Arabic LLM-ready layer (24 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Supplementary Protocol to the Convention for the Suppression of Unlawful Seizure of Aircraft records" % (len(ver), len(llm)))
