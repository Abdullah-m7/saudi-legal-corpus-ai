#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية لاهاي بشأن إلغاء إلزامية المصادقة على الوثائق العمومية الأجنبية «أبوستيل» track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18816),
published 4/6/1443H (2022-01-07). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "hague_apostille_convention", "official_source",
                   "hague_apostille_convention_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "hague_apostille_convention", "verified")
RECORDS = os.path.join(OUT_VER, "hague_apostille_convention_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "hague_apostille_convention_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "hague_apostille_convention_arabic_legal_llm",
                        "hague_apostille_convention_legal_llm_001_015.json")

LAW_ID = "sa-hague-apostille-convention"
LAW_AR = "اتفاقية لاهاي بشأن إلغاء إلزامية المصادقة على الوثائق العمومية الأجنبية «أبوستيل»"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"hague_apostille_convention_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18816), published 4/6/1443H (2022-01-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "hague_apostille_convention",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "HAGUE_APOSTILLE_CONVENTION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "hague-apostille-convention-llm-%03d" % idx,
            ...
            "article_path": "hague_apostille_convention/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Hague Convention Abolishing the Requirement of Legalisation for Foreign Public Documents -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/6/1443H / 2022-01-07)"),
                "source_authority_ar": ("اتفاقية لاهاي بشأن إلغاء إلزامية المصادقة على الوثائق العمومية الأجنبية «أبوستيل» — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 4/6/1443هـ الموافق 2022-01-07م)"),
    ...
    json.dump({
        "law_key": "hague_apostille_convention",
        "layer": "HAGUE_APOSTILLE_CONVENTION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "hague-apostille-convention-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Hague Convention Abolishing the Requirement of Legalisation for Foreign Public Documents — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Hague Convention Abolishing the Requirement of Legalisation for Foreign Public Documents records" % (len(ver), len(llm)))
