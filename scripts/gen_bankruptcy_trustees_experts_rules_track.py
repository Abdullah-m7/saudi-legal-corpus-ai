#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة لعمل أمناء الإفلاس والخبراء track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17399),
...
SRC = os.path.join(ROOT, "sources", "bankruptcy_trustees_experts_rules", "official_source",
                   "bankruptcy_trustees_experts_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "bankruptcy_trustees_experts_rules", "verified")
RECORDS = os.path.join(OUT_VER, "bankruptcy_trustees_experts_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "bankruptcy_trustees_experts_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "bankruptcy_trustees_experts_rules_arabic_legal_llm",
                        "bankruptcy_trustees_experts_rules_legal_llm_001_040.json")

LAW_ID = "sa-bankruptcy-trustees-experts-rules"
LAW_AR = "القواعد المنظمة لعمل أمناء الإفلاس والخبراء"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"bankruptcy_trustees_experts_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17399), published 24/5/1442H (2021-01-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "bankruptcy_trustees_experts_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "BANKRUPTCY_TRUSTEES_EXPERTS_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "bankruptcy-trustees-experts-rules-llm-%03d" % idx,
            ...
            "article_path": "bankruptcy_trustees_experts_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing the Work of Bankruptcy Trustees and Experts -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/5/1442H / 2021-01-08)"),
                "source_authority_ar": ("القواعد المنظمة لعمل أمناء الإفلاس والخبراء — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "bankruptcy_trustees_experts_rules",
        "layer": "BANKRUPTCY_TRUSTEES_EXPERTS_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "bankruptcy-trustees-experts-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (40 مادة، جميعها أصلية)",
        "title_en": "Rules Governing the Work of Bankruptcy Trustees and Experts — Arabic LLM-ready layer (40 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing the Work of Bankruptcy Trustees and Experts records" % (len(ver), len(llm)))
