#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate Chinese remediation Batch P1-003 (P1 retranslation batch; 20 articles, Babs 6/7/8/10).
...
(English guidance only). Confirms the batch covers exactly the 20 authorized P1-003 articles with
verbatim-hashed internal Chinese text, that every record's bab is in [6,7,8,10] and equals the
...
(all P0 batches + their QA, P1-001 + P1-001 QA, P1-002 + P1-002 QA, the Chinese candidate 189, and the
base corpora). No p1_004+/P2/P3 batch dirs, no full Chinese 281 layer, no trilingual alignment.
Read-only and idempotent.

Usage: validate_chinese_remediation_batch_p1_003.py [DATA_JSON_PATH]
...
DATA_DEFAULT = os.path.join(ROOT, "data", "chinese_remediation_batches", "p1_003",
...
def main(argv=None) -> int:
    ...
    if d.get("stage") != "CHINESE_REMEDIATION_BATCH_P1_003":
        problems.append("stage must be CHINESE_REMEDIATION_BATCH_P1_003")
    if d.get("batch_id") != "P1-003":
    ...
    if d.get("remediation_track") != "P1_retranslation_or_manual_review":
        problems.append("remediation_track must be P1_retranslation_or_manual_review")
    if d.get("governing_text_language") != "ar":
        problems.append("governing_text_language must be ar")
    if d.get("remediation_action") != "retranslate_internal_chinese_from_arabic":
        problems.append("remediation_action must be retranslate_internal_chinese_from_arabic")
    ...
    if d.get("expected_babs") != [6, 7, 8, 10]:
        problems.append("expected_babs must be [6, 7, 8, 10]")
    if d.get("scope_articles") != ARTS:
        problems.append("scope_articles must exactly match the authorized P1-003 list")
    ...
    if d.get("source_basis") != "official_source_based":
    ...
    if nums != ARTS:
        problems.append("record article numbers must be exactly the P1-003 list, no extras")
    ...
    req_rec = ("article_number", "bab", "article_title_ar", "arabic_source_file",
               ...
               "prior_candidate_record_id", "prior_candidate_hash_sha256", "qa_priority",
               "qa_recommended_action", "remediated_chinese_text", "remediated_chinese_text_hash_sha256",
    ...
    for r in recs:
        ...
        if r.get("remediation_action") != "retranslate_internal_chinese_from_arabic":
        ...
        if r.get("source_basis") != "official_source_based":
            problems.append("art %s source_basis must be official_source_based" % n)
        ...
        # link to the (unchanged) prior candidate record and the P1 semantic-QA finding
        ...
        if n in qa and r.get("qa_priority") != qa[n].get("priority"):
            problems.append("art %s qa_priority != semantic-QA priority" % n)
        if n in qa and qa[n].get("priority") != "P1":
            problems.append("art %s is not a P1 article in the semantic-QA report" % n)
        if n in qa and r.get("qa_recommended_action") != qa[n].get("recommended_action"):
            problems.append("art %s qa_recommended_action != semantic-QA recommended_action" % n)
    ...
    # P1-001 and P1-002 remediation batches must remain intact (20 records; repository-owner review model)
    for label, path in (("P1-001", P1_001_BATCH), ("P1-002", P1_002_BATCH)):
    ...
    # P1-001, P1-002 and P1-003 are authorized; only p1_004+/P2/P3 dirs remain forbidden
    ...
    if later:
        problems.append("only P1-001/P1-002/P1-003 authorized; no other P1/P2/P3 batch dirs: %s"
    ...
    print("=" * 60)
    print("Chinese remediation Batch P1-003 validation (P1 retranslation batch)")
    print("=" * 60)
    if problems:
        for p in problems:
            print("  -", p)
        print("RESULT: %d problem(s) found ✗" % len(problems))
        return 1
    print("[PASS] Batch P1-003: 20 authorized articles across Babs [6,7,8,10] (each record bab "
          ...
          "P1-002 QA + Chinese candidate 189 + old Chinese 5/23 + Arabic/English/English-reference 281 "
