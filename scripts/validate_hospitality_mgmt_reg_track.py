#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Management of Hospitality Facilities
Regulation track (لائحة إدارة مرافق الضيافة السياحية).

24 records: 23 numbered articles (المادة الأولى .. الثالثة والعشرون) across
5 chapters + 1 appendix-type record (جدول المقابل المالي). All 24 اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "hospitality_mgmt",
                       "verified",
                       "hospitality_mgmt_reg_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "hospitality_mgmt",
                       "verified",
                       "hospitality_mgmt_reg_verified_summary.json")
LLM = os.path.join(ROOT, "data",
                   "hospitality_mgmt_reg_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "hospitality_mgmt_reg_argaam_batch_confirmed",
    "hospitality_mgmt_reg_gazette_date_unconfirmed",
}
AR = "ء-ي"
...
def main():
    ...
    # [2c] chapter_structure coverage (articles 1-23, appendices 1-1)
    ...
    # [2j] anchor facts
    if src.get("decree") != "القرار الوزاري رقم (2293)" \
            or src.get("decree_date_hijri") != "19/5/1444":
        e.append("[2j] decree/decree_date_hijri mismatch with verified 2293, 19/5/1444H")
    ...
    art4 = arts.get("hospitality_mgmt_reg_art_004", {})
    if "فئة" not in art4.get("text", ""):
        e.append("[2j] Article 4 must define the license categories")
    fee = arts.get("hospitality_mgmt_reg_appendix_001", {})
    if "2,250" not in fee.get("text", ""):
        e.append("[2j] Fee table must contain the 2,250-riyal one-year license fee")
    ...
    if e:
        print("FAIL: %d error(s) in Management of Hospitality Facilities Regulation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Management of Hospitality Facilities Regulation")
    print("  - 24 records: 23 numbered articles across 5 chapters + 1 fee-table appendix;")
    print("    all 24 اصلية")
    print("  - Founding: Ministerial Decision (2293), 19/5/1444H, printed on cover + signed")
