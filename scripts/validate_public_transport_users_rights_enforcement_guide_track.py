#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the دليل إجراءات ضبط مخالفات لائحة حقوق والتزامات مستخدمي وسائل النقل العام وآليتها التفصيلية track.
...
RECORDS = os.path.join(ROOT, "sources", "public_transport_users_rights_enforcement_guide", "verified",
                       "public_transport_users_rights_enforcement_guide_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "public_transport_users_rights_enforcement_guide", "verified",
                       "public_transport_users_rights_enforcement_guide_verified_summary.json")
LLM = os.path.join(ROOT, "data", "public_transport_users_rights_enforcement_guide_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"public_transport_users_rights_enforcement_guide_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "12/2/1446":
        e.append("[2j] gazette_publication_date_hijri must be 12/2/1446")
    if src.get("gazette_publication_date_gregorian") != "2024-08-16":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-08-16")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "دليل إجراءات ضبط مخالفات لائحة حقوق والتزامات مستخدمي وسائل النقل العام وآليتها التفصيلية":
    ...
    if e:
        print("FAIL: %d error(s) in Procedural Guide for Enforcing the Public Transport Users' Rights Regulation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Procedural Guide for Enforcing the Public Transport Users' Rights Regulation")
