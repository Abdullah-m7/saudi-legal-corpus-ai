#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة المنظمة للبحث والتطوير في قطاع الصناعات العسكرية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18593),
published 14/4/1443H (2021-11-19). The Gazette IS the official publication of record
...
STRUCTURE: 53 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "military_industries_rnd_regulation", "official_source",
                   "military_industries_rnd_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "military_industries_rnd_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "military_industries_rnd_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "military_industries_rnd_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "military_industries_rnd_regulation_arabic_legal_llm",
                        "military_industries_rnd_regulation_legal_llm_001_053.json")

LAW_ID = "sa-military-industries-rnd-regulation"
LAW_AR = "اللائحة المنظمة للبحث والتطوير في قطاع الصناعات العسكرية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"military_industries_rnd_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18593), published 14/4/1443H (2021-11-19). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "military_industries_rnd_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MILITARY_INDUSTRIES_RND_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "military-industries-rnd-regulation-llm-%03d" % idx,
            ...
            "article_path": "military_industries_rnd_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation Governing Research and Development in the Military Industries Sector -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/4/1443H / 2021-11-19)"),
                "source_authority_ar": ("اللائحة المنظمة للبحث والتطوير في قطاع الصناعات العسكرية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 14/4/1443هـ الموافق 2021-11-19م)"),
    ...
    json.dump({
        "law_key": "military_industries_rnd_regulation",
        "layer": "MILITARY_INDUSTRIES_RND_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "military-industries-rnd-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (53 مادة، جميعها أصلية)",
        "title_en": "Regulation Governing Research and Development in the Military Industries Sector — Arabic LLM-ready layer (53 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation Governing Research and Development in the Military Industries Sector records" % (len(ver), len(llm)))
