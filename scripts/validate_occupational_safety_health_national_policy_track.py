#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the السياسة الوطنية للسلامة والصحة المهنية track.
...
RECORDS = os.path.join(ROOT, "sources", "occupational_safety_health_national_policy", "verified",
                       "occupational_safety_health_national_policy_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "occupational_safety_health_national_policy", "verified",
                       "occupational_safety_health_national_policy_verified_summary.json")
LLM = os.path.join(ROOT, "data", "occupational_safety_health_national_policy_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"occupational_safety_health_national_policy_no_decision_number_in_source", "occupational_safety_health_national_policy_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "23/6/1442":
        e.append("[2j] gazette_publication_date_hijri must be 23/6/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-02-05":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-02-05")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "السياسة الوطنية للسلامة والصحة المهنية":
    ...
    if e:
        print("FAIL: %d error(s) in National Policy for Occupational Safety and Health track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: National Policy for Occupational Safety and Health")
