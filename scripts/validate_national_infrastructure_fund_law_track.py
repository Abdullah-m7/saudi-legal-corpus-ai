#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام صندوق البنية التحتية الوطني track.
...
RECORDS = os.path.join(ROOT, "sources", "national_infrastructure_fund_law", "verified",
                       "national_infrastructure_fund_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "national_infrastructure_fund_law", "verified",
                       "national_infrastructure_fund_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "national_infrastructure_fund_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"national_infrastructure_fund_law_no_decision_number_in_source", "national_infrastructure_fund_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام صندوق البنية التحتية الوطني":
    ...
    if e:
        print("FAIL: %d error(s) in Law of the National Infrastructure Fund track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law of the National Infrastructure Fund")
