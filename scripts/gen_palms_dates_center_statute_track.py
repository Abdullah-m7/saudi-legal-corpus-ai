#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية للمركز الوطني للنخيل والتمور track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21094),
published 26/5/1444H (2022-12-20). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "palms_dates_center_statute", "official_source",
                   "palms_dates_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "palms_dates_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "palms_dates_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "palms_dates_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "palms_dates_center_statute_arabic_legal_llm",
                        "palms_dates_center_statute_legal_llm_001_013.json")

LAW_ID = "sa-palms-dates-center-statute"
LAW_AR = "الترتيبات التنظيمية للمركز الوطني للنخيل والتمور"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"palms_dates_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21094), published 26/5/1444H (2022-12-20). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "palms_dates_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "PALMS_DATES_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "palms-dates-center-statute-llm-%03d" % idx,
            ...
            "article_path": "palms_dates_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the National Center for Palms and Dates -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 26/5/1444H / 2022-12-20)"),
                "source_authority_ar": ("الترتيبات التنظيمية للمركز الوطني للنخيل والتمور — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 26/5/1444هـ الموافق 2022-12-20م)"),
    ...
    json.dump({
        "law_key": "palms_dates_center_statute",
        "layer": "PALMS_DATES_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "palms-dates-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the National Center for Palms and Dates — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the National Center for Palms and Dates records" % (len(ver), len(llm)))
