#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لمركز برنامج تعزيز الشراكة مع القطاع الخاص (شريك) track.
...
RECORDS = os.path.join(ROOT, "sources", "shareek_program_center_statute", "verified",
                       "shareek_program_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "shareek_program_center_statute", "verified",
                       "shareek_program_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "shareek_program_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"shareek_program_center_statute_no_decision_number_in_source", "shareek_program_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "11/8/1444":
        e.append("[2j] gazette_publication_date_hijri must be 11/8/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-03-03":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-03-03")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية لمركز برنامج تعزيز الشراكة مع القطاع الخاص (شريك)":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the Center of the Private Sector Partnership Enhancement Program (Shareek) track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the Center of the Private Sector Partnership Enhancement Program (Shareek)")
