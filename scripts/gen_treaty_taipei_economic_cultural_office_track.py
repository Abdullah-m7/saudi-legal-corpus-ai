#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية بين مجلس الغرف التجارية الصناعية السعودية ومكتب الممثل الاقتصادي والثقافي لتايبيه في المملكة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18209),
published 26/1/1443H (2021-09-03). The Gazette IS the official publication of record
...
STRUCTURE: 27 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "treaty_taipei_economic_cultural_office", "official_source",
                   "treaty_taipei_economic_cultural_office_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_taipei_economic_cultural_office", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_taipei_economic_cultural_office_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_taipei_economic_cultural_office_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_taipei_economic_cultural_office_arabic_legal_llm",
                        "treaty_taipei_economic_cultural_office_legal_llm_001_028.json")

LAW_ID = "sa-treaty-taipei-economic-cultural-office"
LAW_AR = "اتفاقية بين مجلس الغرف التجارية الصناعية السعودية ومكتب الممثل الاقتصادي والثقافي لتايبيه في المملكة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_taipei_economic_cultural_office_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18209), published 26/1/1443H (2021-09-03). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_taipei_economic_cultural_office",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_TAIPEI_ECONOMIC_CULTURAL_OFFICE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-taipei-economic-cultural-office-llm-%03d" % idx,
            ...
            "article_path": "treaty_taipei_economic_cultural_office/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Agreement between the Federation of Saudi Chambers and the Taipei Economic and Cultural Office -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 26/1/1443H / 2021-09-03)"),
                "source_authority_ar": ("اتفاقية بين مجلس الغرف التجارية الصناعية السعودية ومكتب الممثل الاقتصادي والثقافي لتايبيه في المملكة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 26/1/1443هـ الموافق 2021-09-03م)"),
    ...
    json.dump({
        "law_key": "treaty_taipei_economic_cultural_office",
        "layer": "TREATY_TAIPEI_ECONOMIC_CULTURAL_OFFICE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-taipei-economic-cultural-office-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (27 مادة، جميعها أصلية)",
        "title_en": "Agreement between the Federation of Saudi Chambers and the Taipei Economic and Cultural Office — Arabic LLM-ready layer (27 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Agreement between the Federation of Saudi Chambers and the Taipei Economic and Cultural Office records" % (len(ver), len(llm)))
