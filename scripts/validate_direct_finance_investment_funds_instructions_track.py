#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the التعليمات الخاصة بصناديق استثمار التمويل المباشر track.
...
RECORDS = os.path.join(ROOT, "sources", "direct_finance_investment_funds_instructions", "verified",
                       "direct_finance_investment_funds_instructions_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "direct_finance_investment_funds_instructions", "verified",
                       "direct_finance_investment_funds_instructions_verified_summary.json")
LLM = os.path.join(ROOT, "data", "direct_finance_investment_funds_instructions_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"direct_finance_investment_funds_instructions_no_decision_number_in_source", "direct_finance_investment_funds_instructions_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "14/9/1443":
        e.append("[2j] gazette_publication_date_hijri must be 14/9/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-04-15":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-04-15")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "التعليمات الخاصة بصناديق استثمار التمويل المباشر":
    ...
    if e:
        print("FAIL: %d error(s) in Instructions for Direct Financing Investment Funds track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Instructions for Direct Financing Investment Funds")
