#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تعليمات صناديق الاستثمار المبسطة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4000963),
published 28/11/1447H (2026-05-15). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "simplified_investment_funds_instructions", "official_source",
                   "simplified_investment_funds_instructions_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "simplified_investment_funds_instructions", "verified")
RECORDS = os.path.join(OUT_VER, "simplified_investment_funds_instructions_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "simplified_investment_funds_instructions_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "simplified_investment_funds_instructions_arabic_legal_llm",
                        "simplified_investment_funds_instructions_legal_llm_001_031.json")

LAW_ID = "sa-simplified-investment-funds-instructions"
LAW_AR = "تعليمات صناديق الاستثمار المبسطة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"simplified_investment_funds_instructions_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4000963), published 28/11/1447H (2026-05-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "simplified_investment_funds_instructions",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SIMPLIFIED_INVESTMENT_FUNDS_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "simplified-investment-funds-instructions-llm-%03d" % idx,
            ...
            "article_path": "simplified_investment_funds_instructions/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Instructions for Simplified Investment Funds -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/11/1447H / 2026-05-15)"),
                "source_authority_ar": ("تعليمات صناديق الاستثمار المبسطة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 28/11/1447هـ الموافق 2026-05-15م)"),
    ...
    json.dump({
        "law_key": "simplified_investment_funds_instructions",
        "layer": "SIMPLIFIED_INVESTMENT_FUNDS_INSTRUCTIONS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "simplified-investment-funds-instructions-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (31 مادة، جميعها أصلية)",
        "title_en": "Instructions for Simplified Investment Funds — Arabic LLM-ready layer (31 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Instructions for Simplified Investment Funds records" % (len(ver), len(llm)))
