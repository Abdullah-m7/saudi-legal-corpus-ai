#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم هيئة الزكاة والضريبة والجمارك track.
...
RECORDS = os.path.join(ROOT, "sources", "zatca_statute", "verified",
                       "zatca_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "zatca_statute", "verified",
                       "zatca_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "zatca_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"zatca_statute_no_decision_number_in_source", "zatca_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم هيئة الزكاة والضريبة والجمارك":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Zakat, Tax and Customs Authority track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Zakat, Tax and Customs Authority")
