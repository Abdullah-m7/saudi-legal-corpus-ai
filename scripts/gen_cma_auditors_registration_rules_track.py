#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد تسجيل مراجعي حسابات المنشآت الخاضعة لإشراف الهيئة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19694),
published 2/12/1443H (2022-07-01). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "cma_auditors_registration_rules", "official_source",
                   "cma_auditors_registration_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "cma_auditors_registration_rules", "verified")
RECORDS = os.path.join(OUT_VER, "cma_auditors_registration_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "cma_auditors_registration_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "cma_auditors_registration_rules_arabic_legal_llm",
                        "cma_auditors_registration_rules_legal_llm_001_019.json")

LAW_ID = "sa-cma-auditors-registration-rules"
LAW_AR = "قواعد تسجيل مراجعي حسابات المنشآت الخاضعة لإشراف الهيئة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"cma_auditors_registration_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19694), published 2/12/1443H (2022-07-01). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "cma_auditors_registration_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "CMA_AUDITORS_REGISTRATION_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "cma-auditors-registration-rules-llm-%03d" % idx,
            ...
            "article_path": "cma_auditors_registration_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules for Registering Auditors of Entities Supervised by the Authority -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/12/1443H / 2022-07-01)"),
                "source_authority_ar": ("قواعد تسجيل مراجعي حسابات المنشآت الخاضعة لإشراف الهيئة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 2/12/1443هـ الموافق 2022-07-01م)"),
    ...
    json.dump({
        "law_key": "cma_auditors_registration_rules",
        "layer": "CMA_AUDITORS_REGISTRATION_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "cma-auditors-registration-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (19 مادة، جميعها أصلية)",
        "title_en": "Rules for Registering Auditors of Entities Supervised by the Authority — Arabic LLM-ready layer (19 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules for Registering Auditors of Entities Supervised by the Authority records" % (len(ver), len(llm)))
