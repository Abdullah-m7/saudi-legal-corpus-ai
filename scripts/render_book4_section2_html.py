...
# -*- coding: utf-8 -*-
"""
Render Book Four — Section 2 (provisions) outputs.

Writes:
  content/ar/book4_section2.md
  content/zh/book4_section2.md
  content/bilingual/book4_section2_bilingual.md
  dist/book4_section2.html   (clearly labelled: Section 2 provisions ONLY, not full Book Four)
...
MD_AR = os.path.join(ROOT, "content", "ar", "book4_section2.md")
MD_ZH = os.path.join(ROOT, "content", "zh", "book4_section2.md")
MD_BI = os.path.join(ROOT, "content", "bilingual", "book4_section2_bilingual.md")
HTML_OUT = os.path.join(ROOT, "dist", "book4_section2.html")

SECTION_LABEL_AR = "الباب الرابع — القسم 2: مجلس الإدارة والحوكمة (المواد 67–83) — أحكام مختارة فقط"
SECTION_LABEL_ZH = "第四编 — 第二节：董事会与治理（第67–83条）— 仅择要条款（非全节全文）"
INCOMPLETE_BANNER = ("⚠ 仅为第四编第二节的择要条款（源文件明确涵盖的第67、68、71、72、75、77条）；"
                     "并非完整的第四编，第69、70、73、74、76、78–83条未涵盖，仍为 needs_official_text_check。 / "
                     "أحكام مختارة من القسم الثاني فقط (المواد المغطاة صراحةً: 67، 68، 71، 72، 75، 77)؛ "
                     "ليست الباب الرابع كاملاً — المواد 69، 70، 73، 74، 76، 78–83 غير مغطاة (needs_official_text_check).")
...
def render_html(doc, spec):
    ...
    for p in provs:
        ...
        parts.append("</div>")
    parts.append('<footer><p>HTML = searchable/copyable Section-2 provisions view · '
