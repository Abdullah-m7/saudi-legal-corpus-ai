#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة انتخابات رؤساء الحرف والمهن ونوابهم track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27252),
published 16/1/1447H (2025-07-11). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "crafts_professions_heads_elections_regulation", "official_source",
                   "crafts_professions_heads_elections_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "crafts_professions_heads_elections_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "crafts_professions_heads_elections_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "crafts_professions_heads_elections_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "crafts_professions_heads_elections_regulation_arabic_legal_llm",
                        "crafts_professions_heads_elections_regulation_legal_llm_001_013.json")

LAW_ID = "sa-crafts-professions-heads-elections-regulation"
LAW_AR = "لائحة انتخابات رؤساء الحرف والمهن ونوابهم"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"crafts_professions_heads_elections_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27252), published 16/1/1447H (2025-07-11). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "crafts_professions_heads_elections_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CRAFTS_PROFESSIONS_HEADS_ELECTIONS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "crafts-professions-heads-elections-regulation-llm-%03d" % idx,
            ...
            "article_path": "crafts_professions_heads_elections_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on the Election of Heads of Crafts and Professions and their Deputies -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 16/1/1447H / 2025-07-11)"),
                "source_authority_ar": ("لائحة انتخابات رؤساء الحرف والمهن ونوابهم — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 16/1/1447هـ الموافق 2025-07-11م)"),
    ...
    json.dump({
        "law_key": "crafts_professions_heads_elections_regulation",
        "layer": "CRAFTS_PROFESSIONS_HEADS_ELECTIONS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "crafts-professions-heads-elections-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Regulation on the Election of Heads of Crafts and Professions and their Deputies — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on the Election of Heads of Crafts and Professions and their Deputies records" % (len(ver), len(llm)))
