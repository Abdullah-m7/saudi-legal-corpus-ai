#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط إعفاء البضائع المعادة من الرسوم الجمركية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25232),
published 29/12/1445H (2024-07-05). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "returned_goods_customs_exemption_controls", "official_source",
                   "returned_goods_customs_exemption_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "returned_goods_customs_exemption_controls", "verified")
RECORDS = os.path.join(OUT_VER, "returned_goods_customs_exemption_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "returned_goods_customs_exemption_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "returned_goods_customs_exemption_controls_arabic_legal_llm",
                        "returned_goods_customs_exemption_controls_legal_llm_001_008.json")

LAW_ID = "sa-returned-goods-customs-exemption-controls"
LAW_AR = "ضوابط إعفاء البضائع المعادة من الرسوم الجمركية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"returned_goods_customs_exemption_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25232), published 29/12/1445H (2024-07-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "returned_goods_customs_exemption_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "RETURNED_GOODS_CUSTOMS_EXEMPTION_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "returned-goods-customs-exemption-controls-llm-%03d" % idx,
            ...
            "article_path": "returned_goods_customs_exemption_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on the Exemption of Returned Goods from Customs Duties -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 29/12/1445H / 2024-07-05)"),
                "source_authority_ar": ("ضوابط إعفاء البضائع المعادة من الرسوم الجمركية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 29/12/1445هـ الموافق 2024-07-05م)"),
    ...
    json.dump({
        "law_key": "returned_goods_customs_exemption_controls",
        "layer": "RETURNED_GOODS_CUSTOMS_EXEMPTION_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "returned-goods-customs-exemption-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Controls on the Exemption of Returned Goods from Customs Duties — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on the Exemption of Returned Goods from Customs Duties records" % (len(ver), len(llm)))
