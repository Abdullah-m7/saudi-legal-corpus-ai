#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد الأسواق الحرة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21223),
...
SRC = os.path.join(ROOT, "sources", "duty_free_markets_rules", "official_source",
                   "duty_free_markets_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "duty_free_markets_rules", "verified")
RECORDS = os.path.join(OUT_VER, "duty_free_markets_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "duty_free_markets_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "duty_free_markets_rules_arabic_legal_llm",
                        "duty_free_markets_rules_legal_llm_001_023.json")

LAW_ID = "sa-duty-free-markets-rules"
LAW_AR = "قواعد الأسواق الحرة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"duty_free_markets_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21223), published 13/6/1444H (2023-01-06). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "duty_free_markets_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "DUTY_FREE_MARKETS_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "duty-free-markets-rules-llm-%03d" % idx,
            ...
            "article_path": "duty_free_markets_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules of Duty-Free Markets -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/6/1444H / 2023-01-06)"),
                "source_authority_ar": ("قواعد الأسواق الحرة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "duty_free_markets_rules",
        "layer": "DUTY_FREE_MARKETS_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "duty-free-markets-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (23 مادة، جميعها أصلية)",
        "title_en": "Rules of Duty-Free Markets — Arabic LLM-ready layer (23 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules of Duty-Free Markets records" % (len(ver), len(llm)))
