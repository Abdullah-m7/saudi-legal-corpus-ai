#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة رسوم تقديم منتجات التبغ track.
...
RECORDS = os.path.join(ROOT, "sources", "tobacco_products_submission_fees_regulation", "verified",
                       "tobacco_products_submission_fees_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "tobacco_products_submission_fees_regulation", "verified",
                       "tobacco_products_submission_fees_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "tobacco_products_submission_fees_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"tobacco_products_submission_fees_regulation_no_decision_number_in_source", "tobacco_products_submission_fees_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة رسوم تقديم منتجات التبغ":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on Fees for the Submission of Tobacco Products track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on Fees for the Submission of Tobacco Products")
