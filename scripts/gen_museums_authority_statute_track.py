#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم هيئة المتاحف track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25116),
...
SRC = os.path.join(ROOT, "sources", "museums_authority_statute", "official_source",
                   "museums_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "museums_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "museums_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "museums_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "museums_authority_statute_arabic_legal_llm",
                        "museums_authority_statute_legal_llm_001_012.json")

LAW_ID = "sa-museums-authority-statute"
LAW_AR = "تنظيم هيئة المتاحف"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"museums_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25116), published 9/12/1445H (2024-06-15). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "museums_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "MUSEUMS_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "museums-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "museums_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Museums Commission -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/12/1445H / 2024-06-15)"),
                "source_authority_ar": ("تنظيم هيئة المتاحف — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "museums_authority_statute",
        "layer": "MUSEUMS_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "museums-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Statute of the Museums Commission — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Museums Commission records" % (len(ver), len(llm)))
