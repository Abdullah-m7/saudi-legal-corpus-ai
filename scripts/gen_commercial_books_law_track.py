#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the Commercial Books Law track (نظام الدفاتر التجارية م/61).
...
independent-date snapshots (2024-05 + 2025-06): all 16 article bodies are
byte-identical between them (zero differences). Both raw snapshots are committed
under inputs/commercial_books_boe_snapshots/ with recorded sha256, and the
concatenated corpus text carries a recorded sha256.

Royal Decree M/61 dated 17/12/1409H. IN FORCE (ساري). Consolidated issuance:
all 16 اصلية (0 معدلة / 0 ملغاة / 0 مضافة).
...
SRC = os.path.join(ROOT, "sources", "commercial_books", "law", "official_source",
                   "commercial_books_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "commercial_books", "law", "verified")
RECORDS = os.path.join(OUT_VER, "commercial_books_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "commercial_books_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "commercial_books_arabic_legal_llm",
                        "commercial_books_law_legal_llm_001_016.json")

LAW_ID = "sa-commercial-books-law-m61-1409"
LAW_AR = "نظام الدفاتر التجارية"
CORPUS = "commercial_books"
STATUS = "BOE_OFFICIAL_PORTAL_ARCHIVE_CROSS_SNAPSHOT_VERIFIED"
KEY_RE = r"commercial_books_art_(\d{3})(_mukarrar)?$"
LLM_FILE_BASENAME = "commercial_books_law_legal_llm_001_016.json"
N_ARTICLES = 16
RECID_PREFIX = "commercial-books-law-llm-art"
SEARCH_TERM = "نظام الدفاتر التجارية"
...
def main():
    ...
    for key in keys:
        ...
        ver.append({"law_key": CORPUS, "law_component": "law", "language": "ar",
                    "record_layer": "COMMERCIAL_BOOKS_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({"law_key": CORPUS, "layer": "COMMERCIAL_BOOKS_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({"layer_id": "sa-commercial-books-law-arabic-legal-llm-full", "law_id": LAW_ID,
               "law_component": "law",
               "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة؛ إصدار كامل: 16 أصلية)",
               "title_en": "Saudi Commercial Books Law — Arabic LLM-ready layer (16 records)",
               "record_type": "verified_arabic_article", "language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Commercial Books Law records" % (len(ver), len(llm)))
