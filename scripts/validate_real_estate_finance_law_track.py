#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Real Estate Finance Law track (15 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) — all 15 matched outright (mean 0.965, min 0.933), so none
required visual adjudication; articles are numbered by ordinal position 1..15
...
status for every article (no dual-status divergence). FRESH FULL ISSUANCE: all 15
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_finance", "law", "verified",
                       "real_estate_finance_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "real_estate_finance_arabic_legal_llm",
                   "real_estate_finance_law_legal_llm_001_015.json")
PDF = os.path.join(ROOT, "inputs", "real_estate_finance_official_pdfs",
                   "real_estate_finance_law_moj_official_ar.pdf")
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Real Estate Finance Law track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Real Estate Finance Law — 15 records (fresh issuance: all 15 اصلية)")
    print("  - trust gate: 15/15 MATCHES_PDF outright (mean 0.965, min 0.933); no visual adjudication needed")
    print("  - numbered 1..15 by ordinal position (no مكرر); no dual-status divergence")
    print("  - IN-FORCE Royal Decree M/50 (13/8/1433H); committed MOJ PDF hash verified; Arabic governs")
    return 0
