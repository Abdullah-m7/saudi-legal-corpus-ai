#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the دليل المعايير المضمونة track.
...
RECORDS = os.path.join(ROOT, "sources", "guaranteed_standards_guide", "verified",
                       "guaranteed_standards_guide_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "guaranteed_standards_guide", "verified",
                       "guaranteed_standards_guide_verified_summary.json")
LLM = os.path.join(ROOT, "data", "guaranteed_standards_guide_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"guaranteed_standards_guide_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "دليل المعايير المضمونة":
    ...
    if e:
        print("FAIL: %d error(s) in Guaranteed Standards Guide track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Guaranteed Standards Guide")
