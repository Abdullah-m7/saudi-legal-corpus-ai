#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام الأرصاد track.
...
RECORDS = os.path.join(ROOT, "sources", "meteorology_law", "verified",
                       "meteorology_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "meteorology_law", "verified",
                       "meteorology_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "meteorology_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"meteorology_law_no_decision_number_in_source", "meteorology_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام الأرصاد":
    ...
    if e:
        print("FAIL: %d error(s) in Meteorology Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Meteorology Law")
