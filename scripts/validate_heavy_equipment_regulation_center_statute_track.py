#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لمركز تنظيم المعدات الثقيلة track.
...
RECORDS = os.path.join(ROOT, "sources", "heavy_equipment_regulation_center_statute", "verified",
                       "heavy_equipment_regulation_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "heavy_equipment_regulation_center_statute", "verified",
                       "heavy_equipment_regulation_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "heavy_equipment_regulation_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"heavy_equipment_regulation_center_statute_no_decision_number_in_source", "heavy_equipment_regulation_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية لمركز تنظيم المعدات الثقيلة":
    ...
    if e:
        print("FAIL: %d error(s) in Heavy Equipment Regulation Center — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Heavy Equipment Regulation Center — Organisational Arrangements")
