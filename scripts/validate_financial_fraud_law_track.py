#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام مكافحة الاحتيال المالي وخيانة الأمانة track.
...
RECORDS = os.path.join(ROOT, "sources", "financial_fraud_law", "verified",
                       "financial_fraud_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "financial_fraud_law", "verified",
                       "financial_fraud_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "financial_fraud_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"financial_fraud_law_no_decision_number_in_source", "financial_fraud_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "18/9/1442":
        e.append("[2j] gazette_publication_date_hijri must be 18/9/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-04-30":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-04-30")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "نظام مكافحة الاحتيال المالي وخيانة الأمانة":
    ...
    if e:
        print("FAIL: %d error(s) in Anti-Financial-Fraud and Breach of Trust Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Anti-Financial-Fraud and Breach of Trust Law")
