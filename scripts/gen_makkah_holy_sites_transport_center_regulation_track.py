#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية للمركز العام للنقل بمدينة مكة المكرمة والمشاعر المقدسة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23779),
...
SRC = os.path.join(ROOT, "sources", "makkah_holy_sites_transport_center_regulation", "official_source",
                   "makkah_holy_sites_transport_center_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "makkah_holy_sites_transport_center_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "makkah_holy_sites_transport_center_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "makkah_holy_sites_transport_center_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "makkah_holy_sites_transport_center_regulation_arabic_legal_llm",
                        "makkah_holy_sites_transport_center_regulation_legal_llm_001_010.json")

LAW_ID = "sa-makkah-holy-sites-transport-center-regulation"
LAW_AR = "اللائحة التنظيمية للمركز العام للنقل بمدينة مكة المكرمة والمشاعر المقدسة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"makkah_holy_sites_transport_center_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23779), published 14/3/1445H (2023-09-29). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "makkah_holy_sites_transport_center_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "MAKKAH_HOLY_SITES_TRANSPORT_CENTER_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "makkah-holy-sites-transport-center-regulation-llm-%03d" % idx,
            ...
            "article_path": "makkah_holy_sites_transport_center_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for the General Transport Center in Makkah and the Holy Sites -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/3/1445H / 2023-09-29)"),
                "source_authority_ar": ("اللائحة التنظيمية للمركز العام للنقل بمدينة مكة المكرمة والمشاعر المقدسة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "makkah_holy_sites_transport_center_regulation",
        "layer": "MAKKAH_HOLY_SITES_TRANSPORT_CENTER_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "makkah-holy-sites-transport-center-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for the General Transport Center in Makkah and the Holy Sites — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for the General Transport Center in Makkah and the Holy Sites records" % (len(ver), len(llm)))
