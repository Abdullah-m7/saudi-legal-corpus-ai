#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم الأكاديمية المالية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21580),
...
SRC = os.path.join(ROOT, "sources", "financial_academy_statute", "official_source",
                   "financial_academy_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "financial_academy_statute", "verified")
RECORDS = os.path.join(OUT_VER, "financial_academy_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "financial_academy_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "financial_academy_statute_arabic_legal_llm",
                        "financial_academy_statute_legal_llm_001_017.json")

LAW_ID = "sa-financial-academy-statute"
LAW_AR = "تنظيم الأكاديمية المالية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"financial_academy_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21580), published 25/8/1444H (2023-03-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "financial_academy_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "FINANCIAL_ACADEMY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "financial-academy-statute-llm-%03d" % idx,
            ...
            "article_path": "financial_academy_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Financial Academy -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 25/8/1444H / 2023-03-17)"),
                "source_authority_ar": ("تنظيم الأكاديمية المالية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "financial_academy_statute",
        "layer": "FINANCIAL_ACADEMY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "financial-academy-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Statute of the Financial Academy — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Financial Academy records" % (len(ver), len(llm)))
