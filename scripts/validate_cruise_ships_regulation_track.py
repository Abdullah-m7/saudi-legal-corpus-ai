#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية لسفن الرحلات السياحية (الكروز) track.
...
RECORDS = os.path.join(ROOT, "sources", "cruise_ships_regulation", "verified",
                       "cruise_ships_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "cruise_ships_regulation", "verified",
                       "cruise_ships_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "cruise_ships_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"cruise_ships_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية لسفن الرحلات السياحية (الكروز)":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Regulation for Cruise Ships track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Regulation for Cruise Ships")
