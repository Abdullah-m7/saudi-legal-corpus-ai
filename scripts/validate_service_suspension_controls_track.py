#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط إيقاف الخدمات track.
...
RECORDS = os.path.join(ROOT, "sources", "service_suspension_controls", "verified",
                       "service_suspension_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "service_suspension_controls", "verified",
                       "service_suspension_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "service_suspension_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"service_suspension_controls_no_decision_number_in_source", "service_suspension_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "ضوابط إيقاف الخدمات":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on the Suspension of Services track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on the Suspension of Services")
