#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لجائزة الملك عبدالعزيز للجودة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17169),
published 7/7/1442H (2021-02-19). The Gazette IS the official publication of record
...
STRUCTURE: 14 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "king_abdulaziz_quality_award_statute", "official_source",
                   "king_abdulaziz_quality_award_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "king_abdulaziz_quality_award_statute", "verified")
RECORDS = os.path.join(OUT_VER, "king_abdulaziz_quality_award_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "king_abdulaziz_quality_award_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "king_abdulaziz_quality_award_statute_arabic_legal_llm",
                        "king_abdulaziz_quality_award_statute_legal_llm_001_014.json")

LAW_ID = "sa-king-abdulaziz-quality-award-statute"
LAW_AR = "الترتيبات التنظيمية لجائزة الملك عبدالعزيز للجودة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"king_abdulaziz_quality_award_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17169), published 7/7/1442H (2021-02-19). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "king_abdulaziz_quality_award_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "KING_ABDULAZIZ_QUALITY_AWARD_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "king-abdulaziz-quality-award-statute-llm-%03d" % idx,
            ...
            "article_path": "king_abdulaziz_quality_award_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("King Abdulaziz Quality Award — Organisational Arrangements -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/7/1442H / 2021-02-19)"),
                "source_authority_ar": ("الترتيبات التنظيمية لجائزة الملك عبدالعزيز للجودة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 7/7/1442هـ الموافق 2021-02-19م)"),
    ...
    json.dump({
        "law_key": "king_abdulaziz_quality_award_statute",
        "layer": "KING_ABDULAZIZ_QUALITY_AWARD_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "king-abdulaziz-quality-award-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "King Abdulaziz Quality Award — Organisational Arrangements — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready King Abdulaziz Quality Award — Organisational Arrangements records" % (len(ver), len(llm)))
