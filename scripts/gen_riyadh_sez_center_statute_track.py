#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الترتيبات التنظيمية لمركز المناطق الاقتصادية الخاصة بمدينة الرياض track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21160),
published 6/6/1444H (2022-12-30). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "riyadh_sez_center_statute", "official_source",
                   "riyadh_sez_center_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "riyadh_sez_center_statute", "verified")
RECORDS = os.path.join(OUT_VER, "riyadh_sez_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "riyadh_sez_center_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "riyadh_sez_center_statute_arabic_legal_llm",
                        "riyadh_sez_center_statute_legal_llm_001_020.json")

LAW_ID = "sa-riyadh-sez-center-statute"
LAW_AR = "الترتيبات التنظيمية لمركز المناطق الاقتصادية الخاصة بمدينة الرياض"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"riyadh_sez_center_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21160), published 6/6/1444H (2022-12-30). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "riyadh_sez_center_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "RIYADH_SEZ_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "riyadh-sez-center-statute-llm-%03d" % idx,
            ...
            "article_path": "riyadh_sez_center_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Arrangements of the Special Economic Zones Center in Riyadh -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/6/1444H / 2022-12-30)"),
                "source_authority_ar": ("الترتيبات التنظيمية لمركز المناطق الاقتصادية الخاصة بمدينة الرياض — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 6/6/1444هـ الموافق 2022-12-30م)"),
    ...
    json.dump({
        "law_key": "riyadh_sez_center_statute",
        "layer": "RIYADH_SEZ_CENTER_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "riyadh-sez-center-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Organisational Arrangements of the Special Economic Zones Center in Riyadh — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Arrangements of the Special Economic Zones Center in Riyadh records" % (len(ver), len(llm)))
