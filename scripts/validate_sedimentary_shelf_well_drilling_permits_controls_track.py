#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط إصدار رخص حفر الآبار الجديدة على الرف الرسوبي track.
...
RECORDS = os.path.join(ROOT, "sources", "sedimentary_shelf_well_drilling_permits_controls", "verified",
                       "sedimentary_shelf_well_drilling_permits_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "sedimentary_shelf_well_drilling_permits_controls", "verified",
                       "sedimentary_shelf_well_drilling_permits_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "sedimentary_shelf_well_drilling_permits_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"sedimentary_shelf_well_drilling_permits_controls_no_decision_number_in_source", "sedimentary_shelf_well_drilling_permits_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "23/1/1447":
        e.append("[2j] gazette_publication_date_hijri must be 23/1/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-07-18":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-07-18")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط إصدار رخص حفر الآبار الجديدة على الرف الرسوبي":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on Issuing Permits to Drill New Wells on the Sedimentary Shelf track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on Issuing Permits to Drill New Wells on the Sedimentary Shelf")
