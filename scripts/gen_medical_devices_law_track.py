#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام الأجهزة والمستلزمات الطبية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17073),
published 16/7/1442H (2021-02-28). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "medical_devices_law", "official_source",
                   "medical_devices_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "medical_devices_law", "verified")
RECORDS = os.path.join(OUT_VER, "medical_devices_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "medical_devices_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "medical_devices_law_arabic_legal_llm",
                        "medical_devices_law_legal_llm_001_049.json")

LAW_ID = "sa-medical-devices-law"
LAW_AR = "نظام الأجهزة والمستلزمات الطبية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"medical_devices_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17073), published 16/7/1442H (2021-02-28). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "medical_devices_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "MEDICAL_DEVICES_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "medical-devices-law-llm-%03d" % idx,
            ...
            "article_path": "medical_devices_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Medical Devices and Supplies Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 16/7/1442H / 2021-02-28)"),
                "source_authority_ar": ("نظام الأجهزة والمستلزمات الطبية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 16/7/1442هـ الموافق 2021-02-28م)"),
    ...
    json.dump({
        "law_key": "medical_devices_law",
        "layer": "MEDICAL_DEVICES_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "medical-devices-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (49 مادة، جميعها أصلية)",
        "title_en": "Medical Devices and Supplies Law — Arabic LLM-ready layer (49 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Medical Devices and Supplies Law records" % (len(ver), len(llm)))
