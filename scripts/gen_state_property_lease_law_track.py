#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام استئجار الدولة للعقار track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19844),
...
SRC = os.path.join(ROOT, "sources", "state_property_lease_law", "official_source",
                   "state_property_lease_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "state_property_lease_law", "verified")
RECORDS = os.path.join(OUT_VER, "state_property_lease_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "state_property_lease_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "state_property_lease_law_arabic_legal_llm",
                        "state_property_lease_law_legal_llm_001_031.json")

LAW_ID = "sa-state-property-lease-law"
LAW_AR = "نظام استئجار الدولة للعقار"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"state_property_lease_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19844), published 7/1/1444H (2022-08-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "state_property_lease_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "STATE_PROPERTY_LEASE_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "state-property-lease-law-llm-%03d" % idx,
            ...
            "article_path": "state_property_lease_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law on State Leasing of Real Estate -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/1/1444H / 2022-08-05)"),
                "source_authority_ar": ("نظام استئجار الدولة للعقار — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "state_property_lease_law",
        "layer": "STATE_PROPERTY_LEASE_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "state-property-lease-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (31 مادة، جميعها أصلية)",
        "title_en": "Law on State Leasing of Real Estate — Arabic LLM-ready layer (31 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law on State Leasing of Real Estate records" % (len(ver), len(llm)))
