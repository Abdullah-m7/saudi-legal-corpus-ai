#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم هيئة الأفلام track.
...
RECORDS = os.path.join(ROOT, "sources", "film_authority_statute", "verified",
                       "film_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "film_authority_statute", "verified",
                       "film_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "film_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"film_authority_statute_no_decision_number_in_source", "film_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم هيئة الأفلام":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Film Commission track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Film Commission")
