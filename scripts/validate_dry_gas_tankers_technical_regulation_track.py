#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة الفنية للصهاريج (صهاريج نقل الغاز الجاف) track.
...
RECORDS = os.path.join(ROOT, "sources", "dry_gas_tankers_technical_regulation", "verified",
                       "dry_gas_tankers_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "dry_gas_tankers_technical_regulation", "verified",
                       "dry_gas_tankers_technical_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "dry_gas_tankers_technical_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"dry_gas_tankers_technical_regulation_no_decision_number_in_source", "dry_gas_tankers_technical_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة الفنية للصهاريج (صهاريج نقل الغاز الجاف)":
    ...
    if e:
        print("FAIL: %d error(s) in Technical Regulation for Tankers (Dry Gas Transport Tankers) track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Technical Regulation for Tankers (Dry Gas Transport Tankers)")
