#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الضوابط والإجراءات لسفن الرحلات السياحية الكروز track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26663),
...
SRC = os.path.join(ROOT, "sources", "cruise_ships_controls", "official_source",
                   "cruise_ships_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "cruise_ships_controls", "verified")
RECORDS = os.path.join(OUT_VER, "cruise_ships_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "cruise_ships_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "cruise_ships_controls_arabic_legal_llm",
                        "cruise_ships_controls_legal_llm_001_014.json")

LAW_ID = "sa-cruise-ships-controls"
LAW_AR = "الضوابط والإجراءات لسفن الرحلات السياحية الكروز"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"cruise_ships_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26663), published 20/4/1446H (2024-10-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "cruise_ships_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "CRUISE_SHIPS_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "cruise-ships-controls-llm-%03d" % idx,
            ...
            "article_path": "cruise_ships_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls and Procedures for Cruise Ships -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/4/1446H / 2024-10-23)"),
                "source_authority_ar": ("الضوابط والإجراءات لسفن الرحلات السياحية الكروز — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "cruise_ships_controls",
        "layer": "CRUISE_SHIPS_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "cruise-ships-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (14 مادة، جميعها أصلية)",
        "title_en": "Controls and Procedures for Cruise Ships — Arabic LLM-ready layer (14 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls and Procedures for Cruise Ships records" % (len(ver), len(llm)))
