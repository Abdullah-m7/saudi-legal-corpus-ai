#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم مجمع الملك سلمان العالمي للغة العربية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24103),
published 20/1/1442H (2020-09-08). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "king_salman_arabic_language_academy_statute", "official_source",
                   "king_salman_arabic_language_academy_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "king_salman_arabic_language_academy_statute", "verified")
RECORDS = os.path.join(OUT_VER, "king_salman_arabic_language_academy_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "king_salman_arabic_language_academy_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "king_salman_arabic_language_academy_statute_arabic_legal_llm",
                        "king_salman_arabic_language_academy_statute_legal_llm_001_018.json")

LAW_ID = "sa-king-salman-arabic-language-academy-statute"
LAW_AR = "تنظيم مجمع الملك سلمان العالمي للغة العربية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"king_salman_arabic_language_academy_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24103), published 20/1/1442H (2020-09-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "king_salman_arabic_language_academy_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "KING_SALMAN_ARABIC_LANGUAGE_ACADEMY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "king-salman-arabic-language-academy-statute-llm-%03d" % idx,
            ...
            "article_path": "king_salman_arabic_language_academy_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the King Salman Global Academy for the Arabic Language -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 20/1/1442H / 2020-09-08)"),
                "source_authority_ar": ("تنظيم مجمع الملك سلمان العالمي للغة العربية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 20/1/1442هـ الموافق 2020-09-08م)"),
    ...
    json.dump({
        "law_key": "king_salman_arabic_language_academy_statute",
        "layer": "KING_SALMAN_ARABIC_LANGUAGE_ACADEMY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "king-salman-arabic-language-academy-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Statute of the King Salman Global Academy for the Arabic Language — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the King Salman Global Academy for the Arabic Language records" % (len(ver), len(llm)))
