#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the معايير ترخيص المنصات العقارية الإلكترونية وتصنيفها track.
...
RECORDS = os.path.join(ROOT, "sources", "realestate_platform_licensing_criteria", "verified",
                       "realestate_platform_licensing_criteria_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "realestate_platform_licensing_criteria", "verified",
                       "realestate_platform_licensing_criteria_verified_summary.json")
LLM = os.path.join(ROOT, "data", "realestate_platform_licensing_criteria_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"realestate_platform_licensing_criteria_no_decision_number_in_source", "realestate_platform_licensing_criteria_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "23/10/1442":
        e.append("[2j] gazette_publication_date_hijri must be 23/10/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-06-04":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-06-04")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "معايير ترخيص المنصات العقارية الإلكترونية وتصنيفها":
    ...
    if e:
        print("FAIL: %d error(s) in Licensing and Classification Criteria for Electronic Real Estate Platforms track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Licensing and Classification Criteria for Electronic Real Estate Platforms")
