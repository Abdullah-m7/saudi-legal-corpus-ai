#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية لتأجير اليخوت الضخمة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23420),
...
SRC = os.path.join(ROOT, "sources", "superyacht_chartering_regulation", "official_source",
                   "superyacht_chartering_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "superyacht_chartering_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "superyacht_chartering_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "superyacht_chartering_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "superyacht_chartering_regulation_arabic_legal_llm",
                        "superyacht_chartering_regulation_legal_llm_001_010.json")

LAW_ID = "sa-superyacht-chartering-regulation"
LAW_AR = "اللائحة التنظيمية لتأجير اليخوت الضخمة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"superyacht_chartering_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23420), published 17/1/1445H (2023-08-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "superyacht_chartering_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SUPERYACHT_CHARTERING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "superyacht-chartering-regulation-llm-%03d" % idx,
            ...
            "article_path": "superyacht_chartering_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Regulation for Superyacht Chartering -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/1/1445H / 2023-08-04)"),
                "source_authority_ar": ("اللائحة التنظيمية لتأجير اليخوت الضخمة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "superyacht_chartering_regulation",
        "layer": "SUPERYACHT_CHARTERING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "superyacht-chartering-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Organisational Regulation for Superyacht Chartering — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Regulation for Superyacht Chartering records" % (len(ver), len(llm)))
