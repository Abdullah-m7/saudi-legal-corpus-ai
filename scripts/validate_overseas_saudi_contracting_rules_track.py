#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد المنظمة للتعاقد مع السعوديين في الخارج وسلم الأجور track.

11 records: 6 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "overseas_saudi_contracting_rules", "verified",
                       "overseas_saudi_contracting_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "overseas_saudi_contracting_rules", "verified",
                       "overseas_saudi_contracting_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "overseas_saudi_contracting_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {
    "overseas_saudi_contracting_rules_found_by_walking_a_url_section_the_index_never_covered",
    "overseas_saudi_contracting_rules_no_decision_number_in_source",
    "overseas_saudi_contracting_rules_source_is_gazette_html_only",
    "overseas_saudi_contracting_rules_wage_scale_table_is_flattened_into_the_last_article",
}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "21/11/1447":
        e.append("[2j] gazette_publication_date_hijri must be 21/11/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-05-08":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-05-08")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "القواعد المنظمة للتعاقد مع السعوديين في الخارج وسلم الأجور":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Contracting with Saudis Abroad and the Wage Scale track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Contracting with Saudis Abroad and the Wage Scale")
