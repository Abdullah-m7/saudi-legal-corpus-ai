#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام الانضباط الوظيفي track.
...
RECORDS = os.path.join(ROOT, "sources", "job_discipline_law", "verified",
                       "job_discipline_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "job_discipline_law", "verified",
                       "job_discipline_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "job_discipline_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"job_discipline_law_no_decision_number_in_source", "job_discipline_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام الانضباط الوظيفي":
    ...
    if e:
        print("FAIL: %d error(s) in Job Discipline Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Job Discipline Law")
