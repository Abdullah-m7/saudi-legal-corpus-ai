#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لمركز ذاكرة الثقافة السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "saudi_culture_memory_center_statute", "verified",
                       "saudi_culture_memory_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "saudi_culture_memory_center_statute", "verified",
                       "saudi_culture_memory_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "saudi_culture_memory_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"saudi_culture_memory_center_statute_no_decision_number_in_source", "saudi_culture_memory_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية لمركز ذاكرة الثقافة السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the Center for the Memory of Saudi Culture track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the Center for the Memory of Saudi Culture")
