#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنظام إدارة المواد الكيميائية track.
...
RECORDS = os.path.join(ROOT, "sources", "chemicals_management_regulation", "verified",
                       "chemicals_management_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "chemicals_management_regulation", "verified",
                       "chemicals_management_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "chemicals_management_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"chemicals_management_regulation_no_decision_number_in_source", "chemicals_management_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "6/3/1447":
        e.append("[2j] gazette_publication_date_hijri must be 6/3/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-08-29":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-08-29")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنفيذية لنظام إدارة المواد الكيميائية":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation of the Chemicals Management Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation of the Chemicals Management Law")
