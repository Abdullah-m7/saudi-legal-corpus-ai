#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الإطار التنظيمي للطاقة المتجددة للاستهلاك الذاتي track.
...
RECORDS = os.path.join(ROOT, "sources", "self_consumption_renewable_energy_framework", "verified",
                       "self_consumption_renewable_energy_framework_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "self_consumption_renewable_energy_framework", "verified",
                       "self_consumption_renewable_energy_framework_verified_summary.json")
LLM = os.path.join(ROOT, "data", "self_consumption_renewable_energy_framework_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"self_consumption_renewable_energy_framework_no_decision_number_in_source", "self_consumption_renewable_energy_framework_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الإطار التنظيمي للطاقة المتجددة للاستهلاك الذاتي":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Framework for Self-Consumption Renewable Energy track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Framework for Self-Consumption Renewable Energy")
