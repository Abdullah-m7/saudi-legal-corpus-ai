#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام المعالجات التجارية في التجارة الدولية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20969),
...
SRC = os.path.join(ROOT, "sources", "trade_remedies_law", "official_source",
                   "trade_remedies_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "trade_remedies_law", "verified")
RECORDS = os.path.join(OUT_VER, "trade_remedies_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "trade_remedies_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "trade_remedies_law_arabic_legal_llm",
                        "trade_remedies_law_legal_llm_001_037.json")

LAW_ID = "sa-trade-remedies-law"
LAW_AR = "نظام المعالجات التجارية في التجارة الدولية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"trade_remedies_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20969), published 8/5/1444H (2022-12-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "trade_remedies_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "TRADE_REMEDIES_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "trade-remedies-law-llm-%03d" % idx,
            ...
            "article_path": "trade_remedies_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law of Trade Remedies in International Trade -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 8/5/1444H / 2022-12-02)"),
                "source_authority_ar": ("نظام المعالجات التجارية في التجارة الدولية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "trade_remedies_law",
        "layer": "TRADE_REMEDIES_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "trade-remedies-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (37 مادة، جميعها أصلية)",
        "title_en": "Law of Trade Remedies in International Trade — Arabic LLM-ready layer (37 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law of Trade Remedies in International Trade records" % (len(ver), len(llm)))
