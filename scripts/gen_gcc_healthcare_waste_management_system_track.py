#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام الموحد لإدارة نفايات الرعاية الصحية بدول مجلس التعاون لدول الخليج العربي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17459),
...
SRC = os.path.join(ROOT, "sources", "gcc_healthcare_waste_management_system", "official_source",
                   "gcc_healthcare_waste_management_system_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "gcc_healthcare_waste_management_system", "verified")
RECORDS = os.path.join(OUT_VER, "gcc_healthcare_waste_management_system_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "gcc_healthcare_waste_management_system_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "gcc_healthcare_waste_management_system_arabic_legal_llm",
                        "gcc_healthcare_waste_management_system_legal_llm_001_023.json")

LAW_ID = "sa-gcc-healthcare-waste-management-system"
LAW_AR = "النظام الموحد لإدارة نفايات الرعاية الصحية بدول مجلس التعاون لدول الخليج العربي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"gcc_healthcare_waste_management_system_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17459), published 4/9/1442H (2021-04-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "gcc_healthcare_waste_management_system",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "GCC_HEALTHCARE_WASTE_MANAGEMENT_SYSTEM_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "gcc-healthcare-waste-management-system-llm-%03d" % idx,
            ...
            "article_path": "gcc_healthcare_waste_management_system/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("GCC Unified System for Healthcare Waste Management -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 4/9/1442H / 2021-04-16)"),
                "source_authority_ar": ("النظام الموحد لإدارة نفايات الرعاية الصحية بدول مجلس التعاون لدول الخليج العربي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "gcc_healthcare_waste_management_system",
        "layer": "GCC_HEALTHCARE_WASTE_MANAGEMENT_SYSTEM_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "gcc-healthcare-waste-management-system-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (23 مادة، جميعها أصلية)",
        "title_en": "GCC Unified System for Healthcare Waste Management — Arabic LLM-ready layer (23 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready GCC Unified System for Healthcare Waste Management records" % (len(ver), len(llm)))
