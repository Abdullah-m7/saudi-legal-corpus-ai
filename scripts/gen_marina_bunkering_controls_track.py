#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الضوابط والإجراءات لعمليات التزود بالوقود في المراسي البحرية السياحيةقرار مجلس إدارة الهيئة السعودية للبحر الأحمر رقم (1ت-2-2024) وتاريخ 15 /01 /1446ه track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26580),
...
SRC = os.path.join(ROOT, "sources", "marina_bunkering_controls", "official_source",
                   "marina_bunkering_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "marina_bunkering_controls", "verified")
RECORDS = os.path.join(OUT_VER, "marina_bunkering_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "marina_bunkering_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "marina_bunkering_controls_arabic_legal_llm",
                        "marina_bunkering_controls_legal_llm_001_010.json")

LAW_ID = "sa-marina-bunkering-controls"
LAW_AR = "الضوابط والإجراءات لعمليات التزود بالوقود في المراسي البحرية السياحيةقرار مجلس إدارة الهيئة السعودية للبحر الأحمر رقم (1ت-2-2024) وتاريخ 15 /01 /1446ه"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"marina_bunkering_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26580), published 1/4/1446H (2024-10-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "marina_bunkering_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "MARINA_BUNKERING_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "marina-bunkering-controls-llm-%03d" % idx,
            ...
            "article_path": "marina_bunkering_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls and Procedures for Bunkering Operations at Tourist Marinas -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/4/1446H / 2024-10-04)"),
                "source_authority_ar": ("الضوابط والإجراءات لعمليات التزود بالوقود في المراسي البحرية السياحيةقرار مجلس إدارة الهيئة السعودية للبحر الأحمر رقم (1ت-2-2024) وتاريخ 15 /01 /1446ه — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "marina_bunkering_controls",
        "layer": "MARINA_BUNKERING_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "marina-bunkering-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Controls and Procedures for Bunkering Operations at Tourist Marinas — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls and Procedures for Bunkering Operations at Tourist Marinas records" % (len(ver), len(llm)))
