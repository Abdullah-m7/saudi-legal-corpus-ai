#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام الأجهزة والمستلزمات الطبية track.
...
RECORDS = os.path.join(ROOT, "sources", "medical_devices_law", "verified",
                       "medical_devices_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "medical_devices_law", "verified",
                       "medical_devices_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "medical_devices_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"medical_devices_law_no_decision_number_in_source", "medical_devices_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "16/7/1442":
        e.append("[2j] gazette_publication_date_hijri must be 16/7/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-02-28":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-02-28")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "نظام الأجهزة والمستلزمات الطبية":
    ...
    if e:
        print("FAIL: %d error(s) in Medical Devices and Supplies Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Medical Devices and Supplies Law")
