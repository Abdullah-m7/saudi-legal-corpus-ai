#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الأنشطة التجريبية track.
...
RECORDS = os.path.join(ROOT, "sources", "experimental_activities_regulation", "verified",
                       "experimental_activities_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "experimental_activities_regulation", "verified",
                       "experimental_activities_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "experimental_activities_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"experimental_activities_regulation_no_decision_number_in_source", "experimental_activities_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة الأنشطة التجريبية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation of Experimental Activities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation of Experimental Activities")
