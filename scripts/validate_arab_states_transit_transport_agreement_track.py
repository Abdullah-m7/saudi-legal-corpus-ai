#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية تنظيم النقل بالعبور «الترانزيت» بين الدول العربية (المعدلة) track.
...
RECORDS = os.path.join(ROOT, "sources", "arab_states_transit_transport_agreement", "verified",
                       "arab_states_transit_transport_agreement_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "arab_states_transit_transport_agreement", "verified",
                       "arab_states_transit_transport_agreement_verified_summary.json")
LLM = os.path.join(ROOT, "data", "arab_states_transit_transport_agreement_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"arab_states_transit_transport_agreement_no_decision_number_in_source", "arab_states_transit_transport_agreement_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية تنظيم النقل بالعبور «الترانزيت» بين الدول العربية (المعدلة)":
    ...
    if e:
        print("FAIL: %d error(s) in Agreement on the Regulation of Transit Transport among Arab States (Amended) track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Agreement on the Regulation of Transit Transport among Arab States (Amended)")
