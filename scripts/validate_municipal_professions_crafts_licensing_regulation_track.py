#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الترخيص البلدي لمزاولة المهن والحرف track.

20 records: 20 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "municipal_professions_crafts_licensing_regulation", "verified",
                       "municipal_professions_crafts_licensing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "municipal_professions_crafts_licensing_regulation", "verified",
                       "municipal_professions_crafts_licensing_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "municipal_professions_crafts_licensing_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"municipal_professions_crafts_licensing_regulation_no_decision_number_in_source", "municipal_professions_crafts_licensing_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "21/4/1443":
        e.append("[2j] gazette_publication_date_hijri must be 21/4/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-11-26":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-11-26")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة الترخيص البلدي لمزاولة المهن والحرف":
    ...
    if e:
        print("FAIL: %d error(s) in Municipal Licensing Regulation for the Practice of Professions and Crafts track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Municipal Licensing Regulation for the Practice of Professions and Crafts")
    print("  - 20 records: 20 articles, no appendices; all 20 اصلية")
