#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة الحد الأدنى للتطقيم الآمن للسفن track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19275),
...
SRC = os.path.join(ROOT, "sources", "safe_manning_regulation", "official_source",
                   "safe_manning_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "safe_manning_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "safe_manning_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "safe_manning_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "safe_manning_regulation_arabic_legal_llm",
                        "safe_manning_regulation_legal_llm_001_016.json")

LAW_ID = "sa-safe-manning-regulation"
LAW_AR = "لائحة الحد الأدنى للتطقيم الآمن للسفن"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"safe_manning_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19275), published 7/9/1443H (2022-04-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "safe_manning_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SAFE_MANNING_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "safe-manning-regulation-llm-%03d" % idx,
            ...
            "article_path": "safe_manning_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on the Minimum Safe Manning of Ships -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/9/1443H / 2022-04-08)"),
                "source_authority_ar": ("لائحة الحد الأدنى للتطقيم الآمن للسفن — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "safe_manning_regulation",
        "layer": "SAFE_MANNING_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "safe-manning-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Regulation on the Minimum Safe Manning of Ships — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on the Minimum Safe Manning of Ships records" % (len(ver), len(llm)))
