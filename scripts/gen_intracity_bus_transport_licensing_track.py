#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the إجراءات وشروط الترخيص لممارسة النقل العام بالحافلات داخل المدن track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20053),
...
SRC = os.path.join(ROOT, "sources", "intracity_bus_transport_licensing", "official_source",
                   "intracity_bus_transport_licensing_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "intracity_bus_transport_licensing", "verified")
RECORDS = os.path.join(OUT_VER, "intracity_bus_transport_licensing_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "intracity_bus_transport_licensing_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "intracity_bus_transport_licensing_arabic_legal_llm",
                        "intracity_bus_transport_licensing_legal_llm_001_010.json")

LAW_ID = "sa-intracity-bus-transport-licensing"
LAW_AR = "إجراءات وشروط الترخيص لممارسة النقل العام بالحافلات داخل المدن"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"intracity_bus_transport_licensing_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20053), published 13/2/1444H (2022-09-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "intracity_bus_transport_licensing",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "INTRACITY_BUS_TRANSPORT_LICENSING_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "intracity-bus-transport-licensing-llm-%03d" % idx,
            ...
            "article_path": "intracity_bus_transport_licensing/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Procedures and Conditions for Licensing Intra-City Public Bus Transport -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/2/1444H / 2022-09-09)"),
                "source_authority_ar": ("إجراءات وشروط الترخيص لممارسة النقل العام بالحافلات داخل المدن — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "intracity_bus_transport_licensing",
        "layer": "INTRACITY_BUS_TRANSPORT_LICENSING_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "intracity-bus-transport-licensing-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Procedures and Conditions for Licensing Intra-City Public Bus Transport — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Procedures and Conditions for Licensing Intra-City Public Bus Transport records" % (len(ver), len(llm)))
