#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate the Batch P1-003 QA (article-by-article review vs the official Arabic; Babs 6/7/8/10).

Confirms the QA covers exactly the 20 P1-003 articles across Babs 6/7/8/10, that every per-article bab is
in [6,7,8,10] and equals both the P1-003 remediation record bab and the coverage-index expected_bab_number,
...
P1-003 remediation file, embeds no full Arabic/English/Chinese text, and touches no protected layer
(P1-003 data, P1-002 data + QA, P1-001 data + QA, all P0 batches + QA, and the base corpora). Review
only: P1-003 data must be unmodified unless a concrete minor fix is recorded. No p1_004+/P2/P3 batch
dirs; no full Chinese 281 / trilingual. Read-only, idempotent.

Usage: validate_chinese_remediation_batch_p1_003_qa.py [QA_JSON_PATH]
...
MD = os.path.join(ROOT, "reports", "chinese_translation_review",
                  "CHINESE_REMEDIATION_BATCH_P1_003_QA_AR.md")
SRC_REL = "data/chinese_remediation_batches/p1_003/companies_law_m132_1443_zh_internal_remediation_p1_003.json"
...
# P0-002..P0-005 QA plus the P1-001 and P1-002 QA are all QA_PASS and must remain untouched.
...
DIST = {"6": [166, 170, 172, 174, 176, 178, 183], "7": [185],
...
REQ_TOP = ("stage", "qa_stage", "batch_id", "source_batch_file", "scope_articles", "expected_babs",
           ...
           "trilingual_alignment_created", "p1_003_chinese_text_modified", "p1_003_data_modified",
...
def main(argv=None) -> int:
    ...
    problems = []
    for p, label in ((qa_path, "QA JSON"), (MD, "Arabic QA report"), (SRC, "P1-003 remediation file")):
    ...
    if qa.get("stage") != "CHINESE_REMEDIATION_BATCH_P1_003_QA":
        problems.append("stage must be CHINESE_REMEDIATION_BATCH_P1_003_QA")
    if qa.get("qa_stage") != "CHINESE_REMEDIATION_BATCH_P1_003_QA":
        problems.append("qa_stage must be CHINESE_REMEDIATION_BATCH_P1_003_QA")
    if qa.get("batch_id") != "P1-003":
        problems.append("batch_id must be P1-003")
    if qa.get("source_batch_file") != SRC_REL:
        problems.append("source_batch_file must point to the P1-003 remediation JSON")
    if qa.get("scope_articles") != ARTS:
        problems.append("scope_articles must exactly match the P1-003 list")
    if qa.get("article_count") != 20:
        problems.append("article_count must be 20")
    if qa.get("expected_babs") != [6, 7, 8, 10]:
        problems.append("expected_babs must be [6, 7, 8, 10]")
    dist = qa.get("expected_bab_distribution") or {}
    if {str(k): list(v) for k, v in dist.items()} != DIST:
        problems.append("expected_bab_distribution must match the authorized Bab 6/7/8/10 split")
    ...
    if nums != ARTS:
        problems.append("per_article_reviews must cover exactly the 20 P1-003 articles in order")
    ...
    for r in recs:
        ...
        if r.get("bab") not in BABS:
            problems.append("art %s bab must be in [6,7,8,10]" % n)
        if n in srec and r.get("bab") != srec[n].get("bab"):
            problems.append("art %s QA bab %r != P1-003 record bab %r" % (n, r.get("bab"), srec[n].get("bab")))
    ...
    if qa.get("p1_003_chinese_text_modified") is True or qa.get("p1_003_data_modified") is True:
        if not fixes:
            problems.append("p1_003 marked modified but no minor_fixes recorded")
    else:
        if fixes:
            problems.append("minor_fixes recorded but p1_003 marked unmodified")
    ...
    # P1-003 remediation intact; P1-002 + P1-001 remediation + all P0 batches + QA unchanged
    if len(src["records"]) != 20:
        problems.append("P1-003 remediation must remain 20 records")
    for label, path in (("P1-001", P1_001_SRC), ("P1-002", P1_002_SRC)):
    ...
    # P1-001/P1-002/P1-003 authorized; only p1_004+/P2/P3 forbidden; no full-281 / trilingual artifacts
    ...
    if later:
        problems.append("only P1-001/P1-002/P1-003 authorized; no p1_004+/P2/P3 batch dirs: %s"
    ...
    print("=" * 60)
    print("Chinese remediation Batch P1-003 QA validation")
    print("=" * 60)
    if problems:
        for p in problems:
            print("  -", p)
        print("RESULT: %d problem(s) found ✗" % len(problems))
        return 1
    print("[PASS] Batch P1-003 QA: 20 articles across Babs [6,7,8,10] reviewed article-by-article vs the "
          "official Arabic (English secondary); each QA bab matches the P1-003 record and the coverage "
          ...
          "model; no full Arabic/English/Chinese text embedded; review-only (P1-003 data unmodified); "
          "P1-002 data + QA, P1-001 data + QA and all P0 batches + QA + Chinese candidate 189 + old "
