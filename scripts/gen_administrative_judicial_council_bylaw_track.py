#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة الداخلية لمجلس القضاء الإداري track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17890),
published 22/11/1442H (2021-07-02). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "administrative_judicial_council_bylaw", "official_source",
                   "administrative_judicial_council_bylaw_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "administrative_judicial_council_bylaw", "verified")
RECORDS = os.path.join(OUT_VER, "administrative_judicial_council_bylaw_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "administrative_judicial_council_bylaw_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "administrative_judicial_council_bylaw_arabic_legal_llm",
                        "administrative_judicial_council_bylaw_legal_llm_001_016.json")

LAW_ID = "sa-administrative-judicial-council-bylaw"
LAW_AR = "اللائحة الداخلية لمجلس القضاء الإداري"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"administrative_judicial_council_bylaw_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17890), published 22/11/1442H (2021-07-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "administrative_judicial_council_bylaw",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "ADMINISTRATIVE_JUDICIAL_COUNCIL_BYLAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "administrative-judicial-council-bylaw-llm-%03d" % idx,
            ...
            "article_path": "administrative_judicial_council_bylaw/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Internal Bylaw of the Administrative Judicial Council -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/11/1442H / 2021-07-02)"),
                "source_authority_ar": ("اللائحة الداخلية لمجلس القضاء الإداري — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 22/11/1442هـ الموافق 2021-07-02م)"),
    ...
    json.dump({
        "law_key": "administrative_judicial_council_bylaw",
        "layer": "ADMINISTRATIVE_JUDICIAL_COUNCIL_BYLAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "administrative-judicial-council-bylaw-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Internal Bylaw of the Administrative Judicial Council — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Internal Bylaw of the Administrative Judicial Council records" % (len(ver), len(llm)))
