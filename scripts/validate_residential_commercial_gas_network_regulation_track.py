#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة نشاط شبكة الغاز للأغراض السكنية والتجارية track.
...
RECORDS = os.path.join(ROOT, "sources", "residential_commercial_gas_network_regulation", "verified",
                       "residential_commercial_gas_network_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "residential_commercial_gas_network_regulation", "verified",
                       "residential_commercial_gas_network_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "residential_commercial_gas_network_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"residential_commercial_gas_network_regulation_no_decision_number_in_source", "residential_commercial_gas_network_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "10/9/1447":
        e.append("[2j] gazette_publication_date_hijri must be 10/9/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-02-27":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-02-27")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة نشاط شبكة الغاز للأغراض السكنية والتجارية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation of the Gas Network Activity for Residential and Commercial Purposes track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation of the Gas Network Activity for Residential and Commercial Purposes")
