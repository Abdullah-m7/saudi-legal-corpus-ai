#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية لفروع الجامعات الأجنبية track.
...
RECORDS = os.path.join(ROOT, "sources", "foreign_university_branches_regulation", "verified",
                       "foreign_university_branches_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "foreign_university_branches_regulation", "verified",
                       "foreign_university_branches_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "foreign_university_branches_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"foreign_university_branches_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية لفروع الجامعات الأجنبية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for Branches of Foreign Universities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for Branches of Foreign Universities")
