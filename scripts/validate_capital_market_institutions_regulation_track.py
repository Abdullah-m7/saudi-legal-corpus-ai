#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة مؤسسات السوق المالية track.

99 records: 99 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "capital_market_institutions_regulation", "verified",
                       "capital_market_institutions_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "capital_market_institutions_regulation", "verified",
                       "capital_market_institutions_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "capital_market_institutions_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"capital_market_institutions_regulation_no_decision_number_in_source", "capital_market_institutions_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "19/12/1444":
        e.append("[2j] gazette_publication_date_hijri must be 19/12/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-07-07":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-07-07")
    ...
    if e:
        print("FAIL: %d error(s) in Capital Market Institutions Regulations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Capital Market Institutions Regulations")
    print("  - 99 records: 99 articles, no appendices; all 99 اصلية")
