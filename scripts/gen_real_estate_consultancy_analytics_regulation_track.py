#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية للاستشارات والتحليلات العقارية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24279),
published 9/6/1445H (2023-12-22). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "real_estate_consultancy_analytics_regulation", "official_source",
                   "real_estate_consultancy_analytics_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "real_estate_consultancy_analytics_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "real_estate_consultancy_analytics_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "real_estate_consultancy_analytics_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "real_estate_consultancy_analytics_regulation_arabic_legal_llm",
                        "real_estate_consultancy_analytics_regulation_legal_llm_001_010.json")

LAW_ID = "sa-real-estate-consultancy-analytics-regulation"
LAW_AR = "اللائحة التنظيمية للاستشارات والتحليلات العقارية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"real_estate_consultancy_analytics_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24279), published 9/6/1445H (2023-12-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "real_estate_consultancy_analytics_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "REAL_ESTATE_CONSULTANCY_ANALYTICS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "real-estate-consultancy-analytics-regulation-llm-%03d" % idx,
            ...
            "article_path": "real_estate_consultancy_analytics_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for Real Estate Consultancy and Analytics -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/6/1445H / 2023-12-22)"),
                "source_authority_ar": ("اللائحة التنظيمية للاستشارات والتحليلات العقارية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 9/6/1445هـ الموافق 2023-12-22م)"),
    ...
    json.dump({
        "law_key": "real_estate_consultancy_analytics_regulation",
        "layer": "REAL_ESTATE_CONSULTANCY_ANALYTICS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "real-estate-consultancy-analytics-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (10 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for Real Estate Consultancy and Analytics — Arabic LLM-ready layer (10 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for Real Estate Consultancy and Analytics records" % (len(ver), len(llm)))
