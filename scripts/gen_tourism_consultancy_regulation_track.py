#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة الاستشارات السياحية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21118),
...
SRC = os.path.join(ROOT, "sources", "tourism_consultancy_regulation", "official_source",
                   "tourism_consultancy_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "tourism_consultancy_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "tourism_consultancy_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "tourism_consultancy_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "tourism_consultancy_regulation_arabic_legal_llm",
                        "tourism_consultancy_regulation_legal_llm_001_023.json")

LAW_ID = "sa-tourism-consultancy-regulation"
LAW_AR = "لائحة الاستشارات السياحية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"tourism_consultancy_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21118), published 1/6/1444H (2022-12-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "tourism_consultancy_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "TOURISM_CONSULTANCY_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "tourism-consultancy-regulation-llm-%03d" % idx,
            ...
            "article_path": "tourism_consultancy_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Tourism Consultancy Regulation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 1/6/1444H / 2022-12-25)"),
                "source_authority_ar": ("لائحة الاستشارات السياحية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "tourism_consultancy_regulation",
        "layer": "TOURISM_CONSULTANCY_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "tourism-consultancy-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (23 مادة، جميعها أصلية)",
        "title_en": "Tourism Consultancy Regulation — Arabic LLM-ready layer (23 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Tourism Consultancy Regulation records" % (len(ver), len(llm)))
