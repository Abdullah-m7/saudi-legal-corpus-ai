#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية للخيل العربية الأصيلة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17203),
...
SRC = os.path.join(ROOT, "sources", "arabian_horse_regulation", "official_source",
                   "arabian_horse_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "arabian_horse_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "arabian_horse_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "arabian_horse_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "arabian_horse_regulation_arabic_legal_llm",
                        "arabian_horse_regulation_legal_llm_001_009.json")

LAW_ID = "sa-arabian-horse-regulation"
LAW_AR = "اللائحة التنظيمية للخيل العربية الأصيلة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"arabian_horse_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17203), published 7/7/1442H (2021-02-19). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "arabian_horse_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "ARABIAN_HORSE_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "arabian-horse-regulation-llm-%03d" % idx,
            ...
            "article_path": "arabian_horse_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for the Purebred Arabian Horse -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/7/1442H / 2021-02-19)"),
                "source_authority_ar": ("اللائحة التنظيمية للخيل العربية الأصيلة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "arabian_horse_regulation",
        "layer": "ARABIAN_HORSE_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "arabian-horse-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (9 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for the Purebred Arabian Horse — Arabic LLM-ready layer (9 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for the Purebred Arabian Horse records" % (len(ver), len(llm)))
