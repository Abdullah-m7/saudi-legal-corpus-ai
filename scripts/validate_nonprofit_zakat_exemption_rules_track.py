#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد عدم خضوع الجمعيات والمؤسسات الأهلية والوحدات التدريبية لجباية الزكاة track.
...
RECORDS = os.path.join(ROOT, "sources", "nonprofit_zakat_exemption_rules", "verified",
                       "nonprofit_zakat_exemption_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "nonprofit_zakat_exemption_rules", "verified",
                       "nonprofit_zakat_exemption_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "nonprofit_zakat_exemption_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"nonprofit_zakat_exemption_rules_no_decision_number_in_source", "nonprofit_zakat_exemption_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد عدم خضوع الجمعيات والمؤسسات الأهلية والوحدات التدريبية لجباية الزكاة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules on the Non-Subjection of Associations, Non-Profit Foundations and Training Units to Zakat Collection track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules on the Non-Subjection of Associations, Non-Profit Foundations and Training Units to Zakat Collection")
