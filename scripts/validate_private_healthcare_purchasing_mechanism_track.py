#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the آلية تنظيم علاقة شراء الرعاية الصحية من القطاع الخاص track.
...
RECORDS = os.path.join(ROOT, "sources", "private_healthcare_purchasing_mechanism", "verified",
                       "private_healthcare_purchasing_mechanism_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "private_healthcare_purchasing_mechanism", "verified",
                       "private_healthcare_purchasing_mechanism_verified_summary.json")
LLM = os.path.join(ROOT, "data", "private_healthcare_purchasing_mechanism_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"private_healthcare_purchasing_mechanism_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "آلية تنظيم علاقة شراء الرعاية الصحية من القطاع الخاص":
    ...
    if e:
        print("FAIL: %d error(s) in Mechanism Regulating the Purchase of Health Care from the Private Sector track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Mechanism Regulating the Purchase of Health Care from the Private Sector")
