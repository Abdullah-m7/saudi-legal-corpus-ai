#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لمركز دعم هيئات التطوير track.
...
RECORDS = os.path.join(ROOT, "sources", "development_authorities_support_center_statute", "verified",
                       "development_authorities_support_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "development_authorities_support_center_statute", "verified",
                       "development_authorities_support_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "development_authorities_support_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"development_authorities_support_center_statute_no_decision_number_in_source", "development_authorities_support_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "23/12/1443":
        e.append("[2j] gazette_publication_date_hijri must be 23/12/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-07-22":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-07-22")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية لمركز دعم هيئات التطوير":
    ...
    if e:
        print("FAIL: %d error(s) in Development Authorities Support Center — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Development Authorities Support Center — Organisational Arrangements")
