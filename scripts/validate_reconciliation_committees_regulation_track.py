#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية للجان إصلاح ذات البين في إمارات المناطق track.
...
RECORDS = os.path.join(ROOT, "sources", "reconciliation_committees_regulation", "verified",
                       "reconciliation_committees_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "reconciliation_committees_regulation", "verified",
                       "reconciliation_committees_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "reconciliation_committees_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"reconciliation_committees_regulation_no_decision_number_in_source", "reconciliation_committees_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "20/2/1444":
        e.append("[2j] gazette_publication_date_hijri must be 20/2/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-09-16":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-09-16")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنظيمية للجان إصلاح ذات البين في إمارات المناطق":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for the Reconciliation Committees in the Regional Emirates track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for the Reconciliation Committees in the Regional Emirates")
