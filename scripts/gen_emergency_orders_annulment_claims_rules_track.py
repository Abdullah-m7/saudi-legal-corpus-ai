#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد نظر دعاوى إلغاء القرارات المتعلقة بتطبيق أحكام أوامر الطوارئ track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27275),
published 23/1/1447H (2025-07-18). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "emergency_orders_annulment_claims_rules", "official_source",
                   "emergency_orders_annulment_claims_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "emergency_orders_annulment_claims_rules", "verified")
RECORDS = os.path.join(OUT_VER, "emergency_orders_annulment_claims_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "emergency_orders_annulment_claims_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "emergency_orders_annulment_claims_rules_arabic_legal_llm",
                        "emergency_orders_annulment_claims_rules_legal_llm_001_008.json")

LAW_ID = "sa-emergency-orders-annulment-claims-rules"
LAW_AR = "قواعد نظر دعاوى إلغاء القرارات المتعلقة بتطبيق أحكام أوامر الطوارئ"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"emergency_orders_annulment_claims_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27275), published 23/1/1447H (2025-07-18). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "emergency_orders_annulment_claims_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "EMERGENCY_ORDERS_ANNULMENT_CLAIMS_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "emergency-orders-annulment-claims-rules-llm-%03d" % idx,
            ...
            "article_path": "emergency_orders_annulment_claims_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules for Hearing Claims to Annul Decisions Related to Applying the Provisions of Emergency Orders -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/1/1447H / 2025-07-18)"),
                "source_authority_ar": ("قواعد نظر دعاوى إلغاء القرارات المتعلقة بتطبيق أحكام أوامر الطوارئ — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 23/1/1447هـ الموافق 2025-07-18م)"),
    ...
    json.dump({
        "law_key": "emergency_orders_annulment_claims_rules",
        "layer": "EMERGENCY_ORDERS_ANNULMENT_CLAIMS_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "emergency-orders-annulment-claims-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Rules for Hearing Claims to Annul Decisions Related to Applying the Provisions of Emergency Orders — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules for Hearing Claims to Annul Decisions Related to Applying the Provisions of Emergency Orders records" % (len(ver), len(llm)))
