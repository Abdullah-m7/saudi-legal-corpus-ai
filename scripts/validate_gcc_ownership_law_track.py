#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the GCC Citizens Real Estate Ownership Regulation track (6 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) — all 6 matched outright (mean 0.976), so none
required visual adjudication; articles are numbered by ordinal position 1..6
...
status for every article (no dual-status divergence). FRESH FULL ISSUANCE: all 6
...
RECORDS = os.path.join(ROOT, "sources", "gcc_ownership", "law", "verified",
                       "gcc_ownership_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "gcc_ownership_arabic_legal_llm",
                   "gcc_ownership_law_legal_llm_001_006.json")
PDF = os.path.join(ROOT, "inputs", "gcc_ownership_official_pdfs",
                   "gcc_ownership_law_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 6
KEY_RE = r"gcc_ownership_art_(\d{3})$"
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 6}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in GCC Citizens Real Estate Ownership Regulation track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: GCC Citizens Real Estate Ownership Regulation — 6 records (fresh issuance: all 6 اصلية)")
    print("  - trust gate: 6/6 MATCHES_PDF outright (mean 0.976); no visual adjudication needed")
    print("  - numbered 1..6 by ordinal position (no مكرر); no dual-status divergence")
    print("  - IN-FORCE Royal Decree M/22 (3/4/1432H); committed MOJ PDF hash verified; Arabic governs")
    return 0
