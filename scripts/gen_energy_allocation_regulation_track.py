#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة تخصيص الطاقة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24969),
...
SRC = os.path.join(ROOT, "sources", "energy_allocation_regulation", "official_source",
                   "energy_allocation_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "energy_allocation_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "energy_allocation_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "energy_allocation_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "energy_allocation_regulation_arabic_legal_llm",
                        "energy_allocation_regulation_legal_llm_001_015.json")

LAW_ID = "sa-energy-allocation-regulation"
LAW_AR = "لائحة تخصيص الطاقة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"energy_allocation_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24969), published 9/11/1445H (2024-05-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "energy_allocation_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "ENERGY_ALLOCATION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "energy-allocation-regulation-llm-%03d" % idx,
            ...
            "article_path": "energy_allocation_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Energy Allocation Regulation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/11/1445H / 2024-05-17)"),
                "source_authority_ar": ("لائحة تخصيص الطاقة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "energy_allocation_regulation",
        "layer": "ENERGY_ALLOCATION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "energy-allocation-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Energy Allocation Regulation — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Energy Allocation Regulation records" % (len(ver), len(llm)))
