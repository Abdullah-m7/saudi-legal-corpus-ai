#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة الوجهات السياحية track.
...
RECORDS = os.path.join(ROOT, "sources", "tourist_destinations_regulation", "verified",
                       "tourist_destinations_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "tourist_destinations_regulation", "verified",
                       "tourist_destinations_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "tourist_destinations_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"tourist_destinations_regulation_no_decision_number_in_source", "tourist_destinations_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة الوجهات السياحية":
    ...
    if e:
        print("FAIL: %d error(s) in Tourist Destinations Regulation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Tourist Destinations Regulation")
