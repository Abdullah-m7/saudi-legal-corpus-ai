#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام صندوق التنمية العقارية track.
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_development_fund_law", "verified",
                       "real_estate_development_fund_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "real_estate_development_fund_law", "verified",
                       "real_estate_development_fund_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "real_estate_development_fund_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"real_estate_development_fund_law_no_decision_number_in_source", "real_estate_development_fund_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "2/6/1442":
        e.append("[2j] gazette_publication_date_hijri must be 2/6/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-01-15":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-01-15")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "نظام صندوق التنمية العقارية":
    ...
    if e:
        print("FAIL: %d error(s) in Law of the Real Estate Development Fund track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law of the Real Estate Development Fund")
