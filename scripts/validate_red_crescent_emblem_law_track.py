#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the نظام استخدام وحماية شارة واسم الهلال الأحمر وما في حكمهما track.
...
RECORDS = os.path.join(ROOT, "sources", "red_crescent_emblem_law", "verified",
                       "red_crescent_emblem_law_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "red_crescent_emblem_law", "verified",
                       "red_crescent_emblem_law_verified_summary.json")
LLM = os.path.join(ROOT, "data", "red_crescent_emblem_law_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"red_crescent_emblem_law_no_decision_number_in_source", "red_crescent_emblem_law_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "نظام استخدام وحماية شارة واسم الهلال الأحمر وما في حكمهما":
    ...
    if e:
        print("FAIL: %d error(s) in Law on the Use and Protection of the Red Crescent Emblem and Name and their Equivalents track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Law on the Use and Protection of the Red Crescent Emblem and Name and their Equivalents")
