#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد الحد من تعارض المصالح لمن سبق له العمل في السلك القضائي عند مزاولته مهنة المحاماة track.
...
RECORDS = os.path.join(ROOT, "sources", "judicial_service_conflict_of_interest_rules", "verified",
                       "judicial_service_conflict_of_interest_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "judicial_service_conflict_of_interest_rules", "verified",
                       "judicial_service_conflict_of_interest_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "judicial_service_conflict_of_interest_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"judicial_service_conflict_of_interest_rules_no_decision_number_in_source", "judicial_service_conflict_of_interest_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/4/1444":
        e.append("[2j] gazette_publication_date_hijri must be 24/4/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-11-18":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-11-18")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد الحد من تعارض المصالح لمن سبق له العمل في السلك القضائي عند مزاولته مهنة المحاماة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules Limiting Conflicts of Interest for Former Members of the Judiciary Practising Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules Limiting Conflicts of Interest for Former Members of the Judiciary Practising Law")
