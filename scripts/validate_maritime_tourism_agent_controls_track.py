#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الضوابط والإجراءات للوكيل الملاحي السياحي track.
...
RECORDS = os.path.join(ROOT, "sources", "maritime_tourism_agent_controls", "verified",
                       "maritime_tourism_agent_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "maritime_tourism_agent_controls", "verified",
                       "maritime_tourism_agent_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "maritime_tourism_agent_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"maritime_tourism_agent_controls_no_decision_number_in_source", "maritime_tourism_agent_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الضوابط والإجراءات للوكيل الملاحي السياحي":
    ...
    if e:
        print("FAIL: %d error(s) in Controls and Procedures for the Maritime Tourism Agent track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls and Procedures for the Maritime Tourism Agent")
