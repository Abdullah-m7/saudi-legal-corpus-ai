#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية لسفن الرحلات السياحية (الكروز) track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23418),
...
SRC = os.path.join(ROOT, "sources", "cruise_ships_regulation", "official_source",
                   "cruise_ships_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "cruise_ships_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "cruise_ships_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "cruise_ships_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "cruise_ships_regulation_arabic_legal_llm",
                        "cruise_ships_regulation_legal_llm_001_011.json")

LAW_ID = "sa-cruise-ships-regulation"
LAW_AR = "اللائحة التنظيمية لسفن الرحلات السياحية (الكروز)"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"cruise_ships_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23418), published 17/1/1445H (2023-08-04). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "cruise_ships_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "CRUISE_SHIPS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "cruise-ships-regulation-llm-%03d" % idx,
            ...
            "article_path": "cruise_ships_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Organisational Regulation for Cruise Ships -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 17/1/1445H / 2023-08-04)"),
                "source_authority_ar": ("اللائحة التنظيمية لسفن الرحلات السياحية (الكروز) — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "cruise_ships_regulation",
        "layer": "CRUISE_SHIPS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "cruise-ships-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Organisational Regulation for Cruise Ships — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Organisational Regulation for Cruise Ships records" % (len(ver), len(llm)))
