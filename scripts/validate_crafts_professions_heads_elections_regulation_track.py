#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة انتخابات رؤساء الحرف والمهن ونوابهم track.
...
RECORDS = os.path.join(ROOT, "sources", "crafts_professions_heads_elections_regulation", "verified",
                       "crafts_professions_heads_elections_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "crafts_professions_heads_elections_regulation", "verified",
                       "crafts_professions_heads_elections_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "crafts_professions_heads_elections_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"crafts_professions_heads_elections_regulation_no_decision_number_in_source", "crafts_professions_heads_elections_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "16/1/1447":
        e.append("[2j] gazette_publication_date_hijri must be 16/1/1447")
    if src.get("gazette_publication_date_gregorian") != "2025-07-11":
        e.append("[2j] gazette_publication_date_gregorian must be 2025-07-11")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة انتخابات رؤساء الحرف والمهن ونوابهم":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on the Election of Heads of Crafts and Professions and their Deputies track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on the Election of Heads of Crafts and Professions and their Deputies")
