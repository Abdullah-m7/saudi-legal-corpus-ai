#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام القياس والمعايرة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=18476),
...
SRC = os.path.join(ROOT, "sources", "metrology_calibration_regulation", "official_source",
                   "metrology_calibration_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "metrology_calibration_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "metrology_calibration_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "metrology_calibration_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "metrology_calibration_regulation_arabic_legal_llm",
                        "metrology_calibration_regulation_legal_llm_001_017.json")

LAW_ID = "sa-metrology-calibration-regulation"
LAW_AR = "اللائحة التنفيذية لنظام القياس والمعايرة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"metrology_calibration_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=18476), published 23/3/1443H (2021-10-29). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "metrology_calibration_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "METROLOGY_CALIBRATION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "metrology-calibration-regulation-llm-%03d" % idx,
            ...
            "article_path": "metrology_calibration_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Metrology and Calibration Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/3/1443H / 2021-10-29)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام القياس والمعايرة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "metrology_calibration_regulation",
        "layer": "METROLOGY_CALIBRATION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "metrology-calibration-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Metrology and Calibration Law — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Metrology and Calibration Law records" % (len(ver), len(llm)))
