#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the السياسة الاقتصادية للطيران المدني في المملكة العربية السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "civil_aviation_economic_policy", "verified",
                       "civil_aviation_economic_policy_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "civil_aviation_economic_policy", "verified",
                       "civil_aviation_economic_policy_verified_summary.json")
LLM = os.path.join(ROOT, "data", "civil_aviation_economic_policy_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"civil_aviation_economic_policy_no_decision_number_in_source", "civil_aviation_economic_policy_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "السياسة الاقتصادية للطيران المدني في المملكة العربية السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in Economic Policy for Civil Aviation in Saudi Arabia track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Economic Policy for Civil Aviation in Saudi Arabia")
