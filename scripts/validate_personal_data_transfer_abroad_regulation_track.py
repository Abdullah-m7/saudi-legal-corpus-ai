#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة نقل البيانات الشخصية خارج المملكة track.
...
RECORDS = os.path.join(ROOT, "sources", "personal_data_transfer_abroad_regulation", "verified",
                       "personal_data_transfer_abroad_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "personal_data_transfer_abroad_regulation", "verified",
                       "personal_data_transfer_abroad_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "personal_data_transfer_abroad_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"personal_data_transfer_abroad_regulation_no_decision_number_in_source", "personal_data_transfer_abroad_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "22/2/1445":
        e.append("[2j] gazette_publication_date_hijri must be 22/2/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-09-07":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-09-07")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة نقل البيانات الشخصية خارج المملكة":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on the Transfer of Personal Data outside the Kingdom track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on the Transfer of Personal Data outside the Kingdom")
