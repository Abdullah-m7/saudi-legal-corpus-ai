#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد منح المكافآت المالية للعاملين على كشف الجرائم والمخالفات المنصوص عليها في نظام مكافحة التستر track.
...
RECORDS = os.path.join(ROOT, "sources", "crime_disclosure_financial_rewards_rules", "verified",
                       "crime_disclosure_financial_rewards_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "crime_disclosure_financial_rewards_rules", "verified",
                       "crime_disclosure_financial_rewards_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "crime_disclosure_financial_rewards_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"crime_disclosure_financial_rewards_rules_no_decision_number_in_source", "crime_disclosure_financial_rewards_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "2/3/1443":
        e.append("[2j] gazette_publication_date_hijri must be 2/3/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-10-08":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-10-08")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "قواعد منح المكافآت المالية للعاملين على كشف الجرائم والمخالفات المنصوص عليها في نظام مكافحة التستر":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Granting Financial Rewards to Those Who Disclose Crimes and Violations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Granting Financial Rewards to Those Who Disclose Crimes and Violations")
