#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة إنشاء الأوقاف أو تمويلها من خلال جمع التبرعات track.
...
RECORDS = os.path.join(ROOT, "sources", "waqf_establishment_donations_regulation", "verified",
                       "waqf_establishment_donations_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "waqf_establishment_donations_regulation", "verified",
                       "waqf_establishment_donations_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "waqf_establishment_donations_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"waqf_establishment_donations_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "لائحة إنشاء الأوقاف أو تمويلها من خلال جمع التبرعات":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation on Establishing or Funding Endowments through the Collection of Donations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation on Establishing or Funding Endowments through the Collection of Donations")
