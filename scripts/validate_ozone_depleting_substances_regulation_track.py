#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية للمواد المستنفدة لطبقة الأوزون ومركبات الكربون الهيدروفلورية track.
...
RECORDS = os.path.join(ROOT, "sources", "ozone_depleting_substances_regulation", "verified",
                       "ozone_depleting_substances_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "ozone_depleting_substances_regulation", "verified",
                       "ozone_depleting_substances_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "ozone_depleting_substances_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"ozone_depleting_substances_regulation_no_decision_number_in_source", "ozone_depleting_substances_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية للمواد المستنفدة لطبقة الأوزون ومركبات الكربون الهيدروفلورية":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation for Ozone-Depleting Substances and Hydrofluorocarbons track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation for Ozone-Depleting Substances and Hydrofluorocarbons")
