#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Real Estate Unit Ownership Law track (33 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ PDF,
hash re-verified) — all 33 matched outright (mean 0.971, min 0.943), so none
required visual adjudication; articles are numbered by ordinal position 1..33
...
status for every article (no dual-status divergence). FRESH FULL ISSUANCE: all 33
اصلية; none amended, repealed or added. Tatweel is banned EXCEPT the 'هـ' digraph
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_units", "law", "verified",
                       "real_estate_units_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "real_estate_units_arabic_legal_llm",
                   "real_estate_units_law_legal_llm_001_033.json")
PDF = os.path.join(ROOT, "inputs", "real_estate_units_official_pdfs",
                   "real_estate_units_law_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 33
KEY_RE = r"real_estate_units_art_(\d{3})$"
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 33}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Real Estate Unit Ownership Law track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Real Estate Unit Ownership Law — 33 records (fresh issuance: all 33 اصلية)")
    print("  - trust gate: 33/33 MATCHES_PDF outright (mean 0.971, min 0.943); no visual adjudication needed")
    print("  - numbered 1..33 by ordinal position (no مكرر); no dual-status divergence")
    print("  - IN-FORCE Royal Decree M/85 (2/7/1441H); committed MOJ PDF hash verified; Arabic governs")
    return 0
