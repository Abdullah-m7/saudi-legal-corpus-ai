#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط ممارسة أنشطة الوساطة في الامتياز التجاري track.
...
RECORDS = os.path.join(ROOT, "sources", "franchise_brokerage_controls", "verified",
                       "franchise_brokerage_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "franchise_brokerage_controls", "verified",
                       "franchise_brokerage_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "franchise_brokerage_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"franchise_brokerage_controls_no_decision_number_in_source", "franchise_brokerage_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "ضوابط ممارسة أنشطة الوساطة في الامتياز التجاري":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on the Practice of Franchise Brokerage Activities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on the Practice of Franchise Brokerage Activities")
