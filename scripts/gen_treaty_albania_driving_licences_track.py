#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية تعاون في مجال استعمال واستبدال رخص القيادة بين حكومتي المملكة وألبانيا track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19877),
...
SRC = os.path.join(ROOT, "sources", "treaty_albania_driving_licences", "official_source",
                   "treaty_albania_driving_licences_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_albania_driving_licences", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_albania_driving_licences_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_albania_driving_licences_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_albania_driving_licences_arabic_legal_llm",
                        "treaty_albania_driving_licences_legal_llm_001_012.json")

LAW_ID = "sa-treaty-albania-driving-licences"
LAW_AR = "اتفاقية تعاون في مجال استعمال واستبدال رخص القيادة بين حكومتي المملكة وألبانيا"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_albania_driving_licences_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19877), published 14/1/1444H (2022-08-12). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_albania_driving_licences",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_ALBANIA_DRIVING_LICENCES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-albania-driving-licences-llm-%03d" % idx,
            ...
            "article_path": "treaty_albania_driving_licences/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Arabia–Albania Agreement on the Use and Exchange of Driving Licences -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/1/1444H / 2022-08-12)"),
                "source_authority_ar": ("اتفاقية تعاون في مجال استعمال واستبدال رخص القيادة بين حكومتي المملكة وألبانيا — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_albania_driving_licences",
        "layer": "TREATY_ALBANIA_DRIVING_LICENCES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-albania-driving-licences-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Saudi Arabia–Albania Agreement on the Use and Exchange of Driving Licences — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Arabia–Albania Agreement on the Use and Exchange of Driving Licences records" % (len(ver), len(llm)))
