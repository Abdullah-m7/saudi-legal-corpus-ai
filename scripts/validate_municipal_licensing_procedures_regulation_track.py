#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنظام إجراءات التراخيص البلدية track.
...
RECORDS = os.path.join(ROOT, "sources", "municipal_licensing_procedures_regulation", "verified",
                       "municipal_licensing_procedures_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "municipal_licensing_procedures_regulation", "verified",
                       "municipal_licensing_procedures_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "municipal_licensing_procedures_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"municipal_licensing_procedures_regulation_no_decision_number_in_source", "municipal_licensing_procedures_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "20/8/1445":
        e.append("[2j] gazette_publication_date_hijri must be 20/8/1445")
    if src.get("gazette_publication_date_gregorian") != "2024-03-01":
        e.append("[2j] gazette_publication_date_gregorian must be 2024-03-01")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنفيذية لنظام إجراءات التراخيص البلدية":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation of the Municipal Licensing Procedures Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation of the Municipal Licensing Procedures Law")
