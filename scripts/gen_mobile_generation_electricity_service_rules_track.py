#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط تقديم الخدمة الكهربائية عن طريق التوليد المتنقل track.
...
server-rendered HTML page (uqn.gov.sa/details?p=21467),
...
SRC = os.path.join(ROOT, "sources", "mobile_generation_electricity_service_rules", "official_source",
                   "mobile_generation_electricity_service_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "mobile_generation_electricity_service_rules", "verified")
RECORDS = os.path.join(OUT_VER, "mobile_generation_electricity_service_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "mobile_generation_electricity_service_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "mobile_generation_electricity_service_rules_arabic_legal_llm",
                        "mobile_generation_electricity_service_rules_legal_llm_001_013.json")

LAW_ID = "sa-mobile-generation-electricity-service-rules"
LAW_AR = "ضوابط تقديم الخدمة الكهربائية عن طريق التوليد المتنقل"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"mobile_generation_electricity_service_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=21467), published 26/7/1444H (2023-02-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "mobile_generation_electricity_service_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "MOBILE_GENERATION_ELECTRICITY_SERVICE_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "mobile-generation-electricity-service-rules-llm-%03d" % idx,
            ...
            "article_path": "mobile_generation_electricity_service_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls for Providing Electricity Service through Mobile Generation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 26/7/1444H / 2023-02-17)"),
                "source_authority_ar": ("ضوابط تقديم الخدمة الكهربائية عن طريق التوليد المتنقل — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "mobile_generation_electricity_service_rules",
        "layer": "MOBILE_GENERATION_ELECTRICITY_SERVICE_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "mobile-generation-electricity-service-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Controls for Providing Electricity Service through Mobile Generation — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls for Providing Electricity Service through Mobile Generation records" % (len(ver), len(llm)))
