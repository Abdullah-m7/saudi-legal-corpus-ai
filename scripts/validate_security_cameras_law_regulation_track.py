#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنظام استخدام كاميرات المراقبة الأمنية track.
...
RECORDS = os.path.join(ROOT, "sources", "security_cameras_law_regulation", "verified",
                       "security_cameras_law_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "security_cameras_law_regulation", "verified",
                       "security_cameras_law_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "security_cameras_law_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"security_cameras_law_regulation_no_decision_number_in_source", "security_cameras_law_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية لنظام استخدام كاميرات المراقبة الأمنية":
    ...
    if e:
        print("FAIL: %d error(s) in Implementing Regulation of the Law on the Use of Security Surveillance Cameras track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Implementing Regulation of the Law on the Use of Security Surveillance Cameras")
