#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة مخالفات دعم صندوق تنمية الموارد البشرية \"هدف\" track.
...
server-rendered HTML page (uqn.gov.sa/details?p=25399),
...
SRC = os.path.join(ROOT, "sources", "hrdf_support_violations_regulation", "official_source",
                   "hrdf_support_violations_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "hrdf_support_violations_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "hrdf_support_violations_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "hrdf_support_violations_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "hrdf_support_violations_regulation_arabic_legal_llm",
                        "hrdf_support_violations_regulation_legal_llm_001_017.json")

LAW_ID = "sa-hrdf-support-violations-regulation"
LAW_AR = "لائحة مخالفات دعم صندوق تنمية الموارد البشرية \"هدف\""
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"hrdf_support_violations_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=25399), published 19/2/1446H (2024-08-23). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "hrdf_support_violations_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "HRDF_SUPPORT_VIOLATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "hrdf-support-violations-regulation-llm-%03d" % idx,
            ...
            "article_path": "hrdf_support_violations_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulation on Violations of Human Resources Development Fund (HADAF) Support -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 19/2/1446H / 2024-08-23)"),
                "source_authority_ar": ("لائحة مخالفات دعم صندوق تنمية الموارد البشرية \"هدف\" — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "hrdf_support_violations_regulation",
        "layer": "HRDF_SUPPORT_VIOLATIONS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "hrdf-support-violations-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Regulation on Violations of Human Resources Development Fund (HADAF) Support — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulation on Violations of Human Resources Development Fund (HADAF) Support records" % (len(ver), len(llm)))
