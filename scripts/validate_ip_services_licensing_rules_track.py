...
RECORDS = os.path.join(ROOT, "sources", "ip_services_licensing_rules", "verified",
                       "ip_services_licensing_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "ip_services_licensing_rules", "verified",
                       "ip_services_licensing_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "ip_services_licensing_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"ip_services_licensing_rules_no_decision_number_in_source", "ip_services_licensing_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Licensing Rules for the Provision of Intellectual Property Services track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Licensing Rules for the Provision of Intellectual Property Services")
