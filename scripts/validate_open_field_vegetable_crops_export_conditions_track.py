#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the شروط وضوابط تصدير محاصيل الخضار المزروعة في مساحات مكشوفة تعتمد في الري على المياه المتجددة track.
...
RECORDS = os.path.join(ROOT, "sources", "open_field_vegetable_crops_export_conditions", "verified",
                       "open_field_vegetable_crops_export_conditions_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "open_field_vegetable_crops_export_conditions", "verified",
                       "open_field_vegetable_crops_export_conditions_verified_summary.json")
LLM = os.path.join(ROOT, "data", "open_field_vegetable_crops_export_conditions_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"open_field_vegetable_crops_export_conditions_no_decision_number_in_source", "open_field_vegetable_crops_export_conditions_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "16/3/1443":
        e.append("[2j] gazette_publication_date_hijri must be 16/3/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-10-22":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-10-22")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "شروط وضوابط تصدير محاصيل الخضار المزروعة في مساحات مكشوفة تعتمد في الري على المياه المتجددة":
    ...
    if e:
        print("FAIL: %d error(s) in Conditions and Controls for Exporting Vegetable Crops Grown in Open Areas Irrigated with Renewable Water track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Conditions and Controls for Exporting Vegetable Crops Grown in Open Areas Irrigated with Renewable Water")
