#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الآلية التصحيحية للمنشآت القائمة والتي تقدم خدمات المحاسبة قبل صدور قرار مجلس إدارة الهيئة السعودية للمراجعين والمحاسبين رقم (4 /1) وتاريخ 2 /9/ 1443ه track.

7 records: 7 numbered articles, no appendix records. All اصلية.
...
RECORDS = os.path.join(ROOT, "sources", "accounting_services_corrective_mechanism", "verified",
                       "accounting_services_corrective_mechanism_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "accounting_services_corrective_mechanism", "verified",
                       "accounting_services_corrective_mechanism_verified_summary.json")
LLM = os.path.join(ROOT, "data", "accounting_services_corrective_mechanism_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"accounting_services_corrective_mechanism_no_decision_number_in_source", "accounting_services_corrective_mechanism_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "24/1/1445":
        e.append("[2j] gazette_publication_date_hijri must be 24/1/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-08-11":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-08-11")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الآلية التصحيحية للمنشآت القائمة والتي تقدم خدمات المحاسبة قبل صدور قرار مجلس إدارة الهيئة السعودية للمراجعين والمحاسبين رقم (4 /1) وتاريخ 2 /9/ 1443ه":
    ...
    if e:
        print("FAIL: %d error(s) in Corrective Mechanism for Establishments Providing Accounting Services Before the Decision track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Corrective Mechanism for Establishments Providing Accounting Services Before the Decision")
    print("  - 7 records: 7 articles, no appendices; all 7 اصلية")
