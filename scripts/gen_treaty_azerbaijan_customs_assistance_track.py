#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية بين حكومتي المملكة وأذربيجان حول التعاون والمساعدة المتبادلة في المسائل الجمركية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19435),
published 12/10/1443H (2022-05-13). The Gazette IS the official publication of record
...
STRUCTURE: 20 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "treaty_azerbaijan_customs_assistance", "official_source",
                   "treaty_azerbaijan_customs_assistance_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_azerbaijan_customs_assistance", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_azerbaijan_customs_assistance_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_azerbaijan_customs_assistance_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_azerbaijan_customs_assistance_arabic_legal_llm",
                        "treaty_azerbaijan_customs_assistance_legal_llm_001_020.json")

LAW_ID = "sa-treaty-azerbaijan-customs-assistance"
LAW_AR = "اتفاقية بين حكومتي المملكة وأذربيجان حول التعاون والمساعدة المتبادلة في المسائل الجمركية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_azerbaijan_customs_assistance_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19435), published 12/10/1443H (2022-05-13). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_azerbaijan_customs_assistance",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_AZERBAIJAN_CUSTOMS_ASSISTANCE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-azerbaijan-customs-assistance-llm-%03d" % idx,
            ...
            "article_path": "treaty_azerbaijan_customs_assistance/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Arabia–Azerbaijan Agreement on Cooperation and Mutual Assistance in Customs Matters -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 12/10/1443H / 2022-05-13)"),
                "source_authority_ar": ("اتفاقية بين حكومتي المملكة وأذربيجان حول التعاون والمساعدة المتبادلة في المسائل الجمركية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 12/10/1443هـ الموافق 2022-05-13م)"),
    ...
    json.dump({
        "law_key": "treaty_azerbaijan_customs_assistance",
        "layer": "TREATY_AZERBAIJAN_CUSTOMS_ASSISTANCE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-azerbaijan-customs-assistance-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Saudi Arabia–Azerbaijan Agreement on Cooperation and Mutual Assistance in Customs Matters — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Arabia–Azerbaijan Agreement on Cooperation and Mutual Assistance in Customs Matters records" % (len(ver), len(llm)))
