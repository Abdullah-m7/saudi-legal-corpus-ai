#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد المنظمة لمهنة الاستشارات المالية (لغير الأوراق المالية) track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4000290),
published 11/8/1447H (2026-01-30). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "financial_advisory_profession_rules", "official_source",
                   "financial_advisory_profession_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "financial_advisory_profession_rules", "verified")
RECORDS = os.path.join(OUT_VER, "financial_advisory_profession_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "financial_advisory_profession_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "financial_advisory_profession_rules_arabic_legal_llm",
                        "financial_advisory_profession_rules_legal_llm_001_019.json")

LAW_ID = "sa-financial-advisory-profession-rules"
LAW_AR = "القواعد المنظمة لمهنة الاستشارات المالية (لغير الأوراق المالية)"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"financial_advisory_profession_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4000290), published 11/8/1447H (2026-01-30). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "financial_advisory_profession_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "FINANCIAL_ADVISORY_PROFESSION_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "financial-advisory-profession-rules-llm-%03d" % idx,
            ...
            "article_path": "financial_advisory_profession_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Governing the Financial Advisory Profession (Non-Securities) -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 11/8/1447H / 2026-01-30)"),
                "source_authority_ar": ("القواعد المنظمة لمهنة الاستشارات المالية (لغير الأوراق المالية) — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 11/8/1447هـ الموافق 2026-01-30م)"),
    ...
    json.dump({
        "law_key": "financial_advisory_profession_rules",
        "layer": "FINANCIAL_ADVISORY_PROFESSION_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "financial-advisory-profession-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (19 مادة، جميعها أصلية)",
        "title_en": "Rules Governing the Financial Advisory Profession (Non-Securities) — Arabic LLM-ready layer (19 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Governing the Financial Advisory Profession (Non-Securities) records" % (len(ver), len(llm)))
