#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم مدينة الملك عبدالعزيز للعلوم والتقنية track.
...
RECORDS = os.path.join(ROOT, "sources", "kacst_statute", "verified",
                       "kacst_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "kacst_statute", "verified",
                       "kacst_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "kacst_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"kacst_statute_no_decision_number_in_source", "kacst_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم مدينة الملك عبدالعزيز للعلوم والتقنية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the King Abdulaziz City for Science and Technology track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the King Abdulaziz City for Science and Technology")
