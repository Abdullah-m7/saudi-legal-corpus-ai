#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the آلية تنظيم علاقة شراء الرعاية الصحية من القطاع الخاص track.
...
server-rendered HTML page (uqn.gov.sa/details?p=28745),
...
SRC = os.path.join(ROOT, "sources", "private_healthcare_purchasing_mechanism", "official_source",
                   "private_healthcare_purchasing_mechanism_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "private_healthcare_purchasing_mechanism", "verified")
RECORDS = os.path.join(OUT_VER, "private_healthcare_purchasing_mechanism_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "private_healthcare_purchasing_mechanism_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "private_healthcare_purchasing_mechanism_arabic_legal_llm",
                        "private_healthcare_purchasing_mechanism_legal_llm_001_008.json")

LAW_ID = "sa-private-healthcare-purchasing-mechanism"
LAW_AR = "آلية تنظيم علاقة شراء الرعاية الصحية من القطاع الخاص"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"private_healthcare_purchasing_mechanism_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=28745), published 13/7/1447H (2026-01-02). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "private_healthcare_purchasing_mechanism",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PRIVATE_HEALTHCARE_PURCHASING_MECHANISM_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "private-healthcare-purchasing-mechanism-llm-%03d" % idx,
            ...
            "article_path": "private_healthcare_purchasing_mechanism/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Mechanism Regulating the Purchase of Health Care from the Private Sector -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/7/1447H / 2026-01-02)"),
                "source_authority_ar": ("آلية تنظيم علاقة شراء الرعاية الصحية من القطاع الخاص — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "private_healthcare_purchasing_mechanism",
        "layer": "PRIVATE_HEALTHCARE_PURCHASING_MECHANISM_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "private-healthcare-purchasing-mechanism-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Mechanism Regulating the Purchase of Health Care from the Private Sector — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Mechanism Regulating the Purchase of Health Care from the Private Sector records" % (len(ver), len(llm)))
