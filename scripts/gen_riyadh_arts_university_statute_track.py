#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام الأساس لجامعة الرياض للفنون track.
...
server-rendered HTML page (uqn.gov.sa/details?p=4000576),
...
SRC = os.path.join(ROOT, "sources", "riyadh_arts_university_statute", "official_source",
                   "riyadh_arts_university_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "riyadh_arts_university_statute", "verified")
RECORDS = os.path.join(OUT_VER, "riyadh_arts_university_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "riyadh_arts_university_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "riyadh_arts_university_statute_arabic_legal_llm",
                        "riyadh_arts_university_statute_legal_llm_001_020.json")

LAW_ID = "sa-riyadh-arts-university-statute"
LAW_AR = "النظام الأساس لجامعة الرياض للفنون"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"riyadh_arts_university_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=4000576), published 24/9/1447H (2026-03-13). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "riyadh_arts_university_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "RIYADH_ARTS_UNIVERSITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "riyadh-arts-university-statute-llm-%03d" % idx,
            ...
            "article_path": "riyadh_arts_university_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of Riyadh University for the Arts -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/9/1447H / 2026-03-13)"),
                "source_authority_ar": ("النظام الأساس لجامعة الرياض للفنون — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "riyadh_arts_university_statute",
        "layer": "RIYADH_ARTS_UNIVERSITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "riyadh-arts-university-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (20 مادة، جميعها أصلية)",
        "title_en": "Statute of Riyadh University for the Arts — Arabic LLM-ready layer (20 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of Riyadh University for the Arts records" % (len(ver), len(llm)))
