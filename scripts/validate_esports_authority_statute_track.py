#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية للهيئة السعودية للألعاب والرياضات الإلكترونية track.
...
RECORDS = os.path.join(ROOT, "sources", "esports_authority_statute", "verified",
                       "esports_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "esports_authority_statute", "verified",
                       "esports_authority_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "esports_authority_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"esports_authority_statute_no_decision_number_in_source", "esports_authority_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الترتيبات التنظيمية للهيئة السعودية للألعاب والرياضات الإلكترونية":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Esports and Electronic Games Authority — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Esports and Electronic Games Authority — Organisational Arrangements")
