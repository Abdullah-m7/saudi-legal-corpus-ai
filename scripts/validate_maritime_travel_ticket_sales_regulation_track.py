#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة المنظمة لنشاط بيع تذاكر السفر البحرية track.
...
RECORDS = os.path.join(ROOT, "sources", "maritime_travel_ticket_sales_regulation", "verified",
                       "maritime_travel_ticket_sales_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "maritime_travel_ticket_sales_regulation", "verified",
                       "maritime_travel_ticket_sales_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "maritime_travel_ticket_sales_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"maritime_travel_ticket_sales_regulation_no_decision_number_in_source", "maritime_travel_ticket_sales_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة المنظمة لنشاط بيع تذاكر السفر البحرية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation Governing the Sale of Maritime Travel Tickets track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation Governing the Sale of Maritime Travel Tickets")
