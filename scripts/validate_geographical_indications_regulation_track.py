#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لنظام حماية المؤشرات الجغرافية track.
...
RECORDS = os.path.join(ROOT, "sources", "geographical_indications_regulation", "verified",
                       "geographical_indications_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "geographical_indications_regulation", "verified",
                       "geographical_indications_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "geographical_indications_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"geographical_indications_regulation_no_decision_number_in_source", "geographical_indications_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "5/12/1447":
        e.append("[2j] gazette_publication_date_hijri must be 5/12/1447")
    if src.get("gazette_publication_date_gregorian") != "2026-05-22":
        e.append("[2j] gazette_publication_date_gregorian must be 2026-05-22")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنفيذية لنظام حماية المؤشرات الجغرافية":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation of the Geographical Indications Protection Law track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation of the Geographical Indications Protection Law")
