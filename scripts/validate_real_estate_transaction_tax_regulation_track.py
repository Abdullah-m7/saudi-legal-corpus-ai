#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لضريبة التصرفات العقارية track.
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_transaction_tax_regulation", "verified",
                       "real_estate_transaction_tax_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "real_estate_transaction_tax_regulation", "verified",
                       "real_estate_transaction_tax_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "real_estate_transaction_tax_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"real_estate_transaction_tax_regulation_no_decision_number_in_source", "real_estate_transaction_tax_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "21/1/1444":
        e.append("[2j] gazette_publication_date_hijri must be 21/1/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-08-19":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-08-19")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنفيذية لضريبة التصرفات العقارية":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation of the Real Estate Transaction Tax track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation of the Real Estate Transaction Tax")
