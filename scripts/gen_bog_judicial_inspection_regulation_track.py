#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the لائحة التفتيش القضائي بديوان المظالم track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27251),
...
SRC = os.path.join(ROOT, "sources", "bog_judicial_inspection_regulation", "official_source",
                   "bog_judicial_inspection_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "bog_judicial_inspection_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "bog_judicial_inspection_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "bog_judicial_inspection_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "bog_judicial_inspection_regulation_arabic_legal_llm",
                        "bog_judicial_inspection_regulation_legal_llm_001_042.json")

LAW_ID = "sa-bog-judicial-inspection-regulation"
LAW_AR = "لائحة التفتيش القضائي بديوان المظالم"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"bog_judicial_inspection_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27251), published 16/1/1447H (2025-07-11). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "bog_judicial_inspection_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "BOG_JUDICIAL_INSPECTION_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "bog-judicial-inspection-regulation-llm-%03d" % idx,
            ...
            "article_path": "bog_judicial_inspection_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Judicial Inspection Regulation of the Board of Grievances -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 16/1/1447H / 2025-07-11)"),
                "source_authority_ar": ("لائحة التفتيش القضائي بديوان المظالم — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "bog_judicial_inspection_regulation",
        "layer": "BOG_JUDICIAL_INSPECTION_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "bog-judicial-inspection-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (42 مادة، جميعها أصلية)",
        "title_en": "Judicial Inspection Regulation of the Board of Grievances — Arabic LLM-ready layer (42 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Judicial Inspection Regulation of the Board of Grievances records" % (len(ver), len(llm)))
