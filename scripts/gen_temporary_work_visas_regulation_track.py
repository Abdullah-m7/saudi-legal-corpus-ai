#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية لتأشيرات العمل المؤقت والعمل المؤقت لخدمات الحج والعمرة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=26602),
published 8/4/1446H (2024-10-11). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "temporary_work_visas_regulation", "official_source",
                   "temporary_work_visas_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "temporary_work_visas_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "temporary_work_visas_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "temporary_work_visas_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "temporary_work_visas_regulation_arabic_legal_llm",
                        "temporary_work_visas_regulation_legal_llm_001_023.json")

LAW_ID = "sa-temporary-work-visas-regulation"
LAW_AR = "اللائحة التنظيمية لتأشيرات العمل المؤقت والعمل المؤقت لخدمات الحج والعمرة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"temporary_work_visas_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=26602), published 8/4/1446H (2024-10-11). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "temporary_work_visas_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "TEMPORARY_WORK_VISAS_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "temporary-work-visas-regulation-llm-%03d" % idx,
            ...
            "article_path": "temporary_work_visas_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for Temporary Work Visas and Temporary Work for Hajj and Umrah Services -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 8/4/1446H / 2024-10-11)"),
                "source_authority_ar": ("اللائحة التنظيمية لتأشيرات العمل المؤقت والعمل المؤقت لخدمات الحج والعمرة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 8/4/1446هـ الموافق 2024-10-11م)"),
    ...
    json.dump({
        "law_key": "temporary_work_visas_regulation",
        "layer": "TEMPORARY_WORK_VISAS_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "temporary-work-visas-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (23 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for Temporary Work Visas and Temporary Work for Hajj and Umrah Services — Arabic LLM-ready layer (23 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for Temporary Work Visas and Temporary Work for Hajj and Umrah Services records" % (len(ver), len(llm)))
