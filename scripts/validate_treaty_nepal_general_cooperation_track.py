#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية عامة للتعاون بين حكومتي المملكة وجمهورية نيبال track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_nepal_general_cooperation", "verified",
                       "treaty_nepal_general_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_nepal_general_cooperation", "verified",
                       "treaty_nepal_general_cooperation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_nepal_general_cooperation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_nepal_general_cooperation_no_decision_number_in_source", "treaty_nepal_general_cooperation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية عامة للتعاون بين حكومتي المملكة وجمهورية نيبال":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Nepal General Cooperation Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Nepal General Cooperation Agreement")
