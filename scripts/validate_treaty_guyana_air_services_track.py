#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية بين حكومتي المملكة وغويانا التعاونية في مجال خدمات النقل الجوي track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_guyana_air_services", "verified",
                       "treaty_guyana_air_services_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_guyana_air_services", "verified",
                       "treaty_guyana_air_services_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_guyana_air_services_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_guyana_air_services_no_decision_number_in_source", "treaty_guyana_air_services_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اتفاقية بين حكومتي المملكة وغويانا التعاونية في مجال خدمات النقل الجوي":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Guyana Air Services Agreement track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Guyana Air Services Agreement")
