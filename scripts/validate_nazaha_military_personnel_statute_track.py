#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the التنظيم الخاص بالعسكريين في هيئة الرقابة ومكافحة الفساد track.
...
RECORDS = os.path.join(ROOT, "sources", "nazaha_military_personnel_statute", "verified",
                       "nazaha_military_personnel_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "nazaha_military_personnel_statute", "verified",
                       "nazaha_military_personnel_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "nazaha_military_personnel_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"nazaha_military_personnel_statute_no_decision_number_in_source", "nazaha_military_personnel_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "1/12/1444":
        e.append("[2j] gazette_publication_date_hijri must be 1/12/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-06-19":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-06-19")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "التنظيم الخاص بالعسكريين في هيئة الرقابة ومكافحة الفساد":
    ...
    if e:
        print("FAIL: %d error(s) in Provisions Governing Military Personnel at the Oversight and Anti-Corruption Authority track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Provisions Governing Military Personnel at the Oversight and Anti-Corruption Authority")
