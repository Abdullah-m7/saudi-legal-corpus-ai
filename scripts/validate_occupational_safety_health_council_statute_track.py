#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم المجلس الوطني للسلامة والصحة المهنية track.
...
RECORDS = os.path.join(ROOT, "sources", "occupational_safety_health_council_statute", "verified",
                       "occupational_safety_health_council_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "occupational_safety_health_council_statute", "verified",
                       "occupational_safety_health_council_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "occupational_safety_health_council_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"occupational_safety_health_council_statute_no_decision_number_in_source", "occupational_safety_health_council_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "17/7/1443":
        e.append("[2j] gazette_publication_date_hijri must be 17/7/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-02-18":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-02-18")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "تنظيم المجلس الوطني للسلامة والصحة المهنية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the National Council for Occupational Safety and Health track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the National Council for Occupational Safety and Health")
