#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the شروط وضوابط استيراد الخضار والفاكهة الطازجة للمملكة track.
...
RECORDS = os.path.join(ROOT, "sources", "fresh_produce_import_controls", "verified",
                       "fresh_produce_import_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "fresh_produce_import_controls", "verified",
                       "fresh_produce_import_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "fresh_produce_import_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"fresh_produce_import_controls_no_decision_number_in_source", "fresh_produce_import_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "شروط وضوابط استيراد الخضار والفاكهة الطازجة للمملكة":
    ...
    if e:
        print("FAIL: %d error(s) in Conditions and Controls for Importing Fresh Vegetables and Fruit track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Conditions and Controls for Importing Fresh Vegetables and Fruit")
