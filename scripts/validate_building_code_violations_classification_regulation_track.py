#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the لائحة تصنيف مخالفات كود البناء السعودي track.
...
RECORDS = os.path.join(ROOT, "sources", "building_code_violations_classification_regulation", "verified",
                       "building_code_violations_classification_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "building_code_violations_classification_regulation", "verified",
                       "building_code_violations_classification_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "building_code_violations_classification_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"building_code_violations_classification_regulation_no_decision_number_in_source", "building_code_violations_classification_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "9/3/1443":
        e.append("[2j] gazette_publication_date_hijri must be 9/3/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-10-15":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-10-15")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "لائحة تصنيف مخالفات كود البناء السعودي":
    ...
    if e:
        print("FAIL: %d error(s) in Regulation Classifying Saudi Building Code Violations track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulation Classifying Saudi Building Code Violations")
