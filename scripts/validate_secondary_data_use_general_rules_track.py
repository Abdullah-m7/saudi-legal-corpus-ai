#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد العامة للاستخدام الثانوي للبيانات track.
...
RECORDS = os.path.join(ROOT, "sources", "secondary_data_use_general_rules", "verified",
                       "secondary_data_use_general_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "secondary_data_use_general_rules", "verified",
                       "secondary_data_use_general_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "secondary_data_use_general_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"secondary_data_use_general_rules_no_decision_number_in_source", "secondary_data_use_general_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد العامة للاستخدام الثانوي للبيانات":
    ...
    if e:
        print("FAIL: %d error(s) in General Rules for the Secondary Use of Data track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: General Rules for the Secondary Use of Data")
