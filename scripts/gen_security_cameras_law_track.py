#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام استخدام كاميرات المراقبة الأمنية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20291),
...
SRC = os.path.join(ROOT, "sources", "security_cameras_law", "official_source",
                   "security_cameras_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "security_cameras_law", "verified")
RECORDS = os.path.join(OUT_VER, "security_cameras_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "security_cameras_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "security_cameras_law_arabic_legal_llm",
                        "security_cameras_law_legal_llm_001_018.json")

LAW_ID = "sa-security-cameras-law"
LAW_AR = "نظام استخدام كاميرات المراقبة الأمنية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"security_cameras_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20291), published 11/3/1444H (2022-10-07). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "security_cameras_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "SECURITY_CAMERAS_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "security-cameras-law-llm-%03d" % idx,
            ...
            "article_path": "security_cameras_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law on the Use of Security Surveillance Cameras -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 11/3/1444H / 2022-10-07)"),
                "source_authority_ar": ("نظام استخدام كاميرات المراقبة الأمنية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "security_cameras_law",
        "layer": "SECURITY_CAMERAS_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "security-cameras-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Law on the Use of Security Surveillance Cameras — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law on the Use of Security Surveillance Cameras records" % (len(ver), len(llm)))
