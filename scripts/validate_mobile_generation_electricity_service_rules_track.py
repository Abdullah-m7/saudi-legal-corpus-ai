#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط تقديم الخدمة الكهربائية عن طريق التوليد المتنقل track.
...
RECORDS = os.path.join(ROOT, "sources", "mobile_generation_electricity_service_rules", "verified",
                       "mobile_generation_electricity_service_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "mobile_generation_electricity_service_rules", "verified",
                       "mobile_generation_electricity_service_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "mobile_generation_electricity_service_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"mobile_generation_electricity_service_rules_no_decision_number_in_source", "mobile_generation_electricity_service_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "26/7/1444":
        e.append("[2j] gazette_publication_date_hijri must be 26/7/1444")
    if src.get("gazette_publication_date_gregorian") != "2023-02-17":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-02-17")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط تقديم الخدمة الكهربائية عن طريق التوليد المتنقل":
    ...
    if e:
        print("FAIL: %d error(s) in Controls for Providing Electricity Service through Mobile Generation track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls for Providing Electricity Service through Mobile Generation")
