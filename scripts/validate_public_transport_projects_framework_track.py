#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الإطار التنظيمي لمشاريع النقل العام track.
...
RECORDS = os.path.join(ROOT, "sources", "public_transport_projects_framework", "verified",
                       "public_transport_projects_framework_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "public_transport_projects_framework", "verified",
                       "public_transport_projects_framework_verified_summary.json")
LLM = os.path.join(ROOT, "data", "public_transport_projects_framework_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"public_transport_projects_framework_no_decision_number_in_source", "public_transport_projects_framework_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الإطار التنظيمي لمشاريع النقل العام":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Framework for Public Transport Projects track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Framework for Public Transport Projects")
