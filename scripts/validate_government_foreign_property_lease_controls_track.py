#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط استئجار الجهات الحكومية للعقارات خارج المملكة العربية السعودية track.
...
RECORDS = os.path.join(ROOT, "sources", "government_foreign_property_lease_controls", "verified",
                       "government_foreign_property_lease_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "government_foreign_property_lease_controls", "verified",
                       "government_foreign_property_lease_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "government_foreign_property_lease_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"government_foreign_property_lease_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "ضوابط استئجار الجهات الحكومية للعقارات خارج المملكة العربية السعودية":
    ...
    if e:
        print("FAIL: %d error(s) in Controls on the Leasing of Real Estate Abroad by Government Entities track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls on the Leasing of Real Estate Abroad by Government Entities")
