#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط شراء واستئجار الجهات الحكومية للمركبات track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27011),
published 22/8/1446H (2025-02-21). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "government_vehicle_purchase_lease_controls", "official_source",
                   "government_vehicle_purchase_lease_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "government_vehicle_purchase_lease_controls", "verified")
RECORDS = os.path.join(OUT_VER, "government_vehicle_purchase_lease_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "government_vehicle_purchase_lease_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "government_vehicle_purchase_lease_controls_arabic_legal_llm",
                        "government_vehicle_purchase_lease_controls_legal_llm_001_015.json")

LAW_ID = "sa-government-vehicle-purchase-lease-controls"
LAW_AR = "ضوابط شراء واستئجار الجهات الحكومية للمركبات"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"government_vehicle_purchase_lease_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27011), published 22/8/1446H (2025-02-21). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "government_vehicle_purchase_lease_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "GOVERNMENT_VEHICLE_PURCHASE_LEASE_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "government-vehicle-purchase-lease-controls-llm-%03d" % idx,
            ...
            "article_path": "government_vehicle_purchase_lease_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on the Purchase and Lease of Vehicles by Government Bodies -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 22/8/1446H / 2025-02-21)"),
                "source_authority_ar": ("ضوابط شراء واستئجار الجهات الحكومية للمركبات — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 22/8/1446هـ الموافق 2025-02-21م)"),
    ...
    json.dump({
        "law_key": "government_vehicle_purchase_lease_controls",
        "layer": "GOVERNMENT_VEHICLE_PURCHASE_LEASE_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "government-vehicle-purchase-lease-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (15 مادة، جميعها أصلية)",
        "title_en": "Controls on the Purchase and Lease of Vehicles by Government Bodies — Arabic LLM-ready layer (15 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on the Purchase and Lease of Vehicles by Government Bodies records" % (len(ver), len(llm)))
