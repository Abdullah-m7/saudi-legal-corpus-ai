#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the ضوابط تخصيص واسترداد ومناقلة العقارات بين الجهات الحكومية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17337),
published 9/6/1442H (2021-01-22). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "government_property_allocation_transfer_controls", "official_source",
                   "government_property_allocation_transfer_controls_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "government_property_allocation_transfer_controls", "verified")
RECORDS = os.path.join(OUT_VER, "government_property_allocation_transfer_controls_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "government_property_allocation_transfer_controls_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "government_property_allocation_transfer_controls_arabic_legal_llm",
                        "government_property_allocation_transfer_controls_legal_llm_001_013.json")

LAW_ID = "sa-government-property-allocation-transfer-controls"
LAW_AR = "ضوابط تخصيص واسترداد ومناقلة العقارات بين الجهات الحكومية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"government_property_allocation_transfer_controls_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17337), published 9/6/1442H (2021-01-22). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "government_property_allocation_transfer_controls",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "GOVERNMENT_PROPERTY_ALLOCATION_TRANSFER_CONTROLS_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "government-property-allocation-transfer-controls-llm-%03d" % idx,
            ...
            "article_path": "government_property_allocation_transfer_controls/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Controls on the Allocation, Recovery and Transfer of Real Estate between Government Entities -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 9/6/1442H / 2021-01-22)"),
                "source_authority_ar": ("ضوابط تخصيص واسترداد ومناقلة العقارات بين الجهات الحكومية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "government_property_allocation_transfer_controls",
        "layer": "GOVERNMENT_PROPERTY_ALLOCATION_TRANSFER_CONTROLS_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "government-property-allocation-transfer-controls-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (13 مادة، جميعها أصلية)",
        "title_en": "Controls on the Allocation, Recovery and Transfer of Real Estate between Government Entities — Arabic LLM-ready layer (13 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Controls on the Allocation, Recovery and Transfer of Real Estate between Government Entities records" % (len(ver), len(llm)))
