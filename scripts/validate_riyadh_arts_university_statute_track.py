#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the النظام الأساس لجامعة الرياض للفنون track.
...
RECORDS = os.path.join(ROOT, "sources", "riyadh_arts_university_statute", "verified",
                       "riyadh_arts_university_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "riyadh_arts_university_statute", "verified",
                       "riyadh_arts_university_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "riyadh_arts_university_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"riyadh_arts_university_statute_no_decision_number_in_source", "riyadh_arts_university_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "النظام الأساس لجامعة الرياض للفنون":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of Riyadh University for the Arts track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of Riyadh University for the Arts")
