#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the نظام صندوق البنية التحتية الوطني track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24518),
...
SRC = os.path.join(ROOT, "sources", "national_infrastructure_fund_law", "official_source",
                   "national_infrastructure_fund_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "national_infrastructure_fund_law", "verified")
RECORDS = os.path.join(OUT_VER, "national_infrastructure_fund_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "national_infrastructure_fund_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "national_infrastructure_fund_law_arabic_legal_llm",
                        "national_infrastructure_fund_law_legal_llm_001_018.json")

LAW_ID = "sa-national-infrastructure-fund-law"
LAW_AR = "نظام صندوق البنية التحتية الوطني"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"national_infrastructure_fund_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24518), published 6/8/1445H (2024-02-16). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "national_infrastructure_fund_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "NATIONAL_INFRASTRUCTURE_FUND_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "national-infrastructure-fund-law-llm-%03d" % idx,
            ...
            "article_path": "national_infrastructure_fund_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Law of the National Infrastructure Fund -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 6/8/1445H / 2024-02-16)"),
                "source_authority_ar": ("نظام صندوق البنية التحتية الوطني — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "national_infrastructure_fund_law",
        "layer": "NATIONAL_INFRASTRUCTURE_FUND_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "national-infrastructure-fund-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (18 مادة، جميعها أصلية)",
        "title_en": "Law of the National Infrastructure Fund — Arabic LLM-ready layer (18 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Law of the National Infrastructure Fund records" % (len(ver), len(llm)))
