#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط السماح للممارسين الصحيين الحكوميين بالعمل في القطاع الصحي الخاص خارج أوقات الدوام الرسمي track.
...
RECORDS = os.path.join(ROOT, "sources", "government_health_practitioners_private_work_controls", "verified",
                       "government_health_practitioners_private_work_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "government_health_practitioners_private_work_controls", "verified",
                       "government_health_practitioners_private_work_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "government_health_practitioners_private_work_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"government_health_practitioners_private_work_controls_no_decision_number_in_source", "government_health_practitioners_private_work_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "21/9/1443":
    ...
    if src.get("document") != "ضوابط السماح للممارسين الصحيين الحكوميين بالعمل في القطاع الصحي الخاص خارج أوقات الدوام الرسمي":
    ...
    if e:
        print("FAIL: %d error(s) in Controls Permitting Government Health Practitioners to Work in the Private Health Sector outside Official Hours track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls Permitting Government Health Practitioners to Work in the Private Health Sector outside Official Hours")
