#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لنظام الضمان الاجتماعي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17861),
...
SRC = os.path.join(ROOT, "sources", "social_security_regulation", "official_source",
                   "social_security_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "social_security_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "social_security_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "social_security_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "social_security_regulation_arabic_legal_llm",
                        "social_security_regulation_legal_llm_001_028.json")

LAW_ID = "sa-social-security-regulation"
LAW_AR = "اللائحة التنفيذية لنظام الضمان الاجتماعي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"social_security_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17861), published 15/11/1442H (2021-06-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "social_security_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "SOCIAL_SECURITY_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "social-security-regulation-llm-%03d" % idx,
            ...
            "article_path": "social_security_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Executive Regulation of the Social Security Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 15/11/1442H / 2021-06-25)"),
                "source_authority_ar": ("اللائحة التنفيذية لنظام الضمان الاجتماعي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "social_security_regulation",
        "layer": "SOCIAL_SECURITY_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "social-security-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (28 مادة، جميعها أصلية)",
        "title_en": "Executive Regulation of the Social Security Law — Arabic LLM-ready layer (28 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Executive Regulation of the Social Security Law records" % (len(ver), len(llm)))
