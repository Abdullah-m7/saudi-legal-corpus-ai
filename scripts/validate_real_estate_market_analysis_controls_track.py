#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the ضوابط تحليل أسواق العقار track.
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_market_analysis_controls", "verified",
                       "real_estate_market_analysis_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "real_estate_market_analysis_controls", "verified",
                       "real_estate_market_analysis_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "real_estate_market_analysis_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"real_estate_market_analysis_controls_no_decision_number_in_source", "real_estate_market_analysis_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "12/1/1443":
        e.append("[2j] gazette_publication_date_hijri must be 12/1/1443")
    if src.get("gazette_publication_date_gregorian") != "2021-08-20":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-08-20")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "ضوابط تحليل أسواق العقار":
    ...
    if e:
        print("FAIL: %d error(s) in Controls for Real Estate Market Analysis track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls for Real Estate Market Analysis")
