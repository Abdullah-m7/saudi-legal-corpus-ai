#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الضوابط والاجراءات لليخوت الزايرة track.
...
RECORDS = os.path.join(ROOT, "sources", "visiting_yachts_controls", "verified",
                       "visiting_yachts_controls_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "visiting_yachts_controls", "verified",
                       "visiting_yachts_controls_verified_summary.json")
LLM = os.path.join(ROOT, "data", "visiting_yachts_controls_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"visiting_yachts_controls_no_decision_number_in_source", "visiting_yachts_controls_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "الضوابط والاجراءات لليخوت الزايرة":
    ...
    if e:
        print("FAIL: %d error(s) in Controls and Procedures for Visiting Yachts track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Controls and Procedures for Visiting Yachts")
