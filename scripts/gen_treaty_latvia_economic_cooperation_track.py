#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية التعاون الاقتصادي بين حكومة المملكة العربية السعودية وحكومة جمهورية لاتفيا track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24484),
...
SRC = os.path.join(ROOT, "sources", "treaty_latvia_economic_cooperation", "official_source",
                   "treaty_latvia_economic_cooperation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_latvia_economic_cooperation", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_latvia_economic_cooperation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_latvia_economic_cooperation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_latvia_economic_cooperation_arabic_legal_llm",
                        "treaty_latvia_economic_cooperation_legal_llm_001_008.json")

LAW_ID = "sa-treaty-latvia-economic-cooperation"
LAW_AR = "اتفاقية التعاون الاقتصادي بين حكومة المملكة العربية السعودية وحكومة جمهورية لاتفيا"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_latvia_economic_cooperation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24484), published 28/7/1445H (2024-02-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_latvia_economic_cooperation",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_LATVIA_ECONOMIC_COOPERATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-latvia-economic-cooperation-llm-%03d" % idx,
            ...
            "article_path": "treaty_latvia_economic_cooperation/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Arabia–Latvia Economic Cooperation Agreement -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 28/7/1445H / 2024-02-09)"),
                "source_authority_ar": ("اتفاقية التعاون الاقتصادي بين حكومة المملكة العربية السعودية وحكومة جمهورية لاتفيا — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_latvia_economic_cooperation",
        "layer": "TREATY_LATVIA_ECONOMIC_COOPERATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-latvia-economic-cooperation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Saudi Arabia–Latvia Economic Cooperation Agreement — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Arabia–Latvia Economic Cooperation Agreement records" % (len(ver), len(llm)))
