#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد برامج التعليم المهني المستمر track.
...
RECORDS = os.path.join(ROOT, "sources", "continuing_professional_education_rules", "verified",
                       "continuing_professional_education_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "continuing_professional_education_rules", "verified",
                       "continuing_professional_education_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "continuing_professional_education_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"continuing_professional_education_rules_no_decision_number_in_source", "continuing_professional_education_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد برامج التعليم المهني المستمر":
    ...
    if e:
        print("FAIL: %d error(s) in Rules for Continuing Professional Education Programs track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules for Continuing Professional Education Programs")
