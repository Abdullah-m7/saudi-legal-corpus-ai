#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنفيذية لصندوق الآثار والمتاحف والتراث العمراني track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19021),
...
SRC = os.path.join(ROOT, "sources", "antiquities_museums_fund_regulation", "official_source",
                   "antiquities_museums_fund_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "antiquities_museums_fund_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "antiquities_museums_fund_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "antiquities_museums_fund_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "antiquities_museums_fund_regulation_arabic_legal_llm",
                        "antiquities_museums_fund_regulation_legal_llm_001_022.json")

LAW_ID = "sa-antiquities-museums-fund-regulation"
LAW_AR = "اللائحة التنفيذية لصندوق الآثار والمتاحف والتراث العمراني"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"antiquities_museums_fund_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19021), published 24/7/1443H (2022-02-25). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "antiquities_museums_fund_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "ANTIQUITIES_MUSEUMS_FUND_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "antiquities-museums-fund-regulation-llm-%03d" % idx,
            ...
            "article_path": "antiquities_museums_fund_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Implementing Regulation of the Antiquities, Museums and Urban Heritage Fund -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/7/1443H / 2022-02-25)"),
                "source_authority_ar": ("اللائحة التنفيذية لصندوق الآثار والمتاحف والتراث العمراني — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "antiquities_museums_fund_regulation",
        "layer": "ANTIQUITIES_MUSEUMS_FUND_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "antiquities-museums-fund-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (22 مادة، جميعها أصلية)",
        "title_en": "Implementing Regulation of the Antiquities, Museums and Urban Heritage Fund — Arabic LLM-ready layer (22 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Implementing Regulation of the Antiquities, Museums and Urban Heritage Fund records" % (len(ver), len(llm)))
