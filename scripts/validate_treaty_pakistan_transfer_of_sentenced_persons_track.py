#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية بين حكومتي المملكة وجمهورية باكستان الإسلامية لنقل المحكوم عليهم بعقوبات سالبة للحرية track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_pakistan_transfer_of_sentenced_persons", "verified",
                       "treaty_pakistan_transfer_of_sentenced_persons_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_pakistan_transfer_of_sentenced_persons", "verified",
                       "treaty_pakistan_transfer_of_sentenced_persons_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_pakistan_transfer_of_sentenced_persons_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_pakistan_transfer_of_sentenced_persons_no_decision_number_in_source", "treaty_pakistan_transfer_of_sentenced_persons_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "8/8/1443":
        e.append("[2j] gazette_publication_date_hijri must be 8/8/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-03-11":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-03-11")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية بين حكومتي المملكة وجمهورية باكستان الإسلامية لنقل المحكوم عليهم بعقوبات سالبة للحرية":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Pakistan Agreement on the Transfer of Sentenced Persons track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Pakistan Agreement on the Transfer of Sentenced Persons")
