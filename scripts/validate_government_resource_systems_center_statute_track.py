#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the تنظيم المركز الوطني لنظم الموارد الحكومية track.
...
RECORDS = os.path.join(ROOT, "sources", "government_resource_systems_center_statute", "verified",
                       "government_resource_systems_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "government_resource_systems_center_statute", "verified",
                       "government_resource_systems_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "government_resource_systems_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"government_resource_systems_center_statute_no_decision_number_in_source", "government_resource_systems_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "تنظيم المركز الوطني لنظم الموارد الحكومية":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the National Center for Government Resource Systems track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the National Center for Government Resource Systems")
