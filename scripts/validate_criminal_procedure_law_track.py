#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Law of Criminal Procedure track (222 records).

Trust gate: every article MATCHES_PDF (>=0.90 vs the committed official MOJ
PDF, hash re-verified) EXCEPT the 2 visually-adjudicated short/cross-reference
articles (210, 222) confirmed verbatim on the rendered pages; the numbered
sequence is a complete 1..222 (no مكرر); and every article carries an explained
...
SRC = os.path.join(ROOT, "sources", "criminal_procedure", "law", "official_source",
                   "criminal_procedure_law_official_source.json")
RECORDS = os.path.join(ROOT, "sources", "criminal_procedure", "law", "verified",
                       "criminal_procedure_law_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "criminal_procedure_arabic_legal_llm",
                   "criminal_procedure_law_legal_llm_001_222.json")
PDF = os.path.join(ROOT, "inputs", "criminal_procedure_official_pdfs",
                   "criminal_procedure_law_moj_official_ar.pdf")
STATUS = "MOJ_PORTAL_API_CROSS_CHECKED_OFFICIAL_PDF"
N = 222
SIM_FLOOR = 0.90
ALLOWED_STATUS = {"اصلية", "معدلة", "ملغاة", "مضافة"}
EXPECTED_COUNTS = {"اصلية": 219, "معدلة": 3}
VISUALLY_ADJUDICATED = {"jza_law_art_210", "jza_law_art_222"}
AR = "ء-ي"
...
def main():
    ...
    nums = sorted(int(re.match(r"jza_law_art_(\d{3})", k).group(1)) for k in arts
    ...
    if e:
        ...
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Law of Criminal Procedure — 222 records (consolidated: 219 اصلية / 3 معدلة)")
    print("  - trust gate: every article MATCHES_PDF; 2 short/cross-ref articles (210, 222) visually adjudicated")
    print("  - complete 1..222 (no مكرر); committed official MOJ PDF hash verified")
    print("  - 3 amended articles (25, 112, 218) carry amendment history; no repealed/added; no dual-status")
