#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اتفاقية تعاون في مجال استعمال واستبدال رخص القيادة بين حكومتي المملكة وألبانيا track.
...
RECORDS = os.path.join(ROOT, "sources", "treaty_albania_driving_licences", "verified",
                       "treaty_albania_driving_licences_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "treaty_albania_driving_licences", "verified",
                       "treaty_albania_driving_licences_verified_summary.json")
LLM = os.path.join(ROOT, "data", "treaty_albania_driving_licences_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"treaty_albania_driving_licences_no_decision_number_in_source", "treaty_albania_driving_licences_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "14/1/1444":
        e.append("[2j] gazette_publication_date_hijri must be 14/1/1444")
    if src.get("gazette_publication_date_gregorian") != "2022-08-12":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-08-12")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اتفاقية تعاون في مجال استعمال واستبدال رخص القيادة بين حكومتي المملكة وألبانيا":
    ...
    if e:
        print("FAIL: %d error(s) in Saudi Arabia–Albania Agreement on the Use and Exchange of Driving Licences track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Saudi Arabia–Albania Agreement on the Use and Exchange of Driving Licences")
