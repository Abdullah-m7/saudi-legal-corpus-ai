#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المؤسسة العامة للمحافظة على الشعب المرجانية والسلاحف في البحر الأحمر track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24202),
published 24/5/1445H (2023-12-08). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "red_sea_coral_turtles_authority_statute", "official_source",
                   "red_sea_coral_turtles_authority_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "red_sea_coral_turtles_authority_statute", "verified")
RECORDS = os.path.join(OUT_VER, "red_sea_coral_turtles_authority_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "red_sea_coral_turtles_authority_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "red_sea_coral_turtles_authority_statute_arabic_legal_llm",
                        "red_sea_coral_turtles_authority_statute_legal_llm_001_016.json")

LAW_ID = "sa-red-sea-coral-turtles-authority-statute"
LAW_AR = "تنظيم المؤسسة العامة للمحافظة على الشعب المرجانية والسلاحف في البحر الأحمر"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"red_sea_coral_turtles_authority_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24202), published 24/5/1445H (2023-12-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "red_sea_coral_turtles_authority_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "RED_SEA_CORAL_TURTLES_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "red-sea-coral-turtles-authority-statute-llm-%03d" % idx,
            ...
            "article_path": "red_sea_coral_turtles_authority_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the General Authority for the Conservation of Coral Reefs and Turtles in the Red Sea -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/5/1445H / 2023-12-08)"),
                "source_authority_ar": ("تنظيم المؤسسة العامة للمحافظة على الشعب المرجانية والسلاحف في البحر الأحمر — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 24/5/1445هـ الموافق 2023-12-08م)"),
    ...
    json.dump({
        "law_key": "red_sea_coral_turtles_authority_statute",
        "layer": "RED_SEA_CORAL_TURTLES_AUTHORITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "red-sea-coral-turtles-authority-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Statute of the General Authority for the Conservation of Coral Reefs and Turtles in the Red Sea — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the General Authority for the Conservation of Coral Reefs and Turtles in the Red Sea records" % (len(ver), len(llm)))
