#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد والإجراءات الخاصة بالمطورين العقاريين المرخصين باعتبارهم أشخاصا مؤهلين لاسترداد ضريبة القيمة المضافة track.
...
RECORDS = os.path.join(ROOT, "sources", "licensed_realestate_developers_rules", "verified",
                       "licensed_realestate_developers_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "licensed_realestate_developers_rules", "verified",
                       "licensed_realestate_developers_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "licensed_realestate_developers_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"licensed_realestate_developers_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "القواعد والإجراءات الخاصة بالمطورين العقاريين المرخصين باعتبارهم أشخاصا مؤهلين لاسترداد ضريبة القيمة المضافة":
    ...
    if e:
        print("FAIL: %d error(s) in Rules and Procedures for Licensed Real Estate Developers as Qualified Persons track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules and Procedures for Licensed Real Estate Developers as Qualified Persons")
