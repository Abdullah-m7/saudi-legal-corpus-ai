#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لتنظيم الترخيص لمكاتب المحاماة الأجنبية track.
...
RECORDS = os.path.join(ROOT, "sources", "foreign_law_firms_licensing_regulation", "verified",
                       "foreign_law_firms_licensing_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "foreign_law_firms_licensing_regulation", "verified",
                       "foreign_law_firms_licensing_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "foreign_law_firms_licensing_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"foreign_law_firms_licensing_regulation_no_decision_number_in_source", "foreign_law_firms_licensing_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية لتنظيم الترخيص لمكاتب المحاماة الأجنبية":
    ...
    if e:
        print("FAIL: %d error(s) in Implementing Regulation for Licensing Foreign Law Firms track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Implementing Regulation for Licensing Foreign Law Firms")
