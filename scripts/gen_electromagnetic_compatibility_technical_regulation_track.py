#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة الفنية للتوافق الكهرومغناطيسي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24063),
...
SRC = os.path.join(ROOT, "sources", "electromagnetic_compatibility_technical_regulation", "official_source",
                   "electromagnetic_compatibility_technical_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "electromagnetic_compatibility_technical_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "electromagnetic_compatibility_technical_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "electromagnetic_compatibility_technical_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "electromagnetic_compatibility_technical_regulation_arabic_legal_llm",
                        "electromagnetic_compatibility_technical_regulation_legal_llm_001_012.json")

LAW_ID = "sa-electromagnetic-compatibility-technical-regulation"
LAW_AR = "اللائحة الفنية للتوافق الكهرومغناطيسي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"electromagnetic_compatibility_technical_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24063), published 3/5/1445H (2023-11-17). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "electromagnetic_compatibility_technical_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "ELECTROMAGNETIC_COMPATIBILITY_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "electromagnetic-compatibility-technical-regulation-llm-%03d" % idx,
            ...
            "article_path": "electromagnetic_compatibility_technical_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Technical Regulation for Electromagnetic Compatibility -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/5/1445H / 2023-11-17)"),
                "source_authority_ar": ("اللائحة الفنية للتوافق الكهرومغناطيسي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "electromagnetic_compatibility_technical_regulation",
        "layer": "ELECTROMAGNETIC_COMPATIBILITY_TECHNICAL_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "electromagnetic-compatibility-technical-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (12 مادة، جميعها أصلية)",
        "title_en": "Technical Regulation for Electromagnetic Compatibility — Arabic LLM-ready layer (12 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Technical Regulation for Electromagnetic Compatibility records" % (len(ver), len(llm)))
