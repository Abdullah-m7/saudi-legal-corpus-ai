#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the القواعد والإجراءات التنفيذية اللازمة لتطبيق تعريفة استهلاك الكهرباء لنشاط الحوسبة السحابية track.
...
RECORDS = os.path.join(ROOT, "sources", "cloud_computing_electricity_tariff_executive_rules", "verified",
                       "cloud_computing_electricity_tariff_executive_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "cloud_computing_electricity_tariff_executive_rules", "verified",
                       "cloud_computing_electricity_tariff_executive_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "cloud_computing_electricity_tariff_executive_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"cloud_computing_electricity_tariff_executive_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "4/9/1442":
        e.append("[2j] gazette_publication_date_hijri must be 4/9/1442")
    if src.get("gazette_publication_date_gregorian") != "2021-04-16":
        e.append("[2j] gazette_publication_date_gregorian must be 2021-04-16")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "القواعد والإجراءات التنفيذية اللازمة لتطبيق تعريفة استهلاك الكهرباء لنشاط الحوسبة السحابية":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Rules and Procedures for Applying the Electricity Consumption Tariff to Cloud Computing Activity track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Rules and Procedures for Applying the Electricity Consumption Tariff to Cloud Computing Activity")
