#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد حساب زكاة مكلفي التقديري track.
...
server-rendered HTML page (uqn.gov.sa/details?p=17076),
published 14/7/1442H (2021-02-26). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "estimated_assessment_zakat_rules", "official_source",
                   "estimated_assessment_zakat_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "estimated_assessment_zakat_rules", "verified")
RECORDS = os.path.join(OUT_VER, "estimated_assessment_zakat_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "estimated_assessment_zakat_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "estimated_assessment_zakat_rules_arabic_legal_llm",
                        "estimated_assessment_zakat_rules_legal_llm_001_007.json")

LAW_ID = "sa-estimated-assessment-zakat-rules"
LAW_AR = "قواعد حساب زكاة مكلفي التقديري"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"estimated_assessment_zakat_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=17076), published 14/7/1442H (2021-02-26). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "estimated_assessment_zakat_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "ESTIMATED_ASSESSMENT_ZAKAT_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "estimated-assessment-zakat-rules-llm-%03d" % idx,
            ...
            "article_path": "estimated_assessment_zakat_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules for Computing Zakat of Estimated-Assessment Taxpayers -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 14/7/1442H / 2021-02-26)"),
                "source_authority_ar": ("قواعد حساب زكاة مكلفي التقديري — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 14/7/1442هـ الموافق 2021-02-26م)"),
    ...
    json.dump({
        "law_key": "estimated_assessment_zakat_rules",
        "layer": "ESTIMATED_ASSESSMENT_ZAKAT_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "estimated-assessment-zakat-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (7 مادة، جميعها أصلية)",
        "title_en": "Rules for Computing Zakat of Estimated-Assessment Taxpayers — Arabic LLM-ready layer (7 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules for Computing Zakat of Estimated-Assessment Taxpayers records" % (len(ver), len(llm)))
