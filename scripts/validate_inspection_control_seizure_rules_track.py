#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the قواعد الفحص والرقابة والضبط track.
...
RECORDS = os.path.join(ROOT, "sources", "inspection_control_seizure_rules", "verified",
                       "inspection_control_seizure_rules_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "inspection_control_seizure_rules", "verified",
                       "inspection_control_seizure_rules_verified_summary.json")
LLM = os.path.join(ROOT, "data", "inspection_control_seizure_rules_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"inspection_control_seizure_rules_no_decision_number_in_source", "inspection_control_seizure_rules_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "قواعد الفحص والرقابة والضبط":
    ...
    if e:
        print("FAIL: %d error(s) in Rules of Inspection, Supervision and Seizure track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Rules of Inspection, Supervision and Seizure")
