#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية للمركز العام للنقل بمدينة مكة المكرمة والمشاعر المقدسة track.
...
RECORDS = os.path.join(ROOT, "sources", "makkah_holy_sites_transport_center_regulation", "verified",
                       "makkah_holy_sites_transport_center_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "makkah_holy_sites_transport_center_regulation", "verified",
                       "makkah_holy_sites_transport_center_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "makkah_holy_sites_transport_center_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"makkah_holy_sites_transport_center_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنظيمية للمركز العام للنقل بمدينة مكة المكرمة والمشاعر المقدسة":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for the General Transport Center in Makkah and the Holy Sites track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for the General Transport Center in Makkah and the Holy Sites")
