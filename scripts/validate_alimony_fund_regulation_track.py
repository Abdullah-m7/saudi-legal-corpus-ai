#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the Alimony Fund Regulation track (17 records).
...
adjudicated). 7/17 matched outright; the other 10 (arts 1, 4, 5, 7, 8, 9, 10,
11, 12, 15; mean 0.7767, min 0.3987) were visually adjudicated verbatim due
to a known RTL/ligature PDF text-layer extraction artifact. Articles are
numbered by ordinal position 1..17 (no مكرر), flat structure with no
chapter/section wrapper (section_ar empty for every article — not a bug).
Every article carries an explained legal_status consistent with its
is_repealed/is_amended/is_added flags. The section-API status equals the
PDF status for every article (no dual-status divergence). FRESH FULL
ISSUANCE: all 17 اصلية; none amended, repealed or added. Tatweel is banned
EXCEPT the 'هـ' digraph and space-bounded enumerator dashes. Issued by
Council of Ministers Decision 679 (not a Minister of Justice decision) —
the Fund is a semi-independent body administratively linked to the Minister
of Justice."""
...
RECORDS = os.path.join(ROOT, "sources", "alimony_fund", "regulation", "verified",
                       "alimony_fund_regulation_verified_records.jsonl")
LLM = os.path.join(ROOT, "data", "alimony_fund_arabic_legal_llm",
                   "alimony_fund_regulation_legal_llm_001_017.json")
PDF = os.path.join(ROOT, "inputs", "alimony_fund_official_pdfs",
                   "alimony_fund_regulation_moj_official_ar.pdf")
...
VISUALLY_ADJUDICATED = {"alimony_fund_art_%03d" % n for n in (1, 4, 5, 7, 8, 9, 10, 11, 12, 15)}
...
def main():
    ...
    if e:
        print("FAIL: %d error(s) in Alimony Fund Regulation track:" % len(e))
        for x in e[:15]:
            print("  - %s" % x)
        return 1
    print("PASS: Alimony Fund Regulation — 17 records (fresh issuance: all 17 اصلية)")
    print("  - trust gate: 7/17 MATCHES_PDF outright, 10 visually adjudicated (mean 0.7767, min 0.3987)")
    print("  - numbered 1..17 by ordinal position (no مكرر); flat structure, no dual-status divergence")
    print("  - IN-FORCE Council of Ministers Decision 679 (15/11/1438H); committed MOJ PDF hash verified; Arabic governs")
    return 0
