#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام الأساس لجامعة الملك سعود track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20025),
...
SRC = os.path.join(ROOT, "sources", "king_saud_university_statute", "official_source",
                   "king_saud_university_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "king_saud_university_statute", "verified")
RECORDS = os.path.join(OUT_VER, "king_saud_university_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "king_saud_university_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "king_saud_university_statute_arabic_legal_llm",
                        "king_saud_university_statute_legal_llm_001_022.json")

LAW_ID = "sa-king-saud-university-statute"
LAW_AR = "النظام الأساس لجامعة الملك سعود"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"king_saud_university_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20025), published 13/2/1444H (2022-09-09). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "king_saud_university_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "KING_SAUD_UNIVERSITY_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "king-saud-university-statute-llm-%03d" % idx,
            ...
            "article_path": "king_saud_university_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of King Saud University -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 13/2/1444H / 2022-09-09)"),
                "source_authority_ar": ("النظام الأساس لجامعة الملك سعود — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "king_saud_university_statute",
        "layer": "KING_SAUD_UNIVERSITY_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "king-saud-university-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (22 مادة، جميعها أصلية)",
        "title_en": "Statute of King Saud University — Arabic LLM-ready layer (22 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of King Saud University records" % (len(ver), len(llm)))
