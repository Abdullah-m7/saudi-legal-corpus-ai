#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the النظام (القانون) الموحد للنقل البري الدولي بين دول مجلس التعاون لدول الخليج العربية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24925),
published 2/11/1445H (2024-05-10). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "gcc_road_transport_law", "official_source",
                   "gcc_road_transport_law_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "gcc_road_transport_law", "verified")
RECORDS = os.path.join(OUT_VER, "gcc_road_transport_law_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "gcc_road_transport_law_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "gcc_road_transport_law_arabic_legal_llm",
                        "gcc_road_transport_law_legal_llm_001_025.json")

LAW_ID = "sa-gcc-road-transport-law"
LAW_AR = "النظام (القانون) الموحد للنقل البري الدولي بين دول مجلس التعاون لدول الخليج العربية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"gcc_road_transport_law_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24925), published 2/11/1445H (2024-05-10). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "gcc_road_transport_law",
            "law_component": "law",
            "language": "ar",
            "record_layer": "GCC_ROAD_TRANSPORT_LAW_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "gcc-road-transport-law-llm-%03d" % idx,
            ...
            "article_path": "gcc_road_transport_law/law/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Unified Law of International Road Transport for the GCC States -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 2/11/1445H / 2024-05-10)"),
                "source_authority_ar": ("النظام (القانون) الموحد للنقل البري الدولي بين دول مجلس التعاون لدول الخليج العربية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 2/11/1445هـ الموافق 2024-05-10م)"),
    ...
    json.dump({
        "law_key": "gcc_road_transport_law",
        "layer": "GCC_ROAD_TRANSPORT_LAW_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "gcc-road-transport-law-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "law",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (25 مادة، جميعها أصلية)",
        "title_en": "Unified Law of International Road Transport for the GCC States — Arabic LLM-ready layer (25 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Unified Law of International Road Transport for the GCC States records" % (len(ver), len(llm)))
