#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المؤسسة العامة للري track.
...
server-rendered HTML page (uqn.gov.sa/details?p=27274),
...
SRC = os.path.join(ROOT, "sources", "general_irrigation_corporation_statute", "official_source",
                   "general_irrigation_corporation_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "general_irrigation_corporation_statute", "verified")
RECORDS = os.path.join(OUT_VER, "general_irrigation_corporation_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "general_irrigation_corporation_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "general_irrigation_corporation_statute_arabic_legal_llm",
                        "general_irrigation_corporation_statute_legal_llm_001_016.json")

LAW_ID = "sa-general-irrigation-corporation-statute"
LAW_AR = "تنظيم المؤسسة العامة للري"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"general_irrigation_corporation_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=27274), published 23/1/1447H (2025-07-18). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "general_irrigation_corporation_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "GENERAL_IRRIGATION_CORPORATION_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "general-irrigation-corporation-statute-llm-%03d" % idx,
            ...
            "article_path": "general_irrigation_corporation_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the General Irrigation Corporation -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 23/1/1447H / 2025-07-18)"),
                "source_authority_ar": ("تنظيم المؤسسة العامة للري — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "general_irrigation_corporation_statute",
        "layer": "GENERAL_IRRIGATION_CORPORATION_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "general-irrigation-corporation-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Statute of the General Irrigation Corporation — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the General Irrigation Corporation records" % (len(ver), len(llm)))
