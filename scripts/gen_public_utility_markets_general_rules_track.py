#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the القواعد العامة المنظمة لعمل أسواق النفع العام وضوابط أعمال مقدمي الخدمة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=24208),
...
SRC = os.path.join(ROOT, "sources", "public_utility_markets_general_rules", "official_source",
                   "public_utility_markets_general_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "public_utility_markets_general_rules", "verified")
RECORDS = os.path.join(OUT_VER, "public_utility_markets_general_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "public_utility_markets_general_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "public_utility_markets_general_rules_arabic_legal_llm",
                        "public_utility_markets_general_rules_legal_llm_001_019.json")

LAW_ID = "sa-public-utility-markets-general-rules"
LAW_AR = "القواعد العامة المنظمة لعمل أسواق النفع العام وضوابط أعمال مقدمي الخدمة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"public_utility_markets_general_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=24208), published 24/5/1445H (2023-12-08). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "public_utility_markets_general_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "PUBLIC_UTILITY_MARKETS_GENERAL_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "public-utility-markets-general-rules-llm-%03d" % idx,
            ...
            "article_path": "public_utility_markets_general_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("General Rules Governing Public Utility Markets and Service Provider Controls -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/5/1445H / 2023-12-08)"),
                "source_authority_ar": ("القواعد العامة المنظمة لعمل أسواق النفع العام وضوابط أعمال مقدمي الخدمة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "public_utility_markets_general_rules",
        "layer": "PUBLIC_UTILITY_MARKETS_GENERAL_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "public-utility-markets-general-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (19 مادة، جميعها أصلية)",
        "title_en": "General Rules Governing Public Utility Markets and Service Provider Controls — Arabic LLM-ready layer (19 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready General Rules Governing Public Utility Markets and Service Provider Controls records" % (len(ver), len(llm)))
