#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the الإطار التنظيمي للطاقة المتجددة للاستهلاك الذاتي track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23310),
...
SRC = os.path.join(ROOT, "sources", "self_consumption_renewable_energy_framework", "official_source",
                   "self_consumption_renewable_energy_framework_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "self_consumption_renewable_energy_framework", "verified")
RECORDS = os.path.join(OUT_VER, "self_consumption_renewable_energy_framework_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "self_consumption_renewable_energy_framework_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "self_consumption_renewable_energy_framework_arabic_legal_llm",
                        "self_consumption_renewable_energy_framework_legal_llm_001_011.json")

LAW_ID = "sa-self-consumption-renewable-energy-framework"
LAW_AR = "الإطار التنظيمي للطاقة المتجددة للاستهلاك الذاتي"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"self_consumption_renewable_energy_framework_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23310), published 3/1/1445H (2023-07-21). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "self_consumption_renewable_energy_framework",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "SELF_CONSUMPTION_RENEWABLE_ENERGY_FRAMEWORK_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "self-consumption-renewable-energy-framework-llm-%03d" % idx,
            ...
            "article_path": "self_consumption_renewable_energy_framework/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Framework for Self-Consumption Renewable Energy -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/1/1445H / 2023-07-21)"),
                "source_authority_ar": ("الإطار التنظيمي للطاقة المتجددة للاستهلاك الذاتي — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "self_consumption_renewable_energy_framework",
        "layer": "SELF_CONSUMPTION_RENEWABLE_ENERGY_FRAMEWORK_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "self-consumption-renewable-energy-framework-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (11 مادة، جميعها أصلية)",
        "title_en": "Regulatory Framework for Self-Consumption Renewable Energy — Arabic LLM-ready layer (11 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Framework for Self-Consumption Renewable Energy records" % (len(ver), len(llm)))
