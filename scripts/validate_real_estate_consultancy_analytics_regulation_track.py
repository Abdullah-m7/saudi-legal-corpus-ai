#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنظيمية للاستشارات والتحليلات العقارية track.
...
RECORDS = os.path.join(ROOT, "sources", "real_estate_consultancy_analytics_regulation", "verified",
                       "real_estate_consultancy_analytics_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "real_estate_consultancy_analytics_regulation", "verified",
                       "real_estate_consultancy_analytics_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "real_estate_consultancy_analytics_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"real_estate_consultancy_analytics_regulation_no_decision_number_in_source", "real_estate_consultancy_analytics_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "9/6/1445":
        e.append("[2j] gazette_publication_date_hijri must be 9/6/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-12-22":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-12-22")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "اللائحة التنظيمية للاستشارات والتحليلات العقارية":
    ...
    if e:
        print("FAIL: %d error(s) in Regulatory Rules for Real Estate Consultancy and Analytics track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Regulatory Rules for Real Estate Consultancy and Analytics")
