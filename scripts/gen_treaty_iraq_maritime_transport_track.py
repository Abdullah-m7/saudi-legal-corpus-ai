#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اتفاقية تعاون بين حكومتي المملكة والعراق في مجال النقل البحري track.
...
server-rendered HTML page (uqn.gov.sa/details?p=19850),
...
SRC = os.path.join(ROOT, "sources", "treaty_iraq_maritime_transport", "official_source",
                   "treaty_iraq_maritime_transport_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "treaty_iraq_maritime_transport", "verified")
RECORDS = os.path.join(OUT_VER, "treaty_iraq_maritime_transport_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "treaty_iraq_maritime_transport_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "treaty_iraq_maritime_transport_arabic_legal_llm",
                        "treaty_iraq_maritime_transport_legal_llm_001_017.json")

LAW_ID = "sa-treaty-iraq-maritime-transport"
LAW_AR = "اتفاقية تعاون بين حكومتي المملكة والعراق في مجال النقل البحري"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"treaty_iraq_maritime_transport_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=19850), published 7/1/1444H (2022-08-05). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "treaty_iraq_maritime_transport",
            "law_component": "treaty",
            "language": "ar",
            "record_layer": "TREATY_IRAQ_MARITIME_TRANSPORT_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "treaty-iraq-maritime-transport-llm-%03d" % idx,
            ...
            "article_path": "treaty_iraq_maritime_transport/treaty/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Saudi Arabia–Iraq Maritime Transport Cooperation Agreement -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 7/1/1444H / 2022-08-05)"),
                "source_authority_ar": ("اتفاقية تعاون بين حكومتي المملكة والعراق في مجال النقل البحري — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "treaty_iraq_maritime_transport",
        "layer": "TREATY_IRAQ_MARITIME_TRANSPORT_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "treaty-iraq-maritime-transport-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "treaty",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (17 مادة، جميعها أصلية)",
        "title_en": "Saudi Arabia–Iraq Maritime Transport Cooperation Agreement — Arabic LLM-ready layer (17 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Saudi Arabia–Iraq Maritime Transport Cooperation Agreement records" % (len(ver), len(llm)))
