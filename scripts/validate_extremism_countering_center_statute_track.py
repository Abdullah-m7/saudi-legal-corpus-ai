#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the النظام الأساس للمركز العالمي لمكافحة الفكر المتطرف track.
...
RECORDS = os.path.join(ROOT, "sources", "extremism_countering_center_statute", "verified",
                       "extremism_countering_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "extremism_countering_center_statute", "verified",
                       "extremism_countering_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "extremism_countering_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"extremism_countering_center_statute_no_decision_number_in_source", "extremism_countering_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "النظام الأساس للمركز العالمي لمكافحة الفكر المتطرف":
    ...
    if e:
        print("FAIL: %d error(s) in Statute of the Global Center for Combating Extremist Ideology track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Statute of the Global Center for Combating Extremist Ideology")
