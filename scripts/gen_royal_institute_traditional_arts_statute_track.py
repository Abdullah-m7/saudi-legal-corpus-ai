#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the تنظيم المعهد الملكي للفنون التقليدية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23303),
...
SRC = os.path.join(ROOT, "sources", "royal_institute_traditional_arts_statute", "official_source",
                   "royal_institute_traditional_arts_statute_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "royal_institute_traditional_arts_statute", "verified")
RECORDS = os.path.join(OUT_VER, "royal_institute_traditional_arts_statute_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "royal_institute_traditional_arts_statute_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "royal_institute_traditional_arts_statute_arabic_legal_llm",
                        "royal_institute_traditional_arts_statute_legal_llm_001_016.json")

LAW_ID = "sa-royal-institute-traditional-arts-statute"
LAW_AR = "تنظيم المعهد الملكي للفنون التقليدية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"royal_institute_traditional_arts_statute_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23303), published 3/1/1445H (2023-07-21). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "royal_institute_traditional_arts_statute",
            "law_component": "statute",
            "language": "ar",
            "record_layer": "ROYAL_INSTITUTE_TRADITIONAL_ARTS_STATUTE_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "royal-institute-traditional-arts-statute-llm-%03d" % idx,
            ...
            "article_path": "royal_institute_traditional_arts_statute/statute/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Statute of the Royal Institute of Traditional Arts -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 3/1/1445H / 2023-07-21)"),
                "source_authority_ar": ("تنظيم المعهد الملكي للفنون التقليدية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "royal_institute_traditional_arts_statute",
        "layer": "ROYAL_INSTITUTE_TRADITIONAL_ARTS_STATUTE_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "royal-institute-traditional-arts-statute-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "statute",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (16 مادة، جميعها أصلية)",
        "title_en": "Statute of the Royal Institute of Traditional Arts — Arabic LLM-ready layer (16 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Statute of the Royal Institute of Traditional Arts records" % (len(ver), len(llm)))
