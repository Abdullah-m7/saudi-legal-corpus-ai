#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية للمكتب الاستراتيجي لتطوير منطقة جازان track.
...
RECORDS = os.path.join(ROOT, "sources", "jazan_strategic_office_statute", "verified",
                       "jazan_strategic_office_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "jazan_strategic_office_statute", "verified",
                       "jazan_strategic_office_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "jazan_strategic_office_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"jazan_strategic_office_statute_no_decision_number_in_source", "jazan_strategic_office_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "25/11/1443":
        e.append("[2j] gazette_publication_date_hijri must be 25/11/1443")
    if src.get("gazette_publication_date_gregorian") != "2022-06-24":
        e.append("[2j] gazette_publication_date_gregorian must be 2022-06-24")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية للمكتب الاستراتيجي لتطوير منطقة جازان":
    ...
    if e:
        print("FAIL: %d error(s) in Organisational Arrangements of the Strategic Office for the Development of the Jazan Region track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Organisational Arrangements of the Strategic Office for the Development of the Jazan Region")
