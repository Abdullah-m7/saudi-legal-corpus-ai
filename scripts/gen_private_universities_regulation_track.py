#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the اللائحة التنظيمية للجامعات والكليات الأهلية track.
...
server-rendered HTML page (uqn.gov.sa/details?p=23896),
...
STRUCTURE: 59 numbered articles, no appendix records. All اصلية.
...
SRC = os.path.join(ROOT, "sources", "private_universities_regulation", "official_source",
                   "private_universities_regulation_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "private_universities_regulation", "verified")
RECORDS = os.path.join(OUT_VER, "private_universities_regulation_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "private_universities_regulation_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "private_universities_regulation_arabic_legal_llm",
                        "private_universities_regulation_legal_llm_001_059.json")

LAW_ID = "sa-private-universities-regulation"
LAW_AR = "اللائحة التنظيمية للجامعات والكليات الأهلية"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"private_universities_regulation_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=23896), published 5/4/1445H (2023-10-20). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "private_universities_regulation",
            "law_component": "regulation",
            "language": "ar",
            "record_layer": "PRIVATE_UNIVERSITIES_REGULATION_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "private-universities-regulation-llm-%03d" % idx,
            ...
            "article_path": "private_universities_regulation/regulation/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Regulatory Rules for Private Universities and Colleges -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 5/4/1445H / 2023-10-20)"),
                "source_authority_ar": ("اللائحة التنظيمية للجامعات والكليات الأهلية — النص الكامل جُلب مباشرة من صفحة جريدة أم "
    ...
    json.dump({
        "law_key": "private_universities_regulation",
        "layer": "PRIVATE_UNIVERSITIES_REGULATION_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "private-universities-regulation-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "regulation",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (59 مادة، جميعها أصلية)",
        "title_en": "Regulatory Rules for Private Universities and Colleges — Arabic LLM-ready layer (59 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Regulatory Rules for Private Universities and Colleges records" % (len(ver), len(llm)))
