#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the الترتيبات التنظيمية لمركز مشاريع البنية التحتية بمنطقة الرياض track.
...
RECORDS = os.path.join(ROOT, "sources", "riyadh_infrastructure_projects_center_statute", "verified",
                       "riyadh_infrastructure_projects_center_statute_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "riyadh_infrastructure_projects_center_statute", "verified",
                       "riyadh_infrastructure_projects_center_statute_verified_summary.json")
LLM = os.path.join(ROOT, "data", "riyadh_infrastructure_projects_center_statute_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"riyadh_infrastructure_projects_center_statute_no_decision_number_in_source", "riyadh_infrastructure_projects_center_statute_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    # [2j] anchor facts
    if src.get("gazette_publication_date_hijri") != "10/1/1445":
        e.append("[2j] gazette_publication_date_hijri must be 10/1/1445")
    if src.get("gazette_publication_date_gregorian") != "2023-07-28":
        e.append("[2j] gazette_publication_date_gregorian must be 2023-07-28")
    if src.get("legal_status_ar") != "ساري":
        e.append("[2j] legal_status_ar must be ساري")
    if src.get("document") != "الترتيبات التنظيمية لمركز مشاريع البنية التحتية بمنطقة الرياض":
    ...
    if e:
        print("FAIL: %d error(s) in Riyadh Region Infrastructure Projects Center — Organisational Arrangements track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Riyadh Region Infrastructure Projects Center — Organisational Arrangements")
