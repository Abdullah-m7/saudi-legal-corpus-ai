#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate the قواعد الحد من تعارض المصالح لمن سبق له العمل في السلك القضائي عند مزاولته مهنة المحاماة track.
...
server-rendered HTML page (uqn.gov.sa/details?p=20895),
published 24/4/1444H (2022-11-18). The Gazette IS the official publication of record
...
SRC = os.path.join(ROOT, "sources", "judicial_service_conflict_of_interest_rules", "official_source",
                   "judicial_service_conflict_of_interest_rules_official_source.json")
OUT_VER = os.path.join(ROOT, "sources", "judicial_service_conflict_of_interest_rules", "verified")
RECORDS = os.path.join(OUT_VER, "judicial_service_conflict_of_interest_rules_verified_records.jsonl")
SUMMARY = os.path.join(OUT_VER, "judicial_service_conflict_of_interest_rules_verified_summary.json")
LLM_PATH = os.path.join(ROOT, "data", "judicial_service_conflict_of_interest_rules_arabic_legal_llm",
                        "judicial_service_conflict_of_interest_rules_legal_llm_001_008.json")

LAW_ID = "sa-judicial-service-conflict-of-interest-rules"
LAW_AR = "قواعد الحد من تعارض المصالح لمن سبق له العمل في السلك القضائي عند مزاولته مهنة المحاماة"
STATUS_UNCHANGED = "UNCHANGED"
ART_RE = r"judicial_service_conflict_of_interest_rules_art_(\d{3})$"
...
def main():
    ...
    gov_note = ("Arabic governs; PRIMARY full text fetched directly from the Umm Al-Qura "
                ...
                "(uqn.gov.sa/details?p=20895), published 24/4/1444H (2022-11-18). "
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        text = a["text"]
        ver.append({
            "law_key": "judicial_service_conflict_of_interest_rules",
            "law_component": "rules",
            "language": "ar",
            "record_layer": "JUDICIAL_SERVICE_CONFLICT_OF_INTEREST_RULES_ARABIC_VERIFIED_TEXT",
            "article_number": n,
            "is_appendix": False,
        ...
        llm.append({
            ...
            "is_amended": ls == "معدلة",
            "text_complete": a.get("text_complete", True),
            "record_id": "judicial-service-conflict-of-interest-rules-llm-%03d" % idx,
            ...
            "article_path": "judicial_service_conflict_of_interest_rules/rules/%s" % key,
            ...
            "source_trust": {
                "source_authority": ("Rules Limiting Conflicts of Interest for Former Members of the Judiciary Practising Law -- full text fetched directly from the Umm "
                                     "Al-Qura Official Gazette's own HTML rendering "
                                     "(published 24/4/1444H / 2022-11-18)"),
                "source_authority_ar": ("قواعد الحد من تعارض المصالح لمن سبق له العمل في السلك القضائي عند مزاولته مهنة المحاماة — النص الكامل جُلب مباشرة من صفحة جريدة أم "
                                        "القرى الرسمية (تاريخ النشر 24/4/1444هـ الموافق 2022-11-18م)"),
    ...
    json.dump({
        "law_key": "judicial_service_conflict_of_interest_rules",
        "layer": "JUDICIAL_SERVICE_CONFLICT_OF_INTEREST_RULES_ARABIC_VERIFIED_TEXT",
    ...
    json.dump({
        "layer_id": "judicial-service-conflict-of-interest-rules-arabic-legal-llm-full",
        "law_id": LAW_ID,
        "law_component": "rules",
        "title_ar": LAW_AR + " — الطبقة العربية الجاهزة للنماذج اللغوية (8 مادة، جميعها أصلية)",
        "title_en": "Rules Limiting Conflicts of Interest for Former Members of the Judiciary Practising Law — Arabic LLM-ready layer (8 articles; all original)",
        "record_type": "verified_arabic_article",
        "language": "ar",
        "governing_text_language": "ar",
    ...
    print("Wrote %d verified + %d LLM-ready Rules Limiting Conflicts of Interest for Former Members of the Judiciary Practising Law records" % (len(ver), len(llm)))
