#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only validator for the اللائحة التنفيذية لرسوم العقارات الشاغرة track.
...
RECORDS = os.path.join(ROOT, "sources", "vacant_properties_fees_regulation", "verified",
                       "vacant_properties_fees_regulation_verified_records.jsonl")
SUMMARY = os.path.join(ROOT, "sources", "vacant_properties_fees_regulation", "verified",
                       "vacant_properties_fees_regulation_verified_summary.json")
LLM = os.path.join(ROOT, "data", "vacant_properties_fees_regulation_arabic_legal_llm",
...
FLAGGED_DISCREPANCY_KEYS = {"vacant_properties_fees_regulation_no_decision_number_in_source", "vacant_properties_fees_regulation_source_is_gazette_html_only"}
AR = "ء-ي"
...
def main():
    ...
    if src.get("document") != "اللائحة التنفيذية لرسوم العقارات الشاغرة":
    ...
    if e:
        print("FAIL: %d error(s) in Executive Regulation for Vacant Properties Fees track:" % len(e))
        for x in e[:40]:
            print("  - %s" % x)
        return 1
    print("PASS: Executive Regulation for Vacant Properties Fees")
